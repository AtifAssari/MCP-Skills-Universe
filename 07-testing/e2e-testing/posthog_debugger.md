---
title: posthog-debugger
url: https://skills.sh/posthog/skills/posthog-debugger
---

# posthog-debugger

skills/posthog/skills/posthog-debugger
posthog-debugger
Installation
...
Use browser tools to navigate and inspect. Prefer Chrome DevTools MCP (mcp__chrome-devtools__*) or Playwright MCP (mcp__playwright__*) tools.
Check multiple signals. PostHog can be implemented in various ways - check scripts, network requests, and the window object.
...
Navigate to the URL with ?__posthog_debug=true appended
Check if login is required by taking a snapshot and looking for login indicators
...
Look for requests with PostHog-specific paths (/flags/, /array/phc_)
Check network request details for PostHog headers
Look for phc_ tokens in request URLs or bodies
