---
title: xhs-publisher
url: https://skills.sh/pengjiyuan/xhs-skill/xhs-publisher
---

# xhs-publisher

skills/pengjiyuan/xhs-skill/xhs-publisher
xhs-publisher
Installation
$ npx skills add https://github.com/pengjiyuan/xhs-skill --skill xhs-publisher
SKILL.md
小红书自动化发布
...
检测登录后的关键元素（用户信息、侧边栏、内容管理等）
调用小红书用户信息 API 验证登录状态
检查浏览器 cookies 中的登录凭据
免扫码机制
首次登录后，浏览器数据持久化到 browser_data/default/
再次使用时自动恢复登录状态，无需重复扫码
发布命令默认启用自动登录检测，未登录时自动尝试扫码
强制重新登录

如需重新扫码，使用 --force 参数：

# 强制重新登录（清除旧数据后扫码）
python3 scripts/xhs_auto.py login --force
...
正文中的 #标签 格式会自动提取为标签，无需额外传 --tags：

# 这样即可自动提取 #AI#科技#求关注 三个标签
python3 scripts/xhs_auto.py publish \
  --title "标题" \
  --content "正文内容 #AI#科技#求关注"
...
正文声明：content_gen.py 自动在正文末尾追加「📝 本文由 AI 辅助创作」
图片水印：image_gen.py 自动在 AI 配图右下角添加「AI生成」水印（半透明黑底白字，高度 ≥ 最短边 5%）
平台声明：xhs_auto.py 发布时自动勾选「笔记含AI合成内容」声明

不合规后果：限流、限制搜索、降低曝光，多次违规封号。

发布流程
启动浏览器（使用持久化上下文，自动恢复登录）
检测登录状态，未登录则提示扫码
...
调用 xhs_auto.py schedule add 创建本地任务，获取 cron_job 对象
用 cron tool 的 add action 将 cron_job 提交给 OpenClaw，获取 jobId
调用 xhs_auto.py schedule link 回填 jobId 到本地记录
暂停/恢复/删除时，同步操作本地记录和 OpenClaw cron
热点数据采集
