...
url: https://skills.sh/zstmfhy/zlibrary-to-notebooklm/zlibrary-to-notebooklm
---

# zlibrary-to-notebooklm

skills/zstmfhy/zlibrary-to-notebooklm/zlibrary-to-notebooklm
zlibrary-to-notebooklm
Installation
$ npx skills add https://github.com/zstmfhy/zlibrary-to-notebooklm --skill zlibrary-to-notebooklm
SKILL.md
...
优先下载 PDF（保留排版，AI 分析效果更好）
自动降级：如果没有 PDF，下载 EPUB
格式转换：如果下载 EPUB，使用 ebooklib 转换为纯文本
...
询问："请问有 Z-Library 的链接吗？"
如果有链接，执行标准流程
如果没有链接，提示："请提供 Z-Library 书籍页面链接，我可以帮你自动下载并上传到 NotebookLM"
如果用户提供其他来源
...
告知用户："本 Skill 主要用于 Z-Library 链接"
建议："对于本地文件，你可以直接使用 notebooklm source add 命令上传"
📊 技术细节
下载优先级
PDF - 保留排版，AI 分析效果最佳
EPUB - 转换为纯文本（使用 ebooklib）
其他格式 - 尝试转换或提示用户
会话管理
一次登录，永久使用
...
Q: 提示"未找到登录会话" A: 需要先运行 python3 scripts/login.py 登录一次

Q: 下载失败，超时 A: 可能是网络问题，建议重试或检查网络连接

Q: 找不到下载按钮 A: Z-Library 页面结构可能变化，使用备用方案手动下载

Q: NotebookLM 上传失败 A: 检查文件大小（NotebookLM 有上传限制）
...
Skill Version: 1.0.0 Last Updated: 2025-01-14 Author: zstmfhy

Weekly Installs
167
Repository
zstmfhy/zlibrar…tebooklm
GitHub Stars
1.6K
First Seen
Jan 20, 2026
