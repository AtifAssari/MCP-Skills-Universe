---
title: wordpress-live-validation
url: https://skills.sh/notque/claude-code-toolkit/wordpress-live-validation
---

# wordpress-live-validation

skills/notque/claude-code-toolkit/wordpress-live-validation
wordpress-live-validation
Installation
...
article (most WordPress themes)
.entry-content (classic themes)
.post-content (premium themes)
main (fallback)
...
wordpress-uploader creates the post, returns post_url
NAVIGATE: Load post_url, wait for content area
VALIDATE: Run all 7 checks
RESPONSIVE: Screenshots at 375/768/1440px
REPORT: Structured output with pass/fail and screenshots
Example 2: Standalone Live Check

User says: "Check if https://your-blog.com/posts/my-latest/ looks right"

NAVIGATE: Load the URL
VALIDATE: All checks run without expected title/H2 (reports rendered values as INFO)
...
NAVIGATE: Load the URL
VALIDATE: Full check suite (user focuses on SEO/SOCIAL section)
RESPONSIVE: Run for completeness
REPORT: Full report
Error Handling
...
Detect in Phase 1 when first browser tool call fails
Exit with skip report: "Playwright MCP not available. Skipping live validation."
Do not retry—browser validation requires Playwright
Error: Page Returns 4xx/5xx

Cause: Wrong URL, post deleted, or WordPress down Solution:

Capture screenshot of what browser shows
Report HTTP status code
STOP at Phase 1 gate—do not proceed to validation
If draft preview URL, suggest checking that post exists and is accessible
...
Selector chain (article → .entry-content → .post-content → main) covers most themes
If none match: capture screenshot and DOM snapshot
Report FAIL with suggestion: "Content area not found. Try specifying a custom selector."
Still attempt Phase 2 checks against full page (OG tags work without content selector)
...
browser_network_requests reports what browser observed—timeout images appear as failed
If all images fail: likely network issue rather than broken content
Report failure with pattern note: "All {N} images failed—possible network/CDN issue rather than broken content"
Error: Cookie Banner Blocks Content

Cause: GDPR/consent overlay covers content Solution:

Phase 1 Step 4 attempts DOM removal of common banners
If banner persists (non-standard selector): screenshots may show overlay
DOM-level checks (title, H2s, OG tags) still work—they query elements directly
Note in report if consent overlay detected but not dismissed
References
