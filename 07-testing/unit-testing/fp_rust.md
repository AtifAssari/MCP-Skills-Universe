...
rating: ⭐⭐⭐
title: fp-rust
url: https://skills.sh/mpurbo/purbo-skills/fp-rust
---

# fp-rust

skills/mpurbo/purbo-skills/fp-rust
fp-rust
Installation
...
IO/clock/randomness? → Shell. Inject data, not services.
Can params be borrowed? → &T, &str, &[T]. Copy types by value.
Uses mut? → Replace with transform/fold/map. If needed, encapsulate.
Uses unwrap()? → Only in shell/test/provably safe.
Types tight enough? → String → newtype? Option → separate type? bool → enum?
Data pipeline? → .iter() chains or .and_then(). Defer .collect().
Dependency needed? → Reject it. Pass data, not services.
Error handling complete? → Typed enums, exhaustive match, both paths tested.
Documented? → /// on public items, comments explain "why" not "what".
mut Concession Litmus Test

Before using mut in core code:

Can I restructure to avoid it?
Is the mutation encapsulated (invisible to caller)?
Does the function remain deterministic from caller's perspective?
