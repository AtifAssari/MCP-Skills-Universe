---
title: wallet
url: https://skills.sh/synctxai/synctx/wallet
---

# wallet

skills/synctxai/synctx/wallet
wallet
Installation
$ npx skills add https://github.com/synctxai/synctx --skill wallet
SKILL.md
0. Critical Constraints
NEVER write raw Deno/ethers code. Do not import ethers or viem directly; do not manually construct ABI encoding. ALL contract interactions MUST go through the run.ts CLI commands described below.
NEVER fabricate data. Addresses, amounts, and function signatures must come from user input or on-chain queries. If a parameter is unknown, use list-functions to discover it or ask the user.
Write operations require user confirmation — preview with --dry-run, present the details, then execute only after the user approves. Exception: when the SyncTx workflow's Special Authorizations override this (confirmation is skipped).
1. On Load
...
Discover: run list-functions CONTRACT --chain 8453. The exact return type and parameter encoding must come from this list, not from guesses (e.g. instruction()->(string) vs instruction() returns (string)).
Find the target function signature from the output.
Call: read addr "sig" --args [...] for reads, send addr "sig" --args [...] for writes.
After a write: check the tx response for needed data (returned IDs, status, etc.). Only use decode-logs if you need event data not already in the tx response.
On failure: use decode-revert with the revert hex data to get the human-readable reason before doing anything else — never re-attempt a send with tweaked args until you understand the revert.
...
Preview: run send with --dry-run to estimate gas and preview details.
Confirm: present to the user the target contract, function, args, and estimated gas cost.
Execute: run send without --dry-run after user confirmation.
Verify: check the tx response for needed data. Use decode-logs only if additional event data is required.
On failure: use decode-revert with the revert hex to get the reason.
...
10. Rules
Parse $ARGUMENTS and map to the corresponding command.
If PRIVATE_KEY is missing, automatically run generate-wallet and inform the user. Read-only operations (read, list-functions) do not need it.
Read operations execute directly via RPC. Write operations send transactions directly — the user pays gas.
All parameters must be real values — never fabricate addresses, amounts, or signatures.
If required parameters are missing, ask the user.
On error, use decode-revert with the revert hex to get the human-readable reason before troubleshooting.
