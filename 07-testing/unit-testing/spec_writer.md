---
title: spec-writer
url: https://skills.sh/notque/claude-code-toolkit/spec-writer
---

# spec-writer

skills/notque/claude-code-toolkit/spec-writer
spec-writer
Installation
$ npx skills add https://github.com/notque/claude-code-toolkit --skill spec-writer
SKILL.md
Spec Writer Skill
Purpose
...
Review each acceptance criterion—flag any that use subjective language. Replace with measurable assertion: latency threshold, click count, error rate.
Review out-of-scope—flag if fewer than 3 items. Brainstorm adjacent features, related capabilities, and future enhancements that are NOT part of this work to reach minimum 3.
Review story count—flag if more than 7. If scope is too broad to list 3 things this feature does NOT do, the scope is not yet defined. Decompose into multiple specs, one per coherent capability.
Save the spec:
If .feature/ directory exists: save to .feature/SPEC.md
Otherwise: save to SPEC.md in project root
Report the artifact location and suggest next step:
