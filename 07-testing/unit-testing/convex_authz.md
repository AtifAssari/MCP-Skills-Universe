...
rating: ⭐⭐⭐
title: convex-authz
url: https://skills.sh/dbjpanda/convex-tenants/convex-authz
---

# convex-authz

skills/dbjpanda/convex-tenants/convex-authz
convex-authz
Installation
...
Expired role assignments and permission overrides (and their indexed rows) are purged by a scheduled cleanup job that you enable once—no need to add convex/crons.ts yourself. The component embeds @convex-dev/crons; run this once after installing the component to register the daily job:
...
Register both components independently in convex.config.ts
The other component exports its required permissions and roles
