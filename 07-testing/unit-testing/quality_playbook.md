...
url: https://skills.sh/github/awesome-copilot/quality-playbook
---

# quality-playbook

skills/github/awesome-copilot/quality-playbook
quality-playbook
Installation
...
Scan for an index file first. Look for files named INDEX*, CONTEXT.md, README.md, or similar navigation aids. If one exists, read it — it will tell you what's there and how to find things.
...
Existing test suite — tests are implicit specifications. If a test asserts process(x) == y, that's a requirement.
...
Read real data files — If the project has items files, fixture files, config files, or sample data (in pipelines/, fixtures/, test_data/, examples/), read them. Your test fixtures must match the real data shape exactly.
...
For each consumer of state (UI handlers, API endpoints, control flow guards), check: does it handle every possible state? A switch/match without a meaningful default, or an if/elif chain that doesn't cover all states, is a gap.
