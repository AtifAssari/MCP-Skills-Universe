---
title: smart-money-signal-copy-trade
url: https://skills.sh/okx/plugin-store/smart-money-signal-copy-trade
---

# smart-money-signal-copy-trade

skills/okx/plugin-store/smart-money-signal-copy-trade
smart-money-signal-copy-trade
Installation
...
User Customization Encouraged: Users are encouraged to deeply understand the meaning of each parameter and modify them according to their own strategy logic and risk preferences. Every parameter in config.py is annotated with comments for easy customization.
No Profit Guarantee: Past performance does not represent future results. Even parameters that perform well in backtesting may fail in live trading due to changing market conditions.
...
Agentic Wallet uses TEE secure enclave signing. Private keys are never exposed to code/logs/network. No need to set the WALLET_PRIVATE_KEY environment variable.
...
PAUSED = False
Launch
Modify corresponding parameters in config.py based on user responses
Set PAUSED = False (allow bot to run normally after interaction confirmation)
Check prerequisites: onchainos --version, onchainos wallet status
Start bot: python3 bot.py
Show Dashboard link: http://localhost:3248
Inform user: Currently in Paper Mode. To switch to Live Mode, change DRY_RUN = False in config.py
...
First launch defaults to PAUSED=True, no new positions will be opened. After confirming everything is normal, change config.py PAUSED=False. config.py supports hot-reload (importlib.reload). No bot restart needed after changes.
...
advanced-info safety check failure → Fail-Closed, do not buy.
soldRatioPercent > 80% skip — Smart money is already selling, don't catch the falling knife.
K1 price change > 15% skip — Don't chase pumps.
...
SOL_GAS_RESERVE 0.05 SOL is never spent on trades.
Must hold state_lock before writing to positions.json.
