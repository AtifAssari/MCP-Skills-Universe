...
rating: ⭐⭐⭐⭐
title: gcp
url: https://skills.sh/lobbi-docs/claude/gcp
---

# gcp

skills/lobbi-docs/claude/gcp
gcp
Installation
$ npx skills add https://github.com/lobbi-docs/claude --skill gcp
SKILL.md
GCP Skill
...
query = """ SELECT agent_id, COUNT(*) as task_count FROM project.dataset.tasks GROUP BY agent_id ORDER BY task_count DESC """ results = bq_client.query(query) for row in results: print(f"{row.agent_id}: {row.task_count}")
