---
title: drawio-mcp-diagramming
url: https://skills.sh/thomast1906/github-copilot-agent-skills/drawio-mcp-diagramming
---

# drawio-mcp-diagramming

skills/thomast1906/github-copilot-agent-skills/drawio-mcp-diagramming
drawio-mcp-diagramming
Installation
...
Identify the cloud provider — determine whether the diagram uses Azure, AWS, or both (multi-cloud).
Verify icon paths from the static catalogs — no scripts needed at runtime:
...
If diagram uses neither Azure nor AWS icons: skip icon lookup and create diagram directly.
For Azure infrastructure/network diagrams: apply Professional Network Topology Patterns (see Azure section below):
...
For AWS infrastructure/network diagrams: apply AWS Network Topology Patterns (see AWS section below):
...
For cloud-specific diagrams, prefer one icon per major service and use edges for flow semantics (ingress/egress/peering/telemetry).
...
Use the static catalog — references/aws4-complete-catalog.txt contains all 1,037 AWS4 stencil shape names.
