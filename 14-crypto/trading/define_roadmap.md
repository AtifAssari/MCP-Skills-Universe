...
url: https://skills.sh/nesnilnehc/ai-cortex/define-roadmap
---

# define-roadmap

skills/nesnilnehc/ai-cortex/define-roadmap
define-roadmap
Installation
$ npx skills add https://github.com/nesnilnehc/ai-cortex --skill define-roadmap
SKILL.md
...
✅ 用户确认与持久化：用户明确批准，并写入约定路径（默认 docs/process-management/roadmap.md 或依规范 milestones.md）。
✅ 容量分配按 strategic_goal：路线图必须声明每个 strategic_goal 占当前 cycle 容量的百分比，所有目标百分比之和必须为 100%。百分比由用户敲定，不得由本技能推断（依据 ADR 20260417-unified-value-driven-prioritization-model 决策 3.4）。
...
呈现文档草案并获批后写入 docs/process-management/roadmap.md。 结果：路线图持久化，清晰驱动下一步的资源分配与决策。
示例 2：纠正任务列表反模式
