---
title: ecosystem-map-and-blueprint
url: https://skills.sh/timlai666/skills/ecosystem-map-and-blueprint
---

# ecosystem-map-and-blueprint

skills/timlai666/skills/ecosystem-map-and-blueprint
ecosystem-map-and-blueprint
Installation
$ npx skills add https://github.com/timlai666/skills --skill ecosystem-map-and-blueprint
SKILL.md
...
適用時機
複雜社會議題：原住民正義、生態保育、醫療系統等
盤根錯節的系統：多方交織、需要釐清全局脈絡時
服務規劃前期：在畫藍圖之前，先確認「球場上有哪些球員」
核心三元素
1. 參與者 (Actors)
...
適用時機
改善服務效率：找出服務傳遞中的延遲、斷點、低效節點
提升員工體驗：當前端體驗已完善，轉而優化後台人員操作流程
服務成形後：確立與使用者的互動模式及各部門職責分野
既有服務改良：釐清服務結構，找出可供改善的點
藍圖六大組成部分
層次	名稱	說明
...
確立中心位 (C 位)——選定核心參與者
盤點所有參與者（人、組織、地點、系統）
標示參與者之間的價值交換（資訊/貨品/金錢勞力）
依同心圓或角色功能法劃分區塊
標記閃電符號（關係缺口 / 未連結的機會點）
邀請跨部門協作，補充遺漏元素
服務藍圖繪製流程（建議順序）
確立目的：選定特定使用者類型與服務流程範疇
匯入顧客旅程：直接導入既有 CJM 的步驟與接觸點
畫互動線：標示使用者活動
填入前台（含 Tech Layer）
畫可視線：加入後台行為
畫內部互動線：加入支援系統與第三方
標示箭頭與閃電符號：方向性互動 + 缺口診斷
加上實體支援：對應每個步驟的有形物件
