---
title: wechat-cell-writer
url: https://skills.sh/xiaochuan886/xiaochuan886-skills/wechat-cell-writer
---

# wechat-cell-writer

skills/xiaochuan886/xiaochuan886-skills/wechat-cell-writer
wechat-cell-writer
Installation
$ npx skills add https://github.com/xiaochuan886/xiaochuan886-skills --skill wechat-cell-writer
SKILL.md
...
Step 1 检索： references/search-queries.md, references/output-format.md
Step 2 选题： 使用 topics.md 骨架即可，必要时回看 references/column-templates.md
Step 3 写作： references/title-guidelines.md, references/citation-format.md, references/article-template.md, references/column-templates.md
Step 4 配图： references/image-generation-guide.md, references/cover-image-guidelines.md, references/paper-screenshot-guide.md
Step 5 校验： references/review-checklist.md, references/compliance-checklist.md
Step 6 发布： references/publishing-guide.md
Three Article Columns
1. Research Frontiers (科研前沿解读)

Interpret latest research papers and clinical progress.

Title Formula: 「最新研究」 + Topic + Significance Example: 「最新研究」TIL疗法攻克实体瘤，肺癌患者迎来新曙光

2. Cell Science Series (细胞科普系列)

Explain cell types and their functions.

Title Formula: Cell Type + Core Function + User Value Example: NK细胞：你身体里的"天然杀手"，如何守护健康？

3. Health Tips (健康管家Tips)

Practical health advice for readers.

Title Formula: Problem Scenario + Solution Example: 免疫力下降怎么办？科学提升免疫力的5个方法
...
模板和示例见 references/article-template.md。
...
详细规则见 references/paper-screenshot-guide.md。
...
→ 搜索模式: references/search-queries.md → 输出格式: references/output-format.md
...
references/column-templates.md
Step 4: Image Generation (3+张配图)
4.5 引用图补充（自动执行）
...
优先选择概念性图片（机制图、示意图），这类图片能帮助读者理解复杂概念
图片必须与内容自然融合，不能生硬插入
每张图需要解释文字：承接上文 + 解释图片 + 呼应主题

详细规则和示例见 references/reference-image-guide.md。
...
⛔ BLOCKING: 在生成任何图片之前，必须先阅读 references/image-generation-guide.md，按照其中的 封面带标题规则、5维度框架 和 Type × Style 矩阵 构建 prompt。
...
详细配图指南: references/image-generation-guide.md 论文截图指南: references/paper-screenshot-guide.md

生成命令、prompt 模板和示例统一见 references/image-generation-guide.md。
...
人工复核清单见 references/review-checklist.md。

Step 6: Review & Publish

发布主题、颜色和命令见 references/publishing-guide.md。

发布前硬规则：

article.md 和最终发布稿都不包含 URL
图片已生成且正文已引用
论文截图已控制在 1-2 篇核心论文
...
WeChat API: invalid credential	在 微信公众号后台 → 开发 → 基本配置 重新获取 AppID 和 AppSecret
...
References
文件	内容
title-guidelines.md	标题写作指南
citation-format.md	引用格式规范
column-templates.md	各栏目模板
compliance-checklist.md	内容合规检查
cover-image-guidelines.md	封面图设计指南
article-template.md	文章完整模板
review-checklist.md	发布前检查清单
search-queries.md	搜索查询示例
output-format.md	信息检索输出格式
paper-screenshot-guide.md	论文截图指南
Weekly Installs
9
Repository
xiaochuan886/xi…6-skills
GitHub Stars
1
First Seen
Mar 11, 2026
