---
title: alibabacloud-oss-manage-cron-upload
url: https://skills.sh/aliyun/alibabacloud-aiops-skills/alibabacloud-oss-manage-cron-upload
---

# alibabacloud-oss-manage-cron-upload

skills/aliyun/alibabacloud-aiops-skills/alibabacloud-oss-manage-cron-upload
alibabacloud-oss-manage-cron-upload
Installation
...
BucketName must contain only lowercase letters, digits, and hyphens ([a-z0-9-]), be 3–63 characters, and must not start or end with a hyphen.
RegionId must match the Alibaba Cloud region format (e.g., cn-hangzhou, us-west-1, ap-southeast-5).
MaxAge must be a positive integer followed by d (days), h (hours), or m (minutes).
LocalSourcePath, AliyunBinaryPath, and LogPath must be absolute paths and must not contain shell metacharacters ($, `, $(, ;, |, &, >, <, \n).
TargetOssPrefix must contain only alphanumeric characters, /, _, ., and -, and must not start with /.
...
First run list-buckets (above) to confirm the bucket truly does not exist in the account — if it already exists, skip creation and go directly to stat verification.
Only if the bucket is confirmed absent, create it by following the existing creation flow of this skill.
After creation, immediately re-run stat to verify:
aliyun ossutil stat "oss://${BucketName}" --region "${RegionId}" --output-format json \
  --read-timeout 60 --connect-timeout 30 \
  --user-agent AlibabaCloud-Agent-Skills/alibabacloud-oss-manage-cron-upload
...
aliyun ossutil cp --help succeeds.
the canonical aliyun ossutil cp ... -r -u --max-age ... --region ... command completes without permission or endpoint errors.
aliyun ossutil ls ... --region ... shows the expected uploaded objects under the confirmed prefix.
the upload command keeps -u, meaning it uploads only when the target object is missing or the local source file is newer than the existing OSS object.
the local scheduler entry is visible through crontab -l or Task Scheduler history/query, or is documented for the user when crontab is not available in the current environment.
Cleanup
...
Run aliyun version and aliyun configure list on the target machine before live execution.
Never print AK/SK values, never hardcode them in scripts, never read credential files like ~/.aliyun/config.json, and never replace the credential gate with inline secret handling.
...
Always use absolute paths for LocalSourcePath in commands and scripts. Do not use ~ (tilde) because it may not expand inside quoted strings, causing "not a directory" errors.
In generated scripts intended for cron or Task Scheduler, include the -f flag to prevent interactive confirmation prompts from blocking unattended execution.
Reference Links
