...
rating: ⭐⭐
title: qa-metrics
url: https://skills.sh/petrkindlmann/qa-skills/qa-metrics
---

# qa-metrics

skills/petrkindlmann/qa-skills/qa-metrics
qa-metrics
Installation
$ npx skills add https://github.com/petrkindlmann/qa-skills --skill qa-metrics
SKILL.md
Discovery Questions
...
Start with 3 metrics: code coverage, flakiness rate, defect escape rate
Add execution metrics when CI costs or speed become pain points
Add process metrics when the team grows beyond 5 engineers
Add full suite when quality is a product differentiator or compliance requirement
Quality Dashboards
...
Pick 3 metrics. Start with code coverage, flakiness rate, and defect escape rate. These three give the most signal with the least setup.
Automate collection. Coverage comes from your test runner (Istanbul, c8, coverage.py). Flakiness comes from CI run history. Defect escapes come from tagging production bugs.
Set initial targets. Use the table above based on your company stage. Adjust after 2-4 weeks of baseline data.
Assign owners. One engineer owns coverage. One owns flakiness. Engineering manager owns defect escape rate.
Phase 2: Visibility (Week 3-4)
Create the engineering dashboard. Use Grafana, Datadog, or even a shared Google Sheet. The tool matters less than the habit of looking at it.
Add CI annotations. Surface coverage deltas and flakiness warnings directly in PR comments (many CI tools support this natively).
Set up alerts. Flakiness >5%: Slack alert to the team. Coverage drops >2% on a PR: block merge or flag for review.
Establish a review cadence. Review metrics in weekly team standup (5 minutes, not 30).
Phase 3: Quality Gates (Month 2)
Define quality gates for releases. Example gates:
All P0 tests pass
Coverage has not decreased
No new P0/P1 bugs unresolved
Flakiness rate below threshold
E2E smoke suite green
Automate gate enforcement. Use CI pipeline stages, branch protection rules, or deployment gates.
Track gate pass rate. If gates are consistently overridden, the gates are either too strict or the team does not trust them. Adjust.
Phase 4: Advanced Metrics (Month 3+)
Add process metrics (automation rate, test velocity, ROI).
Build the leadership dashboard.
Implement trend alerting (not just threshold alerts, but "coverage has been declining for 3 consecutive sprints" alerts).
Connect metrics to retros. Use data to drive sprint retrospective discussions, not opinions.
