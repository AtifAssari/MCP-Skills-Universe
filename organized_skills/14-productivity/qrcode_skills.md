...
rating: ⭐⭐⭐
title: qrcode-skills
url: https://skills.sh/caoliao/qrcode-skills/qrcode-skills
---

# qrcode-skills

skills/caoliao/qrcode-skills/qrcode-skills
qrcode-skills
Installation
$ npx skills add https://github.com/caoliao/qrcode-skills --skill qrcode-skills
SKILL.md
...
如需二维码生成 URL（无需保存文件即可预览）、更好的解码效果，可使用 qrcode-remote-skills：npx skills add caoliao/qrcode-remote-skills。
...
检测 Python 是否可用：python --version（或 python3 --version）
若 Python 不可用，检测 Node.js：node --version
两者都有时默认用 Python
...
用户提供批量数据文件
执行脚本
若返回 need_column，展示列信息，询问用户选哪一列，加 --column 重新执行
成功后向用户报告结果
...
执行脚本
若返回 need_column，将 columns 和 preview 展示给用户，询问使用哪一列
用户回答后，加上 --column 重新执行
成功后向用户报告结果统计
