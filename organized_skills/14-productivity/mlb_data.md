...
rating: ⭐⭐⭐
title: mlb-data
url: https://skills.sh/machina-sports/sports-skills/mlb-data
---

# mlb-data

skills/machina-sports/sports-skills/mlb-data
mlb-data
Installation
...
Call get_scoreboard() Result: All live and recent MLB games with scores by inning and status
...
Filter results for American League East Result: AL East standings with W-L, PCT, GB, run differential

Example 3: Team roster User says: "Who's on the Yankees roster?" Actions:

Call get_team_roster(team_id="10") Result: Full Yankees roster with name, position, bats/throws, height, weight
...
Call get_game_summary(event_id=<id>) for full box score Result: Complete box score with batting/pitching stats per player

Example 5: Injury report User says: "Who's on the IL for the Yankees?" Actions:

Call get_injuries()
Filter results for New York Yankees (team_id=10) Result: Yankees IL list with player name, position, IL type, and injury detail

Example 6: Player statistics User says: "Show me Shohei Ohtani's stats" Actions:

Derive season year from currentDate
Call get_player_stats(player_id="39832", season_year=<derived_year>) Result: Season stats by category (batting, pitching) with value, rank, and per-game averages
