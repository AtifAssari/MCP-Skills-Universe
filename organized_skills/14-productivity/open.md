...
rating: ⭐⭐
title: open
url: https://skills.sh/spm1001/claude-suite/open
---

# open

skills/spm1001/claude-suite/open
open
Installation
...
Diagnose: Run ~/.claude/scripts/check-symlinks.sh to identify the issue.
3. Orient
...
Read latest handoff — ls -t ~/.claude/handoffs/$ENCODED/*.md 2>/dev/null | head -1 (if empty, no prior sessions here)
Check tracker context — read ~/.claude/.session-context/$ENCODED/bon.txt if it exists
News if relevant — read ~/.claude/.update-news if user asks or it's actionable
...
If yes: mv .handoff* ~/.claude/handoffs/<encoded-path>/
Handoff
...
Read the item's criteria (bon: brief.why/what/done)
Break down into steps from those criteria
Show user: "Breaking this down into: [list]. Sound right?"
Work through steps with explicit pauses for direction checks
...
Break into steps — this catches scope gaps before work begins
Proceed with explicit pause points
...
Break into steps from those criteria
...
Mirrors (GODAR)
Phase	/open	/close
Gate	Load bon, offer todoist	—
Gather	Notifications (stdout) → Read files	Todos, bon, git, drift
Orient	"Where we left off"	Reflect (AskUserQuestion)
Decide	User picks direction	Crystallize actions (STOP)
Act	Draw-down from Bon	Execute, handoff, commit
