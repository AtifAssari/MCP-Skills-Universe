---
rating: ⭐⭐⭐
title: chart-craft-plus
url: https://skills.sh/zephyrwang6/draw-skill/chart-craft-plus
---

# chart-craft-plus

skills/zephyrwang6/draw-skill/chart-craft-plus
chart-craft-plus
Installation
...
brand-design-md 注册表找 slug
运行 cd /tmp && mkdir -p design-md-tmp && cd design-md-tmp && npx getdesign@latest add <slug> 2>&1
读生成的 DESIGN.md，提取 token 应用
Step 5：生成 HTML
...
设计原则
HTML + CSS + 内联 SVG，不用 Canvas / JS。截图清晰、打印友好。
布局用 Flexbox / Grid，不硬编码像素位置。
字体用系统字体栈，中文渲染良好。技术类图表可选 JetBrains Mono 增强质感。
颜色用 CSS 变量，方便一键切换风格。
留白充足，区块之间有呼吸感。
层级清晰：标题 > 区块名 > 内容，三级字体大小对比明显。
语义化配色（技术类）：cyan=前端，emerald=后端，violet=数据库，amber=云服务，rose=安全，slate=外部。
图例必备：复杂图（桑基、热力、时序、C4、威胁建模）必须附图例。
文件保存
