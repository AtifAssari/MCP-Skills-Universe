---
rating: ⭐⭐⭐
title: dify-tool-developer
url: https://skills.sh/skilzy-ai/official-skills/dify-tool-developer
---

# dify-tool-developer

skills/skilzy-ai/official-skills/dify-tool-developer
dify-tool-developer
Installation
$ npx skills add https://github.com/skilzy-ai/official-skills --skill dify-tool-developer
SKILL.md
...
"Does it need to call external APIs? If so, which APIs?"
"Does it require OAuth authentication? (e.g., GitHub, Google, Slack)"
"What inputs should users provide to this tool?"
"What outputs should the tool return?"
...
LOAD: https://docs.dify.ai/plugin-dev-en/0221-initialize-development-tools.md
Follow the installation instructions for the user's operating system
Verify installation with dify version
...
ONE Tool class per .py file - Multiple classes will cause errors
Exact method signature: _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]
Use .get() for parameters - Prevents KeyError exceptions
Return with yield - Not return
Import from correct modules - Match documentation exactly
...
Go to Tools section
Find your plugin (should appear automatically)
Try invoking the tool with test inputs
Observe results
...
Visit the Dify plugin marketplace
...
Define OAuth Schema in provider/*.yaml:
oauth_schema:
  client:
    - name: client_id
      type: string
...
Implement OAuth Methods in provider/*.py:
...
Current Phase: Tell the user which phase you're in
Completed Steps: Show what has been accomplished
Next Actions: Explain what will happen next
User Input Needed: Ask for clarification when needed
Problems Encountered: Report any issues and solutions
...
Setup & Planning - Gather requirements, verify CLI
Initialize Project - Create plugin structure with CLI
Implement Tool - Write YAML configs and Python code
Test & Debug - Run locally, fix issues, add logging
Package & Publish - Create .difypkg file, optionally publish
