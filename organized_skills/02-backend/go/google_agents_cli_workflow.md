...
url: https://skills.sh/google/agents-cli/google-agents-cli-workflow
---

# google-agents-cli-workflow

skills/google/agents-cli/google-agents-cli-workflow
google-agents-cli-workflow
Installation
...
Write/modify code in the agent directory (check GEMINI.md / CLAUDE.md for directory name)
Quick smoke test: Use agents-cli run "your prompt" to verify the agent works after changes — this is the fastest way to check behavior without leaving the terminal
...
Check if the project has a deployment target configured — run agents-cli info to see current config
If the project is a prototype (no deployment target), add deployment support first:
...
Deploy when ready: agents-cli deploy
...
Preservation Check: Confirm that all code, configuration values (e.g., model, version, api_key), comments, and formatting outside the identified target remain identical.
...
Localize — Narrow the cause: is it the agent code, a tool, the config, or the environment? Use agents-cli run "prompt" to isolate agent behavior from deployment issues.
Fix one thing — Change one variable at a time. If you change the instruction AND the tool AND the config simultaneously, you won't know what fixed it (or what broke something else).
