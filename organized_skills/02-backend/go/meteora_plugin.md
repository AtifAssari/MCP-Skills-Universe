---
rating: ⭐⭐⭐
title: meteora-plugin
url: https://skills.sh/okx/plugin-store/meteora-plugin
---

# meteora-plugin

skills/okx/plugin-store/meteora-plugin
meteora-plugin
Installation
$ npx skills add https://github.com/okx/plugin-store --skill meteora-plugin
SKILL.md
...
Run swap (no flags) to preview the quote — outputs "preview": true, estimated_output, price_impact_pct; no transaction submitted
Ask user to confirm the swap details (from/to tokens, amount, estimated output, slippage)
Execute after explicit user approval: meteora-plugin --confirm swap --from-token ... --to-token ... --amount ...
...
Run without --confirm to preview: shows position PDA, bin range, token accounts; no transaction submitted
Ask user to confirm token amounts, pool, and that they understand liquidity provisioning risk
Execute after explicit user approval with --confirm: meteora-plugin --confirm add-liquidity --pool <addr> --amount-x ... --amount-y ...
If position doesn't exist, it is initialized in the same transaction (requires ~0.06 SOL for rent)
...
Run without --confirm to preview: shows bin range, token accounts, and whether the position will be closed
Ask user to confirm — especially if --close is used (permanent, reclaims rent)
Execute after explicit user approval with --confirm
Token X and token Y are returned to the wallet's associated token accounts (created on-chain if missing)
If --close is set and --pct 100, the position account is closed and ~0.057 SOL is returned
...
Returns a ready-to-run add-liquidity command
