...
url: https://skills.sh/antonbabenko/terraform-skill/terraform-skill
---

# terraform-skill

skills/antonbabenko/terraform-skill/terraform-skill
terraform-skill
Installation
...
Assumptions & version floor — runtime (terraform or tofu), exact version, providers, state backend, execution path (local/CI/Cloud/Atlantis), environment criticality. State assumptions explicitly if the user did not provide them.
Risk category addressed — one or more of: identity churn, secret exposure, blast radius, CI drift, compliance gaps, state corruption, provider upgrade risk, testing blind spots.
...
Generate artifacts — HCL, migration blocks (moved, import), CI changes, policy rules.
