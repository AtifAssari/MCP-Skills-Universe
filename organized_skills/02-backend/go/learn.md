...
rating: ⭐⭐
title: learn
url: https://skills.sh/duc01226/easyplatform/learn
---

# learn

skills/duc01226/easyplatform/learn
learn
Installation
...
Enhance -- Run /prompt-enhance on modified file(s) to optimize AI attention anchoring
...
Execute the chosen option. For "Full prevention", create a plan via /plan instead of just saving.
...
Analyze root cause — Why did this mistake happen? What is the underlying pattern?
Generalize — Strip project-specific names, file paths, and tool names. Express the lesson as a universal principle.
Verify universality — Would this lesson help an AI working on a completely different codebase? If no → rewrite until yes.
...
Tell the user: "This lesson fits best in docs/{file}. Confirm? [Y/n]"
On confirm — read target file, find the right section, append the lesson
...
Read file, count characters (wc -c docs/project-reference/lessons.md)
If current + new lesson > 3000 chars → trigger Budget Trim before saving
...
Present to user with AskUserQuestion: "Budget exceeded. Recommend removing/condensing these LOW/MEDIUM items: [list]. Approve?"
