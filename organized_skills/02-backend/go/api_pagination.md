...
url: https://skills.sh/peterbamuhigire/skills-web-dev/api-pagination
---

# api-pagination

skills/peterbamuhigire/skills-web-dev/api-pagination
api-pagination
Installation
...
Key PHP Rules
Always closeCursor() after the COUNT query before running the main query (PDO requirement).
Bind LIMIT/OFFSET as PDO::PARAM_INT — string binding causes MySQL errors.
Cap per_page at 100 to prevent abuse: min(100, max(1, ...)).
Default per_page is 30 for list screens, 50 for stock-level screens.
Response shape is data.items + data.pagination — NEVER return a flat array in data.
Additional Guidance

Extended guidance for api-pagination was moved to references/skill-deep-dive.md to keep this entrypoint compact and fast to load.
