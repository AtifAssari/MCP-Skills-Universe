...
url: https://skills.sh/chromedevtools/chrome-devtools-mcp/a11y-debugging
---

# a11y-debugging

skills/chromedevtools/chrome-devtools-mcp/a11y-debugging
a11y-debugging
Installation
...
Use take_snapshot to capture the accessibility tree.
Check Heading Levels: Ensure heading levels (h1, h2, h3, etc.) are logical and do not skip levels. The snapshot will include heading roles.
...
Locate buttons, inputs, and images in the take_snapshot output.
Ensure interactive elements have an accessible name (e.g., a button should not just say "" if it only contains an icon).
Orphaned Inputs: Verify that all form inputs have associated labels. Use evaluate_script with the "Find Orphaned Form Inputs" snippet found in references/a11y-snippets.md.
...
Use the press_key tool with "Tab" or "Shift+Tab" to move focus.
Use take_snapshot to capture the updated accessibility tree.
Locate the element marked as focused in the snapshot to verify focus moved to the expected interactive element.
...
Call list_console_messages with types: ["issue"].
Look for "Low Contrast" issues in the output.
