---
rating: ⭐⭐
title: eagle-architect
url: https://skills.sh/autumnsgrove/groveengine/eagle-architect
---

# eagle-architect

skills/autumnsgrove/groveengine/eagle-architect
eagle-architect
Installation
$ npx skills add https://github.com/autumnsgrove/groveengine --skill eagle-architect
SKILL.md
Eagle Architect 🦅
...
What problem are we solving? — The user pain point, not the technical solution
What's the scale? — 10 users or 10 million? This changes everything
What are the constraints? — Budget, timeline, team size, existing tech
What's the growth trajectory? — Plan for where you're going, not just where you are
...
Categories: features/, infra/, security/, tooling/ Lifecycle stages: planning/ → planned/ → active/ → completed/
...
🦅 SOAR — "System needs to send emails, push, SMS to millions of users. Constraints: must be reliable, retry failed sends, handle rate limits."

🦅 SURVEY — "Current system sends synchronously during request. This blocks and fails on provider outages. Need async queue, separate service."

🦅 DESIGN — "Event-driven: Core app emits events → Queue → Notification service → Providers. Separate channels per provider for isolation."

🦅 BLUEPRINT — Document: API contract for event publishing, queue schema, retry logic, monitoring dashboard, provider adapter interface

🦅 BUILD — Guide implementation: queue first, then service, then provider adapters, then client integration
