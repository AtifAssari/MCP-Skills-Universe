...
url: https://skills.sh/xzxiaoshan/open-agent-skills/china-holidays
---

# china-holidays

skills/xzxiaoshan/open-agent-skills/china-holidays
china-holidays
Installation
$ npx skills add https://github.com/xzxiaoshan/open-agent-skills --skill china-holidays
SKILL.md
国家法定节假日查询
...
优先读取本地缓存：检查 assets/{年份}.md 是否存在
缓存命中：直接返回缓存的节假日数据（毫秒级响应）
...
确认年份：2026
运行：python scripts/fetch_holidays.py --year 2026
获取国务院办公厅通知内容
整理输出节假日安排
...
确认年份：2026
运行：python scripts/fetch_holidays.py --year 2026 --force
脚本会忽略本地缓存，重新从政府网获取数据
...
分别获取两年的节假日数据
运行：python scripts/fetch_holidays.py --year 2025 和 python scripts/fetch_holidays.py --year 2026
提取春节相关信息进行对比
...
建议用户使用缓存数据（--force 参数移除）或稍后重试
如用户需要，可告知用户直接访问政府网查看
内容解析失败
