...
url: https://skills.sh/discountry/logo-maker/svg-logo-maker
---

# svg-logo-maker

skills/discountry/logo-maker/svg-logo-maker
svg-logo-maker
Installation
$ npx skills add https://github.com/discountry/logo-maker --skill svg-logo-maker
SKILL.md
SVG Logo Maker
...
Select 3–5 reference logos from {SKILL_DIR}/assets/ that are most relevant to the target brand's industry, style, or logo type. Use the Read tool to view the PNG files — AI can visually parse these images.
Read the corresponding SVG files for the selected logos to understand their construction technique.
...
Lock the browser for interaction:
browser_lock

Take a screenshot to capture the rendered logo:
...
Delete the workspace — Remove _logo-workspace.html using the Delete tool.

Confirm — "Logos saved to {path}. Open them in a browser or SVG editor to preview."
