---
rating: ⭐⭐⭐
title: aave-v3-plugin
url: https://skills.sh/okx/plugin-store/aave-v3-plugin
---

# aave-v3-plugin

skills/okx/plugin-store/aave-v3-plugin
aave-v3-plugin
Installation
$ npx skills add https://github.com/okx/plugin-store --skill aave-v3-plugin
SKILL.md
...
Resolves token contract address via onchainos token search (or uses address directly if provided)
Resolves Pool address at runtime via PoolAddressesProvider.getPool()
WETH pre-flight: if supplying WETH, checks on-chain WETH balance. If insufficient but wallet has enough ETH, automatically calls WETH.deposit() to wrap the needed amount first
Non-WETH pre-flight: checks ERC-20 balance; errors with a clear message if insufficient
Ask user to confirm the approval before broadcasting
Approves token to Pool: onchainos wallet contract-call → ERC-20 approve(pool, amount)
Ask user to confirm the deposit before broadcasting
Deposits to Pool: onchainos wallet contract-call → Pool.supply(asset, amount, onBehalfOf, 0)
...
Safety Rules
Simulate first: Always run without --confirm to preview the operation before broadcasting
Confirm before broadcast: Show the user what will happen and only add --confirm after explicit user approval
Never borrow if HF < 1.5 without warning: Explicitly warn user of liquidation risk
Warn at HF < 1.1: Binary emits a warning; advise user to repay before borrowing more
Full repay safety: Use --all flag for full repay — avoids underpayment due to accrued interest
Collateral warning: Before disabling collateral, simulate health factor impact
