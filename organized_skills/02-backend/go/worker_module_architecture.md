...
url: https://skills.sh/tencentblueking/bk-ci/worker-module-architecture
---

# worker-module-architecture

skills/tencentblueking/bk-ci/worker-module-architecture
worker-module-architecture
Installation
...
放置在 com.tencent.devops.plugin.worker.task 包下，自动注册
日志输出规范
// 普通日志
...
Q2: 插件执行失败如何排查？
检查 output.json 输出
查看 Worker 日志（构建机上的 logs/ 目录）
检查插件 SHA 校验是否通过
确认插件运行环境（Python/Node.js 版本等）
Q3: 心跳失败会怎样？

连续 12 次心跳失败后，Worker 会自动退出（exitProcess(-1)）。服务端会将构建标记为异常终止。

Q4: 如何调试 Worker？
设置环境变量 DEVOPS_AGENT_LOG_DEBUG=true 开启调试日志
使用 LoggerService.addDebugLine() 输出调试信息
检查构建机上的日志文件
Q5: 插件包缓存机制？
