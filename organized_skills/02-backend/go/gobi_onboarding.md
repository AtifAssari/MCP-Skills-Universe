---
rating: ⭐⭐⭐
title: gobi-onboarding
url: https://skills.sh/jykim/claude-obsidian-skills/gobi-onboarding
---

# gobi-onboarding

skills/jykim/claude-obsidian-skills/gobi-onboarding
gobi-onboarding
Installation
$ npx skills add https://github.com/jykim/claude-obsidian-skills --skill gobi-onboarding
SKILL.md
When to Use
...
사용자의 기존 환경을 먼저 파악하고 적응한다
연결하면 자동으로 정리되어야 한다
커뮤니티 참여로 지식 공유의 가치를 체험한다
필수만 먼저, 나머지는 Post-boarding으로 점진적 공개
선택을 못하면 대신 골라준다
...
안녕하세요! 온보딩을 시작할게요. 지금부터 네 단계로 진행되는데, 필수 단계만 하면 십 분이면 충분해요. 먼저 프로필을 만들면서 자료를 연결하고, 자동 정리를 설정하고, 커뮤니티에 참여하고, 마지막으로 직접 체험해보는 순서예요. 바로 시작할까요?
...
첫 번째 단계는 자료 연결이에요. 먼저 어떤 분인지 파악해서, 맞춤으로 연결해드릴게요. 참고로, 자료는 모두 내 컴퓨터에만 저장돼요. 외부로 나가지 않으니 안심하세요. 혹시 개인 홈페이지나 블로그가 있으세요?

1-1. 사용자 파악

(홈페이지/블로그가 있는 경우) 주소를 알려주시면 자동으로 가져와서 프로필을 만들어드릴게요. (없는 경우) 괜찮아요. 어떤 분야에서 일하시는지, 관심사가 뭔지 간단히 알려주세요. 프로필을 같이 만들어봐요.

사용자 응답 시나리오
A. 홈페이지 있음
사용자: "제 블로그 주소는 example.com이에요"
에이전트: "좋아요! 지금 가져오고 있어요. 잠시만 기다려주세요." (백그라운드 처리, 완료 시 알림)
BRAIN.md 업데이트: 홈페이지에서 추출한 프로필 정보(이름, 소속, 전문 분야, 관심사 등)를 BRAIN.md에 반영
에이전트: "프로필이 만들어졌어요! 화면에서 확인해보세요. 수정할 부분이 있으면 말씀해주세요."
B. 홈페이지 없음
사용자: "없어요"
에이전트: "괜찮아요. 어떤 일을 하시는지, 또는 관심 있는 분야가 뭔지 알려주시면 제가 프로필 초안을 만들어드릴게요."
BRAIN.md 업데이트: 사용자가 알려준 정보(직업, 관심사 등)를 BRAIN.md에 반영
...
프로필이 준비됐으니, 이제 평소에 어떤 정보를 많이 쓰시는지 알려주세요. 어떤 정보를 볼트에 넣고 싶으세요?
...
에이전트: "그럼 웹 클리핑이 딱이에요. 관심 있는 웹페이지 주소를 저장하면 Ingest/Clippings 폴더에 원본 마크다운이 저장돼요. 그러면 이아이씨 워커가 자동으로 요약하고 정리해서 AI/Summary에 넣어줘요. 피케이엠 관련 좋은 아티클로 먼저 연습해볼까요?"
...
사용자 동의 시: .gobi/syncfiles에 BRAIN.md 추가
사용자 거절/무응답 시: "괜찮아요, 나중에 마음이 바뀌면 '프로필 동기화'라고 말해주세요. 다음 단계로 넘어갈게요."
완료 확인 & 전환

첫 번째 단계 완료! 자료 연결이 잘 됐어요. 다음은 자동 정리 설정이에요. 매일 자료가 알아서 정리되게 해줄 거예요.

2단계: 인덱스 셋업
에이전트 오프닝

두 번째 단계는 인덱스 셋업이에요. 연결한 자료를 에이아이가 자동으로 정리해주는 거예요. 두 가지 옵션이 있는데, 하나는 기본으로 켜져 있어요.

실행 가이드

첫째, 데일리 라운드업. 매일 새로 들어온 자료를 자동으로 요약 정리해줘요. 이건 기본으로 켜져 있어요. 둘째, 토픽 인덱스. 주제별로 자동 분류해줘요. 이건 선택이에요. 토픽 인덱스가 어떤 건지 이미지로 보여드릴게요.

![[Settings/Prompts/files/graph-example.png]]

이렇게 주제별로 연결된 지식 그래프가 만들어져요. 토픽 인덱스에 어떤 주제가 가능한지 보여드릴까요?
...
하나 더 추천해드릴 게 있어요. 매일 아침 다섯 시에 관심 주제의 최신 뉴스와 논문을 자동으로 찾아서 브리핑을 만들어줄 수 있어요. 뉴스 브리핑이라고 하는데, 켜볼까요?
...
두 번째 단계 완료! 이제 자료가 자동으로 정리돼요. 다음은 커뮤니티 설정이에요. 다른 전문가들과 지식을 나눌 수 있는 프로필을 만들 거예요.

3단계: 커뮤니티 셋업
에이전트 오프닝

세 번째 단계는 커뮤니티 셋업이에요. 에이포피케이엠 커뮤니티에서 다른 전문가들과 지식을 나눌 수 있어요. 아까 만든 프로필을 바탕으로 커뮤니티 프로필을 만들어볼게요.

3-1. 커뮤니티 프로필 생성

아까 파악한 정보를 바탕으로 커뮤니티 프로필 초안을 만들었어요. 화면에서 확인해보세요. 수정할 부분이 있으면 말씀해주세요.
...
커뮤니티에서 어떤 주제로 지식을 공유하고 싶으세요? 관심 분야나 전문 영역을 알려주시면 관련 주제를 추천해드릴게요.
...
커뮤니티 프로필이 만들어졌어요! 혹시 커뮤니티에 공유하기 위해 추가로 넣고 싶은 자료가 있나요?
...
세 번째 단계 완료! 커뮤니티 프로필이 준비됐어요. 거의 다 왔어요! 마지막 단계에서 마무리할게요.

4단계: 마무리 & 첫 체험
에이전트 오프닝

마지막 네 번째 단계예요! 지금까지 설정한 걸 확인하고, 에이아이에게 직접 질문해보는 시간이에요.

실행 가이드

이제 볼트에 자료가 들어가고 있어요. 한번 질문을 해볼까요? 아무거나 좋아요. 예를 들어 '내가 어떤 자료를 넣었지?'라든가, 관심 분야에 대해 물어보셔도 돼요.
...
(전체 완료) 축하해요! 온보딩을 모두 완료하셨어요! 프로필을 만들고, 자료를 연결하고, 자동 정리를 켜고, 커뮤니티까지 설정하셨어요. 이제 고비 데스크탑이 매일 나를 도와줄 준비가 됐어요.
...
추가로 설정하면 좋은 것들이 있어요. 첫째, 옵시디언을 설치하면 볼트를 더 편리하게 관리할 수 있어요. 고비 데스크탑만으로도 사용할 수 있지만, 옵시디언이 있으면 노트를 직접 편집하고 연결하기가 훨씬 좋아요. 둘째, 옵시디언 웹 클리퍼를 설치하면 웹 브라우저에서 바로 관심 있는 글을 볼트에 저장할 수 있어요. 웹 클리퍼 설정 화면을 보여드릴게요.

![[Settings/Prompts/files/owc-config.png]]

관심 있으시면 '옵시디언 설치 알려줘'라고 말해주세요.

다음 단계 제안

이제 뭘 하면 좋을까요? 두 가지를 추천해요. 첫째, 내일 아침에 데일리 라운드업을 확인해보세요. 오늘 연결한 자료가 정리돼 있을 거예요. 둘째, 커뮤니티를 둘러보세요. 관심 분야의 다른 전문가들을 만날 수 있어요.
...
태스크 파일 감지: _Settings_/Tasks/에서 해당 task_type의 최신 파일을 폴링 (최대 120초, 5초 간격)
상태 확인: 프론트매터의 status 필드 읽기
PROCESSED → 성공, output 필드에서 결과 파일 경로 확인
FAILED → 실패, generation_log에서 원인 확인
파일 미생성 → 파이프라인 미트리거 (file monitor 또는 워커 설정 확인 필요)
결과 파일 확인: output 경로의 파일이 실제로 존재하는지 검증
온보딩 중 활용
시점	파이프라인	모니터링 대상
