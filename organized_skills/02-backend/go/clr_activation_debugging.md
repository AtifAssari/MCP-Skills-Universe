...
url: https://skills.sh/dotnet/skills/clr-activation-debugging
---

# clr-activation-debugging

skills/dotnet/skills/clr-activation-debugging
clr-activation-debugging
Installation
...
references/log-format.md — Log line format, fields, and all known log message types
references/activation-flow.md — The shim's decision tree for runtime selection
references/com-activation.md — COM (DllGetClassObject) activation specifics, CLSID registry layout
...
ERROR: Unable to find a version of the runtime to use — the shim found no suitable runtime
SEM_FAILCRITICALERRORS is set to {value} — checks the process error mode:
...
Checking if feature-on-demand installation would help — the shim re-runs version computation to see if installing .NET 3.5 would resolve the request
Then either:
...
First activation via ClrCreateInstance / GetRequestedRuntime → succeeds (loads v4.0 via config)
Second activation via DllGetClassObject (COM) → legacy bind, capped → fails
