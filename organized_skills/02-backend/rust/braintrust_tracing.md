...
url: https://skills.sh/parcadei/continuous-claude-v3/braintrust-tracing
---

# braintrust-tracing

skills/parcadei/continuous-claude-v3/braintrust-tracing
braintrust-tracing
Installation
...
Sub-agent's session_id = its trace's root_span_id
...
Sub-agent creates orphaned trace with root_span_id = session_id
...
Export parent session traces (query parent root_span_id)
Export sub-agent traces (query all sessions created within parent's time window)
Match by:
...
In-session logging: Capture SubagentStop session_id in logs or state
Post-session export: Query Braintrust API for parent and sub-agent traces
