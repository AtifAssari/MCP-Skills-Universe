---
rating: ⭐⭐⭐
title: rustdesk-doctor
url: https://skills.sh/majiayu000/claude-arsenal/rustdesk-doctor
---

# rustdesk-doctor

skills/majiayu000/claude-arsenal/rustdesk-doctor
rustdesk-doctor
Installation
$ npx skills add https://github.com/majiayu000/claude-arsenal --skill rustdesk-doctor
SKILL.md
RustDesk 连接诊断工具
...
Clash Verge profiles 中的 RustDesk 配置
...
在 Merge.yaml 中加 route-exclude-address 让服务器 IP 完全绕过 TUN：
tun:
  route-exclude-address:
    - <服务器IP>/32

或让 RustDesk 流量走代理（删除订阅中的 DIRECT 规则）
...
关闭 TUN → 启动 RustDesk（让它检测真实 IP）→ 再开 TUN
或手动清空 RustDesk2.toml 中的 local-ip-addr：
[options]
local-ip-addr = ''
...
切换支持 UDP 的代理节点
对裸 TCP 端口（21115），需加 IP-CIDR 规则让 Mihomo 能路由：
prepend:
