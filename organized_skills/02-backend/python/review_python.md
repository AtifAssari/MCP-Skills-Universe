...
rating: ⭐⭐
title: review-python
url: https://skills.sh/nesnilnehc/ai-cortex/review-python
---

# review-python

skills/nesnilnehc/ai-cortex/review-python
review-python
Installation
$ npx skills add https://github.com/nesnilnehc/ai-cortex --skill review-python
SKILL.md
技能（Skill）：复习Python
目的 (Purpose)
...
上下文管理器（with语句、__enter__/__exit__、@contextmanager）
...
使用场景（用例）
精心安排的审查：当 review-code 运行 Python 项目的范围 -> 语言 -> 框架 -> 库 -> cognitive时，用作语言步骤。
...
审核清单（仅限 Python 维度）
类型提示：对于复杂类型使用 typing 模块，尽可能避免使用 Any，使用 Optional[T] 而不是 T |对于 Python <3.10，无，正确使用 Union、List、Dict、Callable 和泛型类型提示。
异常处理：捕获特定异常，避免裸露的 except:，使用raise ... from进行异常链接，避免在没有日志记录的情况下吞噬异常，正确使用try/finally。
异步/等待：正确使用async def和await，避免异步函数中的阻塞调用，异步上下文中正确的异常处理，使用asyncio.gather，asyncio.create_task实现并发。
上下文管理器：使用with语句进行​​资源管理，实现__enter__/__exit__或使用@contextmanager，避免手动打开/关闭。
依赖管理：将依赖项固定在需求.txt或pyproject.toml中，避免import *，使用虚拟环境，正确使用sys.path操作。
可变默认值：避免可变默认参数（例如 def foo(a=[]):），使用 None 并在函数内部初始化。
命名约定：遵循 PEP8（函数/变量使用 snake_case，类使用 PascalCase，常量使用 SCREAMING_SNAKE_CASE）。
可测试性：避免全局状态，使用依赖注入，模拟外部服务，避免紧密耦合。
语气和参考
...
限制（限制）
硬边界（Hard Boundaries）
不要执行安全、架构或范围选择。遵守 Python 语言和运行时约定。
不要在没有具体地点或可行建议的情况下给出结论。
不要检查非 Python 代码的 Python 特定规则，除非用户明确包含它（例如嵌入的代码片段）。
...
示例 3：异步阻塞调用
输入：异步函数内的async def fetch(): requests.get(url)。
