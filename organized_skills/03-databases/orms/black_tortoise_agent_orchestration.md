---
rating: ⭐⭐
title: black-tortoise-agent-orchestration
url: https://skills.sh/7spade/black-tortoise/black-tortoise-agent-orchestration
---

# black-tortoise-agent-orchestration

skills/7spade/black-tortoise/black-tortoise-agent-orchestration
black-tortoise-agent-orchestration
Installation
$ npx skills add https://github.com/7spade/black-tortoise --skill black-tortoise-agent-orchestration
SKILL.md
...
作業流程（最小化）
定位規則：依 docs/INDEX.md 讀取對應 AGENTS.md → README.md。
拆解：呼叫 sequentialthinking/* 取得步驟；若需正式計畫，用 Software-planning-mcp/* 的 start_planning / save_plan。
查證：
...
保持可刪除性，最小改動。
品質：必要時用 codacy/* 取得分析，再給出可行修正；建議執行 pnpm run lint、pnpm run architecture:gate（不自動執行）。
子代理切換：
