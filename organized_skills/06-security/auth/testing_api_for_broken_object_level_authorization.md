...
url: https://skills.sh/mukul975/anthropic-cybersecurity-skills/testing-api-for-broken-object-level-authorization
---

# testing-api-for-broken-object-level-authorization

skills/mukul975/anthropic-cybersecurity-skills/testing-api-for-broken-object-level-authorization
testing-api-for-broken-object-level-authorization
Installation
$ npx skills add https://github.com/mukul975/anthropic-cybersecurity-skills --skill testing-api-for-broken-object-level-authorization
SKILL.md
Testing API for Broken Object Level Authorization
When to Use
...
Browse the application as User A, exercising all features that involve data creation and retrieval
In Burp, go to Target > Site Map and filter for API paths (e.g., /api/v1/, /graphql)
Look for patterns: /api/v1/users/{id}, /api/v1/orders/{order_id}, /api/v1/documents/{doc_uuid}
Note the object ID format: sequential integers (predictable), UUIDs (less predictable), or encoded values
...
Map all endpoints from the Swagger spec at /api/docs: identify 47 endpoints, 23 of which take object IDs
Capture User A's requests for their own resources: profile, orders, addresses, payment methods, wishlist
Replace User A's object IDs with User B's IDs systematically across all 23 endpoints
Find that GET /api/v1/orders/{id} returns any order regardless of ownership (BOLA on read)
Find that PATCH /api/v1/addresses/{id} allows modifying any user's address (BOLA on write)
Find that GET /api/v1/users/{id}/payment-methods leaks payment card last-four digits for any user
Test batch endpoint POST /api/v1/orders/export - accepts array of order IDs and exports all without ownership check
Verify that DELETE /api/v1/orders/{id} correctly returns 403 for non-owned orders (authorization enforced)
