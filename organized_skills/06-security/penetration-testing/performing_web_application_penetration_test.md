...
url: https://skills.sh/mukul975/anthropic-cybersecurity-skills/performing-web-application-penetration-test
---

# performing-web-application-penetration-test

skills/mukul975/anthropic-cybersecurity-skills/performing-web-application-penetration-test
performing-web-application-penetration-test
Installation
$ npx skills add https://github.com/mukul975/anthropic-cybersecurity-skills --skill performing-web-application-penetration-test
SKILL.md
Performing Web Application Penetration Test
When to Use
...
Map the application through manual browsing and API documentation review, identifying 47 unique endpoints
Test authentication flows including social login (OAuth), standard login, and password reset
Discover IDOR vulnerability in the order retrieval API (/api/orders/{orderId}) where any authenticated user can view any order by iterating order IDs
Identify stored XSS in the product review feature that executes when administrators view the admin dashboard
Find SSRF in the product image import function that allows reading AWS EC2 instance metadata
Test payment logic by manipulating price values in the client-side cart before checkout submission
Report all findings with specific Node.js code-level remediation (parameterized queries, input sanitization with DOMPurify, authorization middleware)
