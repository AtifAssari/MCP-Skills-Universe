...
url: https://skills.sh/neo2025new/crypto-analyzer/crypto-analysis
---

# crypto-analysis

skills/neo2025new/crypto-analyzer/crypto-analysis
crypto-analysis
Installation
$ npx skills add https://github.com/neo2025new/crypto-analyzer --skill crypto-analysis
SKILL.md
...
请求间隔：每次 API 请求间隔至少 100ms
批量获取：优先使用 limit 参数一次获取足够数据，避免多次请求
数据量要求：K 线 limit 必须 ≥ 300（EMA(200) 需要至少 200 根 K 线，额外 100 根作为预热缓冲）
重试机制：遇到错误时使用指数退避重试
第1次重试：等待 1s
第2次重试：等待 2s
第3次重试：等待 4s
...
读取 crypto-analysis-log.md 中最近一次 4h 分析的 3 条路径
使用 15m K线数据验证短期价格行为是否符合路径预期
对每条路径标记状态：✅继续 / ❌淘汰 / 🆕新增
更新概率分配（淘汰路径的概率重新分配给存续路径）
记录概率迭代轨迹（如 55%→60%→65%）
...
读取 crypto-analysis-log.md 中最近 24h 的所有分析记录
构建多周期联动表（1W/1D/4H/1H 四级，带权重）
进行 Wyckoff 阶段判断（勾选格式）
回顾 24h 路径命中率
输出下一日关键价位和策略调整
...
第一幕（诱多）: 突破关键阻力，吸引追多盘入场
第二幕（杀多）: 快速下砸 3-5%，触发多头止损 + 杠杆清算
第三幕（杀空）: 底部 V 型反弹，逼迫空头回补
