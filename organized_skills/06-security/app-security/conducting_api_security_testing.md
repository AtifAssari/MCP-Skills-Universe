...
url: https://skills.sh/mukul975/anthropic-cybersecurity-skills/conducting-api-security-testing
---

# conducting-api-security-testing

skills/mukul975/anthropic-cybersecurity-skills/conducting-api-security-testing
conducting-api-security-testing
Installation
$ npx skills add https://github.com/mukul975/anthropic-cybersecurity-skills --skill conducting-api-security-testing
SKILL.md
Conducting API Security Testing
When to Use
...
Discover BOLA on /api/v1/accounts/{accountId}/transactions allowing any authenticated user to view any account's transaction history
Find mass assignment on the user update endpoint where adding "dailyTransferLimit": 999999 bypasses the configured transfer limit
Identify that the fund transfer endpoint lacks rate limiting, allowing unlimited transfer attempts without throttling
Discover that JWT tokens have a 30-day expiration with no refresh token rotation, enabling long-lived session hijacking
Find that the admin endpoint /api/v1/admin/users is accessible with a standard user token (BFLA)
Report all findings with CVSS scores and specific API code-level remediation guidance
