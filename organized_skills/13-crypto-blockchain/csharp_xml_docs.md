---
rating: ⭐⭐⭐
title: csharp-xml-docs
url: https://skills.sh/creator-hian/claude-code-plugins/csharp-xml-docs
---

# csharp-xml-docs

skills/creator-hian/claude-code-plugins/csharp-xml-docs
csharp-xml-docs
Installation
...
Key Principles
On-Demand Only: XML documentation is NEVER auto-applied. Only generate when explicitly requested
Keep it simple: Concise summaries for straightforward elements
Add context where needed: Complex concepts deserve detailed explanations in remarks
Think about your audience: Choose language (Korean/English/Mixed) that serves your team best
Be consistent: Follow established patterns and use consistent language throughout
Dual-Review Process: Claude-Haiku draft → Expert Review (Gemini/Codex) → Final Approval
Interface vs Implementation: Full docs in interface, <inheritdoc/> + implementation specifics in class
Document exceptions: Use <exception> for exceptions that are part of the method's contract
Property side effects: Use <value> tag when getter/setter have non-obvious behavior
POCU Naming: Use mPascalCase for private fields, camelCase for private methods
IDE Experience
