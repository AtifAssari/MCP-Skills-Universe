...
rating: ⭐⭐
title: x402
url: https://skills.sh/thegreataxios/agent-skills/x402
---

# x402

skills/thegreataxios/agent-skills/x402
x402
Installation
...
Types — PaymentRequirements, PaymentPayload, SettlementResponse, VerifyResponse (transport/scheme-agnostic)
Logic — Payment formation, verification, settlement (scheme+network specific, e.g. exact/EVM)
Representation — How payment data is transmitted (transport-specific: HTTP headers, JSON-RPC, etc.)
Payment Flow
Client requests a protected resource
Server responds with Payment Required + PaymentRequired JSON (contains accepts array)
Client picks a payment method, signs authorization, sends PaymentPayload
Server forwards to facilitator → verify → settle → returns resource + SettlementResponse
Three Actors
...
Check time window (validAfter/validBefore)
Match parameters to payment requirements
Simulate transferWithAuthorization tx
