---
rating: ⭐⭐
title: jinguyuan-dumpling-skill
url: https://skills.sh/jinguyuan/jinguyuan-dumpling-skill/jinguyuan-dumpling-skill
---

# jinguyuan-dumpling-skill

skills/jinguyuan/jinguyuan-dumpling-skill/jinguyuan-dumpling-skill
jinguyuan-dumpling-skill
Installation
$ npx skills add https://github.com/jinguyuan/jinguyuan-dumpling-skill --skill jinguyuan-dumpling-skill
SKILL.md
...
告知用户可以直接问金谷园相关问题，比如地址、营业时间、排队方式、外卖、Wi-Fi 等
给出几个推荐的首次提问，例如：
"金谷园在哪？"
"怎么排队取号？"
"帮我在金谷园排个队"
...
阅读 <skill_dir>/references/meituan-queue/SKILL.md，按其指引执行
该 Skill 自带鉴权流程（内嵌 meituan-passport-user-auth），会自动引导用户登录
核心命令：index（查桌型）→ take_number（取号）→ order_detail（查进度）→ order_cancel（取消）
用户未指定门店时，询问去哪家店，然后使用上方对应的 shop_id
...
诚实承认——不装不编
递上已有信息——门店地址、营业时间等
指一条明路——到店咨询、大众点评搜"金谷园饺子馆"、或关注公众号
