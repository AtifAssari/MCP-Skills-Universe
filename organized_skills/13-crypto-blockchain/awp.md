...
rating: ⭐⭐⭐
title: awp
url: https://skills.sh/awp-core/awp-skill/awp
---

# awp

skills/awp-core/awp-skill/awp
awp
Installation
$ npx skills add https://github.com/awp-core/awp-skill --skill awp
SKILL.md
AWP Registry
...
Most worknets are FREE to join. Worknets with min_stake = 0 require no staking at all. Always prefer these during onboarding. Only mention staking when the user specifically picks a worknet with min_stake > 0.
...
Always fetch contract addresses from the API before write actions — the bundled scripts handle this automatically via registry.get. Never hardcode contract addresses.
...
Do not use stale V1 names: no removeAgent(). Binding changes use bind(newTarget) or unbind().

Never ask the user for a wallet password, private key, seed phrase, or any secret. Ever. awp-wallet init is fully non-interactive — the wallet CLI generates keys internally and stores them securely. The complete wallet setup is:
...
Worknet skill install (Q6): Install awp-worknet skills directly. For third-party sources (not github.com/awp-worknet/*), show a warning and require user confirmation before installing.
...
Multi-chain awareness. Use chain-appropriate explorer links. Include chainId in API params when the user specifies a chain. Default to Base (8453) when unspecified.
...
Deallocate: python3 scripts/onchain-deallocate.py --token $TOKEN --agent <addr> --worknet 1 --amount 5000
Wait for the position lock to expire (check lockEnd timestamp)
Withdraw: python3 scripts/onchain-withdraw.py --token $TOKEN --position 1
...
pause / resume / cancel are the only actions a worknet owner can take on their own NFT. Guardian-only operations: activateWorknet, rejectWorknet, banWorknet, unbanWorknet, deregisterWorknet. End users cannot execute these.

Lifecycle states: Pending → Active ↔ Paused; Active → Banned ↔ Unbanned; Active → Deregistered (final). Cancel only works on Pending (refunds AWP escrow). Ban/unban and deregister are Guardian-only. Banned worknets have allocations frozen (staking.getFrozen). Deregister is irreversible.
