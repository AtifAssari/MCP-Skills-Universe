...
url: https://skills.sh/lucent-snow/style-extractor/style-extractor
---

# style-extractor

skills/lucent-snow/style-extractor/style-extractor
style-extractor
Installation
...
all files under references/9nine-visual-novel/guides/
all files under references/motherduck-design-system-reference/guides/
...
Do not start evidence collection, screenshotting, script execution, or drafting until both reference packages have been reviewed. Extraction is incomplete if the final deliverable does not explicitly state:
...
Motion assessment: does the site have meaningful motion? (determines whether Phase 1B is needed)
Reuse target: web app / marketing site / desktop app / design reference only
Primary reference package selected: 9nine / MotherDuck
...
name by intent (e.g., --color-accent, --motion-300, nav.switch.iconColor)
keep evidence alongside tokens (raw values + element/selector/screenshot)
...
Does every color token have a usage mapping? (not just a hex list)
Does every component have a state matrix with actual computed values?
Can someone reproduce the motion from the documentation alone?
Are all copy-paste examples self-contained (HTML + CSS in one block)?
If the original product disappeared, would this guide still be useful as a design reference?
Did the guide extract style without accidentally cloning the source product's information model?
Failure conditions
