---
rating: ⭐⭐⭐
title: synthesis-fact-checking
url: https://skills.sh/rajivpant/synthesis-skills/synthesis-fact-checking
---

# synthesis-fact-checking

skills/rajivpant/synthesis-skills/synthesis-fact-checking
synthesis-fact-checking
Installation
...
Verification by Source Hierarchy
Common Error Pattern Detection
Quote Verification
Study Verification
Temporal Verification (if backdated)
Documentation
...
For each paragraph, identify every claim that could be verified or falsified by checking a source.
Record each claim in a checklist (see the Documentation Template below).
Do not skip claims that "sound right" -- those are often the ones that are subtly wrong.
...
Primary sources -- The original journal paper, official report, press release, or direct statement. This is the standard against which all claims should be verified.
Authoritative secondary sources -- Reporting by established news organizations that cite primary sources. Useful when the primary source is paywalled or difficult to locate.
Tertiary sources -- Other blog posts, summaries, encyclopedia entries. Helpful for locating primary sources but never the final verification.
LLM training data / AI research outputs -- The lowest tier. These are what you are fact-checking, not what you fact-check against.
...
Verification Steps
Verify the quote exists. Search for the exact quote (or a distinctive phrase within it) using quotation marks to force exact-match results. If no results appear, the quote may be fabricated.
Verify exact wording. Compare the quote word-for-word against the primary source. Even small changes ("can" vs. "could," "will" vs. "may") can alter meaning.
Verify correct attribution. Confirm that the person named actually said or wrote the quote. Famous quotes are frequently misattributed.
Verify the context. Read the surrounding text in the original source to confirm the quote means what the draft implies.
Identify the source type. Note whether the quote comes from a published work, a media interview, a speech, a social media post, or a press release.
Red Flags
...
Verification Steps
Verify the study exists. Search by title, by authors, or by the institution that produced it. Use Google Scholar, the publisher's website, or the institution's publications page.
Verify the working paper or publication number. If a specific paper number is cited, confirm it matches the actual paper.
Verify ALL author names. Check first and last names of every listed author. AI outputs sometimes add, remove, or swap authors.
Verify the journal or publisher. Confirm the paper was published where the draft says it was.
Verify sample size and methodology. If the draft describes the study's methodology, confirm these details against the paper.
Verify the specific findings cited. Read the relevant section and confirm:
...
Check for retraction, correction, or supersession. Search Retraction Watch or the journal's errata page. Check if a newer version or follow-up has been published.
...
Rules
All cited sources must predate the article's stated publication date. If the article is dated October 2024, every source cited should have been published before October 2024.
External sources that postdate the article date require an explicit "Updated on" note.
Time-relative language must be accurate relative to the article date. Words like "recently," "this year," "last month," and "emerging" must make sense from the perspective of the article's stated publication date, not the actual writing date.
Do not cite preliminary data if final data was available by the article date.
