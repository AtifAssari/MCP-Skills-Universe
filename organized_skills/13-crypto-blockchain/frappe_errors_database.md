---
rating: ⭐⭐⭐
title: frappe-errors-database
url: https://skills.sh/openaec-foundation/erpnext_anthropic_claude_development_skill_package/frappe-errors-database
---

# frappe-errors-database

skills/openaec-foundation/erpnext_anthropic_claude_development_skill_package/frappe-errors-database
frappe-errors-database
Installation
$ npx skills add https://github.com/openaec-foundation/erpnext_anthropic_claude_development_skill_package --skill frappe-errors-database
SKILL.md
...
Critical Rules
ALWAYS
Use %(name)s dict params in frappe.db.sql() — NEVER string formatting
Check frappe.db.exists() before get_doc() — or catch DoesNotExistError
Handle DuplicateEntryError on every insert() call
Handle TimestampMismatchError on every save() in APIs
Call frappe.db.commit() in scheduler and background jobs
Paginate large queries — use limit parameter
Check get_value() result for None before using it
Use frappe.db.escape() in dynamic SQL strings
NEVER
Use string formatting (f"", .format(), %) for SQL values
Call frappe.db.commit() in controller hooks or doc_events
Catch bare Exception and pass — log or re-raise specific types
Assume db.set_value() succeeded — it fails silently on missing records
Expose raw database error messages to users — log details, show generic message
Run unbounded queries without limit — memory/timeout risk
