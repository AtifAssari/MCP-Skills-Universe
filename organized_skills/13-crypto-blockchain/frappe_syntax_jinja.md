---
rating: ⭐⭐⭐
title: frappe-syntax-jinja
url: https://skills.sh/openaec-foundation/erpnext_anthropic_claude_development_skill_package/frappe-syntax-jinja
---

# frappe-syntax-jinja

skills/openaec-foundation/erpnext_anthropic_claude_development_skill_package/frappe-syntax-jinja
frappe-syntax-jinja
Installation
...
Use get_formatted() for currency, date, and numeric fields
Use default() filter for optional/nullable fields
Pass parent doc to child row get_formatted("field", doc)
Use frappe.db.get_value() when you need only one field
Keep calculations in Python controllers, not Jinja templates
NEVER
Execute database queries inside loops (N+1 problem)
Use | safe on user-supplied input (XSS vulnerability)
Use Jinja syntax in Report Print Formats (they require JS {%= %})
Use frappe.get_doc() when frappe.db.get_value() suffices
Hardcode strings without _() translation wrapper
Disable safe_render without security review
