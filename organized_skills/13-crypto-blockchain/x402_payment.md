...
rating: ⭐⭐⭐
title: x402-payment
url: https://skills.sh/bofai/skills/x402-payment
---

# x402-payment

skills/bofai/skills/x402-payment
x402-payment
Installation
...
Activate a GasFree account that has not been activated yet. Use --gasfree-info first to check activation status. Defaults: network=nile, token=USDT.
...
Transfers activateFee + transferFee + 1 token from wallet to gasFreeAddress (on-chain TRC20)
...
The data is saved to a temporary file (e.g., /tmp/x402_image_...).
The tool returns JSON with file_path, content_type, and bytes.
