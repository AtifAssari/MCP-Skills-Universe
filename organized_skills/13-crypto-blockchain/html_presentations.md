...
url: https://skills.sh/ericmjl/skills/html-presentations
---

# html-presentations

skills/ericmjl/skills/html-presentations
html-presentations
Installation
$ npx skills add https://github.com/ericmjl/skills --skill html-presentations
SKILL.md
HTML Presentations
...
What It Does
Single file — All CSS, JS, SVG diagrams, and content in one .html file
Themed — Three color presets: terminal.css (default), catppuccin, nord
...
URL hash persistence — The current slide index is stored in the URL hash (e.g. #5). Refreshing the page keeps you on the same slide; sharing a link with a hash opens that slide directly
SVG diagrams — Flowcharts, architecture diagrams, pipelines, comparisons — all themed via CSS custom properties
SVG animations — Path drawing, sequential fade-in, pulse/glow, flow — triggered when a slide becomes active
How It Works
...
Main slides — the core content (definition, why, how, examples, etc.)
Acknowledgments — credit collaborators, tools, sources
Thank You — closing slide with links and a list of supplementary topics
Supplementary — detail slides, FAQ, reference material (linked from main slides and the thank-you slide)
...
Design guidelines
One idea per slide — avoid cramming. More slides is fine.
Large text — if a slide has little content, scale up. Use font-size: 2.5vw or larger.
Diagrams over bullet lists — when explaining a process, architecture, or flow, use an SVG diagram rather than a bulleted list.
Consistent spacing — use vh/vw units for padding and margins so spacing scales with the viewport.
Color hierarchy — --heading for titles, --fg for body, --muted for secondary text, --accent for emphasis.
Code blocks — use <pre><code> with the theme's --code-bg/--code-fg. Keep code short (< 15 lines per slide).
Python in slides — when including Python code, make it runnable with uv run script.py so it is self-contained and does not require external software environments. Use PEP 723 inline script metadata at the top of the script (# /// script, requires-python, dependencies, # ///) and state in the slide or speaker notes that the audience runs it with uv run script.py.
No external assets — everything inline. If an image is needed, use an inline SVG illustration rather than a raster image.
Fill the slide — avoid layouts where content clusters in one corner and the rest is empty space. Use two-column grids (grid-template-columns: 1fr 1fr) to balance text and diagrams side by side, with align-items:center to vertically center both columns.
Taglines over bullets for features — when spotlighting a feature or concept, replace bullet lists with a short tagline (2–3 short lines broken with <br>) and keyword pills beneath. This is punchier and more scannable.
Make SVGs the hero — when a slide has a diagram, give it at least half the slide width. Use generous viewBox dimensions and fill the space with detail: multiple nodes, labels, phases, wireframe lines, loop-back arrows. A tiny diagram floating in whitespace looks unfinished.
Weekly Installs
27
Repository
ericmjl/skills
