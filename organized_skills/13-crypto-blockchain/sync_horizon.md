...
rating: ⭐⭐⭐
title: sync-horizon
url: https://skills.sh/incept5/eve-skillpacks/sync-horizon
---

# sync-horizon

skills/incept5/eve-skillpacks/sync-horizon
sync-horizon
Installation
...
Distill the shipped capability into the reference doc:
...
Check for any system doc coverage: cd ../eve-horizon && cat docs/system/<name>.md (may not exist).
Read the CLI command source if relevant: cd ../eve-horizon && cat packages/cli/src/commands/<name>.ts
Create a reference doc that follows the conventions of existing references in eve-work/eve-read-eve-docs/references/.
Include: purpose, CLI commands/flags, manifest config if relevant, key behaviors, constraints.
...
Read the current eve-work/eve-read-eve-docs/SKILL.md.
For each new reference doc created by other workers, add:
...
Read the shipped plan(s) listed in your task for context on what changed.
List current CLI command modules:
cd ../eve-horizon && ls packages/cli/src/commands/*.ts

Scan the skill for stale claims:
...
Read the current .sync-map.json.
For each new reference doc created, add a reference_docs entry mapping the source files to the target.
For each new skill trigger relationship discovered, add a skill_triggers entry.
For removed source docs, clean up stale mappings.
Ensure watch_paths covers any new directories that should be monitored.
...
Run state-today and progressive-access checks:
