...
rating: ⭐⭐
title: gamekit
url: https://skills.sh/dpearson2699/swift-ios-skills/gamekit
---

# gamekit

skills/dpearson2699/swift-ios-skills/gamekit
gamekit
Installation
...
Set percentComplete from 0.0 to 100.0. GameKit only accepts increases; setting a lower value than previously reported has no effect.
...
Data modes: .reliable (TCP-like, ordered, guaranteed) and .unreliable (UDP-like, faster, no guarantee). Use .reliable for critical game state and .unreliable for frequent position updates. Register the local player as a listener (GKLocalPlayer.local.register(self)) to receive invitations through GKInviteEventListener. For programmatic matchmaking and custom match UI, see references/gamekit-patterns.md.
...
matchDataMaximumSize is 64 KB. Store larger state externally and keep only references in match data.
