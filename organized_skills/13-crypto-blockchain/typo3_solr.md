...
rating: ⭐⭐⭐
title: typo3-solr
url: https://skills.sh/dirnbauer/webconsulting-skills/typo3-solr
---

# typo3-solr

skills/dirnbauer/webconsulting-skills/typo3-solr
typo3-solr
Installation
...
EXT:tika Documentation
Version Matrix
GitHub: TYPO3-Solr/ext-solr
GitHub: main branch (14.0.x-dev)
Apache Solr Reference Guide
Solr Dense Vector Search
Solr Text to Vector (LLM)
typo3-solr.com
hosted-solr.com
ddev-typo3-solr
Mittwald Solr Docs
...
The Version Matrix targets TYPO3 14.3 + EXT:solr 14.0 + Solr 9.10.1 + ext_solr_14_0_0. Upstream development is on main:
...
Never hardcode Solr connection details. Use helhum/dotenv-connector for per-environment configuration:
...
See SKILL-SOLRFAL.md for complete file indexing setup.
...
The built-in suggest uses the devbridge/jQuery-Autocomplete library. For a jQuery-free approach, see SKILL-FRONTEND.md.
...
Prerequisites
Apache Solr 9.x (matrix per EXT:solr version guide) with the appropriate vector / language-model modules enabled (see Solr modules)
...
Current Solr text-to-vector documentation lists OpenAI, Mistral AI, Cohere, and Hugging Face models via LangChain4j support. Verify the exact provider set against the Solr version you deploy. See LangChain4j Embedding Models for model-specific parameters.
...
Note: Full native vector search support in EXT:solr (Index Queue -> vector fields, Fluid templates for vector results) is still evolving. Monitor the GitHub repository for updates.
...
Check that EXT:solr TypoScript templates are included on the root page:
...
Select your core from the dropdown (e.g., core_en)
Click "Query" in the left menu
Leave q as *:* (means "all documents")
Click "Execute Query"
Check numFound in the response:
...
Ask in Slack: TYPO3 Slack #ext-solr -- the maintainers (dkd) are active here
Search GitHub Issues: TYPO3-Solr/ext-solr/issues -- your problem might already be reported
dkd support: Professional support -- paid support from the extension maintainers
Collect info for a bug report: Solr version, EXT:solr version, PHP version, TYPO3 version, error messages from the log, Solr Admin query results
...
EXT:solrdebugtools adds visual debugging to the frontend:
...
Verify site config values: solr_host_read, solr_port_read, solr_scheme_read, solr_path_read
Click "Initialize connection" in the EXT:solr backend module after ANY config change
Test from CLI:
...
DDEV networking: Inside the container, Solr is reachable at hostname typo3-solr on port 8983. From the host, use <project>.ddev.site:8984 with HTTPS.
Firewall: Ensure port 8983 (or 8984 for DDEV) is open
HTTPS: DDEV uses self-signed certificates. Use solr_scheme_read: https with DDEV.
...
Re-indexing
Soft re-index: Backend module -> clear queue -> re-queue -> run scheduler task
Force re-index: Scheduler task "Force Re-Indexing of a site" for specific configurations
Nuclear option (DDEV only): TRUNCATE tx_solr_indexqueue_item; then re-initialize via backend module
...
Monitor Apache Solr security advisories
...
tx_solr.search Reference
tx_solr.index Reference
tx_solr.suggest Reference
Extension Settings
PSR-14 Events
...
EXT:solr
EXT:tika
TYPO3-Solr Organization
main branch (14.0.x-dev)
Docker Images
Apache Solr Server
Reference Guide
Dense Vector Search
Text to Vector (LLM Module)
Solr Modules
Solr 9.8 module Javadoc (historical llm path) — current Reference Guide uses the language-models module and org.apache.solr.languagemodels.* / solr.languagemodels.* class names; confirm against your Solr version's docs.
Tutorial: Vectors
Commercial & Hosting
typo3-solr.com
Changelog EXT:solr
...
hosted-solr.com
hosted-solr.com Pricing
dkd Enterprise Support
dkd Workshops
DDEV
ddev-typo3-solr Addon
DDEV Addons Directory
Mittwald
Solr Documentation
Environment Config
helhum/dotenv-connector
...
Community
TYPO3 Slack #ext-solr
GitHub Issues
TYPO3 Extensions (TER)
Blogs & Tutorials
Sease: Neural Search
Sease: KNN Benchmark
Sease: Semantic Search Text-to-Vector
Sease: SeededKnnVectorQuery (Solr 10)
...
The published Version Matrix lists EXT:solr 14.0 for TYPO3 14.3. Until that version is available as a stable Composer tag, upstream development tracks main (dev-main / 14.0.x-dev). Re-check Packagist and GitHub releases when you plan an upgrade.
