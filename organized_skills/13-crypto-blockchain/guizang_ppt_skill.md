...
url: https://skills.sh/op7418/guizang-ppt-skill/guizang-ppt-skill
---

# guizang-ppt-skill

skills/op7418/guizang-ppt-skill/guizang-ppt-skill
guizang-ppt-skill
Installation
...
template.html 是一个完整可运行的文件——CSS、WebGL shader、翻页 JS、字体/图标 CDN 全已预设好，只有 <main id="deck"> 里面是 3 个示例 slide（封面、章节幕封、空白填充页）。
...
打开 references/themes.md,找到对应主题的 :root 块
整体替换 assets/template.html(已拷贝版本)开头 :root{ 块里标有"主题色"注释的那几行(--ink / --ink-rgb / --paper / --paper-rgb / --paper-tint / --ink-tint)
其他 CSS 都走 var(--...),无需任何其他改动
...
大标题必须是衬线字体——如果显示成非衬线,99% 是 Step 3.0 预检没做,h-hero 类在 template.html 里缺失
图片网格里只用 height:Nvh,不用 aspect-ratio(会撑破)
图片不能堆到页面底部——不要用 align-self:end,用 grid + align-items:start(见 Step 3.2)
...
先读完 SKILL.md(这个文件)了解整体
Step 1 需求澄清完成后,读 themes.md 帮用户选定一套主题色
动手前 Read assets/template.html 的 <style> 块——这是类名的唯一来源,缺类会导致整页样式崩
读 layouts.md 挑布局(顶部有 Pre-flight 类名清单、主题节奏规划、动效 recipe 决策树)
如果在 Codex 中生成配图,读 image-prompts.md 挑图片类型、比例和基础提示词
细节调整时读 components.md 查组件(含 Motion 动效系统章节)
生成后读 checklist.md 自检(顶部 P0-0 规则强制预检 + 动效自检块)
