...
rating: ⭐⭐⭐
title: cc-design
url: https://skills.sh/zeroz-lab/cc-design/cc-design
---

# cc-design

skills/zeroz-lab/cc-design/cc-design
cc-design
Installation
$ npx skills add https://github.com/zeroz-lab/cc-design --skill cc-design
SKILL.md
...
Confirmed Facts — what is grounded in the brief, codebase, or references
Assumptions — unresolved fields converted into explicit assumptions
First Artifact — the first surface or deliverable to build
Variation Axes — what dimensions will change across directions, if any
Verification — how you will verify the final artifact

End with: Approve this plan, or tell me what to change before I build.

Default order of confirmation:

Design Context — existing design system / UI kit / codebase / screenshots?
Direction (new) — confirm own understanding before planning
Variations — how many directions? conservative vs wide exploration?
Fidelity & Scope — wireframe / hi-fi? one screen / one flow / full flow?
Plan Approval — approve the execution plan before full build
...
Step 1 — context → AskUserQuestion with header "Context", question "Do you already have a design system or screenshots I should match?", options:

"Yes, I have references" — brand guide, screenshots, UI kit, or design files to match
"No, work from scratch" — create a new design direction without references

Step 2 — direction confirmation → AskUserQuestion with header "Direction", question "I understand we're building a landing page (Stripe-style) for SaaS buyers, with sign-up as the core action. Is this correct?", options:

"Yes, proceed" — direction is correct, continue to next step
"No, adjust the type" — the output format needs to change
"No, adjust the audience" — the target audience needs to change

Step 3 — variations → AskUserQuestion with header "Variations", question "How broad should the exploration be?", options:

"1 direction" — close to expected, single focused design
"3 directions" — conservative → bold, multiple options to compare

Step 4 — fidelity/scope → AskUserQuestion with header "Scope", question "What should I build first?", options:

"One screen" — single high-fidelity screen
"One complete flow" — full user journey across screens
"Wireframe pass first" — low-fidelity structure before detailed design

Step 5 — plan Present the plan inline, then: → AskUserQuestion with header "Plan", question "Approve this plan, or tell me what to change before I build.", options:

"Approve" — proceed with the build
"Adjust scope" — change fidelity, screen count, or variation breadth
"Adjust direction" — change the design approach or assumptions
...
Output type — page / deck / clickable prototype / animation / design system / critique / export target
Task state — new task / localized edit / approved follow-up
Available context — design system / codebase / screenshots / brand reference / no reference
Interaction or delivery constraints — interactive / iOS / Android / PDF / PPTX / video / none
Primary design risk — layout / typography / color / information hierarchy / interaction / brand tone
...
User's codebase — read token files, 2-3 components, global stylesheet; copy exact hex/px values
User's published product — use Playwright or ask for screenshots
Brand guidelines / logo / existing assets
Competitor references — ask for URL or screenshot, don't work from memory
...
The first section is the minimum viable preview. If the user rejects direction here, zero code is wasted. Use tweaks for variants rather than separate files.
...
Show Phase 2 screenshot(s) — the rendered artifact after final edit
Present exit conditions results — checked items vs failed items from references/exit-conditions.md
...
On "Adjust" → re-enter Verify (Step 7), fix the failed item, re-run all phases. On "Rethink" → enter Iteration Gate in references/workflow.md.
