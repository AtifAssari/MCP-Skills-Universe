...
rating: ⭐⭐
title: search
url: https://skills.sh/exa-labs/exa-mcp-server/search
---

# search

skills/exa-labs/exa-mcp-server/search
search
Installation
$ npx skills add https://github.com/exa-labs/exa-mcp-server --skill search
SKILL.md
...
OAuth (recommended) — client opens auth.exa.ai, user signs in with Google / SSO / email, JWT is attached automatically. No key to copy.
API key — if OAuth isn't available, get one at https://dashboard.exa.ai/api-keys and pass it via Authorization: Bearer …, ?exaApiKey=…, or EXA_API_KEY (local npm).
Anonymous — works without setup but rate-limited.
