---
rating: ⭐⭐⭐
title: openclaw-maintainer
url: https://skills.sh/tyler6204/openclaw-maintainer-skill/openclaw-maintainer
---

# openclaw-maintainer

skills/tyler6204/openclaw-maintainer-skill/openclaw-maintainer
openclaw-maintainer
Installation
...
User: "review PR #2403"
Main agent: spawns opus subagent (high thinking) via sessions_spawn. Subagent reads commands/reviewpr.md and executes.
Opus subagent: sets up worktree, spawns 3 parallel sub-subagents (code analysis, CI/related scan, test coverage), combines results into structured review, saves .local/review.md and .local/related.md, pings back findings.
Main agent: summarizes for user (ready for prep, needs work, concerns)
User: "ok prep it" / "fix X first" / "don't merge"
Main agent: if approved, spawns gpt subagent (xhigh thinking) via sessions_spawn. Subagent reads commands/preparepr.md.
GPT subagent: runs /preparepr fully serially in one agent, installs dependencies, scans lint and review concerns, applies fixes, rebases, updates changelog/docs when needed, runs token-optimized gates, pushes, verifies push. Saves .local/prep.md.
User: "merge it"
Main agent: spawns gpt subagent (xhigh thinking) via sessions_spawn. Subagent reads commands/mergepr.md.
GPT subagent: verifies state, checks GitHub identity, merges via gh pr merge --squash, then spawns a single cleanup sub-subagent (close superseded PRs, close related issues, clean worktree). Pings back merge SHA.
Main agent: confirms to user with merge SHA
ALWAYS USE SUBAGENT
