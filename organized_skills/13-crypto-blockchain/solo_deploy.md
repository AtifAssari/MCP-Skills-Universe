...
rating: ⭐⭐⭐
title: solo-deploy
url: https://skills.sh/fortunto2/solo-factory/solo-deploy
---

# solo-deploy

skills/fortunto2/solo-factory/solo-deploy
solo-deploy
Installation
...
Check if CI matrix includes the required target (.github/workflows/*.yml)
Recommend adding cross-compile toolchain to CI, OR
Recommend replacing native dep with pure-Rust alternative, OR
Add the dep to CI install step: apt-get install / brew install
Report in deploy summary which deps may block CI
...
templates/stacks/{stack}.yaml — relative to this skill's plugin root
.solo/stacks/{stack}.yaml — user's local overrides (from /init)
Search via Glob for **/stacks/{stack}.yaml in project or parent directories
...
Run curl -s -o /dev/null -w "%{http_code}" against the deployment URL.
Verify HTTP 200 (not 404, 500, or redirect loop).
...
NEVER call AskUserQuestion — responses are lost between iterations, causing infinite retry loops (observed: 5 wasted iterations on OpenWok deploy)
Make autonomous deploy decisions:
...
Skip over blockers — browser login, manual approval, missing API keys → note in report, continue
Always signal completion — partial deploy (configs created + code pushed) is better than infinite retry. Output <solo:done/> even if actual deployment requires manual auth step.
Critical Rules
Use installed CLIs — detect vercel, wrangler, supabase, fly, sst before falling back to npx.
Auto-deploy aware — if platform auto-deploys on push, just push. Don't run manual deploy commands unnecessarily.
...
Check build locally first — pnpm build / uv build (or equivalent) before deploying.
Check production logs — always tail logs after deploy, catch runtime errors before declaring success.
Report all URLs — deployment URL + platform dashboard links.
Infrastructure in repo — prefer sst.config.ts or wrangler.toml over manual dashboard config.
Verify before claiming done — HTTP 200 from the live URL + clean logs, not just "deploy command succeeded".
Weekly Installs
22
Repository
