...
rating: ⭐⭐⭐
title: coderabbit
url: https://skills.sh/itechmeat/llm-code/coderabbit
---

# coderabbit

skills/itechmeat/llm-code/coderabbit
coderabbit
Installation
...
Show the user the exact command to authenticate: coderabbit auth login
Wait for the user to complete authentication before retrying
Do NOT attempt to run coderabbit auth login on behalf of the user — it requires interactive browser redirect
Quick Start
Run Review
...
Detect base branch: git symbolic-ref refs/remotes/origin/HEAD 2>/dev/null or fall back to main/master
Run CLI in background: coderabbit review --prompt-only --type uncommitted --base <branch> --no-color
Reviews take 7-30+ minutes — run in background (run_in_background=true)
Read output when process completes
