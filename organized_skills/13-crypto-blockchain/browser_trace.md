...
rating: ⭐⭐⭐
title: browser-trace
url: https://skills.sh/browserbase/skills/browser-trace
---

# browser-trace

skills/browserbase/skills/browser-trace
browser-trace
Installation
...
Sampler: a polling loop calls browse --ws <target> screenshot and browse --ws <target> get html body on an interval (default 2s). --ws is one-shot and bypasses the daemon, so it doesn't fight the main automation.
...
Don't poll faster than ~1s: each sample opens a one-shot CDP connection and screenshots Chrome. 2s is a good default.
...
Use --connect <session-id> for the automation client on remote, not a fresh browse env remote (which would create a new session each time).
