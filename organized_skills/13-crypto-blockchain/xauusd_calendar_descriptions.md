---
rating: ⭐⭐⭐
title: xauusd-calendar-descriptions
url: https://skills.sh/yiyousiow000814/xauusd-calendar-agent/xauusd-calendar-descriptions
---

# xauusd-calendar-descriptions

skills/yiyousiow000814/xauusd-calendar-agent/xauusd-calendar-descriptions
xauusd-calendar-descriptions
Installation
$ npx skills add https://github.com/yiyousiow000814/xauusd-calendar-agent --skill xauusd-calendar-descriptions
SKILL.md
...
Sentence 1 (always): Define what the event measures/decides using precise measurement terms:
rate, policy decision, index, diffusion index, weekly number, % change, balance, statement, minutes, auction
Sentence 2 (optional): Add only if the event can plausibly influence XAUUSD via major pricing channels.
...
Find at least one credible source describing the indicator/decision for that exact country/issuer.
Extract one short supporting excerpt (one line is enough).
Only then write the 1-2 sentence note in natural English.

Source preference order:

Official publisher (central bank / statistics office / ministry / exchange / treasury / agency).
Official press release / PDF / page for that indicator.
Reputable financial calendar / major news outlet as fallback when official sources are hard to access.
...
Each event still needs its own supporting excerpt (one line) and its own final note.
Efficiency comes from reusing official sources (not reusing wording).
...
Using the helper script to search and cache candidate pages (tmp/sources/).
Restricting searches to official domains with site: (for example, site:bls.gov, site:bea.gov, site:census.gov).
Reusing cached official "hub" pages for related variants (level, MoM, YoY) while still writing each event note individually.
...
Pick exactly one event id to work on (for example, USD::pce deflator::m/m).
Find at least one credible source line for that exact event (prefer official publisher pages).
Cache sources under tmp/ (gitignored), extract a one-line excerpt, and record it in tmp/event_notes_research_log.md (do not commit).
Write/replace the note in event_notes.json in natural English (1-2 sentences).
Run the repo quality gate (python scripts/calendar/build_event_notes.py) and rewrite if it flags anything.
...
Official PDF press release / methodology note (often easier to fetch than the main site).
Official API or machine-readable endpoint (JSON/CSV) if the institution provides it.
An official secondary domain or a machine-readable mirror that contains the same text verbatim.
Only if all official routes are blocked: a reputable secondary source (major financial calendar or major news outlet), and keep the note strictly definitional.
...
Banned phrases: never use tracks or growth indicator anywhere in notes.
Sentence limit: a simple punctuation heuristic counts . ! ? as sentence endings.
...
ASCII-safe text: avoid curly quotes (use ' instead of ’) so JSON stays clean and consistent.

Recommended workflow:

Identify the official publisher domain for the event (central bank, statistics office, exchange, etc.).
Find 1-3 "hub" pages that cover many indicators for that publisher (glossary, methodology, release landing page, catalog page).
Cache hub pages locally under tmp/ (gitignored) so you can extract excerpts offline for many events.
For each event, extract a one-line excerpt from cached content (or fetch a specific page if needed), then write the note.
Record evidence per event (URL + excerpt) in tmp/event_notes_research_log.md (do not commit).

Optional helper script:

Use scripts/calendar/research_event_note_sources.py to quickly collect candidate sources and definition-like excerpt lines for one event id.
The script does not change any JSON; it only caches pages under tmp/sources/ and prints copy-paste evidence candidates.
...
For PMI-style diffusion indexes, keep the threshold explanation when it is relevant and known (for example, 50 = expansion). Do not delete this detail just to shorten the note; it materially improves usability for non-specialists.
...
USD and (real) yields / rate expectations
Risk sentiment / safe-haven demand
Inflation expectations feeding into yields
