---
rating: ⭐⭐
title: symdex-code-search
url: https://skills.sh/husnainpk/symdex/symdex-code-search
---
...
Confirm the repo id.
If the repo id is already known, pass repo on every scoped tool call.
If the repo id is unknown, call list_repos and match the current worktree.
Check freshness with get_index_status(repo).
If the current worktree is not indexed, call index_folder(path=".").
If the workspace already has .symdex, treat it as the intended local SymDex state and reuse it.
Reuse the returned repo id for the rest of the task.
...
Index with index_folder if needed.
For broad feature, API, docs, or bug-investigation questions, start with build_context_pack.
For narrow lookups, start with search_symbols, semantic_search, or search_text.
Narrow to get_symbol or get_file_outline.
Check quality fields on returned items before making code-understanding claims.
Use get_callers, get_callees, search_routes, or get_repo_stats for deeper analysis.
Fall back to direct file reads only when SymDex cannot answer precisely enough.
Decision Guide
...
Start with the symptom, route, function, or text clue.
Use build_context_pack when the bug spans multiple symbols, routes, docs, or tests.
Retrieve the exact symbol, route, caller chain, callee chain, and relevant docs.
Inspect quality.index_fresh, quality.confidence, quality.parser_mode, quality.is_generated, and quality.has_embeddings.
If evidence quality is weak, say what is missing before claiming a bug.
Cite the concrete symbol, file, route, or call edge behind any bug hypothesis.
Editing
