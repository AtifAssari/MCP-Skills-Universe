...
rating: ⭐⭐⭐
title: vvvv-channels
url: https://skills.sh/tebjan/vvvv-skills/vvvv-channels
---

# vvvv-channels

skills/tebjan/vvvv-skills/vvvv-channels
vvvv-channels
Installation
...
Critical Rules
NEVER TryAddChannel — only use TryGetChannel (lookup-only)
Always retry-bind — channels appear after model initialization, not immediately
[CanBePublished(true)] required on .NET types for channel publication
Always dispose subscriptions — in the node's Dispose() method
System.Reactive.Unit for bangs — not float or bool
Spread.AsSpreadUnsafe — array must not be mutated after wrapping
Suppression flags — prevent feedback loops when writing to subscribed channels
