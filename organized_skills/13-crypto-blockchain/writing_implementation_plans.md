...
url: https://skills.sh/ed3dai/ed3d-plugins/writing-implementation-plans
---

# writing-implementation-plans

skills/ed3dai/ed3d-plugins/writing-implementation-plans
writing-implementation-plans
Installation
$ npx skills add https://github.com/ed3dai/ed3d-plugins --skill writing-implementation-plans
SKILL.md
Writing Implementation Plans
Overview
...
Identify which ACs this phase implements (look at design phase's "Done when" + component responsibilities)
Copy those AC entries literally into the phase's "Acceptance Criteria Coverage" header section
Ensure tasks produce tests that verify each listed AC case
...
Task NC: Research external dependencies (if phase involves them)
...
Write implementation tasks for this phase (in memory, not to file):
...
Present to user - Output the complete phase plan in your message text:
...
Task ND: Write phase file (if approved)
...
If needs revision: Revise based on feedback, present again (do NOT mark ND as in_progress until approved)
...
List ACs explicitly. Every functionality task specifies which AC cases it verifies in the "Verifies" field.

Describe tests, don't write test code. The AC text is the spec (e.g., "AC1.3: Invalid password returns 401"). Task-implementor generates test code at execution time with fresh codebase context.

Include implementation code when non-obvious. If implementation is complex or project-specific patterns apply, include the code. If it's straightforward given the AC description, describe it.

Specify test type and location. Unit, integration, or e2e? Which file? This ensures consistency across phases.
...
If you find yourself writing "this won't compile until Phase N+1": STOP. You are describing something that belongs in the current phase. Every phase must be executable with all tests passing when the phase completes.
...
Exact description of the blocking issue:
...
Context about why this blocks you:
...
Possible solutions you can see:
...
Review the gaps, misalignments, and issues identified

Fix ALL of them - Critical, Important, AND Minor

Update the relevant phase files

Mark each fix task complete as you address it

Re-run code-reviewer validation

If more issues found, create new individual fix tasks and repeat
...
Create a task for EACH revision (survives compaction):
...
Address each revision, marking tasks complete as you go
