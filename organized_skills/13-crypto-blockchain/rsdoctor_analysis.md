---
rating: ⭐⭐
title: rsdoctor-analysis
url: https://skills.sh/rstackjs/agent-skills/rsdoctor-analysis
---

# rsdoctor-analysis

skills/rstackjs/agent-skills/rsdoctor-analysis
rsdoctor-analysis
Installation
...
Build --filter field selections from reference/rsdoctor-data-types.md so field names match @rsdoctor/types instead of guessing from raw output.
...
Workflow
Locate or verify rsdoctor-data.json. If it is missing or plugin setup is needed, use the install references below.
Choose the command from reference/command-map.md. Start with list + query when possible; fall back to direct <group> <subcommand> when needed.
Before each data fetch, choose the minimal fields needed for the current question, map them with reference/rsdoctor-data-types.md, and pass --filter.
For common cases such as similar packages, media assets, bundle optimization, duplicate packages, and tree-shaking questions, use reference/common-analysis-patterns.md.
...
Raw data fields and --filter construction: reference/rsdoctor-data-types.md
Similar packages, media assets, bundle optimize, and common question patterns: reference/common-analysis-patterns.md
Response Format
