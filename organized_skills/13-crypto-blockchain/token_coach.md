...
rating: ⭐⭐
title: token-coach
url: https://skills.sh/alexgreensh/token-optimizer/token-coach
---

# token-coach

skills/alexgreensh/token-optimizer/token-coach
token-coach
Installation
...
Phase 0: Initialize
Resolve runtime and measure.py path (same as token-optimizer):
...
For Codex, check setup readiness:
...
If quality score < 70 in Claude Code: include "Set up Smart Compaction" as a recommended action (python3 $MEASURE_PY setup-smart-compact)
If quality score < 70 in Codex: include "Install balanced Codex hooks and compact prompt guidance" (TOKEN_OPTIMIZER_RUNTIME=codex python3 $MEASURE_PY codex-install --project .)
If quality score < 50: recommend immediate /compact or /clear before continuing
Flag which actions are quick wins vs deeper changes
Offer to run /token-optimizer for the full audit + implementation if they want to go beyond coaching
