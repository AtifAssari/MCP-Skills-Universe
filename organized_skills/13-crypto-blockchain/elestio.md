...
rating: ⭐⭐⭐
title: elestio
url: https://skills.sh/elestio/elestio-skill/elestio
---

# elestio

skills/elestio/elestio-skill/elestio
elestio
Installation
$ npx skills add https://github.com/elestio/elestio-skill --skill elestio
SKILL.md
Elestio Skill

Version: 2.0 Purpose: Deploy and manage services on Elestio DevOps platform Status: Ready to use Last Updated: 2026-02-19
...
Prerequisites
Create Account: https://dash.elest.io/signup
Verify Email: Check inbox, click verification link
Add Credit Card: https://dash.elest.io/account/payment
Wait for Approval: Usually instant, sometimes 24-48h
Create API Token: https://dash.elest.io/account/security -> Manage API Tokens -> Create Token
Configure skill: Give email + API token to agent
...
Golden Rules
Always authenticate first -- Every session starts with valid JWT
vmID != serverID -- Most endpoints use vmID, backup/notes use serverID (both from elestio services)
Check deployment status -- After deploy, wait for deploymentStatus = "Deployed" before accessing
Never delete without confirmation -- Always require --force flag
Use catalog when possible -- Phase 3 (catalog) is simpler than Phase 4 (CI/CD)
Validate combos -- Provider + datacenter + serverType must match elestio sizes
Account must be approved -- New accounts need credit card + approval before deploying
ALWAYS follow the Interactive Deployment Procedure above -- Never skip parameter questions
Set default project -- Use elestio config --set-default-project <id> to avoid passing --project every time
Service belongs to project -- When using elestio service <vmID>, ensure the vmID belongs to the current default project or specify --project
Defaults
...
Get credentials: elestio credentials <vmID>
Verify running: Open the URL, confirm service works
Enable backups: elestio backups auto-enable <vmID>
Add custom domain: elestio ssl add <vmID> myapp.example.com
Configure firewall: elestio firewall enable <vmID> --rules [...]
Enable auto-updates: elestio updates system-enable <vmID> --security-only
After Creating CI/CD Target

Automated (recommended):

Auto-create pipeline: elestio cicd create --auto --target <vmID> --name my-app --repo owner/repo --mode github --auth-id <id>
Site is live -- CLI handles SSH, Dockerfile, build, start automatically

Manual:

Add SSH key: elestio ssh-keys add <vmID> --name "name" --key "key"
Create pipeline: elestio cicd create pipeline.json
SSH and configure: ssh root@<ipv4>
Add domain: elestio cicd domain-add <vmID> --pipeline <pipelineID> --domain myapp.example.com
After an Error
Re-authenticate: elestio auth test
Check status: elestio service <vmID>
View logs: elestio cicd pipeline-logs <vmID> <pipelineID>
Restart stack: elestio restart-stack <vmID>
Troubleshooting
...
If > 10 minutes, check Elestio dashboard for errors
Contact support@elest.io if still stuck
...
SSH into CI/CD target: ssh root@<ipv4>
Check logs: cd /opt/app/<pipeline-name> && docker-compose logs
Verify docker-compose.yml syntax
Check port mapping: 172.17.0.1:3000 (internal network)
