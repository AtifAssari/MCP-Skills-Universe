...
rating: ⭐⭐⭐
title: team-edict
url: https://skills.sh/catlog22/claude-code-workflow/team-edict
---

# team-edict

skills/catlog22/claude-code-workflow/team-edict
team-edict
Installation
$ npx skills add https://github.com/catlog22/claude-code-workflow --skill team-edict
SKILL.md
Team Edict — 三省六部
...
coordinator	太子	roles/coordinator/role.md	(none)	-	接旨分拣、驱动流程
zhongshu	中书省	role-specs/zhongshu.md	PLAN-*	false	分析旨意、起草执行方案
menxia	门下省	role-specs/menxia.md	REVIEW-*	false	多维审议、准奏/封驳
shangshu	尚书省	role-specs/shangshu.md	DISPATCH-*	false	分析方案、派发六部
gongbu	工部	role-specs/gongbu.md	IMPL-*	true	功能开发、架构设计、代码实现
bingbu	兵部	role-specs/bingbu.md	OPS-*	true	基础设施、部署、性能监控
hubu	户部	role-specs/hubu.md	DATA-*	true	数据分析、统计、资源管理
libu	礼部	role-specs/libu.md	DOC-*	true	文档、规范、UI/UX、对外沟通
libu-hr	吏部	role-specs/libu-hr.md	HR-*	false	Agent 管理、培训、考核评估
xingbu	刑部	role-specs/xingbu.md	QA-*	true	代码审查、测试验收、合规审计
门下省 — 多 CLI 审议配置

门下省审议使用多 CLI 并行分析，同时从多个维度评估方案：
...
Specs Reference
文件	内容	使用方
specs/team-config.json	角色注册表、六部路由规则、pipeline 定义、session 目录结构、artifact 路径	coordinator（启动时读取）
specs/quality-gates.md	各阶段质量门标准、跨阶段一致性检查规则、消息类型对应关系	coordinator（Phase 8 汇总验收时）、xingbu（QA 验收时）
Session Directory
