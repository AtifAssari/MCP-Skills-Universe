...
rating: ⭐⭐⭐
title: configurator
url: https://skills.sh/uniswap/uniswap-ai/configurator
---

# configurator

skills/uniswap/uniswap-ai/configurator
configurator
Installation
...
Pack all uint64s together via encodePacked (concatenate bytes)
Return as hex bytes string with 0x prefix
Encoding Formula
// Solidity equivalent
...
Generate schedule via generate_supply_schedule MCP tool
Encode schedule via encode_supply_schedule MCP tool
Include encoded bytes in final configuration output
Pass to deployment script as auctionStepsData parameter
