...
rating: ⭐⭐⭐
title: plot-approve
url: https://skills.sh/eins78/skills/plot-approve
---

# plot-approve

skills/eins78/skills/plot-approve
plot-approve
Installation
...
Verify all plan PRs are non-draft or already merged
Merge each plan PR sequentially (step 3)
Create all implementation branches (steps 4-5)
...
Check for subsections: ### Tracer and ### Implementation
...
For each line starting with - \``: extract the branch name between backticks, extract the description after — `
...
Change **Phase:** Draft → **Phase:** Approved
Insert an ## Approval section immediately after the ## Status block:
## Approval
...
Change **Phase:** Draft → **Phase:** Approved
Insert an ## Approval section immediately after the ## Status block (same content as step 5):
## Approval
...
Update sprint file (if the plan has a Sprint: field): find the [<slug>] item in the sprint file and add an annotation comment:
...
Check out a branch and start implementing: git checkout <type>/<name>
When implementation is done: /plot-deliver <slug>
Alternative: work on multiple branches in parallel using worktrees
Weekly Installs
17
Repository
eins78/skills
