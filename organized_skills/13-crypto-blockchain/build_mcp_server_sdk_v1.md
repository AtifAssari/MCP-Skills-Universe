---
rating: ⭐⭐
title: build-mcp-server-sdk-v1
url: https://skills.sh/yigitkonur/skills-by-yigitkonur/build-mcp-server-sdk-v1
---

# build-mcp-server-sdk-v1

skills/yigitkonur/skills-by-yigitkonur/build-mcp-server-sdk-v1
build-mcp-server-sdk-v1
Installation
$ npx skills add https://github.com/yigitkonur/skills-by-yigitkonur --skill build-mcp-server-sdk-v1
SKILL.md
Build MCP Server (SDK v1)
...
Create McpServer instance with name, version, and optional description/icons
Define Zod schemas for each tool's input (and output if structuredContent is needed)
Register tools with server.registerTool() — input schema, annotations, async handler
Register resources with server.registerResource() if the server exposes data
Register prompts with server.registerPrompt() if the server provides templates
Create transport and connect: await server.connect(transport)
Handle graceful shutdown with process.on('SIGINT', ...)
