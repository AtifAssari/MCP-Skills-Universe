---
rating: ⭐⭐⭐
title: japanese-deep-translate
url: https://skills.sh/jingx8885/lovjpn/japanese-deep-translate
---

# japanese-deep-translate

skills/jingx8885/lovjpn/japanese-deep-translate
japanese-deep-translate
Installation
...
Read skeleton.json. Do NOT fill it 1-to-1. Apply the merge rules from "Splitting the input" to collapse the N LRC lines into ~(0.5·N) full-sentence study units, then write those directly to input.json:
...
Tell the user the path. On WSL they can open via explorer.exe $(wslpath -w <path>).
...
NetEase outer URL — fastest. Works for most Chinese pop and some international tracks. Fails (302→/404) for version-restricted titles (most YOASOBI, Ado, Vaundy, etc).
yt-dlp / SoundCloud (scsearch1:) — works reliably without auth; has most Japanese chart music. Default fallback.
yt-dlp / YouTube (ytsearch1:) — often triggers "sign in to confirm you're not a bot" from headless environments. Tried last.
None — skeleton.json.audio.source = "none". The page shows a fallback notice: "listen on tonzhon.com" link + instruction to drop a local MP3 as original.mp3.
