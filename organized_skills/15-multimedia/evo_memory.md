...
rating: ⭐⭐
title: evo-memory
url: https://skills.sh/evoscientist/evoskills/evo-memory
---

# evo-memory

skills/evoscientist/evoskills/evo-memory
evo-memory
Installation
...
Read current M_I from /memory/ideation-memory.md
Run the paper's IDE prompt (see above), reasoning through it step by step
...
Read /memory/ideation-memory.md and /memory/experiment-memory.md
Summarize relevant entries to inject into the current context
For research-ideation: Use M_I feasible directions to seed tree branches. Use M_I unsuccessful directions (fundamental failures only) during pruning.
For experiment-pipeline: Use M_E strategies to inform hyperparameter ranges, training schedules, and debugging approaches.
...
Read /memory/ideation-memory.md
Select the top-k_I=2 entries most relevant to the user's current goal. Compare the user's goal statement against each entry's Summary and Retrieval Tags for semantic similarity.
...
For each unsuccessful direction with Failure Classification: Fundamental: flag for pruning. Example injection: "Prior cycle confirmed 'Autoregressive real-time video generation' is a fundamental failure (O(n) latency). Prune any tree branch matching this pattern."
For experiment-pipeline (inject M_E)
Read /memory/experiment-memory.md
Select the top-k_E=1 entry most relevant to the current experiment domain. Compare the experiment's problem description against each entry's Context and Category.
