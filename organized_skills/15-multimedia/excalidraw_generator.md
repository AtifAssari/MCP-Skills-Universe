...
url: https://skills.sh/luongnv89/skills/excalidraw-generator
---

# excalidraw-generator

skills/luongnv89/skills/excalidraw-generator
excalidraw-generator
Installation
...
Diagram type — which visualization fits best (see references/diagram-types.md for the full catalog). If multiple types could work, present them as numbered options.
Key elements — list the nodes/shapes you'll include
Layout options — propose 2-3 layout choices for the user to pick from:
e.g., (A) Top-to-bottom flow, (B) Left-to-right flow, (C) Radial/centered
Style options — pick whatever feels most expressive for the diagram's purpose. No fixed palette or theme. Some dimensions to consider:
...
Estimated complexity — small (< 10 elements), medium (10-30), large (30+)
...
Every element in the elements array must have ALL of these fields — no exceptions: id, type, x, y, width, height, angle, strokeColor, backgroundColor, fillStyle, strokeWidth, strokeStyle, roughness, opacity, groupIds, frameId, roundness, isDeleted, boundElements, updated, link, locked, seed
...
Count the lines: line_count = text.split('\n').length
Calculate minimum text height: min_text_height = line_count * fontSize * 1.25
Calculate minimum shape height: min_shape_height = min_text_height + 40 (20px padding top/bottom)
The container shape's height must be >= min_shape_height
The container shape's width must be >= longest line's approximate pixel width + 20px padding
...
If still NEEDS_FIX and cycle < 3:
