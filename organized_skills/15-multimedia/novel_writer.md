...
rating: ⭐⭐⭐
title: novel-writer
url: https://skills.sh/wpsnote/wpsnote-skills/novel-writer
---

# novel-writer

skills/wpsnote/wpsnote-skills/novel-writer
novel-writer
Installation
$ npx skills add https://github.com/wpsnote/wpsnote-skills --skill novel-writer
SKILL.md
...
提问：向用户确认信息时，必须给出备选项，不让用户面对空白输入框
状态通知：告知正在执行什么操作（调用工具前一句话）、执行结果（章节写完的简报）
...
向用户提 3 个带备选项的问题（故事方向、主角起点、写作节奏）
根据回答 AI 自动推导所有设定（见"冷启动详细流程"）
依次创建设定笔记 → 生成人物图 → 写第一章 → 存档展示
模式 B：续写模式（已有笔记）
...
使用索引中的 note_id 访问笔记时，若返回 BLOCK_NOT_FOUND 或 NOTE_NOT_FOUND，则用 search_notes 重新定位，找到后立即更新 meta 索引
...
当场景在剧情中被改变或破坏（如房间被搜查翻乱、城门被攻破、密室被发现），不修改原始描述，而是在笔记末尾追加变更历史：
<h2>场景变更历史</h2>
<h3>第X章之后</h3>
...
默认图片尺寸：4:3，最大 2048×1536 速率限制：每 10 分钟最多 10 张 失败处理：生图失败（含频率超限）时，先在对应位置写 <p>（图待补）</p> 占位，继续后续流程，不阻塞创作。后续回到该笔记发现占位符时再补生即可，因为文字描述已经记录了所有细节。
...
字数约束：严格控制在 Meta 笔记中"章节目标字数"的 ±20% 以内（硬约束，不可忽略）
不穿帮：所有细节（地名、角色能力、人际关系）必须与设定笔记一致
风格一致：严格遵循 Meta 笔记中的叙述视角、语言风格描述
节奏控制：每章有起伏，不要全程平铺
结尾钩子：每章结尾留悬念或情绪钩子
多样性：不同章节可变换焦点、张弛节奏，避免雷同

写作执行流程：

在内部生成完整章节正文（绝不输出到对话）
立即调用 create_note + batch_edit 写入 WPS 笔记（见"归档 1"）
写入成功后，在对话中只告知用户：章节标题、字数、一句话剧情摘要、笔记已保存
随后自动完成归档 2–5（日志、角色状态、伏笔、进度）
写作后归档（每次必做，全自动）
...
search_notes({ keyword: 角色名/设定名, tags: ["#小说名"] }) 找出所有相关笔记
评估影响范围，告知用户（一句话说明）
确认后，批量更新相关笔记
...
在剧情日志中为跳过的章节预留标记：<p>[待写] 第X章 占位</p>
在 Meta 笔记"当前进度"后追加"跳写记录：第X章已写，第Y章待补"
正常写指定章节并存档
完整工具调用参考
续写序列（快速参考）
