...
rating: ⭐⭐
title: qb-downloader
url: https://skills.sh/therainstorm/my-agent-skills/qb-downloader
---

# qb-downloader

skills/therainstorm/my-agent-skills/qb-downloader
qb-downloader
Installation
$ npx skills add https://github.com/therainstorm/my-agent-skills --skill qb-downloader
SKILL.md
qb-downloader
...
从支持的站点列表中按照站点优先级选择一个站点进行尝试，如果失败则继续下一个站点
...
连载动漫可能没有全集，需要逐集下载
获取磁力链接
根据站点的获取磁链提示，获取种子的磁力链接
添加到 qBittorrent
qb_url: http://docker.op1:9090
如果用户没有指定分类，根据qBittorrent分类选择合适分类
对每个磁力链接执行：
...
介绍：以动漫资源为主的 BT 站
获取磁链提示：无需点开种子详情页，在搜索页面，磁力链接位于表格的 "磁鏈" 列里的向下箭头上（格式magnet:?xt=...）。右键复制地址。
