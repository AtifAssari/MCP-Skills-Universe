...
rating: ⭐⭐
title: tooluniverse
url: https://skills.sh/mims-harvard/tooluniverse/tooluniverse
---

# tooluniverse

skills/mims-harvard/tooluniverse/tooluniverse
tooluniverse
Installation
$ npx skills add https://github.com/mims-harvard/tooluniverse --skill tooluniverse
SKILL.md
ToolUniverse Router
...
Read ALL answer choices before reasoning — don't stop at the first plausible one
ELIMINATE clearly wrong options first (usually 2-3 can be ruled out immediately)
For the remaining options, reason through each: WHY would this be correct? WHY would this be wrong?
If you have two similar-sounding options, look for the KEY DIFFERENCE between them
Your final answer MUST be a single letter — nothing else
COMMON ERROR: reasoning correctly but reporting the wrong letter. After choosing, re-read your choice letter and verify it matches your reasoning.
MC traps: "All/None of the above" is correct only ~25% of the time — don't default to it. Options with absolute language ("always", "never", "only") are usually wrong. The longest/most detailed option is correct more often. When two options are opposites, one of them is usually correct.
Quantitative MC: When an MC question involves a number, COMPUTE the answer first (use Python), THEN match to the closest option. Do not let the listed choices bias your calculation.
For scored MC questions, run mc_analyzer.py (in skills/tooluniverse-computational-biophysics/scripts/) to enforce systematic elimination before committing to an answer.
...
For EACH question independently: read the question, identify the domain, invoke the relevant skill
NEVER carry assumptions from one question to the next
For EVERY MC question: write "Eliminating: [letter] because [reason]" for at least 2 options BEFORE giving your answer
For EVERY numerical question: write and execute Python code — report the computed result, not a mental estimate
For EVERY "what protein/gene/species" question: search a database FIRST, then answer
Between questions: mentally reset. Each question is independent.
