...
rating: ⭐⭐
title: focusavatar
url: https://skills.sh/lintqiu/focusavatar/focusavatar
---

# focusavatar

skills/lintqiu/focusavatar/focusavatar
focusavatar
Installation
$ npx skills add https://github.com/lintqiu/focusavatar --skill focusavatar
SKILL.md
数字人生成技能 (focusavatar)
...
注册账号：前往控制台（如 https://login.joycoreai.com/ 或部署方提供的地址）注册。
购买/开通：完成购买或开通流程。
创建密钥：在控制台创建密钥，复制 accessKeyId 和 accessKeySecret。
...
指令范围（When to use）
需要生成数字人视频（提供 MP3、MP4 地址与文字）。
需要查询数字人任务结果（已知 orderNo）。
...
前置条件：本机已安装 Node.js（含 npm），以便使用 npx。未安装可前往 https://nodejs.org/ 下载安装。
...
未安装请从 https://www.python.org/downloads/ 安装；Windows 安装时建议勾选「Add Python to PATH」。
...
使用流程
① 获取凭证：首次使用前前往控制台获取 accessKeyId、accessKeySecret；使用时按提示输入或提前设置环境变量。
② 选择模式：[1] 提交任务（生成视频）/ [2] 查询任务结果（需 orderNo）。
提交任务时：③ MP3 地址 → ④ MP4 地址 → ⑤ 文字内容 → 确认提交（⑥）；查询时输入 orderNo 即可。
详见上方「体验操作流程」。
隐私说明
本工具仅作客户端调用，不存储、不收集用户的 MP3/MP4 地址、文字内容或生成视频。
所有请求直接发往后端（由 FOCUSAVATAR_API 指定），数据处理与存储由该后端及所属方负责。
