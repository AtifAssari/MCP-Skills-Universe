---
rating: ⭐⭐⭐
title: zhy-article-illustrator
url: https://skills.sh/zhylq/yuan-skills/zhy-article-illustrator
---

# zhy-article-illustrator

skills/zhylq/yuan-skills/zhy-article-illustrator
zhy-article-illustrator
Installation
...
读取 article_path 的完整内容
解析文章结构：标题、各章节标题（## / ###）、段落数、代码块位置
识别核心信息点：
关键概念 / 术语解释 -> 适合信息图
对比 / 差异描述 -> 适合对比图
步骤 / 流程描述 -> 适合流程图
...
创建输出目录：{article_dir}/illustrations/{slug}/
...
先生成文章级 visual_bible，保存到 {article_dir}/illustrations/{slug}/visual-bible.md
visual_bible 必须覆盖：
...
保存到 {article_dir}/illustrations/{slug}/outline.md
同时为每张图生成独立提示词文件，保存到 {article_dir}/illustrations/{slug}/prompts/
...
图片更像编辑专题视觉，而不是普通插图
将所有提示词保存到 {article_dir}/illustrations/{slug}/prompts/ 目录
调用本技能内置脚本生成图片：
...
远程 key 格式：illustrations/{slug}/{filename}
逐张上传，记录每张的 CDN URL
上传失败时保留本地路径，不中断流程
...
复制 article_path 为 article.illustrated.md（同目录）
