---
rating: ⭐⭐⭐
title: macpilot-automation
url: https://skills.sh/adhikjoshi/macpilot-skills/macpilot-automation
---

# macpilot-automation

skills/adhikjoshi/macpilot-skills/macpilot-automation
macpilot-automation
Installation
...
Always focus the app before interacting: macpilot app focus "AppName" must come before clicking, typing, or menu operations. The first click after focus may be consumed by window activation - click the app's content area first, then click the target.

Use --json for parsing: Always add --json when you need to parse output programmatically.

Use ui find-text for coordinates: When you need to click a specific element, first find its coordinates with macpilot ui find-text "label" --app AppName --json, then click at the returned position.

Chain for complex sequences: Use macpilot chain for multi-step keyboard workflows instead of multiple separate commands.

AX value set over keyboard: Setting text field values via macpilot ui set-value is more reliable than keyboard typing when focus is uncertain.

Wait for elements: Use macpilot wait element before interacting with UI elements that may not have appeared yet.
