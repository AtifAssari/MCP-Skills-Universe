---
rating: ⭐⭐⭐
title: create-marp-deck
url: https://skills.sh/omerr/claude-skills/create-marp-deck
---

# create-marp-deck

skills/omerr/claude-skills/create-marp-deck
create-marp-deck
Installation
...
Base font sizes for section, table, blockquote, pre
Header styles for breadcrumb navigation (gray text, blue for active section)
Section classes for the title slide and each part/section divider - these get gradient backgrounds and white text
...
FORMATTING RULES
Sentence case for all headings — only capitalize the first letter and proper nouns (e.g., "Get started in 5 minutes", not "Get Started in 5 Minutes"). Breadcrumb headers follow the same rule
No trailing periods on bullets, text lines, or table cells
Single dash (-) for parenthetical separators, not double dash (--)
Backticks for program names, file names, and technical identifiers (not bold)
Bold for emphasis on key phrases and findings
Tables for data comparisons - keep them concise
Blockquotes (>) for key callouts or distinctions
One idea per slide - if a slide is getting dense, split it into two
Keep bullet lists to 3-5 items max per slide
No parenthetical notes in the slide body unless absolutely necessary
EXPORTING
