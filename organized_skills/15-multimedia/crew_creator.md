...
rating: ⭐⭐⭐
title: crew-creator
url: https://skills.sh/dtyq/magic/crew-creator
---

# crew-creator

skills/dtyq/magic/crew-creator
crew-creator
Installation
...
Employee name: Chinese and English (e.g. 研究助手 / Research Assistant)
Employee role: Chinese and English (e.g. 学术研究员 / Academic Researcher)
One-line description: What does this employee mainly do? (bilingual)
...
Show the generated files to the user via read_files
Ask if they want any adjustments
Clean up the temp config: delete_files(path=".crew_init_config.json")
...
编辑工作流
通用流程（适用于所有文件）
读取当前内容：使用 read_files 读取目标文件的现有内容
加载质量指南：读取 reference 文件 references/prompt-engineering-guide.md
加载格式规范：读取 reference 文件 references/crew-file-format.md
编写/修改内容：遵循格式规范和质量指南进行编辑
展示质量评估：向用户展示修改后的内容和质量评估摘要
用户确认后写入：用户确认无误后，使用 write_file 或 edit_file 写入文件
质量评估摘要格式
...
Read current content: Use read_files to read the target file's existing content
Load quality guide: Read reference file references/prompt-engineering-guide.md
Load format spec: Read reference file references/crew-file-format.md
...
Write after confirmation: After user confirms, use write_file or edit_file to save
