---
rating: ⭐⭐⭐
title: ue5-wwise-setup
url: https://skills.sh/koshimazaki/ue-audio-skills/ue5-wwise-setup
---

# ue5-wwise-setup

skills/koshimazaki/ue-audio-skills/ue5-wwise-setup
ue5-wwise-setup
Installation
$ npx skills add https://github.com/koshimazaki/ue-audio-skills --skill ue5-wwise-setup
SKILL.md
...
Wwise Side (4 steps)
Create AudioLink buses under Main Audio Bus
Create ActorMixer "AudioLink_Sources" via WAAPI
Create Sound SFX objects under ActorMixer (one per channel)
Set Audio Input plugin on each Sound (MANUAL in Wwise UI — WAAPI cannot create SourcePlugin)
# Create ActorMixer
...
Set the StartEvent property to the matching Wwise AkAudioEvent asset. Note: Wwise UE integration auto-generates AkAudioEvent assets from SoundBank metadata.
...
Key Classes
UWwiseAudioLinkSettings — StartEvent (TSoftObjectPtr)
...
Critical Gotchas
WAAPI cannot create SourcePlugin — Audio Input must be set manually in Wwise UI
Never edit .wwu XML files — not with Python ET, not with string replacement. Use WAAPI only
Delete macOS ._*.wwu files on external drives — Wwise tries to parse them as work units
clearAudioFileCache: true required when SoundBanks are empty/stale
onNameConflict: "merge" prevents duplicate creation errors
Paths use backslashes even on macOS: \\Busses\\Default Work Unit\\...
options: {} required in every WAAPI HTTP call (even empty)
SoundBank "Init" is auto-generated — don't include in generate call
return goes in options at top level, NOT in args
ForceReloadProject may show dialog — user must click through
SoundBank stale GUIDs — if events were deleted+recreated, old GUIDs persist in SoundBank inclusions. Fix: delete entire SoundBank, recreate, re-add inclusions fresh
WAAPI setInclusions with "replace" does NOT always update on-disk .wwu file — Wwise caches its own version
Audio Input source name doesn't matter — just select "Wwise Audio Input" plugin, naming is irrelevant
...
This is the exact sequence to build a complete Wwise project from scratch via WAAPI. Tested on Wwise 2025.1.5, UE 5.7.2, macOS.
...
ActionType: 1=Play for Play events, 2=Stop for Stop events. Wire each action's Target reference to the sound container.
...
Wwise writes its settings to [/Script/AkAudio.AkSettings] in DefaultGame.ini. Key fields:
...
For demos without AudioLink: Use EnableUnrealOnly to keep Lyra audio working, show Wwise project separately. For full integration: Use AudioLink (requires bWwiseAudioLinkEnabled=True).

Step 4: AudioLink Enable Flag

AudioLink is controlled by TWO settings:

AudioRouting=AudioLink in [/Script/AkAudio.AkSettings]
bWwiseAudioLinkEnabled=True in same section
...
DefaultGame.ini: RootOutputPath=(Path="../YourProject_WwiseProject/GeneratedSoundBanks")
Per-user settings (Saved/Config/MacEditor/EditorPerProjectUserSettings.ini): RootOutputPathOverride=(Path="/full/path/to/GeneratedSoundBanks")
...
Open Wwise Browser in UE (Window → Wwise Browser)
Select all Events → Right-click → Reconcile Selection
This creates .uasset files in Content/WwiseAudio/ that UE can reference
Without reconciliation, AkAmbientSound/AkComponent can't find events
...
Config goes in DefaultGame.ini — Wwise settings are [/Script/AkAudio.AkSettings] in DefaultGame.ini. Adding [/Script/AkAudio.AkSettings] to DefaultEngine.ini causes Audio Device Manager Initialization Failed
UE overwrites config on save — If you edit DefaultGame.ini while UE is open, UE may overwrite your changes on next save. Edit while UE is closed
AudioRouting enum is strict — Invalid values (like EnableWwiseAndAudioLink) cause FEnumProperty invalid value error and silently revert to default
AudioLink toggle grayed out — The UI toggle only works if bWwiseAudioLinkEnabled is set in config. Set it in DefaultGame.ini, not through UI
Wwise plugin NOT auto-discovered — Even with files in Plugins/Wwise/, must explicitly add to .uproject Plugins array
Wwise replaces UE audio completely — In EnableWwiseOnly mode, ALL UE native audio (MetaSounds, AudioComponents) goes silent. Lyra has no sound because it doesn't post Wwise events
Content Browser hides plugin content — Maps in GameFeature plugins (like L_ShooterPerf) don't appear unless "Show Plugin Content" is enabled. Use Ctrl+P to search instead
MetaSounds Presets are read-only — _Preset suffix assets inherit from parent Source. Cannot add nodes. Must duplicate the parent Source to edit the graph
EnableUnrealOnly for demo without AudioLink — Keeps all Lyra audio working while Wwise project exists for separate showcase
Two plugins required — Wwise (integration) + WwiseSoundEngine (SDK) both needed in .uproject. Missing either = no Wwise
Source Files
