...
rating: ⭐⭐⭐
title: searchnews
url: https://skills.sh/zrt-ai-lab/opencode-skills/searchnews
---

# searchnews

skills/zrt-ai-lab/opencode-skills/searchnews
searchnews
Installation
...
第二步：地毯式循环搜索
读取 prd.json 中处于 pending 状态的源。
每处理一个源，必须更新 prd.json 状态（pending → done/failed）。
每轮迭代必须写入 progress.txt，记录进度和失败原因。
严格校验日期，仅保留目标日期的内容。
抓取失败时 retry_count + 1，最多重试3次。
第二点五步：深度检索（重要！）
...
格式要点
每条新闻必须包含：编号标题、分类|来源|时间、摘要、详情要点（3-5条）
详情要点必须包含具体数据：金额、百分比、时间节点、技术指标等
分类标签参考：AI基础设施、AI产品、投融资、机器人、商业化、AI监管、行业观点、企业战略、AI能力、趣闻等
时间格式：精确到分钟（如 2026/1/19 15:28）
新闻数量要求：每日至少整理 10-20 条新闻，不得偷懒只抓几条！
第五步：确认完成
...
⛔ 输出铁律（违反即解雇！）
被剔除的新闻禁止输出！
只输出符合日期的新闻：最终日报中只能出现目标日期的新闻
剔除的不要提：因日期不符被剔除的新闻，禁止在任何地方输出或提及
不要显示剔除过程：不要告诉用户"我剔除了 xx 条"、"以下是被过滤的"等废话
静默过滤：日期校验是内部逻辑，用户只需要看到最终结果，不需要知道你筛掉了什么
简洁交付：只输出干净的、符合日期的新闻列表，没有任何多余说明
...
正确做法： 静默跳过不符合日期的新闻，只输出符合的，一个字都不要多说。
