---
rating: ⭐⭐⭐
title: brewcode:text-human
url: https://skills.sh/kochetkov-ma/claude-brewcode/brewcode:text-human
---

# brewcode:text-human

skills/kochetkov-ma/claude-brewcode/brewcode:text-human
brewcode:text-human
Installation
...
Parse: split args into scope (first token) and customPrompt (rest)
If customPrompt is present, prepend it to every sub-agent Task prompt as:
CUSTOM INSTRUCTIONS (highest priority, override defaults):
<customPrompt>

Custom prompt wins over default rules on conflict
Architecture
