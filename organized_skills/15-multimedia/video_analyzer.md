...
url: https://skills.sh/zrong/skills/video-analyzer
---
...
确认分析需求：明确用户想了解什么（如概括内容、回答问题、描述场景等）。如果 $ARGUMENTS 非空，将其作为分析提示词。
选择模型：默认使用 models.json 中的 default_model，用户也可指定。
执行分析：运行脚本（在 scripts/ 目录下执行）：
