...
url: https://skills.sh/getsentry/sentry-for-ai/sentry-otel-exporter-setup
---

# sentry-otel-exporter-setup

skills/getsentry/sentry-for-ai/sentry-otel-exporter-setup
sentry-otel-exporter-setup
Installation
...
Run which otelcol-contrib to check if it's on PATH, or check for ./otelcol-contrib in the project
If found, run the appropriate version command and parse the version number
Record the collector path (e.g., otelcol-contrib if on PATH, or ./otelcol-contrib if local) for use in later steps
...
Run uname -s and uname -m to detect OS and architecture
Map to release values:
...
Ask the user if they want to delete the downloaded tarball to save disk space (~50MB):
...
Docker Installation
Verify Docker is installed by running docker --version
Fetch the latest release tag from GitHub (same as above)
Pull the image using the numeric version (without v prefix):
...
Sentry org slug: In Sentry, go to Settings → Organization Settings → Organization Slug. This is also your subdomain (e.g., myorg in https://myorg.sentry.io)
Sentry auth token: Create an Internal Integration in Sentry:
...
Add the OTLPIntegration to the existing sentry_sdk.init() call, setting collector_url to the collector's OTLP traces endpoint:
...
Enable OTLP in the existing Sentry.init block, setting collector_url to the collector's OTLP traces endpoint:
...
Create an instrument file (instrument.mjs) that sets up OTel and Sentry together:
