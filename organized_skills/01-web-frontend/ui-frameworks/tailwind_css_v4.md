...
url: https://skills.sh/shamiul-anik/tailwindcss-skills/tailwind-css-v4
---

# tailwind css v4

skills/shamiul-anik/tailwindcss-skills/Tailwind CSS v4
Tailwind CSS v4
Installation
$ npx skills add https://github.com/shamiul-anik/tailwindcss-skills --skill 'Tailwind CSS v4'
SKILL.md
Tailwind CSS v4.2 — LLM Reference Guide

Current Version: 4.2.1 (February 2026) Tailwind CSS v4 is a ground-up rewrite. If your prior knowledge is based on v3, read the entire "Migration Pitfalls" section before generating any code.
...
CRITICAL: LLMs trained on older data will suggest deprecated v3 patterns. Always use the v4 patterns below.
...
Mobile-first: Start with unprefixed utilities, add sm:, md:, lg: for larger screens.
Use OKLCH colors: More vibrant gradients and perceptually uniform color scales. Prefer oklch() in @theme over hex or rgb.
Leverage CSS variables: Tokens from @theme are available as var(--color-brand-500) in custom CSS or inline styles.
Prefer semantic HTML: Use <header>, <main>, <nav>, <section>, <article>, <footer>.
Layer shadows: Stack shadow-*, inset-shadow-*, ring, and inset-ring for depth.
Use container queries: Prefer @container + @sm: over media queries for reusable components.
Use transition-* wisely: Add transition-colors, transition-transform, or transition-all with duration-* and ease-*.
Dark mode from the start: Always pair light classes with dark: variants.
Avoid arbitrary values when a dynamic utility exists (e.g., use grid-cols-13 not grid-cols-[13]).
Use @layer components for reusable component classes (cards, buttons, badges) — they stay overridable by utilities.
Weekly Installs
–
Repository
shamiul-anik/ta…s-skills
GitHub Stars
1
First Seen
–
