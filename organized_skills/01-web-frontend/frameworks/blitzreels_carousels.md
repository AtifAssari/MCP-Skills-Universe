---
rating: ⭐⭐⭐
title: blitzreels-carousels
url: https://skills.sh/blitzreels/agent-skills/blitzreels-carousels
---

# blitzreels-carousels

skills/blitzreels/agent-skills/blitzreels-carousels
blitzreels-carousels
Installation
...
Value: Core insight, numbered list, or reveal
CTA: "Save this" / "Follow for more" / engagement prompt ("Comment 1, 2, or 3")
5-Slide (Instagram)
Hook: Compelling title or question 2–4. Key Points: One actionable insight per slide
CTA + Recap: Summary + save/follow prompt
Storytelling (either platform)
Problem → 2. Struggle → 3. Discovery → 4. Result → 5. CTA
API workflow
...
POST /projects/{id}/media — upload each slide background image
POST /projects/{id}/timeline/media — place each image on the timeline
POST /projects/{id}/overlays — add text overlays (use "type": "text")
GET /projects/{id}/context?mode=timeline — verify layout
...
Build slides with background images only — skip text overlays in the API
Export ZIP of clean background images
Upload to TikTok as photo slideshow
Type text manually using TikTok's native text tool (don't paste)
This single detail can be the difference between getting pushed and getting shadowbanned

Path B — Convenience (fine for Instagram, acceptable for TikTok):

Build slides with text overlays baked in via the API
Export ZIP with text already rendered on images
Upload directly
Trade-off: slightly less algorithm favor on TikTok, but much faster at scale
...
Film grain — Removes the clean digital look. Use the backgrounds API:
...
Export at 1080p — Lower resolution blends with organic content. 4K screams "produced."

Compression round — After exporting the ZIP, upload each image to Telegram, download it, then upload to TikTok/Instagram. This strips EXIF metadata and adds compression artifacts that real user content has. The algorithm treats compressed images as more authentic.
