...
rating: ⭐⭐
title: seo
url: https://skills.sh/agricidaniel/claude-seo/seo
---

# seo

skills/agricidaniel/claude-seo/seo
seo
Installation
$ npx skills add https://github.com/agricidaniel/claude-seo --skill seo
Summary
...
Analyzes AI search readiness including GPTBot/ClaudeBot/PerplexityBot accessibility, llms.txt compliance, passage-level citability, and Generative Engine Optimization for AI Overviews
Generates prioritized action plans (Critical to Low) with unified SEO Health Score (0-100) and optional live SERP data via DataForSEO extension
SKILL.md
...
If Google API credentials detected (python scripts/google_auth.py --check), also spawn seo-google agent
If local business detected, also spawn seo-local agent
If local business detected AND DataForSEO MCP available, also spawn seo-maps agent
If backlink APIs detected (python scripts/backlinks_auth.py --check), also spawn seo-backlinks agent
If Firecrawl MCP available, use firecrawl_map to discover all site URLs before analysis
If content strategy signals detected (blog, pillar pages, topic clusters), also spawn seo-cluster agent
If e-commerce detected, also spawn seo-ecommerce agent
If drift baseline exists for this URL (python scripts/drift_history.py <url>), also spawn seo-drift agent
Always include seo-sxo in full audits (search experience applies to all sites)
...
Offer PDF report: "Generate a professional PDF report? Use /seo google report full"
...
seo-flow -- FLOW framework integration (Find → Leverage → Optimize → Win, 41 AI prompts, CC BY 4.0)
Subagents
