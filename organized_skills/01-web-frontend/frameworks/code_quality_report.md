...
url: https://skills.sh/khanhuitse05/my-ai-skill/code-quality-report
---

# code-quality-report

skills/khanhuitse05/my-ai-skill/code-quality-report
code-quality-report
Installation
...
Project structure: Read directory tree to identify layers, modules, and organization patterns.
Dependency files: Read package.json, pubspec.yaml, go.mod, requirements.txt, pom.xml, or equivalent to identify framework, language version, and dependencies.
Outdated & vulnerabilities: Run npm outdated / dart pub outdated / go list -u -m all and npm audit / govulncheck or equivalent when possible. If commands cannot be run, note the limitation and use "Unknown" in the report.
Test files & coverage: Look for test directories, test config (jest.config, vitest.config, pytest.ini, etc.), and coverage reports.
CI/CD config: Check for .github/workflows/, Jenkinsfile, .gitlab-ci.yml, bitbucket-pipelines.yml, etc.
Linting & formatting: Check for .eslintrc, biome.json, .prettierrc, analysis_options.yaml, etc.
Security: Scan for .env files committed to repo, hardcoded secrets/tokens, and auth configuration.
README & docs: Read README and any documentation files.
Models & schemas: Look for model/entity files, database migrations, and schema definitions.
Git & branching: Check branch protection rules, PR templates, and git workflow (if accessible).
...
Create Database Diagram (Backend only, or Frontend with DB):
...
Mobile (Flutter, React Native, etc.): features/, widgets|components/, package|lib/, utils/, config/, localization/, common/ — see report_template_frontend.md “Mobile” table.
...
Header: Project / Repo name, Tech stack (language, framework, main version), Assessment date, Report version, Assessed by, Standards, Platform (Frontend / Backend), and for Frontend Scope (Mobile / Web / Mobile & Web).
1. Executive summary: Table 7 criteria (+ optional Performance), score X/10, rating; total score and overall rating.
1b. Risks / Critical findings (optional): 3–5 bullets summarizing critical risks or issues (security, data loss, compliance) — include when applicable.
2. Codebase statistics: Table(s) directory + file count + description.
3. Dependencies, Libraries & Framework Review (required): Framework version; key libraries and versions; outdated count/table; upgrade suggestions (safe / suggest / major); technical risks (deprecated, EOL, incompatibilities); security and vulnerabilities (audit results, CVEs, fix recommendations).
4. Strengths: Bullet list, evidence-based; optional (Criterion: content). Frontend: include Accessibility (a11y) when assessable; both: Performance and CI/CD when applicable.
5. Areas for improvement: Bullet list, specific issues.
6. Recommendations: 🔴 High / 🟡 Medium / 🟢 Low with concrete actions.
7. Conclusion: Summarize score, main strengths, improvement priorities.
8. Action items (optional): Immediate, Short-term, Long-term.
9. Architecture & Database Diagrams (optional but recommended): Mermaid diagrams showing system architecture and database schema (when applicable). Automatically generated based on codebase analysis.
10. Project Governance & PR Standards (required): Single governance table summarizing PR standards, git workflow, security, and process health.
Output Format
Use report_template_backend.md or report_template_frontend.md for exact headings and tables.
...
Generate the report following the standard template.
Save as .md file: Write the full report to a markdown file (e.g. code-quality-report.md or {project-name}-quality-report.md) in the project root or a designated output directory.
Mermaid diagrams: Keep diagrams in Mermaid code blocks — they render natively in GitHub, GitLab, and most markdown viewers.
Ensure formatting:
...
Filename convention: {project-name}-quality-report-{YYYY-MM-DD}.md (e.g. my-app-quality-report-2026-02-09.md)
