---
rating: ⭐⭐
title: blitzreels-clipping
url: https://skills.sh/blitzreels/agent-skills/blitzreels-clipping
---

# blitzreels-clipping

skills/blitzreels/agent-skills/blitzreels-clipping
blitzreels-clipping
Installation
...
Ingest and wait for suggestions. Create the clip with selection_mode: "auto_best" and poll until source.suggestions_status = "ready". The first poll response with suggestions will include selection.alternatives.

Present suggestions to the user. Show the ranked list from selection.alternatives:

title, hook, start/end timestamps, duration, score
Let the user pick one, or provide their own time range.

If the user picks a different suggestion than auto_best selected, call POST /clips/{clip_id}/reselect with selection_mode: "suggestion" and the chosen suggestion_id.

If the user provides manual timestamps, call POST /clips/{clip_id}/reselect with selection_mode: "time_range" and the absolute start_seconds + end_seconds.

Continue the normal poll loop — follow next_action until export.

If the user says "just pick the best one" or doesn't express a preference, skip step 2 and let auto_best proceed.

Default Path
Create the clip with POST /clips.
Poll with GET /clips/{clip_id} while next_action = "poll".
When suggestions are ready, present them to the user for selection (see above).
If next_action = "reselect", pick another candidate from selection.alternatives or switch to a manual time range, then call POST /clips/{clip_id}/reselect.
If next_action = "repair", call POST /clips/{clip_id}/repair once.
If next_action = "export", call POST /clips/{clip_id}/export.
Keep polling until status = "exported", status = "failed", or next_action = "stop".
