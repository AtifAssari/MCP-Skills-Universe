---
rating: ⭐⭐⭐
title: claude-config-advisor
url: https://skills.sh/chujianyun/skills/claude-config-advisor
---

# claude-config-advisor

skills/chujianyun/skills/claude-config-advisor
claude-config-advisor
Installation
...
审查模式 用户已经有 .claude/、CLAUDE.md、CLAUDE.local.md 或相关配置文件。
设计模式 用户还没有配置，或明确表示想先规划再创建。
使用流程
...
.claude/ 目录的一级结构
.claude/skills/ 下被用户点名或最关键的 SKILL.md
.claude/agents/ 下与当前诉求相关的 agent 定义
.claude/commands/ 中具有代表性的命令模板
.claude/hooks/ 的配置文件和关键脚本
...
需要专门评估 CLAUDE.md 时：读 references/claude-md-guide.md
需要做结构化打分或分级建议时：读 references/review-rubric.md
