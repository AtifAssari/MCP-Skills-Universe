...
rating: ⭐⭐
title: hosted-agents
url: https://skills.sh/flora131/atomic/hosted-agents
---

# hosted-agents

skills/flora131/atomic/hosted-agents
hosted-agents
Installation
...
Git config in sandboxes: Missing user.name or user.email causes commit failures in background agents. Always set git identity explicitly during sandbox configuration, never assume it carries over from the image.
State loss on sandbox recycle: Agents lose completed work if the sandbox is recycled or times out before results are extracted. Always snapshot before termination and extract artifacts (branches, PRs, files) before letting the sandbox die.
