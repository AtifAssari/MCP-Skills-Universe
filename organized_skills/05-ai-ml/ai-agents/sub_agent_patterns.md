...
url: https://skills.sh/jezweb/claude-skills/sub-agent-patterns
---

# sub-agent-patterns

skills/jezweb/claude-skills/sub-agent-patterns
sub-agent-patterns
Installation
...
Configuration Notes
Tool access propagates: An orchestrator with Task can spawn any agent the session has access to
Model inheritance: Spawned agents use their configured model (or inherit if set to inherit)
Context isolation: Each spawned agent has its own context window
Results bubble up: Orchestrator receives agent results and can synthesize them
Advanced Patterns
...
Decide: retry that batch OR skip and continue
Don't let one failure block the whole operation
...
Check which agent's change is correct
Manually resolve or re-run one agent
Best Practices
Start with Claude-generated agents: Use /agents to generate initial config, then customize
Design focused sub-agents: Single, clear responsibility per agent
Write detailed prompts: Specific instructions, examples, constraints
Don't give Bash unless needed: Prevents approval spam (see "Avoiding Bash Approval Spam")
Put critical instructions FIRST: Instructions at top of prompt get followed, buried ones get ignored
Remove contradictory instructions: If you want Write tool, remove all bash examples
Default to Sonnet model: Quality matters more than cost savings (see Model Selection)
Version control: Check .claude/agents/ into git for team sharing
Use inherit for model sparingly: Better to explicitly set model for predictable behavior
