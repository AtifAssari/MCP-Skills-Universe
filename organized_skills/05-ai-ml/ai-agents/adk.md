...
rating: ⭐⭐⭐
title: adk
url: https://skills.sh/botpress/skills/adk
---

# adk

skills/botpress/skills/adk
adk
Installation
...
Run adk check --format json and adk status --format json.
If there are errors or warnings (unconfigured integrations, missing models, validation issues), proactively mention them before answering the user's question: "Before we start — I noticed [issue]. Want me to help fix that?"
Only do this once per session. Don't re-run on every question.
Available Commands
...
Use Bash tool to run the appropriate adk command
Parse and present the output to the user
Optionally suggest next steps (e.g., "Run adk add slack@3.0.0 to install")
...
Run adk status --format json to get the structured project overview
Read agent.config.ts for full configuration details
Follow the explanation patterns in references/explain-config.md
Produce a structured explanation covering metadata, models, integrations, state, and primitives
Flag any issues (unconfigured integrations, missing models, hardcoded secrets)
