...
url: https://skills.sh/fluxa-agent-payment/fluxa-ai-wallet-mcp/fluxa-agent-wallet
---

# fluxa-agent-wallet

skills/fluxa-agent-payment/fluxa-ai-wallet-mcp/fluxa-agent-wallet
fluxa-agent-wallet
Installation
...
User authorization flows via browser-based mandate signing; agent requests actions, user approves within defined scope
CLI-based with atomic unit amounts (USDC in 6 decimals), support for USDC, XRP, and FluxA Monetize Credits currencies
SKILL.md
FluxA Agent Wallet
...
Check for reusable mandates first. Before creating a new mandate, check both the current conversation context and ~/.fluxa-ai-wallet-mcp/mandates.json for existing signed, unexpired mandates that fit.
