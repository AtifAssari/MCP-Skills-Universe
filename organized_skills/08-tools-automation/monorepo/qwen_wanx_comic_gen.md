---
rating: ⭐⭐⭐
title: qwen-wanx-comic-gen
url: https://skills.sh/agentbay-ai/agentbay-skills/qwen-wanx-comic-gen
---

# qwen-wanx-comic-gen

skills/agentbay-ai/agentbay-skills/qwen-wanx-comic-gen
qwen-wanx-comic-gen
Installation
...
脚本工作流程
使用 wan2.6-t2i 模型创建异步图像任务(新协议,不再支持 style API 参数,改为在 prompt 里融入风格描述)。
周期性轮询任务状态,直到 SUCCEEDED 或超时(默认 5 分钟)。
解析返回的 choices[].message.content[] 结构,下载图片 URL 到本地目录。
打印 MEDIA: <绝对路径> 行,方便 OpenClaw 自动将图片附在回复中。
助手使用提示
