...
rating: ⭐⭐
title: meme-rush
url: https://skills.sh/binance/binance-skills-hub/meme-rush
---

# meme-rush

skills/binance/binance-skills-hub/meme-rush
meme-rush
Installation
...
Advanced filtering by bonding curve progress, liquidity, volume, holder distribution (top10%, dev%, sniper%, insider%), dev behavior (wash trading, sell%, burn status), and social presence
Topic Rush surfaces AI-generated market hot topics ranked by creation time, rising momentum, or viral status, with associated tokens sorted by net inflow
Both APIs return holder analytics, dev migration history, migration timestamps, and AI-generated narratives to support fast-trading decision workflows
SKILL.md
Meme Rush Skill
Overview
...
Notes
Only chainId and rankType are required; all other parameters are optional filters
Percentage fields (progress, holder %, dev sell %, tax rate) are pre-formatted — append % directly
taxRate for protocol=2001 (Four.meme) only shows on Migrated list; for protocol=2002 (Flap) shows on all lists
Icon URLs require prefix: https://bin.bnbstatic.com + path
Weekly Installs
3.8K
Repository
