---
rating: ⭐⭐
title: crushable-wingman
url: https://skills.sh/yxp934/crushable-wingman-skill/crushable-wingman
---

# crushable-wingman

skills/yxp934/crushable-wingman-skill/crushable-wingman
crushable-wingman
Installation
...
Initialize + load: Load user profile, the active crush profile, and long-term memory (snapshots + recent logs).
Classify intent: reply coaching / relationship analysis / date ideas / screenshot transcript extraction.
...
Read active_handle.txt (if present) and confirm the current target with the user.
Load:
user/profile.md, user/memory.md
crushes/<handle>/profile.md, crushes/<handle>/memory.md
If files are missing or incomplete: follow references/profile-intake.md and complete the profiles in multiple rounds (2-4 questions per round).
Reuse rule: Do not re-ask fields that are already filled unless the user explicitly updates/corrects them.
...
Append one crushes/<handle>/log/YYYY-MM-DD-HHMM.md
Logs are summary-only with small evidence snippets (do not store full raw chat transcripts)
...
Preserve the chronological order as shown in the screenshot(s).
Mark uncertain text as [unclear].
Label each message with Speaker = Me or Other based on layout cues (left/right, bubble color).
Output a structured transcript and ask the user to confirm or correct speaker/text.
