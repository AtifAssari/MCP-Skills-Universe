...
url: https://skills.sh/nesnilnehc/ai-cortex/generate-github-workflow
---

# generate-github-workflow

skills/nesnilnehc/ai-cortex/generate-github-workflow
generate-github-workflow
Installation
...
✅ 最小权限：在工作流程或作业级别将“权限”设置为场景类型所需的最低权限（CI：“内容：读取”；发布：“内容：写入”、“包：写入”）
✅ 堆栈对齐：Runner、语言版本、包管理器和命令与用户指定的堆栈匹配
✅ 写入前用户确认：列出必需的注释和占位符，并在写入 .github/Workflows/ 之前获得用户确认

验收测试：用户替换占位符后，除了秘密名称和环境特定值之外，工作流是否可以在目标存储库中运行而无需进一步修改？
...
窄触发器：on必须指定分支/路径/标签；避免每次推送时触发。常见模式：带有“branches”或“paths”的“push”/“pull_request”。 发布 工作流必须仅在版本标签上触发（例如push:tags:['v*']）并且存在于与 CI 不同的文件中。
最小权限：当工作流需要repo write、PR或Secrets时，将工作流或作业级别的“权限”设置为所需的最低权限；例如CI 内容：读取，释放内容：写入，包：写入；避免“全部”。
...
IN（输入）驱动
若存在 CLAUDE.md 或 .ai-cortex/config.yaml，优先读取其中的 test_command、base_branch 等；否则从用户输入或项目推断。参见 docs/guides/project-config.md。
使用用户的场景（例如“CI：在 PR 上运行测试”、“发布：在标签上构建和上传”）和 堆栈（语言、包管理器、测试/构建命令）来生成工作流；当信息丢失时使用合理的占位符并将其标记为替换；不要发明命令或路径。
交互（互动）
写入前确认：生成YAML后，列出必填注释（占位符、分支名称、用户必须设置的Secret名称），然后要求确认；不要写入 .github/工作流s/ 或在未经用户确认的情况下提交。
...
触发器：分支（例如main、develop）、路径过滤器、可选的工作流_dispatch。
...
限制（限制）
硬边界（Hard Boundaries）
不要违反附录A：输出必须有name、on、jobs，并且每个作业必须有runs-on和steps；不要使用未固定的第三方操作或硬编码的机密。
...
核心成功标准
 符合附录 A：输出满足附录 A 中的所有强制结构和安全需求（名称、工作、运行、步骤、固定操作、无硬编码秘密）
 窄触发器：on块的范围仅限于特定分支/路径/标签 - 没有没有过滤器的裸露on：push
...
以下是此技能生成的工作流文件的强制；使用本附录进行自检。

范围：此技能为项目的 .github/工作流s/ 生成的 YAML 工作流文件。

A.1 文件和路径
位置：必须位于目标项目的 .github/Workflows/ 下。
命名：kebab-case，扩展名.yml或.yaml；名称应反映目的（例如“ci.yml”、“pr-check.yml”、“release.yml”）。
...
上	是的	触发器：push、pull_request、工作流_dispatch等；必须缩小分支/路径/标签；避免没有过滤器的广泛“on:push”。
...
A.3 命名和可读性
作业 ID：kebab-case，含义明确（例如build、test、lint、deploy-preview）。
步骤名称：操作日志的简短、可扫描的描述。
...
制作工作流程后：
 ...
 包含name、on、jobs；每个作业都有“runs-on”和“steps”。
 on 缩小到特定分支/路径/标签。
...
CD（例如release.yml）：仅在版本标签的push上（例如v*）。发布图像和 GitHub 发布。
...
设置 Go (go.mod + 缓存)
去测试./...
govulncheck： go install golang.org/x/vuln/cmd/govulncheck@latest 然后 govulncheck ./...
Docker Buildx（仅设置，单一平台）
构建用于扫描的镜像：单架构linux/amd64、push: false、load: true、标签例如local/your-app:ci-${{ github.sha }}
对该图像的琐碎：severity: HIGH,CRITICAL、ignore-unfixed: true、exit-code: 1 所以 CI 在发现上失败
...
签出（获取深度：0）
设置 Go (go.mod + 缓存)
设置 QEMU： docker/setup-qemu-action, platforms: linux/amd64,linux/arm64
设置 Docker Buildx：id: buildx, driver: docker-container, platforms: linux/amd64,linux/arm64
登录 GHCR：docker/login-action，注册表ghcr.io，密码secrets.GHCR_TOKEN || Secrets.GITHUB_TOKEN, 注销：true
GoReleaser：goreleaser/goreleaser-action@v6，args：release --clean，envGITHUB_TOKEN和BUILDX_BUILDER：${{steps.buildx.outputs.name}}
...
GHCR：GoReleaser 配置中的图像路径和标记； 工作流仅登录并传递GITHUB_TOKEN和Buildx构建器。
Makefile：本地构建/测试可以保留； CI 步骤可以与 Make 目标保持一致，但不必依赖于它们。
B.5 编辑时
全流程：更换一项工作可能会影响整个流程；验证结账 → Go → QEMU → Buildx → 登录 → GoReleaser 订单和依赖项。
Action 版本：使用当前主要版本（例如 checkout@v4、setup-go@v5、setup-buildx-action@v3、goreleaser-action@v6）；升级时检查变更日志以了解重大更改。
Trivy：固定版本（例如@0.33.1）以避免行为变化导致 CI​​ 损坏。
YAML：检查缩进并且没有重复的键；编辑后使用工具进行验证。
B.6 经验教训

|问题 |方法| | ：------------------------------------------------ | :------------------------------------------------------------------------------------------------------------------------------------------------ | |单一工作流程太大 |分为 CI + Release：CI 用于构建/测试/扫描，仅通过 GoReleaser 在标签上发布；更清晰的权限和逻辑。 | | GHCR 身份验证太复杂 |使用最小登录（docker/login-action + token）；避免可能导致错误失败的繁重身份验证。 | |多架构清单验证失败 | 每个平台 拉取并验证，而不是通用清单拉取。 | |日期/版本格式不一致 |在工作流和 Dockerfile 中使用一种格式（例如 ISO8601）；如果使用 GoReleaser 输出，请将 dist/ 添加到 .gitignore。 | | GoReleaser 多架构构建失败 | GoReleaser 需要 Buildx 构建器：在 Buildx 步骤上设置 id: buildx 并传递 BUILDX_BUILDER: ${{ steps.buildx.outputs.name }}。 | |版本漂移 |使用合理的版本限制并在升级时检查发行说明；首先在分支上进行验证。 |
