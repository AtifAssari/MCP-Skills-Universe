...
url: https://skills.sh/ynz012x/skills/git-commit-generator
---

# git-commit-generator

skills/ynz012x/skills/git-commit-generator
git-commit-generator
Installation
$ npx skills add https://github.com/ynz012x/skills --skill git-commit-generator
SKILL.md
Git Commit Generator
...
流程概述
检查前置条件 - 确认暂存区有内容可提交
收集变更信息 - 获取暂存区 diff 和文件列表
推断 type 和 scope - 根据变更内容智能推断
生成并提交 - 生成中文消息后直接执行 git commit
...
9	以上均不匹配	chore
Scope 推断
提取所有变更文件路径（排除测试文件，除非全部为测试文件）
计算共同父目录，取最后一级目录名作为 scope
特殊处理：
monorepo（packages/、apps/）：取包名
src/ 前缀：去除前缀后重新计算
根目录配置文件：用文件名作 scope（如 docker、deps）
跨 3 个以上不同模块：省略 scope
scope 使用 kebab-case，长度不超过 30 字符
Breaking Change 检测

分析 diff 是否包含：删除公共函数/方法、修改 API 签名、移除导出接口。检测到时自动添加 ! 后缀和 BREAKING CHANGE: footer。
