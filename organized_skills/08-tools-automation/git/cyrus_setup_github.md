---
rating: ⭐⭐⭐
title: cyrus-setup-github
url: https://skills.sh/ceedaragents/cyrus/cyrus-setup-github
---

# cyrus-setup-github

skills/ceedaragents/cyrus/cyrus-setup-github
cyrus-setup-github
Installation
...
Set GITHUB_BOT_USERNAME to that username (the one users will type in @mentions, not <slug>[bot])
...
If claude-in-chrome MCP tools are available → use Path A-1
If agent-browser is installed and a Chrome debug session is connected → use Path A-2
Otherwise → use Path B (manual)

Path A-1 (claude-in-chrome):

Navigate to http://localhost:8976/github-app-manifest.html
Click the submit button to POST the manifest to GitHub
...
Open http://localhost:8976/github-app-manifest.html in your browser and click the button
Review the permissions on GitHub and click Create GitHub App
...
✓ GitHub CLI authenticated ✓ Git identity configured: <name> <email>

If webhooks were enabled:

✓ GitHub App created: <GITHUB_APP_SLUG> ✓ Webhook secret and app credentials saved to ~/.cyrus/.env ✓ Private key saved to ~/.cyrus/github-app.pem ✓ App installed (installation ID: <GITHUB_APP_INSTALLATION_ID>) ✓ Cyrus will respond to @<GITHUB_BOT_USERNAME> mentions in PR comments
