---
rating: ⭐⭐⭐
title: gitlab-mr-issue
url: https://skills.sh/dcjanus/prompts/gitlab-mr-issue
---

# gitlab-mr-issue

skills/dcjanus/prompts/gitlab-mr-issue
gitlab-mr-issue
Installation
$ npx skills add https://github.com/dcjanus/prompts --skill gitlab-mr-issue
SKILL.md
...
确保本地分支已推送且 git status 干净。
标题风格：英文、遵循语义化提交规范（如 feat(scope): short summary），保持简洁且描述核心目的；即使标题要求中文，语义化前缀（如 feat、fix）仍需英文。
描述风格：英文、短句和项目符号，优先让不看代码的读者快速理解动机、改动与验证方式。重点是 what/why/testing，避免流水账与过多实现细节。若上下文不足以明确目标或约束，应主动向开发者确认后再撰写。涉及专有名词、函数名、方法名、类名、API 名称或配置键时，使用 inline code（反引号）包裹以提升可读性与准确性。
默认正文格式：
