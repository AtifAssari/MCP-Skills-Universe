...
rating: ⭐⭐⭐
title: git-precommit
url: https://skills.sh/u9401066/med-paper-assistant/git-precommit
---

# git-precommit

skills/u9401066/med-paper-assistant/git-precommit
git-precommit
Installation
...
Copilot Hooks（寫作時即時修正）定義於 auto-paper/SKILL.md。 Pre-Commit Hooks（本檔，git commit 前最終把關）只報告不自動修改。

觸發：「commit」「推送」「做完了」「收工」「快速 commit」

Step 0: 偵測變更範圍

get_changed_files() 或 git diff --cached --name-only。 論文檔案模式：projects/*/drafts/*, projects/*/concept.md, projects/*/references/*, projects/*/.memory/* → 匹配時觸發 Paper Hooks。
