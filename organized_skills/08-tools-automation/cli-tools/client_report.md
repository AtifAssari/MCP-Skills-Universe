...
rating: ⭐⭐
title: client-report
url: https://skills.sh/indranilbanerjee/digital-marketing-pro/client-report
---

# client-report

skills/indranilbanerjee/digital-marketing-pro/client-report
client-report
Installation
...
Pull all metrics for the brand: Query connected MCP servers and run campaign-tracker.py --brand {slug} --action metrics to gather performance data across all active channels for the specified date range
Gather campaign history and execution log: Run execution-tracker.py --brand {slug} --action list --period {date_range} to compile all deliverables completed, campaigns launched, optimizations made, and tests concluded during the period
...
Format for delivery channel: Run report-generator.py --brand {slug} --format {channel} --type {report_type} to produce the channel-specific format (Slack blocks, email HTML, Google Sheets layout, or clean markdown)
Create approval checkpoint: Present the full report preview for review. Risk level: low. Require explicit approval before any external delivery — highlight any sensitive data, unexpected results, or negative findings that may need pre-briefing with the client
