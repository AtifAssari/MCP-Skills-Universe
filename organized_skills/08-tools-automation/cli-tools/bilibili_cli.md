...
rating: ⭐⭐⭐
title: bilibili-cli
url: https://skills.sh/jackwener/bilibili-cli/bilibili-cli
---

# bilibili-cli

skills/jackwener/bilibili-cli/bilibili-cli
bilibili-cli
Installation
...
Prefer --yaml first because it is usually more token-efficient than pretty JSON.
Use --json only when downstream tooling strictly requires JSON.
Keep result sets small with --max, --page, or --offset.
Prefer specific commands over broad ones. Example: use bili user-videos 946974 --max 3 --yaml instead of fetching large timelines.
...
Only fall back to --ai, comments, or audio extraction when subtitles are unavailable or clearly insufficient.
Prerequisites
