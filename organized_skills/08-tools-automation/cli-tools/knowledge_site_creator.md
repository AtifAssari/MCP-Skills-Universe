---
rating: ⭐⭐⭐
title: knowledge-site-creator
url: https://skills.sh/joeseesun/knowledge-site-creator/knowledge-site-creator
---

# knowledge-site-creator

skills/joeseesun/knowledge-site-creator/knowledge-site-creator
knowledge-site-creator
Installation
...
Generates 20-30 core concepts with AI-authored descriptions, examples, and quiz questions tailored to the topic
Implements four core learning modes: flashcards with keyboard shortcuts, progressive learn pages, knowledge quizzes, and searchable concept index
Creates topic-specific homepage copy, statistics, and footer text via AI analysis; no template copying, all pages generated from design system
Deploys to Vercel with built-in PWA support (offline access, installable), complete SEO meta tags, sitemap, and mobile-first responsive design
Enforces strict safety checks: validates data integrity, detects deployment conflicts across projects, auto-backs up before updates, and prevents GitHub repository sharing
SKILL.md
...
learn.html - 学习页（见core-patterns.md §5）

渐进式卡片展示
上一个/下一个导航
标记已掌握功能

flashcard.html - 闪卡页（见core-patterns.md §4）

卡片翻转动画
键盘快捷键（←→翻页，空格翻转）
进度显示

roots.html - 索引页（见core-patterns.md §7）

标题适配：${itemName}索引
搜索框 + 筛选器
卡片网格布局

progress.html - 进度页（见core-patterns.md §8）
...
css/minimal.css - 样式文件（见design-system.md）

统一设计系统
响应式布局

js/storage.js - 存储逻辑（见core-patterns.md §3）

LocalStorage 进度管理

manifest.json - PWA 配置（见pwa-setup.md §1）🆕

App 名称、图标、主题色
支持安装到主屏幕

sw.js - Service Worker（见pwa-setup.md §2）🆕
...
sitemap.xml - 网站地图（见seo-best-practices.md §4）🆕

列出所有页面URL
提交到搜索引擎

robots.txt - 爬虫指令（见seo-best-practices.md §5）🆕
...
✅ 禁止 GitHub 关联：默认不连接 GitHub，避免仓库共享
✅ 强制前置检查：部署前必须移除所有 Git 远程仓库
✅ 强制后置验证：部署后必须检查 projectId 唯一性
✅ 冲突自动检测：发现冲突立即报错，不允许继续
✅ 完整日志记录：所有部署操作记录到 /tmp/
...
生成 evolutionary-psychology-workshop（进化心理学），成功部署
生成 design-aesthetics-workshop（设计美学），成功部署
用户发现 word.qiaomu.ai（原词根词缀网站）显示的是进化心理学内容
检查发现 word-root-workshop 项目的 Git 仓库被进化心理学内容覆盖
...
强制前置检查：部署前自动移除所有 Git 远程仓库
强制后置验证：部署后检查 projectId 唯一性
冲突自动检测：遍历所有项目，发现相同 projectId 立即报错
完整日志记录：所有部署操作记录到 /tmp/
零容忍策略：任何检测到的冲突都不允许继续
...
扫描项目：自动扫描 /Users/joe/Dropbox/code/*-workshop
智能对比：使用 cmp 命令对比 CSS 文件，跳过已是最新版本的项目
安全备份：更新前自动备份旧 CSS 为 .backup 文件
Git 提交：自动 commit CSS 变更（commit message: chore: update CSS from skill template）
重新部署：调用 vercel --prod --yes 重新部署到生产环境
失败回滚：如果部署失败，自动恢复备份的 CSS
统计报告：输出更新统计（成功/跳过/失败项目数量）
使用示例
...
安全特性
智能跳过：自动跳过已是最新版本的项目，避免不必要的部署
自动备份：更新前备份旧 CSS 为 .backup 文件
失败回滚：部署失败时自动恢复备份
Git 记录：所有更新都有 Git commit，可追溯历史
演练模式：--dry-run 模式让你先看看会更新什么
注意事项
