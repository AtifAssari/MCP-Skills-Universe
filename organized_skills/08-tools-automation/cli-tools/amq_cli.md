...
rating: ⭐⭐⭐
title: amq-cli
url: https://skills.sh/avivsinai/agent-message-queue/amq-cli
---

# amq-cli

skills/avivsinai/agent-message-queue/amq-cli
amq-cli
Installation
...
When the user names something that could be a session, always run amq who --json first to check if it matches a known session name
If the name matches a session, use --session <name> on the send command
If it matches both a session and an agent handle, prefer the session interpretation when the user's phrasing implies a group/context ("on X", "in X", "the X team"), and the agent interpretation when it implies a person ("ask X", "tell X")
If the target session differs from your current session ($AM_ROOT basename), use --session <name>
Never guess — if the name doesn't appear in amq who --json output, tell the user (it may need coop exec --session <name> to initialize)
For cross-project routing (different repo), use --project instead — see Cross-Project Routing section
Messaging
