---
rating: ⭐⭐
title: weapp-devtools-e2e-best-practices
url: https://skills.sh/sonofmagic/skills/weapp-devtools-e2e-best-practices
---

# weapp-devtools-e2e-best-practices

skills/sonofmagic/skills/weapp-devtools-e2e-best-practices
weapp-devtools-e2e-best-practices
Installation
$ npx skills add https://github.com/sonofmagic/skills --skill weapp-devtools-e2e-best-practices
SKILL.md
weapp-devtools-e2e-best-practices
用途
...
服务端口已开启
目标 app 使用真实 AppID
同一个 e2e-app 在同一 suite 只启动一次 automator，并在 describe 级别共享。
多场景优先用 miniProgram.reLaunch(route) 切换，不要为了切页反复重启 DevTools。
断言优先页面级、结构级、可稳定复用的 runtime 收集器；截图验收放在路由稳定之后。
