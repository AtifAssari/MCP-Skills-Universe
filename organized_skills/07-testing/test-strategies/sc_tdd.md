...
rating: ⭐⭐⭐
title: sc-tdd
url: https://skills.sh/tony363/superclaude/sc-tdd
---

# sc-tdd

skills/tony363/superclaude/sc-tdd
sc-tdd
Installation
...
Initialize - Detect scope, framework, create state
RED: Write Failing Test - Create test for missing functionality
RED: Validate Failure - Run validator, confirm SEMANTIC failure (assertion, not compile error)
GREEN: Implement Minimally - Write smallest code to pass test
GREEN: Validate Passing - Run validators, confirm test passes + no regressions
REFACTOR: Improve Design - Clean up with tests as safety net
REFACTOR: Validate Stability - Re-run suite, confirm still green
State Machine
...
Help user write test file
Explain test must fail with assertion (not compile error)
When test is written, proceed to validation
...
Confirm current state is RED_CONFIRMED
Implement minimal production code
Explain your implementation approach
Transition to GREEN_PENDING:
python .claude/skills/sc-tdd/scripts/tdd_state_machine.py \
  --scope-root <scope> \
  --phase GREEN_PENDING \
...
If yes, transition to REFACTOR_PENDING
...
Specify manually: --framework pytest
Ensure framework config exists (pytest.ini, jest.config.js, etc.)
...
Fix test to compile and run
Add empty stub if needed: def my_function(): raise NotImplementedError
Re-run validate_red.py
"Full test suite timed out"

Solution:

Use --fast mode for faster cycles
...
Commit/stash other changes
Work on one test at a time
Use multiple micro-cycles
State Recovery

If state file is lost or corrupted:

Validator will attempt recovery from git diff + current test results
You can manually re-initialize: python tdd_state_machine.py --init --scope-root <path>
State is per-scope, so multiple packages can have independent TDD states
Important Reminders
ALWAYS run validators before phase transitions - This is non-negotiable
Trust validator output over assumptions - If validator says blocked, stop
Exit codes matter: Check both exit code AND "allowed" field in JSON
One test at a time - TDD is about smallest increments
Semantic failures only - Compile errors don't count as proper "red"
Full suite matters - Don't skip unless using --fast mode explicitly
