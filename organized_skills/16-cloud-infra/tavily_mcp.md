...
rating: ⭐⭐
title: tavily-mcp
url: https://skills.sh/darraghh1/my-claude-setup/tavily-mcp
---

# tavily-mcp

skills/darraghh1/my-claude-setup/tavily-mcp
tavily-mcp
Installation
...
If results are insufficient, try advanced search:
...
For protected sites or complex pages (tables, dynamic content):
...
Map the site first (URL structure only, no content):
...
Then crawl specific sections (full content):
...
Start with a search to gauge complexity:

tavily_search({ query: "...", search_depth: "advanced" })


If a single search isn't enough, use research:
...
Add the current year to your query: "topic 2026"
Use search_depth: "advanced" for better results
Use include_domains to target official docs or recent sources
Extract Returns Empty or Partial Content
Try extract_depth: "advanced" for JavaScript-rendered pages
Some sites block extraction — fall back to WebFetch or browser_navigate (Playwright MCP)
Check if the URL requires authentication — Tavily cannot access authenticated content
Crawl Produces Too Much Output
Reduce max_depth (default 1 is usually sufficient)
Reduce max_breadth (default 20 — try 5-10 for targeted crawls)
Use tavily_map first to identify the specific URLs you need, then tavily_extract those URLs directly
Research Takes Too Long

tavily_research makes multiple internal API calls. If it's too slow:

Use tavily_search with search_depth: "advanced" instead
Run multiple targeted tavily_search calls in parallel
Only use tavily_research when comprehensive synthesis is truly needed
Weekly Installs
27
Repository
