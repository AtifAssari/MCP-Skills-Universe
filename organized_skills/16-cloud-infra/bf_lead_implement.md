---
rating: ⭐⭐⭐
title: bf-lead-implement
url: https://skills.sh/khaki4/my_skills/bf-lead-implement
---

# bf-lead-implement

skills/khaki4/my_skills/bf-lead-implement
bf-lead-implement
Installation
...
When to Use
bf-lead-orchestrate가 에픽 루프에서 스폰
직접 호출하지 않는다.
Prerequisites
...
Error Handling
S/M Story agent 스폰 실패: 1회 재시도 후에도 실패 시 해당 Story를 stuck으로 처리 (status: in_progress 설정 후 stuck.md에 "agent spawn failed" 기록, ralph_stuck: true 설정). Done 신호에서 "done (stuck: {STORY-ID})" 목록에 포함하여 orchestrate가 skipped로 처리
...
docs/conventions.md를 읽는다.
수정 재실행인 경우: modification.md를 읽어 수정 지시와 대상 Story를 확인한다.
docs/sprint-status.yaml을 읽어서 각 Story의 난이도와 상태를 확인한다.
status: done인 Story는 건너뛴다.
status: todo 또는 in_progress인 Story만 대상으로 한다.
...
conventions.md에 concern-area 섹션이 없으면 전체 내용을 그대로 전달한다 (하위 호환).
Story의 Technical Notes에 변경 대상 파일이 없으면 전체 내용을 전달한다 (안전 fallback).
매칭되는 concern-area 섹션이 있으면, Core 섹션 + 매칭 concern-area 섹션만 추출하여 인라인으로 전달한다.
각 ## 헤딩부터 다음 ## 헤딩 전까지를 하나의 섹션으로 취급한다.
...
Story의 Technical Notes에서 "주요 라이브러리" 항목을 확인한다.
라이브러리가 명시되어 있으면, Story당 최대 3개까지 context7로 조회한다.
라이브러리가 명시되어 있지 않으면 이 단계를 건너뛴다.

context7 조회 절차 (라이브러리당):

resolve-library-id로 library ID를 확인한다.
query-docs로 Story의 AC에 관련된 API 문서를 조회한다.
...
조회 결과에서 해당 Story AC에 직접 관련된 API/패턴만 추출한다 (전체 문서 전달 금지).
...
이 Lead의 모델은 bf-lead-orchestrate가 스폰 시 지정한다:
...
.ralph-progress/{STORY-ID}.json이 존재하면 초기 retry_count/approaches_count를 해당 파일에서 읽어 전달한다 (이전 중단 복구 시).
Agent에게 전달하는 정보:
...
stuck.md를 docs/reviews/{STORY-ID}-stuck.md에 저장한다.
git commit하지 않는다 — docs/ 산출물은 Phase 4 Archive에서 일괄 커밋한다.
다른 Story들은 계속 진행한다 (stuck Story가 있어도 나머지를 중단하지 않음).
stuck Story의 status는 변경하지 않는다 — orchestrate가 자동 판단 시 skipped로 변경한다.
...
구현
Green 확인: 테스트 재실행 → 통과 확인
실패하면 아래 가드레일에 따라 재시도
리팩토링 (필요 시, Green 유지 확인)
git commit: [{TICKET}] {간단한 설명}
docs/ 하위 파일은 커밋에 포함하지 않는다 (sprint-status.yaml, story 문서 등 모두 제외)
Lead에 done 보고: "done" + commit hash + retry_count + approaches_count
Ralph Loop 가드레일

a) 최대 재시도 횟수: 5회

Green 검증 실패 → 구현 수정 → 재실행을 최대 5회까지 반복한다.
retry_count를 0부터 시작하여 매 실패마다 1 증가.

b) 반복 실패 감지 (Stuck Detection)

매 실패 시 에러 메시지의 핵심 내용(에러 타입 + 발생 파일)을 기록한다.
직전 시도와 동일한 근본 원인의 에러가 연속 2회 발생하면, 접근 방식을 전환한다:
테스트 코드의 기대값이 잘못되었는지 재검토
AC 해석이 올바른지 Story 파일 재확인
구현 전략을 근본적으로 변경 (다른 알고리즘/패턴 적용)
approaches_count를 0부터 시작하여 전환 시마다 1 증가.

c) 진행 상태 파일 기록 (크래시 복구용)

매 재시도 후 .ralph-progress/{STORY-ID}.json에 현재 상태를 기록한다:
{
  "retry_count": 2,
  "approaches_count": 1,
...
retry_count >= 5에 도달하면 루프를 즉시 중단한다.
stuck.md를 작성한다:
# Stuck 보고서: {STORY-ID}
...
retry_count, approaches_count를 함께 보고한다.
쟁점 해소 프로토콜 (L/XL Story의 Sub-Lead/Teammates에 적용)
Teammates 직접 대화: SendMessage로 직접 challenge/agree/보완
합의됨 → Sub-Lead에 결론만 보고
미합의 시 Sub-Lead 중재: 프로젝트 방향성(conventions.md) 기준으로 판단
Sub-Lead 결정으로 확정
그래도 미합의 → 버린다 (기록): 더 이상 토큰을 쓰지 않음
