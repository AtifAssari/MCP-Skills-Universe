...
rating: ⭐⭐⭐
title: copaw-ops
url: https://skills.sh/chujianyun/skills/copaw-ops
---

# copaw-ops

skills/chujianyun/skills/copaw-ops
copaw-ops
Installation
...
故障分流
服务无响应
先 /restart，再 copaw daemon reload-config
检查工作区是否正常：ls -la ~/.copaw/workspaces/
检查 agent 是否启动：copaw agent list
仍失败再按部署方式重启进程
配置错误
copaw daemon reload-config + copaw daemon logs -n 200
检查 agent.json 配置：cat ~/.copaw/workspaces/<id>/agent.json
必要时 copaw init --force
渠道断连
copaw channels list --agent-id <id> 检查配置
copaw channels config <channel> --agent-id <id> 查看详细配置
针对特定渠道：
...
/daemon restart 重启服务
模型调用失败
copaw models list --agent-id <id> 检查当前模型
copaw models config-key <provider> --agent-id <id> 检查 API key
copaw models set-llm --agent-id <id> 切换模型
定时任务不执行
copaw cron list --agent-id <id> 列出所有任务
copaw cron state <job_id> --agent-id <id> 检查任务状态
检查 jobs.json：cat ~/.copaw/workspaces/<id>/jobs.json
copaw cron resume <job_id> --agent-id <id> 恢复任务
copaw cron run <job_id> --agent-id <id> 手动执行测试
内置 Skills 故障（新增）
检查 skill 是否存在：ls ~/.copaw/workspaces/<id>/active_skills/
检查依赖项：
...
现象：用户侧症状 + 影响范围
诊断：执行过的命令与关键输出（注明 agent-id）
处理：已执行恢复动作
结果：当前状态是否恢复
建议：后续预防或观察项
多 Agent 环境注意事项
默认 agent：所有命令默认操作 default agent
指定 agent：使用 --agent-id <id> 参数指定
工作区隔离：每个 agent 有独立的工作区、配置、skills
批量操作：暂不支持批量操作所有 agent，需要逐个检查
日志查看：daemon 日志包含所有 agent，需要过滤查看
常见错误
Agent 不存在
...
检查 agent 列表：copaw agent list
创建新 agent：copaw workspace create <id>
工作区损坏
...
检查工作区：ls -la ~/.copaw/workspaces/
重新初始化：copaw init --force
内置 Skill 加载失败
...
检查 skill 目录：ls ~/.copaw/workspaces/<id>/active_skills/
检查依赖项：pip list
查看详细错误：copaw daemon logs -n 200
Weekly Installs
17
Repository
chujianyun/skills
