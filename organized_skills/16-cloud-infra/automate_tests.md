...
url: https://skills.sh/nesnilnehc/ai-cortex/automate-tests
---

# automate-tests

skills/nesnilnehc/ai-cortex/automate-tests
automate-tests
Installation
...
若存在 CLAUDE.md 或 .ai-cortex/config.yaml，优先读取其中的 test_command；否则按以下来源发现。参见 docs/guides/project-config.md。
...
识别：
主要测试入口点（npm test、pnpm test、yarn test、pytest、tox、go test、dotnet test、mvn test、gradle test、cargo test等）
测试层和标记（单元、集成、e2e）
环境先决条件（DB、Redis、Docker Compose、所需的环境变量、秘密）
CI 如何设置依赖关系（服务、缓存、产品）
与启发式方法相比，更喜欢在文档或 CI 中找到明确的说明。
...
检查 package.json 脚本和 .github/Workflows/*。
确定模式“fast”并提出：
...
解析 .github/工作流s/ci.yml 并识别单独的作业：
后端单元测试
前端测试
