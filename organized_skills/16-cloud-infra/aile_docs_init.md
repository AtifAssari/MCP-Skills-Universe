---
rating: ⭐⭐⭐
title: aile-docs-init
url: https://skills.sh/zhucl1006/ailesuperpowers/aile-docs-init
---

# aile-docs-init

skills/zhucl1006/ailesuperpowers/aile-docs-init
aile-docs-init
Installation
$ npx skills add https://github.com/zhucl1006/ailesuperpowers --skill aile-docs-init
SKILL.md
Project Docs Setup
...
测试现状与缺口
模式 B 除 PRD.md、SAD.md 外，强烈建议额外生成
docs/specs/CODEBASE-ANALYSIS.md 用于承接代码仓地图、运行时入口、接口盘点、数据模型盘点、配置矩阵、技术债清单
禁止使用“若干”“一些”“负责某类逻辑”之类的空泛描述，必须尽量写出路径、接口名、数据结构、调用关系和实现状态
对于无法确认的内容，必须集中记录在“待确认事项”中，不能把猜测写成事实
...
使用 Explore agent 分析每个模块
先列出模块对应的目录与核心文件
提取模块的导出接口
分析模块间的依赖关系
补一条关键调用链或 Mermaid 时序
记录配置项、测试现状和技术债
总结模块的核心职责和改造建议
4.7 生成数据库文档
...
查找 ORM 模型文件（Prisma schema, TypeORM entities, Django models 等）
提取实体定义和关系
查找数据库迁移文件
生成实体关系图（文本描述或 Mermaid 图）
4.8 生成 API 文档
...
查找路由定义文件（Express routes, NestJS controllers, Django urls 等）
提取所有 API 端点
分析每个端点的参数和返回值
提取认证和授权逻辑
生成 API 文档（可以是 OpenAPI/Swagger 格式）
...
提供价值：不只是记录，更要分析和建议
分析原则（模式 B 专用）
准确性优先：文档必须准确反映代码实际实现
标注来源：明确标注信息来源（代码提取 vs 用户补充 vs 推断）
识别差异：指出代码与最佳实践的差异
记录债务：记录技术债务和改进建议
证据可追溯：关键结论尽量绑定到具体文件/目录/接口
详单优先于概括：模块文档必须包含文件级详单，而不是只有职责描述
状态显式化：标记“已实现 / 部分实现 / 待确认 / 推断”
质量原则
完整性：文档内容完整，覆盖关键信息
准确性：基于用户提供的信息或代码实现，不臆测
一致性：文档与实际情况（需求或代码）保持一致
可读性：结构清晰，表达简洁
可维护性：便于后续更新和维护
使用工具指南
