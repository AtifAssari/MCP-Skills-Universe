---
rating: ⭐⭐⭐
title: building-qt-apps
url: https://skills.sh/quick-brown-foxxx/coding_rules_python/building-qt-apps
---

# building-qt-apps

skills/quick-brown-foxxx/coding_rules_python/building-qt-apps
building-qt-apps
Installation
...
Never block event loop: no subprocess.run(), no time.sleep(), no sync HTTP
qasync bridges asyncio and Qt event loops
ThreadPoolExecutor wraps blocking third-party APIs
Typed wrappers around untyped libraries, enforced via ruff banned-api
Signals at class level, not in __init__
camelCase for Qt event handlers (ignore ruff N802), snake_case for our slots
Ctrl+C and Shutdown
