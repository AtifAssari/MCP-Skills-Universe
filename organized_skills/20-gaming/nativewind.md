...
rating: ⭐⭐
title: nativewind
url: https://skills.sh/dralgorhythm/claude-agentic-framework/nativewind
---

# nativewind

skills/dralgorhythm/claude-agentic-framework/nativewind
nativewind
Installation
$ npx skills add https://github.com/dralgorhythm/claude-agentic-framework --skill nativewind
SKILL.md
NativeWind
Overview
...
Initial setup:
 ...
 Install: pnpm add nativewind tailwindcss@3
 Create tailwind.config.js with content paths pointing to app/, components/, etc.
 Create global.css with @tailwind base; @tailwind components; @tailwind utilities;
 Update babel.config.js: add nativewind/babel preset
 Update metro.config.js: wrap with withNativeWind
 Create nativewind-env.d.ts with /// <reference types="nativewind/types" />
 Import global.css in root _layout.tsx

Styling a component:

 Import RN component (View, Text, Pressable, etc.)
 Apply className prop with Tailwind utilities
 Use contentContainerClassName for ScrollView/FlashList inner styling
 Test on device/simulator to verify rendering
Guidance
Configuration Files
