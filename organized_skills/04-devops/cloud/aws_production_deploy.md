---
rating: ⭐⭐⭐
title: aws-production-deploy
url: https://skills.sh/aiagentwithdhruv/skills/aws-production-deploy
---

# aws-production-deploy

skills/aiagentwithdhruv/skills/aws-production-deploy
aws-production-deploy
Installation
$ npx skills add https://github.com/aiagentwithdhruv/skills --skill aws-production-deploy
SKILL.md
...
NAT Gateway — ECS Fargate in private subnet can't pull ECR images without NAT or VPC endpoint. Either add NAT Gateway ($32/month) or create ECR VPC endpoints (free but more setup).

ECS Task Role vs Execution Role — Execution Role = ECS agent pulls images + writes logs. Task Role = your application's AWS permissions (S3, SES, etc.). These are DIFFERENT.

MongoDB Atlas Network Access — Whitelist the NAT Gateway's Elastic IP in MongoDB Atlas, not 0.0.0.0/0.

Health Checks — ALB health check must return 200 on / (frontend) and /health (backend). If unhealthy, ECS keeps killing and restarting tasks.

Secrets Management — Never put secrets in task definition JSON. Use AWS Secrets Manager or SSM Parameter Store, reference them in task def with valueFrom.

Container Insights — Enable for ECS to get per-container metrics in CloudWatch:

aws ecs update-cluster-settings --cluster <app>-cluster --settings name=containerInsights,value=enabled


Auto-scaling — Add after initial deploy:
...
Cost Control — Fargate pricing: 0.04048/vCPU/hr + 0.004445/GB/hr. A 0.5 vCPU + 1GB task = ~$30/month. 2 tasks = ~$60/month. NAT Gateway adds $32/month.

Logs Retention — Set retention on CloudWatch log groups or costs grow forever:

aws logs put-retention-policy --log-group-name /ecs/<app>/backend --retention-in-days 30


Rolling Deployments — ECS default is rolling update. Set minimumHealthyPercent=50 and maximumPercent=200 to allow new tasks before killing old ones.
