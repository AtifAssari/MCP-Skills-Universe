---
rating: ⭐⭐⭐
title: alibabacloud-bailian-rag-knowledgebase
url: https://skills.sh/aliyun/alibabacloud-aiops-skills/alibabacloud-bailian-rag-knowledgebase
---

# alibabacloud-bailian-rag-knowledgebase

skills/aliyun/alibabacloud-aiops-skills/alibabacloud-bailian-rag-knowledgebase
alibabacloud-bailian-rag-knowledgebase
Installation
...
Method 2: Configuration File Alibaba Cloud SDK will automatically read credential configuration from ~/.aliyun/config.json or ~/.acs/credentials.
...
Visit Bailian Knowledge Base page
Click the "Activate Now" button
Confirm activation and wait for the service to take effect (usually 1-2 minutes)
...
Visit Alibaba Cloud RAM Access Control
Select the RAM user corresponding to the AK, click the "Add Permission" button, select AliyunBailianFullAccess and AliyunModelStudioReadOnlyAccess permission in the permission policy, and confirm to add the permission.
2) Bailian Workspace Permission Grant
Visit Alibaba Cloud Bailian Permission Management
If the RAM user corresponding to the AK does not exist, click "Add User" in the upper right corner of the page, select the corresponding RAM user and click confirm to add.
Click "Permission Management" on the right side of the RAM user corresponding to the AK, click edit, and grant knowledge base related permissions.
There is a 30s effective time after configuration, please wait patiently for a while.
Available Scripts
...
If there is only one workspace, use it automatically and inform the user
If there are multiple workspaces, display the list for user to select
Record user selection to avoid repeated inquiries (until user wants to switch)
...
Get all workspace lists from Step 2
Iterate through each workspace, call list_indices.js to retrieve its knowledge bases
Merge all knowledge base results, annotate the workspace they belong to
pageNumber starts from 1, pageSize defaults to 100, if current page is not fully retrieved then continue to retrieve next page
...
Organize answer in natural language
Please annotate the information source at the end of the generated answer (knowledge base name; document name; section name), can reference multiple documents and sections.
...
Get workspaceId → ws-123456
Query knowledge base → Returns 3 knowledge bases
Select knowledge base → "Product Documentation" (most relevant)
Retrieve → Get authentication-related document chunks
Answer → "According to product documentation, OAuth2.0, SAML, and API Key authentication methods are supported..."
Notes
