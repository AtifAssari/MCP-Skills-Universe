...
url: https://skills.sh/akin-ozer/cc-devops-skills/dockerfile-generator
---

# dockerfile-generator

skills/akin-ozer/cc-devops-skills/dockerfile-generator
dockerfile-generator
Installation
...
Validate with dockerfile-validator or fallback local tools.
Iterate fixes until stop condition is met.
...
Invoke dockerfile-validator.
Capture findings by severity (error, warning, info).
Prioritize security and reproducibility findings first.

Fallback path (if skill invocation is unavailable):

Try local validator script directly:
bash ../dockerfile-validator/scripts/dockerfile-validate.sh Dockerfile

If that path is unavailable, run available tools directly:
hadolint Dockerfile
checkov -f Dockerfile --framework dockerfile

If one or more tools are unavailable, continue generation and report each skipped check in the final report.
...
If any error exists, apply fixes and re-run validation.
Continue until:
no error remains, or
iteration count reaches 3.
For warning, either fix it or mark it as intentional deviation with justification.
...
Iteration log table from Stage 6.
Intentional deviation report (only when applicable).
Usage instructions.
Optimization metrics and next steps.
...
Docker Multistage Builds Guide
