---
rating: ⭐⭐
title: mx-stock-simulator
url: https://skills.sh/site/skills.volces.com/mx-stock-simulator
---

# mx-stock-simulator

skills/skills.volces.com/eastmoney_stock_simulator
eastmoney_stock_simulator
$ npx skills add https://skills.volces.com/skills/clawhub/jessecq1995
SKILL.md
...
持仓查询：查询指定账户的当前持仓股票。
买入卖出操作：执行买入和卖出操作，支持限价/市价委托，自动识别市场号和价格小数位。
撤单操作：撤销指定委托单，也支持一键撤单。
委托查询：查询账户下的所有委托订单（含已成交、未成交、已撤单）以及账户的历史成交记录。
资金查询：查询账户可用资金与总资产。
配置
...
使用方式
在妙想Skills页面获取apikey。
将apikey存到环境变量，命名为MX_APIKEY，检查本地apikey是否存在，若存在可直接用。
使用post请求接口，务必使用post请求，相关接口在后续章节说明。
功能概览
功能模块	状态	接口路径	说明
...
price	是	委托价格（useMarketPrice=false时必填，支持小数），支持小数
quantity	是	委托数量（股）
