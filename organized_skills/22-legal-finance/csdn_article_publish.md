---
rating: ⭐⭐⭐
title: csdn-article-publish
url: https://skills.sh/wuchubuzai2018/expert-skills-hub/csdn-article-publish
---

# csdn-article-publish

skills/wuchubuzai2018/expert-skills-hub/csdn-article-publish
csdn-article-publish
Installation
$ npx skills add https://github.com/wuchubuzai2018/expert-skills-hub --skill csdn-article-publish
SKILL.md
...
获取请求头方法
登录 CSDN 并打开 https://editor.csdn.net/md/ Markdown风格的文章编辑器
填写标题和内容，点击保存为草稿
打开浏览器开发者工具（F12）
切换到 Network（网络）标签
找到 saveArticle 请求，右键 → Copy → Copy as cURL
从 curl 命令中提取以下请求头：
Cookie: 用户登录Cookie
...
步骤 2：生成 Markdown 文章
根据用户需求生成文章内容（Markdown格式）
保存到当前工作目录的 csdnarticle/ 文件夹中
文件命名建议：{文章标题}.md
# 确保 csdnarticle 目录存在
mkdir -p csdnarticle
...
生成文章内容（Markdown格式）
保存到 csdnarticle/Python异步编程实战.md
调用 save 命令：
# 方式1：使用 --file 参数（推荐）
...
根据新需求更新 Markdown 内容
保存到 csdnarticle/Python异步编程进阶.md
调用 update 命令：
...
引导用户确认 creation_statement、readType、type 等字段
调用 publish 命令：
# 方式1：使用 --file 参数（推荐，若已有本地映射可省略 --id）
