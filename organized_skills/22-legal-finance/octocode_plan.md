...
rating: ⭐⭐⭐
title: octocode-plan
url: https://skills.sh/bgauryy/octocode-mcp/octocode-plan
---

# octocode-plan

skills/bgauryy/octocode-mcp/octocode-plan
octocode-plan
Installation
...
Check Context: Read .octocode/context/context.md (init if missing)
...
Use the host's task tracker to identify parallelizable work
Use the host's parallel subagent mechanism to spawn scoped work
Each agent uses the appropriate research skill or equivalent research tools independently
Synthesize outputs in Plan Phase
IF the host cannot run true parallel work → THEN execute the same scopes sequentially in dependency order
...
Local Style / context.md - Project-specific conventions always win
