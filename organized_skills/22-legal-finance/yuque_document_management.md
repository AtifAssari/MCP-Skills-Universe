---
rating: ⭐⭐⭐
title: yuque-document-management
url: https://skills.sh/doomclouds/lightragnet/yuque-document-management
---

# yuque-document-management

skills/doomclouds/lightragnet/yuque-document-management
yuque-document-management
Installation
$ npx skills add https://github.com/doomclouds/lightragnet --skill yuque-document-management
SKILL.md
语雀文档管理
...
获取目录结构：mcp_YuQueMCP_get_repository_toc_tree 了解组织方式
确定位置：选择合适的 parentUuid 或使用根目录（null）
准备内容：使用Markdown格式
创建文档：调用 mcp_YuQueMCP_create_document，参数：
title（必需）：文档标题
...
常见工作流
文档创建流程
查看结构：get_repository_toc_tree 了解现有组织
确定位置：选择合适的父目录UUID
准备内容：编写Markdown格式内容
创建文档：调用 create_document
验证结果：确认文档出现在预期位置
文档组织流程
获取当前结构：get_repository_toc_tree 查看完整目录树
识别需要移动的文档：确定重组目标
执行移动：使用 move_document 重新定位
验证结构：移动后重新获取目录树确认
错误处理要点
常见错误
...
使用最佳实践
操作前准备
获取知识库信息：先调用 get_default_repository 了解知识库
查看目录结构：使用 get_repository_toc_tree 规划组织方式
搜索验证：创建前搜索确认文档不存在
内容规范
使用清晰、描述性的标题
内容使用标准Markdown格式
