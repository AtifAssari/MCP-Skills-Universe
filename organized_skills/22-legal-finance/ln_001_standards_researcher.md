---
rating: ⭐⭐⭐
title: ln-001-standards-researcher
url: https://skills.sh/levnikolaevich/claude-code-skills/ln-001-standards-researcher
---

# ln-001-standards-researcher

skills/levnikolaevich/claude-code-skills/ln-001-standards-researcher
ln-001-standards-researcher
Installation
...
Check context_store.TECH_STACK if provided → use directly
Else: Glob for indicator files in project root
Store detected_stack.query_prefix for Phases 2-3
...
Standards Research string (Markdown) for insertion into Story Technical Notes
File path (string) for linking: docs/research/rsh-{NNN}-{slug}.md
