...
rating: ⭐⭐
title: qqbot-cron
url: https://skills.sh/sliverp/qqbot/qqbot-cron
---

# qqbot-cron

skills/sliverp/qqbot/qqbot-cron
qqbot-cron
Installation
...
优先使用 cron 工具（如果可用）
如果 cron 工具不可用，用 exec 执行 CLI 命令
可以通过 openclaw cron add --help 查看完整参数列表
🤖 AI 决策指南
时间确认规则
...
payload.deliver 必须是 true
payload.channel 必须是 "qqbot"
payload.to 必须是用户的 openid
...
先用 { "action": "list" } 找到对应任务 ID
再用 { "action": "remove", "jobId": "{id}" } 删除
