---
rating: ⭐⭐
title: spec-product-demo
url: https://skills.sh/zixun-github/aisdlc/spec-product-demo
---

# spec-product-demo

skills/zixun-github/aisdlc/spec-product-demo
spec-product-demo
Installation
...
优先使用输入：用户提供 DEMO_PROJECT_ROOT → 以其为准（仍需满足可运行判定）
未提供则自动查找：
{REPO_ROOT}/demo/
{REPO_ROOT}/prototype/ 或 {REPO_ROOT}/prototypes/
{REPO_ROOT}/apps/demo/、{REPO_ROOT}/packages/demo/
查找失败就停止：以上路径均不存在或不满足可运行判定 → 停止 R4，并要求用户提供 DEMO_PROJECT_ROOT
