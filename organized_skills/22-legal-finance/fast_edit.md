...
rating: ⭐⭐⭐⭐⭐
title: fast-edit
url: https://skills.sh/includewudi/fast-edit/fast-edit
---

# fast-edit

skills/includewudi/fast-edit/fast-edit
fast-edit
Installation
...
用户粘贴代码需要保存时，始终优先尝试 save-pasted。 零 token 输出、零 shell 转义问题。仅当不可用时才降级。
...
你的单次 token 输出上限约 120 行。这是硬性物理限制，任何工具都无法绕过。 fast-generate 的代码本身也是你的 token 输出。generate 代码 MUST ≤80 行。 任何犹豫 → 直接走分段写入。禁止反复评估。
...
代码包含以下任一字符时 → 必须 batch --stdin，禁止 CLI replace：
...
⚠️ Batch JSON 的字段名是 "action" 不是 "type"！ 写成 "type": "replace" 会报错 'action'（KeyError），不会静默忽略。
...
DUPLICATE_LINE → 检查是否 END 需要 +1，用 show 确认后重新 replace
BRACKET_BALANCE → 检查替换内容的括号是否完整闭合
如果是误报（故意的不平衡替换）→ 忽略即可
验证三连（每次编辑后）
