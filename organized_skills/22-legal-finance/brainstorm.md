...
rating: ⭐⭐⭐
title: brainstorm
url: https://skills.sh/shhac/skills/brainstorm
---

# brainstorm

skills/shhac/skills/brainstorm
brainstorm
Installation
$ npx skills add https://github.com/shhac/skills --skill brainstorm
SKILL.md
Brainstorm
...
Mark your task in_progress when you begin work
Read your task with TaskGet — the task description contains everything you need (proposals to review, feedback to incorporate, etc.). Do NOT search the filesystem or other agents' files for this content.
If your task description is missing required content (e.g., a review task with no proposals), tell the lead immediately and park. Do not improvise.
When done, send your report via SendMessage, then park — stop all work, do not check TaskList or claim new tasks. Just wait.
Before acting on any received message, check your task status via TaskGet:
...
Lead Protocol
After reading a teammate's report, mark their task completed (your acknowledgment)
Before sending new instructions, ensure the previous task is completed and the new task is created/assigned
Verify phase completion via TaskList — check that all relevant tasks show the expected status, don't track messages mentally
...
If the problem is underspecified, ask clarifying questions — good brainstorming needs clear constraints
Define evaluation criteria — the dimensions proposals will be compared on. Common ones:
...
Decide the spread — aim for a range of approaches, not N variations of the same idea. For example:
...
Present the framing to the user:
...
Create a proposal task per proposer with TaskCreate
Spawn one general-purpose teammate per proposal using Task with team_name
...
As proposers report back, mark each proposal task completed (acknowledging the report) and give the user brief progress updates
Do NOT shut down proposers when they report back — they are needed for Phase 3 and 4
...
Once all proposal tasks show completed in TaskList, collect all proposals
For each proposer, create a review task with TaskCreate. Include in the task description:
...
Assign each review task to its proposer and send them a message saying their review task is ready — the task description contains everything they need
Send all review messages in parallel — proposers review simultaneously
As reviewers report back, mark each review task completed (acknowledging the report) and give the user brief progress updates
Review Instructions
...
Once all review tasks show completed in TaskList, collect all reviews
For each proposer, create a revision task with TaskCreate. Include in the task description:
...
Assign each revision task to its proposer and send them a message saying their revision task is ready — the task description contains everything they need
Send all revision messages in parallel
As revised proposals come back, mark each revision task completed (acknowledging the report) and give the user brief progress updates
...
Once the user chooses an approach, summarize it as a clear brief:
The chosen proposal (final revised version)
Key decisions and rationale
Known risks and mitigations
Suggested implementation order or first steps
Send shutdown requests to all teammates and wait for each to confirm before wrapping up
The user can then implement directly, or hand the brief to team-solve for structured implementation
Rules
