---
rating: ⭐⭐
title: bunjang-search
url: https://skills.sh/nomadamas/k-skill/bunjang-search
---

# bunjang-search

skills/nomadamas/k-skill/bunjang-search
bunjang-search
Installation
...
auth login 은 headful 브라우저 + TTY / interactive 터미널이 필요하다.
로그인 전에는 검색/상세조회/대량 수집 위주로 답하고, favorite / chat / purchase 는 선택적 로그인 플로우로만 안내한다.
...
검색어가 넓으면 예산/모델/지역을 먼저 좁힌다.
검색 결과 상위 3~5개는 제목/가격 중심 1차 요약만 한다.
description / status / location 판단이 필요하면 item get 또는 --with-detail 로 상세를 먼저 읽는다.
로그인 액션이 필요하면 "지금은 로그인 세션이 없으니 interactive TTY 에서 auth login 후 다시 진행" 이라고 분명히 말한다.
대량 분석이면 JSON export 또는 TOON chunk 생성 경로를 제안한다.
Done when
