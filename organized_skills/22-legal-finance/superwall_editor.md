---
rating: ⭐⭐
title: superwall-editor
url: https://skills.sh/superwall/skills/superwall-editor
---

# superwall-editor

skills/superwall/skills/superwall-editor
superwall-editor
Installation
...
Create an auto-expose URL: scripts/sw-editor.sh expose --application-id <id> --paywall-id <id> --agent-name <agent> --open --wait
Ask the user to complete browser authorization if prompted. The editor auto-exposes; do not ask them to click the expose button.
Discover what is available right now: scripts/sw-editor.sh tools
Invoke tools: scripts/sw-editor.sh call <tool-name> --args '<json>'

Fallback manual flow:

Ask the user for the pairing code shown in the editor UI.
Attach: scripts/sw-editor.sh attach <pairing-code>
Continue with tools and call.
...
Native sw-* elements (multiple-choice, indicator, drawer, picker, lottie, navigation): references/native-elements.md
