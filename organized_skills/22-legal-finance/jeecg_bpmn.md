...
rating: ⭐⭐⭐
title: jeecg-bpmn
url: https://skills.sh/jeecgboot/skills/jeecg-bpmn
---

# jeecg-bpmn

skills/jeecgboot/skills/jeecg-bpmn
jeecg-bpmn
Installation
...
API 地址：JeecgBoot 后端地址（如 https://api3.boot.jeecg.com）
...
Windows 环境下 curl 发送中文会乱码，必须使用 Python 的 urllib 确保 UTF-8 编码
禁止使用 python3 -c "..." 内联方式，因为 BPMN XML 中的 ${applyUserId} 等表达式会被 bash 当作 shell 变量展开，导致 unexpected EOF 错误
必须先用 Write 工具写入 .py 临时文件，再用 Bash 执行，最后删除临时文件
...
taskExtendJson 的 value 属性用单引号包裹 JSON → value='{{"sameMode":2,...}}'（避免与 XML 双引号冲突，同时 JSON 花括号也需双花括号转义）
