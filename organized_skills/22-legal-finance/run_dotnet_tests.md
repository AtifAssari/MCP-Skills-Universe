...
url: https://skills.sh/oocx/tfplan2md/run-dotnet-tests
---

# run-dotnet-tests

skills/oocx/tfplan2md/run-dotnet-tests
run-dotnet-tests
Installation
...
Changes to the src/ directory (where global.json with "runner": "Microsoft.Testing.Platform" exists)
Normalizes any src/-prefixed paths in arguments
...
Run the specific failing test - Use --treenode-filter to isolate it
...
Use the update-test-snapshots skill - Never manually edit snapshot files
Verify the new output is correct - Review the diff carefully
Include SNAPSHOT_UPDATE_OK in commit message - Document the intentional change
