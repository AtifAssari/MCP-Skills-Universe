...
rating: ⭐⭐⭐
title: huashu-design
url: https://skills.sh/alchaincyf/huashu-skills/huashu-design
---

# huashu-design

skills/alchaincyf/huashu-skills/huashu-design
huashu-design
Installation
$ npx skills add https://github.com/alchaincyf/huashu-skills --skill huashu-design
SKILL.md
...
用 Agent Teams 启动3个后台 agent（run_in_background: true）
每个 agent 根据对应风格的提示词DNA + 用户实际内容，生成一个单页 HTML Demo
用 Playwright 截图（npx playwright screenshot file:///path.html output.png --viewport-size=1200,900）
3个 agent 完成后，将3张截图一起展示给用户
...
哲学一致性 - 是否忠实于选定的设计哲学？
视觉层级 - 信息重要性是否清晰？
细节执行 - 对齐、间距、字体、颜色是否精确？
功能性 - 设计是否服务于目标，有无过度装饰？
创新性 - 是否有独特之处，避免了cliché？
...
引用开源图标库 — 选与设计风格最匹配的：Lucide（极简）/ Phosphor（温暖）/ Heroicons（Apple风）/ Tabler（通用）
手写SVG图标 — 当开源库没有合适图标时，用内联 SVG 自行设计，确保线条粗细/圆角与设计系统一致

图片方案（按优先级）：

找到最合适的真实图片 — 搜索并引用外部链接，或下载到本地后在成品中引用本地路径
引用开源图片 — Unsplash / Pexels 等免费图库的直链
AI生成图片 — 调用 nano-banana-pro 生成匹配当前设计风格的图片，下载后本地引用
