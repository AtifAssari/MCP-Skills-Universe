...
rating: ⭐⭐
title: edge-to-edge
url: https://skills.sh/android/skills/edge-to-edge
---

# edge-to-edge

skills/android/skills/edge-to-edge
edge-to-edge
Installation
...
In each Activity, scan for TextField, OutlinedTextField, or BasicTextField. If found, then you MUST verify the IME doesn't hide the input field by following the IME section of this skill.
Step 2: add edge-to-edge support
Add enableEdgeToEdge before setContent in onCreate in each Activity that does not already call enableEdgeToEdge.
Add android:windowSoftInputMode="adjustResize" in the AndroidManifest.xml for all Activities that use a soft keyboard.
Step 3: apply insets
...
When you need an element (e.g. a custom header or decorative scrim) to equal the dimensions of a system bar, use inset size modifiers (e.g. Modifier.windowInsetsTopHeight(WindowInsets.systemBars)). See the Lists section for a code sample.
