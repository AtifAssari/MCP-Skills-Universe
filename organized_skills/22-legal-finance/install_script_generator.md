...
url: https://skills.sh/luongnv89/skills/install-script-generator
---

# install-script-generator

skills/luongnv89/skills/install-script-generator
install-script-generator
Installation
...
Detects the user's OS, architecture, and package manager automatically
Installs all dependencies and the target software
Verifies the installation
Can be executed via a one-liner using GitHub raw user content:
...
Identify the target — What software/module/tool is being installed?
Check the repository — Look for existing build files, Makefile, package.json, setup.py, Cargo.toml, go.mod, etc.
Identify dependencies — What does the software need to build/run?
Determine the GitHub repo — The <owner>/<repo> for the raw URL (check git remote or ask user)
...
Repository owner and name — Check git remote -v for origin URL, or ask the user
Branch — Default to main, check with git branch --show-current
File path — install.sh at the repo root
...
Detect the project type (e.g., Python package, Go binary, Node module)
Check git remote -v to get <owner>/<repo>
Generate install.sh with proper dependency installation and build steps
Output the one-liner command for the README
Example 2: "Generate a curl install command for my CLI tool"
Analyze the project's build system
Generate install.sh that downloads the latest release binary or builds from source
Include architecture detection for pre-built binaries
Output: curl -sSL https://raw.githubusercontent.com/user/tool/main/install.sh | bash
Example 3: "Make my module installable with a single command"
Check for existing Makefile, setup.py, package.json, etc.
Generate install.sh that wraps the build/install process
Add dependency installation (compilers, libraries, runtimes)
Verify the module is available after install
