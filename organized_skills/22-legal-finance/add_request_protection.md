---
rating: ⭐⭐⭐
title: add-request-protection
url: https://skills.sh/arcjet/skills/add-request-protection
---

# add-request-protection

skills/arcjet/skills/add-request-protection
add-request-protection
Installation
$ npx skills add https://github.com/arcjet/skills --skill add-request-protection
SKILL.md
...
Install the correct adapter package.
Check if ARCJET_KEY is set in the environment file (.env.local for Next.js/Astro, .env for others). If not, obtain the key in this priority order:
CLI (preferred): Run arcjet sites get-key --site-id <site-id> (requires arcjet auth login first — see Step 0)
MCP: If the Arcjet MCP server is connected, use list-teams → list-sites → get-site-key
...
Create a shared client file with shield() as the base rule. This file should export the Arcjet instance for reuse across routes with withRule().
