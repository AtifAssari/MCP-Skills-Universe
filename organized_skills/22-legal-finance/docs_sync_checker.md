---
rating: ⭐⭐⭐
title: docs-sync-checker
url: https://skills.sh/notque/claude-code-toolkit/docs-sync-checker
---

# docs-sync-checker

skills/notque/claude-code-toolkit/docs-sync-checker
docs-sync-checker
Installation
...
User created skills/my-new-skill/SKILL.md but forgot to update skills/README.md. Actions:

SCAN discovers my-new-skill in filesystem
CROSS-REFERENCE parses skills/README.md, does not find my-new-skill
DETECT flags as HIGH severity missing entry
REPORT suggests exact table row to add to skills/README.md
Example 2: Removed Agent Still Documented

User deleted agents/old-agent.md but agents/README.md still lists it. Actions:

SCAN does not find old-agent in filesystem
CROSS-REFERENCE finds old-agent in agents/README.md
DETECT flags as MEDIUM severity stale entry
REPORT suggests removing the row from agents/README.md
Example 3: Version Bump Without Doc Update

User updated version: 2.0.0 in skills/code-linting/SKILL.md but docs/REFERENCE.md still shows Version: 1.5.0. Actions:

SCAN reads YAML version as 2.0.0
CROSS-REFERENCE reads documented version as 1.5.0 from docs/REFERENCE.md
DETECT flags as LOW severity version mismatch
REPORT suggests updating version line in docs/REFERENCE.md to 2.0.0
Example 4: Batch Changes After Refactor

User created 3 new skills and deleted 2 old ones in a refactoring PR. Actions:

SCAN discovers 3 new skills in filesystem, does not find 2 removed skills
CROSS-REFERENCE finds 2 stale entries and 3 absent entries in skills/README.md
DETECT flags 3 HIGH (missing) + 2 MEDIUM (stale) issues
REPORT provides exact table rows to add and identifies rows to remove
Error Handling
Error: "YAML Parse Error"

Cause: Invalid frontmatter -- missing --- delimiters, tabs instead of spaces, or missing required fields Solution:

Check file has opening --- on line 1 and closing --- after YAML block
Verify no tab characters in YAML (spaces only)
Confirm required fields present: name, description, version
Validate manually: head -20 {file_path} and check syntax
Error: "Documentation File Not Found"

Cause: Expected README file does not exist at expected path Solution:

Verify --repo-root path is correct
Check that skills/README.md, agents/README.md, commands/README.md exist
If file is legitimately missing, create a placeholder with the expected table/list header
Re-run scan after creating placeholder
Error: "No Tools Discovered"

Cause: Wrong --repo-root path, empty directories, or no SKILL.md files Solution:

Verify the repo root path points to the correct repository
Confirm skills/, agents/, commands/ directories exist and are not empty
Check that skill directories contain SKILL.md (not just other files)
Run with --debug flag for verbose discovery output
Error: "Markdown Parse Error"

Cause: Table missing separator row, mismatched column counts, or malformed list items Solution:

Check table has header row, separator row (|---|---|), and data rows
Verify all rows have the same number of pipe-delimited columns
For lists, verify consistent format: - /command - Description
See references/markdown-formats.md for complete format specifications
References
