---
rating: ⭐⭐
title: easysdd-libdoc
url: https://skills.sh/liuzhengdongfortest/easysdd/easysdd-libdoc
---

# easysdd-libdoc

skills/liuzhengdongfortest/easysdd/easysdd-libdoc
easysdd-libdoc
Installation
...
libdoc 产物不在 easysdd/ 下——API 参考是面向外部读者的可发布产物。
...
确认项目类型和条目粒度——这是什么类型的库？条目粒度是什么？输出路径用 docs/api/ 还是项目已有约定？
扫描源码目录——读 source_root 下的文件结构，识别公开导出，按逻辑分组
生成 manifest.yaml——所有条目初始 status: pending；落盘后用 validate-yaml.py --file docs/api/manifest.yaml --yaml-only 校验语法；展示给用户 review
用户确认范围——用户可标 skipped（内部实现）、调整分类、合并或拆分；确认后清单落盘
...
落盘后 validate-yaml.py --file {条目文档路径} --require doc_type --require entry --require status 校验
更新 manifest.yaml 对应条目 status: current
模式 B：批量模式

适用于清单里有大量 pending 条目。

先出样板——从清单中选 2-3 个有代表性的条目（不同 category），按"读源码 → 提取 → 按模板生成"走一遍并落盘，状态先标 draft（不直接进 current——批量模式下样板的角色是"风格参考样本"，要等整体 review 一起转 current）
用户确认质量标准——review 这 2-3 篇，确认模板、详略度、风格符合预期。这一步不能跳——50 篇全白写就因为用户想要的风格不一样
批量生成——剩余 pending 条目逐条走"读源码 → 提取 → 生成"，可用 subagent 并行；每条生成后 status: draft
整体 review——批量完成展示概况（条目数、跳过数、待确认数）；review 前先 validate-yaml.py --dir docs/api --require doc_type --require entry --require status 批量校验，有报错先修
确认落定——用户确认后把样板和批量产出一起批量改 status: current
...
validate-yaml.py 校验，确认 frontmatter 未被破坏
status: current，last_reviewed 填当天
与其他工作流的关系
来源	关系
...
easysdd-feature-design	方案第 2 节可作 libdoc 补充信息源（但以源码为准）
