---
rating: ⭐⭐
title: wechat-minitest-ui
url: https://skills.sh/wujun8/agent-skills/wechat-minitest-ui
---

# wechat-minitest-ui

skills/wujun8/agent-skills/wechat-minitest-ui
wechat-minitest-ui
Installation
$ npx skills add https://github.com/wujun8/agent-skills --skill wechat-minitest-ui
SKILL.md
...
流程概览
确认项目：项目根目录需包含 project.config.json（含 appid 等）。Taro 项目一般为仓库根目录，编译产物在 dist，由 miniprogramRoot 指定。
启动自动化：用微信开发者工具 CLI 的 cli auto 或 cli auto-replay 打开项目并开启自动化端口（默认 9420）。
截图：录制回放中的截屏步骤，或 Minium Python 的 self.capture(name) / self.app.screen_shot(save_path)。
验证 UI：MiniTest 云测「图片对比」（SSIM），或本地脚本对比基准图与当前截图。
1. 微信开发者工具 CLI
...
2. Minium Python（参考 Minium Python 文档）
2.1 环境与安装
Python：3.8 及以上。
...
操作：.click()、设置/获取页面数据等，详见 Minium Python 文档。
3. 其他截图方式
方式	说明
...
CLI 的 --project 与 Minium 配置的 project_path 均使用项目根目录的绝对路径。
...
6. 执行清单（Agent 可逐步执行）
 确认 project.config.json 存在且含 appid（可为 touristappid）。
 若为 Taro 项目，确认已构建（如 pnpm build:weapp），使 dist 存在。
 使用 CLI 启动自动化：cli auto --project <项目根绝对路径> --auto-port 9420；或录制回放：cli auto-replay --project <项目根绝对路径>。
 若用 Minium：确认 config.json 中 project_path、test_port 正确，运行 minitest -m <模块> -c config.json -g，在关键步骤使用 self.capture(name) 或 self.app.screen_shot(save_path)。
 若用云测：在云测任务中配置录制回放或 Minium 用例，在报告中查看「图片对比」结果。
 根据对比结果或脚本断言，向用户汇报 UI 是否通过验证。
参考链接
Minium Python 文档（官方） — 安装、配置、API、示例
MiniTest 云测服务简介
自动化测试（Monkey / 录制回放 / 自定义测试）
图片对比能力说明
命令行 V2
Weekly Installs
21
