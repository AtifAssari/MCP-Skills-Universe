---
rating: ⭐⭐⭐
title: junli-ai-novel
url: https://skills.sh/ljunn/junli-ai-novel/junli-ai-novel
---

# junli-ai-novel

skills/ljunn/junli-ai-novel/junli-ai-novel
junli-ai-novel
Installation
$ npx skills add https://github.com/ljunn/junli-ai-novel --skill junli-ai-novel
SKILL.md
君黎 AI 网文连载
...
核心目标
最高优先级：连载稳定、追读牵引、卖点兑现、长线不崩。
默认按平台向长篇网文处理，不按出版文学标准做默认决策。
先读档，再下笔；先编排，再正文；先检查，再宣称完成。
任何新内容不得与更高层级记忆硬冲突。
默认入口顺序
...
先看 PROJECT.md
再判断当前任务属于哪个 Workflow
只有需要拆解排查时，才回到底层 Command
...
init 创建的新项目默认就是“全自动生成长篇连载”的就绪状态：治理模板、目标总字数、当前卷/阶段/阶段目标都会自动预置。旧项目或治理模板缺失时，先跑：

python3 scripts/chapter_pipeline.py bootstrap-longform <项目目录>

长篇治理范围内，每 10-20 章必须跑阶段审计；每卷结束必须跑卷审计：
...
如果 preflight 因当前卷 / 当前阶段 / 阶段目标缺失而失败，先跑：
python3 scripts/chapter_pipeline.py governance <项目目录> --current-volume "第一卷" --current-phase "阶段1" --phase-goal "目标"

如果要改全书终局、主角长期弧线、世界底层规则、长线关系边界，必须先更新 docs/变更日志.md，再继续正文。
如果无法执行命令、无法读取关键文件或前置校验失败，必须明确报告阻塞；禁止假装已经恢复上下文、已经检查或已经同步进度。
常见失误
...
先判断它影响的是立项、长篇治理、章节推进还是风格
长篇治理相关优先并入 references/longform-governance.md
大纲修整相关优先并入 references/outline-refinement.md
分类词库、用辞范例、描写辞典类材料优先并入 references/descriptive-taxonomy.md
局部返修相关优先并入 references/micro-revision-ops.md
工具与工作台相关优先并入 references/tooling-adoption-roadmap.md
只吸收可操作、可复用、可验证的内容
只适用于单项目的经验，写进项目文档，不污染通用规则
Weekly Installs
43
Repository
ljunn/junli-ai-novel
GitHub Stars
26
First Seen
Mar 23, 2026
