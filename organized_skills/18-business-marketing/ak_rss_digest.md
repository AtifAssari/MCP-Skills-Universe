...
rating: ⭐⭐
title: ak-rss-digest
url: https://skills.sh/rookie-ricardo/erduo-skills/ak-rss-digest
---

# ak-rss-digest

skills/rookie-ricardo/erduo-skills/ak-rss-digest
ak-rss-digest
Installation
...
Workflow
Run python3 scripts/fetch_today_feed_items.py --format json to collect entries from the configured feeds. This defaults to the most recent 7 days.
Treat feed-level network failures as non-fatal. Continue with the feeds that succeeded and mention major failures only when they materially reduce coverage.
...
Score every serious candidate on the rubric below. Output only items with a score strictly greater than 7.0.
If nothing clears the threshold, say so directly instead of padding the output with mediocre picks.
