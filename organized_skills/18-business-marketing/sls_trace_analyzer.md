---
rating: ⭐⭐⭐⭐⭐
title: sls-trace-analyzer
url: https://skills.sh/abcfed/claude-marketplace/sls-trace-analyzer
---

# sls-trace-analyzer

skills/abcfed/claude-marketplace/sls-trace-analyzer
sls-trace-analyzer
Installation
$ npx skills add https://github.com/abcfed/claude-marketplace --skill sls-trace-analyzer
SKILL.md
...
从域名解析环境和地域
从 t 参数提取时间戳
先查询 gateway access-log 获取 X-B3-TraceId
再用 traceId 查询详细日志
Configuration
Python 运行环境（按需自动初始化）

sls-query.py、requirements.txt、.venv/ 均位于本 SKILL.md 同级目录。

调用规则：

直接用同目录下的 .venv/bin/python 执行 sls-query.py，无需前置检查
仅当执行失败（No such file or directory、ModuleNotFoundError）时，在同目录下执行 python3 -m venv .venv && .venv/bin/pip install -q -r requirements.txt 初始化后重试
不要使用系统 python3 直接运行 sls-query.py，否则缺少依赖
上海 Region (默认)
...
时间范围：默认查询时间戳前后 10 分钟（不是 30 秒，太窄容易漏掉）
路径匹配：使用路径的关键词而非完整路径，例如用 registration 而非 /api-weapp/pe/registration
如果 access-log 模式查不到，可以直接用 gateway topic 查询：
...
从堆栈中找到 FeignClient 调用的方法名（如 tryLockingGoodsStockBatch）
在 RPC SDK 中找到 FeignClient 接口定义：~/IdeaProjects/AbcBisRpcSDK
根据 @PostMapping/@GetMapping 注解找到 URL path
在下游服务中搜索对应的 Controller 和 Service 实现

分析实现逻辑:

阅读 Controller 层代码，理解入参处理
阅读 Service 层代码，理解业务逻辑
特别关注：事务边界、锁机制、数据库操作顺序
对于死锁问题：分析 SQL 执行顺序和锁获取顺序
