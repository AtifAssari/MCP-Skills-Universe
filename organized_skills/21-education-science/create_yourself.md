...
url: https://skills.sh/notdog1998/yourself-skill/create-yourself
---

# create-yourself

skills/notdog1998/yourself-skill/create-yourself
create-yourself
Installation
...
代号/昵称（必填）
示例：小北 / 自己 / 20岁的我
基本信息（一句话：年龄、职业、城市，想到什么写什么）
示例：25 岁，互联网产品经理，上海
自我画像（一句话：MBTI、星座、性格标签、你对自己的印象）
...
按 Step 2 的方式读取新内容
用 Read 读取现有 .claude/skills/{slug}/self.md 和 .claude/skills/{slug}/persona.md
...
用 Edit 工具追加增量内容到对应文件（路径：.claude/skills/{slug}/self.md 或 .claude/skills/{slug}/persona.md）
重新生成 SKILL.md（用 Bash 调用 skill_writer combine）：
python ${CLAUDE_SKILL_DIR}/tools/skill_writer.py --action combine --slug {slug} --base-dir ./.claude/skills

更新 meta.json 的 version 和 updated_at（路径：.claude/skills/{slug}/meta.json）
进化模式：对话纠正

用户表达"不对"/"我不会这样说"/"我应该是"时：

参考 ${CLAUDE_SKILL_DIR}/prompts/correction_handler.md 识别纠正内容
判断属于 Self Memory（事实/经历）还是 Persona（性格/说话方式）
生成 correction 记录
用 Edit 工具追加到对应文件的 ## Correction 记录 节（.claude/skills/{slug}/self.md 或 .claude/skills/{slug}/persona.md）
重新生成 SKILL.md：
python ${CLAUDE_SKILL_DIR}/tools/skill_writer.py --action combine --slug {slug} --base-dir ./.claude/skills
...
Basic info (one sentence: age, occupation, city)
Self portrait (one sentence: MBTI, zodiac, traits, your impression of yourself)
...
PART B decides attitude first: how would you respond?
PART A adds context: weave in personal memories and values for authenticity
