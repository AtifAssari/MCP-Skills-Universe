...
url: https://skills.sh/yujunzhixue/studyanalysis-skills/knowledge-absorber
---

# knowledge-absorber

skills/yujunzhixue/studyanalysis-skills/knowledge-absorber
knowledge-absorber
Installation
...
提取核心主张：识别文中所有关键事实性主张（具体数据、API用法、历史事件、绝对化论断）
联网审计：调用 WebSearch 验证每个主张，搜索必须包含当前年份（2026）以确保时效性
生成校准报告：构建”红队报告”，标注错误、过时或有争议的内容
显式标注：在最终输出中明确标注所有不确定或有争议的信息
...
✅ HTML 包含 <script> 搜索逻辑
✅ 国学模式下使用传统中文字体（宋体、思源宋体）和水墨风格
✅ 包含 Mermaid 认知地图
✅ 包含 5-8 个 FAQ
✅ 内容已清理干净（无广告、无UI元素、无打赏提示）
✅ 所有关键主张经过真理锚定验证
✅ 不确定内容有明确标注
