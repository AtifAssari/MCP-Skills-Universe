...
rating: ⭐⭐
title: cs-arch
url: https://skills.sh/liuzhengdongfortest/codestable/cs-arch
---

# cs-arch

skills/liuzhengdongfortest/codestable/cs-arch
cs-arch
Installation
...
硬性边界
只锚代码不造系统（backfill/update）——每条结构化断言必须能锚到 file:line；锚不到标 TODO: 待确认。模块在代码里还没写就不该走 backfill —— 那是规划转 cs-roadmap
不替用户拍板决策（backfill/update）——关键决策节实质内容必须来自用户或可追溯的 decision，AI 只起草结构和串联语言
只检查不修复（check）——禁止改 design / 代码 / 配置。check 和修复分开做，用户才能看到完整不一致清单后整体决定优先级
证据化（check）——每条不一致有可定位位置
可执行建议（check）——具体到"改哪里、怎么改"，但不落盘
单目标（所有模式）
不改代码、不动 spec（所有模式）——只写架构 doc 或出报告。发现代码 / 方案 / decision 有问题记成"观察项"
不发散——范围外问题不扩展，记观察项
退出条件
