...
rating: ⭐⭐⭐
title: plan-one-shot
url: https://skills.sh/ishk-sftckz/exspecso/plan-one-shot
---

# plan-one-shot

skills/ishk-sftckz/exspecso/plan-one-shot
plan-one-shot
Installation
...
Block execution until user says exactly: Execute Plan.
Do not skip artifact generation even for short tasks.
After approval: execute strictly in task.md order and update statuses continuously.
For existing projects, preserve architecture, naming, folder layout, and dependency choices unless the user asks for a change.
...
First call in a new conversation defaults to new planning. Do not infer an active plan from .exspecso/ history.
Only resume prior plan artifacts when the user explicitly asks to continue/resume an existing plan.
...
Remove filler words when possible (the, a, an, for, to, of).
Keep first 3-6 meaningful words.
Max length 36.
...
Do not scan .exspecso/ to auto-select or continue an older plan on first invocation.
Continue an existing plan only when the user explicitly requests it (for example: "continue existing plan", "resume plan", "continue implementing the plan") and provides a clear reference.
