...
url: https://skills.sh/jackjin1997/clawforge/langsmith-dataset
---

# langsmith-dataset

skills/jackjin1997/clawforge/langsmith-dataset
langsmith-dataset
Installation
...
generate_datasets.py - Create evaluation datasets from traces query_datasets.py - View and inspect datasets
...
User-specified output fields (--output-fields)
...
Naming Convention: Use "Skills: " format for consistency:
...
Always use --root-run-name - Filter for specific agent framework (e.g., "LangGraph")
Start with successful traces - Use recent successful runs for baseline datasets
Use time windows - --last-n-minutes 1440 for last 24 hours of data
Sample for single_step - Use --sample-per-trace 2 to capture conversation evolution
Match depth to needs - --depth 2 typically captures all main tool calls
Review before upload - Use query_datasets.py view-file to inspect first
Iterative refinement - Generate small batches (10-20) first, validate, then scale up
Use --replace carefully - Overwrites existing datasets, useful for iteration
Example Workflow
