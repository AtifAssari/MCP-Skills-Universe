...
rating: ⭐⭐⭐
title: pyomo
url: https://skills.sh/tondevrel/scientific-agent-skills/pyomo
---

# pyomo

skills/tondevrel/scientific-agent-skills/pyomo
pyomo
Installation
$ npx skills add https://github.com/tondevrel/scientific-agent-skills --skill pyomo
SKILL.md
...
Best Practices
Always scale your problem - variables and constraints should be within similar orders of magnitude
Use pyo.value() to extract numerical results from variables after solving
Check solver termination conditions to verify optimality or diagnose issues
Use pyo.Set and pyo.Param for organizing data in complex models
Initialize variables for NLP problems - good starting points are crucial
Use Pyomo-compatible functions (pyo.log(), pyo.exp(), etc.) instead of NumPy/math functions in expressions
Reformulate conditional logic using Big-M notation or binary variables
Avoid non-smooth functions like abs() or max() - use reformulations
Enable dual suffix for shadow prices when economic analysis is needed
Set solver time limits and options appropriately for your problem size
