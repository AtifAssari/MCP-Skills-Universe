...
url: https://skills.sh/uniswap/uniswap-ai/v4-hook-generator
---

# v4-hook-generator

skills/uniswap/uniswap-ai/v4-hook-generator
v4-hook-generator
Installation
...
Permissions to enable — only the callbacks your logic actually uses (beforeSwap, afterSwap, etc.)
Utility libraries — currencySettler, safeCast, transientStorage as needed
Shares — false, ERC20, ERC6909, or ERC1155
Access control — ownable, roles, or managed
Hook inputs — blockNumberOffset, maxAbsTickDelta (only for hook types that use them)
...
Does the hook hold or move user funds? (Determines currencySettler and shares.)
...
Is this for a chain with EVM Cancun support? (Required for transientStorage.)
...
Set hook to the chosen type.
Set each permission flag — default false, enable only what the logic requires.
Set currencySettler, safeCast, transientStorage based on utility library guidance.
Set shares.options based on shares guidance.
Set access based on access control guidance.
Set inputs only if the hook type uses blockNumberOffset or maxAbsTickDelta.
...
Explain the key generated sections (constructor, getHookPermissions, enabled callbacks).
Note any manual steps required (HookMiner for address mining, deployment script updates for constructor args if access is roles or managed).
