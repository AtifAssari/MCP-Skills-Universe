...
rating: ⭐⭐
title: cc4d
url: https://skills.sh/tombensim/claude-for-dummies/cc4d
---

# cc4d

skills/tombensim/claude-for-dummies/cc4d
cc4d
Installation
...
How This Works
Run bash scripts/progress.sh next to get your current step instructions
Follow the ACTION in the step
Verify the CHECK passes
Save what you learned to CLAUDE.md (every step has a CAPTURE section)
Run bash scripts/progress.sh complete N to advance to the next step
Repeat
...
Run bash scripts/progress.sh status to see where they left off
Greet them by name
Run bash scripts/progress.sh next and continue from where they stopped
Progress Commands
...
What to do
Do NOT run progress.sh — no next, no complete, no step system at all.
Read CLAUDE.md to understand the project (name, vibe, audience, tech stack, what was built).
Check if the dev server is running — run npm run dev with run_in_background: true if it's not already up on port 3000.
Wait for the user's actual message — the [WORKSPACE MODE] message is system context, not a user request. The user's next message will describe what they need (add a feature, fix a bug, change the design, deploy, etc.).
Execute the task using the same principles as the wizard: plain language, fix don't instruct, pick sensible defaults, action first explanation second.
Deploy when asked — when the user says "deploy", "put it online", or similar: run git add -A && git commit -m "Update site" && npx vercel --prod --yes (use run_in_background: true for vercel). Report the live URL back.
Update CLAUDE.md after significant changes to keep it current for next time.
What NOT to do
