...
rating: ⭐⭐
title: review-php
url: https://skills.sh/nesnilnehc/ai-cortex/review-php
---

# review-php

skills/nesnilnehc/ai-cortex/review-php
review-php
Installation
...
审查清单（仅限 PHP 维度）
Strict types and declarations: declare(strict_types=1) usage;类型属性和参数；返回类型声明；避免隐式类型强制陷阱。
错误处理：异常与错误； Throwable 层次结构；正确的尝试捕获和重新抛出；避免空捕获或过于宽泛的捕获；相关的“error_reporting”和错误到异常的转换。
资源管理：fopen/fclose、数据库连接、流；确保资源已关闭（try-finally 或短期作用域）；避免资源泄漏。
PSR 标准：PSR-4 自动加载和命名空间到路径映射； PSR-12 编码风格（缩进、大括号、可见性）；类和方法的命名。
命名空间和自动加载：正确的use语句；避免全局命名空间污染；作曲家自动加载对齐。
空安全：空合并（??）、空安全运算符（?->）；避免“@”错误抑制；数组的“isset”与“array_key_exists”。
生成器和迭代：正确的yield用法；正确的迭代器实现；大型数据集的内存高效迭代。
PHP版本兼容性：使用的功能与composer.json中的“php”约束；已弃用的 API 和迁移路径。
可测试性：依赖注入；静态和单例使用；构造函数注入；用于嘲笑的接缝。
语气和参考
