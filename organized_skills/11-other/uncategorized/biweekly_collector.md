---
rating: ⭐⭐⭐
title: biweekly-collector
url: https://skills.sh/niracler/skill/biweekly-collector
---

# biweekly-collector

skills/niracler/skill/biweekly-collector
biweekly-collector
Installation
$ npx skills add https://github.com/niracler/skill --skill biweekly-collector
SKILL.md
Biweekly Collector
...
Suggest a default based on the last entry's date. Scan both Areas/生活(Life)/周记(Weekly)/ and Areas/生活(Life)/月记(Monthly)/ for the most recent entry to calculate the gap.
...
Create the file using the template from templates/monthly.md. The output directory can be either 周记(Weekly)/ or 月记(Monthly)/ — ask the user or accept if they specify a path directly:
...
Preserve the user's original voice — summarize but don't rewrite. Include photo references (![[filename]] or ![](url)) as-is.
...
Hashtags are inline in HTML as <a href="/search/..."> links (e.g., #pinboard, #bilibili, #梦, #暴论, #网易云音乐). Extract via regex for categorization.
pubDate is in GMT (RFC 2822). Strictly filter by date range — only include items whose pubDate falls within the confirmed start/end dates. This is critical when the user specifies a historical range (e.g., 3/1-3/15) because the RSS feed contains recent posts that may be outside the range. Parse the date and compare before including.
Content types vary: bookmarked articles, video shares, personal thoughts, dream journal, music shares, ACG content, tech observations, life updates
...
-ic "Personal,Learn&Create": include only personal calendars, skip "Work" and "Scheduled Reminders" calendars (reminders are captured separately via reminders-cli)
Output includes: event summary, calendar name, time range, and notes

The output is already well-structured with date grouping. Include personal events (meetups, appointments, trips, hobbies) but skip recurring work meetings.
...
The plrom repo (~/code/nini-dev/repos/plrom/) is a standalone git repo that tracks "人 X 社区 X 物" — the user's curated list of people, communities, and things they follow. It auto-syncs to the blog via GitHub Actions.
...
Photos — collect all ![[image]] and ![](url) references from diary entries. List them in context so the user can select which to include.
