...
url: https://skills.sh/marketingjuliancongdanh79-pixel/skill-generator/skill-creator-ultra
---

# skill-creator-ultra

skills/marketingjuliancongdanh79-pixel/skill-generator/skill-creator-ultra
skill-creator-ultra
Installation
$ npx skills add https://github.com/marketingjuliancongdanh79-pixel/skill-generator --skill skill-creator-ultra
SKILL.md
Goal
...
⚡ Quick Mode — nếu user đã mô tả đủ rõ → sinh skill trong 1 lượt
Mở đầu → Hỏi mô tả công việc
Trích xuất TRIGGER, STEPS, INPUT/OUTPUT, RULES, EDGE CASES, TOOLS
Tổng kết → Xác nhận với user
...
Hỏi nền tảng (Antigravity/Claude/Cursor/Windsurf/OpenClaw)
Hỏi scope (Global vs Workspace)
Tạo cấu trúc thư mục theo Complexity Score
Sinh SKILL.md (Frontmatter + Goal + Instructions + Examples + Constraints)
Sinh full package (README, resources/, examples/, scripts/)
Chạy Trigger Eval nhanh cho description (📚 resources/description_optimization.md)
...
Trình bày kết quả (tree cấu trúc + thống kê)
Dry Run — chạy thử với tình huống thực tế
Chỉnh sửa theo feedback
Validation tự động (checklist)
Auto-Optimize — AI tự review + sửa + chấm Quality Score
A/B Variant Testing (nếu complexity ≥ 13)
Deploy & hướng dẫn sử dụng
📦 Package & Publish — đóng gói .skill file, publish lên marketplace
...
Viết 2-3 test cases (bao gồm ≥1 adversarial test)
Chạy test thật (không phải dry run)
7-Dimension Scoring — chấm Correctness, Completeness, Format, Adherence, Safety, Efficiency, Robustness
Security Scanning — 5 checks: prompt injection, PII, secrets, scope, destructive
Radar Report + CI-ready JSON output
Grade S/A/B/C/D/F → quyết định iterate hay deploy
...
Đọc feedback (từ Phase 6 hoặc user)
Sửa skill — generalize, đừng overfit
Re-test → so sánh pass rate
Lặp cho đến: user hài lòng / hết progress
...
Viết 5 câu NÊN trigger + 5 câu KHÔNG NÊN trigger
Verify description cover đủ
Sửa description cho "pushy" hơn
...
Luôn tổng kết + confirm trước khi chuyển phase — sửa hiểu lầm lúc này rẻ hơn nhiều so với sửa sau khi sinh skill
Về chất lượng

Skill tốt = function tốt — làm đúng 1 việc, hoàn hảo. Tên có "and" → tách.

Description quyết định trigger — viết "pushy" hơn, cover nhiều cách user hỏi. (📚 resources/description_optimization.md)

Ví dụ là DNA — 2-3 ví dụ > 50 dòng quy tắc. Thiếu ví dụ = tăng hallucination.

Giữ SKILL.md < 500 dòng. Vượt → tách ra resources/. (📚 resources/skill_writing_guide.md)
