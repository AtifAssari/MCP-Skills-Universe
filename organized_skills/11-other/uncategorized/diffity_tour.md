...
rating: ⭐⭐
title: diffity-tour
url: https://skills.sh/kamranahmedse/diffity/diffity-tour
---

# diffity-tour

skills/kamranahmedse/diffity/diffity-tour
diffity-tour
Installation
...
If the argument is a GitHub PR URL:
...
Otherwise, ensure a tree instance is running: run diffity list --json.
...
Identify the key locations that tell the story — the files and line ranges that someone needs to see to understand the answer.
...
Transition (1 sentence): connect to the previous step — see the examples below.
Explanation (the bulk): what this code does and why it's structured this way — design decisions, trade-offs, concrete examples. Use sub-highlights (focus:X-Y) if the range breaks into distinct sections.
Takeaway (1-2 sentences): a gotcha, edge case, or "this is what you'd touch if..." pointer.
...
Get the running instance port from diffity list --json.
