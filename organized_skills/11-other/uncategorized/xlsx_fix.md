...
rating: ⭐⭐⭐
title: xlsx-fix
url: https://skills.sh/xxxcoffee/shareskills/xlsx-fix
---

# xlsx-fix

skills/xxxcoffee/shareskills/xlsx-fix
xlsx-fix
Installation
$ npx skills add https://github.com/xxxcoffee/shareskills --skill xlsx-fix
SKILL.md
Fix Excel Skill
用途
...
将 xlsx 文件作为 zip 解压
找到 xl/styles.xml 文件
将空的 <fill/> 或 <fill></fill> 标签替换为有效的 <fill><patternFill patternType="none"/></fill>
重新打包为 xlsx 文件
自动创建 .bak 备份文件
使用方法
...
注意事项
修复脚本会自动创建 .bak 备份文件
只支持 .xlsx 格式，不支持 .xls 或 .xlsm
修复后的文件应该能被 openpyxl 正常读取
如果修复后仍有问题，可以尝试手动检查 styles.xml 文件
脚本路径
