...
rating: ⭐⭐⭐
title: svg-design
url: https://skills.sh/tryopendata/skills/svg-design
---

# svg-design

skills/tryopendata/skills/svg-design
svg-design
Installation
...
Copy this skill's assets/preview.html to the project directory using cp with the absolute path from where this skill was loaded (do not read or modify the file).
Design the first logo and write its SVG file.
Write variants.js with just that first variant (format in references/editing-workflow.md).
Open the preview with open preview.html (macOS) or xdg-open preview.html (Linux). The user now sees the first logo while you keep working.
For each subsequent logo: write the SVG file, then update variants.js to add it. The preview auto-reloads both every 3 seconds, so new logos appear in the browser as they're completed.
