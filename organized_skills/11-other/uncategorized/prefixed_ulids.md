---
rating: ⭐⭐
title: prefixed-ulids
url: https://skills.sh/dvf/opinionated-django/prefixed-ulids
---

# prefixed-ulids

skills/dvf/opinionated-django/prefixed-ulids
prefixed-ulids
Installation
...
Add a new CharField column with the prefixed ULID default, nullable at first.
Backfill with a data migration that assigns prefixed_ulid("prd") to every existing row.
Add unique=True and make it non-nullable in a follow-up migration.
Swap it to primary_key=True only after every foreign key has been migrated to reference the new column — this usually means a multi-release cutover.
