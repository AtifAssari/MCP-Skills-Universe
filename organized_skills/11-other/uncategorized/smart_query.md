...
rating: ⭐⭐
title: smart-query
url: https://skills.sh/zrt-ai-lab/opencode-skills/smart-query
---

# smart-query

skills/zrt-ai-lab/opencode-skills/smart-query
smart-query
Installation
...
自然语言查询流程
理解用户意图：分析用户想查什么数据
查阅表结构：读取 references/schema.md 了解表结构
生成SQL：根据表结构编写正确的SQL
执行查询：使用 query.py 执行
解读结果：用通俗语言解释查询结果
配置说明
...
分享给同事
复制整个 smart-query/ 目录
同事复制 config/settings.json.example 为 settings.json
填入自己的 SSH 和数据库连接信息
安装依赖：pip install paramiko sshtunnel pymysql
安全提示
