...
url: https://skills.sh/lachyfs/kanban-skill/kanban-markdown
---

# kanban-markdown

skills/lachyfs/kanban-skill/kanban-markdown
kanban-markdown
Installation
...
Creating Features
Generate ID: lowercase title, keep only a-z 0-9 - space, replace spaces with -, collapse multiple -, trim - from ends, truncate to 50 chars, append -YYYY-MM-DD. If empty, use feature-YYYY-MM-DD.
Set created and modified to current ISO timestamp. Set order by reading existing features in the target column and generating a key that sorts after the last one (see Fractional Index Ordering above).
If status is done, set completedAt to now and place in done/ subfolder.
Write frontmatter in exact field order above, then # Title and body.
Updating Features
