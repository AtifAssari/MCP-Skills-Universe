...
rating: ⭐⭐
title: router-act
url: https://skills.sh/vercel/next.js/router-act
---

# router-act

skills/vercel/next.js/router-act
router-act
Installation
...
Core Principles
Use LinkAccordion to control when prefetches happen. Never let links be visible outside an act scope.
Prefer 'no-requests' whenever the data should be served from cache. This is the strongest assertion — it proves the cache is working.
Avoid retry/polling timers. The act utility exists specifically to replace inherently flaky patterns like retry() loops or setTimeout waits for network activity. If you find yourself wanting to poll, you're probably not using act correctly.
Avoid the block feature. It's prone to false negatives. Prefer includes and 'no-requests' assertions instead.
Act API
Config Options
...
Waits for a requestIdleCallback (captures IntersectionObserver-triggered prefetches)
Fulfills buffered responses to the browser
