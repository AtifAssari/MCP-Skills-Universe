...
rating: ⭐⭐⭐
title: libtv-skill
url: https://skills.sh/libtv-labs/libtv-skills/libtv-skill
---

# libtv-skill

skills/libtv-labs/libtv-skills/libtv-skill
libtv-skill
Installation
...
Provides session-based conversation API: create sessions, send natural language requests, poll progress, upload reference files, and batch download results
Requires LIBTV_ACCESS_KEY environment variable; uses only Python standard library with no additional dependencies
SKILL.md
agent-im 会话（生图 / 生视频）
...
功能
创建会话 / 发消息 - 创建新会话或向已有会话发送一条消息（如「生一个动漫视频」「把纸船换成爱心」）
查询会话进展 - 根据 sessionId 拉取该会话的消息列表，用于轮询生图/生视频结果
切换项目 - 将当前 accessKey 绑定的项目切换到新项目，后续 create_session 将使用新 projectUuid
上传文件 - 上传图片或视频文件到 OSS，返回可访问的 OSS 地址（编辑已有视频/图片时需要先上传）
下载结果 - 将会话中生成的图片/视频批量下载到本地，自动提取 URL 并命名
前置要求
...
上传：用户给了本地文件 → upload_file.py 拿到 OSS URL
传话：把用户的原始描述 + OSS URL 原封不动发给 create_session.py
取件：轮询结果 → 下载到本地 → 展示给用户
