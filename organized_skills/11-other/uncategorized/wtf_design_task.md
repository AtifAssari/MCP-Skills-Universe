---
rating: ⭐⭐
title: wtf.design-task
url: https://skills.sh/xiduzo/wtf/wtf.design-task
---

# wtf.design-task

skills/xiduzo/wtf/wtf.design-task
wtf.design-task
Installation
...
Path A — Human provides frames: Collect frame URLs. For each Gherkin scenario from step 5, check whether a frame covers it. Flag any scenario with no matching frame as a gap. Present the coverage matrix: scenario → frame URL (or ⚠ gap). If gaps exist, ask "How should I handle the uncovered scenarios?" — header Gaps:
...
Partial: Collect available frame URLs, run Path A validation on covered states. For uncovered states, ask "How should I handle the remaining states?" — header Remainder:
