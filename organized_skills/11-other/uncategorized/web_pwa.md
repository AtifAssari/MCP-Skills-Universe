...
rating: ⭐⭐
title: web-pwa
url: https://skills.sh/alphaonedev/openclaw-graph/web-pwa
---

# web-pwa

skills/alphaonedev/openclaw-graph/web-pwa
web-pwa
Installation
...
Basic PWA Setup: Create a service worker to cache static assets. In index.js: if ('serviceWorker' in navigator) { navigator.serviceWorker.register('/sw.js').then(reg => console.log('SW registered')); }. In sw.js: self.addEventListener('install', event => event.waitUntil(caches.open('static').then(cache => cache.add('/'))));. This enables offline loading of the home page.
Push Notifications Integration: Subscribe users and handle messages. In your app: registration.pushManager.subscribe().then(sub => fetch('/subscribe', { method: 'POST', body: JSON.stringify(sub) }));. On the server, use an endpoint like POST /subscribe with $VAPID_PUBLIC_KEY to send notifications via the Push API.
Graph Relationships
