...
url: https://skills.sh/juanmorales1810/pwa-service-worker/pwa-service-worker
---

# pwa-service-worker

skills/juanmorales1810/pwa-service-worker/pwa-service-worker
pwa-service-worker
Installation
$ npx skills add https://github.com/juanmorales1810/pwa-service-worker --skill pwa-service-worker
SKILL.md
...
Implement service worker registration, lifecycle management, and public/sw.js.
Choose and implement caching strategies based on resource types.
...
install — The new SW downloads and fires install. This is the right time to precache static assets.
waiting — If there's already an active SW controlling the page, the new one enters waiting state. It will not activate until all tabs using the old SW are closed.
activate — The new SW takes control. This is the right time to clean up old caches.
...
Best for: versioned static assets (/_next/static/, fonts, images with hashed filenames). Why: these assets are immutable once deployed — serving from cache is always correct and fast.

Network First (Network, falling back to Cache)

Best for: HTML pages, API responses that must be fresh. Why: the user always gets the latest content when online, and still gets something useful when offline.

Stale-While-Revalidate

Best for: assets that should be fast but eventually consistent (non-critical API responses, avatar images, non-hashed CSS). Why: serves the cached version instantly (fast) while fetching a fresh copy in the background for next time.

Network Only

Best for: analytics pings, POST/PUT/DELETE requests, any non-idempotent operations. Why: caching these would cause incorrect behavior (duplicate submissions, stale mutations). Never cache non-idempotent requests.

Cache Only

Best for: explicitly precached assets during install that you know are always available. Why: avoids network round-trips entirely for known resources.
...
In the client component, detect the failed fetch and register a sync event tag.
In the SW, listen for the sync event and replay the queued request.
Store pending requests in IndexedDB (not localStorage, which is unavailable in service workers).
...
Show a custom in-app prompt explaining the value of notifications.
Only call requestPermission() after the user clicks "Enable notifications" in your UI.
Handle all three states: granted, denied (show explanation, no retry possible), default (can ask again).
...
In the registration component, listen for registration.onupdatefound.
When the new SW enters the installed state (meaning it's waiting), show a banner: "A new version is available. Reload to update."
When the user clicks the banner, post a message to the waiting SW telling it to skipWaiting().
...
When the user needs advanced offline capabilities (complex precaching manifests, runtime caching routes with expiration, automatic SW generation), use Serwist — the actively maintained successor to Workbox's Next.js integrations.
...
Setup
Install: npm install @serwist/next @serwist/precaching @serwist/strategies
Configure the Next.js plugin in next.config.js wrapping the existing config.
Create the SW entry file (e.g., app/sw.ts) using Serwist's API instead of raw self.addEventListener.
Serwist's plugin handles injecting the precache manifest at build time.
...
references/strategies-and-snippets.md: Code templates for caching strategies, offline fallback, update notification component, Background Sync, cache versioning, Serwist config, iOS meta tags, and full troubleshooting checklist.
Weekly Installs
16
Repository
juanmorales1810…e-worker
GitHub Stars
1
First Seen
Mar 7, 2026
