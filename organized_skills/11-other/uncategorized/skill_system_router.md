---
rating: ⭐⭐⭐
title: skill-system-router
url: https://skills.sh/arthur0824hao/skills/skill-system-router
---

# skill-system-router

skills/arthur0824hao/skills/skill-system-router
skill-system-router
Installation
...
Load skill-system-router first.
Run the guard-check operation.
If guard-check has NOT succeeded: block all non-router skill loads and emit the failure message:

Router guard failed: load skill-system-router and complete router discovery before using non-router skills.

Only after guard-check succeeds: proceed with non-router skills.
...
The internal policy helper scripts/router.py check-policy returns a structured policy_decisions list so callers can surface the exact allow/deny reasoning. If skill_system.policy_profiles is unavailable, the helper falls back to config/router.yaml policy.default_allowed_effects and reports that fallback explicitly in the output.

Step 4: Execute
Determine OS: OS env = Windows_NT → use windows entrypoint, otherwise unix
Read the operation's entrypoints for your OS
...
For workflow-only planning (no TKT), use scripts/plan-with-workflow.md. For TKT-integrated planning, use the call chain above.
