...
rating: ⭐⭐⭐
title: mthds-build
url: https://skills.sh/mthds-ai/skills/mthds-build
---

# mthds-build

skills/mthds-ai/skills/mthds-build
mthds-build
Installation
...
List all .mthds files in the project (glob for **/*.mthds)
For each file found, read the file header (domain, main pipe code, description) to understand what it does
...
Create the output directory: mkdir -p mthds-wip/<bundle_dir>/
Save concept TOML from Phase 4 to mthds-wip/<bundle_dir>/concepts.toml using the Write tool
Save pipe TOML from Phase 7 to mthds-wip/<bundle_dir>/pipes.toml using the Write tool
Run assemble with file paths (not inline content):
...
Clean up the intermediate files (concepts.toml, pipes.toml) — they are no longer needed.
