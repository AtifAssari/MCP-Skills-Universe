---
rating: ⭐⭐⭐
title: story-to-scenes
url: https://skills.sh/zrt-ai-lab/opencode-skills/story-to-scenes
---

# story-to-scenes

skills/zrt-ai-lab/opencode-skills/story-to-scenes
story-to-scenes
Installation
$ npx skills add https://github.com/zrt-ai-lab/opencode-skills --skill story-to-scenes
SKILL.md
Story To Scenes
...
铁律
单图原则：每个场景/角色生成独立单图，禁止多格拼接、分镜框、边框组合
先人后景：必须先生成并锁定角色胚子，再进行场景生图
确认才锁定：角色胚子、风格胚子必须用户确认后才算锁定
引用生成：场景中出现已锁定角色时，必须引用其胚子图
提示词记录：每张图的完整提示词必须记录，方便复用和微调
进度持久化：生成过程实时保存进度，支持断点续传
详细步骤
Step 1: 项目初始化
...
Step 2: 文本解析与角色提取
自动识别文本类型：故事/课程/脚本/连环画
按语义分割场景（非机械按段落切）
提取所有人物/动物/生物，生成角色清单表
...
命名规范：scene_{序号}_{场景名}.png 存储位置：assets/generated/{项目名}/scenes/
