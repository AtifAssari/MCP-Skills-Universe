...
rating: ⭐⭐
title: css-debug
url: https://skills.sh/majiayu000/claude-arsenal/css-debug
---

# css-debug

skills/majiayu000/claude-arsenal/css-debug
css-debug
Installation
...
请在浏览器中运行以下命令查找 overflow:hidden 的容器：
document.querySelectorAll('*').forEach(el => {
  const style = getComputedStyle(el);
...
请告诉我哪些容器被标记出来了
同时，请检查绝对定位元素的父容器是否有 position: relative
Weekly Installs
45
Repository
