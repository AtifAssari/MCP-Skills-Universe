...
rating: ⭐⭐
title: cs-issue-fix
url: https://skills.sh/liuzhengdongfortest/codestable/cs-issue-fix
---

# cs-issue-fix

skills/liuzhengdongfortest/codestable/cs-issue-fix
cs-issue-fix
Installation
...
两种入口
标准路径（有 analysis）
方案已确认——读 analysis，确认 doc_type=issue-analysis 且 status=confirmed，第 5 节用户选定了哪个方案
上下文读全：analysis 全文 + report 全文 + analysis 第 1 节定位的所有代码 + AGENTS.md + 沉淀目录搜索：
...
确认起点——告诉用户"我将按方案 X 修改 {文件列表}，开始修复"，等用户确认才动手
快速通道（无 analysis，从 report 直接触发）

进入这个入口时 AI 在 report 阶段已读过代码并对根因有把握。

明确陈述根因："{文件}:{行号} 的 {具体代码} 存在 {问题描述}"，让用户确认根因判断准确
给修复方案——改哪里、怎么改（一两句话，不写完整分析文档）
等用户明确说"对，就这样改"才动手——不允许"我觉得对，直接改了"
读 AGENTS.md
补搜沉淀目录——快速通道也要查一遍 compound/（trick + explore），避免误把已知边界条件当新问题
实现期间的约束
只改 analysis 里声明的文件
...
暴露了值得复用的坑点 → "沉淀 learning？（cs-learn）"
沉淀出长期约束 / 规约 / 技术决定 → "归档决定？（cs-decide）"
这个 bug 暴露了项目通用的硬约束 / 命令陷阱 / 环境设置（一两行能讲清、AI 每次会话都该知道）→ "记到 AGENTS.md？（cs-note）"
最后问是否代为提交。同意时按收尾提交规则执行
