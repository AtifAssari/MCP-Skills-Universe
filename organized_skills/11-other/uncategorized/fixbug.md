...
rating: ⭐⭐⭐
title: fixbug
url: https://skills.sh/tercel/code-forge/fixbug
---

# fixbug

skills/tercel/code-forge/fixbug
fixbug
Installation
...
Neither found: Show error — "No review report found in this session. Run /code-forge:review first, or /code-forge:review --save to persist to disk."
...
Present issue summary and confirm:
...
Use Glob to get project structure overview
Use Grep to search for keywords from the bug description in the codebase
Identify files and modules likely related to the bug
Note the tech stack and testing framework used
...
Search both {output_dir}/*/state.json and .code-forge/tmp/*/state.json for all features
For each feature, check if the bug-related files overlap with the feature's task files (read tasks/*.md → "Files Involved" sections)
Match found → load the feature's plan.md and relevant tasks/*.md as additional context. Note the feature name.
No match found → mark as standalone bug. Skip upstream trace-back (Steps 5 and 7). Proceed with code-only fix.
...
Read the feature's state.json
Add a fixes array (if not present) to the feature-level metadata
Append a fix record:
{
...
Update state.json updated timestamp
