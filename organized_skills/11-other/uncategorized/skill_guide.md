...
rating: ⭐⭐⭐
title: skill-guide
url: https://skills.sh/psw7205/skills/skill-guide
---

# skill-guide

skills/psw7205/skills/skill-guide
skill-guide
Installation
$ npx skills add https://github.com/psw7205/skills --skill skill-guide
SKILL.md
Skill Guide

이 레포에서 스킬을 만들거나 수정할 때 따르는 가이드. 상세 스펙은 references/guide.md 참조.
...
Claude가 이 작업을 기본적으로 잘 못하는 부분이 있는가? (있으면 스킬로 만들 가치가 높다)
이 작업을 할 때 매번 같은 설명을 반복하고 있는가?
...
스크립트에 #!/usr/bin/env bash와 set -euo pipefail 포함
SKILL.md에서 bash scripts/<name>.sh로 호출
...
도구 차단 외에 범위 제약도 포함 — self-feedback-loop는 "플랜에 포함된 변경만" 수정하도록 범위를 제한.
ref: trace-change-why의 읽기 전용 제약, self-feedback-loop의 플랜 기반 범위 제한
종료 조건 — 루프형 스킬의 탈출 기준
...
config.json을 스킬 디렉토리에 저장하는 패턴 사용
...
ref: session-history/SKILL.md의 "이력 관리" 섹션
상세 참조
