...
rating: ⭐⭐⭐⭐⭐
title: fix-review
url: https://skills.sh/s-hiraoku/synapse-a2a/fix-review
---

# fix-review

skills/s-hiraoku/synapse-a2a/fix-review
fix-review
Installation
...
Read the referenced file at the specified path
Locate the exact code referenced by the comment using:
...
Check for consistency with implementation — when a comment says "option X should be Y", verify by reading the actual argparse/CLI definition, not just the referenced doc line. Cross-reference:
...
Locate the exact code using symbol names (not just line numbers, which shift)
Analyze the comment body for:
...
After each fix, verify the change is correct — re-read the modified code to confirm it matches the intent
