...
rating: ⭐⭐⭐
title: ziflux-expert
url: https://skills.sh/neogenz/ziflux/ziflux-expert
---

# ziflux-expert

skills/neogenz/ziflux/ziflux-expert
ziflux-expert
Installation
...
Top Anti-Patterns
DataCache outside injection context → NG0203. Must be in class field, constructor, or runInInjectionContext.
DataCache in route-scoped service → cache dies on navigation, killing SWR. Always providedIn: 'root'.
staleTime > expireTime → constructor throws.
Expecting invalidate() to delete → it marks stale. get() still returns { data, fresh: false }.
invalidate([]) for full wipe → no-op. Use clear().
ref.value.set() → value is a read-only Signal, not WritableSignal. Use ref.set() or ref.update().
await mutate() then checking success → mutate() never rejects. Check mutation.error() signal or capture return value (undefined = error).
Component injecting API service directly → breaks the domain pattern. Component → Store → API.
Using Subjects/Observables for state → forbidden. Signals only.
Forgetting params: () => undefined suspends → resource stays 'idle', loader never fires.
Optimistic Updates
