...
rating: ⭐⭐⭐
title: reunion
url: https://skills.sh/yangdongchen66-boop/reunion-skill/reunion
---

# reunion

skills/yangdongchen66-boop/reunion-skill/reunion
reunion
Installation
$ npx skills add https://github.com/yangdongchen66-boop/reunion-skill --skill reunion
SKILL.md
Reunion Skill - 重逢
...
参考 ${SKILL_DIR}/prompts/memory_builder.md 生成记忆内容。 参考 ${SKILL_DIR}/prompts/persona_builder.md 生成人设内容。
...
重要：Claude Code 只通过 SKILL.md 的 name 字段匹配触发词。 为了让 /{slug}、/{slug}-memory、/{slug}-persona 都能工作， 需要创建 3 个独立的 skill 目录：
...
创建 ~/.claude/skills/{slug}/SKILL.md：

frontmatter 中 name: {slug}
包含完整的 PART A、PART B、运行规则

创建 ~/.claude/skills/{slug}-memory/SKILL.md：

frontmatter 中 name: {slug}-memory
只包含 memory.md 内容，用于查看

创建 ~/.claude/skills/{slug}-persona/SKILL.md：
...
读取新内容 + 现有文件
分析增量（参考 ${SKILL_DIR}/prompts/merger.md）
一次性更新所有相关文件
进化模式：对话纠正

用户表达"不对"/"ta 不会这样说"/"ta 应该是"时：

参考 ${SKILL_DIR}/prompts/correction_handler.md 识别纠正内容
