---
rating: ⭐⭐⭐
title: team-planex-v2
url: https://skills.sh/catlog22/claude-code-workflow/team-planex-v2
---

# team-planex-v2

skills/catlog22/claude-code-workflow/team-planex-v2
team-planex-v2
Installation
$ npx skills add https://github.com/catlog22/claude-code-workflow --skill team-planex-v2
SKILL.md
Auto Mode
...
Wave 1 (Planning): One PLAN-NNN task per issue, all independent (no deps), concurrent execution. Wave 2 (Execution): One EXEC-NNN task per issue, each depends on its corresponding PLAN-NNN.
...
Core Rules
Start Immediately: First action is session initialization, then input parsing
...
CSV First: Default to csv-wave for all tasks; interactive only for edge cases
...
Skip on Failure: If PLAN-N failed, skip EXEC-N automatically
Cleanup Temp Files: Remove wave-{N}.csv after results are merged
Two-Wave Pipeline: Wave 1 = Planning (PLAN-), Wave 2 = Execution (EXEC-)
