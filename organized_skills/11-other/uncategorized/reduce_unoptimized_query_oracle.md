---
rating: ⭐⭐⭐
title: reduce-unoptimized-query-oracle
url: https://skills.sh/cockroachdb/cockroach/reduce-unoptimized-query-oracle
---

# reduce-unoptimized-query-oracle

skills/cockroachdb/cockroach/reduce-unoptimized-query-oracle
reduce-unoptimized-query-oracle
Installation
...
Or, internal error or assertion failure. Note the error message for the reduce step.
Or, a panic. Note the error message for the reduce step.
...
Copy reduced.sql to reduced_v2.sql
Make ONE small change to reduced_v2.sql
Run ./test_repro.sh (which tests reduced_v2.sql)
If it reproduces: Copy reduced_v2.sql to reduced.sql, continue simplifying
If it doesn't reproduce after enough attempts: Discard reduced_v2.sql, try a different change (i.e. backtrack).
...
Optimizer random seed: SET testing_optimizer_random_seed = <value> - this specific value often cannot be changed, as it determines which optimizer rules are disabled
...
bisect_run.sh - A script for use with git bisect run
