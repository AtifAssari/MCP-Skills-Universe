...
rating: ⭐⭐⭐
title: ladybugdb
url: https://skills.sh/delexw/claude-code-misc/ladybugdb
---

# ladybugdb

skills/delexw/claude-code-misc/ladybugdb
ladybugdb
Installation
...
Key gotchas
SET n.prop = NULL to remove a property (not REMOVE)
label(n) not labels(n); id(n) not elementId(n)
UNWIND instead of FOREACH
List functions use list_ prefix: list_concat, list_sort, etc.
Variable-length paths must have an upper bound (default 30): [:Follows*1..5]
LOAD FROM (not LOAD CSV FROM) — supports CSV, Parquet, JSON, DataFrames
No manual index creation — primary key index is automatic; use FTS/vector extensions for search
Weekly Installs
10
Repository
