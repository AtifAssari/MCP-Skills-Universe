...
rating: ⭐⭐⭐
title: solo-build
url: https://skills.sh/fortunto2/solo-factory/solo-build
---

# solo-build

skills/fortunto2/solo-factory/solo-build
solo-build
Installation
...
docs/plan/{trackId}/plan.md — task list (REQUIRED). Read the ## Context Handoff section first — it has a compact summary of intent, key files, decisions, and risks. This is your primary orientation.
...
Output the error message clearly.
Suggest a fix (e.g., cargo clean, rm -rf node_modules, disk space check).
Output <solo:redo/> to let review handle it. Do NOT burn iterations on identical failures.
...
Keep test output concise — when running tests, pipe through head -50 or use --reporter=dot / -q flag. Thousands of test lines pollute context. Only show failures in detail. If output is large, use observation masking (write to scratch/, keep summary).
...
Recite the plan — re-read plan.md at the start of each task. Don't rely on memory of what comes next.
Common Issues
"No plans found"
