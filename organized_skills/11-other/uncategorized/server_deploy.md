...
rating: ⭐⭐⭐
title: server-deploy
url: https://skills.sh/majiayu000/claude-arsenal/server-deploy
---

# server-deploy

skills/majiayu000/claude-arsenal/server-deploy
server-deploy
Installation
$ npx skills add https://github.com/majiayu000/claude-arsenal --skill server-deploy
SKILL.md
通用服务器部署
...
Rust 二进制路径：$REMOTE_DIR/target/release/$PROJECT_NAME（方案 A）或 $REMOTE_DIR/$PROJECT_NAME（方案 B） Go 二进制路径：$REMOTE_DIR/$PROJECT_NAME
...
Cloudflare Origin Certificate（推荐） — DNS 托管在 Cloudflare，证书有效期 15 年
Let's Encrypt — 免费公共证书，需定期续期
暂不配置 SSL
Cloudflare Origin Certificate 流程：

告知用户操作：Cloudflare → SSL/TLS → 源服务器 → 创建证书

主机名确保包含 $DOMAIN 和 *.$DOMAIN
私钥类型：RSA，有效期：15 年

用户提供证书和私钥后，写入服务器并拼接证书链：
...
如果缺少，告知用户重新创建
Cloudflare SSL 模式：先设 Full (Strict)，如果 526 错误则降级为 Full
Let's Encrypt 流程：
