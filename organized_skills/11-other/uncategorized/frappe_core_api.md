---
rating: ⭐⭐⭐
title: frappe-core-api
url: https://skills.sh/openaec-foundation/erpnext_anthropic_claude_development_skill_package/frappe-core-api
---

# frappe-core-api

skills/openaec-foundation/erpnext_anthropic_claude_development_skill_package/frappe-core-api
frappe-core-api
Installation
...
Critical Rules
ALWAYS include Accept: application/json header in API requests
ALWAYS add permission checks in @frappe.whitelist() methods
ALWAYS validate and sanitize input in whitelisted methods
ALWAYS use parameterized queries — NEVER string-interpolate SQL
ALWAYS use timeout=30 on external requests calls
ALWAYS store credentials in frappe.conf or env vars — NEVER hardcode
ALWAYS verify webhook signatures with HMAC-SHA256
ALWAYS paginate list responses — NEVER return unbounded result sets
NEVER use allow_guest=True on state-changing endpoints
