...
rating: ⭐⭐
title: copilot-sdk
url: https://skills.sh/intellectronica/agent-skills/copilot-sdk
---

# copilot-sdk

skills/intellectronica/agent-skills/copilot-sdk
copilot-sdk
Installation
...
Treat the top-level SDK README plus docs/ as the source of truth for shared behavior.
Call out preview status when stability or breaking changes matter.
Avoid hardcoding model lists when runtime discovery via listModels() is available.
...
Register event handlers before send() if you need streaming or progress.
Send with send() or sendAndWait().
Wait for session.idle or the returned final message.
