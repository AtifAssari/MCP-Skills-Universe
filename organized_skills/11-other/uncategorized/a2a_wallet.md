...
rating: ⭐⭐⭐
title: a2a-wallet
url: https://skills.sh/planetarium/a2a-x402-wallet/a2a-wallet
---

# a2a-wallet

skills/planetarium/a2a-x402-wallet/a2a-wallet
a2a-wallet
Installation
$ npx skills add https://github.com/planetarium/a2a-x402-wallet --skill a2a-wallet
SKILL.md
a2a-wallet Skill
...
Send a message → agent replies with task.status = input-required and metadata["x402.payment.status"] = "payment-required" plus metadata["x402.payment.required"] containing PaymentRequirements
Sign the requirements with x402 sign:
...
Submit payment by sending back with --task-id and --metadata:
a2a-wallet a2a send \
  --task-id <task-id> \
  --metadata "$METADATA" \
