---
rating: ⭐⭐
title: homelab-add-service
url: https://skills.sh/jamesperenchio1/homelab-add-service/homelab-add-service
---

# homelab-add-service

skills/jamesperenchio1/homelab-add-service/homelab-add-service
homelab-add-service
Installation
$ npx skills add https://github.com/jamesperenchio1/homelab-add-service --skill homelab-add-service
SKILL.md
homelab-add-service
...
Create an Application in Authentik (via API or UI) with slug matching the subdomain
Create a Proxy Provider with:
external_host: https://<subdomain>.gingerbrosshop.com
mode: forward_single
Link the provider to the application
Restart Authentik server so the embedded outpost picks up the new config:
cd /opt/authentik && sudo docker compose restart server


For native OIDC:

Create an OAuth2/OpenID Provider in Authentik with:
...
Configure the service with the OIDC credentials
Restart Authentik server: cd /opt/authentik && sudo docker compose restart server
...
6. Verify
Check Caddy config is valid: caddy validate --config /etc/caddy/Caddyfile
Check the service is reachable locally: curl -s -o /dev/null -w "%{http_code}" http://<backend-address>/
Check the public URL works: curl -s -o /dev/null -w "%{http_code}" -L https://<subdomain>.gingerbrosshop.com/
If using forward auth, verify redirect to Authentik login page
