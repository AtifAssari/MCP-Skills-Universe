...
rating: ⭐⭐
title: using-vit
url: https://skills.sh/solpbc/vit/using-vit
---

# using-vit

skills/solpbc/vit/using-vit
using-vit
Installation
...
Identify what to ship — summarize the feature or fix you just completed. A cap describes a self-contained capability, not a commit message.
Craft the fields:
...
Ship will validate prerequisites (DID, beacon, session) and give actionable errors if anything is missing.
vit beacon <target>
...
vit vet <ref> - Human review command. Tell the user to run it in their terminal. Exception: sandboxed sub-agent vetting. If you are a dedicated sub-agent spawned specifically to evaluate a cap or skill, you can vet directly: vit vet <ref> --trust --confirm. Only do this if:

You are running as an isolated sub-agent (not the primary agent)
You have read and evaluated the full cap/skill content
Your parent agent specifically tasked you with vetting
