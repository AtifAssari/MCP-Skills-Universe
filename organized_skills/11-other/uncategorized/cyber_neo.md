...
rating: ⭐⭐⭐
title: cyber-neo
url: https://skills.sh/hainrixz/cyber-neo/cyber-neo
---

# cyber-neo

skills/hainrixz/cyber-neo/cyber-neo
cyber-neo
Installation
$ npx skills add https://github.com/hainrixz/cyber-neo --skill cyber-neo
SKILL.md
...
If Trivy is available: trivy fs --scanners vuln {TARGET_DIR} --format json --quiet

If npm is available and package.json exists: cd {TARGET_DIR} && npm audit --json 2>/dev/null (NOTE: npm audit is read-only — it does NOT modify anything)

If pip-audit is available and requirements.txt exists: pip-audit -r {TARGET_DIR}/requirements.txt --format json 2>/dev/null

If cargo-audit is available and Cargo.lock exists: cd {TARGET_DIR} && cargo audit --json 2>/dev/null

If NO tools are available, report: "Dependency vulnerability scanning requires external tools. Install one of:
...
If Semgrep is available: semgrep scan --config auto --json --quiet {TARGET_DIR} Parse and include Semgrep findings.
...
Run the Cyber Neo secret scanner: python3 {ABSOLUTE_PATH_TO_SCRIPTS}/scan_secrets.py {TARGET_DIR} (Use the absolute script path resolved in Step 1.3) Parse the JSON output.

If Gitleaks is available: gitleaks detect --source {TARGET_DIR} --report-format json --no-banner 2>/dev/null Parse and merge findings (deduplicate by file+line).
...
6a. Lock File Integrity: Run: python3 {ABSOLUTE_PATH_TO_SCRIPTS}/check_lockfiles.py {TARGET_DIR} (Use the absolute script path resolved in Step 1.3) Parse the JSON output.
...
Executive Summary — risk score, severity counts, top 3 priority actions
Critical & High Findings — each with CWE, OWASP, file:line, evidence, and remediation code
Medium Findings — same format
Low & Informational Findings — can be more concise
Dependency Vulnerabilities — table if SCA ran, or note about missing tools
Supply Chain Assessment — lock file status, dependency pinning, CI/CD
Scan Metadata — scanner version, duration, tools used, files scanned/skipped, coverage %
...
Generic SAST patterns (hardcoded creds, eval, exec, etc.) Report that language-specific analysis is limited.
