---
rating: ⭐⭐⭐
title: apaas-object-skill
url: https://skills.sh/ennann/apaas-object-skill/apaas-object-skill
---

# apaas-object-skill

skills/ennann/apaas-object-skill/apaas-object-skill
apaas-object-skill
Installation
$ npx skills add https://github.com/ennann/apaas-object-skill --skill apaas-object-skill
SKILL.md
aPaaS 数据对象管理
...
schema.create 静默忽略 fields — 传了字段定义也返回成功，但字段不会被创建。必须两步走：先 create 空壳，再 update 添加字段。
text/multilingual 缺 multiline = 整批静默失败 — API 返回 code: "0" + data: null，看似成功但所有字段都没创建。始终用完整 settings 模板。
metadata type ≠ schema type — number 要写 float，option 要写 enum，file 要写 attachment。用错直接报类型不识别。
系统字段 _id/_createdBy 等不能手动创建 — 新建对象自动生成 5 个系统字段，重复定义会冲突报错。
批量上限 10 个对象 — schema.create/update/delete 每次最多 10 个，超过需分批。
前置条件

使用此 skill 前，必须确保以下环境已就绪：

安装 Node.js（>= 16）
安装 SDK：npm install apaas-oapi-client@^0.1.37（schema 模块从 0.1.37 开始提供，低版本会报 Cannot read properties of undefined）
获取凭据（在 aPaaS 平台的应用管理中获取）：
...
不要创建为 aPaaS 对象
在关系的某一方创建 lookup（multiple: true）指向另一方
选择哪一方加 lookup 时，优先选择业务上"主动关联"的一方（如学生选课 → 在 student 上加 lookup 指向 course）
如果中间表有额外业务字段（如 score、enrolled_at），则必须保留为独立对象，两端各用一个 lookup
...
提取所有表（→ aPaaS 对象）和列（→ aPaaS 字段）
识别主键列 → 忽略（aPaaS 用 _id）
识别外键列 → 标记为 lookup 候选
识别中间表 → 标记为"消除"或"保留"
识别 ENUM 列 → 提取枚举值列表
对每个列按「SQL 列类型映射表」和「语义识别规则」确定 aPaaS 类型
...
列出所有对象和字段，标记哪些字段是 lookup / lookup_multi / reference_field
画出依赖关系：lookup 的 referenced_object_api_name 指向谁
分类到各阶段：
...
始终用此流程，即使没有循环依赖也不要尝试在 create 中传 fields
删除顺序（逆向）

删除时必须反向操作：

先删 reference_field（依赖 lookup）
再删 lookup / lookup_multi（依赖目标对象）
最后删对象本身
删除所有对象（完整工作流）
...
lookup_multi（multiple: true）不能作为 reference_field 的引导字段
...
result.code !== '0' — 请求级错误（参数格式问题）
result.code === '0' && result.data === null — ⚠️ 静默失败（最危险：看起来成功，但字段未创建。原因：settings 缺必填项）
result.data.items[].status.code !== '0' — 单个对象级错误
