...
rating: ⭐⭐⭐
title: pr
url: https://skills.sh/jacehwang/skills/pr
---

# pr

skills/jacehwang/skills/pr
pr
Installation
...
Branch guard: If the current branch is the default branch, inform the user that PRs cannot be created from the default branch and stop.
Commit guard: If there are no commits since the default branch (git log <default-branch>..HEAD is empty), inform the user there are no changes to submit and stop.
Uncommitted changes: If there are uncommitted changes, warn the user and ask if they want to commit first. If the user declines to commit, inform the user that uncommitted changes will not be included in the PR and continue.
Push status: If the branch is not pushed or is behind remote, push it with git push -u origin <branch>. If git push fails, inform the user with the error message and stop.
...
Read the existing body from repository context.
Preserve user-authored content: Any text that does not match the generated template patterns (e.g., manually added review notes, discussion context, custom sections) MUST be retained.
Expand the summary: If new commits add functionality, extend the summary to cover the new scope. If new commits are fixes to existing changes, keep the summary as-is.
Replace the commit list entirely: Regenerate the full commit list from all commits since the default branch.
Reassess complexity: If new commits push the PR into a higher complexity level, upgrade the body template accordingly.
...
Title is in English and 40 characters or fewer (excluding prefix).
Every commit hash from repository context appears in the body.
Body structure matches the complexity level from Step 2.
