...
rating: ⭐⭐
title: code2patent
url: https://skills.sh/cat-xierluo/legal-skills/code2patent
---

# code2patent

skills/cat-xierluo/legal-skills/code2patent
code2patent
Installation
$ npx skills add https://github.com/cat-xierluo/legal-skills --skill code2patent
SKILL.md
代码仓库转专利交付
概述
...
读取代码仓库并识别相关模块
将候选技术方案映射到代码中的具体实现
标注哪些技术特征已有直接代码证据，哪些仍需研发补充
输出适合研发人员和专利代理师协作的技术交底书
扩展场景 A：代码到可专利方案挖掘
...
先生成《权利要求布局卡》与《权利要求-代码证据矩阵》
再据此生成完整初稿与《发明专利初稿自检表》
...
是否已有人工整理的候选专利申请清单或技术方案清单
如果有，该清单中哪些方案优先级最高
本次任务的主题是什么（用于 archive/ 归档目录命名）
是否有 PRD、产品方案、需求说明或其他能解释项目目标的文档
目标输出层级是什么（技术交底书 / 权利要求布局卡 / 发明专利初稿）
如果目标是初稿，是否需要直接输出完整初稿（默认仍先生成布局卡和证据矩阵）
是否有客户或代理师指定的交底书模板、初稿模板或特定写法要求
如果没有候选清单，是否有希望优先关注的业务方向；若没有，则默认由 AI 先自动挖掘候选方案清单
...
围绕已知方向做针对性搜索和证据提取
输出代码证据映射、技术交底书
如用户需要起草层，则继续输出《权利要求布局卡》与《权利要求-代码证据矩阵》
在上述中间产物完成后，再输出《发明专利初稿》和《发明专利初稿自检表》
情况 B：用户没有候选专利申请清单

进入“候选方案挖掘模式”，但不应直接生成技术交底书或专利初稿。应先：

先自动读取代码仓库并做项目边界与依赖画像
生成《候选可专利方案清单》
对每个候选方案给出简要依据、优先级建议和不确定点
明确提示用户先进行人工筛选、合并、拆分或排序
形成《人工筛选确认记录》
在人工筛选完成后，回到“定向检索模式”，再继续做代码映射、技术交底书、权利要求布局或专利初稿
...
优先读取 README、PRD/需求说明、架构文档等高层材料
盘点代码结构、入口、核心模块和关键数据流
将候选方案与模块、接口、类、函数、配置进行映射
提取技术特征与实现证据
先生成交底书与权利要求布局材料，再生成专利初稿
...
代码仓库路径或核心模块路径
候选专利方案清单
PRD、产品方案、需求说明或至少一种能解释项目目标的材料
每个方案对应的目标效果或待解决技术问题
是否只做技术交底书，还是继续扩展为权利要求布局 / 中国发明专利初稿
如需初稿，至少补充主要申请主体、发明人候选、优先级信息和是否存在客户格式要求
...
PRD、产品方案、需求说明
客户或技术人员的沟通纪要、录音转写
客户或代理师提供的 Word 版技术交底书模板
客户或代理师提供的初稿模板、权利要求布局习惯、既有申请样式
补充商务材料（费用、国家布局、交付要求等）
代码仓库本身
...
若用户提供了 Word 版 技术交底书模板.docx 或同类模板文件，优先按该模板章节输出技术交底书
若用户没有提供技术交底书模板，则使用 templates/invention-patent-disclosure-template.md
若用户要求形成权利要求布局，则使用：
templates/invention-patent-claim-layout-template.md
templates/invention-patent-claim-evidence-matrix-template.md
若用户要求形成中国发明专利申请文件初稿，则使用：
...
先读用户提供的候选清单、PRD/需求说明、沟通纪要和模板，确定项目目标与客户预期
再读代码仓库中的 README、架构文档和目录说明，建立项目的高层理解
再做项目边界与依赖画像，明确哪些是自研、哪些是第三方
再进入代码仓库做模块映射和证据提取
先按客户模板或内置交底书模板生成技术交底书
再按起草规范生成权利要求布局卡、证据矩阵和初稿
详细规范
...
默认同时准备“方法 + 系统 / 装置 + 计算机可读存储介质 + 计算机程序产品”四类平行布局口径，供代理师择优整合
独立权利要求骨架只能吸收 A 级证据对应的必要技术特征
B 级证据默认优先写入从属权利要求或优选实施例，并标注“基于实现归纳”
C 级证据只能写入待确认事项或替代实施例，不能写成既有实现
...
说明书章节顺序、术语口径和摘要要求按《专利审查指南》第二部分第二章“说明书和权利要求书”的写法组织
初稿默认为“接近可申报版”，但仍必须保留“待研发确认 / 待代理师修订”清单
涉及计算机程序的方案，默认允许输出方法权利要求以及产品权利要求草稿；产品权利要求应优先按装置、计算机可读存储介质、计算机程序产品三类保护主题进行布局
不做正式请求书表格自动填写，只做待填字段清单
步骤 13：生成《发明专利初稿自检表》
...
一个候选方案对应一份独立证据映射
每条技术特征都要回到具体模块、调用链、数据流或配置项
无直接证据的内容不得写成既有实现
无法明确归属的第三方能力不得直接写成申请人的核心创新
A 级证据特征才可进入独立权利要求骨架
B 级证据特征默认进入从属权利要求或说明书优选实施例，并标注来源性质
C 级证据特征只能进入“待确认事项”或“替代实施例”
第六部分：输出层级与交付清单
输出层级
层级	适用场景	主要产物
...
archive/YYYY-MM-DD-主题/00-输入材料摘要.md
archive/YYYY-MM-DD-主题/01-项目边界与依赖画像.md
archive/YYYY-MM-DD-主题/02-项目代码技术盘点.md
archive/YYYY-MM-DD-主题/03-方案代码证据映射-<方案名>.md
archive/YYYY-MM-DD-主题/04-技术交底书-<方案名>.md
archive/YYYY-MM-DD-主题/04A-权利要求布局卡-<方案名>.md
archive/YYYY-MM-DD-主题/04B-权利要求-代码证据矩阵-<方案名>.md
archive/YYYY-MM-DD-主题/05-发明专利初稿-<方案名>.md
archive/YYYY-MM-DD-主题/05A-发明专利初稿自检表-<方案名>.md
archive/YYYY-MM-DD-主题/06-研发补充问题清单.md

如果是“可专利方案挖掘模式”，可改为：

archive/YYYY-MM-DD-主题/00-输入材料摘要.md
archive/YYYY-MM-DD-主题/01-项目边界与依赖画像.md
archive/YYYY-MM-DD-主题/02-项目技术盘点.md
archive/YYYY-MM-DD-主题/03-候选可专利方案清单-待人工筛选.md
archive/YYYY-MM-DD-主题/04-优先申请建议.md
archive/YYYY-MM-DD-主题/05-人工筛选确认记录.md
归档要求
...
一次任务对应一个归档目录
同一归档目录下同时保存输入摘要、中间分析和最终产物
目录名优先体现日期和主题，保证不同项目可以直观看出差异
第七部分：参考模板与规范

当前目录按职责分成两组：

templates/
invention-patent-disclosure-template.md
invention-patent-claim-layout-template.md
invention-patent-claim-evidence-matrix-template.md
invention-patent-draft-template.md
invention-patent-draft-self-check-template.md
references/
project-analysis-spec.md
code-extraction-spec.md
patent-drafting-spec.md

其中：

templates/ 用于最终交付或中间交付的模板
references/ 用于项目分析、代码抽取和起草执行规范
第八部分：与研发和代理师的协作要点
面向研发人员
...
第九部分：注意事项
证据优先：不要把没有代码依据的设想写成既有实现
一个发明点一份交底书：避免把多个创造性方案混写
保密意识：专利申请前需注意保密，避免过早公开技术方案
新颖性与创造性：交底书只是内部材料，不等于一定具备授权前景，必要时应先做专利检索
充分公开：技术交底书应足以让代理师和技术人员继续展开撰写与补充
两步写作默认开启：只要目标是初稿，默认先出“权利要求布局卡 + 证据矩阵”，再出完整初稿
专业审查：权利要求和正式申请文本仍需专利代理师审核
效果克制：没有实验数据或线上指标支持的效果，不要随意量化夸大
软件专利默认发明专利：算法、流程、调度、记忆、鉴权、编排等方案，默认按发明专利组织材料
先分清自研与外部依赖：不要把第三方库、模型平台或中间件的固有能力误写为申请人的创新点
优先贴合客户模板：若用户提供了 Word 版交底书模板，应优先按其章节组织输出；如无客户模板，则选用内置交底书模板或初稿模板
摘要限制：说明书摘要默认控制在 300 字内，且不使用商业性宣传语言
请求书仅做字段清单：首版只列待填字段，不自动生成正式官表
计算机程序相关方案默认并行准备多种保护主题：默认准备方法、系统 / 装置、计算机可读存储介质、计算机程序产品等保护主题草稿，但最终提交策略由代理师结合单一性和审查策略决定
输入/输出
输入
