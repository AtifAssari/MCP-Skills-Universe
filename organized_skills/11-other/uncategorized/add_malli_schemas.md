---
rating: ⭐⭐⭐
title: add-malli-schemas
url: https://skills.sh/metabase/metabase/add-malli-schemas
---

# add-malli-schemas

skills/metabase/metabase/add-malli-schemas
add-malli-schemas
Installation
...
Route Params (the 5 in api/user/id/5)
Query Params (the sort+asc pair in api/users?sort=asc)
Body Params (the contents of a request body. Almost always decoded from json into edn)
The Raw Request map
...
Add parameter schemas - use existing types from ms
Determine return type - check the implementation
Define response schema - inline or named with mr/def
Test - ensure the endpoint works and validates correctly
