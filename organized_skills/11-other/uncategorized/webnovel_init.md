...
rating: ⭐⭐
title: webnovel-init
url: https://skills.sh/lingfengqaq/webnovel-writer/webnovel-init
---

# webnovel-init

skills/lingfengqaq/webnovel-writer/webnovel-init
webnovel-init
Installation
...
允许调用 Read/Grep/Bash/Agent/AskUserQuestion/WebSearch/WebFetch 辅助收集。
用户已明确的信息不重复问；冲突信息优先让用户裁决。
Deep 模式优先完整性，允许慢一点，但禁止漏关键字段。
参考书拆解只返回结构化结果给 init 主流程；用户确认前不得写入 idea_bank.json、.story-system、设定集、大纲、正文、.webnovel/state.json 或任何 canon/read model 文件。
引用加载策略
...
汇总 Step 1.5 已确认的灵感来源：原创想法、参考拆书结果、市场趋势、题材模板或反套路库。
