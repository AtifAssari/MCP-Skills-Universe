---
rating: ⭐⭐
title: workflow-ship-faster
url: https://skills.sh/heyvhuang/ship-faster/workflow-ship-faster
---

# workflow-ship-faster

skills/heyvhuang/ship-faster/workflow-ship-faster
workflow-ship-faster
Installation
...
Resolve/create run_dir/ (see the deterministic backend rules in references/artifact-contract.md).
Ensure core artifacts exist (create if missing; merge/append if already present):
...
Only create evidence/ and logs/ when you actually have large outputs to store.
...
Call workflow-feature-shipper to generate the plan only (no implementation yet).
Defer actual UI implementation until after Step 2 (design-system.md exists), so the demo moment follows the chosen design system.
