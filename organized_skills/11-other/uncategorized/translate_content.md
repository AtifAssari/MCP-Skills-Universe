...
url: https://skills.sh/indranilbanerjee/digital-marketing-pro/translate-content
---

# translate-content

skills/indranilbanerjee/digital-marketing-pro/translate-content
translate-content
Installation
...
Detect source language: If the source language was not specified, run python scripts/language-router.py --action detect --text "{content_or_path}" to identify the source language with confidence score. Report the detected language to the user for confirmation if confidence is below 95%.
...
Score translation quality: Run python scripts/language-router.py --action score --source "{source}" --target "{target}" --original "{source_content}" --translated "{translated_content}" to assess quality across dimensions:
...
Run brand voice check: Execute python scripts/brand-voice-scorer.py --brand {slug} --text "{translated_content}" to assess whether the translated content maintains brand voice characteristics. Flag any voice drift with specific examples and suggestions.
Present translated content with quality metrics: Deliver the final translated content alongside all quality data, formatted for easy review and approval.
Output
