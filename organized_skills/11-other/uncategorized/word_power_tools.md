---
rating: ⭐⭐
title: word-power-tools
url: https://skills.sh/chi111i/word-power-tools/word-power-tools
---

# word-power-tools

skills/chi111i/word-power-tools/word-power-tools
word-power-tools
Installation
...
原则（减少风险 + 提高可复现性）
默认不覆盖原文件：所有会改写的操作必须写到新输出文件（-o/--output）。
先 lint 后 format：排版类任务先生成报告，再应用修复（避免"修坏了但不知道改了什么"）。
复杂任务用脚本输出：脚本可"零上下文执行"，输出更稳定、可测试。
Quickstart
