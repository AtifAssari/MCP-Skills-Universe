---
rating: ⭐⭐
title: wechatpay-basic-payment
url: https://skills.sh/wechatpay-apiv3/wechatpay-skills/wechatpay-basic-payment
---

# wechatpay-basic-payment

skills/wechatpay-apiv3/wechatpay-skills/wechatpay-basic-payment
wechatpay-basic-payment
Installation
$ npx skills add https://github.com/wechatpay-apiv3/wechatpay-skills --skill wechatpay-basic-payment
SKILL.md
微信支付基础支付 & 合单支付接入指引
全局交互规范
...
接入模式前置确认：任何能力使用前须先确认商户模式或服务商模式，已明确则无需重复。两种模式的核心差异见 → 📄 接入模式说明.md。
...
能力概览
产品选型 — 根据场景推荐支付方式（JSAPI/APP/H5/Native/小程序/付款码），判断是否需要合单支付
示例代码 — 各接口的下单、调起、回调、退款、账单等代码结构示例（只展示不写入）
业务知识速查 — 订单状态、退款规则、账单对账、APPID绑定、特约商户进件、开户意愿确认等
接入质量评估 — 签名验签、业务逻辑完整性、回调处理规范性检查（含合单/分账/进件/开户意愿确认专项）
问题排查 — 下单失败、调起异常、回调收不到、退款失败等（含合单支付专项常见问题）
