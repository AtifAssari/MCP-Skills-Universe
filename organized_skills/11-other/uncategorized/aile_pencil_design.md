---
rating: ⭐⭐
title: aile-pencil-design
url: https://skills.sh/zhucl1006/ailesuperpowers/aile-pencil-design
---

# aile-pencil-design

skills/zhucl1006/ailesuperpowers/aile-pencil-design
aile-pencil-design
Installation
...
open_document
get_editor_state
get_guidelines（按场景选择）
batch_get（一次性检索可复用组件）
get_variables（缺失时再 set_variables 补齐最小 token）
Step 2：分批构图（结构优先）
...
snapshot_layout（problemsOnly=true）检查重叠/裁剪/越界
snapshot_layout（常规）复核结构
get_screenshot 抽样关键节点截图（至少主流程 + 异常态）
