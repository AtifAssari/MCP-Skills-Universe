---
rating: ⭐⭐⭐
title: youmind-wechat-article
url: https://skills.sh/youmind-openlab/skills/youmind-wechat-article
---

# youmind-wechat-article

skills/youmind-openlab/skills/youmind-wechat-article
youmind-wechat-article
Installation
...
Install & configure: cd toolkit && npm install && npm run build && cd .. && pip install -r requirements.txt && cp config.example.yaml config.yaml
Get YouMind API Key → fill youmind.api_key in config.yaml
Get WeChat AppID & AppSecret from 微信开发者平台 → fill wechat.appid and wechat.secret, add your public IP (curl -s https://ifconfig.me) to the API IP whitelist
...
Google Gemini (Imagen)	Google AI Studio 创建 API key	image.providers.gemini.api_key
OpenAI (GPT Image)	OpenAI Platform 创建 API key	image.providers.openai.api_key
豆包 (Seedream)	火山引擎控制台 创建 API key	image.providers.doubao.api_key
...
references/theme-dsl.md	Custom theme design language (integrates Impeccable if installed)	When creating custom themes — auto-detects & offers to install Impeccable
...
Read references/writing-guide.md BEFORE writing. The pre-writing framework and de-AI protocol are mandatory.
Zero AI-sounding text. Run the full 4-level de-AI protocol from writing-guide.md.
...
Obey the client's blacklist — both words and topics. No exceptions.
Playbook overrides writing-guide. If playbook.md exists for this client, it takes priority for voice and style decisions.
Before generating visuals, proactively ask about image scope and style. Do not silently assume. If the host supports AskUserQuestion, use it. Otherwise ask a concise plain-text question.
Always publish to drafts. Step 7 publishes directly to WeChat draft box. Do NOT ask — this is mandatory and automatic.
Pipeline Overview
...
References
YouMind API: see references/openapi-document.md
