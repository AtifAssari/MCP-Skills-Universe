...
url: https://skills.sh/sixtysecondsapp/use60/search-documentation
---

# search documentation

skills/sixtysecondsapp/use60/Search Documentation
Search Documentation
Installation
$ npx skills add https://github.com/sixtysecondsapp/use60 --skill 'Search Documentation'
SKILL.md
...
Remove stop words: "how do I connect my Google calendar" -> "connect Google calendar"
Expand abbreviations: "cal" -> "calendar", "CRM" -> keep as-is (it's a known term)
Include synonyms: "link" -> also search "connect", "integrate", "sync"
Preserve proper nouns: "Google Calendar", "Slack", "HubSpot" -- keep exact casing
Extract the subject: The thing the user is asking about (feature, integration, setting)
Extract the action: What they want to do with it (set up, configure, troubleshoot, understand)
...
Read the top 3 articles (by relevance score)
Identify the answer paragraph(s) -- the specific section that addresses the user's question
Compose a direct answer in 2-5 sentences using information from the articles
Cite your sources -- reference which article each piece of information comes from
Provide the "read more" link for users who want the full context
...
Synthesis Strategy
Identify the primary article -- the one most directly answering the question
Identify supporting articles -- prerequisites, related configuration, troubleshooting
Compose the answer starting from the primary article
Weave in supporting context where needed, clearly citing each source
Present a unified narrative -- the user should not be able to tell the answer came from multiple sources
...
To connect Google Calendar, go to Settings > Integrations > Google Calendar and click "Connect." You'll need to authorize with your Google account and select which calendars to sync. (Source: Google Calendar Integration Guide)

Once connected, your calendar events will automatically appear in the meetings section. To see them in your pipeline view, each meeting must be linked to a deal. You can link meetings manually from the deal page, or enable auto-linking in Settings > Meetings > Auto-link Rules. (Source: Meeting-Deal Linking)
...
Acknowledge the gap honestly: "I couldn't find documentation on that specific topic."
Suggest related topics that DO exist: "However, I found articles on [related topic] that might help."
...
If the feature might not exist: "I don't have documentation for that feature. It's possible this isn't currently supported. Check the feature request board or ask support."
Never Say
...
Try the parent/related category
Try a full-text search across all categories
Fall back to "not found" guidance
...
Related topics that DO have documentation
Alternative paths (browse docs, contact support)
Suggest a reformulated question
