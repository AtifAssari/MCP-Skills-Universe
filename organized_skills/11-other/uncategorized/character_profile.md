...
url: https://skills.sh/evan-acg/evan-skills/character-profile
---

# character-profile

skills/evan-acg/evan-skills/character-profile
character-profile
Installation
$ npx skills add https://github.com/evan-acg/evan-skills --skill character-profile
SKILL.md
小说人物档案创建工具
概述
...
自动目录创建：自动创建00_Prepare、02_LoreBible/Characters等标准目录结构
...
新脚本模块
lore_bible_manager.py - LoreBible目录管理和角色扫描
conflict_detector.py - 冲突检测和常识校验
profile_session.py - 会话管理和用户确认流程
subagent_orchestrator.py - 子代理工作流协调
向后兼容性
...
目录权限：确保工作目录有读写权限
配置文件缺失：config/目录需要包含validation_rules.json和workflow_tasks.json
日志查看
