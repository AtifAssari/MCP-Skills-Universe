...
url: https://skills.sh/hubeiqiao/apple-bento-grid/apple-bento-grid
---

# apple-bento-grid

skills/hubeiqiao/apple-bento-grid/apple-bento-grid
apple-bento-grid
Installation
...
Workflow
Read the design system selectively: Reference design-system.md as needed:
...
Choose a theme — suggest light, dark, or both based on the context (see Theme Selection below)
Choose a layout — pick from the 3 layout templates based on card count and orientation
Compose cards — select card types and fill with the user's content; prioritize density (no empty-feeling cards)
Ask about logos/images — if the user has a logo or images, incorporate them (see Logos & Images below)
Generate the HTML — produce a single self-contained HTML file
Visual review — open the HTML in a browser and screenshot it to check for issues (see Visual Review below)
Offer a vertical version — after generating a landscape grid, ask: "Want a vertical (portrait) version for social media?" and generate a Template C adaptation if yes
...
NEVER set align-items: start — default stretch fills cells
Use aspect-ratio on horizontal layouts to lock container shape
Rows: 1fr for horizontal, auto for vertical
Gap: 6px (not 8px)
...
Choose theme — light, dark, or both
...
Verify density — no empty-feeling cards, no unused grid cells, gap 6px
...
Open in Chrome or Safari — verify all grid cells are filled with no visible row gaps
Check font loading — Sora and DM Sans should render (not system fallback)
For dark theme — verify card backgrounds are #1a1a1a, not #fff
Screenshot at the correct viewport width (1200/1100/600) at 2x resolution
Confirm density — no card should have excessive whitespace
Notes
...
Reference Files
design-system.md — Complete design tokens (light + dark), card CSS/HTML, layout templates, and skeleton
scripts/screenshot.mjs — Playwright screenshot capture script
Weekly Installs
126
Repository
hubeiqiao/apple…nto-grid
GitHub Stars
164
First Seen
Mar 17, 2026
