...
url: https://skills.sh/rajivpant/synthesis-skills/synthesis-pr-review
---

# synthesis-pr-review

skills/rajivpant/synthesis-skills/synthesis-pr-review
synthesis-pr-review
Installation
...
Does this change do what it claims? — Read the PR description. Read the code. Do they match?
Does it introduce regressions? — What worked before that might break now?
Is it the right fix? — Does it address root cause, or a symptom?
Is it complete? — Or does it need companion changes to actually solve the problem?
Is it consistent? — Does it follow existing patterns, or does it diverge without justification?
Review Checklist
...
Read the cited code yourself. Do not rely on someone else's summary of what the code does.
Look for alternative code paths. The analysis may describe one path accurately while missing another that handles the same input differently (error handlers, fallback logic, middleware, decorators).
Be skeptical of sweeping conclusions. Phrases like "zero effect," "completely broken," or "never works" are almost always wrong. Reality is usually more nuanced.
Check the system-level view. A function-level analysis may be correct in isolation but miss interactions with caching, middleware, event handlers, or background jobs that change the behavior.
Test the conclusion, not just the intermediate steps. If the analysis says "changing X will fix the bug," verify that claim independently before acting on it.
...
Does the code make sense? — Can you follow the logic without the author explaining it?
Does it match the PR description? — If not, which is wrong — the code or the description?
Would you be comfortable debugging this at 2 AM? — If no, the code needs to be clearer.
Check the analog. — Find the closest similar code in the codebase. Does this PR follow the same pattern?
...
Project-specific standards — white-labeling compliance, UI terminology, deployment safety
Architectural fit — does this change move the codebase in the right direction?
Integration complexity — what will the adopt-and-adapt process look like?
Completeness of the solution — does this fully solve the problem, or is it a partial fix?
...
A project-level CLAUDE.md or equivalent configuration — These files often encode naming rules, terminology requirements, deployment constraints, and other conventions that are not enforced by linters.
Project-level review skills or checklists — Some projects define their own review criteria that supplement this skill.
Convention debt patterns — Recurring violations that the project is actively trying to eliminate.
...
If the PR is clean — merge directly (rare for synthesis-coded projects, but possible as contributor quality improves)
If the PR needs adaptation — the lead synthesist creates an integration branch, applies the adopt-and-adapt pattern, and merges the adapted version
If the PR needs follow-up work — merge what is ready, create tickets for the remaining work, and document the dependency
