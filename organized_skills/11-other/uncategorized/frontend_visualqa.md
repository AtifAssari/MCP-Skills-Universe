---
rating: ⭐⭐
title: frontend-visualqa
url: https://skills.sh/yutori-ai/frontend-visualqa/frontend-visualqa
---

# frontend-visualqa

skills/yutori-ai/frontend-visualqa/frontend-visualqa
frontend-visualqa
Installation
...
Prefer the frontend-visualqa MCP tools when available. They keep browser state warm across calls.
If the page is auth-gated and the user can help, call manage_browser with action="login" and the login URL. That opens a persistent headed browser the user can authenticate in. Reuse the same session_key afterward, and call manage_browser(action="close", ...) when you are done with the authenticated session.
If the user provides a URL and explicit claims, go straight to verify_visual_claims — it captures screenshots internally, so a separate take_screenshot is redundant.
Only call take_screenshot first when you have no claims yet and need to see the page before writing them.
