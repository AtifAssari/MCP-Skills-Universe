...
url: https://skills.sh/breath57/dingtalk-skills/dingtalk-calendar
---

# dingtalk-calendar

skills/breath57/dingtalk-skills/dingtalk-calendar
dingtalk-calendar
Installation
...
工作流程（每次执行前）
识别任务 → 按上表归类后，再选具体 API（见 references/api.md）。
校验配置 → bash scripts/dt_helper.sh --get 读取 DINGTALK_APP_KEY、DINGTALK_APP_SECRET、DINGTALK_MY_USER_ID、DINGTALK_MY_OPERATOR_ID（缺 unionId 时 --to-unionid）。
收集缺失项 → 一次性询问并 --set 写入 ~/.dingtalk-skills/config。
获取新版 Token → NEW_TOKEN=$(bash scripts/dt_helper.sh --token)，请求头 x-acs-dingtalk-access-token。
