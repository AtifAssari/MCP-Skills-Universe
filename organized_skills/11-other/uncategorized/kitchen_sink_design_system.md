---
rating: ⭐⭐⭐
title: kitchen-sink-design-system
url: https://skills.sh/baphomet480/claude-skills/kitchen-sink-design-system
---

# kitchen-sink-design-system

skills/baphomet480/claude-skills/kitchen-sink-design-system
kitchen-sink-design-system
Installation
...
CSS approach -- Tailwind (which version?), vanilla CSS, CSS modules, Sass, etc.
Icon library -- Lucide, Heroicons, inline SVG, icon fonts, etc.
Interactivity layer -- React, Vue, Svelte, vanilla JS, none.
CMS -- TinaCMS (tina/), Sanity, Contentful, plain markdown, none.
Existing component patterns -- Where do components live? What naming conventions are in use? Is there an existing helper like cn()?
Tailwind Detection
...
Ingest -- Read all discovered guide files. Extract:
...
Map -- For every extracted token, identify:
...
Propose -- Generate a design-tokens.md with:
...
Voice -- Define initial voice & tone:
...
Approve -- Present the proposal to the user. Do NOT proceed to Phase 1 until tokens and voice are approved.
...
Base layer -- Shared structural classes: layout, border-radius, font-size, focus ring, transitions, disabled state. These NEVER change between variants.
Variant layer -- Only what differs: colors, borders, shadows, backgrounds. Defined as named variants.
Type / param export -- Always export the variant types (React: VariantProps<>, Astro: Props interface) so consumers (including AI agents) can discover available variants.
Escape hatch -- Accept an additional class prop and merge it last so consumers can override when necessary.
No raw conditionals -- Never use isDestructive ? "bg-red-500" : "bg-blue-500" inline. All visual branching goes through the variant API.
...
Create the component in the project's established component location with a stable export/interface.
Document the interface at the top of the file:
...
Use design tokens from the project's Tailwind config or CSS custom properties; avoid arbitrary/magic values.
Keep components self-contained -- rely only on dependencies already in the project.
Make interactive components actually interactive using whatever the project's interactivity layer is:
React: useState, event handlers
Astro: client:load + framework islands
Static: vanilla JS, Alpine.js, or <details>/<summary>
Apply the variant pattern from Phase 2 (CVA for React, Props for Astro).
Apply voice & tone patterns from Phase 3. Error messages answer what/why/fix. Empty states guide the user.
Apply motion patterns from Phase 4. Use the defined duration and easing tokens.
Wire the component into the sink page immediately. No placeholders.
...
Header -- Title, description, last-updated timestamp, franchise declaration
Design Tokens -- Color palette (primitive -> semantic), typography scale, spacing ramp
Voice & Tone -- Voice definition, tone map, content pattern examples
Illustration Gallery -- Canonical illustrations, reinterpretation examples, prompt template
Site Header -- Rendered inline to test responsive breakpoints and nav states
Site Footer -- Rendered inline to test link columns and brand consistency
Typography -- H1-H6, body, caption, lists, blockquote, inline code
Buttons -- All variants x sizes x states (variant grid)
Badges -- Color variants, dismissible
Cards -- Basic, with header/footer, interactive
Form Controls -- All input types with label + error states
Modals & Dialogs -- Working open/close demo
Alerts -- All severity variants
Motion Sampler -- Interactive demos of hover lift, fade, slide, expand/collapse
Content-Author Specimens -- How MDX components render (if applicable)
Tier 2 components -- Tabs, breadcrumbs, accordion, tooltip, dropdown (if applicable)
Tier 4 components -- Table, stats cards, progress, skeleton (if applicable)
Chaos Laboratory -- Token visualization, state matrix, dark/light side-by-side, responsive stubs
Chaos Laboratory
Token visualization -- Programmatically render design tokens (colors, spacing).
...
State matrix -- Render variants side by side (default, hover, focus, disabled, active).
Theme test -- Light and dark columns, forced via wrapper class.
Responsive stubs -- iframe containers with fixed widths (320px, 768px) to verify mobile layouts.
Phase 7: Verify
