---
rating: ⭐⭐⭐
title: secondme-dev-assistant
url: https://skills.sh/mindverse/second-me-skills/secondme-dev-assistant
---

# secondme-dev-assistant

skills/mindverse/second-me-skills/secondme-dev-assistant
secondme-dev-assistant
Installation
...
Fetch the specified URL to get the current endpoint path, request parameters, response fields, and error codes
Use only the fetched content for API details — do not fall back to memorized or cached specs
If the fetch fails (network error, 404, etc.), inform the user that the doc site is unreachable and ask them to provide the API details manually or retry later
Never hardcode or invent API paths, fields, or error codes that are not in the fetched docs
...
bootstrap the app through SecondMe Develop APIs by default
