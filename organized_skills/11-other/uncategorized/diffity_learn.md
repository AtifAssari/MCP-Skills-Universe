...
rating: ⭐⭐
title: diffity-learn
url: https://skills.sh/kamranahmedse/diffity/diffity-learn
---

# diffity-learn

skills/kamranahmedse/diffity/diffity-learn
diffity-learn
Installation
...
Ensure a tree instance is running for the learning directory: run diffity list --json.
...
Check if learn.json already exists in a learn-<topic>/ directory. If it does, this is a resume — skip to the Resume section.
...
Start a Diffity tree instance from inside the learning directory. Run cd <learning-dir> && diffity tree --no-open using Bash with run_in_background: true. Wait 2 seconds, then verify with diffity list --json. If it fails, check that the directory exists and has a git repo.

Spawn the plan agent to plan the first 3-5 lessons. Write the result to learn.json's lessonPlan. Sanity check: lesson 1 should be the absolute basics — if it isn't, re-prompt.
...
Update lastSession, append to sessionLog.
...
Tell the user: "Good stopping point — progress saved. Pick up anytime with /diffity-learn ."
