...
rating: ⭐⭐⭐
title: nexus-query
url: https://skills.sh/haaaiawd/nexus-skills/nexus-query
---

# nexus-query

skills/haaaiawd/nexus-skills/nexus-query
nexus-query
Installation
...
$SKILL_DIR 为本 Skill 的安装路径（.agent/skills/nexus-query 或独立 repo 路径）。 extract_ast.py 默认排除 .git/、.nexus-map/、node_modules/、__pycache__/、.venv/、dist/、build/ 等噪音目录及文件；默认还会读取 <repo_path>/.gitignore，忽略其中声明的文件和目录。--no-gitignore 仅关闭 .gitignore 规则，不会关闭内置排除项，也不会关闭 --exclude-dirs。
