...
rating: ⭐⭐⭐
title: shared
url: https://skills.sh/boshu2/agentops/shared
---
...
If running in a Claude session and TeamCreate/SendMessage are available, use Claude Native Teams as the primary backend.
If running in a Codex session and spawn_agent is available, use Codex sub-agents as the primary backend.
...
Call the skill tool to load the target skill's content into context
Read and follow the loaded instructions directly — do NOT expect automatic execution
NEVER use slashcommand syntax (e.g., /council) in OpenCode — it triggers a command lookup, not skill loading
If the loaded skill references tools by Claude Code names, use OpenCode equivalents (see tool mapping below)
...
references/strict-delegation-contract.md — canonical contract loaded by /rpi, /discovery, /validation: strict sub-skill delegation is the default for top-level orchestrators.
Weekly Installs
430
Repository
boshu2/agentops
