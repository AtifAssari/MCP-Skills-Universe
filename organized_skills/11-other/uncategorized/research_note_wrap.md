---
rating: ⭐⭐⭐
title: research-note-wrap
url: https://skills.sh/leonsong09/research-note-wrap/research-note-wrap
---

# research-note-wrap

skills/leonsong09/research-note-wrap/research-note-wrap
research-note-wrap
Installation
...
Check whether the current project's AGENTS.md explicitly defines a default output directory for research / analysis summary notes.
If AGENTS.md already defines it, follow that path directly.
If it does not define a path, ask the user for the output directory the first time.
Once the user gives a path, use it for the current task. Suggest adding it to AGENTS.md if this is a recurring workflow.
...
Reuse the current note file by default.
Add a new section such as:
## 追加专题：xx

Only create a new file if the user explicitly asks for a separate note.
Writing Guidance
Good
