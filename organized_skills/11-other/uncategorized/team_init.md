...
rating: ⭐⭐⭐
title: team-init
url: https://skills.sh/killvxk/teamskills/team-init
---

# team-init

skills/killvxk/teamskills/team-init
team-init
Installation
$ npx skills add https://github.com/killvxk/teamskills --skill team-init
SKILL.md
工程团队初始化
...
使用 Bash 创建目录: mkdir -p "{work_dir}/.teams/{project_name}/roles"
使用 Read 读取每个核心角色定义文件和 workflow.md
使用 Write 将每个核心角色定义写入 .teams/{project_name}/roles/{role_code}.md
若有扩展角色（ext_roles[] 非空）:
...
若有远程角色（selected_extension_roles 中含 source: remote 条目）:
...
使用 Write 将 workflow.md 写入 .teams/{project_name}/workflow.md
使用 Write 创建 .teams/{project_name}/team.yaml:
# 团队配置 - 由 /team-init 准备
...
角色定义路径: {work_dir}/.teams/{project_name}/roles/{role_code}.md 工作流路径: {work_dir}/.teams/{project_name}/workflow.md
