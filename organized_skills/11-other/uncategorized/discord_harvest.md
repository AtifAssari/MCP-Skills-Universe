---
rating: ⭐⭐
title: discord-harvest
url: https://skills.sh/t4sh/skills4sh/discord-harvest
---

# discord-harvest

skills/t4sh/skills4sh/discord-harvest
discord-harvest
Installation
$ npx skills add https://github.com/t4sh/skills4sh --skill discord-harvest
SKILL.md
Discord Harvest
...
Source Type — DM or server channel? Who/which server+channel?
Scope — How many messages? (default: last 10) Date range? Content types?
Output — Where to save? First run or incremental update?
...
DM — I'll open Discord in the browser, you navigate to the conversation
Server channel — I'll use the bot API (tell me the server and channel name)
...
Flag suspicious content: Run flag_suspicious() (see references/code-examples.md) over filenames and embed titles. Matches are included in the summary report as warnings — they don’t block downloads, but the user should know what they’re archiving.
...
CRITICAL: Sanitize all filenames and validate all URLs before any curl download. Discord content is untrusted input. Use sanitize_filename(), validate_url(), and redact_cdn_url() — see references/code-examples.md for implementations.
...
Take a snapshot, scroll for history if needed, then run the extraction script from references/code-examples.md via the environment's evaluate / execute JavaScript action (e.g. browser_tool evaluate, or the equivalent on cursor-ide-browser).
...
For full folder naming rules, format examples (links.md, manifest.json), and the summary report template, see references/folder-structure.md.
...
Reference Files
File	Contents
references/code-examples.md	Sanitization functions, URL validation, CDN redaction, DOM extraction script, download commands
references/folder-structure.md	Folder naming, directory structure, links.md/manifest.json formats, repeat-run behavior, summary report
references/troubleshooting.md	Common issues by source type (DMs, server, CDN), troubleshooting Q&A
Tools Referenced
...
Is this a DM or a server channel?
Who is the conversation with (DM) or which server/channel (server)?
How many messages should I scan? (default: last 10)
Is this a first-time harvest or an incremental update?
Do you want all content types (images, files, links) or specific ones?
Related Skills
...
discord-intel — full Discord export pipeline (DiscordChatExporter → SQLite → filtered indexing) with prompt injection protection. Heavier toolchain but more structured output
agent-discord — TypeScript CLI with snapshot and message reading, auto-extracts tokens from Discord desktop app. Useful when bot API access isn't available
discord-reader — read-only Discord access via Chrome DevTools Protocol. No bot token needed — complements the DM browser path
Weekly Installs
22
Repository
t4sh/skills4sh
