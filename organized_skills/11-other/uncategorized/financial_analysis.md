...
url: https://skills.sh/qyqsdtc/financial_analysis_agent_skill/financial-analysis
---

# financial-analysis

skills/qyqsdtc/financial_analysis_agent_skill/financial-analysis
financial-analysis
Installation
...
读取PDF - 使用 PDF 工具读取PDF文件内容
提取数据 - 从文本中识别资产负债表、利润表、现金流量表数据
计算指标 - 使用 scripts/calculator.py 计算财务指标
杜邦分析 - 使用 scripts/dupont_analyzer.py 分解ROE驱动因素
趋势分析 - 使用 scripts/trend_analyzer.py 分析多期数据趋势
深度分析 - 使用 scripts/financial_analyzer.py 进行专业分析
生成报告 - 使用 scripts/report_generator.py 生成报告
核心脚本
...
注意：现金净利比偏低
参考文档
详细代码示例: 见 references/code-examples.md
评分标准: 见 references/scoring-standards.md
依赖
pip install pandas numpy openpyxl matplotlib seaborn openai

注意事项
数据准确性: 解析结果依赖PDF质量，建议人工复核关键数据
行业差异: 不同行业的财务指标标准差异较大，分析时需考虑行业特点
投资风险: 本工具仅供参考，不构成投资建议
Weekly Installs
23
Repository
qyqsdtc/financi…nt_skill
GitHub Stars
2
First Seen
Feb 25, 2026
