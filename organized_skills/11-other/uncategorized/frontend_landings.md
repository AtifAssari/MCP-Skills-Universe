...
url: https://skills.sh/auwalmusa/frontend-landings-claude-skill/frontend-landings
---

# frontend-landings

skills/auwalmusa/frontend-landings-claude-skill/frontend-landings
frontend-landings
Installation
$ npx skills add https://github.com/auwalmusa/frontend-landings-claude-skill --skill frontend-landings
SKILL.md
Frontend Landings
...
Tagline / hero headline (required)
3–6 key features or benefits (required)
Target audience (helpful)
Primary CTA (e.g., "Start Free Trial", "Join Waitlist") — default: "Get Started"
Color preferences (optional — preset will handle this)
Style preset (show the preset menu if not specified)
...
Analyze the existing design (structure, colors, typography, layout)
Identify improvements (animations, responsiveness, accessibility, visual hierarchy)
Ask which style preset to apply (or preserve existing aesthetic)
Regenerate as a single-file HTML with all enhancements
Style Preset System
...
Responsive Design — Mobile-first. Test mental model at 375px, 768px, 1280px.
Dark/Light Mode — prefers-color-scheme auto-detection + manual toggle button in nav.
Scroll Animations — IntersectionObserver-based reveal animations on all sections.
Smooth Scrolling — scroll-smooth on html + JS for nav anchors.
Accessibility — Semantic HTML5, ARIA labels on interactive elements, focus-visible styles, skip-to-content link, sufficient color contrast.
SEO Meta Tags — title, description, og:title, og:description, og:type, twitter:card.
Performance — Lazy-load images, efficient CSS, minimal JS.
Section Generation

Based on user input, intelligently include these sections. Not all are required — match to the product context:

Always Include:
Navigation — Sticky/fixed nav with logo, links, dark mode toggle, primary CTA button
Hero — Large headline + sub-headline + CTA button(s) + visual element (gradient, abstract shape, or image placeholder)
Features — 3–6 feature cards/grid with inline SVG icons
Final CTA — Full-width call-to-action banner with headline + button
Footer — Links, copyright, social icons (inline SVG)
