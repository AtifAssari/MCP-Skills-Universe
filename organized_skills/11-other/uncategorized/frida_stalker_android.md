---
rating: ⭐⭐
title: frida-stalker-android
url: https://skills.sh/yfe404/frida-stalker-skills/frida-stalker-android
---

# frida-stalker-android

skills/yfe404/frida-stalker-skills/frida-stalker-android
frida-stalker-android
Installation
...
Quick Start
If you do not know the thread id yet, start with templates/stalker-start-stop-around-hook.js.
If you already know the thread id and want low overhead, use templates/stalker-call-summary.js.
If you need ordered events, use templates/stalker-onreceive-parse.js and keep event types narrow.
If you only care about calls to one target, start with templates/stalker-call-probe.js.
MCP Usage Notes (If Available)

When driving this through the Frida MCP tools, prefer this flow:

Create or attach a session (mcp__frida__create_interactive_session / mcp__frida__attach_to_process).
Load the selected template with mcp__frida__load_script.
Start tracing through RPC exports using mcp__frida__call_rpc_export (templates expose start() / stop() when appropriate).
Use mcp__frida__get_session_messages to consume output.
