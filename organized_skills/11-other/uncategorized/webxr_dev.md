...
rating: ⭐⭐
title: webxr-dev
url: https://skills.sh/danielcanton/webxr-dev-skill/webxr-dev
---

# webxr-dev

skills/danielcanton/webxr-dev-skill/webxr-dev
webxr-dev
Installation
...
Only these strings are recognized in optionalFeatures / requiredFeatures: anchors, bounded-floor, depth-sensing, dom-overlay, hand-tracking, hit-test, layers, light-estimation, local, local-floor, secondary-views, unbounded, viewer.
...
DoubleSide (recommended): Renders both faces, eliminates all culling ambiguity. The shader determines what to draw via ray direction — double-rendering has zero visual cost.
Standard geometry + BackSide: Works in theory (outward normals, BackSide renders inner faces) but winding order can be ambiguous across geometry types.
Inverted geometry + FrontSide: Also works but equally fragile.
...
Enable Developer Mode via the Meta phone app (Devices > Headset Settings > Developer Mode)
Connect Quest to PC via USB, approve connection dialog in headset
Open chrome://inspect/#devices in desktop Chrome
Your Quest browser tabs appear — click inspect for full DevTools
Wireless option: adb tcpip 5555 then adb connect <quest-ip>:5555
...
Iteration Loop
Make change
Deploy (or use local network URL)
Hard-refresh in Quest Browser
Test specific feature
Check console via remote debugging
...
A separate "Enter AR" button (distinct from "Enter VR")
Using immersive-ar with a complete alpha management strategy tested on-device
Every material and shader must output correct alpha values (alpha=0 → passthrough, alpha=1 → opaque render)
