...
url: https://skills.sh/vamdawn/ai-forge/semver-release
---

# semver-release

skills/vamdawn/ai-forge/semver-release
semver-release
Installation
...
Check if CHANGELOG.md exists — mark as primary (do NOT create yet; defer to Step 7)
Glob for CHANGELOG-*.md and CHANGELOG.*.md to discover multilingual variants
...
Check what went wrong: git status
All file modifications are still staged — fix the issue and create a new commit (do NOT use --amend)
If the user wants to abort entirely, restore all modified files:
...
Suggest: git tag -d vX.Y.Z to remove the old tag, then retry
