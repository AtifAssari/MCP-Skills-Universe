...
rating: ⭐⭐⭐
title: align-backlog
url: https://skills.sh/nesnilnehc/ai-cortex/align-backlog
---

# align-backlog

skills/nesnilnehc/ai-cortex/align-backlog
align-backlog
Installation
...
✅ 找到待办事项和策略来源：识别并解析待办事项和策略层文档（目标、路线图、里程碑）
✅ 生成映射：每个待办项目（或代表性样本）都映射到至少一个战略锚点（目标、里程碑、路线图主题）或标记为孤立/未映射
✅ 未对齐分类：项目按基本原理进行分类（对齐、部分、未对齐、孤儿）；根据调查结果说明影响和根本原因
✅ 列出的更改建议：提供具体建议（添加、删除、合并、重新确定优先级），并提供优先级和策略可追溯性
✅ 报告持久化：Backlog Alignment Report写入约定路径
✅ 避免不安全写入：如果待办位置不确定或建议的编辑具有破坏性，则在应用更改之前请求用户确认
...
定义或重写策略（使用define-mission、设计-strategic-goals、define-routetu等）
...
第 0 阶段：解决待办事项和战略来源
解析待办位置（项目规范，docs/ARTIFACT_NORMS.md，或默认：docs/待办/，repo 待办约定）
解决策略来源：目标、路线图、里程碑（例如docs/project-overview/、docs/process-management/）
如果没有策略文档，则报告受阻，并建议先定义策略（设计-策略-目标、定义-路线图等）
如果没有找到待办事项，报告被阻止并建议待办事项位置或“capture-work-items”来播种一个
第 1 阶段：将待办事项映射到战略
列出待办事项（如果待办非常大，则为抽样集）
对于每个项目，确定可追溯性：它支持哪个目标、里程碑或路线图主题（或标记孤立）
记录映射和证据（例如标签、描述、链接）
第 2 阶段：分类对齐
...
第 3 阶段：提出变更建议
剪切：要删除或存档的项目（孤立的、被取代的或超出范围的）
合并：要合并或删除重复的项目
重新确定优先级：订单发生变化，因此最高战略价值显而易见
添加：待办中策略暗示尚未工作的差距（可选；仅作为建议列出）
...
从 docs/project-overview/, docs/process-management/ 加载目标和路线图
从docs/待办/（或项目约定）加载待办
将每个项目映射到目标/路线图主题或标记孤立项 4.分类：30个对齐，10个部分，5个未对齐，5个孤立
建议：削减 5 个孤立主题，重新调整 10 个部分主题的优先级，合并 2 个重复主题
保存报告到 docs/calibration/待办-alignment.md
示例 2：无策略文档
...
状态：被阻止
消息：未找到策略文档。首先定义目标和路线图（例如设计-策略-目标，定义-路线图），然后重新运行align-待办。
置信度：不适用
示例 3：大量待办事项，子集范围
