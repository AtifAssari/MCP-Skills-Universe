---
rating: ⭐⭐⭐
title: glmv-stock-analyst
url: https://skills.sh/site/modelscope.cn/glmv-stock-analyst
---

# glmv-stock-analyst

skills/modelscope.cn/glmv-stock-analyst
glmv-stock-analyst
$ npx skills add https://www.modelscope.cn/skills/@zai-org/glmv-stock-analyst
SKILL.md
stock-analyst
...
测速对比默认源/清华TUNA/阿里云，选出最快镜像
检测是否已有可用的 venv → 有则直接用
没有则自动创建 venv 并安装 requirements.txt 中的全部依赖
安装完成后输出后续使用的命令格式
...
搜索内容：{用户说的名字} 股票代码 上市 港股 OR A股 OR 美股，时间范围选 past_year 本搜索借用已有web_search能力， 如果搜索失败，继续尝试其他有web_search能力的工具，如果全部失败提醒用户需要先配置一种web_search工具，或直接给出股票代码
...
"{股票名称}" "{股票代码}" 最新 2026年{月} — 近期事件和动态
"{股票名称}" 分析师 评级 目标价 研报 — 专业观点
"{股票名称}" 资金流向 南向资金 卖空 — 资金面（港股必搜）
...
写 report.md 到任务文件夹
转 HTML（md2html.py）
回复用户 webchat 精炼总结（附带 PDF 提示语）
打开浏览器（open report.html）— 可在回复之后异步执行
...
第3部分"近期走势分析"是核心差异点 — 不能只给结论不给过程，用户需要知道"为什么"
所有数据必须来自 Step 1~3 的实际获取结果 — 禁止编造价格、涨跌幅、事件、财务数据
多空对比表必须事实驱动 — 每条都要有具体数据或事件支撑
操作建议必须可执行 — 不是"谨慎观望"这种废话，而是"等站回XX再考虑"/"利用反弹减仓"
总长度控制在聊天内一屏左右可读完 — 大约400-800字
不要假设用户会看HTML报告 — webchat回复本身就要是完整的
...
重要规则（踩坑总结）
先搜代码再跑脚本 — 永远不用记忆中的代码
脚本只采集数据，不写报告 — 报告由模型产出
你必须亲自看图 — 多模态能力的核心价值，不看图等于瞎分析
每次任务独立文件夹 — 路径从脚本 stdout 取，不同次运行互不干扰
不编造数据 — 没获取到就说没获取到
webchat 总结基于实际搜索和数据 — 禁止编造股价、事件、财务数据
新闻要精准 — 不相关的全市场快讯不要塞进报告
图片策略分层 — report.md用真实图，webchat不放文本折线图
K线图放报告最前面 — 不是塞在章节末尾，是读者第一眼看到的
语种跟随用户 — 术语保留英文
敏感标的合规 — 见 references/sensitive_companies.md
HTML 由 md2html 生成 — 不要手写 HTML（会被安全策略拦截长命令）
report.md 是核心产物 — HTML 和 PDF 都从它派生
执行顺序：report.md → HTML → 回复用户 → 打开浏览器 — report.md 和 HTML 先完成，浏览器打开不能漏
Weekly Installs
133
Source
www.modelscope.…-analyst
First Seen
3 days ago