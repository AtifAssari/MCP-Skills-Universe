...
rating: ⭐⭐⭐
title: superplan
url: https://skills.sh/adamos486/skills/superplan
---

# superplan

skills/adamos486/skills/superplan
superplan
Installation
...
Check Availability: Attempt to invoke /slop-or-not

If not installed, inform the user:

"The slop-or-not skill can enhance refactor assessment by detecting AI-generated code anti-patterns (hallucinations, cross-language contamination, security vulnerabilities). Install from: https://github.com/asteroid-belt/skills
...
Run Slop Scan: If available, invoke /slop-or-not targeting:

Files identified in EXPLORE phase as modification candidates
Areas flagged with code smells in Step 2

Apply Reciprocal Rank Fusion: Combine modification paths with slop findings:
...
Output Combined Priority List: Rank refactor candidates by RRF score, noting:
...
What other features are planned for this area in the next 6 months?
How often does this part of the codebase change?
Are there pain points here that slow down development?
What's your risk tolerance for refactoring vs. shipping fast?"
...
Write Overview + Requirements → Save
Write Architecture → Save
Write each Phase separately → Save after each
Write Assumptions + Appendix → Save
Execution Flow
Quick Reference
Intake → Ask for requirements, capture raw input, identify type
Detect → Check CLAUDE.md/AGENTS.md first; if complete, BYPASS scan + RESEARCH
Interview → Ask 3-5 clarifying questions, wait for answers
Research → Web search best practices (BYPASSED if CLAUDE.md/AGENTS.md complete)
Explore → Analyze codebase patterns and integration points (ALWAYS RUNS)
Refactor Assess → Evaluate code smells, interview on roadmap, decide if refactoring needed
Architect → Design solution with diagrams
Phase → Break into parallelizable phases WITH POKER ESTIMATES
Detail → Specify code deltas per phase WITH DEFINITION OF DONE
Test → Define failing tests (TDD) per phase
Document → Write to docs/<feature>-plan.md
