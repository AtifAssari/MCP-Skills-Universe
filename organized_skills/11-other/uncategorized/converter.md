...
rating: ⭐⭐⭐
title: converter
url: https://skills.sh/boshu2/agentops/converter
---

# converter

skills/boshu2/agentops/converter
converter
Installation
...
Add a conversion function to scripts/convert.sh (pattern: convert_<target>)
Update the target table above
Add reference docs to references/ if the target format needs documentation
Examples
...
The converter parses skills/council/SKILL.md frontmatter, markdown body, and any references/ and scripts/ files into a SkillBundle.
The Codex adapter transforms the bundle into a SKILL.md (body + inlined references + scripts as code blocks) and a prompt.md (Codex prompt referencing the skill).
Output is written to .agents/converter/codex/council/.
...
The converter scans every directory under skills/ and parses each into a SkillBundle.
The Cursor adapter transforms each bundle into a .mdc rule file with YAML frontmatter and body content, budget-fitted to 100KB max. Skills referencing MCP servers also get a mcp.json stub.
Each skill's output is written to .agents/converter/cursor/<skill-name>/.
