---
rating: ⭐⭐
title: dual-edition-module-migration
url: https://skills.sh/dtyq/magic/dual-edition-module-migration
---

# dual-edition-module-migration

skills/dtyq/magic/dual-edition-module-migration
dual-edition-module-migration
Installation
...
Keep the default implementation in src/opensource/...
Update callers to import that @/opensource/... path directly
If enterprise behavior differs, add the same path under enterprise/src/opensource/...
Let Vite resolve the enterprise file first at runtime
Remove the old variant facade after callers are switched
...
Keep the default implementation in src/opensource/...
Register it in src/opensource/components/ComponentRender/config/defaultComponents.tsx
Keep commercial-only UI in src/... only when it owns real delta
Register premium overrides only where the factory still needs explicit component replacement
For helper imports such as preload code, prefer direct @/opensource/... imports plus enterprise overlay
...
Extract the changing block into the smallest possible shared child component or hook under src/opensource/...
Keep the shared baseline minimal: null, false, no-op callback, or another tiny safe fallback
Mirror only that child component or hook under enterprise/src/opensource/...
Update the large shared page or layout to import the stable @/opensource/... facade instead of importing commercial modules directly
Do not override the whole page or layout when a banner, modal, slogan, model tag, or hook is the real variability point
...
Add enterprise/src/opensource/<same-path>.ts(x)
Re-export from the existing commercial file under src/...
Switch all shared callers to @/opensource/...
Remove variant files
Later, inline or fold the shim if the codebase is cleaned up further
...
Move the whole closure into enterprise/src/opensource/...
Do not add src/opensource stubs for enterprise-only stores or services unless the user explicitly wants a temporary bridge
Update enterprise callers to use the stable @/opensource/... path, so overlay resolves the real file
Delete the old src/... files once imports are switched
...
Keep a stub in src/opensource/... at the same logical path
Stub returns no-op values: empty array for lists, false for flags, () => {} for callbacks, null for ReactNode
Stub must match the enterprise interface exactly so all callers work without changes
Enterprise overlay provides the full implementation when overlay is enabled

Stub minimization rules:

Keep only the smallest surface that current open-source callers actually import
Do not copy enterprise-only helper files, upload types, or private dependency adapters into src/opensource/... if the stub does not use them
Prefer flattening a stub to one file when the open-source side only needs a no-op export plus one or two lightweight enums or helpers
If a helper already exists globally, reuse it instead of creating a module-local duplicate in the stub
...
Create src/runtime/<domain>/
Export a stable runtime API from that layer
Keep src/opensource/ off that runtime facade unless the user explicitly wants the extra indirection
Justify the runtime layer with a real binding problem, not habit
...
Replace each caller with the corresponding @/opensource/... import
Create enterprise/src/opensource/... overlays for any enterprise-only behavior
Delete src/variant/...
Delete src/opensource/variant/...
5. Verify boundaries
...
src/opensource/... may contain only the shared facade or no-op stub for that feature
The real enterprise implementation must live under the mirrored enterprise/src/opensource/... path
If the user explicitly says the open-source edition must not include the feature, do not put visible runtime behavior into src/opensource/...
...
A short architecture decision: direct shared import vs enterprise overlay vs component override vs runtime facade
A migration plan grouped by shared baseline, enterprise deltas, and forbidden dependencies
The concrete file changes
Verification status for lint, tests, and build
Keep it minimal
...
For concrete decision examples, see examples.md
Weekly Installs
12
Repository
dtyq/magic
