---
rating: ⭐⭐⭐
title: lark-workflow-batch-data
url: https://skills.sh/mmmnhjgh/lark-workflow-batch-data/lark-workflow-batch-data
---

# lark-workflow-batch-data

skills/mmmnhjgh/lark-workflow-batch-data/lark-workflow-batch-data
lark-workflow-batch-data
Installation
...
断点续传 — 维护进度状态变量（offset / batch_index / start_row），失败后从断点恢复
指数退避重试 — 429 速率限制 → 2s→4s→8s，最多 3 次；1254291 并发冲突 → 等 1s 重试
部分失败容错 — 记录失败记录到 failed_records，跳过继续，最终统一汇报
批次自动缩减 — 1254104 批量超限 → 自动将批次从 500 缩减到 250→125
进度汇报 — 每阶段结束向用户报告成功/失败/断点信息
Wiki 链接特殊处理
...
模块索引
模块	参考文档	说明
Base 分页读取	base-read.md	+record-list 分页续读、断点续读、视图筛选
Base 批量写入	base-write.md	+record-upsert 分批写入、断点续写、写前校验
Sheets 批量操作	sheets-ops.md	+read 分片、+write/+append 批量、+export 导出
导入导出	import-export.md	drive +import、sheets +export、超时续查
数据迁移	migration.md	源→目标迁移、字段映射、增量同步
大数据分析	analysis.md	+data-query 服务端聚合、条件分析
错误恢复	error-recovery.md	错误分类、重试策略、断点续传、进度汇报
权限表
操作	所需 scope
...
API 调用失败：当直接 API 调用返回 429（速率限制）、1254291（并发冲突）等错误时
上下文溢出：当 Agent 处理数据时出现上下文长度限制警告
操作中断：当批量操作中途停止，需要断点续传时
3. 模板使用指南
...
创建 Python 包装器：为 lark-cli 命令创建 Python 包装器，使 OpenClaw 能够更方便地调用

添加状态管理：在 OpenClaw 中添加状态管理模块，记录批量操作的进度和断点

智能检测：实现智能检测机制，自动判断何时使用本 Skill，何时使用直接 API 调用

错误处理统一：统一处理 API 错误和 Skill 错误，提供一致的错误恢复机制

进度反馈：在 OpenClaw 界面中显示批量操作的进度，让用户了解当前状态
