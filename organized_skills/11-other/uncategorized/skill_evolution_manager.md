---
rating: ⭐⭐⭐
title: skill evolution manager
url: https://skills.sh/kkkkhazix/khazix-skills/skill-evolution-manager
---

# skill evolution manager

skills/kkkkhazix/khazix-skills/Skill Evolution Manager
Skill Evolution Manager
Installation
...
经验提取 (Experience Extraction)：将非结构化的用户反馈转化为结构化的 JSON 数据（evolution.json）。
智能缝合 (Smart Stitching)：将沉淀的经验自动写入 SKILL.md，确保持久化且不被版本更新覆盖。
使用场景
...
定位 Skill：确定是哪个 Skill 需要进化（例如 yt-dlp 或 baoyu-comic）。
生成 JSON：在内存中构建如下 JSON 结构：
{
