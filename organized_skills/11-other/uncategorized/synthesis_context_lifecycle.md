---
rating: ⭐⭐⭐
title: synthesis-context-lifecycle
url: https://skills.sh/rajivpant/synthesis-skills/synthesis-context-lifecycle
---

# synthesis-context-lifecycle

skills/rajivpant/synthesis-skills/synthesis-context-lifecycle
synthesis-context-lifecycle
Installation
...
Commit AND push with message: "Maintain context: archive sessions, extract reference facts". Stage only the files this invocation modified — do not git add -A. See the Commit Protocol section below for the full rule.
...
Track which files THIS invocation modified (the agent's own action history is the source of truth).
Group those files by containing repo.
For each containing repo: git add ONLY those specific files (never git add -A or git add .), commit, push.
Never touch repos where you did not create or modify files in this invocation.
...
Session never ends cleanly. Long-running conversations resume across days; a "session-end" hook may never fire.
Modified ritual modes skip the day-end checklist. Mid-day syncs, vacation/observer catch-ups, and partial rituals update context without running the full day-end sequence. If commit is only in day-end, these updates never ship.
Multiple invocations compound uncommitted work. By the time someone notices, there's a week of context changes stranded on one machine.
Verification
