...
url: https://skills.sh/phaserjs/phaser/audio-and-sound
---
...
If config.audio.noAudio is true, or the device supports neither Web Audio nor HTML5 Audio, a NoAudioSoundManager is created (all calls are no-ops).
If the device supports Web Audio and config.audio.disableWebAudio is not true, a WebAudioSoundManager is created (preferred).
Otherwise, an HTML5AudioSoundManager is created as fallback.
