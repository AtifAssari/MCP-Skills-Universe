...
rating: ⭐⭐⭐
title: karpathy-kb
url: https://skills.sh/pedronauck/skills/karpathy-kb
---

# karpathy-kb

skills/pedronauck/skills/karpathy-kb
karpathy-kb
Installation
...
Add the topic as a new row in the vault-root README.md topic table.
(Optional, recommended at ~20+ sources) Create a qmd collection via the qmd skill: qmd collection add <topic-slug>/ --name <topic-slug> && qmd embed. At smaller scale, the topic's Concept Index + Source Index are sufficient navigation — see references/tooling-tips.md.
Start ingesting sources (Procedure 2).
Procedure 2: Ingest a raw source
Scrape the URL with the firecrawl skill: firecrawl scrape <url> (single page) or firecrawl crawl <url> (multi-page).
Save the output to <topic>/raw/articles/<descriptive-slug>.md (or raw/github/ for READMEs). Format the body as Obsidian Flavored Markdown — see the obsidian-markdown skill for wikilinks, callouts, and properties.
Prepend frontmatter — copy the matching schema from references/frontmatter-schemas.md. Fill title, source_url, scraped (today's ISO date), and topic-specific tags.
For TweetSmash bookmarks, pull clusters via the tweetsmash-api skill and save to <topic>/raw/bookmarks/<Topic> Bookmarks <Subtopic>.md using the bookmark-cluster frontmatter schema.
Re-index the topic's qmd collection if one exists: qmd collection remove <collection> && qmd collection add <topic>/ --name <collection> && qmd embed.
Append an entry to <topic>/log.md (Procedure 7) — e.g., ## [YYYY-MM-DD] ingest | <slug>.md (<source_kind>).
...
Identify candidate sources via the qmd skill — qmd query "<topic phrase>" scoped to the topic's collection — or read <topic>/wiki/index/Source Index.md.
...
Write the article to <topic>/wiki/concepts/<Article Title>.md using assets/wiki-article-template.md, following the obsidian-markdown skill for wikilink, callout, and frontmatter syntax. Target 3000-4000 words with a Sources section, wikilinks to related articles, and code or diagram blocks where applicable.
...
Update the topic's indexes (Procedure 4).
Update <topic>/CLAUDE.md current-articles list.
Re-index the qmd collection (if present).
Append an entry to <topic>/log.md (Procedure 7) — e.g., ## [YYYY-MM-DD] compile | <Article Title> (<word_count> words, <N> sources).
...
Locate relevant articles. At small scale (<30 articles), the index is enough. At larger scale, supplement with qmd query "<phrase>" scoped to the topic's collection. Also grep the topic for keywords: grep -rl "<keyword>" <topic>/wiki/concepts/.
...
Save the answer to <topic>/outputs/queries/<YYYY-MM-DD> <Question Slug>.md using the research-output frontmatter from references/frontmatter-schemas.md with stage: query.
...
When a filed-back insight contradicts or extends an article's claims, recompile the affected articles (Procedure 3).
...
Append to <topic>/log.md (Procedure 7) — e.g., ## [YYYY-MM-DD] query | <Question Slug> plus a second line ## [YYYY-MM-DD] promote | <Title> if promoted.
