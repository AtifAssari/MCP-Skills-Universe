...
rating: ⭐⭐⭐
title: dida365
url: https://skills.sh/cshen/skills/dida365
---

# dida365

skills/cshen/skills/dida365
dida365
Installation
...
Behavioral Rules
Always run commands with python3 — the script has no shebang-based auto-execution guarantee.
When the user asks to add a task, ask for the brief task description at minimum, which is the notes for the command line. Infer priority/project/date from context if the user provides them naturally. Summarize the task details using no more than 10 words in ENGLISH and use that as the task title.
When the user says "my tasks" or "what's on my plate", default to listing inbox tasks. Ask which project if they have context suggesting a specific one.
After creating a task, report the task ID back to the user — they'll need it for complete/update/delete.
After completing or deleting, confirm the action with the task ID.
If a command fails, read stderr output and explain the issue to the user in plain language.
Never hardcode task IDs — always get them from tasks output or add output first.
Quote titles and notes that contain spaces when building the command.
Weekly Installs
14
Repository
cshen/skills
