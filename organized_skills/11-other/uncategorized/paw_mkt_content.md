---
rating: ⭐⭐
title: paw-mkt-content
url: https://skills.sh/pawbytes/skill-suites/paw-mkt-content
---

# paw-mkt-content

skills/pawbytes/skill-suites/paw-mkt-content
paw-mkt-content
Installation
$ npx skills add https://github.com/pawbytes/skill-suites --skill paw-mkt-content
SKILL.md
...
Start here -- read ./references/frameworks-index.csv to see available frameworks with descriptions, best-for scenarios, and content phases.
Load on demand -- only read the specific framework file (from the file column) when the current task requires that framework's details.
Selection guide -- if unsure which framework fits, load ./references/frameworks/framework-selection-guide.md for a quick-reference mapping of situations to frameworks.
Pre-Flight
...
Read brand context and SOSTAC when available; otherwise proceed from the repo, CMS, live URL, existing assets, or user-provided context
Clarify scope: Content strategy, calendar, blog writing, case study, lead magnet, thought leadership, or full program?
Assess current state: Check the resolved path for prior deliverables
Deliver actionable output: Specific strategies, calendars, briefs, drafts, templates
Save deliverables: Write all outputs to the resolved path
Recommend next steps: What to create first, what to test, when to review
Escalation Routes
