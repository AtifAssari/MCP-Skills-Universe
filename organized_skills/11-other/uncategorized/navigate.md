...
rating: ⭐⭐⭐
title: navigate
url: https://skills.sh/hungv47/meta-skills/navigate
---

# navigate

skills/hungv47/meta-skills/navigate
navigate
Installation
...
Report as a table sorted by date (newest first), with a Location column distinguishing .agents/ pipeline outputs from top-level canonical records. Mark STALE if date: > 30 days old.
...
Recommend the one or two skills that unblock the most, using the dependency graph below. Don't dump a flat list of everything missing — trace the graph, find the root blocker.
Orchestrate

When: Argument starts with orchestrate or user asks for a full workflow plan

Scan .agents/ + brand/ + architecture/ + research/ for existing artifacts (same as Status step 1)
Check .agents/meta/out-of-scope/ — if the goal overlaps with a prior out-of-scope decision, surface it: "This was previously scoped out because [reason]. Revisit condition: [condition]. Proceed anyway?"
Classify the goal, match to skills using the skill registry, check dependencies, identify parallel tracks
Produce a workflow-plan.md artifact with phases, checkpoints, and progress tracking
Return the plan + instructions for Phase 1
...
Freshness — date field < 30 days
Completeness — valid frontmatter present
Status — final preferred; draft triggers warning
...
Scan .agents/ in current directory AND sibling skill repo directories
Report unified table with Project column
