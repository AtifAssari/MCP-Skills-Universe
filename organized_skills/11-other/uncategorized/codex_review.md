...
rating: ⭐⭐⭐
title: codex-review
url: https://skills.sh/yelban/orz99-skills/codex-review
---

# codex-review

skills/yelban/orz99-skills/codex-review
codex-review
Installation
$ npx skills add https://github.com/yelban/orz99-skills --skill codex-review
SKILL.md
Codex Review
...
ARGUMENTS 指定 .md 且內容含 # Plan: 或 ## Overview → plan
codex-plan.md（當前目錄）存在 → plan
.claude/plans/ 最新 .md → plan

Code mode（程式碼審查）：

ARGUMENTS 是檔案路徑（非 .md 計畫）→ code
ARGUMENTS 是 PR 編號（如 #123）→ gh pr diff 123
ARGUMENTS 是 diff 或空白 → git diff + git diff --staged
...
產生 UUID（uuidgen 或 date +%s）供檔名用
從 references/prompt-templates.md 載入對應 template（plan / code）
將審查對象內容 + 相關原始碼 + template 組裝成完整 prompt
寫入 /tmp/codex-review-input-${ID}.md
...
/codex-plan 生成計畫
/codex-review 跨模型審查計畫
實作程式碼
/codex-review diff 跨模型審查程式碼
/code-review Claude 互動式細部審查
Now Execute
