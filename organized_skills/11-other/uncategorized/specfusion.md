...
rating: ⭐⭐⭐
title: specfusion
url: https://skills.sh/wxkingstar/specfusion/specfusion
---

# specfusion

skills/wxkingstar/specfusion/specfusion
specfusion
Installation
$ npx skills add https://github.com/wxkingstar/specfusion --skill specfusion
SKILL.md
...
使用流程
检查服务：用 curl -s http://specfusion.inagora.org/api/health 确认 API 可用
提取关键词：从用户问题中提取搜索词，注意优先级：
用户提到错误码数字（如 60011、40001）→ 直接用数字搜索，系统有专用错误码索引
用户提供了 API 路径（如 /cgi-bin/message/send）→ 直接用路径搜索
否则 → 提取最具体的功能名称作为关键词
搜索文档：用 curl 调用 /search 接口，如果用户指定了平台则添加 source 参数
预览文档：对搜索结果中最相关的文档，先用 /doc/{doc_id}?summary=true 摘要模式预览
获取全文：确认是目标文档后，再调用 /doc/{doc_id} 获取完整内容
回答用户：基于文档内容回答，引用文档标题和来源平台
生成调用示例：如果用户场景涉及 API 调用，主动生成示例代码：
...
缩短关键词：去掉修饰词，保留核心功能名（"创建自建应用审批模板" → "审批模板"）
换同义词：尝试不同表述（"群机器人" <-> "webhook"，"通讯录" <-> "部门列表"）
去掉 source：不确定属于哪个平台时，去掉 source 参数搜全部
用路径搜：有 API 路径片段时直接按路径搜索（如 /cgi-bin/user/get）
用宽泛词：回退到功能域搜索（"消息"、"审批"、"通讯录"、"日历"）
浏览分类：调用 /api/categories 查看有哪些文档分类，找到合适的方向
以上均无结果 → 说明可能尚未收录，引导用户访问官方文档
上下文管理
...
分别用 source=wecom 和 source=feishu 搜索同一功能
对两篇文档都用 summary=true 预览，确认是对等功能
获取全文后，用表格按维度对比：接口路径、请求方式、必填参数、权限要求、调用限制
降级方案
