...
url: https://skills.sh/autoclaw-cc/xiaohongshu-skills/xiaohongshu-skills
---

# xiaohongshu-skills

skills/autoclaw-cc/xiaohongshu-skills/xiaohongshu-skills
xiaohongshu-skills
Installation
...
认证相关（"登录 / 检查登录 / 切换账号"）→ 执行 xhs-auth 技能。
内容发布（"发布 / 发帖 / 上传图文 / 上传视频"）→ 执行 xhs-publish 技能。
搜索发现（"搜索笔记 / 查看详情 / 浏览首页 / 查看用户"）→ 执行 xhs-explore 技能。
社交互动（"评论 / 回复 / 点赞 / 收藏"）→ 执行 xhs-interact 技能。
复合运营（"竞品分析 / 热点追踪 / 批量互动 / 一键创作"）→ 执行 xhs-content-ops 技能。
全局约束
