---
rating: ⭐⭐
title: dokploy-ghcr-deploy
url: https://skills.sh/jeffreyyvdb/dokploy-ghcr-deploy/dokploy-ghcr-deploy
---

# dokploy-ghcr-deploy

skills/jeffreyyvdb/dokploy-ghcr-deploy/dokploy-ghcr-deploy
dokploy-ghcr-deploy
Installation
$ npx skills add https://github.com/jeffreyyvdb/dokploy-ghcr-deploy --skill dokploy-ghcr-deploy
SKILL.md
...
Dockerfile exists and exposes a port. Read it. Grep for EXPOSE. Capture the port number — you'll need it for the Dokploy domain. If there's no Dockerfile or no EXPOSE, stop and ask the user; do not fabricate one from framework defaults (that would be silent guesswork).
.dockerignore exists. If missing, offer to add a minimal one (node_modules, .git, build, dist, tests, .env*). Not a blocker.
gh CLI authenticated. gh auth status should show an active account. If not, stop and ask the user to run gh auth login first.
Dokploy MCP is connected. Call mcp__dokploy-mcp__user-session. If it errors, stop and ask the user to connect the Dokploy MCP before continuing — the skill can't do the Dokploy half without it.
GitHub remote. Parse git remote get-url origin to extract owner/repo. Lowercase it — GHCR rejects uppercase in image paths. If the remote isn't GitHub, this skill doesn't apply; tell the user.
...
Phase A — Repo prep
Write .github/workflows/docker-publish.yml from assets/docker-publish.yml.template. The template uses ${{ github.repository }} and lowercases it at runtime, so it works unmodified for most repos — you don't normally need to substitute anything.
Create a feature branch if not already on one (don't commit straight to main — we want the workflow to land atomically with a PR once secrets are in place).
Stage and commit the workflow. Don't push yet — we want Dokploy and GitHub secrets configured first, otherwise the very first workflow run will fire against an app that isn't ready and leave a failed deployment in the history.
...
Resolve the organization — call mcp__dokploy-mcp__organization-active and save organizationId.
mcp__dokploy-mcp__project-create with name and a short description. The response includes both a projectId and an auto-created default environmentId (named production) — save both.
mcp__dokploy-mcp__application-create with name (the app name) and environmentId. Save applicationId. The app starts with sourceType: "github"; it flips to docker when you save the Docker provider next.
mcp__dokploy-mcp__application-saveDockerProvider with:
applicationId
...
mcp__dokploy-mcp__user-createApiKey with name: "github-actions-deploy-<repo>" and metadata: {"organizationId": "<from step 1>"}. Save the key — it's shown once. Flag to the user that the default rate limit is 10 requests / 24h; offer to raise it via the Dokploy UI if they push to main a lot.
...
GitHub → Settings → Developer settings → Personal access tokens (classic) → Generate new token → scope read:packages only → set expiration to 90 days (GitHub's recommended max for machine credentials; do not pick "No expiration") → generate → copy the token to your clipboard.
Dokploy → Projects → your project → your app → General → Docker provider → paste the token into the Password field → Save.
Reply here with "saved" (no token, no paste).
...
Merge the PR (or push to main). The workflow fires.
Watch the Actions tab: the build should push two tags (latest and sha-<short>) and the final step should return 2xx from Dokploy.
In Dokploy → app → Deployments, confirm a new deployment went from "Pulling" → "Running" → "Done".
Hit the domain; expect the app to respond on the port from the Dockerfile.
