...
url: https://skills.sh/zsy1207/atmospheric-science-research/atmospheric-science-research
---

# atmospheric-science-research

skills/zsy1207/atmospheric-science-research/atmospheric-science-research
atmospheric-science-research
Installation
...
plot-standards.md	Colormap tables, vector specs, projections, figure sizing, quick reject checklist	MUST read before writing ANY plot code
review.md	RR loop procedure, quick fix recipes, sanity checks	MUST read before starting ANY RR loop
readme-template.md	Chinese README template	MUST read after RR reaches PASS
Before Writing Code
...
import cmaps + explicit levels — NO EXCEPTIONS. Domain-specific colormaps carry scientific meaning. NEVER use jet, rainbow, viridis, or any generic matplotlib colormap. These lack the perceptual structure needed for scientific communication and WILL be rejected.

NO full titles, NO stamps — EVER. NEVER call suptitle() or add source/data stamps to figures. Use ONLY ax.set_title("(a) Brief subtitle", loc="left") for panel labels — centered or descriptive-only titles are FORBIDDEN. A figure with a centered title or stamp is an immediate REVISE.

ALWAYS open and inspect the rendered PNG — NEVER review code alone. Code that looks correct can render with overlapping labels, clipped elements, or wrong colors. You MUST open the actual PNG with the Read tool in EVERY RR iteration. Skipping this step is the single most common source of defects.

Reuse existing project layout — do NOT reorganize. Existing directories reflect the user's decisions. ONLY create data_processed/ + figN/ when NO structure exists. Restructuring without explicit permission is FORBIDDEN.

Separate compute from plot — ALWAYS. Atmospheric computations (regridding, EOF, climatology) are expensive. NEVER mix compute and plot in the same script. Compute saves to disk; plot reads from disk. No exceptions.

NO environment setup — EVER. All packages are pre-installed. NEVER run pip install, conda install, import checks, or version probes. These waste time and clutter output.
...
Read existing code. Identify the MINIMUM change needed — nothing more.
...
Open PNG with Read tool
Check layout proportions FIRST (colorbar sizing, element crowding, subplot spacing)
Check against Core Standards + quick reject checklist — every item
Verify physical plausibility (values, units, sign conventions, spatial patterns)
Classify: REVISE / BLOCKED / PASS
