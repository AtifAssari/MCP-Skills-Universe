...
rating: ⭐⭐
title: wgsl
url: https://skills.sh/juzi5201314/cruft/wgsl
---

# wgsl

skills/juzi5201314/cruft/wgsl
wgsl
Installation
...
入口点签名：@vertex/@fragment/@compute 是否齐全？返回值/输出 struct 的 @location/@builtin 是否完整？

类型一致性：所有算术两端类型是否一致？需要时是否做了 f32(...) / u32(...) 等显式转换？
...
数据布局：uniform/storage 里的 struct 是否考虑了 padding？是否避免了 vec3f 带来的隐式 padding 陷阱？
...
Uniformity：fragment 中是否在分支/循环里调用了 dpdx/dpdy/textureSample* 这类对 uniformity 敏感的函数？能否改成 textureSampleLevel/textureLoad？
...
WGSL：完整代码块（wgsl ... ）
Bindings：Markdown 表（group / binding / kind / wgsl type / stages / note）
IO：Markdown 表（stage / location or builtin / name / type）
Weekly Installs
16
Repository
juzi5201314/cruft
