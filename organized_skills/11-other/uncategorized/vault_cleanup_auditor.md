---
rating: ⭐⭐⭐
title: vault-cleanup-auditor
url: https://skills.sh/brianrwagner/ai-marketing-claude-code-skills/vault-cleanup-auditor
---

# vault-cleanup-auditor

skills/brianrwagner/ai-marketing-claude-code-skills/vault-cleanup-auditor
vault-cleanup-auditor
Installation
...
All 4 checks ran — Confirm output exists between each === CHECK N === / === END CHECK N === marker. If a check produced no output, note "0 issues found" in the report — do not skip the section.
Counts are accurate — The summary line counts must match the actual items listed in each section. Re-count if unsure.
Report saved correctly — Confirm the file exists at vault-audit/YYYY-MM-DD-audit.md. If the write failed, report the error and provide the report content inline.
No blank sections — Every section must either list items OR say "No issues found." Never leave a section header with nothing below it.
