...
url: https://skills.sh/tldraw/tldraw/pr-walkthrough
---

# pr-walkthrough

skills/tldraw/tldraw/pr-walkthrough
pr-walkthrough
Installation
$ npx skills add https://github.com/tldraw/tldraw --skill pr-walkthrough
SKILL.md
...
How it works
For each segment, the script builds a prompt: style preamble + segment text.
One API call to gemini-2.5-pro-tts per segment generates a WAV clip directly.
Each clip is validated (duration sanity check vs word count) and retried automatically if the output is bad.
Leading/trailing silence is trimmed from each clip.
