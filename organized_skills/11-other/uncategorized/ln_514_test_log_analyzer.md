---
rating: ⭐⭐⭐
title: ln-514-test-log-analyzer
url: https://skills.sh/levnikolaevich/claude-code-skills/ln-514-test-log-analyzer
---
...
Check if scripts/analyze_test_logs.mjs exists in target project. If missing, copy from references/analyze_test_logs.mjs.
...
Run script: node scripts/analyze_test_logs.mjs --mode {detected} [options]
If no log sources found → return NO_LOG_SOURCES status, skip to Phase 5.
...
Framework config: Python logging.conf / Django LOGGING / Node LOG_LEVEL
Default: assume INFO if not detected
