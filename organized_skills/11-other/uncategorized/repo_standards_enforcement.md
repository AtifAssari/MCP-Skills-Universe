---
rating: ⭐⭐
title: repo-standards-enforcement
url: https://skills.sh/mwillbanks/agent-skills/repo-standards-enforcement
---

# repo-standards-enforcement

skills/mwillbanks/agent-skills/repo-standards-enforcement
repo-standards-enforcement
Installation
$ npx skills add https://github.com/mwillbanks/agent-skills --skill repo-standards-enforcement
SKILL.md
...
project-specific typecheck script if present
bunx tsc --noEmit
tsc --noEmit only if already wired in the repo
...
Run tests
Run linting / formatting remediation loop
If linting / formatting modified files:
re-run build
re-run tests
...
Parse the linting / formatting output, for example Biome JSON
Convert diagnostics into a remediation task list
Resolve issues in bulk across the affected scope without introducing new violations
Run the linting / formatting command again
...
Do not claim standards compliance without validating it. Do not use vague phrases instead of actual command and result summaries. Always include the specific commands executed (e.g., bunx tsc --noEmit, bun test --coverage) and their outcomes when reporting on standards enforcement. If any step could not be completed, document the reason honestly and indicate any potential impact on compliance.
...
This skill defines repository-wide standards. It should not override a more specific skill that exists specifically to execute a narrower concern more correctly.
