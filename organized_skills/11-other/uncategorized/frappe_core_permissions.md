---
rating: ⭐⭐⭐
title: frappe-core-permissions
url: https://skills.sh/openaec-foundation/erpnext_anthropic_claude_development_skill_package/frappe-core-permissions
---

# frappe-core-permissions

skills/openaec-foundation/erpnext_anthropic_claude_development_skill_package/frappe-core-permissions
frappe-core-permissions
Installation
$ npx skills add https://github.com/openaec-foundation/erpnext_anthropic_claude_development_skill_package --skill frappe-core-permissions
SKILL.md
Frappe Permissions
...
Critical Rules
ALWAYS use frappe.has_permission() — NEVER check roles directly for access control
ALWAYS use frappe.get_list() for user-facing queries — NEVER get_all()
ALWAYS escape SQL in query hooks — frappe.db.escape(user)
ALWAYS prefix table names in query hooks — `tabDocType`.fieldname
ALWAYS return None in has_permission hooks by default — NEVER True
ALWAYS clear cache after permission changes — frappe.clear_cache()
ALWAYS document ignore_permissions usage with a comment
NEVER throw errors in has_permission hooks — return False to deny
NEVER grant permlevel 1+ without granting permlevel 0 first
...
has_permission hook — Can only deny (any False = denied)
Sharing — Grants access to shared documents
