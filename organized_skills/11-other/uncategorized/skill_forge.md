...
rating: ⭐⭐⭐
title: skill-forge
url: https://skills.sh/j5ik2o/ai-tools/skill-forge
---

# skill-forge

skills/j5ik2o/ai-tools/skill-forge
skill-forge
Installation
$ npx skills add https://github.com/j5ik2o/ai-tools --skill skill-forge
SKILL.md
Skill Forge
...
Before running any scripts, make sure the Python environment is ready. The scripts depend on packages managed by uv, and .venv is gitignored — so it won't exist in a fresh clone or a git worktree.
...
Aggregate into benchmark — run the aggregation script from the skill-forge directory:
...
Use the model ID from your system prompt (the one powering the current session) so the triggering test matches what the user actually experiences. The same CLI selection is used for both evaluation and description improvement. No separate Anthropic API client setup is required for the optimization loop.
