...
rating: ⭐⭐⭐
title: bisect
url: https://skills.sh/clickhouse/clickhouse/bisect
---

# bisect

skills/clickhouse/clickhouse/bisect
bisect
Installation
...
For each commit, helpers/download.sh fetches the release binary from CI S3
env/<option>.sh installs configs and starts a ClickHouse server (configs go to utils/auto-bisect/data/etc/, never to /etc/)
Your test script is called with $COMMIT_SHA, $CH_PATH (binary), and $GIT_WORK_TREE in the environment
Exit code from your test determines good/bad/skip
