...
rating: ⭐⭐⭐
title: local-issue
url: https://skills.sh/doiiarx/claude-skills/local-issue
---

# local-issue

skills/doiiarx/claude-skills/local-issue
local-issue
Installation
...
更深层的意义在于：每个独立的 issue 文件本质上是一个事件（event）。项目由此从线性的任务列表，演变为事件驱动的结构——多个 agent 可以各自认领不同的 issue 并行推进，CI/CD 流水线可以监听 issue 状态变更并自动触发，代码与任务的边界在 git 历史中完全透明。这或许正是 AI 原生开发的新范式：不是 AI 辅助人类写代码，而是以 issue 为协议，人与 agent 协同驱动一个持续演进的系统。
...
CLAUDE.md / AGENTS.md — AI agent 专属规范
CONTRIBUTING.md — 项目贡献规范
git log --oneline — 从历史提交归纳风格
...
从文件名提取 type：NNN-{type}-description.md 从文件头提取 Priority：grep "^\*\*Priority\*\*:" file.md
...
扫描得最大编号为 046，新建 047
类型：bug
检查 .issues/templates/bug.md → 存在，使用项目模板
文件：.issues/open/047-bug-websocket-reconnect.md
填写模板内容，描述断线重连问题
git commit -m "docs: add issue #047 - WebSocket断线重连bug"
开始分析代码，在进展记录中更新状态
Weekly Installs
13
Repository
doiiarx/claude-skills
GitHub Stars
16
First Seen
Feb 27, 2026
