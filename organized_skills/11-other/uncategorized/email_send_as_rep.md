...
url: https://skills.sh/sixtysecondsapp/use60/email-send-as-rep
---

# email send-as-rep

skills/sixtysecondsapp/use60/Email Send-as-Rep
Email Send-as-Rep
Installation
$ npx skills add https://github.com/sixtysecondsapp/use60 --skill 'Email Send-as-Rep'
SKILL.md
Available Context
...
Load the thread -- fetch all messages, identify the most recent
Set threading headers -- In-Reply-To and References headers from the last message's Message-ID. This is critical for threading. See references/email-composition.md for header format.
Set subject -- Re: [original subject] (do not stack Re: Re: Re:)
Set recipients -- default to Reply (sender only) unless rep specifies Reply All
Compose body -- reference the thread context, respond to what was said, advance the conversation
Quote the previous message -- include the quoted reply block below the new content (standard email convention)
Append signature -- after the body, before the quoted content
2.2 New Email (No Thread)

When composing fresh:

Set subject -- concise, specific, no clickbait. Good: "Meeting follow-up: pricing discussion". Bad: "Quick question" or "Checking in"
Set recipients -- resolve contact names to email addresses via CRM lookup
Compose body -- use the rep's writing style, reference relevant CRM context
Append signature -- at the end of the body
...
Update Slack message: "Sending in 30 seconds... [Cancel Send]"
Start countdown timer
If rep clicks "Cancel Send" within 30 seconds:
Cancel the send
Update Slack: "Email cancelled. Not sent."
Log as cancelled_during_undo
If 30 seconds pass without cancellation, proceed to send
...
OAuth expired during send -- the token expired between approval and send
...
Rate limit hit -- Gmail API rate limit (250 sends/day for workspace, 500 for paid)
...
Recipient rejected -- mailbox full, address doesn't exist, etc.
...
Do not attempt to compose the email. Respond with: "You need to connect your Gmail account before I can send emails on your behalf. Go to Settings > Integrations > Gmail and connect your account. Make sure to grant the 'Send email' permission when prompted."

"The recipient email bounced"

Record the bounce, update the contact record, and inform the rep: "The email to [address] bounced: [reason]. I've updated their contact record. Would you like to try a different email address?"
...
The drafting skill produces the email body
The rep reviews the draft (first approval gate -- content)
email-send-as-rep takes the approved content and presents it for send approval (second approval gate -- sending)
Both approvals are required. The content approval does NOT count as send approval.
...
Attempt silent refresh using the stored refresh_token
If refresh succeeds, continue transparently
If refresh fails, inform the rep: "Your Gmail connection needs to be refreshed. Please click [reconnect link] and I'll retry the send."
Store the composed email in pending state so the rep doesn't lose their work
"Rate limit reached"
...
If the platform daily limit is reached, do not queue silently. Inform the rep and let them decide: "You've hit your daily send limit of [limit] emails. This resets at midnight [timezone]. Would you like me to schedule this for tomorrow morning?"

"Slack is down during approval"

Fall back to the copilot chat approval flow. The rules are identical:

Present the full email preview in chat
Ask for explicit "Confirm" or "Send" response
Apply the same 30-minute expiration
Apply the same 30-second undo window
Log the approval channel as copilot_chat instead of slack
...
Use the edited version, not the original template
Log composition_source: 'rep_edited' in the audit
Do not "correct" their edits or revert to the template
The sequence continues with their edited version as the record of what was sent
Weekly Installs
–
Repository
