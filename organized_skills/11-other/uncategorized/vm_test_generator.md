---
rating: ⭐⭐⭐⭐⭐
title: vm-test-generator
url: https://skills.sh/zeal422/dev-toolbelt-by-vm/vm-test-generator
---

# vm-test-generator

skills/zeal422/dev-toolbelt-by-vm/vm-test-generator
vm-test-generator
Installation
...
tests/ ├── unit/ │ ├── services/ │ │ ├── payment.test.js (NEW) │ │ └── user.test.js (UPDATE) │ ├── models/ │ │ └── Order.test.js (NEW) │ ├── utils/ │ │ ├── validation.test.js (NEW) │ │ └── formatter.test.js (UPDATE) │ └── components/ │ └── CheckoutForm.test.jsx (NEW) ├── integration/ │ ├── checkout-flow.test.js (NEW) │ └── payment-flow.test.js (NEW) └── fixtures/ ├── mockOrders.js (NEW) └── mockUsers.js (UPDATE)
...
Current behavior snapshot - Ensure changes don't break existing functionality
Error path coverage - Test all error conditions
Edge case validation - Boundary values, null/undefined, empty inputs
Integration checkpoints - Verify component interactions
Performance benchmarks - Track performance regressions (when applicable)
...
Execution Notes
Framework detection is critical - Wrong framework = wrong test structure
Analyze existing tests first - Don't duplicate, extend
Generate meaningful test data - Not just placeholder values
Cover error paths - Don't just test happy path
Use real examples - Based on actual code, not generic templates
Organize logically - Follow framework conventions
Include setup/teardown - Proper test isolation
