...
url: https://skills.sh/dotnet/skills/dotnet-aot-compat
---

# dotnet-aot-compat

skills/dotnet/skills/dotnet-aot-compat
dotnet-aot-compat
Installation
...
Common Gotchas
External types without AOT-safe serialization: When a type comes from a dependency you can't modify (e.g., ResponseError from Azure.Core) and it lacks a source-generated serializer, Options.GetConverter<T>() is reflection-based and will produce IL warnings. First check if the type implements IJsonModel<T> (common in Azure SDK) — if so, bypass JsonSerializer entirely:
