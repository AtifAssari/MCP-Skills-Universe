---
rating: ⭐⭐⭐
title: a-stock-daily-report
url: https://skills.sh/zsxink/skills-hub/a-stock-daily-report
---

# a-stock-daily-report

skills/zsxink/skills-hub/a-stock-daily-report
a-stock-daily-report
Installation
...
如需在晚间获取数据，建议：
缓存机制：在收盘后 30 分钟内运行一次并缓存数据
手动维护：使用本地存储的指数数据
降级处理：指数数据不可用时仍可生成板块数据报告
环境变量
...
故障排除
运行失败或数据异常
确认 Node.js 版本 >= 14：node --version
检查网络连接
查看错误日志（stderr 输出）
确认 API 是否可用（非交易时间、周末/节假日可能关闭）
检查是否为交易日并确认运行时间合适
输出格式
