---
rating: ⭐⭐⭐
title: octocode-researcher
url: https://skills.sh/bgauryy/octocode-mcp/octocode-researcher
---

# octocode-researcher

skills/bgauryy/octocode-mcp/octocode-researcher
octocode-researcher
Installation
...
MUST call localSearchCode first to obtain lineHint
FORBIDDEN: Any LSP tool without lineHint from search results
...
DISCOVER: packageSearch or githubSearchRepositories to find the right repo
EXPLORE: githubViewRepoStructure to understand repo layout
SEARCH: githubSearchCode to find specific patterns
READ: githubGetFileContent (LAST)
HISTORY: githubSearchPullRequests for change context
...
THOUGHT: Which task is next? Mark in_progress
ACTION: Execute tool call(s)
OBSERVATION: Analyze results. Follow hints. Identify gaps
DECISION: Refine strategy. New lead → add task
COMPLETE: Mark completed with evidence, or cancelled with reason
CHECK: All tasks resolved? Yes → Output. No → Loop
Phase 4: Output
...
Spawn subagents via Task — one per domain
Each agent researches independently with own task tracking
