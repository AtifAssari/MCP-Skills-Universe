...
rating: ⭐⭐⭐
title: ado-mapper
url: https://skills.sh/anton-abyzov/specweave/ado-mapper
---

# ado-mapper

skills/anton-abyzov/specweave/ado-mapper
ado-mapper
Installation
$ npx skills add https://github.com/anton-abyzov/specweave --skill ado-mapper
SKILL.md
...
Export SpecWeave increments to ADO (Increment → Epic + Features + User Stories + Tasks)
Import ADO Epics as SpecWeave increments (Epic → Increment structure)
Bidirectional sync with conflict detection and resolution
Maintain traceability (store IDs, URLs, timestamps)
Validate mapping accuracy using test cases
Handle edge cases (missing fields, custom workflows, API errors)
...
Create ADO Features OR User Stories:
...
Update ADO Epic (add custom field):
