...
rating: ⭐⭐
title: prd-skill2026
url: https://skills.sh/site/skills.volces.com/prd-skill2026
---

# prd-skill2026

skills/skills.volces.com/product-prd-writer
product-prd-writer
$ npx skills add https://skills.volces.com/skills/clawhub/zhao202404@prd-skill2026
SKILL.md
Product PRD Writer
Instructions
...
摘要（1 段话 + 目标）
背景与问题定义（现状、痛点、为什么现在做）
目标与成功标准（KPI、指标口径、达成方式）
非目标（不做什么）
目标用户与使用场景（角色、旅程/场景）
需求概览（按优先级/模块做总览）
功能需求（逐条列出，可表格化）
非功能需求（性能、安全、可用性、合规、可维护性等）
交互/原型要点（用文字描述关键流程与交互状态）
数据埋点与分析（事件、字段、用于衡量 KPI 的逻辑）
权限与流程（如适用）
验收标准（可测试、可核查）
里程碑与发布计划
依赖、风险与对策
...
用户输入（示例）： “帮我做一个 PRD：一键补贴申请，面向新用户，减少申请流程。”
