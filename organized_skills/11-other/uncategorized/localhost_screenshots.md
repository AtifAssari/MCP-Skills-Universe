---
rating: ⭐⭐
title: localhost-screenshots
url: https://skills.sh/t4sh/skills4sh/localhost-screenshots
---

# localhost-screenshots

skills/t4sh/skills4sh/localhost-screenshots
localhost-screenshots
Installation
...
Ensure a browser tab (create if empty).
Navigate to http://localhost:<port>/<path>.
Take a viewport screenshot.
Optionally resize, then screenshot again.
...
Golden Rules
Always use Playwright’s bundled Chromium. Never use Puppeteer, Selenium, or system Chrome. Do not check for installed browsers.
Prefer HTTP; file:// only for self-contained static HTML. Serve over HTTP whenever a dev server, build output, or npx serve is available. file:// is acceptable only when the page has no fetch/XHR to sibling files, no <script type="module">, no service workers, and no absolute /asset paths — otherwise those will break silently. When in doubt, serve over HTTP.
...
Quick workflow
Ensure the site is served over HTTP (user’s dev server, or npx serve _site -l 3000 &).
Verify the server responds before screenshotting (see playwright-patterns.md § “Verifying the Server”).
Capture all 8 standard breakpoints (320–1920px) unless the user asks for specific sizes. Check the project’s CSS/Tailwind config for custom breakpoints first.
Save to _screenshots/ in the project folder.
...
File	Contents
references/playwright-patterns.md	Pre-flight checks, serving patterns, persistent sessions, breakpoint detection, stdin-friendly script templates
...
references/ai-snapshots.md	Accessibility tree, DOM snapshots, interactive element maps, incremental DOM diff
references/troubleshooting.md	Common issues by project type (SSG, Next.js, Tailwind, WordPress)
Weekly Installs
21
Repository
t4sh/skills4sh
