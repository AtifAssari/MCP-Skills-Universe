---
rating: ⭐⭐
title: mermaid-to-proverif
url: https://skills.sh/trailofbits/skills/mermaid-to-proverif
---

# mermaid-to-proverif

skills/trailofbits/skills/mermaid-to-proverif
mermaid-to-proverif
Installation
$ npx skills add https://github.com/trailofbits/skills --skill mermaid-to-proverif
SKILL.md
Mermaid to ProVerif
...
Extract every participant or actor declaration. Each becomes a ProVerif process.
Count message arrows (->>, -->>, -x, --x). Each distinct A ->> B: label creates a communication step on a channel.
Decide channel model:
...
Functions — constructors and destructors. Destructors use inline reduc so that the process aborts on verification or decryption failure:
...
Generates long-term keys with new
Publishes public keys to the attacker via out(c, pk(sk))
Runs participant processes in parallel under replication (!) to allow multiple sessions
