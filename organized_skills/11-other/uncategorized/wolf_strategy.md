...
rating: ⭐⭐⭐
title: wolf-strategy
url: https://skills.sh/senpi-ai/senpi-skills/wolf-strategy
---

# wolf-strategy

skills/senpi-ai/senpi-skills/wolf-strategy
wolf-strategy
Installation
...
Quick Start
Ensure Senpi MCP is connected (mcporter list should show senpi)
Ensure the dsl-dynamic-stop-loss skill is installed alongside this skill (provides dsl-cli.py and dsl-v5.py)
Create a custom strategy wallet: use strategy_create_custom_strategy via mcporter
Fund the wallet via strategy_top_up with your budget
Determine the user's AI provider — which provider is configured in OpenClaw? (anthropic, openai, or google)
Run setup: python3 scripts/wolf-setup.py --wallet 0x... --strategy-id UUID --budget 6500 --chat-id 12345 --provider anthropic
...
Upgrading from v6.1.x? Migrate existing DSL state files: python3 scripts/wolf-migrate-dsl.py
...
Create the 5 wolf crons + 1 DSL cron per strategy using templates from references/cron-templates.md
...
Call dsl-cli.py delete-dsl {strategyId} {asset} {main|xyz} --state-dir {DSL_STATE_DIR} to archive the DSL state
If CLI output has cron_to_remove → remove that DSL cron (it was the last position for that strategy)
...
Check output for dsl_cron_needed: true — if present, create the DSL cron for this strategy
Alert user
Closing
Close via close_position MCP call (or DSL auto-closes via its own cron)
Call dsl-cli.py delete-dsl {strategyId} {asset} {dex} to archive DSL state — never set active: false directly
If CLI output has cron_to_remove → remove that DSL cron
