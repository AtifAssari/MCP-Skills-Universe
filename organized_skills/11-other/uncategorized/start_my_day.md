...
rating: ⭐⭐⭐
title: start-my-day
url: https://skills.sh/juliye2025/evil-read-arxiv/start-my-day
---

# start-my-day

skills/juliye2025/evil-read-arxiv/start-my-day
start-my-day
Installation
...
搜索 arXiv：调用 arXiv API 获取论文
解析 XML：提取论文信息（ID、标题、作者、摘要等）
筛选论文：根据研究兴趣配置文件筛选
计算评分：综合相关性、新近性、质量等维度
输出 JSON：保存筛选后的结果到 arxiv_filtered.json
scan_existing_notes.py

位于 scripts/scan_existing_notes.py，功能包括：

扫描笔记目录：扫描 20_Research/Papers/ 下所有 .md 文件
提取笔记信息：
文件路径
文件名
...
tags 字段
构建索引：创建关键词到笔记路径的映射表
输出 JSON：保存索引到 existing_notes_index.json
...
读取文本：读取需要处理的文本内容
读取笔记索引：从 existing_notes_index.json 加载笔记映射
