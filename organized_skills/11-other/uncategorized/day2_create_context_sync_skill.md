---
rating: ⭐⭐⭐
title: day2-create-context-sync-skill
url: https://skills.sh/ai-native-camp/camp-1/day2-create-context-sync-skill
---
...
핵심 금지 사항 (절대 위반 금지)
Phase A에서 AskUserQuestion을 호출하지 않는다 (Block 0, 2, 4 제외) — 이 3개 블록은 사용자 선택이 필수이므로 예외
...
Block 0에서 사용자가 도구를 선택하면, templates/context-sync.md 기반으로 스킬 파일을 즉시 생성한다
이후 Block 1~5에서 생성된 스킬의 해당 부분만 수정/확장한다
...
references/block0-tool-selection.md의 EXPLAIN 섹션을 읽고 설명한다
AskUserQuestion으로 도구를 선택받는다 (multiSelect: true)
templates/context-sync.md 템플릿을 읽는다
선택된 도구에 맞춰 사용자의 프로젝트에 .claude/skills/my-context-sync/SKILL.md를 생성한다
생성된 파일의 전체 구조만 간략히 보여주고 Stop한다 (세부 내용은 이후 블록에서)
Block 2 예외 규칙
...
references/block2-tool-connection.md의 MCP 안내를 따른다
scripts/mcp_servers.py를 사용하여 GitHub README.md에서 적합한 MCP 서버를 검색한다
검색 결과를 보여주고, Claude가 .mcp.json에 서버를 등록한다
/mcp 명령으로 서버 연결 상태를 함께 확인한다

API 선택 시:

Claude가 직접 API 호출 코드를 작성한다
사용자의 스킬 scripts/ 폴더에 저장한다
Block 4 예외 규칙
