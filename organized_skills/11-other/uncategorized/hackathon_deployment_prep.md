---
rating: ⭐⭐
title: hackathon-deployment-prep
url: https://skills.sh/bernieweb3/hackathon-ai-devkit/hackathon-deployment-prep
---

# hackathon-deployment-prep

skills/bernieweb3/hackathon-ai-devkit/hackathon-deployment-prep
hackathon-deployment-prep
Installation
...
Rules
deployment_checklist must be executable in order from a fresh machine or cloud shell.
Every item in mvp_demo_flow must have a corresponding demo_data_setup entry.
Every item in demo_blockers must have a corresponding fallback_plan entry.
go_no_go_criteria must be binary pass/fail — no partial states.
If judging_format includes live_demo, a screen recording fallback is mandatory.
Flag any deployment step that requires >15 minutes as [TIME-RISK].
demo_environment_plan must specify exact browser, zoom level, window layout, and test account state.
Output Format
