...
rating: ⭐⭐⭐
title: cancel
url: https://skills.sh/yeachan-heo/oh-my-claudecode/cancel
---

# cancel

skills/yeachan-heo/oh-my-claudecode/cancel
cancel
Installation
...
Self-Improve (standalone — clear state, clean orphaned worktrees, preserve iteration_state for resume, set status: "user_stopped" in the resolved <self-improve-root>/state/agent-settings.json; new runs use .omc/self-improve/topics/<topic-slug>/, with flat .omc/self-improve/ retained only for legacy single-track resumes)
Force Clear All
...
state_list_active enumerates .omc/state/sessions/{sessionId}/… to find every known session.
state_clear runs once per session to drop that session’s files.
...
Call state_list_active to enumerate .omc/state/sessions/{sessionId}/… and discover every active session.
...
Any cancellation logic in this doc mirrors the dependency order discovered via state tools (autopilot → ralph → …).
...
Checks ps aux (Unix) or tasklist (Windows) for processes with --team-name matching the deleted team
For each orphan whose team config no longer exists: sends SIGTERM, waits 5s, sends SIGKILL if still alive
...
Read ${CLAUDE_CONFIG_DIR:-~/.claude}/teams/*/config.json to find active teams
If multiple teams exist, cancel oldest first (by createdAt)
For each non-lead member, call SendMessage(type: "shutdown_request", recipient: member-name, content: "Cancelling")
Wait briefly for shutdown responses (15s per member timeout)
Re-read config.json to check for remaining members (reconciliation pass)
Call TeamDelete() to clean up
Clear team state: state_clear(mode="team", session_id)
Report structured summary to user
If Autopilot Active

Autopilot handles its own cleanup including linked ralph and ultraqa.

Read autopilot state via state_read(mode="autopilot", session_id) to get current phase
Check for linked ralph via state_read(mode="ralph", session_id):
...
Check for linked ultraqa via state_read(mode="ultraqa", session_id):
If active, clear it: state_clear(mode="ultraqa", session_id)
Mark autopilot inactive (preserve state for resume) via state_write(mode="autopilot", session_id, state={active: false, ...existing})
If Ralph Active (but not Autopilot)
Read ralph state via state_read(mode="ralph", session_id) to check for linked ultrawork
If linked_ultrawork: true:
...
Read ultrawork state via state_read(mode="ultrawork", session_id)
If linked_to_ralph: true, warn user to cancel ralph instead (which cascades)
Otherwise clear: state_clear(mode="ultrawork", session_id)
...
Use state_list_active to enumerate known session ids and state_get_status to learn the active mode (autopilot, ralph, ultrawork, etc.) for each session.
When operating in default mode, call state_clear with that session_id to remove only the session’s files, then run mode-specific cleanup (autopilot → ralph → …) based on the state tool signals.
In force mode, iterate every active session, call state_clear per session, then run a global state_clear without session_id to drop legacy files (.omc/state/*.json, compatibility artifacts) and report success. Swarm remains a shared SQLite/marker mode outside session scoping.
Team artifacts (~/.claude/teams/*/, ~/.claude/tasks/*/, .omc/state/team-state.json) remain best-effort cleanup items invoked during the legacy/global pass.
Always clear skill-active state as the final step, regardless of which mode was active or whether --force was used:
...
Check for active MCP workers: Look for heartbeat files at .omc/state/team-bridge/{team}/*.heartbeat.json
Send shutdown signals: Write shutdown signal files for each active worker
Kill tmux sessions: Run tmux kill-session -t omc-team-{team}-{worker} for each worker
Clean up heartbeat files: Remove all heartbeat files for the team
Clean up shadow registry: Remove .omc/state/team-mcp-workers.json
