...
rating: ⭐⭐⭐
title: genviral
url: https://skills.sh/fdarkaou/genviral-skill/genviral
---

# genviral

skills/fdarkaou/genviral-skill/genviral
genviral
Installation
$ npx skills add https://github.com/fdarkaou/genviral-skill --skill genviral
SKILL.md
...
Run trend-brief first for the niche keyword (7d baseline, then 24h for freshness if needed).
Extract and report: top hashtags, top sounds, top creators, posting windows (UTC), recommended hook angles.
Run competitor deep-dive using docs/references/competitor-research.md (3-5 accounts minimum).
Produce a short actionable output with:
3 hook angles to test
2 CTA suggestions
2 best posting windows
5 hashtags to start with
1 "gap to exploit" insight
Save findings to workspace/performance/competitor-insights.md and use them in subsequent content prompts.
...
Run copy-tiktok-preview first and capture preview_id, source image URLs, and slide count.
Run copy-tiktok-import with exactly one pack source (pack_id or pack_images) so text overlays are extracted and mapped into editable slides.
Generate replacement images through Studio using google/nano-banana-2 with each source slide image as reference input:
...
Build prompts with product context from workspace/context/product.md so generated visuals stay relevant to the advertised product.
Replace slide backgrounds with generated URLs (update --slides-file ...) while preserving/editing imported text elements.
Render (render) and review (review) before posting; never skip visual QA.
Analytics Correlation Mode (When user asks "match analytics back to posts")
Pull analytics-posts --json.
Prefer genviralPostId for matching analytics rows back to Genviral posts.
If you need the caller's own stable ID, use externalId.
Treat analyticsId/legacy id as the analytics-row identifier only.
Treat platformPostId as the platform-native TikTok/Instagram/YouTube post identifier only.
For BYO TikTok drafts (MEDIA_UPLOAD), do not assume a draft upload is correlated immediately. The draft publish_id becomes matchable only after the human posts from TikTok and Genviral resolves it to the final public TikTok video ID.
