---
rating: ⭐⭐⭐
title: universal-pptx-generator
url: https://skills.sh/ajaxhe/universal-pptx-generator-skill/universal-pptx-generator
---

# universal-pptx-generator

skills/ajaxhe/universal-pptx-generator-skill/universal-pptx-generator
universal-pptx-generator
Installation
...
精确字号 - sz="8000" 表示 80pt (sz ÷ 100)
精确位置 - <a:off x="..." y="..."/> EMU单位，转换为英寸或百分比
精确颜色 - srgbClr val="FFFFFF" 直接使用
字体粗细 - b="1" 表示粗体
⭐ 对齐方式 - algn="ctr" 居中, algn="r" 右对齐, algn="l" 或无属性表示左对齐
1.1 解压 PPTX 模板
...
使用 fit: 'shrink' - 自动缩小字号以适应宽度（推荐）
使用足够大的宽度 - 确保文本框宽度足够容纳所有文字
根据文字长度动态计算宽度 - 中文约每字符 0.7-1.0 × 字号(pt) / 72 英寸
// ⭐ 计算文本所需宽度的辅助函数
...
常见错误：假设目录项字号是 60pt 或 32pt，但实际可能只有 20-30pt。 必须从模板 XML 的 sz 属性精确提取！
...
精确提取参数 - 所有字号、位置、颜色必须从 slide XML 精确提取
精确提取字体 - ⭐⭐⭐ 字体名称必须从 <a:latin typeface="..."/> 和 <a:ea typeface="..."/> 精确提取，不同模板字体完全不同！
精确提取对齐方式 - ⭐⭐ 对齐方式必须从 algn 属性提取，不同模板的对齐设置差异很大！
不要添加额外元素 - 如果模板目录页没有"目录"标题，就不要添加
保持布局一致 - 位置和尺寸必须与模板匹配
单位换算正确 - sz ÷ 100 = pt, EMU ÷ 914400 = inch
常见错误避免
错误	正确做法
