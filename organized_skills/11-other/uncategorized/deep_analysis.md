...
rating: ⭐⭐⭐
title: deep-analysis
url: https://skills.sh/wbh604/uzi-skill/deep-analysis
---

# deep-analysis

skills/wbh604/uzi-skill/deep-analysis
deep-analysis
Installation
$ npx skills add https://github.com/wbh604/uzi-skill --skill deep-analysis
SKILL.md
...
读 .cache/_global/update_prompt.md（不存在 = 无需提示，跳过即可）
若存在 · 把文件完整内容作为 第一条消息 展示给用户
用 AskUserQuestion 或自然语言收集用户回答（y / s / n）
调用 python3 -c "from lib.update_check import handle_answer; print(handle_answer('<ans>', '<latest>'))" 把用户回答写回状态文件 · latest 从提示文案里的版本号提取
处理完删除 .cache/_global/update_prompt.md，这样同一会话不会重复弹
根据用户选择：
...
读 _resolve_error.json 拿到 user_input 和候选列表
用 AskUserQuestion 把 Top 3-5 候选呈现给用户（"你是不是想输入 X？"）
用户确认后，用选中的代码（如 000582.SZ）而不是原始名字重跑 stage1()
若候选为空（真查不到），告知用户并建议直接输入代码
...
读 _resolve_error.json，拿 label / why / top_holdings
向用户明确说明："本插件是个股深度分析引擎，{label} 未覆盖"
若是 ETF（top_holdings 非空）：
列出前 10 大持仓（已在 payload 里）：rank / name / code / weight_pct
用 AskUserQuestion 问："你想分析 ETF 里的哪只成分股？"
用户选定后用成分股代码（如 601899.SH）重跑 stage1()
若是 LOF 基金：告知"基金评估用专门工具，本插件只分析个股"
若是可转债：建议"分析正股或用集思录可转债工具"
...
读 skills/deep-analysis/personas/{investor_id}.yaml（id 跟 panel.json 里一致，如 buffett.yaml / zhao_lg.yaml）
对 flagship persona（12 个）· YAML 优先级 > Rules headline：
...
对 stub persona（39 个 · _meta.status=auto_generated_stub）· Rules 引擎输出优先：
YAML 仅补充身份信息（school / group）
不要假装比 Rules 知道更多
可以按 group 风格模板补充简短 voice，但不得编造具体历史言论
prefix-stable system message（如果走 lib.personas.build_system_message）：
...
读 task2.5-qualitative-deep-dive.md — 这是详尽操作手册（6 维每维 4-7 问、6 条跨域 因果链、各维度浏览器 URL 模板、输出 schema）
Spawn 3 个并行 sub-agent（Agent tool · subagent_type=general-purpose）：
...
合并三个 sub-agent 的输出，写入 .cache/{ticker}/agent_analysis.json 的 qualitative_deep_dive 字段（schema 见 task2.5 第 5 节）
质量硬红线：
...
Stage1 末尾的 autofill_via_playwright 只跑一次 · 如果那时某维度 data 非空 但全是 "—"，_dim_needs_fallback 判不需要兜底，会被跳过
Agent 介入后往往知道"哪些维度不够"（自查报告里报 warning/critical）· 应该 主动再跑一次 Playwright · 而不是在 dim_commentary 里写"数据缺失，参考同业" 这种废话
...
调 WebSearch 或 web_search_trusted 补原始资料
调 mx_api（若 MX_APIKEY 已设）
最后降级到 agent 的常识 + 明确标注"基于公开信息推断，非一手"
...
查 dim_16_lhb 前 10 大游资席位是否含 lib.quant_signal.KNOWN_PRIVATE_QUANTS （幻方 / 九坤 / 灵均 / 鸣石 / 因诺 / 明汯 / 玄信 / 衍复 / 宽德 / 念空）
web_search "{name} 幻方 OR 九坤 OR 灵均 OR 明汯 重仓"
akshare.stock_main_stock_holder({code}) 看大股东列表 若交叉验证有 ≥ 1 家私募 + ≥ 1 家公募 → 升级 detected_style 为 quant_factor
...
v2.7 时这是软要求（agent 可能跳过）。v2.9 起是硬编码 block —— assemble_report 跑前强制跑 review，critical 不过就 RuntimeError。

⛔ HARD-GATE-FACTCHECK · 禁止编造未在 raw_data 出现的事实（v2.6）

每条 dim_commentary / debate_say / risks 必须满足：

引用的公司业务必须在 raw_data.dimensions["0_basic"].data.main_business 或 ["5_chain"] 或 ["15_events"] 中能找到出处
引用的财务数字必须在 ["1_financials"] 中
引用的政策必须在 ["13_policy"] snippets 里
不确定的关联用 "据公开报道"/"待 web search 验证" 而不是肯定语气
任何"X 公司是 Y 行业供应链一环"这种宏大叙事，必须 cite raw_data 里的具体证据
...
非 Claude 模型（Codex/国产模型）尤其容易踩这个坑。stage2 不会自动检测事实正确性， 质量靠 agent 自查。

⛔ HARD-GATE-DATAGAPS · 数据缺口 agent 必须接管（v2.3）
浏览器自动化（最稳）— 用 Chrome/Playwright MCP 打开 xueqiu.com/S/{code} 或 quote.eastmoney.com/{code}.html 手动抓字段。特别是 push2.eastmoney.com 被反爬时，浏览器是唯一能拿到实时行情的通道。
MX 妙想 API（若 MX_APIKEY 已设置）— 用 mx_api.MXClient.query()
WebSearch 精确到代码（不要只搜公司名 — 会命中同名无关内容）
逻辑推导 — 从已有数据算（净利率 = 净利润 / 营收；PE = 市值/净利润）
...
绝对禁止：在补不到数据时用默认值（0 / 空字符串 / "—"）当真实数据用、让规则引擎 对空 features 打分（会产生"没数据的幻觉"，像之前"北部港湾"那次 panel 35.4% 共识 其实是空 features 导致的全 fail_msg）。
...
调用 cloudflared tunnel 映射到 https://xxx.trycloudflare.com
