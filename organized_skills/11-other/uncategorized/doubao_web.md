...
rating: ⭐⭐⭐
title: doubao-web
url: https://skills.sh/pjf6568/doubao-web-image/doubao-web
---

# doubao-web

skills/pjf6568/doubao-web-image/doubao-web
doubao-web
Installation
...
UI Automation (UI 自动化): Locate the input box, auto-fill Help me generate an image: {prompt} and simulate pressing Enter. (定位输入框，自动填入 帮我生成图片：{prompt} 并模拟回车。)
Network Interception (网络拦截): Listen to the POST request response of /samantha/chat/completion to get the complete SSE data stream. (监听 /samantha/chat/completion 的 POST 请求响应，获取完整的 SSE 数据流。)
Data Parsing (数据解析): Use regex to match the image_ori URL in the response stream. (使用正则匹配响应流中的 image_ori 的 URL。)
