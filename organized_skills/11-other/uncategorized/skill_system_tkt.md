---
rating: ⭐⭐⭐
title: skill-system-tkt
url: https://skills.sh/arthur0824hao/skills/skill-system-tkt
---

# skill-system-tkt

skills/arthur0824hao/skills/skill-system-tkt
skill-system-tkt
Installation
$ npx skills add https://github.com/arthur0824hao/skills --skill skill-system-tkt
SKILL.md
Skill System TKT
...
--reviewer-close is reserved for ticketless reviewer-owned bundles and rejects bundles that already have TKT-*.yaml files

Close-time artifacts:

review.yaml auto-fills evidence_summary and acceptance_results
review.yaml also includes transition_stats derived from transition_log
done worker/audit tickets auto-store learnings to memory through mem.py store unless --no-memory is used
close JSON includes next_steps_hint for the startup review cycle
...
roadmap decision_gates for the target stage must be passed unless --force
...
python3 "<this-skill-dir>/scripts/tickets.py" claim-ticket --ticket-id TKT-001 [--session-id ses_xxx] [--claimant-role worker|audit|integrator]

--claimant-role is optional for backward compatibility
when provided, the claimant role must match the durable ticket ticket_type
actionable claim-time isolation errors prevent audit/worker role mixups before close-time
block-ticket
...
PM Agent Workflow
Interpret Intent — Parse what the user wants, map to roadmap goals
Create Bundle — Break work into tickets (000 + workers + audit)
Monitor — Track bundle progress, unblock agents
Review — After bundle closes, generate review bundle
Discuss — Present findings to user, gather feedback
Update Roadmap — Reflect completed work and new directions
...
Transitions require --reason and are logged in roadmap.yaml stage_history. Use --force to bypass gate checks (logged but not recommended).
...
DB backend requires PostgreSQL (via skill-system-memory)
{
  ...
  "operations": {
    ...
    "batch-update": {
      "description": "Update multiple filesystem tickets in one command.",
      "input": {
        "bundle": { "type": "string", "required": true },
        "status": { "type": "string", "required": true },
        "evidence": { "type": "string", "required": false },
      ...
      "output": {
        ...
        "fields": { "status": "string", "updated": "integer" }
      },
      "entrypoints": {
        "agent": "Invoke scripts/tkt.sh batch update --bundle <bundle> --status <status> [--evidence <evidence> | --evidence-file <path>] <ticket-id>..."
      }
    },
    "express": {
    ...
    "claim-ticket": {
      ...
      "input": {
        ...
        "claimant_role": { "type": "string", "required": false, "description": "Optional explicit claimant role (worker|audit|integrator)" }
      },
