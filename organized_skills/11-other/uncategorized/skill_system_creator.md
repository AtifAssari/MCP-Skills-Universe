---
rating: ⭐⭐⭐
title: skill-system-creator
url: https://skills.sh/arthur0824hao/skills/skill-system-creator
---

# skill-system-creator

skills/arthur0824hao/skills/skill-system-creator
skill-system-creator
Installation
$ npx skills add https://github.com/arthur0824hao/skills --skill skill-system-creator
SKILL.md
Skill Creator

This skill provides a brainstorm-first surface for exploring ideas with memory + graph context, while still preserving the older skill init/package utilities as subcommands.
...
Create behavior spec (SKILL.spec.yaml via skill-system-behavior:create-spec)
Validate the spec (skill-system-behavior:validate-spec)
Define acceptance tests (structural + behavioral, from spec)
Initialize the skill (run init_skill.py)
Implement the skill (resources + SKILL.md — make tests pass)
Verify structural tests (skill-system-behavior:verify-structural)
Generate behavior contract (skill-system-behavior:generate-contract)
...
PostgreSQL DB targeting standard (mandatory when storage is PostgreSQL)
Resolve DB target by SKILL_PGDATABASE first.
Reject ambient PGDATABASE when SKILL_PGDATABASE is absent.
Otherwise auto-detect from project name as <project>-memory.
Avoid hardcoded DB names in runtime scripts.
...
Implement changes and test again
{
  ...
  "version": "1.0.0",
  "capabilities": ["skill-create", "skill-validate", "skill-package", "artifact-gap-analysis", "artifact-generate", "artifact-brainstorm", "artifact-grill", "artifact-eval"],
  ...
  "operations": {
    ...
    "brainstorm": {
      "description": "Run a context-aware brainstorm and write a decision doc.",
      "input": {
        ...
        "mode": { "type": "string", "required": false, "description": "brainstorm or grill-me" },
        "topic": { "type": "string", "required": false, "description": "Optional brainstorm topic" }
      },
      "output": {
        "description": "Ranked brainstorm suggestions, Socratic prompts, summaries, and a decision doc",
        "fields": { "suggestions": "array", "prompts": "array", "decision_doc": "string" }
      },
      "entrypoints": {
    ...
    "grill-me": {
      "description": "Run challenge-oriented Socratic stress-test and write a decision doc.",
      "input": {
        "limit": { "type": "number", "required": false, "description": "Suggestion limit" },
        "memory_limit": { "type": "number", "required": false, "description": "Memory sample limit" },
        "topic": { "type": "string", "required": false, "description": "Optional stress-test topic" }
      },
      "output": {
        "description": "Challenge-oriented prompts and a decision doc",
        "fields": { "suggestions": "array", "prompts": "array", "decision_doc": "string" }
      },
      "entrypoints": {
        "unix": ["python3", "scripts/creator_tool.py", "grill-me"],
        "windows": ["python", "scripts/creator_tool.py", "grill-me"]
      }
    },
    "eval": {
