...
rating: ⭐⭐⭐
title: paper-writer
url: https://skills.sh/kgraph57/paper-writer-skill/paper-writer
---

# paper-writer

skills/kgraph57/paper-writer-skill/paper-writer
paper-writer
Installation
...
Create data/raw/, data/processed/, data/analysis/ directories
...
Inspect data: Load data/processed/cohort_final.csv, check shape, dtypes, missing values
Table 1: Run scripts/table1.py to generate baseline characteristics table → tables/table1.md
Primary analysis: Choose analysis type based on study design:
...
Use references/pubmed-query-builder.md to construct proper PubMed queries
Have the user validate the final literature list for completeness
Verify every AI-found citation exists (see references/citation-verification.md)
...
Summary of main findings 2-N. Comparison with prior literature (use 00_literature/literature-matrix.md) N+1. Limitations — read ~/.claude/skills/paper-writer/templates/limitations-guide.md for categories, templates, and bilingual examples N+2. Clinical implications / future directions
...
PRISMA flow diagram (Figure 1 — mandatory)
Study characteristics table
Risk of bias summary
Results of individual studies
Results of syntheses (forest plots if meta-analysis)
Reporting biases (funnel plots if ≥10 studies)
...
Verify authenticity: For EVERY AI-suggested reference, confirm the paper exists via WebSearch with the exact title. AI frequently fabricates plausible-sounding citations.
...
Log the submission in log/timeline.md
...
Builder モード: 全セクションから引用収集→ジャーナル形式でフォーマット
Verifier モード: WebSearchで各文献の実在確認→捏造フラグ
...
checklists/escalation-log.md に未解決の Must Fix 一覧と説明を記録
ユーザーにメッセージ表示（該当ファイルパスと問題点を明示）
ワークフローを一時停止
ユーザーが手動修正後「continue」で再開 → イテレーションカウンターを0にリセット
