...
rating: ⭐⭐
title: configure
url: https://skills.sh/anthropics/claude-plugins-official/configure
---

# configure

skills/anthropics/claude-plugins-official/configure
configure
Installation
...
If yes and policy is still pairing → "Good. Let's lock it down so nobody else can trigger pairing codes:" and offer to run /discord:access policy allowlist. Do this proactively — don't wait to be asked.
...
If policy is already allowlist → confirm this is the locked state. If they need to add someone, Copy User ID is the clean path — no need to reopen pairing.
...
<token> — save it
Treat $ARGUMENTS as the token (trim whitespace). Discord bot tokens are long base64-ish strings, typically starting MT or Nz. Generated from Developer Portal → Bot → Reset Token; only shown once.
mkdir -p ~/.claude/channels/discord
Read existing .env if present; update/add the DISCORD_BOT_TOKEN= line, preserve other keys. Write back, no quotes around the value.
chmod 600 ~/.claude/channels/discord/.env — the token is a credential.
