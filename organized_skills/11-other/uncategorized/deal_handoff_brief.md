---
rating: ⭐⭐⭐
title: deal handoff brief
url: https://skills.sh/sixtysecondsapp/use60/deal-handoff-brief
---

# deal handoff brief

skills/sixtysecondsapp/use60/Deal Handoff Brief
Deal Handoff Brief
Installation
$ npx skills add https://github.com/sixtysecondsapp/use60 --skill 'Deal Handoff Brief'
SKILL.md
...
Deal record: execute_action("get_deal", { id: deal_id }) — stage, value, close date, health, MEDDICC fields
All contacts: execute_action("get_deal_contacts", { deal_id }) — stakeholder map with roles, engagement levels
Full activity history: execute_action("get_deal_activities", { deal_id, limit: 100 }) — meetings, emails, calls, notes (capture the relationship arc)
Open tasks: execute_action("list_tasks", { deal_id }) — what is in flight
All meetings: execute_action("get_meetings", { deal_id }) — meeting history with summaries/transcripts
Pipeline context: execute_action("get_pipeline_summary", {}) — deal priority relative to pipeline
Company data: execute_action("get_company", { id: company_id }) — firmographics, industry, tech stack
...
Acknowledge the transition (briefly, without over-explaining internal changes)
Introduce the new owner (credentials, relevant experience)
Reassure continuity (the deal is not starting over)
Reference recent context (prove the new owner is already up to speed)
Confirm next steps (no disruption to timeline)
