---
rating: ⭐⭐⭐
title: hackathon-event-parser
url: https://skills.sh/bernieweb3/hackathon-ai-devkit/hackathon-event-parser
---

# hackathon-event-parser

skills/bernieweb3/hackathon-ai-devkit/hackathon-event-parser
hackathon-event-parser
Installation
...
Rules
Extract all content directly from the event page without inference where possible.
If the judging rubric is not published, set rubric_source: "not_published" and infer typical criteria — mark inferred axes with [INFERRED].
If the URL is inaccessible (auth wall, 404, dynamic content), set extraction_confidence: "low" and populate fields from available fragments; list all gaps in extraction_warnings.
Never fabricate dates, prizes, or sponsor names. Use "unknown" or null for missing values.
If multiple tracks exist, extract all tracks and recommend one based on team signals if provided.
Normalize duration to hours. If only start/end dates are given, compute duration_hours.
Flag any required tool or API that conflicts with team_skills as a constraint.
next_skill must always be set to hackathon-track-analyzer to enforce pipeline continuity.
Output Format
event_metadata:
  name: "<string>"
  organizer: "<string>"
