---
rating: ⭐⭐⭐
title: twitter-to-binance-square
url: https://skills.sh/6551team/6551-twitter-to-binance-square/twitter-to-binance-square
---

# twitter-to-binance-square

skills/6551team/6551-twitter-to-binance-square/twitter-to-binance-square
twitter-to-binance-square
Installation
...
Extract core message: Keep the tweet's main content
Remove Twitter-specific elements: Remove @mentions (except source attribution), remove t.co links
Tool attribution (default): Append Publish by using 6551 twitter mirror tool — controlled by show_tool_attribution (default: true)
Source attribution (optional): Credit the original author with Source: @username on X — controlled by show_source (default: true)
Preserve hashtags: Keep original hashtags, add relevant crypto hashtags if missing
Language handling: If translate is enabled, translate to the target language while preserving technical terms
Length check: Binance Square has content length limits; trim if needed (keep under 2000 chars)
...
Agent Behavior
Verify credentials first: Check both TWITTER_TOKEN and SQUARE_API_KEY exist before starting
Prompt for missing config: If mode/accounts/keywords not specified, ask the user
Show preview before first post: On the first run, show the first transformed tweet and ask for confirmation
Respect rate limits: Honor poll_interval_seconds and max_posts_per_run
Daily limit awareness: Track daily post count, stop when approaching Binance Square daily limit
Deduplication: Never post the same tweet ID twice (persisted in state_file)
Graceful shutdown: On interrupt, save state before exiting
Logging: Print each action (fetch, skip, transform, post) with timestamps
Run script via: python3 scripts/auto_mirror.py
Usage Examples
...
Notes
Binance Square 每日最多发帖 100 条 — the script tracks and respects this limit
Content may be flagged as sensitive by Binance — those tweets are skipped automatically
The 6551 API has rate limits — max 100 results per request
Always attribute the original author to comply with content policies
Weekly Installs
173
Repository
6551team/6551-t…e-square
GitHub Stars
13
First Seen
Mar 7, 2026
