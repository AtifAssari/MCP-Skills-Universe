...
url: https://skills.sh/t0dorakis/murmur/heartbeat-cron
---

# heartbeat-cron

skills/t0dorakis/murmur/heartbeat-cron
heartbeat-cron
Installation
...
Run murmur init {absolute_workspace_path} — this auto-registers the workspace in ~/.murmur/config.json (with { path, lastRun: null }). If HEARTBEAT.md already exists, it skips creating it but still registers.
Verify registration: murmur workspaces list
Schedule, timeout, model, and other settings live in the HEARTBEAT.md frontmatter — no need to edit config.json for these.
Only edit ~/.murmur/config.json directly if you need a permissions.deny list (frontmatter only supports permissions: skip).
