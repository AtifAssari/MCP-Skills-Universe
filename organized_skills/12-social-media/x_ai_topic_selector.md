---
rating: ⭐⭐⭐
title: x-ai-topic-selector
url: https://skills.sh/vigorx777/x-ai-topic-selector/x-ai-topic-selector
---

# x-ai-topic-selector

skills/vigorx777/x-ai-topic-selector/x-ai-topic-selector
x-ai-topic-selector
Installation
$ npx skills add https://github.com/vigorx777/x-ai-topic-selector --skill x-ai-topic-selector
SKILL.md
...
单选类: 在对应的选项 label 后添加 (上次选择) 标记。
自定义数值: 如果上次使用的是自定义数值（不在预设选项中），应动态添加一个新选项。
API Key: 如果已保存且用户选择 AI 模式，自动复用，无需再次输入。
列表 URL: 在 question 文本中包含 📌 上次使用: <value>，并在 options 中将该值作为第一个选项提供。
交互流程
...
检查 GEMINI_API_KEY 环境变量或 config.geminiApiKey
...
脚本路径 = ${SKILL_DIR}/scripts/<script-name>.ts
将本文档中所有 ${SKILL_DIR} 替换为实际路径
脚本	用途
