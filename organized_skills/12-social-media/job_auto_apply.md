...
url: https://skills.sh/liruihan000/claude-job-auto-apply/job-auto-apply
---

# job-auto-apply

skills/liruihan000/claude-job-auto-apply/job-auto-apply
job-auto-apply
Installation
...
Run node ${CLAUDE_SKILL_DIR}/scripts/bootstrap.js → load config + check readiness
...
Count Submitted column entries with today's date → today_submitted
List ⬜ NOT SUBMITTED entries with materials ready
Report: "Today: X/{config.daily_target} submitted, Y pending"
If today_submitted >= config.daily_target → report done and stop
Otherwise → enter Auto-Apply Pipeline immediately
...
Read the job posting (via WebFetch, Playwright, or user-provided text)
Analyze: role requirements, tech stack, team, company info
Compare against the user's resumes in uploaded-resumes/ and user-profile.md
Discuss with the user: fit assessment, pros/cons, resume strategy, questions to ask
Do NOT enter the Auto-Apply Pipeline. Stay in conversation until the user says to apply or ends the discussion.
If the user says "apply" or "submit" → add to TRACKER.md and proceed to Phase 2 for this job only.
Auto-Apply Pipeline
...
Fetch the full JD text (do not add to TRACKER yet)

Apply ALL filters from ${CLAUDE_SKILL_DIR}/references/selection-strategy.md — especially sponsorship and citizenship checks. If the job fails any filter → skip entirely, do not create folder, do not add to TRACKER

Only if the job passes all filters — complete ALL of the following before proceeding to the next job: a. Add a row to TRACKER.md as ⬜ NOT SUBMITTED b. Create folder applications/YYYY-MM-DD_Company_Role/ c. Save full JD text to {APP_FOLDER}/jd.md d. Extract JD keywords → {APP_FOLDER}/jd-keywords.json (format: {"required_skills":[], "preferred_skills":[], "keywords":[]}) e. Find and read the company About/Mission page; save summary to {APP_FOLDER}/notes.md (include: industry, product, size, stage, culture, JD URL) f. Write {APP_FOLDER}/STATUS.md as ⬜ NOT SUBMITTED
...
Navigate to LinkedIn: linkedin.com/search/results/people/?keywords={COMPANY}&network=%5B%22F%22%5D (filters to 1st-degree connections)
If connections found at this company → add their name and title to the job's Notes column in TRACKER.md (e.g. "🤝 John Doe - SWE @ Company")
Move jobs with connections to higher priority
...
Read {APP_FOLDER}/jd.md — full JD text
Read {APP_FOLDER}/jd-keywords.json — extracted keywords
Read {APP_FOLDER}/notes.md — company info
Select template from uploaded-resumes/ per ${CLAUDE_SKILL_DIR}/references/template-guide.md
Read user-profile.md for personal info (contact, work auth, EEO)
Tailor resume per ${CLAUDE_SKILL_DIR}/references/tailoring-guide.md
Generate cover letter per ${CLAUDE_SKILL_DIR}/references/cover-letter-guide.md (if config.prepare.cover_letter_required)
Generate DOCX + PDF
Write STATUS.md as 📁 PREPARED — NOT SUBMITTED
...
Content: Apply the same rules from ${CLAUDE_SKILL_DIR}/references/tailoring-guide.md to verify compliance.
Format: Read the generated PDF and check for rendering issues — stray markdown characters (*, **, #), broken formatting, garbled text, missing sections.
Page fit: Verify exactly 1 page — not overflowing, not too short.
AI writing detection + ATS keyword score: Run node ${CLAUDE_SKILL_DIR}/scripts/ats_score.js {APP_FOLDER}/resume.md {APP_FOLDER}/jd-keywords.json. (jd-keywords.json was already created in Phase 1 — do not regenerate it.) Check results:
...
ATS parse check: Run node ${CLAUDE_SKILL_DIR}/scripts/ats_parse_check.js {APP_FOLDER}/resume.pdf. Check results:
...
New ATS + friction → create ${CLAUDE_SKILL_DIR}/ats-handlers/{platform}.md with the workaround
Known ATS + friction → append fix as one bullet to existing handler
No friction lines → skip, no update needed
