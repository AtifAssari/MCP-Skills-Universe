---
rating: ⭐⭐
title: marketplace-catalog-sync
url: https://skills.sh/vtex/skills/marketplace-catalog-sync
---

# marketplace-catalog-sync

skills/vtex/skills/marketplace-catalog-sync
marketplace-catalog-sync
Installation
$ npx skills add https://github.com/vtex/skills --skill marketplace-catalog-sync
SKILL.md
...
Use PUT on /suggestions/{sellerId}/{sellerSkuId} under https://api.vtex.com/{accountName} (Send SKU Suggestion) to register or update pending suggestions. The seller does not own the catalog — every new SKU must go through the suggestion/approval workflow.
...
Change notification (marketplace SKU ID) — POST .../changenotification/{skuId}; path uses marketplace SKU ID only
Change notification (seller ID and seller SKU ID) — POST .../changenotification/{sellerId}/{skuId}; use this for seller connector catalog integration
Send SKU Suggestion (PUT /suggestions/{sellerId}/{sellerSkuId}) — base URL https://api.vtex.com/{accountName}
Marketplace API — Suggestions — full Suggestions API reference
