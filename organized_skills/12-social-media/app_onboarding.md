...
url: https://skills.sh/kimny1143/claude-code-template/app-onboarding
---

# app-onboarding

skills/kimny1143/claude-code-template/app-onboarding
app-onboarding
Installation
...
Value Before Registration — 登録前に価値を体験させる。体験してから登録を求める
Learn by Doing — 説明スライドではなく、実際の操作を通じて学ばせる。ツールチップ5個の連打は厳禁
First Win in 60 Seconds — 初回起動から60秒以内に「できた」感を提供する
...
反面教師パターン（避けるべき設計）
情報過多ウェルカムスライド: 3枚以上の説明スライドはスキップ率が急上昇
価値体験前の登録強制: 登録→通知→価値の順番は動機付け失敗
過剰なパーミッション要求: 起動直後にカメラ・位置情報・通知を一括要求
見せかけのパーソナライズ: 質問だけして体験が変わらない → 信頼喪失
長すぎるチュートリアル: ツールチップ5個以上の連打 → 80%がスキップ
空のホーム画面: 登録完了後に何もない画面 → 次のアクションがわからず離脱
短期的圧力戦術: 「今だけ50%OFF」等のオンボーディング中のプレッシャー → 信頼毀損
使い方ガイド
新規オンボーディング設計: Step 0 で Aha Moment 定義 → Phase 0-6 を順に設計 → Mode 2 でフロー図 → Mode 3 でワイヤーフレーム → Mode 4 でプロトタイプ
既存オンボーディング改善: Mode 1 で診断レポート → 最大の改善機会を特定 → Mode 2 で改善後フロー設計 → 実装
A/Bテスト設計: 計測設計テンプレートを適用 → 変更箇所の仮説を明文化 → テスト期間・サンプルサイズを算出
リファレンス
