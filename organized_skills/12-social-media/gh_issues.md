...
rating: ⭐⭐⭐
title: gh-issues
url: https://skills.sh/steipete/clawdis/gh-issues
---

# gh-issues

skills/steipete/clawdis/gh-issues
gh-issues
Installation
...
Spawn a single sub-agent for that one issue with cleanup: "keep" and runTimeoutSeconds: 3600
If --model was provided, include model: "{MODEL}" in the spawn config
If --notify-channel was provided, include the channel in the task so the sub-agent can notify
Do NOT await the sub-agent result — fire and forget
Write claim: After spawning, read the claims file, add {SOURCE_REPO}#{N} with the current ISO timestamp, and write it back
Immediately report: "Spawned fix agent for #{N} — will create PR when complete"
...
Discover open fix/issue-* PRs (Step 6.1)
Fetch review comments (Step 6.2)
