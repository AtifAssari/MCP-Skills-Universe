---
rating: ⭐⭐⭐
title: tiktok-app-marketing
url: https://skills.sh/upload-post/upload-post-larry-marketing-skill/tiktok-app-marketing
---

# tiktok-app-marketing

skills/upload-post/upload-post-larry-marketing-skill/tiktok-app-marketing
tiktok-app-marketing
Installation
...
Sign up at upload-post.com — create an account
Connect TikTok — this is the main one. Go to your profile → Connect TikTok → Authorize
Connect Instagram — same process. This doubles your reach for free.
Create a Profile — this is the profile name used in API calls (e.g., "mybrand"). Note this down.
Get the API key — Dashboard → API Keys → Generate. This is how the agent talks to Upload-Post programmatically.
(Optional) Connect YouTube, LinkedIn, Threads, etc. for even more reach — same content, different algorithms.
...
Pulls platform analytics from Upload-Post (followers, impressions, reach trends)
Pulls upload history to check post success/failure across platforms
Pulls conversion data from RevenueCat (if connected)
Cross-references impressions with conversions to diagnose what's working
...
Generate a test set of 6 images using the prompts you built together
Show them the results and ask: "How do these look? Too polished? Too dark? Wrong vibe? Wrong furniture?"
Tweak based on feedback — adjust the base prompt, regenerate
Repeat until they're happy — this might take 2-5 rounds, that's fine
Lock the prompt style once it looks right — save to config
...
3-4 lines per slide is ideal — more lines are fine, they won't overflow
...
Sends them to Upload-Post's POST /upload_photos endpoint
Includes all configured platforms in one request
Uses async_upload=true for background processing
Returns a request_id for tracking (saved in meta.json)
...
Groups uploads by request_id (one post = multiple platform entries)
Shows per-post success/failure status and post URLs
Saves a snapshot to analytics-snapshot.json
...
Upload-Post → platform analytics (followers, impressions, reach) + upload history (per-post success, post URLs)
...
Pulls platform analytics from Upload-Post (followers, impressions, reach timeseries)
Pulls upload history for the last 3 days (per-post status, post URLs)
If RevenueCat is connected, pulls conversion events in the same window (24-72h attribution)
Cross-references: which posts drove views AND which drove paying users
Applies the diagnostic framework (below) to determine what's working
Generates tiktok-marketing/reports/YYYY-MM-DD.md with findings
Messages the user with a summary + suggested hooks for today
