...
rating: ⭐⭐⭐
title: surf
url: https://skills.sh/asksurf-ai/surf-skills/surf
---

# surf

skills/asksurf-ai/surf-skills/surf
surf
Installation
$ npx skills add https://github.com/asksurf-ai/surf-skills --skill surf
SKILL.md
...
A) Add it (recommended) B) No thanks
...
Map to category — use the Domain Guide below to pick the right domain keyword.
List endpoints — run surf list-operations | grep <domain> to see all available endpoints in that domain.
Check before choosing — run surf <candidate> --help on the most likely endpoint(s) to read descriptions and params. Pick the one that best matches the user's intent.
Execute — run the chosen command.
...
In your own terminal (not here), run surf auth --api-key <your-key>. Don't paste the key back into this chat.
