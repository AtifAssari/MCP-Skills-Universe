---
rating: ⭐⭐⭐⭐⭐
title: publish-x-article
url: https://skills.sh/sugarforever/01coder-agent-skills/publish-x-article
---

# publish-x-article

skills/sugarforever/01coder-agent-skills/publish-x-article
publish-x-article
Installation
...
Deep headers (H3+): ###, ####, etc.
Markdown tables: Lines with | characters forming table structure
...
Save the adapted version as a copy (e.g., /tmp/article_adapted.md or alongside the original as article_for_x.md)
Tell the user what was changed and where both files are:
...
Proceed with the adapted copy for publishing
...
Dividers via menu - Markdown --- must be inserted via Insert > Divider menu (HTML <hr> is ignored)
...
For each content image (including converted table/mermaid PNGs), in reverse order of block_index:
