---
rating: ⭐⭐⭐⭐⭐
title: qiaomu-info-card-designer
url: https://skills.sh/joeseesun/info-card-designer/qiaomu-info-card-designer
---
...
qiaomu-info-card-designer
Installation
$ npx skills add https://github.com/joeseesun/info-card-designer --skill qiaomu-info-card-designer
SKILL.md
Info Card Designer
...
找核心论点：文章最反直觉、最颠覆认知的 1 个观点，作为主标题或核心金句
找关键数据：文中的具体数字（百分比、倍数、年份、金额），数字比文字更有冲击力
找因果链：A 导致 B，B 导致 C → 每个环节就是一个要点
砍到 4-6 个要点：不是所有内容都值得放，只放"删了会损失信息"的
每个要点 ≤ 2 句话：第一句给事实/数据，第二句给洞察/结论
...
themeVariables 中必须写固定 hex 值，不能写 var(--xxx)（Mermaid 不解析 CSS 变量）
...
from PIL import Image img = Image.open("/tmp/info-card-xxx.png") print(f"Dimensions: {img.size[0]}x{img.size[1]}") print(f"File size: {os.path.getsize('/tmp/info-card-xxx.png') / 1024:.0f}KB")
