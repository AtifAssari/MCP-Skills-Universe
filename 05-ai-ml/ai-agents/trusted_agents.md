...
url: https://skills.sh/ggonzalez94/trusted-agents/trusted-agents
---

# trusted-agents

skills/ggonzalez94/trusted-agents/trusted-agents
trusted-agents
Installation
$ npx skills add https://github.com/ggonzalez94/trusted-agents --skill trusted-agents
SKILL.md
...
OWS is checked (and installed if missing)
A new wallet is created (or the specified one is used)
A signing policy is created with chain restrictions (or a compatible one is reused)
A scoped API key is issued for the agent
...
If the user chooses Taiko read the full Servo Agent Skill. It will help you use the smart account and the paymaster effectively.
...
Name — a display name. Be creative and suggest the user options based on what you know about him or why he's created you.
Description — a short phrase describing what the agent does.
Capabilities — public discovery labels that tell other agents what this one can do. Ask the user something like: "What are you planning to use this agent for?"
...
Agent A creates an invite: tap invite create
Agent B connects with it: tap connect "<invite-url>"
Agent A picks up the request (via sync or listener) and auto-accepts valid invites
Both agents converge on an active contact
...
Transfer requests ask a peer to send ETH or USDC. If an existing grant exists that covers the amount, the transfer can happen automatically, if not it MUST ALWAYS BE SURFACED TO THE USER FOR APPROVAL. When showing the human transfer requests or summaries always show the reason if it exists.
...
Alice's agent checks her calendar, sends a proposal with ranked time slots
Bob's agent checks Bob's calendar against the proposal:
...
When agents converge on a slot, both humans confirm
Calendar events are created on both sides
...
Read the escalation details (peer, request type, amount if transfer, proposed times if scheduling)
For transfer requests: tap permissions show <peer> and check the permissions ledger
For scheduling proposals: show the proposed times in the user's timezone, the meeting title, and who's asking
Send the user a message with a clear summary: who's asking, what they want, and relevant context
Wait for the user's decision
Resolve: tap_gateway resolve_pending with requestId and approve: true/false, or use tap_gateway respond_meeting for scheduling

AUTO-REPLY — an active contact sent you a message that needs a reply:

Read the peer's message from the notification text
Optionally check conversation history with tap conversations list --with <peer> then tap conversations show <id> for context
Reply using tap_gateway send_message with autoGenerated: true
Summarize to the user what the peer said and what you replied, in a single message. Do NOT send the user two separate messages (one for receiving, one for replying).
...
Run the status check immediately — don't explain what you're about to do
Let the command outputs tell you what's needed next
Only ask the user when you genuinely need their input
Ask one question at a time, act on the answer, then continue
...
Reads the existing private key from identity/agent.key
Preserves the XMTP database encryption key (computed from the old key and saved to config)
Imports the key into an OWS wallet
Creates a signing policy and scoped API key
Verifies the OWS wallet address matches the original
Updates config.yaml with the ows block
Deletes identity/agent.key
