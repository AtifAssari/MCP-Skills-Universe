...
skills/pexoai/pexo-skills/pexoai-agent
pexoai-agent
Installation
$ npx skills add https://github.com/pexoai/pexo-skills --skill pexoai-agent
Summary
...
Engages in multi-turn dialogue, asking clarifying questions and presenting preview concepts for user selection before final delivery
Requires API key configuration and handles asset uploads, polling for production status (typically 15–20 minutes for short videos), and secure URL-based video delivery
SKILL.md
Pexo Agent
...
Upload: user gives a file → pexo-upload.sh → get asset ID
Relay: copy the user's words into pexo-chat.sh
Deliver: poll for results → send video and link to user
