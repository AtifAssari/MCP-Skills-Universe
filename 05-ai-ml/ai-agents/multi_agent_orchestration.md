...
url: https://skills.sh/peterfile/devpilot-agents/multi-agent-orchestration
---

# multi-agent-orchestration

skills/peterfile/devpilot-agents/multi-agent-orchestration
multi-agent-orchestration
Installation
...
MUST ask BEFORE running init_orchestration.py or any other orchestration step
If user selects "No" or declines: HALT immediately and report cancellation
...
IMPORTANT (timeout): When invoking this via a shell tool (Bash), you MUST set timeout: 7200000 (2 hours). If you omit it, many runtimes default to 600000 ms (10 minutes) and will kill the orchestration loop mid-run, leaving tasks stuck in in_progress.
...
Check error message
If "codeagent-wrapper not found": Ensure it is installed/in PATH, or set CODEAGENT_WRAPPER=/path/to/codeagent-wrapper (scripts also probe ./bin/)
If tmux errors (connect/permission/missing): set CODEAGENT_NO_TMUX=1 and retry
If timeout: Retry once, then report to user
