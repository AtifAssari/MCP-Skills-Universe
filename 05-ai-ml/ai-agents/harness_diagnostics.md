...
url: https://skills.sh/junh0328/playground/harness-diagnostics
---

# harness-diagnostics

skills/junh0328/playground/harness-diagnostics
harness-diagnostics
Installation
$ npx skills add https://github.com/junh0328/playground --skill harness-diagnostics
SKILL.md
Harness Diagnostics
...
핵심 원칙
Read-only: 기본 동작은 진단/제안만 수행
Evidence-first: 모든 판단은 파일 경로 근거 포함
Principle-driven: 12원칙 + 성숙도 프레임워크 기반 채점
Self-referential: 이 skill 자체도 동일 기준으로 진단 가능
모드 선택
모드	트리거	목적	워크플로우
...
프레임워크 공식 스캐폴드로 최소 프로젝트 생성
Setup 모드로 현재 레포 상태 진단
사용자가 별도로 제공한 요구사항을 목표 상태 설계 입력으로 반영
Setup 리포트를 기준으로 AGENTS.md, docs/, lint/test/CI, 구조 규칙을 먼저 정리
그 다음 실제 기능 구현 진행
...
사용자 요구사항은 현재 상태의 근거가 아니라 목표 상태를 위한 설계 입력으로 취급한다.
...
references/principles.md를 기준으로 P1~P12를 0-10점으로 평가한다. P1 Agent Entry Point, P2 Map, P3 Invariant, P4 Convention, P5 Progressive Disclosure, P6 Layered, P7 Garbage Collection, P8 Observability, P9 Knowledge, P10 Reproducibility, P11 Modularity, P12 Self-Documentation.
...
가중치/산식은 references/maturity-framework.md를 따른다.

실행 절차
모드 결정
대상 판별 (코드베이스/Skill/Self)
해당 워크플로우 실행
references/report-template.md 형식으로 리포트 출력
자동화 도구
