...
rating: ⭐⭐⭐
title: coding-agent
url: https://skills.sh/steipete/clawdis/coding-agent
---

# coding-agent

skills/steipete/clawdis/coding-agent
coding-agent
Installation
...
Capture the notification route from the current conversation before spawning:
notifyChannel
notifyTarget
...
Start the coding CLI with background:true immediately.
Include the notification route in the worker prompt and require the worker to call openclaw message send on completion.
Monitor with process action:log / poll.
If the worker needs input or fails before notifying, handle that explicitly yourself. Do not rely on heartbeat.
...
NEVER start Codex inside your OpenClaw state directory ($OPENCLAW_STATE_DIR, default ~/.openclaw) - it'll read your soul docs and get weird ideas about the org chart!
NEVER checkout branches in ~/Projects/openclaw/ - that's the LIVE OpenClaw instance!
Always inject the Completion Prompt Snippet into the worker prompt before spawning. The simplified examples below omit it for brevity — never spawn a worker without it.
...
Capture notify routing before spawn.
Completion messaging must have a real route.
Use direct completion messaging.
...
Never checkout branches in ~/Projects/openclaw/.
Learnings
