---
title: gather-plan-apply
url: https://skills.sh/mgd34msu/goodvibes-plugin/gather-plan-apply
---

# gather-plan-apply

skills/mgd34msu/goodvibes-plugin/gather-plan-apply
gather-plan-apply
Installation
...
discover batches ALL discovery queries into 1 call — NEVER use separate precision_glob, precision_grep for discovery
Plan steps produce ZERO tool calls — they are cognitive (agent thinks in text)
One call per tool type per phase — if you need 3 files read, batch them in 1 precision_read call
...
Identify which calls are discovery → batch into 1 discover call
Identify which calls are memory/file reads → batch into 1 precision_read call
