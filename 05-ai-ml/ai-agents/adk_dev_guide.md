...
rating: ⭐⭐⭐
title: adk-dev-guide
url: https://skills.sh/google/adk-docs/adk-dev-guide
---
...
Read DESIGN_SPEC.md thoroughly
...
Use make playground (or adk web .) for interactive testing during development
...
Run evaluations: adk eval (or make eval if the project has a Makefile)
...
Deploy when ready — see /adk-deploy-guide for deployment options
...
Preservation Check: Ensure all code, configuration values (e.g., model, version, api_key), comments, and formatting outside the identified target remain identical.
