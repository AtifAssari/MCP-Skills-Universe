...
rating: ⭐⭐⭐
title: authsome
url: https://skills.sh/manojbajaj95/authsome/authsome
---

# authsome

skills/manojbajaj95/authsome/authsome
authsome
Installation
...
Research — search the provider's official developer docs to find what auth methods they offer (OAuth2, API key, or both). Note endpoints, supported flows, and whether DCR is available.

Confirm with the user — present what you found and ask which method they want:
...
Write and register the provider JSON — follow the provider registration guide to write the provider JSON. Save the file to a local path (e.g. /tmp/<provider>.json), then register it:

authsome register /tmp/<provider>.json


Return to Step 2 — Login.
