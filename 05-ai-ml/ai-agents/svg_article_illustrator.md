...
url: https://skills.sh/cat-xierluo/legal-skills/svg-article-illustrator
---

# svg-article-illustrator

skills/cat-xierluo/legal-skills/svg-article-illustrator
svg-article-illustrator
Installation
...
遵循共享设计原则：references/core-principles.md
第三阶段：生成与输出
解析占位符：提取所有 [[ILLUSTRATION:ID:描述]]
并行/顺序生成：
≥ 8 张：并行生成（多 Task Agent）
< 8 张：顺序生成
替换占位符：将生成的 SVG 代码替换占位符
...
保存 SVG 文件到源文章目录
使用 scripts/svg2png.js 转换为 PNG
在 Markdown 中插入图片引用 ![](path.png)
第四阶段：归档
