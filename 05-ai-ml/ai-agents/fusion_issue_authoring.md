---
rating: ⭐⭐⭐
title: fusion-issue-authoring
url: https://skills.sh/equinor/fusion-skills/fusion-issue-authoring
---
...
mcp_github::issue_write create/update with the full known payload (title, body, and include labels, assignees, type only when supported)
Optional single follow-up mcp_github::issue_write only when required fields were unknown in step 1 and become available later
mcp_github::sub_issue_write only when relationship/order changes are requested
mcp_github::add_issue_comment only when blocker/status notes are explicitly requested
