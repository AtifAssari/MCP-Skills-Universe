...
rating: ⭐⭐⭐
title: sonarqube-mcp
url: https://skills.sh/giuseppe-trisciuoglio/developer-kit/sonarqube-mcp
---

# sonarqube-mcp

skills/giuseppe-trisciuoglio/developer-kit/sonarqube-mcp
sonarqube-mcp
Installation
...
Always check quality gate before merge — Run get_project_quality_gate_status as part of any PR review workflow
Shift left on security issues — Use analyze_code_snippet during development, not only in CI
Prioritize by severity — Address BLOCKER and HIGH issues first; document decisions for MEDIUM and LOW
Use show_rule for unfamiliar keys — Never dismiss a rule without understanding its intent
Paginate large result sets — Use p and ps parameters; handle multi-page responses for complete coverage
Never change issue status autonomously — Always present issues to the user and get explicit confirmation before calling change_sonar_issue_status
Provide language hints — Specify language in analyze_code_snippet for more accurate analysis
