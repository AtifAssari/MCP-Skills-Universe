---
title: cc10x-router
url: https://skills.sh/romiluz13/cc10x/cc10x-router
---

# cc10x-router

skills/romiluz13/cc10x/cc10x-router
cc10x-router
Installation
...
Extract workflow_uuid from the wf: line.
Read all CC10X tasks whose descriptions contain that wf:.
Derive runnable tasks from status and blockedBy.
Reconstruct the memory task as the unique pending/in_progress kind:memory task in the same wf:.
...
Contradiction-free: No acceptance criterion contradicts a stated constraint or non-goal. If contradictions exist, halt and persist pending_gate="intent_contradiction".
...
Generate a stable workflow UUID before TaskCreate():
...
kind:remfix + `origin:code-reviewer	silent-failure-hunter
...
project CLAUDE.md / repo standards / user standards
...
After Skill(skill="cc10x:brainstorming"), parse the fenced YAML block under ### Brainstorming Handoff (MACHINE-READABLE).
...
TaskGet({ taskId }) or TaskList() to verify final task state.
WRITE agents:
...
Update .claude/cc10x/v10/workflows/{workflow_uuid}.json with:
