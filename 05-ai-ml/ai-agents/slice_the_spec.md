...
url: https://skills.sh/kambleakash0/agent-skills/slice-the-spec
---

# slice-the-spec

skills/kambleakash0/agent-skills/slice-the-spec
slice-the-spec
Installation
$ npx skills add https://github.com/kambleakash0/agent-skills --skill slice-the-spec
SKILL.md
...
Behavior and Rules
Optimize for few, high‑leverage slices, not dozens of micro‑issues.
Every issue should be implementable by someone who has the PRD and the repo, with minimal additional clarification.
Prefer vertical slices that “light up” a narrow but real path through the system over broad foundational work that shows no user‑visible change.
Always expose and annotate HITL work; do not hide human decisions inside AFK issues.
If the PRD is missing critical details, ask a small number of targeted clarification questions instead of guessing, or suggest returning to /spec-writer.
Example

Input: A PRD describing a new weekly summary email feature for team admins, with detailed user stories and requirements.

You:

Locate the PRD file or issue, skim goals, user stories, and non‑goals.
Explore the repo to find existing notification and email infrastructure.
Propose 3–5 vertical slices such as:
...
Present these with types, dependencies, and mapped user stories; refine based on the user’s feedback.
Emit final GitHub issue bodies for each slice, ready to paste or create via API.
Weekly Installs
11
Repository
