...
url: https://skills.sh/vercel-labs/vercel-plugin/agent-browser-verify
---

# agent-browser-verify

skills/vercel-labs/vercel-plugin/agent-browser-verify
agent-browser-verify
Installation
...
Page loads — agent-browser open succeeds without timeout
No blank page — snapshot shows meaningful content (not empty body)
No error overlay — no Next.js/Vite error overlay detected
Console errors — evaluate document.querySelectorAll('[data-nextjs-dialog]') for error modals
Key elements render — snapshot -i shows expected interactive elements
Navigation works — if multiple routes exist, verify at least the home route
Error Detection
...
Screenshot the error state: agent-browser screenshot error-state.png
Capture the error overlay text or console output
