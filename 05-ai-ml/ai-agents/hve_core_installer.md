...
url: https://skills.sh/microsoft/hve-core/hve-core-installer
---

# hve-core-installer

skills/microsoft/hve-core/hve-core-installer
hve-core-installer
Installation
...
Execute extension installation (see Extension Installation Execution below)
Validate installation success
Display success report or offer fallback options
...
Determines workspace root via git rev-parse --show-toplevel
Calculates target path based on method from table
Checks if target already exists
Clones if missing: git clone https://github.com/microsoft/hve-core.git <target>
Reports success with ✅ or skip with ⏭️
...
Create .hve-core/ directory
Add .hve-core/ to .gitignore (create if missing)
Clone into .hve-core/
...
Combine only the selected server configurations into a single JSON object
Include inputs array only if ado server is selected
Merge all selected servers under a single servers object
...
[!IMPORTANT] Generated scripts in this phase require PowerShell 7+ (pwsh). Windows PowerShell 5.1 is not supported.
