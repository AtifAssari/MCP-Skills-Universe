---
title: news-summary
url: https://skills.sh/zjfls/zhoujie-claude-skills/news-summary
---

# news-summary

skills/zjfls/zhoujie-claude-skills/news-summary
news-summary
Installation
...
两个按钮："查看原文" 和 "AI解读"
必须在 前包含：<script src="/news-ai.js"></script>
4. AI 解读功能
...
搜索新闻（WebSearch 优先；Brave Search 需间隔 1 秒）
分析和整理新闻信息
获取当前工作目录：使用 Bash 命令 pwd 获取绝对路径
创建目录：<当前工作目录>/news-summary/<timestamp>_<topic>/
生成 HTML：
文件路径：<当前工作目录>/news-summary/<timestamp>_<topic>/news_summary_<topic>.html
必须在 前添加：<script src="/news-ai.js"></script>
...
等待 2 秒并验证
打开浏览器：http://localhost:3456/news-summary/<timestamp>_<topic>/news_summary_<topic>.html
HTML 模板要求
现代简洁设计，响应式布局
卡片式新闻展示
每个新闻卡片必须包含 data 属性：
...
相对时间显示（如：2天前）
必须在 前引入：<script src="/news-ai.js"></script>（使用绝对路径）
