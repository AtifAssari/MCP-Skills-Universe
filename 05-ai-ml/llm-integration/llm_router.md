---
title: llm-router
url: https://skills.sh/jamesrochabrun/skills/llm-router
---

# llm-router

skills/jamesrochabrun/skills/llm-router
llm-router
Installation
...
Best Practices
Always check installation first - Run check_install_cli.sh before routing requests
Configure provider explicitly - Use configure_provider.sh to ensure correct setup
Verify API keys - Check that the appropriate API key is set for the target provider
Choose the right model - Match the model to the task (coding, reasoning, general chat)
Use sessions for continuity - Leverage --session-id for multi-turn conversations
Enable debug mode for troubleshooting - When issues arise, debug mode provides valuable insights
Reference provider documentation - Consult references/providers.md for detailed provider information
Examples
