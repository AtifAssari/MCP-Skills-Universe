...
skills/supercent-io/skills-template/image-generation-mcp
image-generation-mcp
Installation
$ npx skills add https://github.com/supercent-io/skills-template --skill image-generation-mcp
Summary
...
Includes validation workflows across multiple agents for prompt completeness, style consistency, and brand alignment before generation
Generates 2-4 image variants per request with metadata tracking (prompts, model, ratio, timestamp) for reproducibility and iteration
SKILL.md
...
Best practices
Specify ratio early: 의도하지 않은 크롭 방지
Use style anchors: 일관된 미적 스타일 유지
Iterate with constraints: 한 번에 하나의 변수만 변경
Track prompts: 재현 가능성 확보
Batch similar requests: 일관된 스타일 세트 생성
Common pitfalls
