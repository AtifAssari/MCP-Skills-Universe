---
title: codex-plan
url: https://skills.sh/yelban/orz99-skills/codex-plan
---

# codex-plan

skills/yelban/orz99-skills/codex-plan
codex-plan
Installation
...
Understand the user's planning request (shown below as ARGUMENTS:)
...
Clear objective — What plan needs to be created
All requirements — Everything learned from clarifying questions
Constraints — Technology choices, timeline, team size
Target Files — The file paths list from Step 3
Plan structure — Read from references/plan-template.md and inject its full content
Output instructions — Write to codex-plan.md in current directory
...
Read the generated codex-plan.md
Check structure: file exists, is valid Markdown, contains at minimum:
# Plan: heading
## Overview section
At least one ### Task entry
If checks fail OR codex exec returned an error/timeout:
Adjust the prompt (simplify requirements, fix ambiguities)
Retry execution once
If retry also fails: show the error to the user and explain what went wrong
If checks pass: show the plan results to the user
Important Rules
ALWAYS ask at least 1 clarifying question — Do not skip Step 2 (1-5 questions based on complexity)
ALWAYS use gpt-5.4 with xhigh reasoning — No exceptions (unless fast-path)
ALWAYS tell Codex not to ask questions — It should just execute
ALWAYS use --full-auto flag
ALWAYS verify output in Step 6 — Check structure, retry once on failure
