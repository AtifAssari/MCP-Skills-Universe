...
skills/softaworks/agent-toolkit/gepetto
gepetto
Installation
...
Integrates feedback from external LLM reviewers (Gemini, Codex) and supports resuming interrupted planning workflows
Outputs execution files compatible with ralph-loop plugin and Ralphy CLI for autonomous implementation
SKILL.md
Gepetto
...
First, parse sections/index.md to get the SECTION_MANIFEST list
Then launch ALL section Tasks in a single message (parallel execution):
