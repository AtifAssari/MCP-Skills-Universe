...
rating: ⭐⭐⭐
title: cross-review
url: https://skills.sh/dmitriyyukhanov/claude-plugins/cross-review
---

# cross-review

skills/dmitriyyukhanov/claude-plugins/cross-review
cross-review
Installation
...
Preflight check — verify codex is reachable before any work begins; ABORT if not
Detect artifact type — determine what is being reviewed (plan, code, architecture, design)
Run review round — launch Claude agents AND Codex simultaneously in parallel, then collect both results
Triage findings — classify and cross-validate findings from both reviewers
...
Clean up intermediate files — run bash "${CLAUDE_PLUGIN_ROOT}/scripts/cleanup-reviews.sh" <output_dir> (mandatory, regardless of exit reason)
Core Workflow
...
Know this is Round N (and if N > 1, focus on changes from Round N-1 fixes — use git diff to identify the delta for code artifacts)
Output findings in severity format (Critical / High / Medium / Minor)
...
Wait for all Claude agent messages to arrive, synthesize into docs/plans/review-claude-round-N.md
Wait for the Codex background bash job to complete using TaskOutput (do NOT just read the file — you must consume the background task so the completion notification doesn't arrive later), then verify the output file:
...
Shut down the Claude agent team for this round
...
Project-specific skills first (e.g., unity-coder, unity-architect)
General-purpose skills second (e.g., writing-plans, brainstorming)
If no matching skill found AND fixes are trivial → apply inline (direct edits)
...
Disagreements found in triage? → EXIT LOOP. Present needs-decision items, then clean up.
All issues resolved? → EXIT LOOP. Present summary, then clean up.
Max rounds reached? (default: 3) → EXIT LOOP. Present remaining issues, then clean up.
No skill available for non-trivial fixes? → EXIT LOOP. Ask user, then clean up.
