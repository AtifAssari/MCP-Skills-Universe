---
title: transcendence-memory
url: https://skills.sh/leekkk2/transcendence-memory/transcendence-memory
---

# transcendence-memory

skills/leekkk2/transcendence-memory/transcendence-memory
transcendence-memory
Installation
...
Get a connection token from the server (through the /export-connection-token endpoint or from an administrator)
Run /tm connect <token> to finish setup automatically
...
A PostToolUse hook fires after every shell command. If the command was a git commit and auto-memory is enabled, the agent is instructed to store a one-line commit summary as a memory tagged auto-commit.
Enable / disable
