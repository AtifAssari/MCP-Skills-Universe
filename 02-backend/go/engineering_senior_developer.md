...
url: https://skills.sh/peterhdd/agent-skills/engineering-senior-developer
---

# engineering-senior-developer

skills/peterhdd/agent-skills/engineering-senior-developer
engineering-senior-developer
Installation
$ npx skills add https://github.com/peterhdd/agent-skills --skill engineering-senior-developer
SKILL.md
...
Read README and config files (5 min) — Identify language, framework, build/test/deploy commands, and entry points.
Run the app locally (10 min) — If it fails, fixing the dev setup is task zero. Nothing else matters until the app runs.
Run the test suite (5 min) — Note: total tests, duration, failures. Failing tests on main are a red flag to document.
Trace the critical path (15 min) — Pick the core user action. Trace from HTTP handler → business logic → database → response. Note file and function at each step.
Map architecture layers (10 min) — Entry points (routes/controllers), business logic, data access, external services. Note where boundaries are clean vs tangled.
Check dependency health (5 min) — npm outdated / pip list --outdated. Flag anything >2 major versions behind.
Read last 20 commits (5 min) — Understand active work areas and commit conventions.
Identify test gaps (5 min) — What types exist (unit/integration/E2E)? Where are the blind spots?
