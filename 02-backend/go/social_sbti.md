---
title: social-sbti
url: https://skills.sh/backtthefuture/huangshu/social-sbti
---

# social-sbti

skills/backtthefuture/huangshu/social-sbti
social-sbti
Installation
...
读 raw.json: Read 工具读 $RAW_FILE,进入 posts[] 数组。
...
读 rubric: Read 工具读 $SBTI_HOME/docs/SCORING_RUBRIC.md。对照每个维度的 L/M/H 锚点打分。

逐条评分 15 维 (顺序固定): S1 S2 S3 E1 E2 E3 A1 A2 A3 Ac1 Ac2 Ac3 So1 So2 So3
...
personality_description: 200-300 字"本人定制版"描述 —— 必须包含对象的 具体事实(职业、项目、口头禅、代表观点),不要套通用文案
...
可选字段: 如果动态里非常明显地大量提"喝酒/白酒/灌杯",加 "drunk": true, 会触发 DRUNK 彩蛋。
...
match.py → 匹配 27 人格模板,写回 scores.json 补上 pattern + personality
make_card.py → 生成 HTML 卡片
render_png.py → 用 playwright 截成 PNG(没装就跳过,会给提示)
...
整体印象: 复述 overall_impression 一整段
