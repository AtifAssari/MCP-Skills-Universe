...
url: https://skills.sh/huggingface/skills/huggingface-jobs
---

# huggingface-jobs

skills/huggingface/skills/huggingface-jobs
huggingface-jobs
Installation
$ npx skills add https://github.com/huggingface/skills --skill huggingface-jobs
SKILL.md
...
Add token to secrets: MCP uses "$HF_TOKEN" (auto-replaced), Python API uses get_token() (must pass real token)
...
Verify token in secrets — MCP: "$HF_TOKEN", Python API: get_token() (NOT "$HF_TOKEN")
...
Use tokens securely - MCP: secrets={"HF_TOKEN": "$HF_TOKEN"}, Python API: secrets={"HF_TOKEN": get_token()} — "$HF_TOKEN" only works with MCP tool
