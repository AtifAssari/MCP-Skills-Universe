...
url: https://skills.sh/deanpeters/product-manager-skills/context-engineering-advisor
---

# context-engineering-advisor

skills/deanpeters/product-manager-skills/context-engineering-advisor
context-engineering-advisor
Installation
...
"I paste entire documents into AI" — Full PRDs, complete user interview transcripts, entire codebases
"AI gives vague, hedged responses despite having 'all the context'" — Responses like "it depends," "consider these options," non-committal
"I have to retry prompts 3+ times to get usable output" — Inconsistency is normalized
"Token costs are escalating but accuracy isn't improving" — Spending more, getting same or worse results
"I keep adding more context hoping it'll help" — Reflexive expansion without strategy
"My agents pass everything to each other" — Agent A → Agent B → Agent C with full context chain
"I don't have clear criteria for what to include/exclude" — No context boundary definitions
"None of these—my AI usage is working well" — Skip to advanced optimization
...
Generate a Context Architecture Blueprint for your specific use case?
Provide implementation guidance (tools, techniques, best practices)?
Design a retrieval strategy for your episodic context?
...
[Delete context with no falsifiable failure mode from Q4]
[Apply Research→Plan→Reset→Implement to your next AI task]
[Document context boundary in Context Manifest]

Foundation Building (Next 2 Weeks):

[Build constraints registry with 20+ entries]
[Create operational glossary with 20-30 key terms]
[Implement two-layer memory architecture]

Long-Term Optimization (Next Month):

[Set up semantic retrieval for episodic context]
[Assign context boundary owner + quarterly audit schedule]
[Implement Contextual Retrieval (Anthropic) for RAG]
...
Generate specific implementation docs (Context Manifest, PLAN.md template, etc.)?
Provide advanced techniques (Contextual Retrieval, LLM-powered ETL)?
Review your current context setup (provide feedback on specific prompts/workflows)?
Examples
...
Immediate: Delete all context that fails Q4 test → keeps 20% of original
Week 1: Build constraints registry (10 technical constraints, 5 strategic)
Week 2: Create operational glossary (25 terms)
Week 3: Implement Research→Plan→Reset→Implement for next PRD
...
Immediate: Define bounded context per agent (Agent A outputs only 2-page synthesis to Agent B, not full research)
Week 1: Assign context boundary owner (Lead PM)
Week 2: Create Context Manifest (what persists, what's retrieved, what's excluded)
Week 3: Implement Research→Plan→Reset→Implement between Agent B and Agent C
...
Immediate: Limit retrieval to top 5 chunks per query (down from 50)
Week 1: Implement Contextual Retrieval (Anthropic) — prepend explanatory context to each chunk during indexing
Week 2: Add metadata to documents (category, recency, authority)
Week 3: Product team defines retrieval intent per query type (discovery = customer insights, feasibility = technical constraints)
