...
url: https://skills.sh/okx/plugin-store/raydium-plugin
---

# raydium-plugin

skills/okx/plugin-store/raydium-plugin
raydium-plugin
Installation
...
Check wallet — run raydium-plugin quickstart. This resolves your Solana wallet, checks SOL and USDC balances, and returns a status field indicating your readiness:
...
Find a token to swap — ask what tokens the user wants to trade. Help them find mint addresses using raydium-plugin get-token-price --symbol <TOKEN> or raydium-plugin get-price --input-mint <MINT> --output-mint <MINT>.
Get a quote first — always run raydium-plugin get-swap-quote (or swap without --confirm) before executing. Show the user the outputAmount, priceImpactPct, and fees. Ask for explicit confirmation before proceeding.
Execute the swap — only after the user confirms the quote details, re-run the swap command with --confirm.
...
Run without --confirm first to preview quote (no on-chain action)
Ask user to confirm the swap details, price impact, and fees
Execute with --confirm only after explicit user approval — pre-flight balance check runs automatically before swap
Reports transaction hash(es) on completion
