---
title: compound-v3-plugin
url: https://skills.sh/okx/plugin-store/compound-v3-plugin
---

# compound-v3-plugin

skills/okx/plugin-store/compound-v3-plugin
compound-v3-plugin
Installation
$ npx skills add https://github.com/okx/plugin-store --skill compound-v3-plugin
SKILL.md
...
Run without --confirm to preview the approve + supply steps
Ask user to confirm the supply amount, asset, and market before proceeding
Re-run with --confirm to execute on-chain
Execute ERC-20 approve: onchainos wallet contract-call → token.approve(comet, amount)
Wait 3 seconds (nonce safety)
Execute supply: onchainos wallet contract-call → Comet.supply(asset, amount)
Report approve txHash, supply txHash, and updated supply balance
...
Pre-check: simulate the borrow call on-chain to catch NotCollateralized() before spending gas
Run without --confirm to preview — output includes min_borrow_amount (market's baseBorrowMin)
Ask user to confirm the borrow amount and ensure they understand debt accrues interest
Re-run with --confirm to execute on-chain
Execute: onchainos wallet contract-call → Comet.withdraw(base_asset, amount)
Report txHash and updated borrow balance
...
Below baseBorrowMin (Arbitrum USDC.e only): the requested borrow is smaller than the market's minimum position size (~100 USDC.e). Increase the borrow amount. The error message includes the market's baseBorrowMin to distinguish between these cases. Use get-position to check current collateral value.
...
Read current borrowBalanceOf and wallet token balance
Run without --confirm to preview
Ask user to confirm the repay amount before proceeding
Re-run with --confirm to execute on-chain
Execute ERC-20 approve: onchainos wallet contract-call → token.approve(comet, amount)
Wait 3 seconds
Execute repay: onchainos wallet contract-call → Comet.supply(base_asset, repay_amount)
Report approve txHash, repay txHash, and remaining debt
...
Pre-check: borrowBalanceOf must be 0. If debt exists, prompt user to repay first.
Run without --confirm to preview
Ask user to confirm the withdrawal before proceeding
Re-run with --confirm to execute on-chain
Execute: onchainos wallet contract-call → Comet.withdraw(asset, amount)
Report txHash
...
Re-run with --confirm to execute on-chain
Execute: onchainos wallet contract-call → CometRewards.claimTo(comet, wallet, wallet, true)
Report txHash and confirmation
Key Concepts
