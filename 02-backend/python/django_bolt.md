---
title: django-bolt
url: https://skills.sh/dj-bolt/django-bolt/django-bolt
---

# django-bolt

skills/dj-bolt/django-bolt/django-bolt
django-bolt
Installation
$ npx skills add https://github.com/dj-bolt/django-bolt --skill django-bolt
SKILL.md
Django-Bolt
Critical rules
...
Migration -- porting from FastAPI, DRF, or Django Ninja (see references/migration-playbook.md)
Step 2: Start from the minimal shape
Add "django_bolt" to INSTALLED_APPS
Create an api.py in the project or app directory
Instantiate BoltAPI()
Add async route handlers with typed parameters
...
Verify python manage.py runbolt --dev is being used, not uvicorn or gunicorn
Check that the Rust extension is built: run just build from the project root
Ensure api.py exists in the Django project root or in an installed app directory
Route not found (404)
Confirm api.py is in a directory runbolt discovers: project root (same dir as settings.py) or an installed app
Verify the api variable is a BoltAPI() instance named exactly api
Check route path matches (leading slash required: /items, not items)
Auth returns 401 unexpectedly
For JWT: verify token is passed in Authorization: Bearer <token> header
For JWT: check token expiration and secret key configuration
