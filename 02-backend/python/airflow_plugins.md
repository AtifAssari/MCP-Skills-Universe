...
url: https://skills.sh/astronomer/agents/airflow-plugins
---

# airflow-plugins

skills/astronomer/agents/airflow-plugins
airflow-plugins
Installation
...
Am I using fastapi_apps / FastAPI — not appbuilder_views / Flask?
Are all HTML/JS asset paths and fetch() calls relative (no leading slash)?
Are all synchronous SDK or SQLAlchemy calls wrapped in asyncio.to_thread()?
Do the static/ and assets/ directories exist before the FastAPI app mounts them?
If the endpoint must be private, did I add explicit FastAPI authentication?
...
Astro UI → open the Deployment → Access → API Tokens → + Deployment API Token
