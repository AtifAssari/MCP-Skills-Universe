---
rating: ⭐⭐⭐
title: frappe-core-cache
url: https://skills.sh/openaec-foundation/erpnext_anthropic_claude_development_skill_package/frappe-core-cache
---

# frappe-core-cache

skills/openaec-foundation/erpnext_anthropic_claude_development_skill_package/frappe-core-cache
frappe-core-cache
Installation
...
ALWAYS use frappe.local.cache for data needed multiple times within a single request — it avoids Redis round-trips entirely.
NEVER use frappe.clear_cache() as a routine invalidation strategy — it clears ALL cache keys for the site, causing a cold-cache performance hit.
ALWAYS prefix custom cache keys with your app name (e.g., myapp|exchange_rate) to avoid collisions with Frappe internals.
Redis Configuration
