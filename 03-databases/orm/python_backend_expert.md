---
rating: ⭐⭐⭐
title: python-backend-expert
url: https://skills.sh/hieutrtr/ai1-skills/python-backend-expert
---

# python-backend-expert

skills/hieutrtr/ai1-skills/python-backend-expert
python-backend-expert
Installation
...
Route receives UserCreate (Pydantic validates the request body)
Route calls UserService.create_user(data) via Depends()
Service checks business rule (email uniqueness) via UserRepository.get_by_email()
Service hashes password, creates User model instance
Service calls UserRepository.create(user) to persist
Repository adds to session, flushes, refreshes to get generated fields
Route converts the ORM model to UserResponse via model_validate()
