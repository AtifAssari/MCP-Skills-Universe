...
url: https://skills.sh/sgaabdu4/appwrite-backend/appwrite-backend
---

# appwrite-backend

skills/sgaabdu4/appwrite-backend/appwrite-backend
appwrite-backend
Installation
...
Critical Rules
Use TablesDB API — Collections API deprecated 1.8.0
Use ID.unique() for all IDs — Row IDs (rowId:) + entity IDs in columns. Custom gen w/ names/timestamps overflow column limits, leak data. ~20-char hex client-side.
Use Query.select() — Relationships return IDs only without
...
Use explicit string types — string deprecated; use varchar or text/mediumtext/longtext
Use appwrite generate — Type-safe SDK from schema
Use Channel helpers — Type-safe realtime subs, not raw strings
...
Cost Optimization
Query.select() — cuts bandwidth
Cursor pagination + total: false — fastest queries
Realtime over polling — one connection vs repeated calls
