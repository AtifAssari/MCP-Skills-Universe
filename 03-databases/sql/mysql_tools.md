...
rating: ⭐⭐
title: mysql-tools
url: https://skills.sh/huangzt/my-agent-skills/mysql-tools
---

# mysql-tools

skills/huangzt/my-agent-skills/mysql-tools
mysql-tools
Installation
...
常见用例
探索新数据库
测试连接：mysql_connect.py
列出所有表：mysql_tables.py
查看关键表结构：mysql_schema.py --table TABLE_NAME
查询示例数据：mysql_query.py --query "SELECT * FROM TABLE_NAME LIMIT 5"
数据分析
获取表记录数：mysql_query.py --query "SELECT COUNT(*) FROM TABLE_NAME"
分析数据分布：mysql_query.py --query "SELECT column, COUNT(*) FROM TABLE_NAME GROUP BY column"
参考更多 SQL 示例
