...
rating: ⭐⭐
title: neon-postgres
url: https://skills.sh/sickn33/antigravity-awesome-skills/neon-postgres
---

# neon-postgres

skills/sickn33/antigravity-awesome-skills/neon-postgres
neon-postgres
Installation
...
Provides guidance on choosing between Neon's neon-http driver (single queries) and neon-serverless driver (transactions and WebSocket sessions) for edge and serverless environments
Documents known sharp edges and gotchas when working with Neon's serverless architecture
SKILL.md
Neon Postgres
...
if (process.env.NEON_KEEPALIVE === 'true') { setInterval(async () => { try { await prisma.$queryRawSELECT 1; } catch (error) { console.error('Keepalive failed:', error); } }, KEEPALIVE_INTERVAL); }
