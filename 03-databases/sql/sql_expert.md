...
rating: ⭐⭐⭐
title: sql-expert
url: https://skills.sh/questfortech-investments/claude-code-skills/sql-expert
---

# sql-expert

skills/questfortech-investments/claude-code-skills/sql-expert
sql-expert
Installation
...
Use transactions for related operations to ensure atomicity
Add appropriate constraints (PRIMARY KEY, FOREIGN KEY, NOT NULL, CHECK)
Include timestamps (created_at, updated_at) on tables
...
Not using LIMIT for exploratory queries on large tables
Implicit type conversions preventing index usage
Using COUNT(*) when EXISTS is sufficient
Not handling NULLs properly (NULL = NULL is always NULL, not TRUE)
Using SELECT DISTINCT as a band-aid instead of fixing the query
Forgetting transactions for related operations
Using functions on indexed columns preventing index usage
...
Quick Start
For basic queries, use the patterns shown above
For optimization, start with EXPLAIN and check references/query-optimization.md
For schema design, review normalization patterns and see examples/schema_examples.sql
For complex scenarios, check references/advanced-patterns.md
For utilities, use scripts/sql_helper.py
Workflow

When working with SQL databases:

Understand requirements - What data needs to be queried or stored?
Design schema - Apply normalization, choose appropriate data types
Create indexes - Index foreign keys and frequently queried columns
Write queries - Start simple, add complexity as needed
Optimize - Use EXPLAIN to identify bottlenecks
Test - Verify with sample data and edge cases
Document - Add comments for complex queries

For migrations:

Plan changes - Identify affected tables and dependencies
Write migration - Create both up and down migrations
Test on copy - Test on development database first
Backup - Always backup before running migrations
Execute - Run migrations during low-traffic periods
