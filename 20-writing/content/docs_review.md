---
title: docs-review
url: https://skills.sh/cloudflare/cloudflare-docs/docs-review
---

# docs-review

skills/cloudflare/cloudflare-docs/docs-review
docs-review
Installation
...
Decision logic
Explicit instruction wins. If the user says "suggest", "only make suggestions", or "do not make changes" — post suggestions via gh CLI, never push commits. If they say "fix", "address this", or "update" — edit files directly and commit.
Different actor = suggest. If the person invoking the review is not the PR author (and no explicit fix instruction was given), post suggestions so the author retains control.
...
Missing best practices — no TypeScriptExample, hardcoded compat dates, bare code fences
Style and structure — heading levels, link format, prose quality
...
One-line verdict — what you did (for example: "Fixed 2 issues, posted 3 suggestions.") or your assessment (for example: "No issues found." or "2 issues need fixing before merge.").
Issues list — only items that need action, grouped by severity. Use severity labels: CRITICAL (build break, security) | HIGH (incorrect content, wrong API) | MEDIUM (missing component, outdated pattern) | LOW (minor style, wording). Include file path and line number for each.
Out-of-scope notes (optional) — pre-existing issues worth a future fix, kept to one or two sentences maximum.
...
State what you changed, with file paths and line numbers.
Stop.
