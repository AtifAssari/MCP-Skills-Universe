---
rating: ⭐⭐⭐
title: geo-citability
url: https://skills.sh/zubair-trabzada/geo-seo-claude/geo-citability
---

# geo-citability

skills/zubair-trabzada/geo-seo-claude/geo-citability
geo-citability
Installation
$ npx skills add https://github.com/zubair-trabzada/geo-seo-claude --skill geo-citability
SKILL.md
...
Extract the main content area (exclude navigation, footer, sidebar, ads).
Preserve heading structure (H1-H6 tags).
Preserve paragraph boundaries, lists, and tables.
Calculate total word count of main content.
Step 2: Segment Content into Blocks
Split content at each heading (H2 or H3) to create content blocks.
...
Calculate the average of all block scores for the page-level citability score.
Identify the top 3 highest-scoring blocks (highlight as strengths).
Identify the bottom 3 lowest-scoring blocks (flag for rewriting).
Calculate the percentage of blocks scoring above 70 (the "citability coverage" metric).
Step 5: Generate Rewrite Suggestions

For each block scoring below 60, generate a specific rewrite suggestion:

Identify the primary weakness (buried answer, lack of facts, poor structure, etc.).
Propose a rewritten opening sentence using a definition or answer-first pattern.
Suggest specific statistics or facts that could be added.
Recommend structural improvements (add list, add table, split paragraph).
Output Format
