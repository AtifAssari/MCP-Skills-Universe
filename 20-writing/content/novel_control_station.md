---
title: novel-control-station
url: https://skills.sh/jingtai123/novel-control-station-skill/novel-control-station
---

# novel-control-station

skills/jingtai123/novel-control-station-skill/novel-control-station
novel-control-station
Installation
$ npx skills add https://github.com/jingtai123/novel-control-station-skill --skill novel-control-station
SKILL.md
Novel Control Station
Overview
...
Read document-templates.md when creating or restoring these files.
...
This script is the looping continuation entry point for marathon mode. It should be ready before the project leaves startup. Use the exact template in assets/codex-continue-novel.ps1 and replace only the project root placeholder before writing the file into the project root. Read bootstrap-and-marathon-handoff.md when starting a project or handing off into marathon mode.
...
When cast density, plotline interference, or foreshadow volume makes linear rereading too blunt, load graph-and-recall-control.md.
...
foundational-literary-principles.md
critical-evaluation-standards.md
epoch-and-people-resonance.md
...
Use research-source-notes.md only when provenance or source refresh is needed.

Benchmark Rule Loading

At project launch, outline design, and chapter review, read:

popular-fiction-common-laws.md
genre-benchmark-rules.md
benchmark-trigger-matrix.md
...
Use benchmark-source-trace.md only when source provenance matters.
...
Run the startup flow using interview-and-handoff-flow.md and character-construction-methods.md.
...
Before a full outline is accepted or a chapter roadmap is locked, read chapter-architecture-rules.md and run an outline benchmark check.
...
List the missing, conflicting, or risky items.
Explain what each issue threatens.
Offer exactly two branches:
refine details
draft directly
...
Read chapter-title-method.md when the project uses chapter titles, when the user asks for named chapters, or when a numbered-only chapter system may need to change.
...
If the project is newly launching, structurally redirecting, or under high-level review, read the foundational canon files first.
Read required files:
00-project-overview.md
03-cast-bible.md
05-main-plotlines.md
...
08-dynamic-state.md
09-style-guide.md
If the project has dense recurrence, large cast rotation, or multiple active interference lines, prepare a retrieval slice using graph-and-recall-control.md.
...
Read only the selected internal style module documents needed for this chapter.
...
read the relevant internal style modules from style-modules/index.md
read each selected module's core.md first
drill into deeper style documents only if the chapter needs them
Read only the execution-method references needed for this chapter.
...
Generate a chapter control card using chapter-control-card.md.
...
If the project uses chapter titles, generate 3-5 candidate titles using chapter-title-method.md.
...
If the risk scan is serious, use the missing information branch.
Draft the chapter from the control card.
when the chapter needs tighter control, write scene by scene or pressure unit by pressure unit using scene-execution-patterns.md
...
Run the chapter benchmark check.
If the benchmark check fails, apply rewrite escalation before accepting the chapter.
Run the authenticity pass using authenticity-and-de-ai-pass.md.
...
Run a post-authenticity mini recheck.
...
If the project uses chapter titles, run the final title check.
...
Review the chapter for continuity, style integrity, thematic pressure, critical standards, and whether return-pressure handling stayed causal rather than token.
Update dynamic and structural files.
keep chapters/ and control-cards/ aligned with the accepted chapter title and number
Record the chapter and file updates in the writing log.
Chapter Benchmark Check

Read quality-and-writeback-checks.md and reader-retention-and-ai-failure-modes.md before accepting a chapter. Hard gates:
...
When the project is long, dense, or heavily serialized, run forgotten-elements-and-line-heat.md during planning and review.
...
After structure and benchmark logic are acceptable, run authenticity-and-de-ai-pass.md.

Use a fixed three-part order:

remove generic AI habits, false depth, abstract summaries, empty conclusions, and textureless filler
cut or translate over-specialized diction, analysis-language drift, and reader-hostile jargon unless the point of view or setting genuinely needs the term
restore concrete detail, rhythm variation, speaker distinction, and project-specific voice
...
load bootstrap-and-marathon-handoff.md
...
Read quality-and-writeback-checks.md for the full writeback checklist. After every accepted chapter:

update 08-dynamic-state.md using dynamic-state-template.md
...
apply the logging rules in logging-rules.md
...
route style loading through style-modules/index.md
...
Read quality-and-writeback-checks.md when applying adaptive weighting, originality discipline, chapter quality gates, final writeback, or common-failure review.

Weekly Installs
36
Repository
jingtai123/nove…on-skill
GitHub Stars
225
First Seen
Mar 18, 2026
