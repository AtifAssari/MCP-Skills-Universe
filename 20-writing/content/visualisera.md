---
title: visualisera
url: https://skills.sh/jgabor/agentera/visualisera
---

# visualisera

skills/jgabor/agentera/visualisera
visualisera
Installation
...
▸ Set up enforcement: point to references/enforcement-patterns.md for the three-layer enforcement model (validation, linting, audit) ▸ Build to the spec: run /realisera to implement UI that respects the design tokens ▸ Document it: run /dokumentera to add the design system to project documentation ▸ Refine later: run /visualisera again to evolve the design as the project matures

Refine mode

Evolve an existing design system based on what's changed.

Step markers: display ── step N/4: verb before each step. Steps: read, propose, audit, update.

Step 1: Read current state
Current DESIGN.md: all token blocks, constraints, prose
Codebase: focused on changes since DESIGN.md was written (git log, new components)
VISION.md Identity: has verbal identity evolved?
PROGRESS.md: UI work and inline design decisions
TODO.md: design-related issues
...
Step markers: display ── step N/3: verb before each step. Steps: validate, check, report.
...
Token usage: undeclared colors, fonts, or spacing values in code
Constraint violations: prohibited properties in use (e.g., shadows when banned)
Component drift: undeclared variants or prohibited props
Consistency: ad-hoc styling on similar elements
Step 3: Report
...
Present with file:line references. For each finding, offer to: ▸ Fix DESIGN.md: add missing tokens or constraints ▸ File to TODO.md: if the code is wrong (design is right, code drifted) ▸ Skip: intentional or not worth fixing
...
Format: ─── ◰ visualisera · status ─── followed by a summary sentence. For flagged, stuck, and waiting: add ▸ bullet details below the summary.
...
/visionera: create VISION.md with Identity section (who the project IS)
/visualisera: create DESIGN.md (how it LOOKS), coherent with the Identity
/realisera: build UI to the design spec
Existing project: capture the visual identity
/visualisera: reads existing styles, proposes tokens from what's already there
Review and refine the generated DESIGN.md
Set up enforcement (see references/enforcement-patterns.md)
