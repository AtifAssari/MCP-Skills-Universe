---
title: marp-deck-visual-designer
url: https://skills.sh/raffaelecamanzo/skills/marp-deck-visual-designer
---

# marp-deck-visual-designer

skills/raffaelecamanzo/skills/marp-deck-visual-designer
marp-deck-visual-designer
Installation
...
Prerequisite — package.json must exist in the project root. task excalidraw-file runs npm install before every render. If the project has no package.json, the install silently fails (exit 254) and no PNG is produced.
...
Then run npm install once manually before invoking the task. Subsequent task excalidraw-file calls will reuse the installed module.
...
Choose inter_row_gap so predicted_ratio lands in 1.8–2.4: inter_row_gap ≈ (1.4 × node_height) is a reliable starting point
...
Predict the ratio using the formula above; adjust layout if needed before exporting.
Export: task excalidraw-file FILE=diagrams/<diagram-name>.excalidraw
Validate actual PNG dimensions: task mermaid-check-file FILE=diagrams/<diagram-name>.excalidraw
The validation script reads PNG IHDR bytes regardless of source format.
If actual ratio is outside target: adjust inter_row_gap or node positions, then re-export.
