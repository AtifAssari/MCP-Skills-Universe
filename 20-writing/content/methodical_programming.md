---
title: methodical-programming
url: https://skills.sh/vmvarela/skills/methodical-programming
---

# methodical-programming

skills/vmvarela/skills/methodical-programming
methodical-programming
Installation
...
Expression E can be evaluated without errors.
P ⟹ Q[x ← E] (the precondition implies the postcondition with x replaced by E).
...
Completeness: Pre ⟹ B₁ ∨ B₂ ∨ ... ∨ Bₙ (at least one branch is open).
Correctness per branch: {Pre ∧ Bᵢ} Sᵢ {Post} for each i.
Function Call
<x₁, ..., xₘ> := f(e₁, ..., eₖ)

Before the call: prove the arguments satisfy the function's precondition.
After the call: the result variables satisfy the postcondition (with parameter/result renaming).
...
Signature: Types (genres) and operations with their arities.
Equations: Algebraic identities that define operation behavior.
Constructors: Minimal set of operations that can build any value of the type (used as the basis for induction).
Common Structures
...
Verification Steps
Direct case correctness: Q(x) ∧ d(x) ⟹ R(x, h(x))
Recursive case — legal call: Q(x) ∧ ¬d(x) ⟹ Q(s(x)) (precondition holds for recursive argument)
Recursive case — correctness: Q(x) ∧ ¬d(x) ∧ R(s(x), v) ⟹ R(x, c(x, v))
Bounding function t(x): Define t: params → ℕ such that:
...
Specify the function: header, precondition, postcondition.
Identify the bounding function t(x) — the expression for induction over ℕ.
Analyze cases: Identify at least one direct case and one recursive case. Ensure all cases are covered.
Program and verify each case. For recursive cases, assume (by induction hypothesis) that recursive calls satisfy the specification.
Validate termination: Prove t(x) decreases strictly at each recursive call.
...
Weaken the postcondition (possibly introducing new variables).
Strengthen the precondition with the weakened postcondition — require a partial version of the result as input.
Add immersion parameters so the new precondition makes sense.
Keep the original postcondition.
...
Introduce new variables replacing sub-expressions in the postcondition.
Drop some equalities → weaker requirement.
The dropped equalities become the initializations to recover the original function from the immersion.
Strengthen the precondition with domain conditions on the new parameters as needed.
...
Given f(x) = c(f(s(x)), x) (linear recursive case), define immersion g(y, w) = c(f(y), w).
Unfold: Substitute f's cases into g's definition.
Fold: Manipulate the expression back into g's form.
The result is a tail-recursive g, with f(x) = g(x, neutral) where neutral is the identity element of c.
...
Define an invariant I: a condition that holds at the start of every iteration.
Initialization: Pre ⟹ I (the precondition implies the invariant).
Maintenance: {I ∧ B} S {I} (the body preserves the invariant).
Exit: I ∧ ¬B ⟹ Post (invariant + loop exit condition implies postcondition).

Phase 2 — Termination:

Define a bounding function t with values in ℕ.
Bounded: I ∧ B ⟹ t > 0.
Decreasing: {I ∧ B ∧ t = T} S {t < T} (the body strictly decreases the bound).
...
The invariant core is typically a weakened postcondition.
Design guarded instructions that preserve the invariant and decrease the bounding function.
Sufficient cases are covered when: Invariant ∧ ¬(B₁ ∨ B₂ ∨ ... ∨ Bₙ) ⟹ Post.
Workflow Summary

When writing any function, follow this process:

Specify: Write precondition and postcondition. Identify input/output types.
Analyze the postcondition: Determine if the solution requires assignment, conditionals, recursion, or iteration.
Choose strategy:
...
Verify: Ensure all proof obligations are met — or derive code so they hold by construction.
Applying This Skill
...
inline for over comptime array for representative cases derived from the specification (base, minimal inductive, edge values).
zig test --fuzz for no-crash / no-panic invariants — the built-in fuzzer feeds arbitrary bytes and catches panics, out-of-bounds accesses, and undefined behavior.
const std = @import("std");
