...
url: https://skills.sh/juliusbrussee/cavekit/cavekit-writing
---

# cavekit-writing

skills/juliusbrussee/cavekit/cavekit-writing
cavekit-writing
Installation
$ npx skills add https://github.com/juliusbrussee/cavekit --skill cavekit-writing
SKILL.md
Cavekit Writing
...
Why Hierarchical?
Context window efficiency — agents load only the domains they need
Parallel work — different agents can own different spec domains
Review efficiency — humans can review domain-by-domain
Cross-referencing — domains link to each other explicitly
...
Template Rules
Number requirements sequentially (R1, R2, R3...) — agents reference them by ID
Every requirement gets acceptance criteria — no exceptions
Out of Scope is mandatory — explicit exclusions prevent scope creep
Cross-References section is mandatory — even if it says "None"
Scope section is one paragraph — concise boundary description
...
Process
Place all reference materials in context/refs/
Run cavekit generation — agent reads all refs, decomposes into domains
Agent produces:
...
Human reviews kits for completeness and correctness
Iterate — refine kits based on review feedback
...
Process
Agent explores the existing codebase and generates reference documents
Reference docs capture the current system's behavior, APIs, data models, and UI patterns
Agent generates kits from reference docs — implementation-agnostic requirements
Validate kits against existing code — verify acceptance criteria match current behavior
Proceed with normal Hunt — kits drive the new implementation
...
How to Compact
Identify resolved content: completed tasks, resolved issues, archived dead ends
Archive removed content to a separate file (e.g., impl/archive/impl-domain-v1.md)
Preserve in the compacted file:
...
Read kits (intended behavior) and implementation tracking (what was built)
For each cavekit requirement, check if acceptance criteria are satisfied
...
Report gaps with: which cavekit, which criterion, what is missing
Feed gaps into revision — update kits if needed, then re-implement
...
Explore project context — check existing files, docs, commits before asking questions
Ask clarifying questions one at a time — understand purpose, constraints, success criteria
Propose 2-3 domain decomposition approaches — with tradeoffs and a recommendation
Present the design incrementally — section by section, get approval per domain
Generate kits only after design approval — formalize with acceptance criteria
Cavekit review loop — automated reviewer checks quality, up to 3 iterations
User review gate — explicit approval before transitioning to Architect phase
...
WHAT, not HOW — describe behavior, not implementation
Every requirement gets testable acceptance criteria — if agents cannot validate it, it will not be met
Hierarchical with an index — progressive disclosure for context efficiency
Cross-referenced — related domains link to each other
Explicitly scoped — out-of-scope section prevents over-building
Compact when large — archive resolved content, keep active files under 500 lines
Living documents — kits evolve through revision as gaps are discovered
Weekly Installs
15
Repository
