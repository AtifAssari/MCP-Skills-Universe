---
title: secondme-reference
url: https://skills.sh/mindverse/second-me-skills/secondme-reference
---

# secondme-reference

skills/mindverse/second-me-skills/secondme-reference
secondme-reference
Installation
...
MCP 服务端或 API 路由读取 Authorization header
解析出 bearer token，缺失或格式错误时直接返回 401
用该 token 调用上游 user/info 接口识别当前 SecondMe 用户
用上游用户 id 映射到本地 users.oauth_id 或等价外部 id
查不到时自动 upsert 本地用户
用解析出的本地 user.id 执行业务逻辑
推荐分层
