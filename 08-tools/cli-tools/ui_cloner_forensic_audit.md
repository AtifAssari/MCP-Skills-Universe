---
title: ui-cloner-forensic-audit
url: https://skills.sh/ragnar-pwninskjold/tech-snacks/ui-cloner-forensic-audit
---

# ui-cloner-forensic-audit

skills/ragnar-pwninskjold/tech-snacks/ui-cloner-forensic-audit
ui-cloner-forensic-audit
Installation
$ npx skills add https://github.com/ragnar-pwninskjold/tech-snacks --skill ui-cloner-forensic-audit
SKILL.md
...
Count ALL direct children of the deepest content container (not just the first few)
Scroll through the section in ~500px increments, taking a screenshot at each stop
Document every distinct component found — components at the END of a tall section are the most likely to be missed
Never assume a section contains only one type of component based on the first few children
...
Find the section's deepest content-holding container (the element whose direct children ARE the visual components)
Enumerate ALL children of that container — not just the ones matching an expected pattern
For EACH child, log: index, tagName, className, offsetHeight, position, top, and first 80 chars of textContent
If any child has a different class name from the majority, a different height (>30% deviation), or different position value — flag it as a potential sub-section and investigate fully
If the section height is >2x viewport (>2000px), you MUST scroll through its FULL height taking screenshots at intervals to catch components only visible mid-scroll
...
Take a full-viewport screenshot of the section at its most visually complete state
Identify EVERY distinct visual element in the composition — count them
Produce a COMPOSITION MAP using this format:
...
For Lottie/SVG/canvas elements specifically:
...
NEVER reduce a multi-element composition to a single-line description. If you see a phone with cards behind it, you must describe the phone AND every card.
...
OPAQUE MEDIA DECOMPOSITION (Lottie / SVG / Canvas): When an animation is rendered by Lottie, complex SVG, or canvas — you cannot read its internals from the DOM. Do NOT describe it as a single unit. Instead:

Take a screenshot of its rendered state
Cross-reference with your Step 1.3b Composition Map
Document what the animation DOES to each element in the composition (not just "Lottie plays")
If the Lottie animates elements into position (e.g., cards fan out, phone scales up), describe the before and after states of EACH element
