---
title: remove-model-cliche
url: https://skills.sh/cafe3310/public-agent-skills/remove-model-cliche
---

# remove-model-cliche

skills/cafe3310/public-agent-skills/remove-model-cliche
remove-model-cliche
Installation
$ npx skills add https://github.com/cafe3310/public-agent-skills --skill remove-model-cliche
SKILL.md
...
复述上述「基本建议」，告知用户接下来要做出的编辑，等待用户确认编辑。写到新的文件而不是原始文件。
编辑后，询问用户是否要搜索并移除当前互联网上搜到的模型刻板用词。如果是，则做个 web_search，搜索这些刻板用词，给用户看一下文章中包含的这些表达。用户确认后，基于 1 的输出文件，做个替换，换成更自然的形式。写到另一个新的文件而不是原始文件。
