---
title: setup
url: https://skills.sh/q00/ouroboros/setup
---

# setup

skills/q00/ouroboros/setup
setup
Installation
$ npx skills add https://github.com/q00/ouroboros --skill setup
SKILL.md
/ouroboros:setup
...
MCP server registration (~/.claude/mcp.json) — one-time, global across all projects
...
If which uvx succeeds: {"command": "uvx", "args": ["--from", "ouroboros-ai[claude]", "ouroboros", "mcp", "serve"]}
If which ouroboros succeeds: {"command": "ouroboros", "args": ["mcp", "serve"]}
If python3 -c "import ouroboros" succeeds: {"command": "python3", "args": ["-m", "ouroboros", "mcp", "serve"]}
If none of the above → do NOT write to mcp.json. Instead show the prerequisites message from Step 1 and stop.
...
Load the brownfield MCP tool: ToolSearch query: "+ouroboros brownfield"
Call scan+register:
...
Remove ouroboros entry from ~/.claude/mcp.json (if exists)
Remove <!-- ooo:START --> to <!-- ooo:END --> block from CLAUDE.md (if exists)
Confirm: "Ouroboros plugin configuration removed. To remove plugin files, run: claude plugin uninstall ouroboros"
