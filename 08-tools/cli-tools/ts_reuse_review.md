---
title: ts-reuse-review
url: https://skills.sh/trancong12102/agentskills/ts-reuse-review
---

# ts-reuse-review

skills/trancong12102/agentskills/ts-reuse-review
ts-reuse-review
Installation
...
If the primary replacement lives in a native API, recommend the native (highest priority — zero deps).
Else if an installed project lib (from Step 2) has a canonical match, recommend that lib's import. Example: effect installed → prefer Effect.retry over es-toolkit retry.
Else if a fixed external lib matches (es-toolkit/date-fns/zod), recommend the import and — if the lib is not installed — suggest bun add es-toolkit (or npm/pnpm equivalent detected from the lockfile).
Drop the match if none apply.
