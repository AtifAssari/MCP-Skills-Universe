...
url: https://skills.sh/everyinc/compound-engineering-plugin/resolve-pr-feedback
---

# resolve-pr-feedback

skills/everyinc/compound-engineering-plugin/resolve-pr-feedback
resolve-pr-feedback
Installation
...
Group by category + spatial proximity. Form groups from all categorized items -- new and previously-resolved together, not new items only. Two items form a potential cluster when they share a concern category AND are spatially proximate (same file, or files in the same directory subtree).
...
Resolve using scripts/resolve-pr-thread:
