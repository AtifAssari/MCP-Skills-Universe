---
rating: ⭐⭐
title: wecomcli-manage-schedule
url: https://skills.sh/wecomteam/wecom-cli/wecomcli-manage-schedule
---

# wecomcli-manage-schedule

skills/wecomteam/wecom-cli/wecomcli-manage-schedule
wecomcli-manage-schedule
Installation
...
根据用户意图计算时间范围（如"今天"→当日 00:00:00 至 23:59:59，"这周"→本周一至周日）
调用 get_schedule_list_by_range 获取日程 ID 列表
调用 get_schedule_detail 批量获取详情，将 Unix 时间戳转为可读时间
若用户提到关键词（如"项目评审"），在 summary 中匹配筛选；未找到则逐步扩大范围至前后 30 天上限
展示日程列表时包含标题、时间、地点、参与者等关键信息，方便用户快速了解
创建日程
...
解析用户意图，提取时间、标题、地点、参与人、提醒设置等信息
若涉及参与人，先通过 wecomcli-lookup-contact 查询 userid；存在同名时展示候选让用户选择
若用户未指定提醒，默认设置提前 15 分钟提醒（remind_before_event_secs: 900）
若用户说"全天"，设置 is_whole_day: 1，时间设为当天 00:00:00 至 23:59:59
向用户确认日程信息（标题、时间、地点、参与人等）后调用 create_schedule
修改日程
...
先通过查询工作流定位目标日程（根据用户提到的时间、标题等关键词匹配）
若匹配到多个日程，展示候选列表让用户确认
向用户确认要修改的字段和目标值
调用 update_schedule，只传入需修改的字段
取消日程
...
先通过查询工作流定位目标日程
向用户确认取消的日程信息（标题、时间等），避免误操作
确认后调用 cancel_schedule
管理参与人
...
通过 wecomcli-lookup-contact 获取目标人员 userid；存在同名时展示候选让用户选择
通过查询工作流定位目标日程
调用 add_schedule_attendees 或 del_schedule_attendees 完成添加/移除
查询闲忙并安排会议
...
通过 wecomcli-lookup-contact 获取相关人员 userid
调用 check_availability 查询指定时间范围内各用户的忙碌时段
分析所有用户的忙碌时段，计算出共同空闲时段并推荐给用户
用户确认时段后，调用 create_schedule 创建会议并自动添加参与人
Weekly Installs
2.4K
Repository
wecomteam/wecom-cli
