---
rating: ⭐⭐⭐
title: feishu-cli-board
url: https://skills.sh/riba2534/feishu-cli/feishu-cli-board
---

# feishu-cli-board

skills/riba2534/feishu-cli/feishu-cli-board
feishu-cli-board
Installation
...
飞书画板全功能操作
前置条件
feishu-cli：如尚未安装，前往 riba2534/feishu-cli 获取
...
思维导图、时序图、类图、饼图、流程图 → 默认走 Mermaid（board import）
用户输入包含 Mermaid 语法 → 走 Mermaid
其他所有类型 → 走 OpenAPI JSON（board create-notes）
Step 2: 生成节点 JSON / Mermaid

JSON 路径（大多数场景）：

按 references/content.md 规划信息量和分组
按 references/layout.md 选择布局模式，计算每个节点的 x/y 坐标
按 references/style.md 上色（未指定时用经典色板）
按 references/schema.md 语法输出完整 JSON
连线参考 references/connectors.md，排版参考 references/typography.md
...
关键约束速查表
先形状后连线 -- 连接线通过 ID 引用形状节点，必须在形状创建后再创建连线
最小字段集 -- 多余字段导致 2890002 invalid arg，只用 schema.md 列出的安全字段
背景色块 fill_opacity <= 60 -- 否则完全遮挡上层节点
z_index 分层 -- 背景 0-1、次级 2-3、常规节点 10、连线 50
坐标系为绝对坐标 -- 没有自动布局引擎，每个节点必须手算 x/y/width/height
节点文字保持简短 -- 标题 + 简短说明（12 字以内），不写长段落
同组节点视觉一致 -- 同一分组内所有节点使用相同的 fillColor/borderColor
症状 → 修复表
看到的问题	改什么
