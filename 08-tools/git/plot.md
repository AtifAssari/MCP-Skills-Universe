---
title: plot
url: https://skills.sh/eins78/skills/plot
---

# plot

skills/eins78/skills/plot
plot
Installation
...
Check CI logs: gh pr checks <number>
Fix on the implementation branch, push
Wait for CI to pass, then merge normally
...
Hold off — go finish the work, then re-run /plot-deliver
Deliver anyway — accept the gap (the skill asks for confirmation)
Update the plan — if scope changed, edit the plan file on main to match what was actually built, then re-run /plot-deliver
...
Create the plan retroactively — /plot-idea <slug>: <title>, approve it
Just merge it — if the work is small, skip the plan and merge directly
Delete the branch — if the work is abandoned
Release check finds missing release notes

/plot-release cross-check reports gaps. Options:

Add the missing entries — update CHANGELOG.md or add changeset files
Proceed anyway — if the gap is intentional (internal change, no user impact)
Go back to deliver — if a plan was missed entirely
Sprint past end date with incomplete must-haves

/plot-sprint close shows incomplete must-haves. Options:

Close anyway — must-haves stay unchecked in place as a record
Move to Deferred — explicitly acknowledge they didn't make the timebox
Extend the sprint — edit the End date (but consider: is this hiding a scope problem?)
Automation Output
...
Create a PR from a fresh worktree of https://github.com/eins78/skills on a new branch, fixing the issue directly
Or file an issue on https://github.com/eins78/skills with: what failed, the actual behavior, and the suggested fix
