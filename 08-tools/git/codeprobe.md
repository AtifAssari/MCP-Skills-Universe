---
title: codeprobe
url: https://skills.sh/nishilbhave/codeprobe/codeprobe
---

# codeprobe

skills/nishilbhave/codeprobe/codeprobe
codeprobe
Installation
...
Read the shared preamble from shared-preamble.md (in this skill's directory). This contains the output contract, execution modes, and constraints shared by all sub-skills.
...
The shared preamble (from shared-preamble.md) — output contract, modes, constraints.
The sub-skill name to invoke (e.g., codeprobe-security).
The mode — one of full or scan.
...
Config overrides — severity overrides and skip rules from .codeprobe-config.json.
...
codeprobe-security — Security vulnerability detection
codeprobe-error-handling — Error handling & resilience
codeprobe-solid — SOLID principles analysis
codeprobe-architecture — Architecture analysis
codeprobe-patterns — Design patterns advisor
codeprobe-performance — Performance & scalability
codeprobe-code-smells — Code smell detection
codeprobe-testing — Test quality & coverage
...
Group findings by location. Normalize each finding's location to {file}:{start_line}. Two findings overlap if they share the same file AND their line ranges overlap (i.e., start_line_A <= end_line_B AND start_line_B <= end_line_A).
...
If still ambiguous, the category with the higher weight (Section 7) wins b. Mark duplicates. For each non-primary finding in the group, append to its problem field: [Duplicate of {primary_id} — counted there] and change its severity to suggestion so it does not affect the score of its own category. c. Cross-reference the primary. Append to the primary finding's suggestion field: Also flagged by: {list of duplicate category:id pairs}
...
Assemble an in-memory "report bundle": {dashboard_data, exec_summary, critical[], major[], minor_counts[], suggestion_counts[], fix_order[]}.
...
Save confirmation line — --> Report saved to ./codeprobe-reports/{project}-{cmd}-{YYYY-MM-DD-HHMMSS}.md (no emoji; ASCII arrow). This is the LAST line in the terminal output.
...
Build the full markdown using templates/full-audit-report.md placeholders — this includes sections that are NOT streamed to the terminal (major findings table, minor/suggestion counts, full prioritized fix order).
Derive the filename as {project}-{cmd}-{YYYY-MM-DD-HHMMSS}.md:
...
Ensure ./codeprobe-reports/ exists (mkdir -p ./codeprobe-reports via Bash if missing).
Write to ./codeprobe-reports/{project}-{cmd}-{YYYY-MM-DD-HHMMSS}.md using the Write tool.
If the write fails (read-only filesystem, permission denied, etc.), surface a short inline note in the terminal but do not re-emit the summary.
...
Next Step: Suggest running /codeprobe audit for the complete picture.
...
Skip file_stats.py, dependency_mapper.py, and all script-dependent steps (sub-skills fall back to LLM-only analysis — architecture loses deterministic cycle detection).
Skip /codeprobe diff, /codeprobe report, the Codebase Stats row of the audit dashboard, and the report-save step (no filesystem write). Still render scores, hot spots, and findings to the terminal.
...
Load config: Check for .codeprobe-config.json at project root. Apply defaults if absent.
Auto-detect stack: Scan target path for technology signals. Load matching references.
Apply config overrides: If framework is set in config, adjust detection. Apply skip_categories and skip_rules.
...
Deduplicate findings: Apply the cross-category deduplication procedure (Section 7A). Adjust severity of duplicates to suggestion. Recount severity totals per category.
...
Render report: Format output using the appropriate template or inline format. Use the tiered output format for /codeprobe audit.
