---
title: jj-workflow
url: https://skills.sh/trevors/dot-claude/jj-workflow
---

# jj-workflow

skills/trevors/dot-claude/jj-workflow
jj-workflow
Installation
$ npx skills add https://github.com/trevors/dot-claude --skill jj-workflow
SKILL.md
jj Workflow
Philosophy
Commits are cheap, descriptions are mandatory. The working copy is always a commit. Never leave it as "(no description set)".
Experiment freely, the oplog is your safety net. Every mutation is recorded. jj undo and jj op restore make anything reversible.
Conflicts are state, not emergencies. jj stores conflicts in commits as structured data. Rebase succeeds even with conflicts. Resolve when ready.
...
Use absorb over manual squash routing. When fixing across a stack, let jj figure out where each hunk belongs.
Colocated = invisible to the team. Teammates see standard git. They don't know you use jj.
