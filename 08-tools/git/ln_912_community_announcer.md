---
rating: ⭐⭐⭐
title: ln-912-community-announcer
url: https://skills.sh/levnikolaevich/claude-code-skills/ln-912-community-announcer
---

# ln-912-community-announcer

skills/levnikolaevich/claude-code-skills/ln-912-community-announcer
ln-912-community-announcer
Installation
...
Read strategy Section 2 -- verify this qualifies as an announcement
Read CHANGELOG.md -- extract the latest entry (or the entry matching $ARGUMENTS date if provided)
Read README.md -- check current version badge, any WARNING/IMPORTANT callouts
Run: git log --oneline -20 -- recent commits for context
If $ARGUMENTS contains a topic keyword (not a date), use it as the announcement subject
Run: git diff --name-only (uncommitted) or git diff --name-only HEAD~1..HEAD (last commit) -- build the list of changed files
Read key source files from the diff (max 5 files, prioritize by relevance to $ARGUMENTS topic):
...
Create a matching GitHub Release if a version tag exists: gh release create vX.Y.Z --notes "See discussion: URL"
Update the repo description if the announcement changes the project scope
Definition of Done
