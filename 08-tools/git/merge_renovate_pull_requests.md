---
title: merge-renovate-pull-requests
url: https://skills.sh/kjetilhartveit/merge-renovate-prs-skill/merge-renovate-pull-requests
---

# merge-renovate-pull-requests

skills/kjetilhartveit/merge-renovate-prs-skill/merge-renovate-pull-requests
merge-renovate-pull-requests
Installation
...
When we want to log agent merges we append the following information to AGENT_MERGE.md (in the root folder of the repo).
...
You are on the main branch and pull latest git changes.
You have installed latest package versions (with e.g. pnpm install).
Have created a new branch and PR with an unique name with this format agent-merge-{yyyy}-{MM}-{hh}-{mm}-{ss} (replace placeholders/vars with current date in UTC).
...
Install latest package versions (with e.g. pnpm install).
...
After we have processed all open pull requests and checks are passing, then we can merge the new branch into the main branch.
Weekly Installs
12
Repository
kjetilhartveit/…rs-skill
First Seen
Mar 27, 2026
Security Audits
