---
title: beads
url: https://skills.sh/vinnie357/claude-skills/beads
---

# beads

skills/vinnie357/claude-skills/beads
beads
Installation
...
Key Principles
One task = one branch = one PR - Keep changes atomic
Claim before working - bd update --status in_progress
Close with completion - Document what was done
Minimal PRs - Title + bullets only, no templates
Wait for user - Never auto-merge or assume approval
Clean up - Delete local branch after merge
...
Identify the task domain from its title, description, and labels
Consult references/skill-catalog.md for the keyword-to-skill mapping (Tier 1: static catalog)
Check available skills in the current session for additional matches beyond the catalog (Tier 2: runtime discovery)
Add 1-3 skill: labels for the most relevant skills from either tier
