---
title: forge-ship
url: https://skills.sh/mgratzer/forge/forge-ship
---

# forge-ship

skills/mgratzer/forge/forge-ship
forge-ship
Installation
$ npx skills add https://github.com/mgratzer/forge --skill forge-ship
SKILL.md
Ship
...
Read forge-implement and execute its Steps 1 through 8 (Understand → Plan → Branch → Implement → Pattern Audit → Docs → Quality Gates → Push & Create PR).
...
Read forge-reviewer role
Read review dimensions — the four reviewer checklists
Read review rubric — severity calibration
...
subagent tool (e.g., Pi with pi-interactive-subagents): spawn four concurrent sub-agents with agent: "forge-reviewer", one for each dimension task.
Runtime context forking (e.g., Claude Code Task tool): delegate four fresh-context review tasks in parallel.
Inline fallback: if no sub-agent mechanism is available, execute the four review dimensions sequentially in the current context using the role and checklists already collected in Step 2.
...
In unattended mode: auto-triage using severity from the review rubric:
...
Markdown: create a new issue file per the plan-folder-spec and commit it
...
Components: Composes forge-implement and forge-reflect. After peer review: Use forge-address-pr-feedback to address reviewer comments.
