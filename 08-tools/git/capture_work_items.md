...
url: https://skills.sh/nesnilnehc/ai-cortex/capture-work-items
---

# capture-work-items

skills/nesnilnehc/ai-cortex/capture-work-items
capture-work-items
Installation
...
✅ 已识别的类型：分类为需求、错误或问题的工作项
✅ 必填字段完成：该类型的所有必填字段均已填写（无推断；缺失时询问用户）
✅ 状态设置：前面的初始状态：已捕获
✅ strategic_goal_id 已标：每个工作项必填 strategic_goal_id，映射到项目 strategic-goals 中的某一目标（依据 ADR 20260417-unified-value-driven-prioritization-model 决策 3.4）
✅ priority 标为 unset：新建工作项 frontmatter 必含 priority: unset，等待 prioritize-backlog 批量评分
✅ 检测到的路径：根据项目文档结构选择的输出路径（请参阅路径检测）
✅ 工件持久化：工作项写入所选路径
✅ 用户确认：用户明确确认或委托写入
✅ 批量捕获后建议优先级评分：批量捕获结束时，统计未评分 backlog 总数并建议（不自动执行）运行 prioritize-backlog
...
按 specs/artifact-contract.md §8 Runtime Norms Resolution Protocol 实现（取代 v1.x 的"解决项目规范"步骤）：

按 §8.2 发现顺序解析项目规范 → 确定 backlog-item 的 path_pattern（默认：docs/process-management/project-board/backlog/YYYY-MM-DD-{slug}.md 或 fallback docs/backlog/YYYY-MM-DD-{slug}.md；项目可覆盖为聚合式）
按 §8.3 占位符语法替换；未解析占位符按 §8.6 追问用户
若调用方 frontmatter 输入含 upstream_ref（应指向上游 roadmap 条目或 requirement）：在产出制品的 frontmatter emit parent: <upstream_ref>
记录 resolved_path + frontmatter 增量供后续写入使用
...
类型	必填字段
要求	标题、问题/需求、验收标准、strategic_goal_id
错误	标题、描述、重现步骤、预期与实际、严重性、strategic_goal_id
问题	标题、描述、类型（任务|改进|问题）、strategic_goal_id
...
对于任何缺少的必填字段，请询问用户一次一个问题。不要推断或猜测。

第 3 阶段：坚持 — 编写工件
运行“解决项目规范”，然后运行“路径检测”（见上文）
如果目标路径与默认路径不同，请与用户确认
使用适当的模板使用 YAML front-matter 编写 Markdown（请参阅输出模板）
在 front-matter 中设置 status: capture
第 4 阶段：确认
...
扫描 backlog 目录，统计 priority: unset 的条目总数
若 ≥ 阈值（默认 ≥ 3）或本次批量捕获 ≥ 2 项，则输出建议：

"Backlog 当前有 N 个未定优先级的条目。建议在下次 planning 前运行 prioritize-backlog 批量评分。此技能不会自动调用 —— 由用户决定时机（批量评分在有对照组时质量更高）。"

不自动执行 prioritize-backlog（理由见 ADR 2 决策 3.7.1：两技能节律不同，强行链式破坏评分质量）
输入与输出 (Input & Output)
输入（输入）
用户的需求、错误或问题的原始描述
...
核心成功标准（必须满足所有标准）
 ...
 strategic_goal_id 已标：已映射到 strategic-goals 中具体目标
 priority 为 unset：frontmatter 包含 priority: unset
 状态集：前面的内容中的“状态：已捕获”
 检测到路径：输出路径遵循路径检测规则
 工件保留：文件写入所选路径
 用户确认：用户确认或委托写入
 建议批量评分：批量场景下建议用户运行 prioritize-backlog（但未自动执行）
流程质量检查
