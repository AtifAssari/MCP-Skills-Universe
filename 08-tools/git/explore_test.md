---
title: explore-test
url: https://skills.sh/jacehwang/harness/explore-test
---

# explore-test

skills/jacehwang/harness/explore-test
explore-test
Installation
...
Grep — search for call sites of changed functions/classes.
Glob — search for related test files (*test*, *spec*).

Then sequentially:

Read each changed file to understand full context. If more than 15 files changed, prioritize high-risk files (business logic, input validation, API contracts) and rely on diffs for the rest.
If existing test files are found, Read them to assess current coverage.
...
Scenario Components
Target: Specific function/endpoint extracted from Step 1 (file:line)
Risk Hypothesis: "If [specific input/condition], [specific function] may [specific failure mode]. Reason: [code evidence]"
Test Input Table: Concrete values derived from extracted types/constants/constraints
Expected vs Risk Behavior: Referencing actual return types and error types
Test Method: Executable shell command, function call, or specific manual steps
...
Critical Rules
Every scenario MUST reference specific code elements (function names, parameter types, constants, error types) extracted in Step 1. Generic descriptions like "valid input" or "module test" are prohibited.
Test input tables MUST contain concrete values derived from actual types, constants, and constraints in the code — abstract placeholders like "valid value", "invalid value", "boundary value" are prohibited.
Every scenario MUST include an executable test method — a shell command, function call, or specific manual steps that a developer can run immediately.
Weekly Installs
15
Repository
jacehwang/harness
