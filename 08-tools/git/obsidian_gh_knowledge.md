---
title: obsidian-gh-knowledge
url: https://skills.sh/karlorz/agent-skills/obsidian-gh-knowledge
---

# obsidian-gh-knowledge

skills/karlorz/agent-skills/obsidian-gh-knowledge
obsidian-gh-knowledge
Installation
$ npx skills add https://github.com/karlorz/agent-skills --skill obsidian-gh-knowledge
SKILL.md
...
If VAULT_DIR does not exist and the user has confirmed a vault repo URL, run the bootstrap script, then re-resolve VAULT_DIR.
If VAULT_DIR exists and prefer_local is not false, use local mode.
In local mode, prefer official Obsidian CLI only if:
...
[!warning] Common Mistake Agents often navigate to the wrong project folder (e.g., going to trends/ when working on cmux). This wastes context and confuses the user.
...
Always read <project>/_Overview.md first to confirm you're in the right place before any other reads or writes.
...
Read before write (obsidian read ...).
For large edits, write to a draft note first, then merge intentionally.
Keep the remote in sync: pull/rebase, commit, push (no force-push).
After a successful note sync, append a short execution summary to /root/lifecycle/memory/daily/<YYYY-MM-DD>.md when the vault workflow or task context expects it.
...
list the parent folder to copy the exact path
search for the filename/keyword if unknown
read using the returned exact path
Repo resolution policy

Resolve repo in this order:

--repo <owner/repo> if provided.
--repo <key> (no /) resolved from repos.<key> in config.
default_repo from config.
If none are available, stop and ask for repo/config.
...
In GitHub mode, always write on a feature branch.
...
Create folder in the right category (GitHub, Infrastructure, Research).
Create _Overview.md first.
Include quick navigation and documentation index.
Link related notes from _Overview.md.
Templates
