---
title: inspirera
url: https://skills.sh/jgabor/agentera/inspirera
---

# inspirera

skills/jgabor/agentera/inspirera
inspirera
Installation
$ npx skills add https://github.com/jgabor/agentera --skill inspirera
SKILL.md
INSPIRERA
...
Step markers: display ── step N/5: verb before each step. Steps: identify, read, explore, map, deliver.
...
Read README
Read key source directories until you understand: core abstractions, design patterns, notable primitives, dependencies, clever approaches worth borrowing
...
Glob to map the directory structure (e.g. **/*.{ts,go,py,rs})
Read README.md if one exists
Check dependency manifests (package.json, go.mod, Cargo.toml, pyproject.toml, etc.)
Grep for patterns, imports, or abstractions relevant to the source's concepts
Read key source files to understand architecture and current patterns
Remote GitHub repos
...
Read the README
Read dependency manifests and key source files
...
Format: ─── ⬚ inspirera · status ─── followed by a summary sentence. For flagged, stuck, and waiting: add ▸ bullet details below the summary.
