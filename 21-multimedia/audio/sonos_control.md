---
title: sonos-control
url: https://skills.sh/slzatz/sonos_mcp/sonos-control
---

# sonos-control

skills/slzatz/sonos_mcp/sonos-control
sonos-control
Installation
$ npx skills add https://github.com/slzatz/sonos_mcp --skill sonos-control
SKILL.md
Sonos Control
Overview
...
Search - Use sonos:search_for_track or sonos:search_for_album
Select - Use sonos:add_track_to_queue or sonos:add_album_to_queue with position number
Find Position - Use sonos:list_queue to see where track(s) were added
Play - Use sonos:play_from_queue with the position from step 3
Verify - Use sonos:current_track to confirm correct track is playing
...
Switch speaker first - Use sonos:set_master_speaker with room name
Verify switch - Check response confirms speaker change
...
Use sonos:list_queue to find track position
Use sonos:add_to_playlist_from_queue with playlist name and position

From search:

Use sonos:search_for_track to find track
Use sonos:add_to_playlist_from_search with playlist name and position
...
Use sonos:list_queue to see all tracks and count them
For each track position (1 through N), use sonos:add_to_playlist_from_queue with the new playlist name
The first call creates the new playlist; subsequent calls append tracks
Optionally verify with sonos:list_playlist_tracks
...
Use sonos:add_playlist_to_queue to load all tracks (optionally with shuffle=True)
Use sonos:play_from_queue 1 to start playing
...
Check for conflicts - Use sonos:list_native_sonos_playlists to see if the name already exists
Convert - Use sonos:create_native_sonos_playlist_from_local with the local playlist name
Optionally specify different name - Pass native_playlist_name parameter if desired
Verify - Use sonos:list_native_sonos_playlists to confirm creation
