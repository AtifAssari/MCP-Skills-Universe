---
title: ui-sound-design
url: https://skills.sh/dannyjpwilliams/ui-sound-design-skill/ui-sound-design
---

# ui-sound-design

skills/dannyjpwilliams/ui-sound-design-skill/ui-sound-design
ui-sound-design
Installation
...
Have an audio reference file? If the user has a .wav or .mp3 file they want to match, direct them to run the analyzer:
...
Load references/audio-file-references.md for interpretation guidance
Read the synthesis_suggestion block for initial parameters
Match to the closest sound category using recipe_starting_point
Load that recipe from references/sound-recipes.md
Override recipe defaults with the profile's suggested parameters
Apply any vocabulary bridge terms from the profile's VOCABULARY MATCH section
Proceed to Generate as normal
...
Load the recipe from references/sound-recipes.md
Apply the vocabulary bridge to translate adjectives into parameter changes
For novel sounds not covered by recipes, compose from building blocks in references/web-audio-api.md
Output format: HTML preview by default (adapt assets/sound-preview.html), or ES module / React hook / class if requested
3. Listen
...
Load rules from references/audio-rules.md
Scan the code against each rule, starting with Critical priority
Report findings using the format in audio-rules.md — one line per violation with file:line — [rule-id] description
Provide a summary table (pass/fail counts by priority)
