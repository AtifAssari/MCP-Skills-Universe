...
url: https://skills.sh/notque/claude-code-toolkit/image-to-video
---

# image-to-video

skills/notque/claude-code-toolkit/image-to-video
image-to-video
Installation
...
Install via package manager: brew install ffmpeg (macOS), sudo apt install ffmpeg (Ubuntu)
Verify with ffmpeg -version after install
...
On Ubuntu: sudo apt install ffmpeg (full package)
Fall back to --visualization static which requires no special filters
Error: "Could not determine audio duration"

Cause: Audio file is corrupted or uses an unsupported container format Solution:

Test the audio independently: ffprobe /path/to/audio.mp3
Convert to a known format: ffmpeg -i input.audio -acodec pcm_s16le output.wav
