---
rating: ⭐⭐
title: ln-632-test-e2e-priority-auditor
url: https://skills.sh/levnikolaevich/claude-code-skills/ln-632-test-e2e-priority-auditor
---
...
Parse Context: Extract tech stack, critical paths, user journeys, test file list, output_dir from contextStore
Identify Critical Paths (Layer 1): Scan codebase for critical paths (Money, Security, Data) 2b) Context Analysis (Layer 2 -- MANDATORY): For each candidate critical path, ask:
...
Identify Core Journeys: Identify core user journeys (multi-step flows)
Check Critical Path Coverage: Check E2E coverage for critical paths (Priority >=20)
Check Journey Coverage: Check E2E coverage for user journeys (Priority 15-19)
Validate E2E Tests: Validate existing E2E tests (Usefulness Score >=15)
...
Write Report: Build full markdown report in memory per shared/templates/audit_worker_report_template.md, write to {output_dir}/ln-632--global.md in single Write call
...
Scan codebase for critical keywords: payment, refund, login, auth, export
Extract critical functions/endpoints
