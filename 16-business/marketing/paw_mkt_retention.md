---
title: paw-mkt-retention
url: https://skills.sh/pawbytes/skill-suites/paw-mkt-retention
---
...
paw-mkt-retention
Installation
$ npx skills add https://github.com/pawbytes/skill-suites --skill paw-mkt-retention
SKILL.md
...
Route the starting context — Read ./references/shared-patterns.md for Starting Context Router. Decide: strategy (retention program design), codebase implementation (cancel flow build), or live URL audit (existing flow review).
Read strategic context — Pre-Flight: brand and SOSTAC first when available; otherwise use existing churn data or cancel flow as working source of truth.
Load the workflow — Read ./references/workflow.md and identify the appropriate workflow phase based on the user's request.
Diagnose before prescribing — Always start with churn diagnosis. Ask for churn rate, churn type breakdown (voluntary vs involuntary), exit survey data, and cohort retention curves before recommending interventions.
...
Execute the workflow phase after approval — Follow the phased structure, entry/exit conditions, and deliverable requirements defined in ./references/workflow.md. Address involuntary churn (payment failures) first — it has the highest recovery rate.
...
Read ./references/frameworks-index.csv to find the right file by retention_stage or tags
Load only the specific file from ./references/frameworks/ that matches the task
Task	Load This File
