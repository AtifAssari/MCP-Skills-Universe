---
title: verify-claims
url: https://skills.sh/indranilbanerjee/digital-marketing-pro/verify-claims
---

# verify-claims

skills/indranilbanerjee/digital-marketing-pro/verify-claims
verify-claims
Installation
...
Extract claims from content: Execute scripts/claim-verifier.py --action extract-claims --text "{content}" to identify all verifiable assertions. The extractor categorizes claims by type:
...
Verify claims against evidence: If an evidence file is provided, execute scripts/claim-verifier.py --action verify --text "{content}" --evidence {evidence_file}. For each extracted claim, the verifier:
...
Handle missing evidence file: If no evidence file is provided, skip the verification step. Instead, present all extracted claims with their types and flag each as "unverified — no evidence provided." Guide the user on creating an evidence file:
...
Recommend corrections: For unverified and contradicted claims, provide specific recommendations:
