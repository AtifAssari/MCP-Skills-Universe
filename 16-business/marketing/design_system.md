---
title: design-system
url: https://skills.sh/juliusbrussee/cavekit/design-system
---

# design-system

skills/juliusbrussee/cavekit/design-system
design-system
Installation
$ npx skills add https://github.com/juliusbrussee/cavekit --skill design-system
SKILL.md
...
Before implementing: Read DESIGN.md (or the specific sections referenced in the task)
During implementation: Use design tokens, not hardcoded values
In commit messages: Note which DESIGN.md sections were followed
If a new pattern is needed: Implement it following existing DESIGN.md conventions and flag for design update
Revision Patterns
...
Revision Protocol
Identify the visual change in the diff
Check if DESIGN.md covers this pattern
If not covered: add the pattern to the appropriate section
If covered but wrong: update the token/value
Log the change to context/designs/design-changelog.md
Collection Import
...
Import Workflow
Choose a template: vercel, claude, stripe, github, linear, etc.
Fetch the raw DESIGN.md from the collection
Present to the user as a starting point — not a finished product
Walk through each section for customization (brand colors, typography, specific component needs)
Write the customized version to project root
After Import
...
Anti-Patterns
Generic atmosphere — "Clean, modern, professional" describes every SaaS app and none
Missing interaction states — A button without hover/focus is incomplete
Orphan colors — Colors defined in the palette but never used in components
Arbitrary spacing — Values that don't follow the spacing scale
Missing dark mode — If the app supports dark mode, every light color needs a mapping
Hex-only references in components — Use semantic names, not raw values
No Agent Prompt Guide — Section 9 is what makes DESIGN.md actionable for AI agents
Duplicating DESIGN.md into kits — Reference by section/token name only
Weekly Installs
17
Repository
