---
title: review-scoring-docx
url: https://skills.sh/timlai666/skills/review-scoring-docx
---

# review-scoring-docx

skills/timlai666/skills/review-scoring-docx
review-scoring-docx
Installation
...
Attribute catalog — a minimum of 30 product attributes inferred from the full review corpus, each anchored to a Maslow need tier and accompanied by an evaluation dimension statement.
Product × attribute score matrix — every product scored 0–10 on every attribute, colour-coded by performance band, with per-attribute averages and per-product totals.
...
Identify all uploaded review files from the conversation.
For each file, auto-detect the text-bearing column (common names: body, Body, review, text, content).
Load all valid reviews — no sampling, no row limit.
...
Read through all reviews across all products to build a holistic picture of the corpus.
Identify recurring themes, complaint patterns, praise patterns, and unmet needs.
Formulate attributes bottom-up from the corpus — do not start from a fixed template.
Ensure all five Maslow tiers are covered at or above minimum floor counts.
Write out the complete catalog with all four required fields before scoring begins.
Freeze it. The catalog must not change after scoring starts.
...
Read through all its valid reviews.
For each attribute in the frozen catalog, tally the direction and intensity of reviewer mentions.
Weight by frequency (how many reviews mention it) and intensity (how strongly positive or negative).
Assign a single integer 0–10 per the scale defined above.
...
Hard rules
Never truncate. Every valid review (length > 15 chars) is read for scoring. No sampling.
All languages count. Non-English reviews carry equal weight.
Freeze before scoring. The catalog is finalised before any product is scored.
Minimum 30 attributes. Exception: corpus < 50 total reviews → 20 is acceptable; state the reason.
Integer scores only. Individual scores are whole numbers 0–10.
Absolute table widths. Percentage widths corrupt layout.
Present with present_files. Never ask the user to navigate to the file manually.
Weekly Installs
12
Repository
timlai666/skills
