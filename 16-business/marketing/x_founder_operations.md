---
title: x-founder-operations
url: https://skills.sh/qverisai/open-qveris-skills/x-founder-operations
---

# x-founder-operations

skills/qverisai/open-qveris-skills/x-founder-operations
x-founder-operations
Installation
$ npx skills add https://github.com/qverisai/open-qveris-skills --skill x-founder-operations
SKILL.md
X Founder Operations
...
Core Philosophy
Three Pillars
Authenticity First: Your persona is a focused version of who you really are, not a character you play. People follow people, not brands.
Value Before Promotion: 80% of content should deliver genuine value. Trust is built slowly and lost quickly.
Data-Driven Evolution: Every post generates data. Every data point informs the next decision. Strategy evolves through empirical results, not gut feeling.
...
When API calls fail: STOP, report the failure honestly, and never proceed with fabricated data. See Data Validation Script for the full validation workflow.
...
Scan: Check overnight mentions, core circle activity, trending topics
Plan: Select today's content from calendar, prepare tweet drafts
Warm up: Spend 15-20 min replying to 10-15 accounts from your core circle (this primes the algorithm before your own posts)
Execution Phase: Create + Post + Engage
Post 2-3 tweets at optimal windows (see Cron Schedule), spaced 2-3 hours apart
Stay active 30-60 min after each post — reply to every comment in the first hour
Engage with core circle and community 2-3 times throughout the day
No external links in main tweet body — always put links in the first reply
Evening Phase: Check + Reflect
Collect metrics for today's posts
Quick reflection using Daily Review Template
Plan tomorrow: draft content, identify engagement targets
Weekly/Monthly/Quarterly
Weekly (Sunday evening): Performance review, content audit, A/B test assessment, next week planning. Use Weekly Review Template.
Monthly: Goal progress evaluation, strategy assessment, competitor analysis, persona check. Use Monthly Review Template.
...
Key Rules
First 30 minutes determine reach: Engagement velocity in the first 15-30 min drives 80%+ of total reach
Reply > Retweet > Bookmark > Like: Algorithm weights replies highest (~1x), likes lowest (~0.5x)
External links are penalized: 30-50% fewer impressions. Links go in first reply.
Consistency rewards: Sudden posting changes trigger spam detection. Maintain steady cadence.
Niche specificity: Stay focused on 2-3 topics. Random off-topic posts dilute your algorithm placement.
Negative signals are devastating: Mute/block/report carry ~-74x weight. Never post hostile content.
Video gets priority: Native video gets ~10x distribution vs text-only.
Goal Framework
