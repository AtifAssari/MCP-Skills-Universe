...
url: https://skills.sh/autom8minds/seo-skills/seo-technical-audit
---

# seo-technical-audit

skills/autom8minds/seo-skills/seo-technical-audit
seo-technical-audit
Installation
...
Slow server response (TTFB > 800ms) → optimize server, use CDN
Render-blocking resources → defer non-critical CSS/JS
Slow resource load → optimize/compress LCP image, use preload
...
Long JavaScript tasks blocking main thread → break into smaller tasks
Heavy event handlers → debounce, use requestAnimationFrame
Large DOM size → reduce DOM nodes (target < 1,500)
...
Minimize render-blocking resources (critical CSS inline, defer JS)
Enable compression (Brotli preferred, gzip minimum)
...
Crawlability: analyze_robots_txt → check for blocking issues
Sitemap: analyze_sitemap → validate structure and URLs
Page-level: analyze_page → meta robots, canonical, redirects, status
Performance: check_core_web_vitals → LCP, INP, CLS scores
Mobile: check_mobile_friendly → viewport, tap targets, fonts
Schema: extract_schema → structured data validation
GSC data: gsc_index_coverage → real indexing status from Google
