---
title: reelclaw
url: https://skills.sh/dansugc/reelclaw/reelclaw
---

# reelclaw

skills/dansugc/reelclaw/reelclaw
reelclaw
Installation
...
Critical Rules
MAX 15 SECONDS per reel — non-negotiable for virality
...
Always use Gemini 3.1 Flash Lite for video intelligence — model: gemini-3.1-flash-lite-preview
...
NEVER construct dansugc.com URLs for downloading — the only valid download URLs are the storage URLs returned by mcp__dansugc__purchase_videos. Do NOT fabricate URLs like /api/broll/download, /api/v1/broll/download, or any other dansugc.com download path. These are internal browser routes that require session cookies and will fail with API keys.
...
Check balance with mcp__dansugc__get_balance
Purchase selected clips with mcp__dansugc__purchase_videos
Download using the storage URLs returned in the purchase response's download_url field
...
Share ratio	0.3%+	1%+	3%+
Replicate Winners
Same hook emotion + different UGC creator
Same text style + same demo angle
Same music + spread across accounts
Search for more clips: mcp__dansugc__search_videos(emotion="shocked", min_virality=70)
...
Broad niche search — keyword search for the main niche term
Specific product search — keyword search for the exact product/brand
Creator discovery — user search to find niche influencers
Creator deep-dive — profile videos sorted by popular for top 2-3 creators
Cross-platform — Repeat key searches on Instagram for format diversity
...
Search for the format keyword (e.g., "supplement review TikTok")
...
Search for the brand name directly
...
Check Green Zone specs in ./references/green-zone.md
