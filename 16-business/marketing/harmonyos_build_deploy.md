---
title: harmonyos-build-deploy
url: https://skills.sh/fadinglight9291117/arkts_skills/harmonyos-build-deploy
---

# harmonyos-build-deploy

skills/fadinglight9291117/arkts_skills/harmonyos-build-deploy
harmonyos-build-deploy
Installation
...
Clean: hvigorw clean --no-daemon
Install dependencies: ohpm install --all
Build: hvigorw assembleApp --mode project -p product=default -p buildMode=release --no-daemon
Deploy to device (see Push and Install below)
...
Read AppScope/app.json5 to get bundleName
Check outputs/ for existing build outputs. If empty or missing, collect signed HAP/HSP from each module's build directory ({srcPath}/build/default/outputs/default/*-signed.*) into outputs/. See module-discovery.md for details.
Deploy to device (see Push and Install below)
Launch: hdc -t <UDID> shell "aa start -a EntryAbility -b <bundleName>"
Report success/failure with details
...
Clean cache: hdc -t <UDID> shell "bm clean -n <bundleName> -c"
Clean data: hdc -t <UDID> shell "bm clean -n <bundleName> -d"
