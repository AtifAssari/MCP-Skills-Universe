---
title: axiom-ios-build
url: https://skills.sh/charleswiltgen/axiom/axiom-ios-build
---

# axiom-ios-build

skills/charleswiltgen/axiom/axiom-ios-build
axiom-ios-build
Installation
...
Want automated build fix (environment-first diagnostics)? → build-fixer (Agent)
Want build time optimization scan? → build-optimizer (Agent)
...
Need to reproduce crash interactively / inspect runtime state? → axiom-lldb
Code signing error (certificate, profile, entitlement, Keychain)? → code-signing / code-signing-diag
Need to see runtime console output (print/os_log)? → xclog-ref or /axiom:console
...
User: "No signing certificate found when I try to build" → Invoke: /skill axiom-code-signing-diag

User: "errSecInternalComponent in my GitHub Actions CI" → Invoke: /skill axiom-code-signing-diag

User: "How do I set up code signing for GitHub Actions?" → Invoke: /skill axiom-code-signing

User: "What is my app printing to the console?" → Invoke: /skill axiom-xclog-ref or /axiom:console
