---
title: rxjava-to-coroutines-migration
url: https://skills.sh/new-silvermoon/awesome-android-agent-skills/rxjava-to-coroutines-migration
---

# rxjava-to-coroutines-migration

skills/new-silvermoon/awesome-android-agent-skills/rxjava-to-coroutines-migration
rxjava-to-coroutines-migration
Installation
...
Convert the Source: Change the return type in the repository or data source layer first. Convert to suspend functions for one-shot operations, and Flow for streams.
Rewrite Operators: Translate the RxJava operators to their Flow or Coroutine equivalents. Note that many RxJava operators can simply be replaced by standard Kotlin collection/sequence operations inside a map or onEach block.
Update the Subscription: Replace .subscribe(...) with launch { ... } and .collect { ... } in the ViewModel or Presenter. Ensure the launch is tied to the correct lifecycle scope (e.g., viewModelScope).
Handle Errors: Replace onError blocks with try/catch around suspend functions, or .catch { } operators on Flows.
Handle Threading: Remove .subscribeOn() and .observeOn(). Use withContext where necessary, or .flowOn() to change the context of the upstream flow.
