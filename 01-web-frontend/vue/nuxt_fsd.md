...
rating: ⭐⭐
title: nuxt-fsd
url: https://skills.sh/adamkasper/nuxt-fsd-skills/nuxt-fsd
---

# nuxt-fsd

skills/adamkasper/nuxt-fsd-skills/nuxt-fsd
nuxt-fsd
Installation
$ npx skills add https://github.com/adamkasper/nuxt-fsd-skills --skill nuxt-fsd
SKILL.md
...
Key mapping rules
src/ is the FSD root. All FSD layers (pages/, widgets/, features/, entities/, shared/) live here.
app/ is the Nuxt runtime shell. It follows Nuxt conventions, not FSD. It consumes FSD code from src/ via imports.
app/pages/ contains thin routing shells only — they delegate to src/pages/ page slices. See "Thin page pattern" below.
server/ is outside FSD entirely. Server routes follow Nuxt server conventions.
Thin page pattern
...
Extract to a lower layer — if two features share a concept, move it to entities or shared
Event-based communication — use a shared event bus (mitt) or Pinia store for loose coupling
Composition at a higher layer — let a page or widget compose multiple features together
...
Create src/ directory with FSD layer subdirectories
Add typescript includes (tsConfig, sharedTsConfig, nodeTsConfig) to nuxt.config.ts
Start all new code in FSD structure under src/
Migrate existing code slice-by-slice when touched
Convert fat pages to thin routing shells + src/pages/ slices
Update imports to use ~~/src/ and public APIs
Remove old directories once empty
Weekly Installs
12
Repository
adamkasper/nuxt…d-skills
GitHub Stars
1
First Seen
Mar 3, 2026
