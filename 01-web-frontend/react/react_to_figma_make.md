---
rating: ⭐⭐⭐
title: react-to-figma-make
url: https://skills.sh/lintendo/axhub-skills/react-to-figma-make
---

# react-to-figma-make

skills/lintendo/axhub-skills/react-to-figma-make
react-to-figma-make
Installation
$ npx skills add https://github.com/lintendo/axhub-skills --skill react-to-figma-make
SKILL.md
React 导出到 Figma Make
概述

本技能将帮助你把任意 React 组件或应用转换为符合 Figma Make 项目规范的标准结构。转换完成后，项目将具备：
...
assets/empty-canvas.fig：空白 canvas 模板 （运行脚本前，请先在技能的 scripts 目录下执行 npm install 安装依赖 pako 和 kiwi-schema。）
...
将 App.tsx 改造为薄壳模式
创建根目录 index.tsx
迁移样式到 src/styles/globals.css
补齐元文件
生成 canvas.fig
场景 B：Next.js 项目（V0 等）
移除所有 Next.js 特定代码（"use client"、next/image、next/link、路由等）
将 app/page.tsx 或 pages/index.tsx 的内容迁移到 src/components/
将 app/globals.css 迁移到 src/styles/globals.css
将 public/ 下的静态资源迁移到 src/assets/ 或 images/
排除 Next.js 专属依赖（next、@vercel/*），保留 UI 组件依赖
创建双入口（index.tsx + src/App.tsx），补齐元文件
场景 C：AI Studio 项目
从 index.html 的 Import Map 提取依赖，转为 npm 包安装
提取 <style> 标签内容到 src/styles/globals.css
process.env.* 改为 import.meta.env.VITE_*
CDN 图片引用可保留，或下载到 src/assets/
创建双入口，补齐元文件
场景 D：已有 Figma Make 导入的项目（补齐导出能力）

此时重点不是从头创建结构，而是确保现有项目的导出壳子与当前页面同步。

检查 index.tsx（根入口）和 src/App.tsx（导出壳子）是否同步
如果不同步，让 src/App.tsx 复用根入口的共享组件
不要删除已有的 canvas.fig、meta.json、images/ 等原始资产
重新执行 pack 回写 canvas.fig
刷新 canvas.code-manifest.json 和 meta.json.exported_at
常见注意点
产物和资源
