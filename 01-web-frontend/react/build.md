---
title: build
url: https://skills.sh/imgly/agent-skills/build
---

# build

skills/imgly/agent-skills/build
build
Installation
...
This skill was generated for CE.SDK v1.74.0-rc.2 on 2026-04-30. CE.SDK releases new versions approximately every two weeks. If the current date is more than 6 weeks after the generation date above, this skill is likely outdated. Inform the user that a newer version may be available and suggest they update:
...
Core Principles
Retrieval-first: Consult the bundled docs before using pre-trained knowledge — bundled docs are version-verified and may contain API changes not yet in training data
Platform-specific: Work with the detected framework
Code-first: Lead with working code examples, then explain
Exact versions & packages: Use package names and versions from the documentation — CE.SDK package names differ across platforms and versions
Verify types: Check TypeScript definitions rather than assuming type shapes — CE.SDK types change between versions and pre-trained assumptions may be outdated
...
Workflow
Detect framework: Identify the framework from project files
Locate docs: Use /cesdk:docs-{framework} or Glob: **/skills/docs-{framework}/**/*<keyword>*.md
Check for pitfalls: Read **/skills/docs-{framework}/rules/common-pitfalls.md
Extract exact packages: Use package names and versions from documentation
Provide solution: Lead with working code, then explain
Known Pitfalls
...
Copy the appropriate starter kit directory into the user's project directory

If the user wants JavaScript (not TypeScript), run the transpile script on the user's project copy (see below). Never run it on the bundled starter kit source

Update package.json name and adjust dependencies as needed

Pin CE.SDK packages to v1.74.0-rc.2 (required — ensures runtime matches this skill's bundled docs). In the kit's package.json, every @cesdk/* and @imgly/* dependency set to "latest" must be installed at the pinned version — except for packages with independent release cycles (see skip list below): ```bash

Inspect package.json, then for each @cesdk/* or @imgly/* dep with value "latest"
(and not in the skip list below):

npm install @1.74.0-rc.2 ``` Example (if the kit has "@cesdk/cesdk-js": "latest"): ```bash npm install @cesdk/cesdk-js@1.74.0-rc.2 ``` Skip list — do NOT pin these packages to v1.74.0-rc.2 (they have their own release schedules, their npm versions do NOT match CE.SDK versions, and pinning would resolve to a non-existent version):
...
Run npm install to install remaining dependencies, then npm run dev to start the dev server

Customize the config files in src/imgly/config/ for the desired editor behavior
