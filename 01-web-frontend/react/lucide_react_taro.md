---
title: lucide-react-taro
url: https://skills.sh/louisyoungx/lucide-react-taro/lucide-react-taro
---

# lucide-react-taro

skills/louisyoungx/lucide-react-taro/lucide-react-taro
lucide-react-taro
Installation
...
注意事项
颜色默认值：默认颜色为 currentColor，在小程序 Image 组件中可能不会继承父元素颜色，建议显式设置 color 属性。
性能优化：组件内部已实现 base64 缓存，相同参数组合只计算一次。
兼容性：已内置 base64 编码 polyfill，无需额外配置即可在微信小程序中使用。
TabBar 图标：小程序 TabBar 不支持 SVG/base64，请使用 CLI 工具生成 PNG 图标。
TabBar 图标路径：iconPath 和 selectedIconPath 必须添加 ./ 前缀（如 ./assets/tabbar/house.png），否则图片无法正确加载。
Weekly Installs
14
Repository
louisyoungx/luc…act-taro
GitHub Stars
1
First Seen
Mar 4, 2026
