...
url: https://skills.sh/trancong12102/agentskills/react-native-advanced
---

# react-native-advanced

skills/trancong12102/agentskills/react-native-advanced
react-native-advanced
Installation
...
Memoize the flattened items array with useMemo — data.pages.flatMap() creates a new reference each render.
...
Missing focusManager/onlineManager setup — refetchOnWindowFocus and offline pausing do nothing without manual AppState and NetInfo wiring.

Awaiting prefetchQuery before router.push — makes navigation feel slow. Fire prefetch without await, let React Query cache serve the destination screen.

Using useGlobalSearchParams instead of useLocalSearchParams — global re-renders on every navigation event. Always prefer local.

useEffect for screen focus refetch — useEffect doesn't re-run when navigating back (screens stay mounted). Use useFocusEffect from expo-router.

getNextPageParam returning null — must return undefined to signal no next page. Coerce API nulls: lastPage.nextCursor ?? undefined.

FlashList v2 requires New Architecture — v2 is a ground-up rewrite for Fabric. estimatedItemSize is deprecated (ignored). overrideItemLayout only supports span.

MMKV getString returns undefined — Zustand's StateStorage.getItem must return null for missing keys. Always coerce: ?? null.

Creating MMKV instance inside a component — creates new instances each render. Declare at module level.

keyboardShouldPersistTaps not set on form ScrollView — first tap dismisses keyboard instead of pressing Submit button.

retry: 3 (default) on mobile — with flaky connections, 3 retries with exponential backoff can take 30+ seconds. Consider retry: 1 for time-sensitive UI.

Mutating shared value properties — sv.value.x = 50 breaks reactivity. Must replace entire value: sv.value = { x: 50, y: 0 }.

GestureHandlerRootView missing flex: 1 — wrapper has zero height. Gestures appear to not work at all.

Unmounting TrueSheet while open — the native sheet does NOT dismiss automatically. Always call dismiss() before removing from tree.

auto detent with scrollable in TrueSheet — auto-sizing and scroll pinning conflict. Use fixed detents when scrollable={true}.

Missing setNotificationHandler — all foreground notifications silently suppressed. No error.

checkForUpdateAsync on slow network — can freeze Android UI for minutes. Run after first render with a manual timeout.

expo-image recyclingKey in FlashList — without it, recycled cells flash the previous item's image.

expo-secure-store biometric invalidation — keys with requireAuthentication become permanently unreadable when biometrics change. No recovery except clearing app data.
