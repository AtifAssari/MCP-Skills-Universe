---
title: uniwind-best-practices
url: https://skills.sh/imfa-solutions/skills/uniwind-best-practices
---

# uniwind-best-practices

skills/imfa-solutions/skills/uniwind-best-practices
uniwind-best-practices
Installation
...
Never execute installation commands — Present bun add, npm install, npx, pod install, and build commands as instructions for the user to run. Do not execute them directly.
Never read credential or auth config files — Do not read .npmrc, .yarnrc.yml, .env, or any file that may contain tokens, secrets, or authentication credentials. When diagnosing setup issues, only check package.json, metro.config.js, babel.config.js, global.css, tsconfig.json, and app.json — these are project configuration files, not credential stores.
Package manager trust configuration is user-managed — When advising about trustedDependencies, postinstall scripts, or package manager settings, present the configuration for the user to apply. Do not modify these files directly.
...
Critical Rules
Tailwind v4 only — Uniwind does not support Tailwind v3. Use @import 'tailwindcss' (not @tailwind base).
Never construct classNames dynamically — Use complete string literals. Tailwind scans at build time.
Never use cssInterop — Uniwind does not override global components like NativeWind.
No tailwind.config.js needed — All config is in global.css via @theme and @layer theme.
No ThemeProvider required — Use Uniwind.setTheme() directly.
withUniwindConfig must be outermost Metro config wrapper.
@theme variables only affect styled components — <Text> without className uses RN defaults.
Font families: single font only — No fallbacks in RN. --font-sans: 'Roboto-Regular' not 'Roboto', sans-serif.
rem default is 16px — NativeWind used 14px. Configure polyfills: { rem: 14 } if migrating.
Core Patterns
Styling Components
