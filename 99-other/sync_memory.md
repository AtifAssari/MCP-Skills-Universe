---
title: sync-memory
url: https://skills.sh/indranilbanerjee/digital-marketing-pro/sync-memory
---

# sync-memory

skills/indranilbanerjee/digital-marketing-pro/sync-memory
sync-memory
Installation
...
Update sync state: After all items are processed, update sync-state.json with the new checkpoint timestamp, cumulative items synced, items skipped, items failed (with content hashes for retry), and per-layer storage status. If any items failed, their hashes are queued for automatic retry on the next sync run.
Report sync summary: Present the complete sync report with actionable details — what was synced, what was skipped and why, what failed and how to fix it, storage utilization status, and the current state of persistent memory for this brand.
Output
