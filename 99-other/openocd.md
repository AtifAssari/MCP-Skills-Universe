---
title: openocd
url: https://skills.sh/zhinkgit/embeddedskills/openocd
---

# openocd

skills/zhinkgit/embeddedskills/openocd
openocd
Installation
...
读取 .embeddedskills/config.json 获取工程级配置（board/interface/target 等）
读取 .embeddedskills/state.json 获取历史状态
参数解析优先级：CLI 参数 > 工程配置 > state.json > 默认值
已知 board 时优先使用 -f board/*.cfg，否则组合 -f interface/*.cfg -f target/*.cfg
board、interface、target 同时缺失时，不自动拼接组合，直接要求用户补充
按 operation_mode 决定是否需要确认后执行
根据子命令调用对应脚本
统一解析输出中的 Info、Error 和就绪日志，返回结构化结果
成功执行后，将确认过的参数写回 .embeddedskills/config.json
脚本调用
