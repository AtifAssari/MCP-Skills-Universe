---
title: scenes
url: https://skills.sh/phaserjs/phaser/scenes
---

# scenes

skills/phaserjs/phaser/scenes
scenes
Installation
$ npx skills add https://github.com/phaserjs/phaser --skill scenes
SKILL.md
Scenes
...
INIT (1) - scene.init(data) is called if defined. Data comes from settings.data.
START (2) - Systems.start() fires. Events start and ready are emitted.
LOADING (3) - scene.preload() is called if defined. Loader runs. On completion, proceeds to create.
CREATING (4) - scene.create(data) is called if defined. Same data as init.
RUNNING (5) - scene.update(time, delta) called every frame. Scene renders.
PAUSED (6) - No update, but still renders.
SLEEPING (7) - No update, no render. State preserved in memory.
SHUTDOWN (8) - Scene is shut down, systems emit shutdown. Can be restarted.
