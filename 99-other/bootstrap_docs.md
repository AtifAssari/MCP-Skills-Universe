...
url: https://skills.sh/nesnilnehc/ai-cortex/bootstrap-docs
---

# bootstrap-docs

skills/nesnilnehc/ai-cortex/bootstrap-docs
bootstrap-docs
Installation
$ npx skills add https://github.com/nesnilnehc/ai-cortex --skill bootstrap-docs
SKILL.md
...
✅ 强制执行命名约定：所有路径都使用严格的短横线大小写； ADR 文件遵循 YYYYMMDD-slug-title.md 格式
...
按 §8.2 发现顺序解析项目规范 → 确定各 artifact_type（特别是 adr）的 path_pattern（默认：docs/process-management/decisions/YYYYMMDD-{slug}.md；项目可覆盖为聚合式）
按 §8.3 占位符语法替换；未解析占位符按 §8.6 追问用户
若调用方 frontmatter 输入含 upstream_ref（ADR 通常指向关联的 design / requirement）：在产出制品的 frontmatter emit parent: <upstream_ref>
记录 resolved_path + frontmatter 增量供后续写入使用

注：初始化模式下项目可能尚无 ARTIFACT_NORMS.md，Stage 0 fall-through 到技能默认，不阻断初始化工作。
...
初始化模式步骤
确定项目规模：小型、中型或大型（根据用户或上下文）。
按规模从模板中选择文档子集：
小：项目概述、开发指南、用户指南
中：+架构、设计、需求和规划
大型：+流程管理、操作指南、合规性、社区和贡献
从“TEMPLATE_BASE_URL”获取模板（请参阅附录）或使用本地克隆。
仅选定的文档复制到项目“docs/”。当规模允许时，根据 specs/artifact-contract.md 创建合同对齐目录：docs/design-decisions/（中+）、docs/calibration/、docs/process-management/project-board/待办/、docs/process-management/decisions/（大）。可以选择从合同创建“docs/ARTIFACT_NORMS.md”（根据 specs/artifact-norms-schema.md）供用户自定义。除非用户明确请求，否则不要创建其他空目录。
使用项目元数据（名称、日期、技术堆栈）填充占位符，并提示缺少的关键数据。
创建一个“VERSION”文件（例如“1.0.0”），除非用户明确请求不创建新文件。
验证：没有未替换的占位符、链接有效、表格对齐。
调整模式步骤
使用 project-documentation-template 作为结构、约定和文件/目录名称的目标参考。
扫描“docs/”下的现有文档（结构、占位符、链接、版本）。
与模板进行比较并找出差距：
结构和路径不匹配（非标准目录/文件名）
文档可与模板对齐（重命名、移动或合并）
未填充的占位符、损坏的链接、版本不一致
制作推荐列表并呈现给用户；申请前要求确认。
确认后，将更改就地应用到现有文件。除非用户明确请求，否则不要创建空目录或添加模板文件。 6.该技能是幂等的：可以重复运行以迭代地组织和细化文档。
约定（来自 llms.txt）
...
ADR 文件：每个 specs/artifact-contract.md docs/process-management/decisions/YYYYMMDD-slug.md
...
模式覆盖（可选）：初始化 | 调整
输出（输出）
初始化：在docs/、VERSION下填充文档，以及创建文件的简短摘要
