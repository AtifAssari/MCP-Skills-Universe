---
title: cs-issue-report
url: https://skills.sh/liuzhengdongfortest/codestable/cs-issue-report
---
...
启动检查
确认是 bug 不是新功能需求——描述"想加 X 功能"的告诉他走 cs-feat
看有没有相关 issue 目录——Glob codestable/issues/，有同类问题先和用户确认是新建还是更新
快速通道判断（唯一正式判定点）——按用户线索读一下相关代码（Grep / Read 定位）：
...
确定 issue 目录名——跟用户商定 slug，日期前缀用今天（环境信息 currentDate）。目录不存在就创建。快速通道也要建 issue 目录，{slug}-fix-note.md 放那
必答 5 问
