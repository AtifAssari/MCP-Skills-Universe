...
rating: ⭐⭐⭐
title: rust-refactor
url: https://skills.sh/pproenca/dot-skills/rust-refactor
---
...
Add new types and From bridges
Plumb new types through runtime alongside old
Migrate consumers one at a time (one commit per subsystem)
Remove legacy type only after ALL consumers migrated
...
Remove flag check from all call sites
Rename gated functions: init_if_enabled -> init
Simplify return types: Option<Handle> -> Handle
Move the flag to Stage::Removed in the features registry
Remove conditional branches entirely (do not just change the default)
Check #[cfg(feature = "...")] in Cargo.toml -- make those deps unconditional
Remove comparison/discrepancy metrics that tracked old vs new path
Delete the feature flag definition
...
Identify a domain-coherent boundary with self-contained dependencies
Create new crate with minimal Cargo.toml
Move code, keeping re-exports in the original crate for backward compat
Update workspace manifests and lockfiles
Verify: cargo test -p parent-crate && cargo test -p new-crate
