---
title: fund-advisor
url: https://skills.sh/realqiyan/fund-advisor/fund-advisor
---

# fund-advisor

skills/realqiyan/fund-advisor/fund-advisor
fund-advisor
Installation
$ npx skills add https://github.com/realqiyan/fund-advisor --skill fund-advisor
SKILL.md
...
注意事项
环境要求：需要先运行 init 命令确保 mcporter 和 qieman-mcp 配置正确
批量限制：MCP 服务单次最多查询 10 只基金
数据时效：基金净值和持仓数据有延迟，注意查看净值日期
CSV 编码：支持 utf-8, utf-8-sig, gbk, gb18030 编码
目录结构：工程代码位于 tools/ 目录下
高级功能场景
场景1：投资组合诊断分析
...
检查 mcporter 是否已安装
检查 QIEMAN_API_KEY 环境变量是否已配置
自动生成 ~/.mcporter/mcporter.json 配置文件
测试 MCP 服务连接
参考文档
MCP工具完整清单
CSV导入格式规范
