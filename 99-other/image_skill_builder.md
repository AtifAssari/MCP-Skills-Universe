---
title: image-skill-builder
url: https://skills.sh/zephyrwang6/myskill/image-skill-builder
---

# image-skill-builder

skills/zephyrwang6/myskill/image-skill-builder
image-skill-builder
Installation
$ npx skills add https://github.com/zephyrwang6/myskill --skill image-skill-builder
SKILL.md
批量生图 Skill 创建助手
...
理解用户需求 → 2. 组装风格模板 → 3. 生成完整 Skill 文件 → 4. 验证可用性
工作流程
Phase 1: 场景理解（3 个问题）
...
关键设计原则
每次只问一个问题 — 不要同时抛多个问题，降低用户认知负担
提供明确选项+自定义入口 — 数字选择快速，同时允许自由描述
智能默认值 — 根据前面的回答推断后续默认值（如选了文章配图 → 默认16:9、自动插入文档）
即时反馈 — 每个回答后简短确认理解，让用户有掌控感
允许跳过和回退 — 用户可以说"跳过"或"回去改第X个"
参考文件
文件	用途	何时读取
