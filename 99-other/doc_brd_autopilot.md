---
title: doc-brd-autopilot
url: https://skills.sh/vladm3105/aidoc-flow-framework/doc-brd-autopilot
---

# doc-brd-autopilot

skills/vladm3105/aidoc-flow-framework/doc-brd-autopilot
doc-brd-autopilot
Installation
$ npx skills add https://github.com/vladm3105/aidoc-flow-framework --skill doc-brd-autopilot
SKILL.md
doc-brd-autopilot
Purpose
...
If --iplan is an existing file path, use it.
If --iplan value matches IPLAN-NNN (no path), resolve in order:
work_plans/IPLAN-NNN*.md
governance/plans/IPLAN-NNN*.md
If multiple matches are found, fail with disambiguation output listing candidate files.
...
`--iplan <path	IPLAN-NNN>`	Implementation plan
Detection Algorithm
...
Scan for docs/00_REF/
If found, list available subfolders:
...
No - creating from scratch (default)
Yes - select reference folder(s)
...
Skill Delegation: This phase follows rules defined in doc-brd skill. See: .claude/skills/doc-brd/SKILL.md Section "BRD Categorization: Platform vs Feature"
...
Skill Delegation: This phase follows rules defined in doc-brd skill. See: .claude/skills/doc-brd/SKILL.md for complete BRD creation guidance.
...
Check template (BRD-MVP-TEMPLATE.md) for deliverable_type field

If missing, run auto-detection on generated content

If still unclear, default to code

Log the selection reason for audit trail
...
Generate Section 7.2: Architecture Decision Requirements:
...
Master Glossary Handling (NEW in v2.3):
...
BRD Index Handling (NEW in v2.4):
...
Read existing BRD-00_index.md
Parse Document Registry table
...
Skill Delegation: This phase uses validation rules from doc-brd-audit skill (unified quality gate). See: .claude/skills/doc-brd-audit/SKILL.md for complete validation and review rules.
...
Total Deductions: 0 Total PRD-Ready Score: 100/100 (Target: >= 90) Status: READY FOR PRD GENERATION
...
Compute hash immediately for each upstream document using bash:

sha256sum <upstream_file_path> | cut -d' ' -f1


Store in drift cache with format:
...
Never use placeholders - The following are INVALID and must not be written:
...
Re-compute hash for each upstream document via bash
Compare with stored hash
Update cache with new hash
Flag drift if hash differs
...
| 3. Score Calculation | Deduction-based formula, category deductions, final score | | 4-N. Validation Details | Section-specific validation results | | N+1. Issues Summary | Errors, non-blocking issues, info categorized | | N+2. Recommendations | Priority-ordered fix recommendations | | N+3. Traceability | Parent document reference |
