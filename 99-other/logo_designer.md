---
title: logo-designer
url: https://skills.sh/neonwatty/logo-designer-skill/logo-designer
---

# logo-designer

skills/neonwatty/logo-designer-skill/logo-designer
logo-designer
Installation
...
Create the logos/concepts/ directory first
Dispatch one Task agent per concept, all in the same message so they run concurrently. Each agent should:
...
After all agents complete, generate logos/preview.html and present the results
...
Generate logos/preview.html using the preview template below
Tell the user to open logos/preview.html in their browser
Briefly describe each concept (1 sentence each) so the user can match descriptions to visuals
...
Dispatch one Task agent per variation, all in the same message
Each agent receives: the base SVG content (copy the full SVG inline in the prompt), the specific variation to apply, the target file path, and the full SVG conventions
After all agents complete, regenerate logos/preview.html and present the results
...
Copy the chosen concept as the starting point — save the first refinement as logos/iterations/iteration-1.svg
Apply the user's feedback and save each new version with an incrementing number
Regenerate logos/preview.html after each iteration, showing all iterations (most recent first) so the user can compare
Tell the user to refresh their browser after each iteration
...
Create the logos/export/ directory
Copy the final SVG to logos/export/logo.svg
Run the bundled export script to generate PNGs:
...
Identify target files — Check the repo for existing icon/logo files: public/favicon.svg, public/favicon.ico, public/pwa-*.png, public/apple-touch-icon.png, assets/logo.svg, ios/.../AppIcon.appiconset/, public/manifest.json, etc.
Clone and branch — Clone the repo (or use the existing checkout), create a branch like chore/new-logo
Replace files — Copy the final SVG as the favicon/logo. Generate platform-specific sizes:
...
Commit and PR — Commit with a message like chore: replace logo with new [description], push, and create a PR with a summary of what was updated
Weekly Installs
109
Repository
neonwatty/logo-…er-skill
GitHub Stars
17
First Seen
1 day ago
