---
title: invest-analyzer
url: https://skills.sh/zhangyangx/invest-analyzer/invest-analyzer
---

# invest-analyzer

skills/zhangyangx/invest-analyzer/invest-analyzer
invest-analyzer
Installation
$ npx skills add https://github.com/zhangyangx/invest-analyzer --skill invest-analyzer
SKILL.md
股票分析 Skill
一、工作流程
用户输入股票代码或股票名称
AI 调用 scripts 目录下的脚本获取数据
AI 输出结构化的 Markdown 分析报告
二、脚本列表
...
多个搜索结果 → 询问用户确认
3.2 核心分析流程
优先直接调用 stock_technical_snapshot.py 获取聚合后的技术快照
stock_technical_snapshot.py 内部会自动完成以下步骤：
# 实时行情
python3 stock_quote.py 600519
...
若需要分步排查，再分别调用 stock_quote.py、stock_kline.py、stock_indicators.py、stock_technical_analysis.py
输出报告需包含两个维度的技术分析
3.3 新闻搜索触发条件
