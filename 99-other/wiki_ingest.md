---
title: wiki-ingest
url: https://skills.sh/djalmajr/essential-skills/wiki-ingest
---

# wiki-ingest

skills/djalmajr/essential-skills/wiki-ingest
wiki-ingest
Installation
$ npx skills add https://github.com/djalmajr/essential-skills --skill wiki-ingest
SKILL.md
...
When searching for existing pages that should absorb the new source, use QMD (local hybrid search) if it is set up for this wiki. Setup is one-time per repo — see docs/wiki/qmd-setup.md.
...
Check if it has already been ingested by consulting raw/index.md:
...
Read the source in raw/ completely (in parts if needed due to context limits).
...
Create the source summary in wiki/sources/<slug>.md:
...
Read the source and the existing summary.
...
Wait for approval and execute.
Update the summary, raw/index.md, and wiki/log.md.
Rules
