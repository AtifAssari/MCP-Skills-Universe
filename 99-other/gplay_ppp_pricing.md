---
title: gplay-ppp-pricing
url: https://skills.sh/tamtom/gplay-cli-skills/gplay-ppp-pricing
---

# gplay-ppp-pricing

skills/tamtom/gplay-cli-skills/gplay-ppp-pricing
gplay-ppp-pricing
Installation
...
Try your update with any version (e.g., "2022/02")
If the version is outdated, the API error will tell you the latest:
Invalid regions version 2024/01, latest value is ExternalRegionLaunchVersionId{versionId=2025/03}

Retry with the version from the error message
...
Fetch the current product/subscription to get ALL existing regional configs.
Recompute the PPP-adjusted price with the new base price or new multiplier.
Merge the new PPP prices into the complete regional configs list.
Update with the merged JSON (must include ALL regions, not just the changed ones).
For subscriptions with active subscribers, run migrate-prices for each affected base plan.
Verify with a fetch + summary review.
...
Common Pitfalls
Sending only PPP regions — The API rejects updates that remove existing regions. Always fetch-then-merge.
Missing --regions-version — Required for any pricing update. The API error will tell you the latest version if you guess wrong.
Missing --update-mask — Use "basePlans" for subscriptions, "purchaseOptions" for one-time products.
Missing otherRegionsConfig — If a subscription base plan previously had otherRegionsConfig set, it must be included in the update.
Missing newRegionsConfig — If a one-time product purchase option previously had newRegionsConfig set, it must be included. Omitting it causes: "Cannot remove currency for new regions once it has been added."
Wrong currency codes — Don't assume currencies. Fetch from Google Play, as they change between regions versions.
Mixing API formats — Legacy IAPs use priceMicros/currency. New subscriptions and one-time products use units/nanos/currencyCode. Don't mix them.
--dry-run is a global flag — Place it before the subcommand: gplay --dry-run subscriptions update ..., NOT gplay subscriptions update --dry-run .... The latter fails with "flag provided but not defined."
Batch-update JSON format — For onetimeproducts batch-update and subscriptions batch-update, regionsVersion and updateMask go inside each request object in the JSON, not as CLI flags.
Product IDs are permanent — Google Play permanently reserves product IDs after deletion. If you create a product via gplay iap create then delete it, the ID cannot be reused — not even with gplay onetimeproducts create. Always choose product IDs carefully. Do not create a product via the legacy API and then try to recreate it via the new API.
New OTP products start in DRAFT — After gplay onetimeproducts create, the purchase option is in DRAFT state. You must activate it with gplay purchase-options batch-update-states before it's available to users.
--regions-version is required for create too — onetimeproducts create uses PATCH with allowMissing=true internally, so --regions-version is required even when creating new products, not just when updating.
Discover commands with gplay --help — Purchase option activation is under gplay purchase-options, not under gplay onetimeproducts. Always run gplay --help to see all top-level command groups.
Notes
