---
title: review-code
url: https://skills.sh/nesnilnehc/ai-cortex/review-code
---

# review-code

skills/nesnilnehc/ai-cortex/review-code
review-code
Installation
...
✅ 衍生的风险信号：风险信号是根据最终的重复数据删除结果和变化背景生成的（而不是根据个人技能）
...
直接代码分析（使用原子审查技能：review-diff、review-codebase、review-security等）
...
范围 = 差异	—	确认：*包括未跟踪的文件？*默认是。确保 diff + 未跟踪的内容可用于审查 diff。
...
review-security：安全调查结果。
review-performance：性能调查结果。
review-architecture：架构发现。
review-testing：测试结果（存在性、覆盖率、质量、边缘情况）。 （为将来保留：审查-可靠性，审查-可维护性。）
收集所有发现。
...
游标/代理摘要
执行此技能时，依次应用：
review-diff 或 review-codebase（范围）
review-dotnet 或 review-java 或 review-go 或 review-php 或 review-powershell 或 review-sql（语言，可选）
复习-vue或其他框架技能（可选）
图书馆技能（可选，如果有的话）
审查安全性，然后审查性能，然后审查架构，然后审查测试（cognitive）
将调查结果汇总并删除重复内容到一份报告中
从最终结果 + 变更背景中得出最终的“risk_signals”（和可选的“risk_confidence”）
...
不要求每个原子技能发出风险信号。风险标签由协调者拥有，并且仅在最终聚合时生成。
