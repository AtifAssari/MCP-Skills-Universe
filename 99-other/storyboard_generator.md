...
url: https://skills.sh/zephyrwang6/myskill/storyboard-generator
---
...
storyboard-generator
Installation
$ npx skills add https://github.com/zephyrwang6/myskill --skill storyboard-generator
SKILL.md
分镜故事板生成器
...
工作流程
Phase 1: 故事润色与分镜拆分
接收故事 - 获取用户描述的故事内容（文字、大纲、或灵感片段）
润色故事线 - 补充叙事细节，梳理情感弧线，输出结构化故事概要：
故事概要：一句话概括
情感弧线：[起点] → [转折] → [结局]

拆分分镜 - 将故事拆为 6-10 个关键帧，每帧包含：
场景标题
镜头类型（远景/中景/近景/特写等）
画面描述（人物动作、环境、光线）
情绪基调
对白/文字（如有）
展示分镜脚本 - 以表格形式展示，等待用户确认或调整
...
风格前缀 - 选定风格的标准描述
镜头指令 - 如 "Wide establishing shot" / "Close-up"
画面叙述 - 用完整语句描述场景内容（人物、动作、环境、光线、氛围）
情绪指令 - 明确画面传递的情感
一致性锚定 - IMPORTANT: This is panel [N] of [TOTAL] in a storyboard sequence. Maintain exact same art style, color palette, character design, and visual consistency with all other panels. Same character must look identical across all panels.
中文指令（如需）- ALL text, signs, and dialogue in the image MUST be in Chinese (Simplified Chinese). Technical terms may remain in English.
