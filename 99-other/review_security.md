...
url: https://skills.sh/nesnilnehc/ai-cortex/review-security
---

# review-security

skills/nesnilnehc/ai-cortex/review-security
review-security
Installation
...
行为（行为）
该技能的范围
分析：给定代码范围中的安全维度（调用者提供的文件或差异）。不决定范围；接受代码范围作为输入。
不要：执行范围选择、语言/框架约定或架构审查。只关注安全。
审查清单（仅限安全维度）
注入：SQL注入（参数化、原始查询）；命令注入（shell、exec）；模板注入（用户控制的模板）；路径遍历；相关的 LDAP/XML 注入。
敏感数据和日志记录：日志或错误消息中的秘密、令牌或 PII； URL 或客户端存储中的敏感数据；响应或缓存中的暴露。
身份验证和授权：身份验证缺失或较弱；访问控制损坏（IDOR，权限升级）；会话处理和 CSRF；对每个敏感操作进行权限检查。
依赖项和 CVE：已知的易受攻击的依赖项（版本、建议）；未固定或版本范围过于宽泛；供应链和完整性。
配置和秘密：硬编码的秘密；配置文件或环境中的秘密；安全的默认配置；生产中的功能标志和调试模式。
加密和散列：较弱或已弃用的算法（例如用于安全的 MD5、SHA1）；加密使用不当；密钥管理和存储；密码散列（例如 bcrypt、Argon2）。
语气和参考
