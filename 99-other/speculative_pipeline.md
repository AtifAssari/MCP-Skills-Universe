---
title: speculative-pipeline
url: https://skills.sh/juliusbrussee/cavekit/speculative-pipeline
---

# speculative-pipeline

skills/juliusbrussee/cavekit/speculative-pipeline
speculative-pipeline
Installation
...
Why It Works
Stage 2 starts after a 1.5-hour offset. By then, Stage 1 has produced a meaningful set of partial specs.
Stage 2 generates plans from whatever specs are available. Some plans will be built on incomplete information.
Stage 1 keeps refining specs. When Stage 2 loops back for its next pass, it picks up the newly completed specs and adjusts its plans accordingly.
Stage 3 starts after a 3-hour offset. By then, both specs and plans exist in draft form.
All stages self-correct through iteration. Each pass re-reads the latest upstream artifacts. Mistakes caused by working from partial input are washed out on subsequent passes.
...
Rules of Thumb
Delay >= 1 upstream iteration. The follower should not start until the leader has completed at least one full iteration and committed results.
Delay < 50% of upstream duration. If the delay is longer than half the upstream time, the time savings are marginal.
More follower iterations compensate for shorter delays. If you start the follower early (aggressive delay), give it more iterations to converge.
...
What to Watch
Follower diff sizes per iteration. If the follower's diffs are large on every iteration (not decreasing), it is thrashing -- the delay was too short or the upstream output is too unstable.
Follower convergence rate. The follower should converge within 1-2 iterations of the leader finishing. If it takes many more, the stages may have a hard dependency.
Git commit frequency. Both leader and follower should be committing regularly. If commits stall, the agent may be stuck.
Convergence Signals
...
Increasing the delay (give the leader more time to stabilize)
Reducing follower iterations (let upstream settle first)
Adding a "wait for upstream convergence" gate between stages
