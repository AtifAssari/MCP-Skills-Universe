...
url: https://skills.sh/nabinkhair42/structural-grid-skill/structural-grid
---

# structural-grid

skills/nabinkhair42/structural-grid-skill/structural-grid
structural-grid
Installation
...
Consistent letter-spacing — Use tracking-wide on all section labels and inline labels. Never mix tracking-widest and tracking-wider
Every card hovers — All grid cells get transition-colors hover:bg-white/[0.02] for interactive feedback
CSS Foundation
...
Common Pitfalls
overflow: hidden breaks sticky — Always use overflow-x: clip on the rails container
Grid borders extending past rails — Use .rail-bounded (margin-based) instead of mx-auto max-w-6xl
Orphaned grid items on mobile — Plan item counts around your column counts
Border-left on single-column mobile — Use sm:max-lg:border-l for tablet-only left borders
Section padding on grid parents — Apply padding to grid children, not the container
Rails not reaching page bottom — Ensure .page-rails wraps all content
Missing border-border on dashed dividers — Always include the color class
Anchor links behind sticky navbar — Add scroll-padding-top: 5rem to html
Inconsistent tracking classes — Standardize on tracking-wide
Missing hover states on grid cards — Every grid cell needs hover:bg-white/[0.02]
Both color classes applied at once — Use a conditional, never both simultaneously
4-column tables on mobile — Always wrap in overflow-x-auto with min-w-[600px]
Dashed lines can't reach rails — Use calc(var(--rail-offset) - Xrem) margins
Auth route groups — Use Next.js (auth) route group with shared layout.tsx
Component organization — UI primitives in components/ui/, page-specific in components/{section}/
Weekly Installs
13
Repository
nabinkhair42/st…id-skill
GitHub Stars
4
First Seen
Feb 18, 2026
