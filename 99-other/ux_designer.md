---
title: ux-designer
url: https://skills.sh/gabeosx/agent-skills/ux-designer
---

# ux-designer

skills/gabeosx/agent-skills/ux-designer
ux-designer
Installation
$ npx skills add https://github.com/gabeosx/agent-skills --skill ux-designer
SKILL.md
UX Designer
...
No Native Browser UI: Never use native alert() or confirm(). Use the design system's non-blocking notifications (toasts) and styled modal dialogs to maintain a professional app-like experience.
Safe Destructive Actions: High-risk actions (e.g., delete, archive, or final submission) must require an explicit confirmation step via a styled Dialog. Use high-contrast "Danger" styling (e.g., red buttons) to visually signal the risk level to the user.
...
Preserve Semantics: Use the native <select> or <input> element as the interactive base.
Reset Appearance: Apply appearance: none; to strip browser-specific UI.
Layer Visuals: Use a wrapper with position: relative to place custom icons (like chevrons) over the control. Apply pointer-events: none; to these overlays.
Match Standards: Apply the Control Height Standards (32/40/48px) directly to the native element to ensure layout alignment.
Prototyping Strategy

When building rapid prototypes (HTML/CSS):

Link the Core Assets: Include design-system-core.css from the assets/ directory.
Note: This includes a global box-sizing: border-box reset. Do not add your own resets.
Create a Local Stylesheet: Create a style.css to map the CSS variables (e.g., var(--space-4)) to your component classes.
Avoid Inline Styles: Keep logic in CSS classes to maintain the "system" approach.
3. Required Inputs
...
Check Computed Styles: Confirm the source of the override.
Enforce Utility: Use the ! modifier (e.g., !text-white) to force the utility style.
Fix the System: Ideally, adjust the global reset to have lower specificity (e.g. using :where()) so utilities always win.
Weekly Installs
39
Repository
