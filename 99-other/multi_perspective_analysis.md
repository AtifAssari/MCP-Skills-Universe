...
url: https://skills.sh/yunshu0909/yunshu_skillshub/multi-perspective-analysis
---

# multi-perspective-analysis

skills/yunshu0909/yunshu_skillshub/multi-perspective-analysis
multi-perspective-analysis
Installation
$ npx skills add https://github.com/yunshu0909/yunshu_skillshub --skill multi-perspective-analysis
SKILL.md
多视角深度分析
...
核心原则
独立性是生命线：每个 agent 必须完全独立分析，不能看到其他 agent 的输出。这是多视角分析的全部价值所在
必须并行：所有 agent 在同一条消息中启动，不要串行。串行会导致后面的 agent 受前面的影响
深度优先：宁可每个视角写 3000 字的深度报告，也不要 5 个 500 字的浅层总结
必须有锐度：每个 agent 都要给出"最刺耳的一句话"。礼貌但无用的分析不如不做
汇总要有主见：交叉汇总不是简单罗列，要分析共识为什么形成、分歧为什么产生
用户决策权：分析是工具，决策是用户的。报告最后给建议，不做决定
Weekly Installs
18
Repository
