...
url: https://skills.sh/dbt-labs/dbt-agent-skills/migrating-dbt-core-to-fusion
---

# migrating-dbt-core-to-fusion

skills/dbt-labs/dbt-agent-skills/migrating-dbt-core-to-fusion
migrating-dbt-core-to-fusion
Installation
...
Step 0: Ask whether to run dbt debug
Step 1: Run or confirm dbt-autofix, then review its changes
Step 2: Classify remaining issues
...
Ask user: "Have you run dbt-autofix on this project yet?"
Check git history for recent autofix-related commits
Check for autofix log files
If NOT run yet:
...
Identify it as blocked
Explain why (Fusion engine gap, known bug, etc.)
Link the GitHub issue if one exists
Suggest alternative approaches while clearly describing the risks (e.g., workarounds may be fragile, may break on next Fusion update, may have semantic differences)
Let the user decide whether to apply a workaround or wait for the Fusion fix
...
Static Analysis (Highest Confidence): Error code < 1000 (e.g., dbt0209, dbt0404) — Category A or B
Known User-Fixable Patterns: Match against Category A and B patterns above
Fusion Engine Gaps (Need GitHub Check): If error suggests a Fusion limitation (MiniJinja, parser, missing features), search site:github.com/dbt-labs/dbt-fusion/issues <error_code> <keywords> — Category D if open issue with no workaround
Unknown: No pattern match, needs investigation
...
Category A: Get confirmation, apply automatically, validate
Check: Did autofix already attempt this? Don't duplicate
Category B: Show diff for ONE fix at a time, get approval, apply, validate
Check: Does this conflict with autofix changes?
Category C: Present options, wait for user decision, apply chosen fix, validate
Consider: Did autofix cause this issue?
Category D: Document the blocker clearly with GitHub links, explain why it's blocked, suggest alternative approaches while describing the risks, and let the user decide whether to apply a workaround or wait for the Fusion fix.
