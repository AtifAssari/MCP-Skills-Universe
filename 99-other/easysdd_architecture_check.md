---
title: easysdd-architecture-check
url: https://skills.sh/liuzhengdongfortest/easysdd/easysdd-architecture-check
---

# easysdd-architecture-check

skills/liuzhengdongfortest/easysdd/easysdd-architecture-check
easysdd-architecture-check
Installation
$ npx skills add https://github.com/liuzhengdongfortest/easysdd --skill easysdd-architecture-check
SKILL.md
easysdd-architecture-check
...
术语一致性——多份文档对同一概念的称呼是否统一，有没有同义词漂移或同名异义
模块边界一致性——A 文档说某职责归模块 X，B 文档里是不是也这么说；有没有两份文档都声称自己拥有同一块职责
跨文引用有效性——文档里 see xxx.md / 定义见 yyy.md 这类引用，目标文件和目标小节真的存在吗
接口/契约对齐——多份文档涉及同一接口/类型时，签名、字段、语义是否一致
依赖关系闭环——A 文档声明依赖 B 提供的能力，B 文档里真的暴露了该能力吗；有没有单向悬空依赖
同类聚合与命名——同 type 文档是否遵循 {type}-{slug}.md，根目录某 type 已 ≥6 份是否还在平铺（参照 easysdd/reference/shared-conventions.md）
Phase 4：输出检查报告
...
单目标——一次只能锁定 design-internal / design-vs-code / architecture-folder-internal 三者之一
