---
title: ux-antipatterns
url: https://skills.sh/cassiozen/ux-antipatterns/ux-antipatterns
---

# ux-antipatterns

skills/cassiozen/ux-antipatterns/ux-antipatterns
ux-antipatterns
Installation
...
Workflow
Read references/antipatterns.md to load the full detection heuristics.
Scan the code under review against each applicable anti-pattern category.
Report findings grouped by anti-pattern, citing specific file:line locations.
For each finding, state: the anti-pattern name, the user harm, and a concrete fix.
If no anti-patterns are found, state that the code is clean rather than manufacturing findings.
