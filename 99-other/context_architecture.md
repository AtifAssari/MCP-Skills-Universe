...
url: https://skills.sh/juliusbrussee/cavekit/context-architecture
---

# context-architecture

skills/juliusbrussee/cavekit/context-architecture
context-architecture
Installation
...
Look in context/plans/
If not found, fall back to context/sites/
If found in sites/, use it — no auto-migration, no breakage
...
project/CLAUDE.md — project-level conventions
project/src/CLAUDE.md — source code conventions
project/src/auth/CLAUDE.md — "implements cavekit-auth.md R1-R3"
...
How Agents Navigate
Enter at root — read context/CLAUDE.md to understand the 4 tiers
Select tier — based on current task, navigate to the relevant tier's CLAUDE.md
Read index — the tier's overview file is the DAG hub, listing all domains with one-line summaries
Follow edges — read only the domain files relevant to the current task
Cross-reference — if a domain references another, follow that edge only if needed
Nest deeper — if a domain has subdirectories, its root file is the sub-index; spider from there
Index File Format
...
Scan all src/*/CLAUDE.md files for references to the changed requirement
Flag those modules as potentially affected
New requirements with no source-tree CLAUDE.md references are unimplemented
Bootstrapping

Run /ck:init to create the full hierarchy. It:

Scans existing project structure
Creates context directories (refs/, kits/, plans/, impl/)
Creates CLAUDE.md files using standard templates
Creates empty index files (cavekit-overview.md, plan-overview.md, impl-overview.md)
Offers migration if legacy context/sites/ exists
