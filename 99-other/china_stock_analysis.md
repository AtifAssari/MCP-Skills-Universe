...
Automated financial anomaly detection flags suspicious patterns in receivables, cash flow divergence, inventory buildup, margin shifts, and shareholder transactions
Supports three analysis depths: summary (key metrics + conclusion), standard (full financial + valuation + industry comparison + risks), and deep (5-year historical tracking)
Requires akshare Python library for fetching public financial data; outputs structured JSON and markdown reports in Chinese with safety margin calculations and risk ratings
SKILL.md
