...
url: https://skills.sh/bahayonghang/academic-writing-skills/latex-thesis-zh
---

# latex-thesis-zh

skills/bahayonghang/academic-writing-skills/latex-thesis-zh
latex-thesis-zh
Installation
...
Reviews experiment chapters for language and discussion layering; optimizes titles and detects AI-writing traces in visible prose
Preserves all LaTeX citations, references, labels, math environments, and template macros while returning findings in thesis-review format
SKILL.md
LaTeX 中文学位论文助手
...
Workflow
Parse $ARGUMENTS，先锁定入口文件，再根据用户诉求推断模块；若缺参数，只追问缺失项。
若请求同时覆盖多个兼容模块，按“路由规则”中的顺序串行执行，并分模块回报结果。
Read the one reference file tied to that module (see "Read next" column).
Run the corresponding script with uv run python ....
Return findings as % Module (L##) [Severity] [Priority]: .... Report exact command and exit code on failure.
If template and structure are both unclear, run template first, then structure.
Safety Boundaries
