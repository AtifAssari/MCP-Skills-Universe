---
title: easysdd-issue-fix
url: https://skills.sh/liuzhengdongfortest/easysdd/easysdd-issue-fix
---

# easysdd-issue-fix

skills/liuzhengdongfortest/easysdd/easysdd-issue-fix
easysdd-issue-fix
Installation
...
根因分析存在且方案已确认——读 {slug}-analysis.md，确认 frontmatter 有 doc_type=issue-analysis 且 status=confirmed，第 5 节"修复方案"用户选定了哪个方案
把上下文读全：
根因分析全文
问题报告全文
根因分析第 1 节定位到的所有代码文件
...
向用户明确陈述根因："{文件}:{行号} 的 {具体代码} 存在 {问题描述}"，让用户确认根因判断准确
给出修复方案——改哪里、怎么改（一两句话即可，不写成完整分析文档）
等用户明确说"对，就这样改"后才动手——不允许"我觉得对，直接改了"
读 AGENTS.md
补搜沉淀目录——快速通道也要用 search-yaml.py --dir easysdd/compound 查一遍，--filter doc_type=trick 看同类技巧，--filter doc_type=explore 避免误把已知边界条件当成新问题
实现期间的约束
只改根因分析里声明的文件
...
最后补问一次是否需要代为提交本次修复。用户同意时按收尾提交规则执行到 commit 完成。
