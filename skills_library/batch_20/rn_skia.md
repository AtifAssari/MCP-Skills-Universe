---
title: rn-skia
url: https://skills.sh/imfa-solutions/skills/rn-skia
---

# rn-skia

skills/imfa-solutions/skills/rn-skia
rn-skia
Installation
...
Critical Rules
Never read .value in JSX render — pass shared values directly as props: <Circle cx={x} /> not <Circle cx={x.value} />.
Never create Skia objects in render — use useMemo for Paint, Path, PictureRecorder, RuntimeEffect.
Always mark worklet functions with "worklet" directive in useDerivedValue callbacks.
Animate properties, not structure — keep the component tree static; only animate numeric/color props.
...
Load fonts/images at component level with useFont/useImage — handle null loading state before rendering.
...
Scan all files importing @shopify/react-native-skia.
Check against Critical Rules and Anti-Patterns table.
Report in table: | File | Line | Issue | Severity | Fix |
Severity: CRITICAL (crash/broken render), HIGH (frame drops), MEDIUM (sub-optimal), LOW (style).
Fix all issues if confirmed, then re-scan to verify zero violations.
Reference Files
