---
title: paw-mkt-dashboard
url: https://skills.sh/pawbytes/skill-suites/paw-mkt-dashboard
---

# paw-mkt-dashboard

skills/pawbytes/skill-suites/paw-mkt-dashboard
paw-mkt-dashboard
Installation
...
Ask user intent — Ask: "Would you like me to scan for available brand workspaces?" Only proceed with discovery after confirmation.

Check for existing dashboards — For each brand, check if {brand-path}/dashboard/package.json exists.

Detect feature gaps — For existing dashboards, scan src/routes/ to identify which features from the Feature Registry are implemented. Compare against the full registry to identify gaps.

Present a contextual summary with:
...
Offer contextual actions based on what exists:
...
Start the dev server: npm run dev (or npm run dev -- --port {port} if conflicts)
Present the URL to open in browser
Stay available for follow-up (add features, fix issues, export data)
If user wants to update/regenerate:
Load references — Read ./references/workflow.md, ./references/code-patterns.md, and ./references/design-guide.md
Export existing data first — If dashboard.db exists, export to JSON before touching anything
Analyze data structure — Rescan brand folders for new/changed data
Present change plan — Show what will be updated (new routes, changed components, data re-imports) and ask: "Ready to proceed with regeneration?"
Regenerate after approval — Update routes and components, import data back
Present summary — What changed, how to run
If user wants to add missing features or specific features:
Load references — Read ./references/code-patterns.md and ./references/design-guide.md
...
Run migration if needed — If schema changed, export data, update schema, re-seed
Present summary — What was added, how to verify
If user wants full generation (new dashboard):
Load references — Read ./references/workflow.md, ./references/code-patterns.md, and ./references/design-guide.md
Present generation plan — Summarize which features, routes, and data sources will be included. Ask: "Does this plan look right before I generate?"
Execute from Stage 2 after approval — Schema design → scaffolding → route generation → summary
