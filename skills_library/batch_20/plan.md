---
title: plan
url: https://skills.sh/tstelzer/skills/plan
---

# plan

skills/tstelzer/skills/plan
plan
Installation
$ npx skills add https://github.com/tstelzer/skills --skill plan
SKILL.md
Plan
Overview
...
Workflow
Inspect the repository before drafting. Read relevant source files, tests, docs, configs, and local guidance such as AGENTS.md when present.
Identify the current behavior, affected surfaces, exact files to touch, invariants, dependencies, test strategy, and verification commands.
Resolve unknowns from repo context where possible. Put remaining decisions only in Open Questions, and block dependent tasks explicitly.
Draft the saved standalone plan with tasks ordered by dependency and each task independently verifiable.
Before finalizing, run a fresh-reader pass for standalone context, concrete tasks, explicit tests, exact files, and no answered questions left behind.
Plan Structure
# Plan: <Feature Name>
