---
title: axiom-swiftui-debugging
url: https://skills.sh/charleswiltgen/axiom/axiom-swiftui-debugging
---

# axiom-swiftui-debugging

skills/charleswiltgen/axiom/axiom-swiftui-debugging
axiom-swiftui-debugging
Installation
...
Don't pass it back: Remove the mutated state from init params — let the callback update parent without flowing it back
Separate state: Use a different @State for the child's display logic vs the parent's cache
Child-owned lookup: Child queries its own data source instead of receiving parent state
...
Restart Preview Canvas: Cmd+Option+P
Restart Xcode completely (File → Close Window, then reopen project)
Nuke derived data: rm -rf ~/Library/Developer/Xcode/DerivedData
Rebuild: Cmd+B
...
Restart Preview Canvas: Cmd+Option+P (30 seconds)
Restart Xcode (2 minutes)
Nuke derived data: rm -rf ~/Library/Developer/Xcode/DerivedData (30 seconds)
Rebuild: Cmd+B (2 minutes)
Still broken? Use the dependency or initialization decision trees above
