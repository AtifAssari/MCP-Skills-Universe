...
url: https://skills.sh/kimny1143/claude-code-template/ios-app-store-submission
---

# ios-app-store-submission

skills/kimny1143/claude-code-template/ios-app-store-submission
ios-app-store-submission
Installation
...
App Store Connect → アプリ → TestFlight タブ
ビルドが「終了」（処理完了）であることを確認
Internal Testers グループを確認
自動追加されていれば即テスト可能
...
Diagnose ボタンを削除（またはタップ回数イースターエッグ化）
apiClient.ts 等の console.log('[API] Token obtained...') 等のデバッグログを削除
デバッグ用の一時UIコンポーネント（バナー、ステータス表示等）を削除
これらを確認してからビルド → 審査提出する。デバッグUIが残ったまま審査提出しない
Phase 4: ビルド前検証
