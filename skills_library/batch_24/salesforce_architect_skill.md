---
title: salesforce-architect-skill
url: https://skills.sh/salesforcediariesbysanket/copilot-skills-salesforce/salesforce-architect-skill
---

# salesforce-architect-skill

skills/salesforcediariesbysanket/copilot-skills-salesforce/salesforce-architect-skill
salesforce-architect-skill
Installation
$ npx skills add https://github.com/salesforcediariesbysanket/copilot-skills-salesforce --skill salesforce-architect-skill
SKILL.md
...
Data model design, object relationships, LDV, data skew, sharing model, junction objects, Big Objects	references/data-model-patterns.md
Integration patterns, API selection, event-driven architecture, middleware, Pub/Sub API, Data 360, batch sync	references/integration-patterns.md
Well-Architected reviews, security validation, performance, compliance, CI/CD, testing strategy	references/well-architected-checklist.md
...
Developer Skill: ../salesforce-developer/SKILL.md — Apex patterns, LWC guide, SOQL optimization, deployment, Agentforce
...
Mandatory Rules
Well-Architected First — Every design must align with the Salesforce Well-Architected pillars: Trusted, Easy, Adaptable (in that priority order).
Standard Before Custom — Recommend declarative solutions (Flows, formulas, validation rules) before custom Apex or external tools.
Security by Design — OWD set to most restrictive, permission sets over profiles, FLS on sensitive fields, OAuth for all API integrations.
Design for Scale — Account for LDV (>1M records), data skew, and API limits from day one.
One Trigger Per Object — Layered architecture: Trigger → Handler → Service → Selector.
Integration Documentation — Every integration must document: data ownership, direction, timing, volume, error handling, auth, rate limits, idempotency, rollback.
Environment Strategy — Define Dev → QA → UAT → Staging → Production with source control and CI/CD.
Latest GA API Version — All new metadata must use the latest GA API version (currently 66.0). Update when Salesforce releases a new version.
...
Trusted — Is the solution secure, compliant, and reliable?
Easy — Does the solution deliver value fast, and is it intentional, engaging, and maintainable?
Adaptable — Can the solution evolve with the business, and is it resilient and composable?
...
Platform Fundamentals — Understand multi-tenant constraints (governor limits, order of execution, metadata vs data)
Well-Architected Assessment — Evaluate against Trusted/Easy/Adaptable
Decision Guide Selection — Use the appropriate decision guide for specific design choices
Integration Pattern Selection — Choose integration patterns based on data ownership, timing, and volume
Data Model Design — Design objects, relationships, and sharing rules
Security Architecture — Layer authentication, authorization, encryption, and field-level security
Automation Strategy — Select the right tool (Flow, Apex, Platform Events) for each use case
...
When: The business needs something to happen when a record changes. Key decisions:
...
When: Users need to input data. Options (from simplest to most flexible):
...
When: Data needs to move between Salesforce and external systems. First question: Should the data be stored in Salesforce?
...
When: You need loosely coupled, real-time communication between systems. Tools: Platform Events, Change Data Capture (CDC), Pub/Sub API, AWS Event Relay When to use EDA: decoupled services, messaging between orgs, real-time event responses Anti-patterns: unnecessarily large payloads, using real-time platform events for batch scenarios

5. Migrating Changes Between Environments (ALM/DevOps)

Spectrum from simplest to most mature:

Change Sets (small teams, simple changes)
Salesforce CLI + Metadata API (intermediate)
DevOps Center (Salesforce-native CI/CD)
Full CI/CD with scratch orgs + unlocked packages (enterprise scale)
...
This is critical for understanding data integrity. The platform processes record saves in a specific order: load record → field values overwritten → system validation (required fields, formats) → before-save record-triggered flows → before triggers → system validation + custom validation rules → duplicate rules → record saved to DB (not committed) → after triggers → assignment/auto-response rules → workflow rules → after-save record-triggered flows → roll-up summary → criteria-based sharing → DML committed → post-commit logic (email, async Apex).
...
Executive Summary (business context, goals, success criteria)
Current State Assessment (existing systems, pain points, technical debt)
Proposed Architecture (reference architecture diagram Level 1-2)
Data Model (ERD at Level 4)
Integration Architecture (patterns selected, sequence diagrams at Level 3)
Security Architecture (authentication, authorization, encryption, sharing model)
Automation Design (decision guide rationale for each automation)
DevOps Strategy (ALM approach, environments, release cadence)
...
Developer skill (Apex, LWC, SOQL patterns) → ../salesforce-developer/SKILL.md
Data model, sharing, LDV, data skew → references/data-model-patterns.md
Integration patterns, API selection, event-driven → references/integration-patterns.md
Well-Architected checklist, security, compliance → references/well-architected-checklist.md
Weekly Installs
37
Repository
salesforcediari…lesforce
GitHub Stars
25
First Seen
Feb 24, 2026
