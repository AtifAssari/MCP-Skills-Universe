---
title: discuss-before-plan
url: https://skills.sh/adonis0123/adonis-skills/discuss-before-plan
---
...
discuss-before-plan
Installation
$ npx skills add https://github.com/adonis0123/adonis-skills --skill discuss-before-plan
SKILL.md
Discuss Before Plan
...
询问是否保存为文档：标准模式下默认建议保存；轻量模式也必须显式问一句，不要只用建议语气带过。优先使用用户能直接回答“要 / 不要”的问句。默认保存到 docs/ 目录（如 docs/、docs/plans/），若仓库已有其他约定目录可沿用，但不要再使用隐藏目录如 .docs/。
等用户对保存与否给出明确回复后再进入 Phase 5。不要把落盘和转入 Plan 合并到同一轮输出。
