---
title: plot-sprint
url: https://skills.sh/eins78/skills/plot-sprint
---

# plot-sprint

skills/eins78/skills/plot-sprint
plot-sprint
Installation
...
Compare current HEAD SHA of the PR branch to the review_sha in the annotation
If same → no new commits since last review, skip
If different → new commits, needs re-review
If status is merged → never needs review
...
Close anyway (must-haves stay unchecked in place)
Move incomplete must-haves to Deferred — move each unchecked - [ ] line from ### Must Have to ### Deferred, preserving the original text
Hold off (don't close yet)
...
Review the retrospective action items
Carry deferred items to the next sprint: /plot-sprint <new-slug>: <goal>
If all planned work is delivered: /plot-release to cut a release
Scope Change
