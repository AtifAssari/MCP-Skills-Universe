---
title: peer-review
url: https://skills.sh/juliusbrussee/cavekit/peer-review
---

# peer-review

skills/juliusbrussee/cavekit/peer-review
peer-review
Installation
...
Builder agent implements the feature and commits
Generate a diff of all changes: git diff {BASE_BRANCH}...HEAD
Send the diff to the peer review model with a code review prompt
Parse the peer reviewer's findings into actionable items
Builder agent applies fixes for valid findings
Optionally: send fixes back to peer reviewer for re-review
...
Builder agent drafts an architecture or plan
Send the plan + kits to the peer review model
Peer reviewer proposes alternative approaches or critiques the plan
Builder validates both approaches against kits
Human makes the final decision if there is a genuine trade-off
...
Start a session with the peer review model presenting the problem
Use reply-to-session to continue the conversation across multiple turns
Maintain the thread ID throughout the discussion
Summarize conclusions when the discussion converges
...
Team lead spawns a teammate specifically for peer review coordination
The teammate owns the peer reviewer MCP interaction
Teammate manages multi-turn review sessions
Teammate summarizes findings and reports to the team lead
Team lead assigns fixes to the appropriate builder teammates
...
Present both perspectives to the peer review model
Ask it to evaluate the trade-offs of each approach
Ask it to recommend one, with explicit reasoning
Use the recommendation to inform the decision (human has final say)
...
Run test coverage analysis on the codebase
Generate a coverage report (which files/functions are covered)
Send the coverage report + kits to the peer review model
Peer reviewer identifies: untested edge cases, missing integration tests, cavekit requirements without corresponding tests
Builder adds missing tests
...
Codex CLI delegation (primary) — scripts/codex-review.sh calls codex directly in --approval-mode full-auto with a structured review prompt. Faster, no MCP server overhead. Findings parsed and appended to context/impl/impl-review-findings.md.
MCP server (legacy fallback) — Codex configured as an MCP server in .mcp.json. Claude calls the MCP tool on review iterations. Used only when Codex CLI delegation is unavailable.
...
Validates Codex CLI is installed (or MCP fallback is configured).
Configures Codex as an MCP server in .mcp.json if CLI delegation is unavailable.
Builds a Ralph Loop prompt that embeds:
...
Starts the Ralph Loop via the stop hook mechanism.
...
OpenAI API key configured: Codex needs authentication (via codex login or env var).
Cavekit context directory: Cavekit file must exist at the given path.
