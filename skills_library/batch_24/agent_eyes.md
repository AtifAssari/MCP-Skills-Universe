---
title: agent-eyes
url: https://skills.sh/mrxyy/agents-eyes/agent-eyes
---

# agent-eyes

skills/mrxyy/agents-eyes/agent-eyes
agent-eyes
Installation
...
Apply Order
If @agent-eyes/agent-eyes is missing, install it first.
If installation was performed in this run, help the user finish minimal plugin configuration for their bundler.
If AGENTS.md is missing or lacks the Agent Eyes rule, create or update it.
If the task is element-anchored/ambiguous, request GET /context/selected before editing code.
If the response is data: null, the agent MUST continue with the default workflow and skip selected-context injection only.
The agent MUST NOT block execution or require element selection as a prerequisite; it may optionally suggest re-selection only when precision risk is high.
Quick Workflow
Check whether @agent-eyes/agent-eyes is installed.
If missing, help install and provide minimal setup guidance.
...
Check whether the project already has AGENTS.md.
If missing, create it. If present, append or refine the Agent Eyes workflow rule.
...
Determine the project path from explicit user project path first; otherwise use the current workspace path.
Resolve the git root from that path.
Read only <gitRoot>/.code-inspector/record.json.
Run resolvePreferredProjectEntry(Object.keys(recordJson), projectPath) to get the single matched project directory.
Probe only the matched project directory port first, then other ports from the same record.json, then fallback 5678.
