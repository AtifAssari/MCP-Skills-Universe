---
title: dg
url: https://skills.sh/v1r3n/dinesh-gilfoyle/dg
---

# dg

skills/v1r3n/dinesh-gilfoyle/dg
dg
Installation
...
Dispatch Gilfoyle agent (Agent tool, general-purpose) with:
...
Display Gilfoyle's banter to the user.

Check convergence: Parse Gilfoyle's FINDINGS section. If all findings are repeats from previous rounds → converge.

Dispatch Dinesh agent (Agent tool, general-purpose) with:
...
Display Dinesh's banter to the user.

Check convergence: Parse Dinesh's FINDINGS section. If every point is [concede] with zero [defend] or [dismiss] → converge.

Append both responses to debate_history, increment round.

If round >= cap: Ask user: "These two could go all night. Continue for more rounds? (y/N)"
...
Read the comic template from comic-template.html in this skill's directory.
...
Select the best moments for panels. Prioritize:
...
Write the filled HTML to a timestamped file: /tmp/dg-comic-{timestamp}.html
...
Upload the image and comment on the PR:
...
Tell the user the comment was posted with a link to the PR.
Key Principles
