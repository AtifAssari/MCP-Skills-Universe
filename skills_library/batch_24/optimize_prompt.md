...
url: https://skills.sh/orq-ai/assistant-plugins/optimize-prompt
---

# optimize-prompt

skills/orq-ai/assistant-plugins/optimize-prompt
optimize-prompt
Installation
$ npx skills add https://github.com/orq-ai/assistant-plugins --skill optimize-prompt
SKILL.md
Optimize Prompt
...
Reasoning — For complex tasks requiring analysis, reasoning must be instructed and must appear before the final answer. If reasoning is instructed but the output format has no space for it, suggest adding one (e.g., a reasoning key in JSON)
Examples — Few-shot examples using <example> XML tags to demonstrate desired behavior, with proper variable formatting inside
Remove unnecessary content — No unnecessary markdown, emojis, XML tags, or contradictions
Proper variable usage — Variables with {{double curly brackets}} should only appear once near the end; earlier references should use XML tags
Recap — A one-sentence recap of the task and format at the end of the prompt
...
orq MCP tools — query live data first (search_entities, get_agent); API responses are always authoritative
