...
url: https://skills.sh/daymade/claude-code-skills/youtube-downloader
---

# youtube-downloader

skills/daymade/claude-code-skills/youtube-downloader
youtube-downloader
Originally fromcomposiohq/awesome-claude-skills
Installation
...
Quote URLs in shell commands (zsh treats ? as a glob). Example: 'https://www.youtube.com/watch?v=VIDEO_ID'.
Ensure proxy is active for both yt-dlp and PO Token providers (HTTP_PROXY/HTTPS_PROXY/ALL_PROXY).
...
If cookies are in use, prefer the web_safari player client. Otherwise prefer mweb for PO tokens.
Keep the browser window open while WPC is minting tokens. Ensure Chrome can reach YouTube through the same proxy.
...
Install PO token provider (see Step 1 above)
...
Request permission and use browser cookies (--cookies-from-browser chrome).
Ensure the browser and yt-dlp use the same IP/proxy.
Retry with web_safari client if needed.
...
If cookies are in use, prefer web_safari client and retry.
...
If still failing and PO tokens are disabled, use Android client: yt-dlp --extractor-args "youtube:player_client=android" "VIDEO_URL"
