---
title: kotlin-tooling-agp9-migration
url: https://skills.sh/kotlin/kotlin-agent-skills/kotlin-tooling-agp9-migration
---

# kotlin-tooling-agp9-migration

skills/kotlin/kotlin-agent-skills/kotlin-tooling-agp9-migration
kotlin-tooling-agp9-migration
Installation
...
Check if the project uses a Gradle version catalog (gradle/libs.versions.toml). If it exists, read it for current AGP/Gradle/Kotlin versions. If not, find versions directly in build.gradle.kts files (typically in the root buildscript {} or plugins {} block). Adapt all examples in this guide accordingly — version catalog examples use alias(libs.plugins.xxx) while direct usage uses id("plugin.id") version "x.y.z"
...
Resolve Sub-dependency Variants (Product Flavors / Build Types): Because the new KMP Android library plugin enforces a single-variant architecture, it does not natively understand how to resolve dependencies that publish multiple variants (like debug/release build types, or product flavors like free/paid). Configure fallback behaviors using localDependencySelection:
kotlin {
    android {
