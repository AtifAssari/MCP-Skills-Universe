---
title: ln-741-linter-configurator
url: https://skills.sh/levnikolaevich/claude-code-skills/ln-741-linter-configurator
---

# ln-741-linter-configurator

skills/levnikolaevich/claude-code-skills/ln-741-linter-configurator
ln-741-linter-configurator
Installation
...
Copy eslint_template.ts to project root as eslint.config.ts
Copy prettier_template.json as .prettierrc
...
Copy editorconfig_template.ini as .editorconfig
Copy directory_build_props_template.xml as Directory.Build.props
Ensure analyzers are included (SDK 5+ includes them by default)

Python:

Copy ruff_template.toml as ruff.toml
OR merge into existing pyproject.toml under [tool.ruff]
Copy mypy_template.toml as mypy.toml
OR merge into existing pyproject.toml under [tool.mypy]
Update known-first-party in isort config to match project package name
Update files in mypy config to match project source directories
Generate advanced tool configs in pyproject.toml:
...
Generate .editorconfig from editorconfig_template.ini -- adapt max_line_length to match ruff line-length
...
Copy lint_script_template.sh to scripts/lint.sh
Uncomment ALL check lines matching detected stack -- both basic AND advanced tools:
Stack	Checks	TOTAL
...
Set TOTAL to match active checks count
Uncomment matching auto-fix commands in --fix block
Make executable: chmod +x scripts/lint.sh
For TypeScript: ensure "lint:all" script exists in package.json
...
Run bash scripts/lint.sh --fix to auto-fix what ruff can
Fix remaining errors manually (file by file)
For C901 (complexity) on functions that cannot be refactored now: add # noqa: C901
For deptry false positives: add [tool.deptry.per_rule_ignores] entries
Repeat until bash scripts/lint.sh passes with 0 failures
Critical Rules
