---
title: github-commit-recovery
url: https://skills.sh/gadievron/raptor/github-commit-recovery
---

# github-commit-recovery

skills/gadievron/raptor/github-commit-recovery
github-commit-recovery
Installation
...
GitHub Archive → Found zero-commit PushEvent with before SHA
GitHub API → GET /repos/istio/istio/commits/{SHA}.patch
TruffleHog → Identified valid GitHub PAT in commit diff
GitHub API → Verified token permissions via /user endpoint
High-Value Secret Categories

From scanning recovered force-pushed commits, the most impactful secrets found in order:

GitHub PATs - Often have org-wide or admin permissions
AWS Credentials - IAM keys with production access
MongoDB Connection Strings - Direct database access
API Keys - Stripe, Twilio, SendGrid with billing access
