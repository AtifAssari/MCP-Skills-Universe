---
title: vtable-development-assistant
url: https://skills.sh/visactor/vtable/vtable-development-assistant
---

# vtable-development-assistant

skills/visactor/vtable/vtable-development-assistant
vtable-development-assistant
Installation
...
表格类型（ListTable / PivotTable / PivotChart）— 影响配置结构和查询路径
数据结构或样例（records 的字段和格式）— 影响列定义和字段映射
...
先用关键词命中路由表，按顺序加载对应文件（优先 knowledge，再补 type）
只加载解决当前问题必需的 1-3 个文件，不要一次性加载全部 references
若用户要“可运行 demo/完整配置”，再补充加载 references/examples/ 中最接近的示例
用户意图关键词	查询文件
...
代码生成规范
始终使用 TypeScript，import 来源为 '@visactor/vtable'
表格实例化必须指定 container（HTMLElement）
列定义使用 ColumnsDefine 类型（ListTable）或 rows/columns/indicators（PivotTable）
样式属性支持两种形式：静态值 和 (arg: StylePropertyFunctionArg) => value 回调函数
事件监听使用 tableInstance.on('event_name', handler)
销毁表格必须调用 tableInstance.release()
透视表数据分析需要配置 dataConfig 中的 aggregationRules
自定义布局优先推荐 JSX 方案（customLayout），低级需求用 customRender
脚本生成强制规则
...
反模式（必须避免）
不要在同一个 container 上重复 new 表格实例而不 release（会导致内存与事件泄漏）
不要在 style 回调或 customRender 等函数中做重计算/创建大量对象（会拖慢滚动与交互）
不要在事件回调里递归触发布局/重绘类 API（容易造成卡顿或连锁更新）
Weekly Installs
32
Repository
visactor/vtable
