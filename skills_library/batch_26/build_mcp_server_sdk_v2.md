---
title: build-mcp-server-sdk-v2
url: https://skills.sh/yigitkonur/skills-by-yigitkonur/build-mcp-server-sdk-v2
---

# build-mcp-server-sdk-v2

skills/yigitkonur/skills-by-yigitkonur/build-mcp-server-sdk-v2
build-mcp-server-sdk-v2
Installation
...
What does the server wrap? (API, database, filesystem, CLI)
Transport? stdio for local, Streamable HTTP for remote
Framework? Express or Hono (both have dedicated adapters)
Auth? Client-side only in v2 (server-side OAuth removed)
...
5 — Build or extend
Create McpServer with name, version, optional description/icons
Define Zod v4 schemas: z.object({ field: z.string() }) (not raw shapes)
Register tools with server.registerTool() — input schema, annotations, handler with (args, ctx) pattern
Register resources with server.registerResource() if exposing data
Register prompts with server.registerPrompt() if providing templates
