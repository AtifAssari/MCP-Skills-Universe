---
title: red-team
url: https://skills.sh/boshu2/agentops/red-team
---

# red-team

skills/boshu2/agentops/red-team
red-team
Installation
...
--personas-file=<path> → load custom personas from YAML
.agents/red-team/personas/*.yaml → load project-specific personas
Built-in defaults from council red-team preset (see references/persona-format.md)
...
--scenarios-file=<path> → load custom scenarios
.agents/red-team/scenarios/*.yaml → load project-specific scenarios
Auto-generate from target surface
...
Detects docs surface
Loads 3 default personas (panicked-sre, junior-engineer, first-time-consumer)
Auto-generates 4-6 scenarios per persona (discoverability, completeness, copy-paste, jargon)
Spawns 3 agents in parallel, each constrained to docs/ and README.md
Collects findings, deduplicates cross-persona
Runs /council --preset=red-team --quick for consolidation
Writes report to .agents/red-team/
...
Detects skills surface (SKILL.md exists)
Loads zero-context-agent persona
Auto-generates 3-5 scenarios (step executability, examples, error handling, flags)
Spawns 1 agent restricted to skills/evolve/SKILL.md and skills/evolve/references/
Agent attempts to follow the SKILL.md workflow step-by-step
Collects findings on ambiguities, missing steps, undefined terms
Writes report
...
See Also
skills/council/SKILL.md — Multi-model consensus (used for consolidation)
skills/vibe/SKILL.md — Code quality validation (complementary)
skills/doc/SKILL.md — Doc generation/validation (complementary)
skills/bug-hunt/SKILL.md — Systematic audit (closest cousin)
Reference Documents
references/persona-format.md — Persona YAML schema with context restriction fields
references/scenario-format.md — Scenario YAML schema with pass/fail criteria
references/surface-types.md — Supported surfaces and probing dimensions
references/report-format.md — Report template and verdict rules
Weekly Installs
22
Repository
boshu2/agentops
