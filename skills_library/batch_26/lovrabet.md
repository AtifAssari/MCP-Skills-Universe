---
title: lovrabet
url: https://skills.sh/lovrabet/lovrabet-cli/lovrabet
---

# lovrabet

skills/lovrabet/lovrabet-cli/lovrabet
lovrabet
Installation
$ npx skills add https://github.com/lovrabet/lovrabet-cli --skill lovrabet
SKILL.md
...
结构化输出与 jq：需要机器可读 JSON 时优先 --format compress（单行信封）；需缩进时用 --format json。二者均可叠加全局 --jq '<expr>' 缩小输出；语义与 rabetbase 一致，详见 输出格式与 --jq。
...
User AK：.lovrabet.json 或 LOVRABET_ACCESS_KEY 配 accessKey（适合 CI）；有 accessKey 时走 client-ak
Cookie：历史兼容读取，已不是推荐主路径；新文档与 Agent 指导默认按 AK 流程处理
...
不要把 auth init 当作普通登录命令；它会清空当前作用域下已有配置，再只写回新的认证结果
...
不要在用户未要求时主动修改本地配置或加 --global；优先用显式 --app / --appcode 满足本次操作。
config set / config delete：属于高级本地配置维护命令。无本地配置文件且未传 --global 时，CLI 会拒绝执行，避免静默污染全局配置。
app list：默认走远端优先 + 本地缓存；--local 只读缓存；--no-cache 强制打线上并刷新缓存。
app pull：只刷新本地 app cache，不把远端应用列表写入 .lovrabet.json；它是手动刷新命令，平时优先使用 app list
app use / app import：只操作本地用户意图配置（defaultApp / 顶层 appcode）。如果前一步用 app list --env daily 查到应用，app use 也要带同一个 --env daily，避免按默认环境查缓存。详见 应用管理。
Agent 禁止行为
...
用户只给了业务需求，没有给 app 线索
例如：“帮我查订单数据集”“看一下 CRM 里的客户表”
当前没有显式 --appcode / --app，且需求中有业务域、对象名或数据集线索
当前配置中存在多个候选 app，且需求描述不足以直接锁定一个
...
先判断当前是否已有明确 app 上下文
若有 defaultApp，先 lovrabet dataset list --name <关键词> 验证默认候选
默认候选命中合理时，继续 dataset detail 或 data filter
默认候选无命中或不合理时，再 lovrabet app list
按业务关键词挑 1-2 个候选 app，并执行 dataset list --app <name> --name <关键词> 验证
不要这样做
不要每次都先 app list
...
--jq '<expr>'	对 json / compress 最终打印的 JSON 做 jq 过滤；与 rabetbase 行为一致，详见 lovrabet-output-format-jq.md
--app <name>	多应用模式下指定应用
...
可选本地配置文件 .lovrabet.json（完整字段、优先级、环境变量见 配置参考）：
...
Instant API	instant-api-workflow.md
