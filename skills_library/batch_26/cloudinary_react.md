---
title: cloudinary-react
url: https://skills.sh/cloudinary-devs/skills/cloudinary-react
---

# cloudinary-react

skills/cloudinary-devs/skills/cloudinary-react
cloudinary-react
Installation
$ npx skills add https://github.com/cloudinary-devs/skills --skill cloudinary-react
SKILL.md
...
Setup: Create config file with cld instance (see Project setup section)
Display image: const img = cld.image('id').resize(fill().width(800)); <AdvancedImage cldImg={img} />
Upload Widget: Script in index.html + poll for createUploadWidget in useEffect
Image overlay: Use source(text(...)) or source(image(...)) - see Import reference table for exact paths
Signed uploads: See references/signed-uploads.md
...
Script in index.html (required): Add <script src="https://upload-widget.cloudinary.com/global/all.js" async></script> to index.html. Do not rely only on dynamic script injection from React — it's fragile.
Poll in useEffect (required): In useEffect, poll with setInterval (e.g. every 100ms) until typeof window.cloudinary?.createUploadWidget === 'function'. Only then create the widget. A single check (even in onload) is not reliable because window.cloudinary can exist before createUploadWidget is attached.
Add a timeout: Set a timeout (e.g. 10 seconds) to stop polling and show an error if the script never loads. Clear both interval and timeout in cleanup.
Create widget once: When createUploadWidget is available, create the widget and store it in a ref. Clear the interval and timeout. Pass options: { cloudName, uploadPreset, sources: ['local', 'camera', 'url'], multiple: false }.
Open on click: Attach a click listener to a button that calls widgetRef.current?.open(). Remove the listener in useEffect cleanup.
...
Clear Vite cache: rm -rf node_modules/.vite/ (or delete node_modules/.vite folder)
Restart dev server: npm run dev
Hard refresh browser: Cmd+Shift+R (Mac) or Ctrl+Shift+F5 (Windows/Linux)
If still empty, use a static config workaround (create a cloudinaryConfig.ts with a hardcoded cloud name for dev, then switch back to env later) and see Common Errors → "VITE_ prefix required or env var is undefined"
...
Is VITE_CLOUDINARY_UPLOAD_PRESET set in .env? (must match preset name exactly)
Does the preset exist in the dashboard under Settings → Upload → Upload presets?
Is the preset set to Unsigned (not Signed)?
Was the dev server restarted after adding/updating .env?
...
If an import fails, verify the package version (@cloudinary/url-gen in package.json) and the Cloudinary URL-Gen SDK docs or Transformation Builder reference.
...
responsive() - First (handles breakpoints)
placeholder() - Second (shows placeholder while loading)
lazyload() - Third (delays loading until in viewport)
accessibility() - Last (if needed)
...
Use server/.env in .gitignore for API key/secret
API key can be sent to client; API secret must stay server-only
...
Most Common Issues
Environment variables undefined: Clear node_modules/.vite/, restart dev server, hard refresh browser
"createUploadWidget is not a function": Poll with setInterval until typeof window.cloudinary?.createUploadWidget === 'function'
Wrong imports: Use ONLY paths from Import reference table - don't guess subpaths
...
Overlay issues: Import text/image from qualifiers/source, not actions/overlay
