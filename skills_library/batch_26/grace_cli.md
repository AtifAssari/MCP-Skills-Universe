---
title: grace-cli
url: https://skills.sh/osovv/grace-marketplace/grace-cli
---

# grace-cli

skills/osovv/grace-marketplace/grace-cli
grace-cli
Installation
...
Run grace status when you first need to understand the current project state.
Run grace status --with modules when project-level health is not enough and you need module summaries.
Run grace lint when integrity or drift matters.
Run grace lint --profile autonomous before long autonomous execution.
Run grace lint --explain <code> when one issue needs targeted remediation guidance.
Run grace module find to resolve the target module from the user's words, a stack trace, or a changed path.
Run grace module show, grace module health, and grace verification show for the narrowed shared/public truth.
Run grace file show for the file-local/private truth.
