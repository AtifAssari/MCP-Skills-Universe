---
title: prd-v05-technical-stack-selection
url: https://skills.sh/mattgierhart/prd-driven-context-engineering/prd-v05-technical-stack-selection
---

# prd-v05-technical-stack-selection

skills/mattgierhart/prd-driven-context-engineering/prd-v05-technical-stack-selection
prd-v05-technical-stack-selection
Installation
...
Workflow Overview
Discover Existing Assets → Read SOT or interview user for current tech stack
Categorize Layers → Reuse / Extend / New / Replace for each capability
Evaluate Vendor Fit → Check vendor options for New/Replace layers (if constraints exist)
Evaluate Alternatives → Research non-vendor options where gaps exist
Check Product Family → Shared infrastructure, component reuse, cross-product UX
Create TECH- Entries → Write entries using standard or reuse template
Map to Risks → Cross-reference every TECH- entry with RISK- constraints
Decision Categories
...
"What is the current tech stack?" (ask per area)
...
"Are there vendor constraints (e.g., must use AWS, Azure, or another preferred platform)?"
...
If your team has vendor constraints (AWS-only, Azure-first, etc.), check if the preferred vendor has a product for this capability
Evaluate fit, compliance status, and cost
If vendor product fits → create TECH- entry with Category: Buy
If vendor product has poor fit or gaps → proceed to Step 4
...
Research 2-3 non-vendor alternatives
Score against the same criteria from Step 3
If a clear winner exists → create TECH- entry with decision
If high uncertainty → create TECH- entry with Category: Research, including evaluation criteria and deadline
...
For each RISK- entry, verify at least one TECH- entry addresses it
For each TECH- entry, verify it doesn't introduce new unmitigated risks
Document the mapping in a summary table:
