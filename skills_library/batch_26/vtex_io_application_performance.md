---
title: vtex-io-application-performance
url: https://skills.sh/vtex/skills/vtex-io-application-performance
---

# vtex-io-application-performance

skills/vtex/skills/vtex-io-application-performance
vtex-io-application-performance
Installation
$ npx skills add https://github.com/vtex/skills --skill vtex-io-application-performance
SKILL.md
VTEX IO application performance
...
Layer 2 — VBase — Shared across pods; platform data is partitioned by account / workspace like other IO resources. Pair with hash or trySaveIfhashMatches when the client supports concurrency-safe updates (see Clients).
...
Deduplicate within a request: set ctx.state flags when a resolver chain might call the same loader twice.
Parallelize independent ctx.clients calls in phased Promise.all groups.
...
Reference
VTEX IO Clients — vbase client methods
Implementing cache in GraphQL APIs for IO apps — SEGMENT cache and cookies
