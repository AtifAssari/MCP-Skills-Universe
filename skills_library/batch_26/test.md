---
title: test
url: https://skills.sh/boshu2/agentops/test
---

# test

skills/boshu2/agentops/test
test
Installation
...
Uncovered functions -- exported/public functions with 0% coverage.
Uncovered branches -- functions with partial coverage (conditionals, error paths).
Missing edge cases -- functions that only test the happy path.
...
Generation Rules
Read the source function first. Understand inputs, outputs, error conditions, and branches.
Cover every branch. Each if, switch, error return, and edge case gets at least one test case.
Assert exact expected values. Use == expected, not != nil or != "".
Name tests descriptively. The test name should describe the scenario: "empty input returns error", not "test1".
Test error paths explicitly. Verify error messages or error types, not just that an error occurred.
One assertion focus per test case. Each table row or parametrized case tests one specific behavior.
...
Determine if the test is wrong or the code has a bug.
If the test is wrong: fix the test assertion or setup.
If the code has a bug: report it but fix the test to match current behavior, noting the bug in the output.
Re-run until green.
...
Understand the feature requirement from the user.
...
Write the minimum code to make the test pass. No extra logic, no optimization.
Run the test -- it MUST pass now.
go test -v -run TestNewFeature ./...
# Expected: PASS

Refactor
Clean up the implementation. Remove duplication, improve naming, simplify logic.
Run ALL tests -- everything must still pass.
go test ./...
# Expected: all PASS
...
Inventory existing tests. Count test files, test functions, assertion density.
Classify test types. Unit, integration, end-to-end, benchmark.
Identify structural gaps. Missing test directories, no CI integration, no fixtures.
...
Output to .agents/test/strategy.md.
