---
title: harness
url: https://skills.sh/revfactory/harness/harness
---

# harness

skills/revfactory/harness/harness
harness
Installation
...
에이전트 정의(.claude/agents/)와 스킬(.claude/skills/)을 생성한다.
에이전트 팀을 기본 실행 모드로 사용한다.
...
2-2. 아키텍처 패턴 선택
작업을 전문 영역으로 분해
에이전트 팀 구조 결정 (아키텍처 패턴은 references/agent-design-patterns.md 참조)
파이프라인: 순차 의존 작업
팬아웃/팬인: 병렬 독립 작업
전문가 풀: 상황별 선택 호출
생성-검증: 생성 후 품질 검수
...
Should-trigger 쿼리 (8~10개) — 스킬을 트리거해야 하는 다양한 표현 (공식적/캐주얼, 명시적/암시적)
Should-NOT-trigger 쿼리 (8~10개) — 키워드가 유사하지만 이 스킬이 아닌 다른 도구/스킬이 적합한 "near-miss" 쿼리
