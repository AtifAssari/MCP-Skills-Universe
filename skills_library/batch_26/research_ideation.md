...
url: https://skills.sh/evoscientist/evoskills/research-ideation
---

# research-ideation

skills/evoscientist/evoskills/research-ideation
research-ideation
Installation
$ npx skills add https://github.com/evoscientist/evoskills --skill research-ideation
SKILL.md
Research Ideation
...
Read M_I at /memory/ideation-memory.md (refer to evo-memory skill)
...
Process
Analyze literature + challenge-insight tree → identify 3 fundamentally different research directions
Generate one idea per direction using Innovator persona
Each idea must follow one of two methodological paths:
...
Enhancement through Grounding: Strengthen with literature citations
Improving Coherence: Fix logical flaws in the mechanism
Inspiration and Combination: Combine with a different concept from literature
Simplification: Strip down to a clean, testable hypothesis
Literature-Driven Pivot: Abandon the mechanism; propose a new approach from literature
...
Save the top-3 directions to /direction-summary.md
Trigger IDE protocol via evo-memory skill with the direction summary
Each top direction is added to M_I as a feasible direction with its ELO score
Any ideas that were clearly unworkable during refinement (Step 4) are recorded as unsuccessful directions with failure classification (fundamental vs implementation)
...
Identify the field from the research goal and literature
Start with universal sections (Abstract, Problem, Related Work, Method, Evaluation, Conclusion)
Add field-specific sections (e.g., Ethics/IRB for medical research, Safety analysis for chemistry, Statistical power analysis for clinical trials, Ablation design for ML)
Adapt terminology to the field's conventions (e.g., "Study Design" in medicine, "Methodology" in social sciences, "Proposed Method" in engineering)
...
Problem selection > solution design: Choosing WHAT to solve matters more than HOW
Pursue new failure cases, not incremental improvements: Find settings where existing methods break
If a well-established solution exists, switch problems: Improvement space is too small
Technology is creative combination, not concatenation: Simple A→B pipelines are not contributions
Quantity before quality in generation: Generate many candidates before evaluating any
Feasibility is not optional: Brilliant but infeasible ideas waste research cycles
