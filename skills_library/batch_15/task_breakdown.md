...
url: https://skills.sh/hungv47/meta-skills/task-breakdown
---

# task-breakdown

skills/hungv47/meta-skills/task-breakdown
task-breakdown
Installation
$ npx skills add https://github.com/hungv47/meta-skills --skill task-breakdown
SKILL.md
...
Artifacts on disk — architecture/system-architecture.md, .agents/spec.md, every .agents/product/flow/*.md
Defer to discover — if neither exists, recommend running /discover first. Do not conduct your own interview — clarification is discover's job.
...
Layer 1 dispatch — send brief + scope mode + shared context to decomposer-agent and dependency-mapper-agent in parallel.
Layer 2 sequential chain — pass both outputs to ordering-agent, then ordered list to acceptance-agent, then complete breakdown to critic-agent.
Revision loop — if critic returns FAIL, re-dispatch affected agents with feedback. Maximum 2 rounds.
Assembly — merge into the task artifact format. Every task block gets a seeded **History:** entry ({{today}} · task-breakdown · created) so the audit trail has an origin. Save to .agents/tasks.md.
Routing Rules
Condition	Route
...
Read task list fully — Status Index first, then Shared Context, then individual task blocks
...
Read the Status Index table in execution order, top to bottom. Do not scan task blocks yet.
Find the first row where: Status: pending AND every ID in Depends on has Status: done in the index. This is the next task.
If none exists, check for Status: in_progress rows:
...
If all tasks are done or removed → report completion and stop.
If only blocked tasks remain → surface the blockers to the user. Do not silently skip them.
Claim the task: flip Status: pending → in_progress, set Updated: to today + your agent identity, in both the index row and the task block. Commit this status change before starting work so concurrent agents see the claim (see Concurrency Model below).
Execute Per-Task Protocol below.
On completion: flip Status: in_progress → done, fill Evidence: with commit SHA or test output, update the index row. Then loop back to step 1 for the next task.
...
Per-Task Protocol
State which task you're starting (by ID, e.g. "Starting T3"). Note the task's current Revision: number if present.
Write minimum code to pass acceptance.
Before committing work, re-read the task block. Abort current work and restart Resume Protocol if any of: (a) Revision: bumped since you claimed — spec changed mid-flight, (b) Status flipped back to pending — PM unclaimed the task, (c) your agent identity is no longer in Updated: — another agent overwrote your claim. On abort, re-read the new Acceptance before re-claiming.
State exactly what to test and expected result.
AFK tasks: Run the acceptance test. Pass → write Evidence, flip to done, commit, move to next task without waiting. Fail → fix and re-test (max 2 attempts, then flip to blocked with reason and flag to user).
HITL tasks: Stop and present the result. Wait for user confirmation. Pass → write Evidence, flip to done, commit, announce next task. Fail → fix the specific issue only, don't expand scope.
Coding Rules
...
Bump **Revision:** counter on the task block (add the field if absent; start at 2 on first revision).
Rewrite the changed field in place (e.g., replace Acceptance: with the new test).
Append to **History:** below the task: - 2026-04-21 · user · revision 2: tightened acceptance to require RLS test, not just row creation.
If Status was in_progress, flip it back to pending — the in-flight work is now against a stale spec. Agent must re-claim.

Remove (task is no longer needed):

Flip Status: removed in both the index and the task block.
Keep the block — do not delete. Dependents (T7 depends on T4) must continue to resolve IDs.
Append to **History:**: - 2026-04-21 · user · removed: superseded by T9 (unified auth flow).
Check downstream: any task with this ID in Depends on needs review. Flag them in the response, don't auto-rewrite their deps. For any downstream task already in_progress or done at removal time, call it out explicitly — the in-flight or completed work may need reopening.

Reopen (a done, blocked, or removed task needs to run again):

Flip Status back to pending (or directly to in_progress if claiming immediately).
Clear Evidence: to — (the old evidence no longer proves the current spec).
Append to **History:**: - 2026-04-21 · user · reopened: production bug in T5's email template, re-running task.
Update the index row to match.
