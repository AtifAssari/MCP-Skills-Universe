---
title: mp-weixin-skills
url: https://skills.sh/liangxp1990/mp-wechat-skills/mp-weixin-skills
---

# mp-weixin-skills

skills/liangxp1990/mp-wechat-skills/mp-weixin-skills
mp-weixin-skills
Installation
...
首先要求 AI 将文章转换为微信公众号格式的 HTML
然后使用 Python 脚本上传素材和创建草稿
...
文档	内容	何时阅读
平台特征参考	用户画像、内容调性、阅读行为	需要了解公众号平台特征时
转换规则参考	标题优化、开头格式、正文结构、视觉元素	AI 转换文档时
样式规范参考	完整样式配置、移动端优化	需要自定义样式时
封面生成参考	封面规格、生成方式、设计原则	需要生成封面时
图片上传参考	图片格式、批量上传、优化建议	需要上传图片时
工作流参考	完整工作流、场景示例、自动化脚本	需要了解完整流程时
图片素材上传

当文章包含本地图片时：

AI 会自动：扫描图片引用，批量上传到素材库
手动上传：使用 upload-images 命令
图片规范：
格式：JPG, PNG
