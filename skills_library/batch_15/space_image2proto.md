---
title: space-image2proto
url: https://skills.sh/zephyrwang6/pm-skills/space-image2proto
---

# space-image2proto

skills/zephyrwang6/pm-skills/space-image2proto
space-image2proto
Installation
$ npx skills add https://github.com/zephyrwang6/pm-skills --skill space-image2proto
SKILL.md
...
"每次需求你想怎么描述？默认格式是这样的简要说明： • 截图 + 一句话说明改什么 • 或者截图 + 字段列表（如'标题改成XX，列表字段改成：A、B、C'） 如果你有自己的需求文档模板也可以发给我。"
...
references/config.json — output directory, naming rules, format preferences
references/design_system.json — the user's design system (colors, components, spacing)
references/learning_log.jsonl — last 20 entries of accumulated interaction history
...
Fidelity first. Match the screenshot as closely as possible. If the screenshot shows a specific shade of blue for buttons, use that exact shade, not "close enough".

Clean, semantic HTML. Use meaningful class names that describe the component (.modal-header, .f-row, .type-tag), not generic names (.box1, .div-wrapper).

CSS organization. Group styles by component with comments. Put layout/structure first, then component styles, then state styles.

Functional interactivity. If the screenshot implies interactive behavior (tab switching, dropdown opening, step wizard navigation, toggle switches), implement it with vanilla JS. The prototype should feel alive, not static.

No watermarks, no emoji. Use clean inline SVG icons (stroke-based, not filled) by default. Only use emoji if the user specifically asks for them.

Responsive modals. Set reasonable max-width, use max-height with overflow-y: auto for scrollable content areas.
