---
title: workflow-test-fix
url: https://skills.sh/catlog22/claude-code-workflow/workflow-test-fix
---

# workflow-test-fix

skills/catlog22/claude-code-workflow/workflow-test-fix
workflow-test-fix
Installation
...
Phase File Hygiene: Phase files reference workflowPreferences.* for preferences, no CLI flag parsing
3. Usage
...
🔄 sentinel fallback → Phase 5 包含 compact sentinel；若 compact 后仅存 sentinel 而无完整 Step 协议，必须立即 Read("phases/05-test-cycle-execute.md") 恢复
7. Core Rules
...
Entry Point Routing: workflow-test-fix skill → Phase 1-5; workflow-test-fix skill → Phase 5 only
8. Input Processing
...
After successful iteration (pass rate increased): test-cycle: iteration N - strategy (pass: old% → new%)
Before rollback (regression detected): test-cycle: rollback iteration N - regression detected
