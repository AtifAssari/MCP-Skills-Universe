---
title: install-rules
url: https://skills.sh/nesnilnehc/ai-cortex/install-rules
---

# install-rules

skills/nesnilnehc/ai-cortex/install-rules
install-rules
Installation
...
✅ 源已解析：读取规则源（此存储库“rules/”或指定的 Git 存储库），并从“INDEX.md”或目录列表构建规则列表
✅ 分析目标：在任何写入之前扫描现有目标文件并识别冲突/重复项
✅ 显示安装计划：显示每个规则操作的显式计划（“创建/跳过/冲突/更新”）和向用户显示目标路径
✅ 用户确认：用户在创建或修改任何文件之前明确批准该计划
✅ 已安装的规则：所选规则以正确的格式写入目标目的地（光标“.mdc”文件或 Trae 托管块）
✅ 报告的输出：安装后摘要包括执行的操作、目标路径以及任何转换注释或失败
...
列出规则：输出可安装规则的列表（名称、简短描述或范围）。让用户按名称选择“全部”或子集。
...
块内容：
在块内，使用“## Rule: ”标头以稳定的顺序呈现每个规则的一个部分（首选“rules/INDEX.md”注册表顺序（如果可用））。
保留每个规则主体（源头内容之后的所有内容）。
按托管块内的规则名称进行重复数据删除（一个名称 → 一个部分）。
幂等性：如果重新渲染的托管块与当前托管块相同，则“跳过”并且不重写文件。

输出：安装后，报告计划、执行的操作、目标路径以及任何转换说明或失败。
