---
title: backport-pr-assistant
url: https://skills.sh/cockroachdb/cockroach/backport-pr-assistant
---

# backport-pr-assistant

skills/cockroachdb/cockroach/backport-pr-assistant
backport-pr-assistant
Installation
...
Start the backport: Run backport --no-browser <pr> -r <release> -j "<justification>" for the target branch
When conflicts occur: The tool stops and lists conflicting files
Analyze conflicts: Read the conflicting files, understand what's different between branches
Resolve conflicts: Edit files to resolve, then git add the resolved files
Continue: Run backport --continue to resume
...
PR creation: Only create a PR if the user explicitly asks for it. By default, stop after the backport tool pushes the branch. If the user does ask for a PR, use gh pr create (see "Creating Backport PRs" below)
...
After resolving: git add <files> then backport --continue
Weekly Installs
73
Repository
