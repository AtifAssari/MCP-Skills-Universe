---
title: infinite-dev
url: https://skills.sh/robertwang4/infinite-dev-skill/infinite-dev
---

# infinite-dev

skills/robertwang4/infinite-dev-skill/infinite-dev
infinite-dev
Installation
$ npx skills add https://github.com/robertwang4/infinite-dev-skill --skill infinite-dev
SKILL.md
...
Copy dev-agent.py from this skill's scripts/ directory to the project root
Copy AGENTS.md from this skill's templates/ directory to the project root
Then proceed to Initializer Mode or Coding Agent Mode depending on project state
...
Read the project spec (spec.md, app_spec.txt, README, or ask the user)
Generate feature_list.json (20-200 features, sorted by priority)
Create init.sh (idempotent environment setup script)
Initialize git repo, first commit
Scaffold project structure
Create claude-progress.txt
Ask user: Mode A or Mode B? Then start Coding Agent Mode
...
永远不要删除、修改描述、修改步骤、合并或重排 feature
Testing Strategy
Web Apps
...
运行 python dev-agent.py find-parallel --count 3 找到独立 feature
对每个 feature 用 Agent tool 启动 subagent（设置 isolation: "worktree"）
各 subagent 在隔离 worktree 中实现 → 测试 → 提交
合并分支，python dev-agent.py complete <id>
清理 worktree
Mode B 下的 Team Mode
...
find_next_features(N) 找到 N 个独立 feature
为每个 feature 创建 git worktree（.worktrees/feature-<id>）
同时启动 N 个 claude -p 子进程（各在自己的 worktree 中）
等待全部完成
逐个合并回主分支（git merge --no-ff）
清理 worktree + 删除分支
重新加载 feature_list.json，继续下一批
