---
title: gh-address-cr
url: https://skills.sh/rbbtsn0w/gh-address-cr-skill/gh-address-cr
---

# gh-address-cr

skills/rbbtsn0w/gh-address-cr-skill/gh-address-cr
gh-address-cr
Installation
...
Start from the public main entrypoint: review <owner/repo> <pr_number>.
If review returns WAITING_FOR_EXTERNAL_REVIEW, fill the handoff files in the PR workspace:
producer-request.md
incoming-findings.json
incoming-findings.md
Rerun the same review command. It will auto-consume findings JSON or fixed finding blocks and continue orchestration.
...
Use threads, findings, adapter, and review-to-findings only as advanced/internal integration surfaces.
...
A GitHub thread is not terminally clean unless reply evidence exists with a concrete reply URL from the current authenticated GitHub login.
Outdated / STALE GitHub threads are still unresolved until explicitly handled.
For local findings, terminal handling must include a note.
producer=code-review must emit findings JSON before session handling starts.
Never declare completion before python3 scripts/cli.py final-gate passes.
Low-level resolve flows must refuse resolve-only handling when reply evidence is absent.
Automatic Iteration
