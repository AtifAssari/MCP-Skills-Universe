---
title: golang-ddd
url: https://skills.sh/baotoq/agent-skills/golang-ddd
---

# golang-ddd

skills/baotoq/agent-skills/golang-ddd
golang-ddd
Installation
...
Pointer receivers for entities - Mutable domain objects use *T receivers
Value receivers for value objects - Immutable types use T receivers
Factory functions - Use NewX() constructors to enforce invariants at creation
Interface in domain, implementation in infrastructure - Repository interfaces live with aggregates
One package per aggregate - Each aggregate root gets its own package under internal/domain/
Context propagation - Pass context.Context as first parameter in repository and service methods
Project Structure
internal/
...
Define value objects - Create self-validating types for domain primitives (Email, Money, Address)
Define entities - Create types with identity, unexported fields, and behavior methods
...
Implement infrastructure - Create repository implementations in infrastructure/ package
Wire application layer - Create command/query handlers that orchestrate domain operations
Pattern Details
