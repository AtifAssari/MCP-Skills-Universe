---
title: neo4j-agent-memory-skill
url: https://skills.sh/neo4j-contrib/neo4j-skills/neo4j-agent-memory-skill
---

# neo4j-agent-memory-skill

skills/neo4j-contrib/neo4j-skills/neo4j-agent-memory-skill
neo4j-agent-memory-skill
Installation
...
GLiNER — zero-shot entity extraction with typed schemas; GLiREL for relationships
LLM fallback — most accurate, most expensive; used when structure is rich or ambiguous
...
Wrong canonical docs URL — it's neo4j.com/labs/agent-memory, not a Vercel preview URL.
Inferred API surface — if code samples weren't run, flag them; prefer patterns from the GitHub README or official examples.
Missing "Labs" framing — experimental/community-supported should be clear.
Conflating with other Neo4j MCP servers — there are several (mcp-neo4j-cypher, mcp-neo4j-memory — the old knowledge graph memory server, etc.). neo4j-agent-memory's MCP server is distinct and ships as part of the package under the [mcp] extra.
Confusing NAMS with the self-hosted library — same underlying project, different consumption models. Connection strings, auth, and tool sets differ: self-hosted uses a local uvx invocation and a Neo4j --password; NAMS uses an SSE MCP URL and a nams_-prefixed bearer token. Don't mix them.
Over-promising NAMS availability — the hosted service is not yet referenced in the GitHub README or neo4j.com/labs/agent-memory/. Avoid "officially supported," SLAs, pricing claims, or "production-ready" framing. Treat it as early-access.
