---
title: statamic-with-eloquent
url: https://skills.sh/jordidolphiq/statamic-with-eloquent-skill/statamic-with-eloquent
---

# statamic-with-eloquent

skills/jordidolphiq/statamic-with-eloquent-skill/statamic-with-eloquent
statamic-with-eloquent
Installation
...
Add the set definition to the Replicator field in the blueprint
Create the Blade partial (e.g., block_<name>.blade.php)
View Composers
...
Common Pitfalls
Forgetting to unwrap Values — Statamic field data is wrapped in Value objects. Always unwrap before using in conditions or passing to components.
Asset fields returning arrays — An assets field may return an array even for max_files: 1. Handle both single and array returns.
Link fields with entry:: prefix — Link fieldtype stores entry references as entry::<uuid>. Resolve via Entry::find().
Global sets need site context — Always chain ->inCurrentSite() when reading global set values for multi-site support.
Tree/structure queries — Use Collection::findByHandle()->structure()->in('default') for hierarchical collections, not flat Entry::query().
Bard content is augmented — Bard fields return augmented HTML when accessed as Values. Use ->value() for raw ProseMirror JSON.
Component name conflicts — When using Flux UI alongside Statamic Blade components, watch for name collisions (e.g., header, footer). Register explicit Blade aliases if needed.
Weekly Installs
14
Repository
jordidolphiq/st…nt-skill
First Seen
Mar 11, 2026
Security Audits
