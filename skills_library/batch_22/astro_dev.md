---
title: astro-dev
url: https://skills.sh/gigio1023/astro-dev-skill/astro-dev
---

# astro-dev

skills/gigio1023/astro-dev-skill/astro-dev
astro-dev
Installation
...
MCP-first workflow
For "how does X work?" → Use MCP: search_astro_docs({ query: "X" })
For "what's the right pattern for X?" → Use this skill's reference files
Before generating any Astro code → Check the guardrails below to avoid known mistakes
No MCP available? → Fall back to references/doc-endpoints.md for LLM-optimized doc URLs
...
Check Astro version: package.json → "astro" version determines API surface
Check Node version: Astro 6 requires Node 22.12.0+
Check config format: .ts or .mjs (.cjs no longer supported), which integrations are installed
Check content schema: Must be src/content.config.ts (not src/content/config.ts — errors in v6)
Check Tailwind setup: @tailwindcss/vite in astro config vs @astrojs/tailwind
Then write code using the correct API for the detected versions
Weekly Installs
27
Repository
gigio1023/astro…ev-skill
GitHub Stars
13
First Seen
Mar 22, 2026
