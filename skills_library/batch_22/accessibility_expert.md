...
url: https://skills.sh/cin12211/orca-q/accessibility-expert
---

# accessibility-expert

skills/cin12211/orca-q/accessibility-expert
accessibility-expert
Installation
...
Minimal: Add alt text to images, associate labels with form controls, fix obvious contrast issues
Better: Implement proper heading hierarchy, add ARIA labels where semantic HTML isn't sufficient
Complete: Comprehensive WCAG AA audit with automated testing, implement design system with accessible color palette
...
Minimal: Fix role mismatches, ensure referenced IDs exist, add basic live regions
Better: Implement proper state management for ARIA attributes, use semantic HTML before ARIA
Complete: Create reusable accessible component patterns, implement comprehensive ARIA patterns library
...
Minimal: Add keyboard event handlers to clickable elements, ensure focus indicators are visible
Better: Implement proper tab order with logical flow, add focus management for SPAs and modals
Complete: Create focus trap utilities, implement comprehensive keyboard shortcuts with escape hatches
...
Minimal: Fix heading order, add basic landmarks, improve link text
Better: Add table headers and scope, implement semantic HTML structure
Complete: Create comprehensive page structure with proper document outline, implement dynamic content announcements

Testing Priority (2025):

NVDA (Windows) - Free, most common, comprehensive testing
VoiceOver (macOS/iOS) - Built-in, essential for mobile testing
JAWS (Windows) - Professional environments, advanced features
...
Minimal: Use relative units (rem/em), add alt text to text images, remove autoplay
Better: Implement high contrast color palette, add motion preferences support
Complete: Comprehensive responsive design audit, implement adaptive color schemes
...
Minimal: Associate labels with inputs, add required indicators, connect errors to fields
Better: Group related fields with fieldset/legend, provide clear instructions
Complete: Implement comprehensive form validation with live regions, success confirmations
...
Div Button Syndrome: Using <div onClick> instead of <button>
Color-Only Information: Relying solely on color to convey meaning
...
Add Skip Links: <a href="#main" class="skip-link">Skip to main content</a>
Basic ARIA Labels: Add aria-label to unlabeled buttons/links
Focus Indicators: Add button:focus { outline: 2px solid blue; }
Form Labels: Associate every input with a label element
Alt Text: Add descriptive alt attributes to all informative images
Live Regions: Add <div aria-live="polite" id="status"></div> for status messages
