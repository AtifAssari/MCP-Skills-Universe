...
url: https://skills.sh/pulumi/agent-skills/provider-upgrade
---

# provider-upgrade

skills/pulumi/agent-skills/provider-upgrade
provider-upgrade
Installation
...
Bump the provider dependency
Preview with required CLI flags
Diff Checkpoint - categorize every non-same resource (see next section)
Fix Category A - code changes that didn't produce same
Investigate Category B - diffs on resources you didn't change
...
pulumi state delete '<urn>' - manual step, removes from tracking only
Add replacement resources (if any) with imports - adopts the cloud state under new types
Verify with preview
