---
title: claude-code-dispatch
url: https://skills.sh/win4r/claude-code-dispatch/claude-code-dispatch
---

# claude-code-dispatch

skills/win4r/claude-code-dispatch/claude-code-dispatch
claude-code-dispatch
Installation
...
Gotchas
Must use PTY wrapper — Direct claude -p can hang in exec environments
Hook fires twice — Stop + SessionEnd both trigger; .hook-lock deduplicates (30s window)
Hook stdin is empty in PTY — Output read from task-output.txt, not stdin
tee pipe race — Hook sleeps 1s for pipe flush before reading output
Meta freshness — Hook validates meta age (<2h) and session ID
Agent Teams cost — Use --max-budget-usd to cap spend on multi-agent tasks
Rate limits — Claude Code has daily rate limits resetting at 11:00 UTC; check hook.log for "limit" messages
Weekly Installs
9
Repository
win4r/claude-co…dispatch
GitHub Stars
44
First Seen
Mar 5, 2026
