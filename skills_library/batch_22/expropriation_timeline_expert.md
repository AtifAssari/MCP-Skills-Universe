---
title: expropriation-timeline-expert
url: https://skills.sh/reggiechan74/vp-real-estate/expropriation-timeline-expert
---

# expropriation-timeline-expert

skills/reggiechan74/vp-real-estate/expropriation-timeline-expert
expropriation-timeline-expert
Installation
...
Calculate project duration using critical path method (CPM)
Identify critical path tasks (zero float tasks that drive project duration)
Perform PERT analysis (optimistic/most_likely/pessimistic time estimates)
Assess timeline risks (statutory deadline compliance, float analysis)
Calculate resource requirements (staff, consultants, budget by phase)
Optimize project schedule (identify bottlenecks, dependency chains)
Generate Gantt charts for visualization and communication
...
Start with day 0
Early start = max(predecessor early finishes)
Early finish = early start + duration

Backward pass: Calculate latest start/finish times

Start with project end date
Late finish = min(successor late starts)
...
Most Likely (M): Realistic scenario (normal conditions)
Pessimistic (P): Worst-case scenario (significant problems)
PERT Formulas
...
Executive Summary: Project duration, critical path percentage, risk summary
Critical Path Analysis: Sequence of critical tasks with schedule
Statutory Deadlines: OEA milestone dates and compliance status
Risk Assessment: Timeline risks by severity (critical/high/medium/low)
Task Details Table: All tasks with early/late dates, float, criticality
Gantt Chart: Text-based visualization of timeline
Resource Requirements: Total and peak resource needs
...
Tasks with peak resource requirements (> 5 concurrent staff)
Critical path tasks with high resource needs (no flexibility)
Resource conflicts (multiple tasks requiring same consultant)
...
Outsource work to expedite
Fast-track (overlap dependent tasks if possible)
...
Input: User creates JSON file with tasks, dependencies, deadlines
Validation: Schema validation ensures data integrity
Calculation: PERT/CPM analysis computes critical path
Risk Assessment: Identifies deadline compliance and timeline risks
Output: Markdown report with Gantt chart, JSON data export
