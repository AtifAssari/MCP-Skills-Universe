...
url: https://skills.sh/steelan9199/wechat-publisher/chrome-devtools-skill
---

# chrome-devtools-skill

skills/steelan9199/wechat-publisher/chrome-devtools-skill
chrome-devtools-skill
Installation
...
最佳实践
自动处理所有前置步骤 - 不要要求用户手动启动 Chrome 或 MCP 服务器
使用固定的用户数据目录 - 保持 --user-data-dir 一致，避免每次都重新加载
先用 take_snapshot 了解页面结构
复杂提取用 evaluate_script 执行 JavaScript
确保返回 JSON 可序列化的数据
处理动态内容时使用 wait_for
技术原理
...
关键注意事项
响应匹配: WebSocket 会收到多种消息（consoleAPICalled、executionContextCreated 等），必须通过 id 匹配请求和响应
等待页面加载: 执行操作后使用 await asyncio.sleep(2) 或等待特定元素
编码问题: Windows 终端需要 sys.stdout.reconfigure(encoding='utf-8') 来正确显示中文
实战经验与踩坑记录
问题 1: Windows 中文输出乱码
...
先使用 Read 工具读取文件（如果不存在会报错，需要用 Bash 创建空文件）
或者使用 Bash 的 echo 或 cat 命令直接写入
或者使用 Python 脚本动态生成并执行代码
问题 6: 动态内容提取不完整

现象: 提取的搜索结果中某些字段（如摘要）为空 原因:

页面使用了动态加载，需要滚动才能显示全部内容
不同网站使用不同的 class 名称
某些内容是异步加载的
...
必须先 focus() 输入框
打字间隔 0.1-0.15 秒最合适
百度搜素按钮坐标约 x:650, y:230（1920x1080分辨率）
需要启用 Input 域：Input.enable
...
启用 Runtime → 2. 填充输入框 → 3. 点击按钮 → 4. 等待页面加载 → 5. 提取数据
...
直接说明需求 - 描述你想对网页做什么
使用 /chrome-devtools-skill 命令 - 显式调用 skill
提及网页操作 - 任何涉及"打开网页、抓取数据、截图"等关键词
提示词示例
...
提示词技巧
明确 URL - 提供完整的网页地址
说明目标 - 清楚描述你想获取什么数据或执行什么操作
指定格式 - 如果需要特定格式，可以说明（如"以表格形式返回"）
复杂任务分步 - 对于多步骤任务，可以分步描述
示例对话
