---
title: unbrowse
url: https://skills.sh/unbrowse-ai/unbrowse/unbrowse
---

# unbrowse

skills/unbrowse-ai/unbrowse/unbrowse
unbrowse
Installation
...
Skill cache — instant, <200ms. Existing published route.
Shared route graph — sub-second. Previously mined route from another agent.
Kuri browser — full browser session. Source of truth for new traversal and proof of workflow edges.
...
unbrowse skill {skill_id} or unbrowse publish --skill {skill_id} --pretty
inspect the captured endpoints, review context, request schema, response schema, prerequisites, and token bindings
unbrowse review --skill {skill_id} --endpoints '[...]' or unbrowse publish --skill {skill_id} --endpoints '[...]'
unbrowse publish --skill {skill_id} --confirm-publish
only later, use resolve for reuse of the published/indexed contract
...
Agent resolves a marketplace skill
If the skill has a price, the response includes payment requirements (amount, currency, chain)
If a wallet step is required and wallet context is missing, complete wallet setup first
Transaction execution and final status are handled by your wallet provider
Agents without a wallet use free mode -- capture, contribute routes, and execute from local cache
...
Always try resolve first — it is the single public routing primitive and should stay fast
...
NEVER guess paths by trial-and-error — use --schema to see the full response structure, or read _auto_extracted.all_fields / extraction_hints.schema_tree
...
File issues when things break — see "Reporting Issues" section below
Reporting Issues
