---
title: earnings-preview-single
url: https://skills.sh/anthropics/financial-services-plugins/earnings-preview-single
---

# earnings-preview-single

skills/anthropics/financial-services-plugins/earnings-preview-single
earnings-preview-single
Installation
...
Parse the single company ticker from $ARGUMENTS (strip whitespace).
Run mkdir -p /tmp/earnings-preview to create the working directory.
Call get_latest() to establish current reporting period context.
Call get_info_from_identifiers — record market cap, industry.
Call get_company_summary_from_identifiers — record business description.
Call get_next_earnings_from_identifiers — record upcoming earnings date and fiscal quarter name.
...
Call get_latest_earnings_from_identifiers to get the most recent completed earnings call key_dev_id.
Call get_transcript_from_key_dev_id for that transcript.
Immediately write /tmp/earnings-preview/transcript-extracts.txt with the following sections. Write this file WHILE you still have the transcript in context — do not wait:
...
Call get_competitors_from_identifiers with competitor_source="all".
Select top 5-7 most relevant public competitors.
...
Write /tmp/earnings-preview/calculations.csv with all derived values:
...
cat /tmp/earnings-preview/kensho-findings.txt
cat /tmp/earnings-preview/earnings-dates.csv
cat /tmp/earnings-preview/calculations.csv
...
Header banner — immediately before the cover header, as a centered yellow banner: <div class="ai-disclaimer">Analysis is AI-generated — please confirm all outputs</div>
Footer — inside the page-footer div, as a prominent yellow banner: <div class="footer-disclaimer">Analysis is AI-generated — please confirm all outputs</div>
Appendix — as the first line of the appendix section, before the table: <div class="ai-disclaimer">Analysis is AI-generated — please confirm all outputs</div>
...
Phase 8: Output
Write the complete HTML file to earnings-preview-[TICKER]-YYYY-MM-DD.html in the current working directory.
Open it in the browser: open earnings-preview-[TICKER]-YYYY-MM-DD.html
Tell the user the file has been created and summarize the key findings.
Writing Guidelines
