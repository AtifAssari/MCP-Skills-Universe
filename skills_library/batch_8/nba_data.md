---
title: nba-data
url: https://skills.sh/machina-sports/sports-skills/nba-data
---
...
Call get_scoreboard() Result: All live and recent NBA games with scores and status
...
Filter results for Western Conference Result: Western Conference standings table with W-L, PCT, GB per team

Example 3: Team roster User says: "Who's on the Lakers roster?" Actions:

Call get_team_roster(team_id="13") Result: Full Lakers roster with name, position, jersey number, height, weight
...
Call get_injuries()
Filter results for Los Angeles Lakers (team_id=13) Result: Lakers injury list with player name, position, status, and injury type

Example 6: Player statistics User says: "Show me LeBron's stats this season" Actions:

Derive season year from currentDate
Call get_player_stats(player_id="1966", season_year=<derived_year>) Result: Season stats by category with value, rank, and per-game averages
