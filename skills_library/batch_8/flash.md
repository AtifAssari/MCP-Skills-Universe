---
title: flash
url: https://skills.sh/runpod/skills/flash
---

# flash

skills/runpod/skills/flash
flash
Installation
...
Forgetting await -- all decorated functions and client methods need await.
Missing dependencies -- must list in dependencies=[].
...
Client vs decorator -- image=/id= = client. Otherwise = decorator.
Auto GPU switching requires workers >= 5 -- pass a list of GPU types (e.g. gpu=[GpuGroup.ADA_24, GpuGroup.AMPERE_80]) and set workers=5 or higher. The platform only auto-switches GPU types based on supply when max workers is at least 5.
