---
title: improve-game
url: https://skills.sh/opusgamelabs/game-creator/improve-game
---

# improve-game

skills/opusgamelabs/game-creator/improve-game
improve-game
Installation
...
Constants first — add all new config values to Constants.js. Zero hardcoded values.
Events next — add any new events to EventBus.js using domain:action naming.
State if needed — add new state fields to GameState.js with proper reset.
New files in proper directories — entities in entities/, systems in systems/, UI in ui/.
Wire through orchestrator — register new systems in Game.js with proper lifecycle.
EventBus for communication — modules never import each other directly.
...
Run npm test if tests exist — confirm all tests still pass
If tests fail because of intentional changes (new scenes, changed elements), fix the tests to match the new behavior
If the game has visual regression tests, update snapshots: npm run test:update-snapshots
Step 6: Report
