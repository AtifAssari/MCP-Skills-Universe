...
url: https://skills.sh/greensock/gsap-skills/gsap-scrolltrigger
---

# gsap-scrolltrigger

skills/greensock/gsap-skills/gsap-scrolltrigger
gsap-scrolltrigger
Installation
...
scrollTriggers — Array of the ScrollTrigger instances that fired. Use for progress, direction, or kill().
...
Build a tween that animates the inner content’s x or xPercent (e.g. to x: () => (targets.length - 1) * -window.innerWidth or a negative xPercent to move left). Use ease: "none" on that tween.
Attach ScrollTrigger to that tween with pin: true, scrub: true
