...
url: https://skills.sh/anthropics/financial-services-plugins/fx-carry-trade
---

# fx-carry-trade

skills/anthropics/financial-services-plugins/fx-carry-trade
fx-carry-trade
Installation
...
Get Spot Rate: Call fx_spot_price for the currency pair. Note bid-ask spread as a liquidity indicator.
Price the Forward: Call fx_forward_price at the target tenor. Compute annualized carry from forward points.
Map Carry Curve: Call fx_forward_curve (list then calculate). Compute annualized carry at each tenor. Identify the sweet-spot tenor with best risk-adjusted carry.
Assess Vol Risk: Call fx_vol_surface. Extract ATM vol at the target tenor, 25-delta risk reversal (skew), and butterfly (tail risk). Compute carry-to-vol ratio.
Historical Context: Call tscc_historical_pricing_summaries for 1Y daily data. Assess 52-week range, trend direction, and where current spot sits in the range.
