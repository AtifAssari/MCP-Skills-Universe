---
title: twitter
url: https://skills.sh/himself65/finance-skills/twitter
---

# twitter

skills/himself65/finance-skills/twitter
twitter
Installation
...
Key rules
Check auth first — run twitter status --yaml before any other command
Use --yaml or --json for structured output when processing data programmatically
Use -c / --compact when token efficiency matters (LLM context)
Use --full-text when the user wants complete tweet content (text is truncated by default in rich tables)
Use --max N to limit results — start with 10-20 unless the user asks for more
For search, use filters — --from, --since, --lang, --exclude retweets, --has links
Tweet IDs vs URLs — both work: twitter tweet 1234567890 or twitter tweet https://x.com/user/status/1234567890
Use twitter show N to reference tweet #N from the most recent list output
