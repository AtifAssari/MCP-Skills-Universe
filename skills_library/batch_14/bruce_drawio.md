---
title: bruce-drawio
url: https://skills.sh/brucevanfdm/bruce-drawio/bruce-drawio
---

# bruce-drawio

skills/brucevanfdm/bruce-drawio/bruce-drawio
bruce-drawio
Installation
...
General Principles
Grid alignment: all x, y coordinates must be multiples of 10 (snap to grid)
Generous spacing: minimum 80px gap between node edges (not centers)
Center alignment: nodes in the same column share the same x; nodes in the same row share the same y
Consistent sizing: same-type nodes use identical width and height
Page margins: keep at least 60px from the canvas edge (pageWidth/pageHeight)
Use whiteSpace=wrap;html=1; on all nodes so long text wraps instead of overflowing
Balanced gutters: outer padding around a row/column should visually match the internal gaps; avoid one oversized blank side
Symmetry first: centered groups should have roughly equal left/right and top/bottom whitespace
Dense fill: containers, sub-groups, and sidebars should fit content plus consistent padding; do not leave large dead zones just because the canvas is large
...
Read the existing .drawio file first to understand current structure
Edit based on the existing XML — do not regenerate from scratch
Apply the user's requested changes while preserving the overall layout and style
Run the same self-review and export steps
Reference
