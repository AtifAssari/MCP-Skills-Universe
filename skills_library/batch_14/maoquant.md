---
title: maoquant
url: https://skills.sh/fidingks/mao-quant/maoquant
---

# maoquant

skills/fidingks/mao-quant/maoquant
maoquant
Installation
$ npx skills add https://github.com/fidingks/mao-quant --skill maoquant
SKILL.md
...
Resolve stock names yourself. "茅台" = 600519.SH, "平安" = 601318.SH. Use catquant.resolve.resolve(query) to map names to codes. NEVER ask the user "请问茅台的代码是什么".
Validate data BEFORE running backtest — NEVER skip this. Call check_available(code) and if it returns False, STOP immediately and tell the user. Do NOT assume data exists. Do NOT create a script without checking first. Most stocks are NOT covered by the free API.
Handle errors gracefully. If data source fails, suggest alternatives (TDX local data, or contact developer). Never show raw Python tracebacks to the user.
...
STOP — do NOT proceed to create or run any backtest script
Tell the user the stock is not covered, in plain language
...
Do NOT silently fail, do NOT produce an empty backtest, do NOT skip this check
...
Resolve stock name → code (Step 0 above)
Validate data availability (Step 0 above)
Read catquant-expert for API and strategy patterns
Create script at backtesting/{strategy_name}/{symbol}_{strategy}_backtest.py
Script must: load data, compute signals, run backtest, export JSON, render charts with indicator lines, print metrics
Run the script
Explain the backtest report in plain language to the user
Chart Rendering
...
Parse criteria into pre_filter (PriceData fields) and filter_fn (K-line indicators)
Create script at scanning/{name}_scan.py
...
策略总结 — one paragraph summarizing what the strategy does and how it performed
优势 — bullet list of strong points (high win rate / controlled drawdown / good Sharpe / etc.)
风险提示 — bullet list of weaknesses (high drawdown / low win rate / short sample / etc.)
改进建议 — concrete suggestions:
...
Constraints
Raw data never enters AI context — use catquant.data_engine inside scripts only
A-share compliance auto-enforced by backtest.run():
T+1, price limits (10%/20%/30%), lot sizing (100 shares), fees
Scripts go in backtesting/ or scanning/
No emoji in code or output
Charts via catquant.chart.render()
PDF reports must use a registered CJK font (see PDF Reports section above)
Reference
catquant-expert — API, indicators, strategy patterns
data — Data engine, SecurityData, symbol format
Weekly Installs
9
Repository
fidingks/mao-quant
