---
title: hola-poke-yeoksam
url: https://skills.sh/nomadamas/k-skill/hola-poke-yeoksam
---

# hola-poke-yeoksam

skills/nomadamas/k-skill/hola-poke-yeoksam
hola-poke-yeoksam
Installation
...
사용자가 참여 의사를 밝히면 휴대폰 번호를 먼저 받는다.
이름·이메일은 받지 않고 번호만 받는다.
번호는 결과 대조용이며 별도 마케팅 발송/3자 공유 용도가 아니라고 한 번 고지한다.
enter_event(phone) 를 호출한다.
phone_format 이면 서버 message 를 그대로 보여주고 다시 받는다.
already_entered_today 이면 서버 message 를 그대로 보여주고 더 이상 재시도하지 않는다.
정상 응답이면 message, code, next_action 을 함께 전달한다.
3. 응답 원칙
