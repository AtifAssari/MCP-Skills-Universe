---
title: orca-cli
url: https://skills.sh/stablyai/orca/orca-cli
---

# orca-cli

skills/stablyai/orca/orca-cli
orca-cli
Installation
...
accessing repos known to Orca Do not use orca when plain shell tools are simpler and Orca state does not matter.
...
Interact using refs (@e1, @e3, etc.) to click, fill, or select.
Re-snapshot after interactions to see the updated page state.
...
Run orca tab list --json.
Read tabs[].browserPageId from the result.
Pass --page <browserPageId> to follow-up commands like snapshot, click, goto, screenshot, tab switch, or tab close.
...
Note: Per-request intercept continue and intercept block are not yet supported. They will be added once agent-browser supports per-request interception decisions.
...
"Ref not found" / browser_stale_ref Page changed since the snapshot. Run orca snapshot --json again, then use the new refs.
...
Fill/type doesn't work on a custom input Some components intercept key events. Use keyboard inserttext:

orca focus --element @e1 --json
orca exec --command "keyboard inserttext \"text\"" --json


browser_no_tab error No browser tab is open in the current worktree. Open one with orca tab create --url <url> --json.
