...
url: https://skills.sh/elastic/agent-skills/cloud-network-security
---

# cloud-network-security

skills/elastic/agent-skills/cloud-network-security
cloud-network-security
Installation
...
Before creating a filter, always run list-filters and check whether an existing filter for the same purpose already has the required source rules. Filters can be associated with multiple projects, so one filter with the right rules is better than duplicates.
...
Deleting a filter fails if it is still associated with a project. Disassociate first using cloud-manage-project (PATCH the project with the filter removed from the traffic_filters list), then delete.
