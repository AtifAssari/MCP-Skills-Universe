...
url: https://skills.sh/elastic/agent-skills/kibana-alerting-rules
---

# kibana-alerting-rules

skills/elastic/agent-skills/kibana-alerting-rules
kibana-alerting-rules
Installation
...
Missing kbn-xsrf header. All POST, PUT, DELETE requests require kbn-xsrf: true or any truthy value. Omitting it returns a 400 error.

Wrong consumer value. Using an invalid consumer (e.g., observability instead of infrastructure) causes a 400 error. Check the rule type's supported consumers via GET /api/alerting/rule_types.

Immutable fields on update. You cannot change rule_type_id or consumer with PUT. You must delete and recreate the rule.

Rule-level notify_when and throttle are deprecated. Setting these at the rule level still works but conflicts with action-level frequency settings. Always use frequency inside each action object.
...
Kibana Alerting API Reference
Alerting Concepts
Create and Manage Rules (UI)
Rule Action Variables
Alerting Production Considerations
Terraform: elasticstack_kibana_alerting_rule
