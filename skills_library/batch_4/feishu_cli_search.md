---
title: feishu-cli-search
url: https://skills.sh/riba2534/feishu-cli/feishu-cli-search
---
...
搜索并开通缺失的 scope（例如 search:docs:read、search:message），或复制 README 的完整 JSON 一次性导入
等待 tenant 管理员审批（如果需要）
审批通过后再执行 feishu-cli auth login --scope "search:docs:read search:message" --json
