---
title: markdown-new
url: https://skills.sh/jpcaparas/skills/markdown-new
---

# markdown-new

skills/jpcaparas/skills/markdown-new
markdown-new
Installation
$ npx skills add https://github.com/jpcaparas/skills --skill markdown-new
SKILL.md
markdown.new
...
Verified Behaviors
Bare domains are normalized. https://markdown.new/example.com?format=json resolved to https://example.com in testing.
...
GitHub raw file URLs work well. GitHub blob URLs returned HTML-heavy noise in testing.
Crawl jobs are cached. Re-running a crawl on https://example.com returned a cached job created on April 3, 2026.
Gotchas
Use POST for complex target URLs: URLs with their own query strings were reliable with POST / but a fully percent-encoded GET path returned HTTP 400 in testing.
Do not trust method as provenance: the service documents a native Markdown-first pipeline, but a known Markdown endpoint still reported Cloudflare Workers AI in a live test.
Expect weak error reporting: missing url, malformed JSON, bogus crawl IDs, and invalid paths returned 400/500 without a useful error body in testing.
Do not assume tokens always exists: browser-mode responses omitted tokens in testing even when the conversion succeeded.
Crawl limits are documented inconsistently: the homepage says 100 pages in one section, while the crawl page says 500 pages and accepts limit values up to 500.
Helper Script
