---
title: design
url: https://skills.sh/arjunkshah/design-skill/design
---

# design

skills/arjunkshah/design-skill/design
design
Installation
$ npx skills add https://github.com/arjunkshah/design-skill --skill design
SKILL.md
Design Skill
...
Use prepare() + layout() to test candidate widths and line counts for key copy.
For advanced composition, use prepareWithSegments() + walkLineRanges() to find aesthetically balanced widths.
...
component_families: hero, feature, proof, pricing, CTA, nav, timeline, stats, testimonials, forms, footers, etc.
style_modes: editorial, industrial, playful, premium, utilitarian, experimental, brutalist-lite, minimal, etc.
interaction_modes: static, hover-rich, scroll-narrative, sticky/pinned, gesture-like, kinetic.
surface_modes: flat, framed, layered glass, textured, outlined, mixed-density.
...
Per major output, create at least 3 bespoke components that are not direct copies of common startup templates.
Per major output, create at least 2 bespoke motion patterns tied to section intent.
At least 30% of sections must use uncommon composition patterns (not standard grid-of-cards).
...
Use at least 8 distinct section archetypes.
Use at least 5 distinct component families beyond basic cards/buttons.
Use at least 3 interaction models (for example: static utility, sticky narrative, progressive reveal).
Use at least 2 non-rectilinear or asymmetric compositions.
Limit any single reusable component pattern to max 2 appearances per page.
...
at least 4 custom components invented for this brief
at least 2 custom section-level interaction models
at least 1 signature section that cannot be described as a common template block
...
Part A — Structure: choose layout archetype, funnel sequence, and section jobs.
Part B — Style: choose typography, palette energy, surface language, and imagery mode.
Part C — Motion: map section jobs to motion pairings and set intensity by context.
Part D — Review: run anti-pattern and quality gates before final output.
...
Default target is 6-10 pages for product/business websites unless the user explicitly requests fewer.
Each major page should include 6-10 meaningful sections (not filler blocks).
...
The landing page must be substantial by default, typically 8-14 sections with clear narrative flow.
