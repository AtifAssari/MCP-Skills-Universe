...
url: https://skills.sh/geekjourneyx/travel-guidebook/travel-guidebook
---

# travel-guidebook

skills/geekjourneyx/travel-guidebook/travel-guidebook
travel-guidebook
Installation
$ npx skills add https://github.com/geekjourneyx/travel-guidebook --skill travel-guidebook
SKILL.md
...
技术要求
使用 heredoc 分批写入（cat > 首批，cat >> 后续），单引号 << 'HTMLEOF'
¥ 用 ¥ 替代，— 用 —，→ 用 →，° 用 °
每批写完后 wc -c 确认文件增长
总批次 3-5 批，按逻辑段落分割
所有 CSS 内嵌 ，所有 SVG 内联，零外部图片
最后一批必须包含 闭合标签
...
使用单引号 heredoc（<< 'HTMLEOF'）防止 shell 变量展开
¥ 符号在 heredoc 中用 &#165; 替代，避免编码问题
每批写完后用 wc -c 确认文件增长正常
总批次控制在 3-5 批，按逻辑段落分割（不要在 HTML 标签中间断开）
Stage 5: HTML 交付与检查
交付内容
HTML 文件：{路线名称}_guidebook.html（单文件，零外部依赖）
工作报告：制作过程记录（模板见 references/report-template.md）
HTML 检查点
...
设计原则
秒级可达 — 关键信息 3 秒内找到。日程卡片、地名、方向是最高优先级。
三速阅读 — 支持扫视（3s）、浏览（30s）、沉浸（3min）三种模式。
温暖如纸 — Claude 羊皮纸暖色调，数字路书拥有纸质书温度。
编辑品味 — 像 Monocle 精选，不堆砌。每个推荐都带立场和温度。
时空网格 — "天 × 地点"网格架构，支持随机访问。
双重身份 — 旅行前是规划工具，旅行后是记忆载体。
大地色系 — 赤陶、琥珀、橄榄、鼠尾草。没有冷色，没有荧光。
Weekly Installs
24
Repository
geekjourneyx/tr…uidebook
GitHub Stars
55
First Seen
2 days ago
