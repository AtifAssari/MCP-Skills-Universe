---
title: skill-creator-advanced
url: https://skills.sh/jpcaparas/skills/skill-creator-advanced
---

# skill-creator-advanced

skills/jpcaparas/skills/skill-creator-advanced
skill-creator-advanced
Installation
...
Otherwise inspect the current repo, the current installation context, and common global skill roots. Use scripts/infer_destination.py or apply the same logic manually.
Prefer a repo-local skill root when the current repo already stores skills there.
Otherwise prefer the currently installed skill family or the global root with the most existing skills.
If nothing is established yet, default to <repo-root>/.agents/skills/ inside a git repo, or the current harness's global root outside a repo.
...
For API Wrappers
Fetch the actual API docs — use WebFetch, Firecrawl, or context7 MCP to get current documentation
Identify auth patterns — API key, OAuth, service account, bearer token
List all endpoints/operations — group by domain (CRUD, admin, analytics, etc.)
Find rate limits, quotas, pricing — these go in gotchas
Test a real API call — execute at least one operation to verify the API is reachable and the auth pattern works. Save the working command to scripts/
For CLI Tools
Run --help on the tool and capture output
Identify subcommands — group by function
Test the most common operation — execute it and verify output
Find version — pin the version in the skill
For Progressive Docs
Map the domain — list all products/topics that need coverage
Identify the decision tree — how does a user choose between options?
Estimate reference count — if >20 references, plan for the 5-file structure (see references/patterns.md)
For Improving Existing Skills
Read the entire existing skill — SKILL.md + all references, scripts, assets
Run scripts/validate.py against it to get a structural audit
Identify disclosure gaps — content that should be in references but is inline, or vice versa
Preserve what works — don't rewrite from scratch unless structurally broken
...
Layer 0 (always loaded): Frontmatter name + description (~100 tokens)
Layer 1 (on trigger): SKILL.md body — decision trees, quick reference, pointers
Layer 2 (on demand): Reference files — deep dives loaded when needed
Layer 3 (on execute): Scripts — run without loading into context
...
SKILL.md Body Rules
Open with a one-line summary of what this skill does
Decision tree within the first 50 lines — if there are multiple paths, force disambiguation early
Quick reference table — the most common operations in a scannable table
Pointers to references — explicit "Read references/X.md when you need Y" guidance
Gotchas section — at least 3 non-obvious pitfalls. This is the highest-value content
Stay under 500 lines — if approaching this, move content to references
...
Structure validation — run scripts/validate.py <skill-path> from this skill's directory
Example verification — for EACH code example or command in the skill:
...
Cross-reference integrity — every file path mentioned in SKILL.md must exist
...
Disclosure metrics — SKILL.md must be <500 lines; no reference file >1000 lines without a TOC
Description coverage — the frontmatter description must mention every major capability
...
Identify the class of error — structural, content, disclosure, verification, or style
Extract the generalizable rule — not "add X to skill Y" but "when wrapping APIs that use pagination, always include a pagination patterns section"
Check for existing rules — read references/gotchas.md to see if this is already covered
If new: append the rule to references/gotchas.md under the appropriate category
If contradicts existing: update the existing rule and note the date
...
Apply the fix to the current skill
Update references/gotchas.md with the lesson (if generalizable)
Re-run scripts/validate.py to confirm the fix didn't break structure
