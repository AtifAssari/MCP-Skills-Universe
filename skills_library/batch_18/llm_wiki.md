---
title: llm-wiki
url: https://skills.sh/sdyckjq-lab/llm-wiki-skill/llm-wiki
---

# llm-wiki

skills/sdyckjq-lab/llm-wiki-skill/llm-wiki
llm-wiki
Installation
$ npx skills add https://github.com/sdyckjq-lab/llm-wiki-skill --skill llm-wiki
SKILL.md
llm-wiki — 个人知识库构建系统
...
将 Step 1 JSON 写入 {wiki_root}/.wiki-tmp/step1-latest.json
调用 bash ${SKILL_DIR}/scripts/validate-step1.sh {wiki_root}/.wiki-tmp/step1-latest.json
验证完成后删除 {wiki_root}/.wiki-tmp/step1-latest.json
...
更新 index.md 和 log.md（缓存已由 create-source-page.sh 自动更新）
...
展示文件列表，确认处理范围（按 WIKI_LANG 切换语言）：
...
每 5 个文件后暂停，展示进度并询问是否继续（按 WIKI_LANG 切换语言）：
...
输出报告（按 WIKI_LANG 切换语言，整合 Step 0 脚本输出 + Step 3 AI 判断结果）：
...
生成 Mermaid 图表文件 wiki/knowledge-graph.md：
...
用编辑器打开 wiki/knowledge-graph.md
参考 .wiki-schema.md 里的"关系类型词汇表"（实现 / 依赖 / 对比 / 矛盾 / 衍生）
把最重要的 3-5 条箭头改写成 A -->|实现| B 之类的带标注写法
保存后用 Obsidian / VS Code / Typora 重新渲染
...
关键决策和原因
值得记录的结论
生成 wiki/synthesis/sessions/{主题}-{日期}.md，格式参考 templates/synthesis-template.md
