---
title: hz-iwsdk-webxr
url: https://skills.sh/meta-quest/agentic-tools/hz-iwsdk-webxr
---

# hz-iwsdk-webxr

skills/meta-quest/agentic-tools/hz-iwsdk-webxr
hz-iwsdk-webxr
Installation
...
Verify API and platform details against current docs before coding
Edit the code and run the Vite dev server
Reload in Quest Browser or the IWSDK emulator/dev browser
Observe logs, screenshots, and runtime state
Iterate until runtime behavior matches the request
...
Test on a real Quest device early and often because desktop emulation does not capture all input, comfort, and performance issues
Keep draw calls low (roughly under 100) for smooth performance on Quest hardware
References
Skill References
