---
title: build
url: https://skills.sh/slamb2k/mad-skills/build
---

# build

skills/slamb2k/mad-skills/build
build
Installation
$ npx skills add https://github.com/slamb2k/mad-skills --skill build
SKILL.md
...
Load project context — invoke /prime to load domain-specific context (CLAUDE.md, specs, memory). If /prime is unavailable, fall back to manually scanning CLAUDE.md and specs/ directory.

Detect project type using references/project-detection.md to populate PROJECT_CONFIG (language, test_runner, test_setup)
...
File detection — If the argument contains / or ends with .md, .yaml, .json, or .txt, treat it as a file path:
...
Review EXPLORE_REPORT questions and potential_issues
...
Compile into DEBRIEF_ITEMS (see references/stage-prompts.md#stage-10). Categorise each as: unresolved_risk, deferred_fix, open_question, assumption, or tech_debt.

If no items found, skip to Final Report.

Present numbered summary via AskUserQuestion grouped by category. Each item shows: [category] summary (effort).
