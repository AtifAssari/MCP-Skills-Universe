---
title: cq
url: https://skills.sh/mozilla-ai/cq/cq
---

# cq

skills/mozilla-ai/cq/cq
cq
Installation
$ npx skills add https://github.com/mozilla-ai/cq --skill cq
SKILL.md
cq Skill
...
Call query with domain tags derived from the error context (e.g. the library, tool, or API involved) before attempting any fix.
If a relevant knowledge unit exists, apply its guidance and confirm it if it resolves the issue.
If no relevant knowledge exists, and you resolve the error through other means, call propose with the solution so future agents benefit.
...
Call query with domains: ["bundler", "webpack", "nodejs-polyfills"] and frameworks: ["react"].
No relevant results returned. Proceed normally.
Debug the issue: webpack 5 removed Node.js polyfills. Add resolve.fallback: { stream: require.resolve("stream-browserify") } to the config.
Call propose:
