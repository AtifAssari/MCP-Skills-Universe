...
url: https://skills.sh/wei-shaw/sub2api/code-review-expert
---

# code-review-expert

skills/wei-shaw/sub2api/code-review-expert
code-review-expert
Installation
...
文件分片不重叠：每个文件只分配给一个主要维度（按文件类型/路径自动判断），不要多维度重复审核同一文件。高风险文件（auth、crypto、payment）例外，可分配给最多 2 个维度。

单文件读取上限：子 Agent 读取单个文件时，使用 Read 工具的 limit 参数，每次最多读取 300 行。超过 300 行的文件分段读取，仅审核关键段落。

子 Agent prompt 精简：传递给子 Agent 的 prompt 只包含：
...
结果输出精简：子 Agent 找到问题后只输出 JSON Lines，不要输出解释性文字、思考过程或总结。完成后只输出 status 行。

子 Agent max_turns 限制：每个子 Agent 使用 max_turns 参数限制最大轮次：
...
子 Agent 使用 run_in_background：所有子 Agent Task 调用设置 run_in_background=true，主 Agent 通过 Read 工具轮询 output_file 获取结果，避免子 Agent 的完整输出回填到主 Agent 上下文。
...
将文件分配给 5 个审核维度（每个文件可被多维度审核）

Phase 0.5 — Context7 Documentation Warm-up (反幻觉第一重)

详细流程见 references/context7-integration.md

扫描依赖清单（go.mod, package.json, requirements.txt, Cargo.toml, pom.xml 等）
提取核心直接依赖，按优先级筛选最多 10 个关键库：
P0 框架核心（web 框架、ORM）→ P1 安全相关 → P2 高频 import → P3 其余
对每个库调用 resolve-library-id → get-library-docs（每库 ≤ 5000 tokens）
构建 C7 知识缓存 JSON，传递给所有子 Agent
降级：Context7 不可用时跳过，报告标注 "未经官方文档验证"
...
Security & Compliance — 注入漏洞(10 类)、认证授权、密钥泄露、密码学、依赖安全、隐私保护
Architecture & Design — SOLID 原则、架构模式、API 设计、错误策略、模块边界
Performance & Resource — 算法复杂度、数据库性能、内存管理、并发性能、I/O、缓存、资源泄漏
Reliability & Data Integrity — 错误处理、空值安全、并发安全、事务一致性、超时重试、边界条件、优雅关闭
Code Quality & Observability — 复杂度、重复、命名、死代码、测试质量、日志、可观测性、构建部署
