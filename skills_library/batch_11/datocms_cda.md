---
title: datocms-cda
url: https://skills.sh/datocms/agent-skills/datocms-cda
---

# datocms-cda

skills/datocms/agent-skills/datocms-cda
datocms-cda
Installation
...
Read package.json and check for @datocms/cda-client.

If not installed, recommend: npm install @datocms/cda-client

Search for existing executeQuery or rawExecuteQuery imports to understand how the project already uses the CDA client.

Check .env, .env.local, or similar files for a DatoCMS API token. Look for variable names like:
...
Error handling — Ensure ApiError is caught at appropriate boundaries
Pagination — If the collection could exceed 500 records, use executeQueryWithAutoPagination
Draft mode — If includeDrafts is used, ensure it is intentional (not accidentally showing unpublished content in production)
excludeInvalid — Recommend for stable schemas. If the schema is changing (migrations, new required fields), use filter: { _isValid: { eq: true } } instead to avoid re-validation errors
Type safety — No type assertions (as) used to silence errors
Imports — CDA client imports come from @datocms/cda-client; keep project-generated GraphQL helper imports when type generation is already wired
Variables — All dynamic values use GraphQL variables, not string interpolation
Structured text completeness — If querying structured text, all relevant sub-fields (value, blocks, links, inlineBlocks) are included
Fetch integration — If the solution uses framework-native fetch, ensure CDA headers/init come from buildRequestHeaders() / buildRequestInit() instead of hand-rolled request wiring
Type generation — If the project uses gql.tada or graphql-codegen, ensure queries use the project's graphql() function (not plain template literal strings) and that scalar mappings are configured
Cross-Skill Routing
