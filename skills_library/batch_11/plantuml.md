---
title: plantuml
url: https://skills.sh/magicjie/plantuml-skill/plantuml
---

# plantuml

skills/magicjie/plantuml-skill/plantuml
plantuml
Installation
...
"create [diagram type]" OR "generate diagram" OR "draw diagram" → Route to: Resilient Workflow (PRIMARY) - references/workflows/resilient-execution-guide.md → Script: python scripts/resilient_processor.py file.md [--format png] → Backup Route: Diagram Creation Workflow (Step 2) if script unavailable
...
Skip syntax reference if user provides clear diagram description
Use inline minimal examples instead of loading full example guides
Load basic styling instead of comprehensive guide
Reference examples directory without loading guide files
Load only relevant section of troubleshooting guide (navigate to specific diagram type)
...
Load references/troubleshooting/toc.md - Error decision tree
Identify error category
Load specific guide: references/troubleshooting/[category]_guide.md Resources: 12 guides covering 215+ common errors (installation, syntax, arrows, styling, diagrams, performance)
6. Styling
...
Use !theme plain as neutral baseline
For sequence diagrams, group participants by architecture layer using box "..." #Color
Use soft background colors for layer boxes; keep participant/arrow styling mostly default
Add <style> only for focused readability improvements, not full recoloring
...
Identify type → Load reference
Create file → Structured naming: ./diagrams/<md>_<num>_<type>_<title>.puml
Convert → Handle errors with troubleshooting guides (max 3 retries)
Validate → Integrate into markdown Error Fallback: Perplexity → Brave Search → Gemini → WebSearch Benefits:
...
Load: references/plantuml_reference.md (Installation section)
Syntax Error
Load: references/troubleshooting/toc.md (~400 tokens)
Use error decision tree to identify category
Load specific guide: references/troubleshooting/[category]_guide.md (~500-1,000 tokens)
Apply recommended fix from guide
...
Load: references/toc.md (~780 tokens)
Present options
...
Classify intent using Step 1 routing
Load diagram references or run target script for identified task
Load supporting resources only as workflow requires
Track token budget throughout process
Use Read tool for surgical loading
