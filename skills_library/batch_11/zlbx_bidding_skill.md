---
title: zlbx-bidding-skill
url: https://skills.sh/site/skills.volces.com/zlbx-bidding-skill
---

# zlbx-bidding-skill

skills/skills.volces.com/tender-search
tender-search
$ npx skills add https://skills.volces.com/skills/clawhub/liu-jiapeng
SKILL.md
知了标讯 - 全网招中标数据平台
API 概览
...
检查环境变量 ZLBX_API_KEY 是否正确设置
确认 API Key 未过期
如需申请 Key，访问：https://ai.zhiliaobiaoxun.com
...
bid_id（推荐）：{"bid_id": 12345678}
bid_url：{"bid_url": "https://www.zhiliaobiaoxun.com/content/1234567890/b1"}
uniq_key：{"uniq_key": "xxx"}
...
注意事项
company 参数：支持公司全称（字符串）或平台内部 ID（整数），二选一
金额单位：请求参数中金额单位为元；响应中 money 为元，money_wan 为万元
积分消耗：aggregate_bids_advanced 消耗 2-3 积分，其他工具一般为 1 积分
时间格式：日期参数统一为 YYYY-MM-DD 格式
get_bid_detail 的 bid_id、uniq_key、bid_url 三者至少填一个
keyword_groups 参数：用于复杂组合查询，可针对不同字段分别指定关键词，实现"关键词A在字段1中 AND 关键词B在字段2中"的查询逻辑
match_modes 全文搜索：使用 fulltext 模式可进行全文检索，搜索范围最广但响应较慢，建议优先使用 sm、title 等精准模式
获取公告原文：只能通过 get_bid_detail 接口获取，设置参数时不需要其他字段，直接调用即可
获取联系人：只能通过专门的 get_company_contacts 接口获取
互联网增强分析
...
数据优先级原则
标讯数据为主：历史中标记录、项目金额、合作关系等客观数据
互联网信息为辅：用于背景补充、趋势验证、动态更新
信息源优先级：公司官网 > 可靠媒体 > 政策网站 > 一般新闻
互联网信息来源
信息类型	来源	用途
