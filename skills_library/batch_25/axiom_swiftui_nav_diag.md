---
title: axiom-swiftui-nav-diag
url: https://skills.sh/charleswiltgen/axiom/axiom-swiftui-nav-diag
---

# axiom-swiftui-nav-diag

skills/charleswiltgen/axiom/axiom-swiftui-nav-diag
axiom-swiftui-nav-diag
Installation
...
If link tap does nothing AND no onChange → Link outside NavigationStack (check hierarchy)
If onChange fires but nothing pushes → navigationDestination not in scope (check placement)
If pushes then immediately pops → View identity change or path reset (check @State location)
If deep link fails → Timing or MainActor issue (check thread)
If crash → Force unwrap on path decode or missing type registration
...
Navigation tap does nothing → Add onChange logging FIRST, then Pattern 1
Unexpected pop → Find WHAT is modifying path (logging), then Pattern 2
Deep link fails → Verify URL received (print in onOpenURL), then Pattern 3
State lost → Identify WHEN lost (tab switch vs background), then Pattern 4
Crash → Get full stack trace, then Pattern 5
