---
title: aptx-api-plugin-retry
url: https://skills.sh/haibaraaiaptx/aptx-skill/aptx-api-plugin-retry
---

# aptx-api-plugin-retry

skills/haibaraaiaptx/aptx-skill/aptx-api-plugin-retry
aptx-api-plugin-retry
Installation
...
可用于实现"仅在首次失败时重试"等逻辑
在接入重试插件时，按以下步骤执行：
使用 createRetryMiddleware({ retries, delayMs, retryOn }) 并通过 client.use() 挂载到 RequestClient。重试中间件应位于管道前端。
明确 retryOn 规则，利用 req.method、req.url 等上下文信息，避免对业务错误或不可重试请求无差别重试。
把指数退避、抖动策略放到 delayMs 函数里实现，不修改 core。
保持 ctx.attempt 可观测，便于定位重试链路。
在测试中覆盖成功重试、放弃重试、延迟重试三类场景。
...
详细文档
文档	内容
延迟策略	固定延迟、指数退避、抖动策略、生产环境推荐配置
测试指南	测试用例、Mock 工具、测试覆盖清单
插件集成	与 auth/csrf/缓存 middleware 的协调、幂等性控制、SSR 注意事项
Weekly Installs
26
Repository
