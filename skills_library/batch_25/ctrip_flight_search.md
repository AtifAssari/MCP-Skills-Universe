---
title: ctrip-flight-search
url: https://skills.sh/stvlynn/dingtalk-wukong-skills/ctrip-flight-search
---

# ctrip-flight-search

skills/stvlynn/dingtalk-wukong-skills/ctrip-flight-search
ctrip-flight-search
Installation
...
核心操作SOP流程
基础查询流程
导航到携程首页: navigate → https://www.ctrip.com
进入航班查询页面: backbone → 定位机票按钮 → click
输入出发地: type → 城市名称 → search → 选择具体机场 → click
输入目的地: search → 目标城市 → click
设置日期: 确认日期输入框已正确设置
执行查询: press → Enter（优先方案）或点击搜索按钮
关键操作要点
出发地/目的地输入
...
注意事项
页面加载时间: 携程页面可能需要一定加载时间，操作间需适当等待
网络环境: 确保网络连接正常，携程在国内访问稳定
浏览器兼容性: 基于现代浏览器的自动化工具，确保工具版本兼容
验证码处理: 如遇验证码或登录要求，需人工干预处理
多城市测试结论: 技能在不同城市组合下表现一致，核心操作流程无需调整
扩展方向
添加往返航班查询支持
