---
title: eve-manifest-authoring
url: https://skills.sh/incept5/eve-skillpacks/eve-manifest-authoring
---

# eve-manifest-authoring

skills/incept5/eve-skillpacks/eve-manifest-authoring
eve-manifest-authoring
Installation
...
How It Works
Manifest sync stores CLI metadata alongside api_spec.
Job workspace setup (after clone) runs chmod +x and symlinks the binary to /usr/local/bin/.
For image-based CLIs, an init container copies the binary from the image.
The CLI reads EVE_APP_API_URL_{SERVICE} and EVE_JOB_TOKEN (already injected) -- no manual auth or URL configuration.
CLI naming rules
