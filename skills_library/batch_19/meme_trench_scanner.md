---
title: meme-trench-scanner
url: https://skills.sh/okx/plugin-store/meme-trench-scanner
---

# meme-trench-scanner

skills/okx/plugin-store/meme-trench-scanner
meme-trench-scanner
Installation
...
Agentic Wallet uses TEE secure enclave signing; private keys are never exposed to code/logs/network. No need to set WALLET_PRIVATE_KEY environment variable.
...
Set PAUSED = False (allow bot to run normally after interactive confirmation)
Check prerequisites: onchainos --version, onchainos wallet status
Start bot: python3 scan_live.py
Show Dashboard link: http://localhost:3241
Inform user: Currently in Paper Mode. To switch to Live Trading, modify PAPER_TRADE = False in config.py
...
NEVER delete a position based on a single balance check. Must have zero_balance_count >= 3.
NEVER call save_positions() outside of pos_lock.
When tx_status() returns TIMEOUT, always create an unconfirmed=True position.
