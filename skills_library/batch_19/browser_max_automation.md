---
title: browser-max-automation
url: https://skills.sh/aktsmm/agent-skills/browser-max-automation
---

# browser-max-automation

skills/aktsmm/agent-skills/browser-max-automation
browser-max-automation
Installation
...
対象ブラウザを全部閉じる
デバッグポート付きで起動する
mcp.json で --cdp-endpoint http://localhost:9222 を指定する
VS Code をリロードする
...
直接 WebSocket CDP の起動フラグ、websocket-client 接続、SPA hash navigation、virtual scroll 操作は references/instructions/cdp-direct-websocket.instructions.md を参照する。
...
判断基準: 発注確定など不可逆操作の直前・直後は browser_take_screenshot で証跡を残し、中間のフォーム入力は evaluate で高速化するのが実用的なバランス。
...
CDP ポート確認: Invoke-WebRequest -Uri 'http://localhost:<port>/json/version' -TimeoutSec 5 — 接続拒否ならブラウザが落ちている
ブラウザ再起動: 同じ --remote-debugging-port と --user-data-dir で起動する
CDP 起動確認: 上の確認コマンドで Browser: が返ること
MCP 再接続: browser_close（古いセッション破棄）→ browser_navigate で新しいページを開く
認証が必要なサイトは、MCP を browser_close してから Python ログインスクリプトを実行し、完了後に MCP で再接続する
CDP context / page 選択
...
すべての browser.contexts を走査する
各 context の既存 page を、目的ドメインや管理画面URLを優先して並べる
候補 page ごとにログイン/権限/目的画面への到達を確認する preflight を実行する
preflight が通った context/page だけを以後の操作に使う
全候補が失敗したら、失敗したURL・title・理由をJSONに残して停止する
