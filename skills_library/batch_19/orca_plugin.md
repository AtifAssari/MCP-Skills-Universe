---
title: orca-plugin
url: https://skills.sh/okx/plugin-store/orca-plugin
---

# orca-plugin

skills/okx/plugin-store/orca-plugin
orca-plugin
Installation
...
Security scan of output token via onchainos security token-scan
...
Run get-quote to check estimated output, price impact, and fees
Run swap (no flags) to preview — returns "preview": true with no broadcast
Ask user to confirm all details before proceeding
Re-run with --confirm to broadcast — pre-flight balance check runs automatically
...
Always preview first (run swap without --confirm) and show the output to the user before executing. Only add --confirm (global flag, before the subcommand) after the user has approved.
If pool TVL < $10,000, warn user about high slippage risk
Use native SOL mint (11111111111111111111111111111111) for SOL swaps. Using the wSOL mint (So11111111111111111111111111111111111111112) causes the balance check to use only the wSOL token account balance, not the native SOL balance.
Weekly Installs
21
Repository
okx/plugin-store
