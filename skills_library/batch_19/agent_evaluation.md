...
url: https://skills.sh/mlflow/skills/agent-evaluation
---

# agent-evaluation

skills/mlflow/skills/agent-evaluation
agent-evaluation
Installation
...
Scorer output — Did all 3 scores come back as 0 or None? If so, the scorer is misconfigured (check return values — "pass"/"fail" are silently cast to None; use "yes"/"no" instead).
...
The script reads {scorer_name}/value and {scorer_name}/rationale columns from the CSV. It also accepts the legacy JSON format from mlflow traces evaluate for backward compatibility:
