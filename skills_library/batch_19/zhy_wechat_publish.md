---
title: zhy-wechat-publish
url: https://skills.sh/zhylq/yuan-skills/zhy-wechat-publish
---

# zhy-wechat-publish

skills/zhylq/yuan-skills/zhy-wechat-publish
zhy-wechat-publish
Installation
...
自动生成封面时，需保证 zhy-article-illustrator 的生图环境可用
安装说明
...
读取文章内容并提取标题/摘要
调用 zhy-article-illustrator/scripts/image-gen.ts 生成单张 16:9 封面图
调用 upload_image.js --json 上传封面，获得 thumb_media_id
调用 wechat_draft.js 推送草稿，并自动带上：
author
thumb_media_id
need_open_comment=1
...
微信 section 背景样式兼容
首个 h1 去除，避免与标题重复
正文图片上传到 cgi-bin/media/uploadimg
原生列表转换为普通段落列表
错误排查
