---
title: pancakeswap-v3-plugin
url: https://skills.sh/okx/plugin-store/pancakeswap-v3-plugin
---

# pancakeswap-v3-plugin

skills/okx/plugin-store/pancakeswap-v3-plugin
pancakeswap-v3-plugin
Installation
$ npx skills add https://github.com/okx/plugin-store --skill pancakeswap-v3-plugin
SKILL.md
...
Binary installed: pancakeswap --version — if not found, run the install script above
Wallet connected: onchainos wallet addresses — confirm wallet is logged in and active address is set
...
Fetch token metadata (decimals, symbol) via eth_call.
Check wallet balance via balanceOf — bail immediately with a human-readable error if insufficient (skipped in --dry-run).
Get best quote across all fee tiers via QuoterV2 eth_call.
Compute amountOutMinimum using the slippage tolerance.
Present the full swap plan (input, expected output, minimum output, fee tier, SmartRouter address).
Without --confirm: print preview calldata and exit.
With --confirm: submit Step 1 — ERC-20 approve via onchainos wallet contract-call (tokenIn → SmartRouter). Waits for on-chain confirmation before proceeding.
Submit Step 2 — exactInputSingle via onchainos wallet contract-call to SmartRouter.
...
Fetch pool address and current tick via slot0().
Tick range: if --tick-lower/--tick-upper are omitted, auto-compute ±10% price range (~±1000 ticks) from the current pool tick, aligned to tickSpacing. If provided, validate they are multiples of tickSpacing.
...
Slippage minimums: compute the actual deposit amounts using V3 liquidity math (based on current sqrtPrice and tick range), then apply slippage tolerance to those amounts. This prevents "Price slippage check" reverts caused by applying slippage to desired amounts instead of actual amounts.
...
Submit Step 3 — mint(MintParams) to NonfungiblePositionManager.
...
Fetch position data (pair, tick range, liquidity) via eth_call on NonfungiblePositionManager.
Fetch current pool price via slot0().
Slippage minimums: compute expected token amounts using V3 liquidity math (based on current sqrtPrice, tick range, and liquidity to remove), then apply slippage tolerance. This ensures sandwich protection even when tokensOwed = 0 (new positions with no accrued fees).
Present the full plan (expected out, min amounts, owed fees).
Submit Step 1 — decreaseLiquidity to NonfungiblePositionManager. Credits tokens back to the position but does NOT transfer them.
Submit Step 2 — collect to NonfungiblePositionManager. Transfers the credited tokens to the wallet.
