---
title: pendle-plugin
url: https://skills.sh/okx/plugin-store/pendle-plugin
---
...
Re-run with --confirm to execute on-chain
...
Run without any flags to get a real SDK preview — binary calls the Pendle SDK, returns calldata + "preview":true, no on-chain action
Show the user: amount in, expected amount out (expected_*_out), implied APY (for PT), price impact (price_impact_pct)
Ask user to confirm before executing on-chain
If price_impact_pct > 5%, surface the warning field prominently before asking for confirmation. Note: price_impact_pct is a relative metric vs the pool's theoretical rate — for cross-asset routes it may appear elevated on small amounts even when the trade is profitable. Always cross-check expected_token_out when a warning fires.
Execute only after explicit user approval — re-run with --confirm
Report approve tx hash(es) (approve_txs), main tx_hash, and outcome
...
Run without flags to preview — binary calls SDK and returns calldata + "preview":true with no on-chain action
Show preview to user — display expected_pt_out (PT you will receive) and ask for confirmation
Re-run with --confirm to execute; binary handles ERC-20 approval (if needed) then the swap
Return tx_hash confirming PT received
...
Run without flags for preview (returns "preview":true, no on-chain action)
Show preview — display expected_token_out (tokens you will receive) and price_impact_pct
If warning is present (price impact > 5%) — surface it prominently before asking for confirmation; cross-check expected_token_out to verify actual output
Ask user to confirm, then re-run with --confirm
Submit PT approval if required
Binary calls onchainos wallet contract-call to submit the swap transaction
Return tx_hash
...
Run without flags for preview (returns "preview":true, no on-chain action)
Show preview — display expected_yt_out (YT you will receive); remind user that YT is a leveraged yield position
Ask user to confirm, then re-run with --confirm
...
Run without flags for preview (returns "preview":true, no on-chain action)
Show preview — display expected_token_out and price_impact_pct
If warning is present (price impact > 5%) — surface it prominently before asking for confirmation; cross-check expected_token_out to verify actual output
Ask user to confirm, then re-run with --confirm
...
Run without flags for preview (returns "preview":true, no on-chain action)
Show preview — display expected_lp_out (LP tokens you will receive); ask user to confirm
Re-run with --confirm to execute; submit input token approval if required
Binary calls onchainos wallet contract-call to submit the liquidity transaction
Return tx_hash and expected_lp_out
...
Run without flags for preview (returns "preview":true, no on-chain action)
Show preview — display expected_token_out (tokens you will receive); ask user to confirm
Re-run with --confirm to execute; submit LP token approval if required
Binary calls onchainos wallet contract-call to submit the removal transaction
Return tx_hash and expected_token_out
...
Run without flags for preview (returns "preview":true, no on-chain action)
Show preview — display expected_py_out (PT+YT amount you will receive); ask user to confirm
Re-run with --confirm to execute; submit input token approval if required
Binary calls onchainos wallet contract-call to submit the mint transaction
Return tx_hash and expected_py_out
...
Run without flags for preview (returns "preview":true, no on-chain action)
Show preview — display expected_token_out (underlying tokens you will receive); ask user to confirm
Re-run with --confirm to execute; submit PT and/or YT approvals if required (checked separately for each)
Binary calls onchainos wallet contract-call to submit the redemption transaction
Return tx_hash and expected_token_out
