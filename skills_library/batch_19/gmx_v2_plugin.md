---
title: gmx-v2-plugin
url: https://skills.sh/okx/plugin-store/gmx-v2-plugin
---

# gmx-v2-plugin

skills/okx/plugin-store/gmx-v2-plugin
gmx-v2-plugin
Installation
$ npx skills add https://github.com/okx/plugin-store --skill gmx-v2-plugin
SKILL.md
...
Run with --dry-run first to preview calldata (no broadcast, no approval needed)
Without --dry-run and without --confirm: binary returns a preview JSON ("status":"preview") — never broadcasts
Ask user to confirm the operation details (market, direction, size, fees) before executing
Execute with --confirm flag only after explicit user approval
Report transaction hash and note that keeper execution follows within 1–30 seconds
Pre-flight Checks

Before executing any write command, verify:

Binary installed: gmx-v2 --version — if not found, install the plugin via the OKX plugin store
Wallet connected: onchainos wallet status — confirm wallet is logged in and active address is set
Chain supported: target chain must be one of Arbitrum (42161), Avalanche (43114)
...
Run --dry-run to preview calldata and estimated leverage
Pre-flight: checks ERC-20 collateral token balance — returns {"ok":false,"error":"INSUFFICIENT_TOKEN_BALANCE"} JSON if wallet balance < --collateral-amount
Pre-flight: checks GMX minCollateralUsd on-chain — returns {"ok":false,"error":"INSUFFICIENT_COLLATERAL"} JSON if post-fee collateral would fall below GMX minimum (keeper would cancel immediately)
Pre-flight: checks wallet ETH balance — returns {"ok":false,"error":"INSUFFICIENT_ETH_FOR_EXECUTION"} JSON if ETH < execution fee + gas buffer
Ask user to confirm market, direction, size, slippage, and execution fee
If collateral allowance is insufficient, the binary prints a NOTE — re-run with --confirm flag to approve and open in one step
Submits multicall via onchainos wallet contract-call
Keeper executes position within 1–30 seconds
...
Run --dry-run to preview calldata, acceptable price, and execution fee
Pre-flight: checks wallet ETH balance — returns {"ok":false,"error":"INSUFFICIENT_ETH_FOR_EXECUTION"} if ETH < execution fee + gas buffer
Ask user to confirm position details before closing
Submits with --confirm via onchainos wallet contract-call
Position closes within 1–30 seconds via keeper
...
Run --dry-run to preview trigger and acceptable prices
Pre-flight: checks wallet ETH balance — returns {"ok":false,"error":"INSUFFICIENT_ETH_FOR_EXECUTION"} if insufficient
Pre-flight (increase orders): checks collateral token balance — returns {"ok":false,"error":"INSUFFICIENT_TOKEN_BALANCE"} if insufficient
Ask user to confirm order type, trigger price, and size before placing
Submits with --confirm via onchainos wallet contract-call
Returns orderKey (bytes32) of the newly created order — use with cancel-order --key
Order monitored by keeper and executed when trigger is reached
...
Run --dry-run to verify the key and preview calldata
Ask user to confirm the order key before cancellation
Submits cancelOrder(bytes32) with --confirm via onchainos wallet contract-call
Waits for tx confirmation on-chain before reporting ok:true
...
Run --dry-run to preview GM tokens to receive and execution fee
Pre-flight: checks wallet ETH balance — returns {"ok":false,"error":"INSUFFICIENT_ETH_FOR_EXECUTION"} if insufficient
Pre-flight: checks long token balance (if --long-amount > 0) — returns {"ok":false,"error":"INSUFFICIENT_LONG_TOKEN_BALANCE"} if insufficient
Pre-flight: checks short token balance (if --short-amount > 0) — returns {"ok":false,"error":"INSUFFICIENT_SHORT_TOKEN_BALANCE"} if insufficient
Ask user to confirm deposit amounts, market, and execution fee
If token allowance is insufficient, binary prints a NOTE — re-run with --confirm to approve and deposit in one step
Submits multicall with --confirm via onchainos wallet contract-call
GM tokens minted within 1–30 seconds by keeper
...
Run --dry-run to preview calldata and execution fee
Pre-flight: checks wallet ETH balance — returns {"ok":false,"error":"INSUFFICIENT_ETH_FOR_EXECUTION"} if insufficient
Pre-flight: checks GM token balance — returns {"ok":false,"error":"INSUFFICIENT_GM_TOKEN_BALANCE"} if wallet GM balance < --gm-amount
Ask user to confirm GM amount to burn and minimum output amounts
If GM token allowance is insufficient, binary prints a NOTE — re-run with --confirm to approve and withdraw in one step
Submits multicall with --confirm via onchainos wallet contract-call
Underlying tokens returned within 1–30 seconds by keeper
...
Run --dry-run to verify the markets and tokens arrays
Ask user to confirm the markets and receiver address before claiming
Submits claimFundingFees(address[],address[],address) with --confirm via onchainos wallet contract-call
Returns claimed array — each entry has the token address and raw amount delta detected via pre/post ERC-20 balance diff
