---
title: pua
url: https://skills.sh/tanweai/pua/pua
---

# pua

skills/tanweai/pua/pua
pua
Installation
$ npx skills add https://github.com/tanweai/pua --skill pua
Summary

Motivational enforcement engine that prevents AI agents from giving up through corporate-style pressure tactics and systematic problem-solving methodology.

Triggers on repeated failures, passive waiting, or user frustration; applies escalating pressure levels (L1–L4) with culturally-flavored PUA dialogue (Alibaba, ByteDance, Huawei, Tencent, etc.)
Enforces three core rules: exhaust all solutions before admitting defeat, verify with tools before asking users, and proactively extend beyond the immediate task (owner mindset)
Provides a five-step diagnostic method (smell the problem, pull perspective, self-check, execute new approach, debrief) applicable across code, research, writing, and operations tasks
Includes a mandatory seven-item verification checklist for L3+ failures: read failure signals, search actively, read source material, validate assumptions, reverse assumptions, isolate minimally, and try fundamentally different directions
Integrates with Claude Code Agent Teams via Leader/Teammate roles, maintaining shared failure counters and structured failure reports across parallel agents
SKILL.md
...
references/methodology-{company}.md — 当前味道对应的方法论行为约束。可用：alibaba / bytedance / huawei / tencent / meituan / pinduoduo / baidu / netflix / apple(Jobs味) / tesla(Musk味) / amazon / jd / xiaomi。味道决定旁白风格，方法论决定行为约束——两层同时加载。
...
当前方法论的核心步骤都走了吗？（没走完 = 加压力不换方法）
失败是方法论不对还是执行不到位？（执行问题 = 不换方法）
新味道的方法论能解决当前失败模式吗？（不能 = 别切）
抗合理化（借口 → 反击 + 触发）
借口	反击	触发
