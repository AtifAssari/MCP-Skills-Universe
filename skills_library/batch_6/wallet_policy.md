---
title: wallet-policy
url: https://skills.sh/starchild-ai-agent/official-skills/wallet-policy
---

# wallet-policy

skills/starchild-ai-agent/official-skills/wallet-policy
wallet-policy
Installation
...
Ask about chain first — Determine if the user needs EVM, Solana, or both policies.
Clarify addresses — If the user says "allow transfers to my friend", ask for the address.
Confirm amounts — Convert user-friendly amounts (e.g. "1 ETH") to wei/lamports in the output.
Explain trade-offs — If a policy is overly permissive, warn the user. If too restrictive, note what will be blocked.
Combine smartly — Use multiple conditions per rule for AND, multiple rules for OR.
Always include deny-export — Recommend a DENY exportPrivateKey rule unless the user explicitly needs key export.
Output valid JSON — Every json:policy block must be valid, parseable JSON. Double-check addresses and values.
Rule name ≤ 50 characters — Privy API enforces a 50-character limit on rule names. NEVER include full on-chain addresses in rule names. Use short descriptive names like "Allowlist recipients", "Max 0.1 ETH per tx", or "Allow SOL to treasury". If you need to reference an address, abbreviate it (e.g. 0xba86...BE6E).
Do NOT
