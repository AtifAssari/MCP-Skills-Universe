...
url: https://skills.sh/mongodb/agent-skills/atlas-stream-processing
---

# atlas-stream-processing

skills/mongodb/agent-skills/atlas-stream-processing
atlas-stream-processing
Installation
...
Wait for confirmation before calling atlas-streams-teardown
...
Validate connections: atlas-streams-discover → list-connections + inspect-connection for each — verify names match targets, present summary to user
Call search-knowledge to validate field names. Fetch relevant examples from https://github.com/mongodb/ASP_example
...
Start with basic $source → $merge pipeline (validate connectivity)
Add $match stages (validate filtering)
Add $addFields / $project transforms (validate reshaping)
