...
url: https://skills.sh/jkeskikangas/skills/reviewing-skills
---
...
Look for directories in CWD that contain SKILL.md.
...
Read the minimum necessary context (in order)
<skill>/SKILL.md
<skill>/agents/openai.yaml (if present)
Any files under <skill>/scripts/ referenced by SKILL.md (only those)
Any files under <skill>/references/ referenced by SKILL.md (only those)
(If in a git repo) gather change context
...
Identify issues and merge duplicates
