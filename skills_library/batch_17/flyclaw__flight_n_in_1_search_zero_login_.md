---
title: flyclaw (flight n-in-1 search zero login)
url: https://skills.sh/ai4mse/flyclaw/flyclaw-(flight-n-in-1-search-zero-login)
---

# flyclaw (flight n-in-1 search zero login)

skills/ai4mse/flyclaw/flyclaw (Flight N-in-1 Search Zero Login)
flyclaw (Flight N-in-1 Search Zero Login)
Installation
$ npx skills add https://github.com/ai4mse/flyclaw --skill 'flyclaw (Flight N-in-1 Search Zero Login)'
SKILL.md
...
功能 / Features
按航班号查询：查询指定航班的动态信息（状态、时刻、延误、机型等）
按航线搜索：查询两地之间的航班列表（含价格、经停、时长等）
城市级搜索：输入城市名自动搜索该城市所有机场（如"上海"→PVG+SHA）
高级搜索：往返搜索、多旅客配置、舱位选择、排序、直飞过滤
中英文输入：支持中文城市名、英文名、IATA 代码，7912 机场全覆盖
