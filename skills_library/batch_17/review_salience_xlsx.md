---
title: review-salience-xlsx
url: https://skills.sh/timlai666/skills/review-salience-xlsx
---

# review-salience-xlsx

skills/timlai666/skills/review-salience-xlsx
review-salience-xlsx
Installation
...
Score every review on every attribute (salience 0–7) → review × attribute matrix
Reduce attributes to latent dimensions via PCA
...
A table with one row per review and one column per attribute, plus metadata columns (review_id, product, review_text). Column names follow the pattern s01, s02 … sN. This is the input to both PCA and K-means.
...
Read each review, understand its meaning, assign scores. No keyword matching. See references/worked-example.md for calibration examples.
...
Standardise: X_std = StandardScaler().fit_transform(X)
Fit PCA: use Kaiser criterion (eigenvalue > 1) to choose n_components
Compute factor loadings: loadings = components_.T × sqrt(eigenvalues_)
Name each PC by its dominant loadings (|≥ 0.30|)
Save PC scores matrix for clustering: PC = pca.fit_transform(X_std)
...
After convergence: call km.predict(ALL_reviews) — never discard pruned reviews
Name clusters by PC centroid pattern and top attribute means
Step 7 — Output
...
Never discard pruned reviews. After iterative pruning converges, use km.predict() on the full corpus to assign every review — including those removed during pruning — to the nearest final centroid.
Use absolute column widths. Percentage widths break in Google Sheets.
Present with present_files. Never ask the user to navigate to the file.
Weekly Installs
12
Repository
timlai666/skills
