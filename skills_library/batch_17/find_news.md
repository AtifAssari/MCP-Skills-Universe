---
title: find-news
url: https://skills.sh/lazyboyjgn99/find-news/find-news
---

# find-news

skills/lazyboyjgn99/find-news/find-news
find-news
Installation
$ npx skills add https://github.com/lazyboyjgn99/find-news --skill find-news
SKILL.md
Find News 技能
概述
...
最佳实践
默认使用 max_results: 3, crawl_results: 0（快速获取摘要）
只有用户明确要求"完整内容"时 才设置 crawl_results > 0
搜索前 先检查是否有缓存（相同查询 30 分钟内免费）
合理控制数量 max_results 不要设置过大，避免不必要的成本
注意事项
默认使用 crawl_results: 0 以获得快速响应
设置 crawl_results > 0 会显著增加延迟（10-30秒/页）
注意控制请求频率，避免超出限流
相同查询 30 分钟内会使用缓存，免费且更快
所有请求必须在 Header 中包含 Authorization: Bearer sk_test_a6f84bf78896f10b2d28aebd7857744c
