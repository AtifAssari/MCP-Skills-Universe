---
title: w00-workflow-checkpoint
url: https://skills.sh/qiao-925/qiao-skills/w00-workflow-checkpoint
---

# w00-workflow-checkpoint

skills/qiao-925/qiao-skills/w00-workflow-checkpoint
w00-workflow-checkpoint
Installation
$ npx skills add https://github.com/qiao-925/qiao-skills --skill w00-workflow-checkpoint
SKILL.md
...
返回：最后状态 + 下一步
存档判定流程（更新哪一个）
用户给出 #issue：直接更新该 issue。
上下文有绑定 Issue #：更新该 issue。
否则拉取当前仓库候选 issue（open + repo:*）。
分支：
0 条：触发下文“额外条件（创建并存档）”。
1 条：自动选中并回报。
...
读档判定流程（读哪一个）
用户给出 #issue：直接读档。
上下文有绑定 Issue #：优先读档该 issue。
否则列出当前仓库候选（优先 status:in-progress / status:blocked）。
确定后输出读档摘要。
