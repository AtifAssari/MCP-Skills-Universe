---
title: paw-ps-audience
url: https://skills.sh/pawbytes/skill-suites/paw-ps-audience
---

# paw-ps-audience

skills/pawbytes/skill-suites/paw-ps-audience
paw-ps-audience
Installation
...
curated/product-context.md — What product are we building? What stage?
curated/audience-intelligence.md — What do we already know? Skip questions already answered.
curated/market-intelligence.md — What market signals inform audience understanding?
...
Read before asking — Check audience-intelligence.md for existing answers. Never re-ask answered questions.
Identify capability needed — Based on user request, determine which capability applies.
Load reference — Read the appropriate reference file for methodology.
Execute method — Follow the reference guidance to produce the output.
Write to memory — Update audience-intelligence.md with findings.
Log activity — Append to daily log with timestamp and summary.
Recommend next step — Suggest related capability or handoff to strategist.
Headless Mode

When invoked with --headless or -H:

Named task detection — If specific task (e.g., --headless:personas), execute only that capability.
Full synthesis — If no named task, run all capabilities based on available context.
Write outputs — Save all findings to audience-intelligence.md.
Log completion — Append summary to daily log.
Return summary — Brief overview of what was produced.
...
Persona construction → Load ./references/persona-construction.md
Problem discovery → Load ./references/problem-discovery.md
Value mapping → Load ./references/value-signals.md
Language capture → Load ./references/language-capture.md
