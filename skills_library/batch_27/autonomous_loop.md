...
url: https://skills.sh/pixel-process-ug/superkit-agents/autonomous-loop
---

# autonomous-loop

skills/pixel-process-ug/superkit-agents/autonomous-loop
autonomous-loop
Installation
...
Steps
Knowledge Gathering — Deploy up to 250 parallel subagents via the Agent tool (with subagent_type="Explore" and model="sonnet") to study specs, existing implementation plans, and utility libraries
Code Analysis — Deploy up to 500 parallel subagents via the Agent tool (with subagent_type="Explore") to study src/* against specs/*, identifying gaps between specification and implementation
Synthesis — Deploy a synthesis subagent via the Agent tool (with model="opus") to synthesize findings and prioritize incomplete work
Plan Refresh — Update IMPLEMENTATION_PLAN.md as organized, prioritized bullet list
