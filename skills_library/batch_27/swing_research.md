...
url: https://skills.sh/whynowlab/swing-skills/swing-research
---

# swing-research

skills/whynowlab/swing-skills/swing-research
swing-research
Installation
$ npx skills add https://github.com/whynowlab/swing-skills --skill swing-research
SKILL.md
...
Gotchas
Never invent a URL. If search returns nothing, say "Unverified (no source found)" — do not guess a plausible-looking URL. Fabricated sources destroy all credibility.
Two articles citing the same study = 1 source. Trace claims to their origin. A press release quoted by 5 news sites is still one source, not five.
Vendor benchmarks are Tier B, not A. Even official engineering blogs have bias when benchmarking their own product. Label the bias explicitly.
Hedging is not labeling. "아마도", "seems like", "probably" are banned. Either a claim is Verified (2+ independent sources) or it gets an explicit Unverified/Contested label.
Don't skip the Verification Matrix. It's the accountability layer. Without it, the report looks rigorous but has no audit trail.
Integration Notes
