---
title: xano-init
url: https://skills.sh/xano-inc/xano-developer-mcp/xano-init
---

# xano-init

skills/xano-inc/xano-developer-mcp/xano-init
xano-init
Installation
...
Run xano profile list to see existing profiles
Check if any profile already targets this workspace (match by workspace ID)
If a suitable profile exists, confirm with the user: "I see profile [name] targeting workspace [id]. Should I use this, or create a new one?"
If the branch isn't set or is wrong on the existing profile, update it: xano profile edit [name] -b [dev-branch]
...
Store the profile name — it will be used with -p [profile] for all subsequent CLI commands in the playbook.
...
Pull workspace: xano workspace pull ./xano -p [profile] -b [dev-branch]
Edit .xs files
Validate XanoScript
Preview changes: xano workspace push ./xano -p [profile] -b [dev-branch] --dry-run
Push: xano workspace push ./xano -p [profile] -b [dev-branch]
Deploying to live
[specific steps based on user's merge/promotion workflow]
No-Go Zones
...
Environment status (what's working)
Workspace connected and status confirmed
Where the playbook was saved
The top 3 rules they should know
Any recommendations (e.g., "You should create a dev branch", "Consider setting up separate data sources", "Your CLI is outdated")
