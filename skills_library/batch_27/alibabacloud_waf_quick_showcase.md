---
title: alibabacloud-waf-quick-showcase
url: https://skills.sh/aliyun/alibabacloud-aiops-skills/alibabacloud-waf-quick-showcase
---

# alibabacloud-waf-quick-showcase

skills/aliyun/alibabacloud-aiops-skills/alibabacloud-waf-quick-showcase
alibabacloud-waf-quick-showcase
Installation
$ npx skills add https://github.com/aliyun/alibabacloud-aiops-skills --skill alibabacloud-waf-quick-showcase
SKILL.md
...
Enable AI-Mode (before any CLI commands):
...
Disable AI-Mode (after workflow completion):

aliyun configure ai-mode disable


[MUST] CLI User-Agent — Every aliyun CLI command invocation must include: --user-agent AlibabaCloud-Agent-Skills/alibabacloud-waf-quick-showcase
...
CLI Version: Run aliyun version to confirm version >= 3.3.3. If not installed or version too low, run curl -fsSL https://aliyuncli.alicdn.com/setup.sh | bash to install/update. Then [MUST] run aliyun plugin update to ensure that any existing plugins on your local machine are always up-to-date.
...
Account Balance: Confirm Alibaba Cloud account balance >= 100 CNY
...
Region: cn-hangzhou (or other regions you prefer, such as cn-shanghai, cn-beijing)
VPC CIDR Block: 192.168.0.0/16
Zone: cn-hangzhou-j
ECS Specification: ecs.e-c1m2.large
ECS Password: Please provide your ECS login password (8-30 characters, containing uppercase letters, lowercase letters, and numbers)
...
Select an existing VPC from the response results (prioritize those with names containing "waf" or "test")
Prompt the user: "Your VPC quota is full, will use existing VPC (VpcId: [xxx]) to continue deployment."
Create new VSwitch, Security Group, and ECS under this VPC
...
Stop Immediately: Must not continue executing subsequent steps
Inform User: Clearly explain the failure reason (such as insufficient resources, insufficient permissions, insufficient quota, etc.)
Prohibit Substitution: Must not use existing resources instead
...
Note: When creating a WAF instance for the first time, you need to complete the service linked role authorization in the console. If CLI reports an error indicating authorization is required, please visit WAF Console and click "Create Service Linked Role" to complete authorization before trying again.
