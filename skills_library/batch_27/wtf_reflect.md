---
title: wtf.reflect
url: https://skills.sh/xiduzo/wtf/wtf.reflect
---

# wtf.reflect

skills/xiduzo/wtf/wtf.reflect
wtf.reflect
Installation
...
If Create them now → invoke wtf.steer-tech. Note that wtf.steer-tech will offer to chain to the other steer-* skills at the end — let the user complete that flow, then return here. When control returns, re-run the ls check to see which docs now exist. If Skip → set all four doc paths to the fallback: docs/steering/LEARNINGS.md.
...
If the target doc does not exist (from the map built in step 1) → use docs/steering/LEARNINGS.md instead. Create it with a # Overflow Learnings heading if it doesn't exist yet.
Read the current file.
Look for a ## Hard-Won Lessons section.
If it exists → append the new bullet(s) under it.
If it does not exist → append the section at the end of the file, inserting it before the <!-- MANUAL ADDITIONS START --> marker if present.
