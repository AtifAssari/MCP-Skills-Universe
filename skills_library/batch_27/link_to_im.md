---
title: link-to-im
url: https://skills.sh/d-wwei/agents-to-im/link-to-im
---

# link-to-im

skills/d-wwei/agents-to-im/link-to-im
link-to-im
Installation
...
Interactive skill hosts — AskUserQuestion tool is available. Use it for interactive setup wizards.
Non-interactive hosts — AskUserQuestion is NOT available. Fall back to non-interactive guidance: explain the steps, show SKILL_DIR/config.env.example, and ask the user to create ~/.link-to-im/config.env manually.
...
The helper writes ~/.agent-to-im/runtime/weixin-login.html and tries to open it automatically in the local browser.
...
Use Bash to create directory structure: mkdir -p ~/.link-to-im/{data,logs,runtime,data/messages}
Use Write to create ~/.link-to-im/config.env with all settings in KEY=VALUE format
Use Bash to set permissions: chmod 600 ~/.link-to-im/config.env
...
On success, tell the user: "Setup complete! Run /link-to-im start to start the bridge."
start
...
reconfigure
Read current config from ~/.link-to-im/config.env
...
Remind user: "Run /link-to-im stop then /link-to-im start to apply the changes."
