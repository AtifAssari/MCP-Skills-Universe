---
title: orchestrator-v4
url: https://skills.sh/site/skills.volces.com/orchestrator-v4
---

# orchestrator-v4

skills/skills.volces.com/orchestrator-v4
orchestrator-v4
$ npx skills add https://skills.volces.com/skills/clawhub/eviost
SKILL.md
Orchestrator V4
...
告知用户关键发现（一句话概括 + 2-3 个 [重要] 标记）
更新进度：N/总数 完成
前一批全部完成后，立即派发下一批（重复 Step 3-4）
小模块可合并派发：一个子代理负责 3-5 个小模块
Step 6：汇总报告
...
派子代理时必须输出任务表格
子代理完成时立即通知用户进度和关键发现
执行过程中主会话保持可交互，用户随时可打断
分析类任务的子代理 prompt 必须包含文件读取约束
超时的子任务缩小范围重跑，不原样重试
小模块合并到一个子代理时，prompt 里列出所有模块的目录
用户说"暂停"时停止派发新子代理，说"继续"时恢复
空结果检测与处理：子代理返回结果后，检查内容是否有效：
空结果判定条件（满足任一即视为空结果）：
结果长度 < 100 字符
...
处理流程：
先检查磁盘：确认子代理是否已创建/修改了目标文件（检查文件是否存在且非空）
如果文件已创建且内容完整 → 任务实际完成，无需重跑，标记为"文件已就位，总结被截断"
如果文件不存在或为空 → 真正的空结果，缩小范围重跑
预防措施：
...
默认排除目录：扫描器自动排除 .next、node_modules、.git、dist、build、out、.turbo、coverage 等构建产物和依赖目录，无需手动跳过
修复模式（v2.2 新增）
