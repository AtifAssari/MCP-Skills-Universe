---
title: openspec-task-loop
url: https://skills.sh/luongnv89/skills/openspec-task-loop
---
...
Workflow Decision
If /opsx:* commands are supported in the current coding tool: use native OPSX flow.
If not supported: use manual fallback by creating files under openspec/changes/<change-id>/.
...
Ensure OpenSpec tree exists (openspec/changes, openspec/specs).
Scaffold one change folder with:
proposal.md
design.md
tasks.md
specs/<capability>/spec.md
Read references/openspec-task-templates.md before scaffolding task artifacts.
Implement task and update checkboxes.
Run local validation/tests.
Merge spec deltas into openspec/specs/ and move change to openspec/changes/archive/<date>-<change-id>/.
