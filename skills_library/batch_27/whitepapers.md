---
title: whitepapers
url: https://skills.sh/gtmagents/gtm-agents/whitepapers
---

# whitepapers

skills/gtmagents/gtm-agents/whitepapers
whitepapers
Installation
$ npx skills add https://github.com/gtmagents/gtm-agents --skill whitepapers
SKILL.md
Whitepapers Skill
When to Use
Deep-dives for regulated or technical buyers (finance, healthcare, energy).
Support ABM campaigns with detailed solution explainers.
Repurpose product research into demand-gen anchors.
Framework
Audience & Compliance – confirm regulatory constraints, required disclaimers, reading level.
Outline – Executive summary, background, methodology, findings, recommendations, CTA.
Research Layer – cite third-party analysts, customer benchmarks, proprietary data.
Design Considerations – include charts, call-out boxes, and glossary; ensure accessibility.
Peer Review – involve product/legal SMEs for accuracy.
Templates
Draft manuscript with references + citations.
Data appendix or methodology section.
Design brief with layout guidance.
Campaign kit (landing page copy, nurture email, social teasers).
Tips
Maintain a citation log to accelerate compliance reviews.
Use modular sections so data refreshes don’t require full rewrites.
Pair launch with derivative assets (blogs, webinars) to extend shelf life.
Version-control legal-approved language for regulated industries.
Weekly Installs
59
Repository
gtmagents/gtm-agents
GitHub Stars
180
First Seen
Feb 27, 2026
Security Audits
Gen Agent Trust HubPass
SocketPass
SnykPass