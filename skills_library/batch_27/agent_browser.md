---
title: agent-browser
url: https://skills.sh/clawdbrunner/skill-agent-browser/agent-browser
---

# agent-browser

skills/clawdbrunner/skill-agent-browser/agent-browser
agent-browser
Installation
...
Tips for AI Agents
Always use --json — Structured output is easier to parse than text
Use -i flag — Interactive-only snapshots are smaller, faster, cleaner
Re-snapshot after actions — Verify state changed as expected
Trust refs over selectors — @e2 from snapshot > #id that might change
Use semantic locators when refs expire — find role button click is robust
Session persistence — One open, many commands, one close
