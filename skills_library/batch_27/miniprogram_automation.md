---
title: miniprogram-automation
url: https://skills.sh/whinc/wechat-miniprogram-skills/miniprogram-automation
---

# miniprogram-automation

skills/whinc/wechat-miniprogram-skills/miniprogram-automation
miniprogram-automation
Installation
...
开发者工具必须完全退出（Cmd+Q），否则报 Failed to launch wechat web devTools——launch 不能与已运行实例共存。
首次运行弹出「信任项目」确认框，需用户手动点击，因此 timeout 建议 120000（2 分钟）。
...
await page.waitFor('稳定选择器')
await page.waitFor(async () => 条件成立)
await page.waitFor(数字毫秒) 作为兜底
...
miniprogram-automator 版本是否 >= 0.12.0（旧版会报 code 31）
开发者工具是否已完全退出（launch() 不能与已运行实例共存）
开发者工具是否已安装并登录
CLI 路径是否正确
安全设置 → 服务端口是否已开启
projectPath 是否真的是开发者工具打开的目录（含 project.config.json）
是否设置了足够长的 timeout（首次启动建议 120000）
