...
url: https://skills.sh/luongnv89/skills/release-manager
---

# release-manager

skills/luongnv89/skills/release-manager
release-manager
Installation
...
Version changes — read $WORKSPACE/version-bumper/version-changes.md and show which files will be updated
Changelog — read $WORKSPACE/changelog-generator/changelog-entry.md and show the formatted entry
Doc updates — read $WORKSPACE/docs-updater/docs-changes.md and show proposed documentation changes
...
Update version strings in all files identified by the version-bumper
Prepend the changelog entry to CHANGELOG.md (create the file with a # Changelog header if it doesn't exist). Do NOT create or update a RELEASE_NOTES.md file
Apply documentation updates identified by the docs-updater
Step 7: Build (inline)
...
Ask the user: "This project has a build step (<command>). Should I run it?"
Run the build
If it fails, stop and help debug — do not continue with a broken build
