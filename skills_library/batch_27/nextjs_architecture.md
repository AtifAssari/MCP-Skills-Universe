...
url: https://skills.sh/alpoxdev/hypercore/nextjs-architecture
---

# nextjs-architecture

skills/alpoxdev/hypercore/nextjs-architecture
nextjs-architecture
Installation
...
project mode still matches the edited surface (app/, pages/, or mixed)
route segment file placement is valid
'use client' boundaries are as small as possible
client code does not import server-only modules or private env
data freshness after mutations is explicit (revalidatePath, revalidateTag, redirect flow, or documented alternative)
