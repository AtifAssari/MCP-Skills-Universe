---
title: wtf.verify-task
url: https://skills.sh/xiduzo/wtf/wtf.verify-task
---

# wtf.verify-task

skills/xiduzo/wtf/wtf.verify-task
wtf.verify-task
Installation
...
Ask "Did this scenario pass?" — header Result:
...
After recording the result, immediately update the Task issue with the current state of the Test Mapping table (do not wait until all scenarios are done). The table must include a Bug Filed column:
...
Ask "Did this edge case pass?" — header Result:
...
Ask "Was this observability item present and correct?" — header Result:
