---
title: codex
url: https://skills.sh/alpoxdev/hypercore/codex
---

# codex

skills/alpoxdev/hypercore/codex
codex
Installation
$ npx skills add https://github.com/alpoxdev/hypercore --skill codex
SKILL.md
...
"Research Codex sandbox modes and tell me what they do." Use this skill only if the user wants the codex CLI involved; otherwise route to research or direct documentation work.
Critical: Non-Interactive Mode

Always use the codex exec subcommand for non-interactive Codex runs. Calling codex "prompt" without a subcommand starts the interactive TUI.
...
Use codex exec resume --last for the latest non-interactive run. Use codex exec resume <session-id> when the user wants a specific session or the latest run is not the right one. When resuming, keep the existing session behavior unless the user explicitly asks to change the model, profile, sandbox, or approval policy. Use codex fork --last only when the user wants to branch from the existing session instead of reusing it.
