---
title: meihua-yishu
url: https://skills.sh/jwcodewrote/agent_skills_plugin/meihua-yishu
---

# meihua-yishu

skills/jwcodewrote/agent_skills_plugin/meihua-yishu
meihua-yishu
Installation
$ npx skills add https://github.com/jwcodewrote/agent_skills_plugin --skill meihua-yishu
SKILL.md
...
Local Calculation (Python): Precise hexagram generation using meihua_calc.py
Strategy Database (References): Traditional rules from hexagram-strategy.md
AI Deep Reading (Gemini API): Modern interpretation with web search grounding
...
✅ 无需 API Key
解卦 SOP (8步流程)
查阅爻辞 → references/yaoci.md
确定体用 → 动爻位置决定
五行生克分析 → 体用关系判断吉凶
分析互卦 (中期发展) → 2-4爻, 3-5爻
分析变卦 (最终结果) → 动爻变化
分析卦象关系 → references/hexagram-relationships.md
推断应期 → references/yingqi-calc.md
策略建议 (必做!) → references/hexagram-strategy.md
Reference Materials
