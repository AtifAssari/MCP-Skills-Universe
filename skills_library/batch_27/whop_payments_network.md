---
title: whop-payments-network
url: https://skills.sh/whopio/whop-payments-network-skill/whop-payments-network
---

# whop-payments-network

skills/whopio/whop-payments-network-skill/whop-payments-network
whop-payments-network
Installation
...
Top-ups — charge a saved payment method (ACH, card). Best for platforms that collect funds externally (wire, invoice) and need to fund their Whop balance for payouts.
Direct charges — customers pay connected accounts directly, platform takes an application_fee_amount. Money flows through checkout.
Transfers model — customers pay the platform via checkout, platform distributes to connected accounts via transfers.create().
...
12. Common Gotchas
Transfers use cents, Withdrawals use dollars — transfers.create({ amount: 4500 }) = $45.00, but withdrawals.create({ amount: 45 }) = $45.00.
application_fee_amount must be > 0 AND < total price — zero or equal-to-total will error.
Webhook event field, not type — The webhook body uses event as the key. Some docs incorrectly show type.
Webhook events may use underscores — membership_went_valid instead of membership.went.valid. Normalize with .replace(/_/g, ".").
returnUrl is required for external payment methods — Apple Pay, Google Pay, PayPal redirects fail without it.
Webhook secret must be base64-encoded — Pass btoa(process.env.WHOP_WEBHOOK_SECRET) to SDK's webhookKey.
Return 2xx quickly from webhooks — Whop retries on timeout. Use waitUntil() or background jobs for heavy processing.
Access tokens expire — Default 1 hour, max 3 hours. Refresh before expiry for embedded components.
Company metadata SDK typing gap — Update metadata via type cast: (client.companies as any).update(id, { metadata }).
SDK init without appID is valid — Company API Keys don't need appID.
Checkout config response includes purchase_url and plan.id — Use these for redirect flows or plan references.
Sandbox checkout URL — https://sandbox.whop.com/checkout/{planId}.
setupFutureUsage: "off_session" — Required on checkout embed when you plan to charge the user later via chargeUser API.
Permission re-approval — After adding new app permissions, API calls fail with 403 until the company re-approves.
Always use idempotence keys — On transfers and any money-movement operation to prevent duplicates.
Payout methods pagination duplicates — Payout methods list may return duplicates during pagination — deduplicate by .id when consuming the full list.
Auto-withdraw after transfer — After approving a transfer, consider auto-initiating a withdrawal to the recipient's default payout method so they don't have to manually withdraw. Note: transfers use cents, withdrawals use dollars.
