...
url: https://skills.sh/glittercowboy/taches-cc-resources/create-meta-prompts
---

# create-meta-prompts

skills/glittercowboy/taches-cc-resources/create-meta-prompts
create-meta-prompts
Installation
...
Save: Create folder in .prompts/{number}-{topic}-{purpose}/
Present: Show decision tree for running
...
Create folder: .prompts/{number}-{topic}-{purpose}/
Create completed/ subfolder
Write prompt to: .prompts/{number}-{topic}-{purpose}/{number}-{topic}-{purpose}.md
Prompt instructs output to: .prompts/{number}-{topic}-{purpose}/{topic}-{purpose}.md </file_creation> </step_1_generate>
...
Archive prompt to completed/ subfolder
Report results with next-step options </single_prompt>
...
Parse each prompt for @.prompts/{number}-{topic}/ patterns
...
Execute normally </re_running_completed>
...
If conflict detected: Ask user - Overwrite? / Rename? / Cancel? </output_conflicts>
...
Maintains user control over prompt creation </recursive_prompts> </special_cases> </step_3_execute>
...
references/intelligence-rules.md - Extended thinking, parallel tools, depth decisions </reference_guides>
