...
url: https://skills.sh/manutej/luxor-claude-marketplace/kafka-stream-processing
---

# kafka-stream-processing

skills/manutej/luxor-claude-marketplace/kafka-stream-processing
kafka-stream-processing
Installation
...
Topics: Named categories for organizing messages
Partitions: Ordered, immutable sequences of records within topics
Brokers: Kafka servers that store and serve data
Producers: Applications that publish records to topics
Consumers: Applications that read records from topics
Consumer Groups: Coordinated consumers sharing workload
ZooKeeper/KRaft: Cluster coordination (ZooKeeper legacy, KRaft modern)
...
key.serializer / value.serializer: Data serialization
...
Use Batching: Configure linger.ms and batch.size for throughput
Enable Idempotence: Set enable.idempotence=true for reliability
Handle Errors: Implement proper callback error handling
Use Compression: Enable compression for large messages
Partition Keys: Use meaningful keys for ordered processing
Close Producers: Always close producers in finally blocks
Monitor Metrics: Track producer metrics (record-send-rate, compression-rate)
Resource Pools: Reuse producer instances when possible
Consumers
...
Choose Right Auto-commit: Disable for at-least-once semantics
Handle Rebalancing: Implement ConsumerRebalanceListener
Process Efficiently: Minimize poll() call duration
Graceful Shutdown: Close consumers properly
Monitor Lag: Track consumer lag metrics
Partition Assignment: Understand assignment strategies
Thread Safety: Kafka consumers are NOT thread-safe
...
Versioning: Track schema versions per subject
Schema Formats
...
Brokers: Typically 3+ brokers for fault tolerance
Replication: Replication factor 3 for production
Partitions: More partitions = more parallelism
ZooKeeper/KRaft: 3 or 5 nodes for quorum
...
Configure Acks Properly: Balance durability and throughput
Use Compression: Reduce network and storage costs
Batch Messages: Configure linger.ms and batch.size
Handle Retries: Implement proper retry logic
Monitor Metrics: Track send rates and error rates
Partition Strategy: Use meaningful keys for ordering
Close Gracefully: Call close() with timeout
Consumer Best Practices
Manual Offset Management: For at-least-once semantics
Handle Rebalancing: Implement ConsumerRebalanceListener
Minimize Poll Duration: Process efficiently
Monitor Consumer Lag: Alert on high lag
Thread Safety: One consumer per thread
Graceful Shutdown: Close consumers properly
Error Handling: Retry transient failures, DLQ for permanent
Seek Capability: Use seek() for replay scenarios
Kafka Streams Best Practices
Explicit Naming: Use Named, Grouped, Materialized for stability
State Store Management: Configure changelog topics properly
Error Handling: Implement ProductionExceptionHandler
Scaling: Match application instances to input partitions
Testing: Use TopologyTestDriver for unit tests
Monitoring: Track lag, processing rate, error rate
Exactly-Once: Enable for critical applications
...
Retention: Set based on use case (time or size)
Compaction: Use for changelog and lookup topics
Naming Convention: Consistent naming scheme
Documentation: Document topic purpose and schema
Access Control: Implement proper ACLs
Monitoring: Track partition metrics
Operational Best Practices
Monitoring: Comprehensive metrics collection
Alerting: Alert on critical metrics
Capacity Planning: Monitor disk, network, CPU
Backup: Implement disaster recovery strategy
Upgrades: Rolling upgrades with testing
...
Rolling Upgrades
Prepare: Test new version in staging
Upgrade Brokers: One broker at a time
Verify: Check cluster health after each broker
Upgrade Clients: Producers, consumers, streams apps
Monitor: Watch metrics throughout process
