...
url: https://skills.sh/openclaudia/openclaudia-skills/email-sequence
---

# email-sequence

skills/openclaudia/openclaudia-skills/email-sequence
email-sequence
Installation
$ npx skills add https://github.com/openclaudia/openclaudia-skills --skill email-sequence
SKILL.md
...
Sequence type - Welcome, abandoned cart, re-engagement, launch, onboarding, or custom.
Product/service - What is being sold or promoted?
Target audience - Who receives this sequence? Segment details.
Primary goal - Convert, activate, retain, re-engage, upsell?
Sender - Who is the "from" name and email? (Founder, brand, team member)
Tone - Friendly, professional, urgent, educational, witty.
ESP - What email platform? (Affects merge tags and HTML capabilities.)
Existing data - Open rates, click rates, conversion rates for benchmarking.
...
Email Structure
Hook (first 2 lines) - Must earn the scroll. Ask a question, state a bold claim, or reference something personal.
Body (3-8 sentences) - One idea per email. Do not try to cover everything.
CTA (1 primary) - One clear action. Link it 2-3 times in the body.
PS line (optional) - Second-most-read part of an email. Use for urgency or bonus info.
...
Body Copy Rules
One idea per email. Do not try to educate, sell, and announce in the same email.
Short paragraphs. 1-3 sentences maximum. Emails are read on mobile.
Use "you" more than "we." Flip the ratio: 3:1 "you" to "we."
Write at a 5th-8th grade level. Short words, short sentences.
Link the CTA 2-3 times. Once in the middle, once at the end, optionally in a PS.
Use a PS line. 79% of readers scan the PS. Use it for urgency or a secondary point.
...
Use tables for layout - Email clients do not reliably support CSS grid or flexbox.
Inline all CSS - Many email clients strip <style> blocks.
Max width: 600px - Standard email width for cross-client compatibility.
Use web-safe fonts - Arial, Helvetica, Georgia, Times New Roman.
Include alt text on all images - Many readers have images disabled.
Test in multiple clients - Gmail, Outlook, Apple Mail, Yahoo at minimum.
Mobile-first - 60%+ of emails are opened on mobile. Stack columns on small screens.
Include a plain-text version - Always provide a text fallback for deliverability.
...
The response returns a JSON object with an id field for tracking. Replace the from, to, subject, and html fields with the actual values from the generated sequence.
...
Important Notes
Domain verification required: The from address domain must be verified in your Resend account. If you have not verified a domain, Resend provides a shared onboarding@resend.dev address for testing.
