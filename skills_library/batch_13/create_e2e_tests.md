---
title: create_e2e_tests
url: https://skills.sh/shiplightai/agent-skills/create_e2e_tests
---

# create_e2e_tests

skills/shiplightai/agent-skills/create_e2e_tests
create_e2e_tests
Installation
$ npx skills add https://github.com/shiplightai/agent-skills --skill create_e2e_tests
SKILL.md
...
Open a browser session — call new_session with the app's starting_url.
Walk through the flow — use inspect_page to see the page, then act to perform each action. This captures locators from the response.
Capture locators — use get_locators for additional element info when needed.
Build the YAML — construct the .test.yaml content following the best practices below.
Save and validate — write the .test.yaml file, then call validate_yaml_test with the file path to check locator coverage (minimum 50% required).
Close the session — call close_session when done.
...
Update test-spec.md — mark skipped scenarios with reason, add scenarios that emerged during implementation, update edge cases to reflect what was tested
Update test-plan.md — correct file structure, note deviations from the original plan
Show diff summary — tell the user what changed and why
