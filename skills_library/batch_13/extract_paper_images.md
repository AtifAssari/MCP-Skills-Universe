---
title: extract-paper-images
url: https://skills.sh/juliye2025/evil-read-arxiv/extract-paper-images
---

# extract-paper-images

skills/juliye2025/evil-read-arxiv/extract-paper-images
extract-paper-images
Installation
$ npx skills add https://github.com/juliye2025/evil-read-arxiv --skill extract-paper-images
SKILL.md
...
查找源码包中的PDF文件（如dr_pipelinev2.pdf）
将PDF页面转换为PNG图片
...
Logo等非核心图片：PDF中的logo、图标、装饰元素被当成图片
矢量图无法识别：论文中的架构图可能是LaTeX矢量图（TikZ/PGFplots），不是独立图片对象
多层PDF结构：实验结果图可能是复杂渲染对象
优先级3：TikZ/PGFplots 矢量图处理（重要补充！）

当源码包中无图片文件且 PDF 中无嵌入式图片时（常见于纯 TikZ 论文）：

检测 TikZ 论文：在 .tex 文件中搜索 \begin{tikzpicture} 或 \usepackage{pgfplots}
查找源码中的独立 figure PDF：arXiv 源码有时包含由 TikZ 编译的 .pdf figure 文件
如果源码中有 figure PDF：用 PyMuPDF 将 .pdf 转为 .png
如果只有内联 TikZ 代码：
...
过滤小图标：只保留宽度 > 200px 或高度 > 200px 的图片

arXiv源码包的优势：

真正的论文图：pics/目录包含作者准备的原始图片
高质量：源码中的图通常是高分辨率矢量图
清晰命名：文件名描述图片内容（如dr_pipelinev2.pdf）
输出格式
图片索引文件（index.md）
# 图片索引
...
核心图片：images/final_results_combined_page1.png等（前3-5张）
