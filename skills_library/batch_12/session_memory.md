...
url: https://skills.sh/romiluz13/cc10x/session-memory
---

# session-memory

skills/romiluz13/cc10x/session-memory
session-memory
Installation
...
Read current plan, design, or research files from activeContext.md ## References only if the workflow needs them.
Load approved project/domain skills from ## Project SKILL_HINTS after memory is loaded.
If context is already getting heavy, apply references/context-budget-and-checkpointing.md.
During
...
End
Distill decisions, learnings, verification, and deferred items into MEMORY_NOTES.
Keep each note factual and reusable.
Assume the router/workflow finalizer will persist the markdown files.
Do not report the workflow as truly complete if the memory cycle is missing.
