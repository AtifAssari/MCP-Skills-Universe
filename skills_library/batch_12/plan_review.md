---
title: plan-review
url: https://skills.sh/duc01226/easyplatform/plan-review
---

# plan-review

skills/duc01226/easyplatform/plan-review
plan-review
Installation
...
Rules: NEVER rely on Round 1 memory for Round 2. Final verdict must incorporate BOTH rounds. Report must include ## Round 2 Findings section.
...
Resolve Plan — Use $ARGUMENTS path or active plan from ## Plan Context
...
If FAIL — Fix issues in plan files directly, then re-review (loop back to step 2, max 3 iterations)
...
Worked example of FAIL → PASS: FAILS: "Phase 2: Data Layer — Set up database models, Create repositories, Implement data access patterns. Effort: 4h, Files: ~8" PASSES after split: "Phase 2A: Database Schema (1h, 3 files) — Create src/models/user.entity.ts, Create src/models/session.entity.ts, Create migrations/001-create-users-sessions.ts" + "Phase 2B: Repository Layer (1.5h, 3 files) — Create src/repos/user.repository.ts, Create src/repos/session.repository.ts, Register in src/app.module.ts"
...
Detect plan type from keywords: Code/Feature, Documentation, Refactor, Infrastructure, Mixed
State top 3 concerns specific to THIS plan (not generic)
Check complexity from frontmatter SP: ≤3 → 2 rounds min, 4-8 → 3 rounds, >8 → 3 rounds + code-proof mandatory
Round 1: Structural Validation (Checklist)

Standard checklist from Step 2 above. Score PASS/WARN/FAIL per category. CRITICAL: Even if all checks PASS, do NOT declare final verdict. Proceed to Round 2.
...
"I open the file. Does the class/method exist as described?"
"I write the code. What imports/DI do I need? Does the plan mention them?"
"I run it. What fails first? Timing issue? Missing wait?"
"I run it again (restart). Duplicates? Idempotency?"
"I run in staging. All product scopes? All environments?"
...
Iteration Rules
Max 3 iterations — if issues persist after 3 review-fix cycles, STOP and escalate to user via AskUserQuestion
Track iteration count — log "Plan review iteration N/3" at the start of each cycle
PASS/WARN = exit — when all Required checks pass, proceed (WARN is acceptable)
Diminishing scope — each iteration should find FEWER issues. If iteration N finds MORE than N-1, STOP and escalate
Fix scope — only fix issues flagged as FAIL (Required check failures). Do NOT rewrite the plan.
Fix approach:
...
After each fix — re-read the modified plan files before re-reviewing (don't review stale content)
Next Steps
