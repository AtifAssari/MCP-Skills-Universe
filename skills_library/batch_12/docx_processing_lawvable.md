---
title: docx-processing-lawvable
url: https://skills.sh/lawvable/awesome-legal-skills/docx-processing-lawvable
---

# docx-processing-lawvable

skills/lawvable/awesome-legal-skills/docx-processing-lawvable
docx-processing-lawvable
Installation
$ npx skills add https://github.com/lawvable/awesome-legal-skills --skill docx-processing-lawvable
SKILL.md
DOCX Live Editor
...
How It Works
Write custom command to path/to/.superdoc/{docname}.json
Extension executes and overwrites file with response
...
Document must be open in VS Code before editing:
...
Add a word to sentence	insertContent	existing text added word
Add a new paragraph	insertContent	entire new paragraph
Add a new section	insertContent	entire new section
Add disclaimer	insertContent	disclaimer text
...
Use when: Changing existing text to something different. The found text is DELETED and replaced. Track changes effect: Shows as deleted text + new text (strikethrough + underline). Scope: Replaces ALL occurrences by default. Use occurrence parameter to replace only the Nth match.
...
Use when: Adding new text/elements while keeping the anchor text intact. The anchor text STAYS, new content is added before/after. Track changes effect: Shows as new text only (underline, no strikethrough).
...
Use getNodes with type paragraph to get all paragraph positions
Identify which paragraphs are section titles (by text content, bold styling, numbering, etc.). Numbered paragraphs include a marker field (e.g., "marker": "1.") — the TOC automatically prepends it to the entry text.
Pass their positions and heading levels to insertTableOfContents
