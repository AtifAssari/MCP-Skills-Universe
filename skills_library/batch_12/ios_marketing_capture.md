---
title: ios-marketing-capture
url: https://skills.sh/parthjadhav/ios-marketing-capture/ios-marketing-capture
---

# ios-marketing-capture

skills/parthjadhav/ios-marketing-capture/ios-marketing-capture
ios-marketing-capture
Installation
...
A DEBUG-only MarketingCapture.swift file lives in the main app target
When launched with -MarketingCapture 1, the app seeds data, then a coordinator walks a list of CaptureSteps — each step navigates, waits for settle, snapshots, and cleans up
PNGs are written to the app's sandbox Documents/marketing/<locale>/ directory
A shell script builds once, installs, then loops locales by relaunching with -AppleLanguages (xx) -AppleLocale xx, pulling files out via simctl get_app_container
Process
...
Locales — "Which locales? (a) all locales in your Localizable.xcstrings, (b) an App Store subset I'll specify, or (c) let me give you an explicit list." If (a), grep the .xcstrings file for locale codes:
...
Device — "Which simulator? (6.1" iPhone 17 recommended for iOS 26 design features)" — verify the device is available via xcrun simctl list devices available.
Appearance — "Light only, dark only, or both?"
