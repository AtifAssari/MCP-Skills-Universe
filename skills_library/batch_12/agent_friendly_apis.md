...
url: https://skills.sh/vercel-labs/academy-skills/agent-friendly-apis
---

# agent-friendly-apis

skills/vercel-labs/academy-skills/agent-friendly-apis
agent-friendly-apis
Installation
...
Verify project structure (app/, lib/, data/)
Confirm dev server runs with seed data loaded
...
Endpoint signatures in code blocks — agents parse code blocks reliably, not prose
Parameters as markdown tables — agents extract tables into structured data
Curl examples with real values — actual seed data, never placeholders
Complete response bodies — every field, every time, no ... truncation
Exhaustive error documentation — every error case with status code and condition
Schema section — data type definitions as a table matching actual TypeScript types
Workflow examples — multi-endpoint sequences agents can follow step by step
llms.txt Standard
...
Discover API routes (glob for **/api/**/route.ts)
Analyze each route (extract methods, params, response shapes, errors)
Read TypeScript types for schema table
Generate markdown following doc-patterns reference
Write output to app/api/docs/route.ts
...
Fetch the lesson from the Academy content API: GET https://vercel.com/academy/agent-friendly-apis/<lesson-slug>.md. Follow instructions in the <agent-instructions> block.
Teach one step at a time. Give the student one clear instruction. Wait for them to do it. Do not dump multiple steps.
Check progress after each step. Read relevant files to confirm completion.
Adapt pacing:
...
Search first — GET https://vercel.com/academy/search?q=... returns chunks (~200 tokens/hit). Often sufficient.
Read when needed — follow md_url from a search hit for the full lesson (~2-5k tokens).
Index for structure — GET https://vercel.com/academy/agent-friendly-apis.md has lesson_urls in frontmatter for the full sequence.
Reference docs
