---
title: sync-template
url: https://skills.sh/blogic-cz/blogic-marketplace/sync-template
---

# sync-template

skills/blogic-cz/blogic-marketplace/sync-template
sync-template
Installation
...
Create branch: chore/sync-template-packages-$(date +%y%m%d-%H%M)
Update only already-used packages to template version or newer.
Commit package updates separately before template file sync commits.
Verify with project checks before continuing.
...
If it exists, diff it against the template version
If it doesn't exist, flag it as "missing from project"
...
Execute task (delegate when available; otherwise run directly)
Verify acceptance criteria
Run QA scenario
Mark TODO completed
Commit as specified in plan
