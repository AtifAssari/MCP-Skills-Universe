...
url: https://skills.sh/mgd34msu/goodvibes-plugin/task-orchestration
---

# task-orchestration

skills/mgd34msu/goodvibes-plugin/task-orchestration
task-orchestration
Installation
...
Decompose the task into work agents
Spawn the initial work agents
Execute <gv> directives issued by the runtime engine — mechanically, immediately, without judgment
...
Scope is explicit (file paths, not vague descriptions)
Skills are listed with usage guidance
Expected outcome is concrete and verifiable
Output format includes the <gv> tag (emitted automatically by the runtime hook; the orchestrator does not need to instruct agents about this)
...
Spawn new agents as chains complete (when runtime issues complete directive)
...
Stop everything — do not complete any in-progress reasoning about other tasks
Read the directive — identify the action, wid, type, and task fields
Execute immediately:
...
Then resume other orchestration work

No exceptions. A directive is never skipped, reordered, or conditioned on another event.

WRFC Spawn Rules
Initial spawns only: Orchestrator spawns ONLY the initial work agents from task decomposition. All reviewer and fix agent spawns come from runtime directives.
Parallel when possible: Spawn all tasks in the same dependency wave together
Wait for blockers: Do NOT spawn dependent tasks until blocking tasks have received complete directives
Respect the limit: Do NOT exceed 6 concurrent chains
Escalation handling: If runtime issues escalate, report to user — do not attempt to fix the chain yourself
Agent Communication
...
Wait for complete directive for create-profile-types
Spawn create-profile-api and create-profile-ui in parallel (both unblocked)
Wait for complete directives for both
Spawn test-profile-feature (blocked by API + UI)
Wait for complete directive for test-profile-feature
...
Wait for complete directive for diagnosis
Spawn fix-redirect-loop with diagnosis context
Wait for complete directive for fix
Return to user
...
Wait for complete directive for planning
Spawn analyze-auth-patterns
Wait for complete directive for analysis
Spawn create-auth-hooks
Wait for complete directive for hooks
...
Spawn initial work agents with structured prompts
Monitor active chains (up to 6 concurrent)
Execute <gv> directives from runtime immediately and mechanically
Aggregate results when all chains receive complete directives
