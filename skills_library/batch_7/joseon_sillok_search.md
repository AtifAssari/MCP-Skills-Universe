---
title: joseon-sillok-search
url: https://skills.sh/nomadamas/k-skill/joseon-sillok-search
---

# joseon-sillok-search

skills/nomadamas/k-skill/joseon-sillok-search
joseon-sillok-search
Installation
...
w: 원문 검색
Workflow
python3 scripts/sillok_search.py --query "..." 로 공식 검색 endpoint를 호출한다.
검색 결과 HTML에서 결과 수, 왕별 분류, 기사 링크, 요약을 파싱한다.
필요하면 --king, --year 로 결과를 추가로 좁힌다.
선택된 기사마다 /id/<article_id> 상세 페이지를 열어 국역/원문 excerpt를 가져온다.
구조화된 JSON으로 반환한다.
CLI examples
