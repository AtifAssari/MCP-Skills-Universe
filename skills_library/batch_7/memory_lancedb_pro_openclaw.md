---
title: memory-lancedb-pro-openclaw
url: https://skills.sh/aradotso/trending-skills/memory-lancedb-pro-openclaw
---
...
Vector search — semantic similarity via embeddings (weight: vectorWeight, default 0.7)
BM25 full-text search — keyword matching (weight: bm25Weight, default 0.3)
Score fusion — results merged with weighted RRF (Reciprocal Rank Fusion)
Cross-encoder rerank — top candidatePoolSize candidates reranked by a cross-encoder model
Score filtering — results below hardMinScore are dropped
"retrieval": {
  "mode": "hybrid",
  "vectorWeight": 0.7,
  "bm25Weight": 0.3,
...
Confirm retrieval.mode is "hybrid" not "bm25" alone (BM25 requires indexed content)
Lower rerank.hardMinScore temporarily (try 0.4) to see if results exist but are being filtered
Check embedding model is consistent between store and recall operations — changing models requires re-embedding
