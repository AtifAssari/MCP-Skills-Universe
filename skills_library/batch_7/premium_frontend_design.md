...
url: https://skills.sh/kv0906/cc-skills/premium-frontend-design
---

# premium-frontend-design

skills/kv0906/cc-skills/premium-frontend-design
premium-frontend-design
Installation
...
Performance Rules
Always use requestAnimationFrame via useFrame
Reduce particle counts on mobile (check window.innerWidth)
Use useMemo for geometry/position calculations
Cleanup animations in useEffect return
Set transparent and opacity for depth sorting
Component Patterns
...
Layout Flow Recipes
Hero (signature flourish) → Key metrics/value props (calm) → Product proof (screens) → Testimonials/logos → CTA (glow button).
Dashboard page: Above-the-fold summary (minimal) → Feature grid (asymmetrical) → Live data (terminal/sparkline) → Help/resources (calm).
Docs/Platform: Clean header (mesh-gradient-hero in CSS mode) → Navigation grid → Content area (monospace) → CTA.
