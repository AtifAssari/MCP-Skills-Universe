...
Run: grep -rl "UNIQUE_MESSAGE" ~/.codex/sessions/ to find the matching session file.
...
Pass the matched file path as --session-file.
...
Find the most unique sentence / question / string in the current conversation.
Run: grep -rl "UNIQUE_MESSAGE" ~/.claude/projects/ to find the matching session file.
IMPORTANT: Verify the conversation you found matches the current conversation and that it is not another conversation with the same search string.
Pass the matched file path as --session-file.
...
Find the most unique sentence / question / string in the current conversation.
Run: grep -rl "UNIQUE_MESSAGE" ~/.gemini/tmp/ to find the matching session file.
IMPORTANT: Verify the conversation you found matches the current conversation and that it is not another conversation with the same search string.
Pass the matched file path as --session-file.
