---
title: used-car-price-search
url: https://skills.sh/nomadamas/k-skill/used-car-price-search
---

# used-car-price-search

skills/nomadamas/k-skill/used-car-price-search
used-car-price-search
Installation
...
Workflow
차종 키워드가 없으면 먼저 질문한다.
SK렌터카, 롯데렌탈, 레드캡렌터카 비교 결과를 짧게 기억하고, 현재 공급자는 SK렌터카 다이렉트 타고BUY 임을 유지한다.
https://www.skdirect.co.kr/tb HTML 을 가져온다.
HTML 의 __NEXT_DATA__ JSON 에서 carListProd inventory snapshot 을 읽는다.
차종 키워드로 inventory 를 필터링한다.
상위 결과에서 인수가, 월 렌트료, 연식, 주행거리, 연료, 변속기를 정리한다.
같은 차종이라도 재고가 수시로 바뀔 수 있으므로 snapshot 기준 응답임을 짧게 알린다.
Node.js example
