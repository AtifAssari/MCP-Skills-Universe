---
title: release
url: https://skills.sh/madappgang/claude-code/release
---

# release

skills/madappgang/claude-code/release
release
Installation
$ npx skills add https://github.com/madappgang/claude-code --skill release
SKILL.md
...
Wrong: frontend-v3.3.0, v3.3.0, frontend/v3.3.0 Correct: plugins/frontend/v3.3.0 ✅
...
✅ Update CHANGELOG.md, RELEASES.md, CLAUDE.md
✅ Commit: git commit -m "feat({plugin}): v{X.Y.Z} - {summary}"
✅ Tag: git tag -a plugins/{plugin}/v{X.Y.Z} -m "..."
✅ Push: git push origin main && git push origin plugins/{plugin}/v{X.Y.Z}
✅ Verify: /plugin marketplace update shows new version
Example Release
# 1. Update versions
...
Q: Users still see old version A: Check .claude-plugin/marketplace.json - version must match plugin.json

Q: Tag already exists A: git tag -d {tag} then git push origin :{tag} then recreate

Q: How to test locally? A: /plugin marketplace add /path/to/claude-code
