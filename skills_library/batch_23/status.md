---
title: status
url: https://skills.sh/q00/ouroboros/status
---

# status

skills/q00/ouroboros/status
status
Installation
$ npx skills add https://github.com/q00/ouroboros --skill status
SKILL.md
/ouroboros:status
...
How It Works
Session Status: Queries the current state of an execution session
Drift Measurement: Measures how far the execution has deviated from the original seed goal
Instructions
...
Use the ToolSearch tool to find and load the status MCP tools:
ToolSearch query: "+ouroboros session status"

The tools will typically be named with prefix mcp__plugin_ouroboros_ouroboros__ (e.g., ouroboros_session_status, ouroboros_measure_drift). After ToolSearch returns, the tools become callable.
...
If the user asks about drift (or says "am I drifting?"), also call ouroboros_measure_drift:
