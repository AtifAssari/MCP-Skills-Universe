---
title: frappe-agent-validator
url: https://skills.sh/openaec-foundation/erpnext_anthropic_claude_development_skill_package/frappe-agent-validator
---

# frappe-agent-validator

skills/openaec-foundation/erpnext_anthropic_claude_development_skill_package/frappe-agent-validator
frappe-agent-validator
Installation
...
Any import statements? --> FATAL
Any self. references? --> FATAL (use doc.)
Any try/except? --> WARNING (usually wrong)
Uses frappe.throw() for validation? --> GOOD
Uses doc.field for access? --> GOOD
Client Script: 5-point check
Any frappe.db.* calls? --> FATAL
Any frappe.get_doc() calls? --> FATAL
frappe.call() without callback? --> FATAL
Uses frm.doc.field for access? --> GOOD
Uses frm.refresh_field() after changes? --> GOOD
Controller: 5-point check
Modifying self.* in on_update? --> FATAL
Missing super().method() calls? --> ERROR
self.save() in lifecycle hook? --> FATAL
Imports at top of file? --> GOOD
Error handling for external calls? --> GOOD
hooks.py: 5-point check
Valid Python syntax? --> Check
Function paths exist? --> Check
v16-only hooks marked? --> Check
required_apps complete? --> Check
Fixture filters present? --> Check
Bench/Ops: 5-point check
bench migrate after changes? --> REQUIRED
Backup before destructive ops? --> REQUIRED
Scheduler enabled? --> Check
Workers running? --> Check
SSL configured (production)? --> Check
