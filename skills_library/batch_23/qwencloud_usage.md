---
title: qwencloud-usage
url: https://skills.sh/qwencloud/qwencloud-ai/qwencloud-usage
---

# qwencloud-usage

skills/qwencloud/qwencloud-ai/qwencloud-usage
qwencloud-usage
Installation
...
qwencloud auth status --format json → authenticated: true → skip to commands
qwencloud auth login --init-only --format json → extract verification_url → open in browser
qwencloud auth login --complete --format json → poll until success event
...
Parse the JSON and extract the relevant data for the user's question
...
Display CLI output EXACTLY AS-IS — no modification, no reformatting
