---
title: openmm-order-management
url: https://skills.sh/qbt-labs/openmm-ai/openmm-order-management
---

# openmm-order-management

skills/qbt-labs/openmm-ai/openmm-order-management
openmm-order-management
Installation
$ npx skills add https://github.com/qbt-labs/openmm-ai --skill openmm-order-management
SKILL.md
OpenMM Order Management
...
Safety Guidelines
Always confirm with the user before placing orders. Show the order details (exchange, symbol, side, type, amount, price) and get explicit approval.
Check balance first. Run openmm balance --exchange <ex> to verify sufficient funds before placing an order.
Check current price. Run openmm ticker --exchange <ex> --symbol <sym> to verify the market price before placing limit orders.
Respect minimum order values:
MEXC / Gate.io / Bitget: 1 USDT minimum per order
Kraken: 5 EUR/USD minimum per order
Bitget requires passphrase. Ensure BITGET_PASSPHRASE is set alongside BITGET_API_KEY and BITGET_SECRET.
Double-check symbol format. Always use BASE/QUOTE format (e.g. BTC/USDT, ADA/EUR).
For limit orders, verify price is reasonable. Compare the limit price against the current ticker price to avoid placing orders far from market.
