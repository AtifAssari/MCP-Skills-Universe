---
title: auth-manager
url: https://skills.sh/aaaaqwq/claude-code-skills/auth-manager
---

# auth-manager

skills/aaaaqwq/claude-code-skills/auth-manager
auth-manager
Installation
...
全部正常则静默
铁律 ⚠️
禁止使用 fbu / fast-browser-use — 已废弃
统一使用 profile='openclaw' — 所有 browser 操作
旧 profile 目录 ~/.openclaw/chrome-profiles/<platform>/ 已废弃，仅保留 session.json 供 curl 读取 token/cookie
超时 ≠ 过期 — 网络问题标记为 error，不标记为 expired
每个检查最多 15 秒 — 避免阻塞
迁移说明（v3 → v4）
