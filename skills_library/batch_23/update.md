---
title: update
url: https://skills.sh/q00/ouroboros/update
---

# update

skills/q00/ouroboros/update
update
Installation
$ npx skills add https://github.com/q00/ouroboros --skill update
SKILL.md
/ouroboros:update
...
This skill runs inside Claude Code, so always use ouroboros-ai[claude] (includes claude-agent-sdk and anthropic required for MCP tools).
...
Note: The [claude] extra is critical — it installs claude-agent-sdk and anthropic which are required for MCP tool execution. Omitting it causes MCP tools to fail silently at call time.
...
This ensures ~/.claude/mcp.json has the latest MCP command and args (e.g., ouroboros-ai[claude] extras). Skips if already up to date.
