...
url: https://skills.sh/bbwdadfg/wechat-ai-publisher/wechat-ai-publisher
---

# wechat-ai-publisher

skills/bbwdadfg/wechat-ai-publisher/wechat-ai-publisher
wechat-ai-publisher
Installation
$ npx skills add https://github.com/bbwdadfg/wechat-ai-publisher --skill wechat-ai-publisher
SKILL.md
微信公众号 AI 热点自动发布工具
完整链路
1. 询问用户配置（主题、风格、配图数量）
...
如果是交互式运行，可以询问用户；如果是定时任务/自动运行（比如 OpenClaw Cron），则不要提问，直接从触发消息里解析参数或使用默认值。
...
覆盖规则（让用户“诉说”即可）
如果用户/定时任务消息里出现 主题=... / 风格=... / 配图=... / 作者=...，则覆盖默认值。
其余自然语言描述（例如“写得更科普一些”“多强调边界和风险”“更偏护理视角”）视为写作约束，AI 可以自由调整结构与措辞以提升可读性。
冲突时以用户指令为准；不确定就保守处理（不编造数据、不夸大结论）。
...
相关性：与主题直接相关，不是泛泛而谈
深度：有具体案例、数据、代码，不是纯新闻稿
新鲜度：优先最近 3 天的内容
来源质量：优先技术博客、官方文档、知名媒体
阶段 2：深度抓取（获取完整原文）
...
参考示例文章：examples/nursing-tech-article.md
...
注意事项
图片语言：中文文章用中文配字，英文文章用英文配字
图片域名：公众号只能显示 mmbiz.qpic.cn 域名的图片
中文编码：JSON 必须用 ensure_ascii=False
标题长度：最多 64 字节（约 21 个中文字符）
access_token：有效期 2 小时，每次需重新获取
定时任务：自动运行时不要提问，直接按参数/默认值执行
Weekly Installs
125
Repository
bbwdadfg/wechat…ublisher
GitHub Stars
37
First Seen
Feb 27, 2026
