...
url: https://skills.sh/dirnbauer/webconsulting-skills/typo3-workspaces
---

# typo3-workspaces

skills/dirnbauer/webconsulting-skills/typo3-workspaces
typo3-workspaces
Installation
$ npx skills add https://github.com/dirnbauer/webconsulting-skills --skill typo3-workspaces
SKILL.md
TYPO3 Workspaces
...
Restriction Builder (TYPO3 Explained / Doctrine DBAL)
...
Overlay workspace version (search for t3ver_oid=<uid> AND t3ver_wsid=<current_ws>)
Overlay language translation (search for l10n_parent=<uid> in target language)
...
Files (FAL) are NOT versioned. This is the single most important limitation of TYPO3 Workspaces. See also Section 2a below for the additional file collection limitation (folder-based collections).
...
The new PDF is uploaded to fileadmin/ immediately (files are live!)
An external person guesses the URL by incrementing the year
geschaeftsbericht2025.pdf is accessible before the content element referencing it is published
...
Inline / IRRE child tables (Deprecation #106821): If versioningWS is missing on a child table, Core logs a deprecation and may auto-add the flag at TCA build time — it does not throw a hard error. Set the flag explicitly in TCA; it will become a hard requirement in v15.
...
CRITICAL: Extbase DataMapper does NOT populate t3ver_* system fields. If you add $t3verWsid, $t3verState, or $t3verOid properties to an Extbase domain model, they will always remain at their default value (typically 0). The t3ver_* columns are ctrl fields in TCA, not columns — Extbase's DataMapFactory does not create column maps for them.
...
Translations created in a workspace are single records with t3ver_state = 1 (workspace-new)
The l10n_parent field points to the live default language record UID
Modified translations have t3ver_oid pointing to the live translation UID
...
Editor switches to the custom workspace via the backend sidebar selector (v14.2+)
