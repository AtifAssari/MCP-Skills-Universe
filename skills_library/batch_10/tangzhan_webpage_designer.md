---
title: tangzhan-webpage-designer
url: https://skills.sh/tangc/tangzhan-skills/tangzhan-webpage-designer
---
...
核心逻辑
分析内容 (Analyze Content)：确定用户文本的语气、受众和目的。
选择风格 (Select Style)：
...
加载提示词 (Load Prompt)：从 references/ 目录读取相应的设计系统文件（例如 references/cyberpunk.md）。
生成网页 (Generate)：将读取到的风格提示词与用户内容结合，生成 HTML 代码。
风格矩阵 (Style Matrix)
...
选择 (Selection)：首先，告诉用户你选择了哪种风格，以及为什么。
示例：“考虑到您内容的专业技术性质，我选择了 'Terminal' (终端) 风格，以为其赋予一种复古未来的黑客氛围。”
工具使用 (Tool Use)：使用 read 工具读取上述矩阵中对应的 参考文件 (Reference File) 内容。
生成 (Generate)：
