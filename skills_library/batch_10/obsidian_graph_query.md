---
title: obsidian-graph-query
url: https://skills.sh/azuma520/obsidian-graph-query/obsidian-graph-query
---

# obsidian-graph-query

skills/azuma520/obsidian-graph-query/obsidian-graph-query
obsidian-graph-query
Installation
$ npx skills add https://github.com/azuma520/obsidian-graph-query --skill obsidian-graph-query
SKILL.md
Obsidian Graph Query
...
執行模式
步驟
讀取設定：從 references/vault-config.md 讀取 CLI 路徑、vault 名稱、排除資料夾、關係欄位
讀取模板：從 references/query-templates.md 讀取對應 JS 模板
代入參數：
...
寫入暫存檔：用 Write 工具寫到 /tmp/obsidian_graph_query.js
執行：用 Bash 工具執行：
<CLI> vault="<VAULT>" eval code='eval(require("fs").readFileSync("C:/tmp/obsidian_graph_query.js","utf8"))'


Windows 路徑：Write 工具用 /tmp/...，eval 中用 C:/tmp/... macOS/Linux：兩者都用 /tmp/...

解析：輸出為 JSON 字串，解析後轉為 Markdown 呈現
筆記名稱解析

用戶通常只給部分名稱（如「那篇關於 BFS 的筆記」）。解析流程：

用 Obsidian CLI 的 search 指令搜尋：
<CLI> vault="<VAULT>" search query="BFS" limit=5

從結果中取得完整路徑（如 notes/演算法/BFS.md）
將完整路徑代入模板的 {{NOTE_PATH}}
...
先跑 hubs/orphans-rich 取得結果
對結果中的筆記用 properties 指令逐一查 frontmatter
過濾不符條件的筆記
