---
title: bn-fit-modify
url: https://skills.sh/letta-ai/skills/bn-fit-modify
---

# bn-fit-modify

skills/letta-ai/skills/bn-fit-modify
bn-fit-modify
Installation
...
Use library methods for fitting - Always prefer model.fit(data) over manual parameter computation
Verify fitted parameters - Print and inspect CPD parameters after fitting:
for cpd in model.get_cpds():
    print(cpd)

For Linear Gaussian BNs, verify:
Intercept values
Coefficient values for each parent
Variance estimates
Compare against expected values if test cases provide them
Phase 3: Intervention (do-calculus)

When performing causal interventions:

Understand intervention semantics: do(X=x) means:
...
Create the interventional DAG correctly:
...
Re-fit or transfer parameters appropriately:
...
Verify array dimensions at each step to catch broadcasting errors early
...
Structure verification: Compare learned edges against expected structure
Parameter verification: Compare fitted parameters against expected values
Intervention verification: Confirm correct edges removed and added
Sampling verification: Check sample statistics match expected distributions
Statistical Validation

For final samples:

Compute mean and variance of sampled values
Compare against theoretical expectations from the fitted model
Use statistical tests (e.g., chi-square, KS test) to validate distributions
A p-value of 0.0 indicates fundamental errors in sampling logic
Debugging Checklist
