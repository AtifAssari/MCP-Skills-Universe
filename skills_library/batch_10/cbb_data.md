---
title: cbb-data
url: https://skills.sh/machina-sports/sports-skills/cbb-data
---

# cbb-data

skills/machina-sports/sports-skills/cbb-data
cbb-data
Installation
...
Call get_rankings() Result: AP Top 25 and Coaches Poll with rank, previous rank, record, and points

Example 2: Conference standings User says: "Show me SEC basketball standings" Actions:

Derive season year from currentDate
Call get_standings(group=23, season=<derived_year>) (group 23 = SEC) Result: SEC standings with W-L records per team

Example 3: Today's scores User says: "What are today's college basketball scores?" Actions:

Call get_scoreboard() Result: All live and recent CBB games with scores and ranked status

Example 4: Team roster User says: "Show me Duke's roster" Actions:

Call get_team_roster(team_id="150") Result: Full Duke roster with name, position, jersey number

Example 5: March Madness futures User says: "Who's favored to win March Madness?" Actions:

Call get_futures(limit=10) Result: Top National Championship contenders with odds values

Example 6: Team statistics User says: "Show me Duke's team stats" Actions:

Derive season year from currentDate
Call get_team_stats(team_id="150", season_year=<derived_year>) Result: Duke's season stats by category with value, rank, and per-game averages
