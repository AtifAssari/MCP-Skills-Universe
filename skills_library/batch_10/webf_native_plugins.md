...
url: https://skills.sh/openwebf/webf/webf-native-plugins
---

# webf-native-plugins

skills/openwebf/webf/webf-native-plugins
webf-native-plugins
Installation
...
Open the Flutter project's pubspec.yaml
Add the plugin dependency:
dependencies:
  webf_share: ^1.0.0  # Replace with actual plugin name

Run flutter pub get
Register the plugin in your main Dart file:
...
Read the Plugin Development Guide: https://openwebf.com/en/docs/developer-guide/native-plugins
Study existing plugins: https://github.com/openwebf/webf/tree/main/webf_modules
...
Check that the Flutter package is in pubspec.yaml
Verify the module is registered with WebF.defineModule() in main.dart
Run flutter pub get
Rebuild the Flutter app
Restart WebF Go or your app
...
Check the plugin documentation for required permissions
Add necessary permissions to AndroidManifest.xml or Info.plist
Some plugins require additional native configuration
...
Flutter App: Make sure all required plugins are in pubspec.yaml
npm Packages: Include all plugin packages in package.json
Permissions: Configure all required permissions in app manifests
Testing: Test on real devices for all target platforms
Documentation: Document which plugins your app requires
Resources
...
Read plugin documentation: Each plugin has specific APIs and requirements
Test on devices: Native features work differently than web APIs
Handle errors: Native calls can fail due to permissions or platform limitations
Consider alternatives: Check webf-api-compatibility for web API alternatives
Summary
