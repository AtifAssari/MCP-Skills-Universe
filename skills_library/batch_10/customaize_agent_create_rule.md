---
title: customaize-agent:create-rule
url: https://skills.sh/neolabhq/context-engineering-kit/customaize-agent:create-rule
---

# customaize-agent:create-rule

skills/neolabhq/context-engineering-kit/customaize-agent:create-rule
customaize-agent:create-rule
Installation
$ npx skills add https://github.com/neolabhq/context-engineering-kit --skill customaize-agent:create-rule
SKILL.md
Create Rule
...
What Rules Provide
Behavioral constraints — What to do and what NOT to do
Code standards — Formatting, patterns, architecture decisions
Quality gates — Conditions that must be met before proceeding
Domain conventions — Project-specific terminology and practices
...
Eliminates ambiguity — the agent sees the exact boundary between acceptable and unacceptable
Prevents rationalization — harder to argue "this is close enough" when the wrong pattern is explicitly shown
Enables self-correction — agents can compare their output against both patterns
...
Global (no paths frontmatter) — applies to all work in the project
Path-scoped (paths frontmatter with glob patterns) — applies only when working with matching files
User-level (~/.claude/rules/) — applies across all projects for personal preferences
...
Start with the Incorrect pattern — write the exact code or behavior the agent produces that needs correction
Write the Correct pattern — show the minimal fix that addresses the issue
Verify contrast is clear — the difference between Incorrect and Correct must be obvious and focused on exactly one concept
...
Add YAML frontmatter with title, impact, tags, and optionally paths
Write the heading and description
Add the Incorrect section with description and code
Add the Correct section with description and code
Optionally add a Reference section with links
...
File location — rule exists at .claude/rules/<rule-name>.md
Frontmatter — contains at minimum title and impact
Contrastive examples — both Incorrect and Correct sections present with code blocks
Token budget — description is 50-200 words (excluding code)
Path scoping — if paths is set, glob patterns match intended files
No overlap — rule does not duplicate guidance in CLAUDE.md or other rules
...
What behavior does this enforce?
Why does it matter?
How does right differ from wrong? (shown through contrastive examples)
