---
title: webpay-api
url: https://skills.sh/xoverlap/webpay-skill/webpay-api
---

# webpay-api

skills/xoverlap/webpay-skill/webpay-api
webpay-api
Installation
...
Create transaction: Your server calls POST /rswebpaytransaction/api/webpay/v1.2/transactions with the order details. You receive a token and a url.

Redirect to Webpay: Redirect the user's browser to the returned url, passing the token via a form POST field called token_ws:
...
User pays: The user enters their card details on Transbank's secure form and authorizes the payment.

Webpay redirects back: After authorization (or rejection/abort), Webpay redirects the user back to your return_url via POST, sending:
...
Confirm transaction: Your server calls PUT /rswebpaytransaction/api/webpay/v1.2/transactions/{token} to confirm and get the result.

Show result: Display the payment result to the user.
...
First with TBK_TOKEN, TBK_ORDEN_COMPRA, TBK_ID_SESION (and empty token_ws)
A second request with just TBK_TOKEN (a POST from the user's browser)
...
response_code equals 0
status equals "AUTHORIZED"
...
Complete the certification process with Transbank
Obtain your production commerce code and API secret
Change the base URL from https://webpay3gint.transbank.cl to https://webpay3g.transbank.cl
Replace the integration credentials with your production credentials
...
The buy_order must be unique for each transaction. It cannot be reused.

Always confirm (commit) the transaction after receiving the redirect. If you don't call commit, the transaction remains in INITIALIZED state and the payment will not be processed.

Commit has a time limit: You must confirm the transaction within a few seconds after the user is redirected back. If too much time passes, the transaction may expire.

Handle all redirect scenarios at your return_url:
...
Refund vs Reversal: The API automatically determines whether to reverse (same day, before accounting cutoff) or nullify (after accounting cutoff). You always call the same endpoint.

Partial refunds are supported: You can refund any amount up to the original transaction amount. Multiple partial refunds are allowed until the balance reaches zero.

Deferred capture requires a special commerce code and configuration with Transbank. Not all merchants have this feature enabled.

The vci field should NOT be used for business logic validation. Use response_code instead. The vci field is informational only and new values are added without documentation updates.

The response_code is the definitive indicator of whether a transaction was approved (0) or rejected (non-zero).

Weekly Installs
16
Repository
xoverlap/webpay-skill
First Seen
Feb 27, 2026
Security Audits
