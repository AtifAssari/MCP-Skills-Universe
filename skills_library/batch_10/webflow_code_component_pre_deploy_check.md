---
title: webflow-code-component:pre-deploy-check
url: https://skills.sh/webflow/webflow-skills/webflow-code-component:pre-deploy-check
---

# webflow-code-component:pre-deploy-check

skills/webflow/webflow-skills/webflow-code-component:pre-deploy-check
webflow-code-component:pre-deploy-check
Installation
...
Validate imports in .webflow.tsx files:
...
Webpack custom config validation (if bundleConfig is specified):
...
Blocked properties that are silently ignored: entry, output, target
module.rules must be a function, not an array: rules: (currentRules) => { ... }
ModuleFederationPlugin and MiniCssExtractPlugin are auto-deduplicated
Common Build Errors
Error	Cause	Fix
