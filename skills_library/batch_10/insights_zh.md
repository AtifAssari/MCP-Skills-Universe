---
title: insights-zh
url: https://skills.sh/akira82-ai/insights-zh/insights-zh
---

# insights-zh

skills/akira82-ai/insights-zh/insights-zh
insights-zh
Installation
$ npx skills add https://github.com/akira82-ai/insights-zh --skill insights-zh
SKILL.md
Insights 中文报告生成器
...
✅ 检查报告年龄（3 天内为新鲜）
✅ 提取 HTML 中的所有文本块
✅ 保存为 TSV 格式到 /tmp/insights_blocks.tsv
✅ 显示翻译提示
...
提示用户运行 /insights 命令生成报告
支持指定自定义路径：--path <your-report.html>
报告过期

如果报告超过 3 天未更新：

提示用户报告已过期及过期天数
建议运行 /insights 生成最新报告
可使用 --force 参数强制翻译过期报告
嵌套会话错误
...
原因：使用了 subprocess.run(['claude', 'prompt', ...]) 解决：使用 Task 工具启动独立子代理
...
提取阶段使用 HTMLParser，提取了 311 个文本块
合并阶段使用 BeautifulSoup.find_all(string=True)，找到 315 个文本节点
两者的遍历顺序和节点计数不同，导致索引错位
解决方案
