---
title: deep-research
url: https://skills.sh/webcpu/deep-research-skill/deep-research
---

# deep-research

skills/webcpu/deep-research-skill/deep-research
deep-research
Installation
...
If $ARGUMENTS contains --file <path>, extract that as the file path. Everything else is the topic.
Else if a token ends in .md or contains /, treat it as the file path. Everything else is the topic.
Else treat ALL of $ARGUMENTS as the topic. Auto-generate the file path: <topic-slug>.md in the current working directory (e.g., "swift concurrency" → swift-concurrency.md).
...
Official docs and authoritative sources
Community discussion and practitioner blogs
(Optional) Social media and recent reports
...
Gotchas, pitfalls, and troubleshooting
Tools, libraries, and ecosystem
(Optional) Advanced techniques and emerging trends
...
The subject and specific angle this agent covers (e.g., "best practices for Swift async/await" or "Claude Code opus 4.7 official docs")
...
Search instructions: Tell the agent to use WebSearch with queries derived from the subject and topic. Pattern:
...
Output format: For each finding, return:
...
Rules: MUST only return findings with real source URLs. NEVER return findings without sources. Focus on practitioner insights (what people actually do), not marketing. Prefer concrete techniques, numbers, and gotchas over general advice.
...
Filter: Drop anything without a source URL or that duplicates existing content
Locate: Find the exact section in the playbook where each finding belongs
Edit: Use the Edit tool to merge the finding into the existing section
...
Source list: Add any new source URLs to the Sources list at the bottom
...
Filter: Drop anything without a source URL
Group: Cluster findings by theme into logical sections (aim for 5-10 sections)
Write: Use the Write tool to create the file. Structure:
...
Source list: Collect all source URLs into the Sources section at the bottom
