---
title: duckdb-sql
url: https://skills.sh/robdmc/claude_skills/duckdb-sql
---

# duckdb-sql

skills/robdmc/claude_skills/duckdb-sql
duckdb-sql
Installation
...
Default behavior: If duckdb_sql_assets/ already exists, proceed directly to answering the user's question using the existing documentation. Do NOT check for schema changes unless explicitly asked. See Schema Update Detection for trigger phrases.
...
Start with Quick Reference - Identify relevant tables and key relationships in data_dictionary.md
Check Domain Overview - Understand which tables work together
Read Table Details - Get field info, enum values, and query patterns
Validate in Schema - Confirm exact column names in schema_<filename>.sql files
Check Important Query Patterns - Use documented patterns for soft deletes, status filters, etc.
...
Check if duckdb_sql_assets/ exists - if it does, inform user assets already exist
If no specific files are mentioned in the request, proceed to step 2 below to prompt for files
...
Cardinality: distinct_count <= 20
Ratio: distinct_count / sampled_rows < 0.05 (less than 5%)
...
Read tables_inventory.json to identify relevant tables
Read data_dictionary.md for business context
Verify against schema_<filename>.sql for exact column names
Provide clear answer with table names, key fields, and context
...
Verify all table names exist - Check tables_inventory.json or schema files
Verify all column names exist - Check the specific schema_<filename>.sql
Verify column types match usage - Ensure you're comparing compatible types (e.g., don't compare VARCHAR to INTEGER without casting)
...
User provides: SELECT * FROM orders WHERE status = 'pending' User asks: "Add customer name to this"
...
Extract schema: duckdb new_file.ddb -c ".schema"
Create schema_new_file.sql
Update tables_inventory.json with file_type: "ddb"
Add stub entries to data_dictionary.md
Run enum detection on new tables
Present findings for approval (inline for 1-2 enums, bulk summary for 3+)
If approved, update data_dictionary.md with enum documentation

For .csv or .parquet files:

Extract schema: duckdb -c "DESCRIBE SELECT * FROM '/path/to/file.csv';"
Convert to CREATE TABLE format and save as schema_<table_name>.sql
Compute table name from filename (lowercase, snake_case, alphanumeric)
Update tables_inventory.json with file_type: "csv" or "parquet"
Add stub entry to data_dictionary.md
Run enum detection on VARCHAR columns
