---
title: refac-module-to-subpackage
url: https://skills.sh/dagster-io/erk/refac-module-to-subpackage
---

# refac-module-to-subpackage

skills/dagster-io/erk/refac-module-to-subpackage
refac-module-to-subpackage
Installation
...
Author's sections — Comment headers like # === Tests for Foo === are the strongest signal
Feature cohesion — Code testing/implementing the same feature belongs together
Import cohesion — Code sharing the same specialized imports likely belongs together
Size balance — Avoid files with fewer than ~20 lines (merge with a neighbor) or more than ~500 lines (split further)
...
Write the module docstring (brief, describes what this file tests/contains)
Add only the imports needed by the functions in this file
Copy the functions/classes verbatim from the monolith
Preserve section comment headers within the file
Merge into existing files

When a target file already exists:

Read the existing file completely
Compare tests/functions by behavior, not by name — two functions testing the same thing with different names are duplicates
Append only non-duplicate code to the end of the existing file
Add appropriate section headers to separate old and new content
