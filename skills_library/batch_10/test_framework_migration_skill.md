...
url: https://skills.sh/lambdatest/agent-skills/test-framework-migration-skill
---

# test-framework-migration-skill

skills/lambdatest/agent-skills/test-framework-migration-skill
test-framework-migration-skill
Installation
...
Locators — Convert using the API mapping table (e.g. By.id("x") → page.getByRole(...) or page.locator('#x')).
...
If target is Playwright: use auto-wait assertions (expect(locator).toBeVisible()), not raw waitForTimeout.
If target is Cypress: no async/await with cy commands; use chain style.
If target is Selenium: use explicit WebDriverWait, never Thread.sleep.
