---
title: octocode-documentation-writer
url: https://skills.sh/bgauryy/octocode-mcp/octocode-documentation-writer
---

# octocode-documentation-writer

skills/bgauryy/octocode-mcp/octocode-documentation-writer
octocode-documentation-writer
Installation
...
<phase_6> QA Validator (Phase 6) Agent Role: Fast validation model What: Validates documentation quality using LSP-powered verification Input: documentation/*.md + analysis.json + questions.json + research.json Output: qa-results.json + QA-SUMMARY.md </phase_6>
...
<execution_rules> STOP. Verify parallel spawn requirements. REQUIRED: Use host parallelism when available. FALLBACK: Sequential execution is allowed only when the host cannot run true parallel work. <agent_count>4</agent_count> Discovery and Analysis <spawn_instruction>Prefer one-response fan-out; otherwise run sequential fallback and preserve exclusive scopes</spawn_instruction> Run all 4 agents concurrently when the host supports it; otherwise use sequential fallback Wait for ALL 4 to complete before aggregation Must aggregate 4 partial JSONs into analysis.json
...
Required Actions
Define Directories (CONTEXT_DIR, DOC_DIR)
