---
title: sdd:plan
url: https://skills.sh/neolabhq/context-engineering-kit/sdd:plan
---

# sdd:plan

skills/neolabhq/context-engineering-kit/sdd:plan
sdd:plan
Installation
...
Provide the value of ${CLAUDE_PLUGIN_ROOT} so agents can resolve paths like @${CLAUDE_PLUGIN_ROOT}/scripts/create-scratchpad.sh
...
Automatic retry: Re-launch the phase agent with judge feedback
Human-in-the-loop check: If phase is in HUMAN_IN_THE_LOOP_PHASES, trigger human checkpoint before the next judge retry (after implementation retry but before re-judging)
After MAX_ITERATIONS reached: Proceed to next stage automatically (do NOT ask user unless --human-in-the-loop includes this phase)
Log warning in completion summary: ⚠️ Phase X did not pass quality threshold (X.X/THRESHOLD) after MAX_ITERATIONS iterations
Retry Flow
