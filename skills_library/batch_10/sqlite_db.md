---
title: sqlite-db
url: https://skills.sh/bfollington/terma/sqlite-db
---

# sqlite-db

skills/bfollington/terma/sqlite-db
sqlite-db
Installation
$ npx skills add https://github.com/bfollington/terma --skill sqlite-db
SKILL.md
...
Tables — DDL for all domain entities with constraints, indexes, and foreign keys
Indexes — B-tree indexes for common queries, FTS indexes for search
FTS — Virtual tables and sync triggers for full-text search
Views — Saved queries as first-class database objects
Triggers — Auto-timestamps, FTS sync, validation
Links vocabulary — Named relationship types (same as memhub: linksTo, derivedFrom, partOf, etc.)
Database path — Where the .db file lives (e.g., .sqlite/notes.db)
ID prefixes — Conventions for generating human-readable IDs
Workflows — Step-by-step SQL examples for common tasks
Design Principles
