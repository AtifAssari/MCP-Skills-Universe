---
title: unity-ui
url: https://skills.sh/bigdra50/unity-cli/unity-ui
---

# unity-ui

skills/bigdra50/unity-cli/unity-ui
unity-ui
Installation
$ npx skills add https://github.com/bigdra50/unity-cli --skill unity-ui
SKILL.md
unity-ui
...
query でフィルタした要素をランダムに click し、コンソールエラーを監視する。 -c (USS class) か -t (type) のフィルタが必須（フィルタなしでは要素が見つからない）。 事前に u uitree query -p <panel> -c <class> で有効な class 名を確認してから monkey に渡す。 --seed で操作順を再現可能。
...
Phase 1 で確認した操作シナリオを、pytest テストとして永続化する。 unity-cli の Python API (UITreeAPI, ConsoleAPI, EditorAPI) を直接使う。

手順:

u play + u uitree dump で構造を把握する
操作可能な要素（ボタン名、ラベル名）を特定する
以下のテンプレートに従い、tests/integration/ にテストファイルを生成する
uv run python -m pytest tests/integration/<file> -v で実行して確認する
...
Phase 2 の pytest テストから安定したシナリオを選ぶ
以下のテンプレートに従い、Assets/Tests/PlayMode/ に C# テストを生成する
asmdef に UI コントローラの参照を追加する（必要に応じて）
u tests run play で実行して確認する
...
u api schema --type <Type> で対応メソッドを検索
u api call で実行
該当なしの場合のみ YAML 直接編集
Weekly Installs
8
