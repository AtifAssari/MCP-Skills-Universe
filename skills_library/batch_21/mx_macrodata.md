---
title: mx-macrodata
url: https://skills.sh/site/skills.volces.com/mx-macrodata
---

# mx-macrodata

skills/skills.volces.com/MX_MacroData
MX_MacroData
...
上层规划引擎在调用 MX_MacroData 后，必须执行以下工作流，以确保最终结果的完整性：

阶段 A: 任务解析与预期规划 (Task Parsing & Expectation Planning)
解析原始用户意图：从用户查询中提取核心要素。
构建预期全集 (Expected Universe)：
...
获取输出文件列表：获取 MX_MacroData 生成的所有 CSV 文件路径。
读取并分析每个文件：
使用规划引擎内置的代码执行能力（如 Python Sandbox）读取 CSV。
提取每个文件中实际覆盖的 (地区, 指标) 组合及其数量。
执行差异计算：
对每一种频率的文件，分别计算 Missing_Pairs_<Frequency> = Expected_Universe - Actual_From_CSV_File。
