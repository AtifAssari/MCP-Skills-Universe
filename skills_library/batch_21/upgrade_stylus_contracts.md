---
title: upgrade-stylus-contracts
url: https://skills.sh/openzeppelin/openzeppelin-skills/upgrade-stylus-contracts
---

# upgrade-stylus-contracts

skills/openzeppelin/openzeppelin-skills/upgrade-stylus-contracts
upgrade-stylus-contracts
Installation
...
Add UUPSUpgradeable (and access control) as fields in the #[storage] struct
Call self.uups.constructor() and initialize access control in the constructor
Expose initialize calling self.uups.set_version() — invoked via proxy after deployment
Implement IUUPSUpgradeable — upgrade_to_and_call guarded by access control, upgrade_interface_version delegating to self.uups
Implement IErc1822Proxiable — proxiable_uuid delegating to self.uups
...
Access control — restrict upgrade_to_and_call (e.g., self.ownable.only_owner())
Proxy context enforcement — only_proxy() reverts if the call is not via delegatecall
Proxiable UUID validation — proxiable_uuid() must return the ERC-1967 implementation slot, confirming UUPS compatibility
Reactivation
