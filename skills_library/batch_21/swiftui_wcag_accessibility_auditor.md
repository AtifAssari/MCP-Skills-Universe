---
title: swiftui-wcag-accessibility-auditor
url: https://skills.sh/ramzesenok/ios-accessibility-audit-skill/swiftui-wcag-accessibility-auditor
---

# swiftui-wcag-accessibility-auditor

skills/ramzesenok/ios-accessibility-audit-skill/swiftui-wcag-accessibility-auditor
swiftui-wcag-accessibility-auditor
Installation
...
Load Order
Read references/ios-audit-workflow.md for code-only audit process, statuses, evidence rules, and baseline report format.
Read references/ios-audit-checklist.md for WCAG SC coverage priorities and code signals.
Read references/wcag2mobile-ios-reference.md when interpreting mobile-specific applicability or draft maturity of a criterion.
Read references/ios-accessibility-api-examples.md, starting with the SwiftUI sections only.
Read references/swiftui-remediation-guide.md for patch strategy, non-goals, priority model, and SwiftUI fix patterns.
Use references/swiftui-manual-checklist.md only when generating user follow-up checks or a final manual validation list.
Scope Rules
...
Define the scoped feature flow and in-scope screens/states.
Identify the SwiftUI entry views and related subviews for the feature.
Run the WCAG checklist (ios-audit-checklist.md) and record evidence with statuses.
Use the SwiftUI API examples to avoid false positives and identify missing semantics.
Generate patch-ready fix suggestions using swiftui-remediation-guide.md.
