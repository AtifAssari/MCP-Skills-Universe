---
title: pinchtab
url: https://skills.sh/mxyhi/ok-skills/pinchtab
---

# pinchtab

skills/mxyhi/ok-skills/pinchtab
pinchtab
Installation
...
One-off browsing: pinchtab instance start → use --server http://localhost:<port> for commands.
Reuse a profile: pinchtab instance start --profile work --mode headed → switch to --mode headless after login is stored.
Create profile via HTTP: POST /profiles with {"name":"..."}, then POST /profiles/<name>/start.
Human-assisted login: Start headed, human signs in, agent reuses the profile headless.
HTTP-only agent: Use POST /instances/start, then target the instance port with curl. Send X-Agent-Id for attribution.
