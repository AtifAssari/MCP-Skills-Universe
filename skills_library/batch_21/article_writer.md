...
url: https://skills.sh/mwguerra/claude-code-plugins/article-writer
---

# article-writer

skills/mwguerra/claude-code-plugins/article-writer
article-writer
Installation
$ npx skills add https://github.com/mwguerra/claude-code-plugins --skill article-writer
SKILL.md
Article Writer
...
Quick Start
Determine author (from task or default author in database)
Create folder structure (including code/ folder)
Load author profile from database
Load settings (including article_limits.max_words)
Follow phases: Initialize → Plan → Research → Draft → Companion Project → Integrate → Review → Condense → Translate → Finalize
Workflow Overview
...
Voice Analysis Data (if present in voice_analysis)
