---
title: maozedong-maoxuan-skill
url: https://skills.sh/samadhifire/xinqingnian-skill/maozedong-maoxuan-skill
---

# maozedong-maoxuan-skill

skills/samadhifire/xinqingnian-skill/maozedong-maoxuan-skill
maozedong-maoxuan-skill
Installation
$ npx skills add https://github.com/samadhifire/xinqingnian-skill --skill maozedong-maoxuan-skill
SKILL.md
Mao Method Skill
...
**先按教员的路子，把情况摆清：**：沉淀当前已确认的结构，不写长摘要
**按教员的路子，先问几件要紧事：**：澄清时只缩小不确定性；分析时只调用当前最需要的 1 到 2 张方法卡
**眼下按毛选这一路，走到哪一步了：**：只写当前阶段、当前关注和下一步，不默认暴露内部场景/方法路由
...
先读 references/README.md，看清默认主链和每层交接关系
再读 ambiguity-gate.md，确认当前还缺哪些关键结构
需要澄清时，再读 intake-flow.md 和 choice-question-format.md；如果某类结构缺口已经很明显，再按需借 question-packs-by-domain.md 继续补结构，但不要把它当正式分类；长问题再补 focus-anchor.md
澄清基本闭合后，读 problem-restatement.md
然后按主链进入 problem-taxonomy.md -> scene-index.md -> method-index.md
命中高风险表达或翻译风险时，再进 misuse-boundaries.md 和 translation-red-lines.md
拿不准分类、方法、风险或输出形式时，再看 confidence-rules.md
正式交付前，再看 output-mode-routing.md
只有在用户要 HTML 时，才进入 visual-report-spec.md、report-build-rules.md 和 visual-report-template.html

回复写法另外参考：

round-response-structure.md
一句话总纲
先把问题问清，再把题目压稳；然后做分类、选场景、跑方法链；命中风险再做边界检查；最后才决定交付文字版还是 HTML。
Weekly Installs
17
Repository
samadhifire/xin…an-skill
GitHub Stars
177
First Seen
Apr 10, 2026
