---
title: @gw-git-worktree-workflows
url: https://skills.sh/mthines/gw-tools/@gw-git-worktree-workflows
---

# @gw-git-worktree-workflows

skills/mthines/gw-tools/@gw-git-worktree-workflows
@gw-git-worktree-workflows
Installation
...
Creates a gw_root branch for the bare repository
Auto-detects the default branch (main, master, etc.)
Creates the gw configuration at .gw/config.json
Creates your first worktree for the default branch
Names the repository directory with .git suffix (bare repo convention)
...
Detects that feat/new-feature doesn't exist locally or on remote
Fetches the latest version of main from the remote (origin/main)
Creates your new branch from the fresh remote ref
...
Fetching the latest version of main (or specified branch) from remote
Updating your current branch using either merge or rebase strategy
Handling conflicts and providing clear guidance
...
Merge parent feature first: feature-auth → main
Rebase or merge child onto updated main
Merge child feature: feature-auth-social → main
...
Fetches PR information using the GitHub CLI (gh)
Fetches the PR branch (works for forks too)
Creates a worktree with the PR's branch name
Copies auto-copy files (same as gw add)
...
Share node_modules with symlinks (advanced, use with caution):
# In feature worktree
rm -rf node_modules
ln -s ../main/node_modules node_modules

Use pnpm (shares packages automatically):
...
✅ Troubleshooting common issues
Next Steps
Try creating your first worktree with gw add
Set up auto-copy configuration (see config-management skill)
Explore autonomous workflows (see autonomous-workflow skill)
