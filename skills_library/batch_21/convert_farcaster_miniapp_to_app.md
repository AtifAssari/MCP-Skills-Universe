...
url: https://skills.sh/base/skills/convert-farcaster-miniapp-to-app
---

# convert-farcaster-miniapp-to-app

skills/base/skills/convert-farcaster-miniapp-to-app
convert-farcaster-miniapp-to-app
Installation
...
Detect framework from package.json (next, vite, react-scripts, @remix-run/*)
Count Farcaster packages in dependencies and devDependencies
...
Remove import of farcasterMiniApp or miniAppConnector from @farcaster/miniapp-wagmi-connector
Remove the farcasterMiniApp() / miniAppConnector() call from the connectors array
If no wallet connector remains, add:
import { coinbaseWallet } from "wagmi/connectors";

coinbaseWallet({ appName: "<app name>" })

If coinbaseWallet already exists, leave it as-is
Clean up empty lines and stale imports
...
Replace with:
Get wallet address from useAccount() (wagmi)
Create a SIWE message with siwe
Sign with useSignMessage() (wagmi)
Send signature + message to the backend for verification
...
Verify the signature with siwe or viem
Use recovered wallet address as the normal app identity
