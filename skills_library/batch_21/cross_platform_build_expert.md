---
title: cross-platform build expert
url: https://skills.sh/martinholovsky/claude-skills-generator/cross-platform-build-expert
---

# cross-platform build expert

skills/martinholovsky/claude-skills-generator/Cross-Platform Build Expert
Cross-Platform Build Expert
Installation
...
2. Core Principles
TDD First - Write build configuration tests before implementing
Performance Aware - Optimize build times, bundle sizes, and startup
Test on all target platforms - Don't assume cross-platform compatibility
Use platform abstractions - Rust std, Tauri APIs for platform differences
Handle path differences - Forward vs backward slashes, case sensitivity
Respect platform conventions - File locations, UI guidelines
Sign all releases - Users trust signed applications
Protect signing credentials - Never commit certificates
Verify signatures - Check before distribution
Use timestamping - Signatures remain valid after certificate expiry
...
Testing on each target platform (not just your development machine)
Proper code signing for user trust
Platform-specific configurations and dependencies
Awareness of distribution requirements
