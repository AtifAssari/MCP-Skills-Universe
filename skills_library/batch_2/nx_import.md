---
title: nx-import
url: https://skills.sh/nrwl/nx-ai-agents-config/nx-import
---

# nx-import

skills/nrwl/nx-ai-agents-config/nx-import
nx-import
Installation
...
Install ESLint deps first: pnpm add -wD eslint@^9 @nx/eslint-plugin typescript-eslint (plus framework-specific plugins)
Create root eslint.config.mjs (copy from source or create with @nx/eslint-plugin base rules)
Then npx nx add @nx/eslint to register the plugin in nx.json
...
Run npx nx add @nx/jest — registers @nx/jest/plugin in nx.json and updates namedInputs
Create jest.preset.js at workspace root (see references/JEST.md for content) — nx add only creates this when a generator runs, not on bare nx add
Install test runner deps: pnpm add -wD jest jest-environment-jsdom ts-jest @types/jest
Install framework-specific test deps as needed (see references/JEST.md)
...
Remove "noEmit": true. If inherited via extends chain, set "noEmit": false explicitly.
Add "composite": true, "emitDeclarationOnly": true, "declarationMap": true
Add "outDir": "dist" and "tsBuildInfoFile": "dist/tsconfig.tsbuildinfo"
Add "extends": "../../tsconfig.base.json" if missing. Remove settings now inherited from base.
