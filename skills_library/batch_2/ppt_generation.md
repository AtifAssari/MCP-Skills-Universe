...
url: https://skills.sh/bytedance/deer-flow/ppt-generation
---

# ppt-generation

skills/bytedance/deer-flow/ppt-generation
ppt-generation
Installation
...
Generates slide images sequentially with reference chaining—each slide uses the previous one as a visual reference to maintain consistent aesthetics across the entire presentation
Requires a JSON presentation plan defining structure, style guidelines, aspect ratio, and per-slide content before image generation begins
Composes final PPTX from ordered slide images after all images are generated; supports custom titles, subtitles, key points, and visual descriptions per slide
SKILL.md
...
For subsequent slides (slide 2+), use the PREVIOUS slide as a reference image:
{
...
Continue for all remaining slides, always referencing the previous slide:
