...
url: https://skills.sh/nomadamas/k-skill/korean-law-search
---

# korean-law-search

skills/nomadamas/k-skill/korean-law-search
korean-law-search
Installation
...
한국 법령/조문/판례/유권해석/자치법규 조회가 필요할 때 기본 경로로 korean-law-mcp를 먼저 사용하고, 기존 서비스가 동작하지 않을 때는 승인된 fallback 표면인 법망(https://api.beopmang.org) 으로 이어간다.
...
이 스킬은 자체 npm/python 패키지를 만들지 않는다. 한국 법령 관련 조회는 기본적으로 korean-law-mcp 로 처리하고, 해당 경로가 막히거나 실패가 반복될 때만 승인된 fallback 표면인 법망을 사용한다.
...
로컬 CLI 또는 로컬 MCP server 경로는 LAW_OC 가 필요하다. remote MCP endpoint는 사용자 LAW_OC 없이 url만으로 연결한다.
...
설치가 막힌 환경에서는 remote endpoint를 사용한다. 이 upstream 예시는 사용자 LAW_OC 없이 url만 등록한다.
...
Response policy
한국 법령 관련 요청은 항상 korean-law-mcp를 먼저 사용한다.
...
법적 판단이 필요한 경우 검색 결과 요약과 원문 출처까지만 제공하고 법률 자문처럼 단정하지 않는다.
Done when
