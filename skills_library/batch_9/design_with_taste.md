...
url: https://skills.sh/cristicretu/family-taste-skill/design-with-taste
---

# design-with-taste

skills/cristicretu/family-taste-skill/design-with-taste
design-with-taste
Installation
...
Animate numbers and live charts. Values that change (prices, counts, balances) should count/flip/morph. Commas should shift position smoothly as numbers grow — never just swap. For real-time line charts, use liveline (npm i liveline) — one canvas, no dependencies beyond React 18, 60fps interpolation. For 60fps value overlays, update the DOM directly rather than through React state to avoid re-render overhead.
...
Static tab switches. No directional slide = digital whiplash.
Modals that pop from nowhere. Grow from trigger or slide from edge. Never just opacity: 0 → 1 centered.
Skeleton screens that don't match the real layout. If the skeleton has 3 bars and the content has 5 lines, you've broken the illusion.
Redundant animations. A persistent header that fades out and back in during a page transition. Persistent = stays.
Linear easing. Nothing in the physical world moves linearly.
"No items" empty text. First impression. Treat it like one.
Uniform sizing in stacked layers. Two sheets the same height = no sense of depth or progression.
Toasts for important outcomes. Toasts = background info. Success/error/completion = inline, contextual, animated.
Forms that are just stacked inputs. Step-by-step with transitions between them.
Buttons that don't respond to interaction. Hover, active, and focus states. Always.
Animating unchanged text. If only one word changes in a sentence, only that word moves.
Spinner left at origin after action. Move it to where the user will look for the result.
...
Read this before every UI task. Not after. Before.
Apply pillars in order. Simplicity → Fluidity → Delight. You can't polish a bad layout with animations.
Run the checklist before delivering.
Pair with frontend-design skill for visual aesthetics (typography, color, layout). This skill handles feel and interaction quality.
When in doubt, animate. Easier to tone down than to add life to a dead interface.
Record and review at 0.5x. Slow motion reveals every teleport, every jarring cut, every missed opportunity.
