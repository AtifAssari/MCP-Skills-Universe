---
title: web-search
url: https://skills.sh/netease-youdao/lobsterai/web-search
---

# web-search

skills/netease-youdao/lobsterai/web-search
web-search
Installation
...
Total results count
Search duration
Individual result titles and URLs
...
Optimization Tips
Browser persistence - The browser stays alive across searches by default. Connections and pages are reused automatically. Set WEB_SEARCH_CLEANUP=1 to close the browser after each search if needed.
Limit results - Request only what you need (5-10 is usually enough)
Batch searches - If multiple searches needed, do them consecutively to reuse connection
Specific queries - More specific = faster and better results
...
Limitations
Current Limitations
No CAPTCHA handling - If Google or Bing shows CAPTCHA, user must solve manually
Engine availability varies by network/region - Auto mode falls back between Google and Bing
English/Chinese focus - Optimized for English and Chinese results
Basic extraction - Extracts titles and snippets, not full page content
No authentication - Cannot search pages requiring login
Not Suitable For
...
Be specific in queries - Include version numbers, dates, or specific aspects
Parse results carefully - Don't just copy-paste, synthesize information
Verify with multiple sources - Cross-check important information
Cite sources - Tell user which sources you're using
Explain limitations - If search doesn't find good results, tell user
Use follow-up searches - One search might not be enough, do multiple if needed
Check result dates - Prefer recent articles for current info
Look for official sources - Prioritize official docs and authoritative sources
Technical Details
Technologies Used
...
Run basic test: node SKILLs/web-search/scripts/test-basic.js
