---
title: streaming-ssr
url: https://skills.sh/patternsdev/skills/streaming-ssr
---

# streaming-ssr

skills/patternsdev/skills/streaming-ssr
streaming-ssr
Installation
...
renderToPipeableStream(element, options) for Node.js HTTP responses.
renderToReadableStream(element, options) for Web Streams runtimes such as edge environments.
...
Frameworks that use the server-render-pass to generate markup that needs to be added to the document before the SSR-ed chunk. Examples are frameworks that dynamically determine which CSS to add to the page in a preceding <style> tag.

Code, where renderToStaticMarkup is used to generate the page template and renderToString calls are embedded to generate dynamic content. Since the string corresponding to the component is expected in these cases, it cannot be replaced by a stream. For example:
