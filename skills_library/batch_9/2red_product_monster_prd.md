---
title: 2red-product-monster-prd
url: https://skills.sh/kira2red/kira-product-monster-skills/2red-product-monster-prd
---

# 2red-product-monster-prd

skills/kira2red/kira-product-monster-skills/2red-product-monster-prd
2red-product-monster-prd
Installation
$ npx skills add https://github.com/kira2red/kira-product-monster-skills --skill 2red-product-monster-prd
SKILL.md
2RED Product Monster PRD
Overview
...
核心写作铁律 (Strict Rules)
绝对精简与“大白话” (文风约束)：极端去修饰化，平铺直叙。每句话必须是主谓宾明确的动作描述。严禁使用主观形容词、情绪化表达和夸张的副词。
严禁大厂黑话 (词汇黑名单)：严禁出现“赋能、抓手、沉网、沉淀、顶层架构、底层逻辑、闭环、羁绊、夺权、剥夺”等无实际业务意义的包装词汇。
严禁纯技术视角 (越界黑名单)：PRD 是业务交互文档，严禁跨界指导开发的技术实现或选型。禁止出现“Redis锁、DOM操作、Blob转码、Payload报文、落库”等前端/后端代码开发级词汇。
排版公式化约束 (结构约束)：
描述入口时，强制使用路径法：模块 - 页面 - 按钮。
描述交互时，强制采用极简组合：【动作/条件】 -> 【系统反馈/交互表现】。
所有段落必须打碎为短句 bullet points（短项目符号），绝对禁止生成超过两行连贯的长段落大段文字。
严格的序号层级：必须遵循 1. -> 1.1 -> 1.1.1 -> • -> 。 的层级嵌套关系。绝对不允许层级混乱或随意更改缩进。
核心写作原则
1. 根据功能复杂度选择编写方式
...
采用渐进式编写,一个模块一个模块完成
每完成一个模块后:
必须输出该模块的完整 Markdown 原文(用代码块包裹)
询问用户:"这个模块的描述是否清晰?有需要调整的地方吗?"
根据用户反馈调整后续模块的写法和详细程度
