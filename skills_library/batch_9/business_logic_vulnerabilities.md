...
url: https://skills.sh/yaklang/hack-skills/business-logic-vulnerabilities
---

# business-logic-vulnerabilities

skills/yaklang/hack-skills/business-logic-vulnerabilities
business-logic-vulnerabilities
Installation
...
For the formal 5-phase workflow — Business Modeling → State Machine → Attack-Surface Matrix → Checklist-Driven Testing → Human Judgement — load METHODOLOGY.md. It includes a single-page decision tree (Q1 ~ Q7) for "I'm staring at a request and don't know what to try first".

10. HIGH-IMPACT CHECKLISTS

For the full per-module list (login / register / password recovery / payment / coupon / order / IDOR / privacy / VIP / URL redirect / cookie & token / race / comments) with why and verify columns — load CHECKLIST.md.
...
For full item / why / verify triplets including reproduction steps and tooling per item, load CHECKLIST.md. This section keeps only the item line for fast scanning.
...
Print or screenshot the relevant 1-2 sub-sections for the target's surface.
For each □ mark: NOT-APPLICABLE / NOT-VULN / VULN / NEEDS-RECHECK.
Mark the EXACT endpoint + parameter + payload that proved the bug (or proved it absent), so the report is reproducible.
Cross-reference with METHODOLOGY.md Q1~Q7 decision tree when an item triggers something unexpected — the tree tells you which neighboring items are likely also vulnerable.
For deep payloads / curl-ready commands / Burp screenshots per item, load CHECKLIST.md (full triplets) and SCENARIOS.md (real cases).
Weekly Installs
331
Repository
yaklang/hack-skills
