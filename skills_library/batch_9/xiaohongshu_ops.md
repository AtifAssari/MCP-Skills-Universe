---
title: xiaohongshu-ops
url: https://skills.sh/xiangyu-cas/xiaohongshu-ops-skill/xiaohongshu-ops
---

# xiaohongshu-ops

skills/xiangyu-cas/xiaohongshu-ops-skill/xiaohongshu-ops
xiaohongshu-ops
Installation
$ npx skills add https://github.com/xiangyu-cas/xiaohongshu-ops-skill --skill xiaohongshu-ops
Summary

Xiaohongshu operations across account positioning, topic research, content production, publishing, and post-launch review.

Covers the full Xiaohongshu workflow: account positioning and content direction, topic ideation with controversy mapping, competitive benchmarking, pre-publish rehearsal, and post-launch analytics including comment management and trend tracking
Includes Viral Copy mode: input a high-performing post URL and generate a publishable replica with matched cover design, title, body text, and hashtags while avoiding plagiarism
Enforces mandatory use of OpenClaw's built-in browser with profile="openclaw" for all browsing, search, publishing, and comment tasks unless explicitly requested otherwise
Provides reusable SOP templates for each operation type (positioning variables, topic structure, content templates with title/hook/body/CTA/hashtags, comment reply patterns) plus vertical-specific case modules (drama-watch example included)
Includes runtime rules for failure recovery, browser status validation, and safe publishing workflows that stop before the final publish button for user confirmation
SKILL.md
...
记录可复用字段：title, hook, angle, 结构标签, 评论信号, 互动CTA, 标签组
汇总前 10-20 条到候选池
A.1 首页推荐流分析（新增）
