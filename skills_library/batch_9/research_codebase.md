...
url: https://skills.sh/flora131/atomic/research-codebase
---

# research-codebase

skills/flora131/atomic/research-codebase
research-codebase
Installation
...
The agent fetches live web content using the playwright-cli skill (or bunx @playwright/cli / curl). Instruct it to apply the token-efficient fetch order: (1) try curl https://<site>/llms.txt for an AI-friendly index (see llmstxt.org), (2) try curl <url> -H "Accept: text/markdown" to get pre-converted Markdown (supported on Cloudflare-hosted docs via Markdown for Agents), (3) fall back to HTML parsing via playwright-cli
