---
title: feishu-cli-card
url: https://skills.sh/riba2534/feishu-cli/feishu-cli-card
---

# feishu-cli-card

skills/riba2534/feishu-cli/feishu-cli-card
feishu-cli-card
Installation
...
职责：把用户的意图 → 构造出视觉丰富的 v2 卡片 JSON，写入 /tmp/<name>-card.json， 再交给 feishu-cli msg send --msg-type interactive --content-file 发送。

只做 v2：所有输出必须以 "schema": "2.0" 开头，用 body 包裹 elements。 v1 相关迁移见 references/v2-vs-v1.md。
...
schema: "2.0" — 顶层声明，v2 必须
config — 全局行为：update_multi v2 只支持 true；width_mode 用 fill/compact/省略（默认 600px）
header — 标题区：template 定主题色，title 必填，subtitle 可选
body.elements — 组件数组，按顺序从上到下渲染
3. 工作流（6 步）
选模板：按决策树确定 templates/<name>.json，Read 进来作为起点
填内容：替换所有 __PLACEHOLDER__ 标记位（标题、字段值、URL、数据、人员 ID）
选配色：header.template 按场景挑（参见 references/design.md 配色矩阵），markdown 内 <font> 强调色不超过 3 种
校验：
python -m json.tool /tmp/xxx-card.json 过 JSON 解析
检查无 v2 禁用字段（见第 6 节禁区）
落盘：Write 到 /tmp/<name>-card.json（复用已有路径避免污染）
发送：
feishu-cli msg send \
...
tag	用途	关键属性
button	按钮	type + text + `behaviors: [{type: "open_url"
...
发送成功标志：返回 消息 ID: om_xxxxx。如果报 content format error 或 invalid element， 回到第 6 节禁区表逐项检查。
