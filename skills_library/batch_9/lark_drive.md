...
skills/larksuite/cli/lark-drive
lark-drive
Installation
...
用户要在 Drive 里上传、创建、读取、覆盖更新原生 .md 文件（不是导入成 docx），切到 lark-markdown。
...
+status	Compare a local directory with a Drive folder by SHA-256 content hash; reports new_local / new_remote / modified / unchanged (read-only diff primitive for sync workflows). --local-dir 必须是 cwd 内的相对路径，越界路径 CLI 会直接拒绝；目标在 cwd 外时引导用户切换 agent 工作目录，不要私自 cd 绕过。
