...
url: https://skills.sh/yuan-93/smart-folder-organizer-skill/smart-folder-organizer
---

# smart-folder-organizer

skills/yuan-93/smart-folder-organizer-skill/smart-folder-organizer
smart-folder-organizer
Installation
...
Core Mandates
Safety First: NEVER delete any file. Move files intended for deletion to specific subfolders within to_remove/ (e.g., to_remove/duplicates/ or to_remove/empty_folders/).
Strict Scoping: Operate ONLY within the current directory and its subdirectories. Never move or access files in the parent directory (..).
Preservation: If a file's category is ambiguous, move it to a Needs_Review/ folder rather than guessing incorrectly.
Large Files: For any file larger than 20MB, skip deep content analysis (such as reading PDF text) to prevent memory issues. Move these unanalyzed large files to a dedicated Needs_Review/LargeFiles/ directory.
