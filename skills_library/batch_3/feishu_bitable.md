...
url: https://skills.sh/alextangson/feishu_skills/feishu-bitable
---
...
用 POST .../records/search 拉取全量记录后在客户端计算
在多维表格中创建公式字段（如 SUM、COUNT）后通过 API 读取字段值
最佳实践
批量操作优先（减少 API 调用）
字段类型严格匹配（避免写入失败）
日期用毫秒时间戳（Python: int(datetime.timestamp() * 1000)）
关联字段实现关系型能力
