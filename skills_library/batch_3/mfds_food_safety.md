---
title: mfds-food-safety
url: https://skills.sh/nomadamas/k-skill/mfds-food-safety
---

# mfds-food-safety

skills/nomadamas/k-skill/mfds-food-safety
mfds-food-safety
Installation
$ npx skills add https://github.com/nomadamas/k-skill --skill mfds-food-safety
SKILL.md
식품 안전 체크
...
k-skill-proxy의 food-safety route들이 있는 hosted/self-host 프록시에 접근 가능할 것
...
Workflow
증상/섭취상황이 있으면 인터뷰를 먼저 진행한다.
red flag 가 있으면 즉시 응급 안내로 전환한다.
"이거 먹어도 되나?" 류 질문:
...
제품명, 업체명, 기능성, 섭취량, 주의사항, 부적합 사유를 짧게 정리하고, 먹어도 되는지 단정하지 않는다.
프록시가 FOODSAFETYKOREA_API_KEY 없이 동작 중이면 결과가 sample feed 기반일 수 있음을 warnings 로 확인한다.
CLI examples
