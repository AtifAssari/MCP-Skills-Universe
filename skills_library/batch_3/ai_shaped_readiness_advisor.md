...
url: https://skills.sh/deanpeters/product-manager-skills/ai-shaped-readiness-advisor
---

# ai-shaped-readiness-advisor

skills/deanpeters/product-manager-skills/ai-shaped-readiness-advisor
ai-shaped-readiness-advisor
Installation
...
Identifies foundational gaps—Context Design is prerequisite for orchestration and acceleration—and recommends which competency to prioritize based on your current state
Provides tailored 4-week action plans for each competency, from building constraints registries and memory architecture to compressing learning cycles and establishing team norms
Includes diagnostic framework to detect context stuffing (volume without intent) versus context engineering (structure for attention)
SKILL.md
Purpose
...
Context Design is foundational — If Level 1-2, this must be priority #1 (Agent Orchestration and Outcome Acceleration depend on it)
Agent Orchestration enables Outcome Acceleration — If Context Design is Level 3+, but Agent Orchestration is Level 1-2, prioritize orchestration
Team-AI Facilitation is parallel — Can be developed alongside others, but required for scale
Strategic Differentiation requires Levels 3+ on others — Don't focus here until foundational competencies are built
...
Accept recommendation — [Agent provides detailed action plan]
Choose different priority — [Agent warns about dependencies but allows override]
Focus on multiple simultaneously — [Agent suggests parallel tracks if feasible]
...
Run through the 5 diagnostic questions (see context-engineering-advisor)
Identify what you're persisting that should be retrieved
Assign context boundary owner
Create Context Manifest (what's always-needed vs. episodic)

Phase 1: Document Constraints (Week 1)

Create a constraints registry:
...
Apply diagnostic question #4 to each constraint: "What fails if we exclude this?"
Format: Structured file AI agents can parse (YAML, JSON, or Markdown with frontmatter)
Version control in Git

Phase 2: Build Operational Glossary (Week 2)

List top 20-30 terms your team uses (e.g., "user," "customer," "activation," "churn")
Define each unambiguously (avoid "it depends")
Include edge cases and exceptions
Add to CLAUDE.md or project instructions
This becomes your long-term persistent memory (Declarative Memory)
...
Reject: "I think," "We feel," "It seems like"
Define context boundaries using the 5 diagnostic questions:
What specific decision does each piece of context support?
Can retrieval replace persistence?
Who owns the context boundary?
Create Context Manifest document
Codify in team docs

Phase 4: Implement Memory Architecture + Workflows (Week 4)

Set up two-layer memory:
...
Implement Research→Plan→Reset→Implement cycle:
...
Update AI prompts to reference constraints registry and glossary
Test: Ask AI to cite constraints when making recommendations
Measure: % of AI outputs that cite evidence vs. hallucinate; token usage efficiency
...
Pick your most frequent AI use case (e.g., "analyze user feedback")
Document every step you currently take:
Copy/paste feedback into ChatGPT
Ask for themes
Manually categorize
Write summary
Identify pain points (manual handoffs, inconsistent results)

Phase 2: Design Orchestrated Workflow (Week 2)

Define workflow loop:
...
Each step must be traceable (show sources, reasoning)
...
Run on 3 past examples; compare to manual process
Measure: Time saved, consistency improved, traceability added

Phase 4: Document and Scale (Week 4)

Version-control prompts (Git)
Document workflow steps for team
Train 2 teammates; observe results
...
Map your current learning cycle (e.g., hypothesis → experiment → analysis → decision)
Time each step
Identify slowest step (usually: validation lag, approval delays, or meeting overhead)

Phase 2: Design AI Intervention (Week 2)

Ask: "What if this step happened overnight?"
...
Design minimal AI workflow to eliminate bottleneck

Phase 3: Run Pilot (Week 3)

Test AI intervention on 1 real initiative
Measure cycle time: before vs. after
Validate quality: Did AI maintain rigor, or cut corners?

Phase 4: Scale (Week 4)

If successful (cycle time down 50%+, quality maintained), apply to 3 more initiatives
Document workflow
Train team
...
Codify rule: "AI outputs are drafts, not finals"
...
AI must cite sources (no hallucinations)
Reject outputs that say "I think" or "it seems"
Require: "According to [source], [fact]"
Add to team operating docs

Phase 3: Define Decision Authority (Week 3)

Clarify: AI recommends, humans decide
Document who has authority to override AI recommendations (PM, team lead, cross-functional consensus)
Create escalation path (what if AI and human disagree?)

Phase 4: Build Psychological Safety (Week 4)

Team exercise: Share an AI mistake you caught (normalize catching errors)
Reward critical thinking ("Good catch on that AI hallucination!")
Avoid: "Why didn't you just use AI?" (shaming)
...
Ask: "What could we do with AI that competitors can't replicate by adding headcount?"
...
List 5 candidates
Prioritize by defensibility (how hard to copy?)
...
Sketch MVP (minimum viable moat)

Phase 3: Build and Test (Weeks 3-4)

Build prototype (can be PoL probe, not production)
Test with 5 customers
Measure: Does this create value competitors can't match?

Phase 4: Validate Moat (Week 5)

Ask: "How would a competitor replicate this?"
...
Decide: Build full version, pivot, or kill
...
Yes, create tracker — [Agent generates Markdown checklist]
No, I'll track separately — [Agent provides summary]
Examples
...
External Frameworks
Dean Peters — AI-First Is Cute. AI-Shaped Is Survival. (Dean Peters' Substack, 2026)
Dean Peters — Context Stuffing Is Not Context Engineering (Dean Peters' Substack, 2026) — Deep dive on Competency #1 (Context Design)
Further Reading
