---
title: okx-cex-skill-mp
url: https://skills.sh/okx/agent-skills/okx-cex-skill-mp
---
...
Always try okx skill add <name> first — this downloads the skill and installs it to all detected agents (Claude Code, OpenClaw, Cursor, Windsurf, etc.) in one step.
Only if add fails, fall back to manual download:
...
Runs npx skills add to install to all locally detected agents
Records the installation in ~/.okx/skills/registry.json
