...
skills/jimliu/baoyu-skills/baoyu-xhs-images
baoyu-xhs-images
Installation
...
Provides preset shortcuts (knowledge-card, tutorial, poster, cinematic, etc.) that bundle style + layout for common content scenarios
Includes three outline strategies (story-driven, information-dense, visual-first) with smart confirmation workflow; detailed mode generates three variants for comparison before image generation
Maintains visual consistency across series using reference image anchoring and optional watermark support
SKILL.md
...
Step 1: Analyze Content → analysis.md
Save the source (backup rule applies if source.md exists).
Run the deep analysis in references/workflows/analysis-framework.md: content type, hook potential, audience, engagement signals, visual opportunity map, swipe flow.
Detect source language, pick recommended image count (2-10).
Auto-recommend strategy + style + layout + palette using the Auto-Selection table above.
Write everything to analysis.md.
...
Write the full prompt to prompts/NN-{type}-{slug}.md in the user's preferred language (backup rule applies).
Generate:
