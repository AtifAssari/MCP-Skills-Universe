---
title: okx-sentiment-tracker
url: https://skills.sh/okx/agent-skills/okx-sentiment-tracker
---

# okx-sentiment-tracker

skills/okx/agent-skills/okx-sentiment-tracker
okx-sentiment-tracker
Installation
$ npx skills add https://github.com/okx/agent-skills --skill okx-sentiment-tracker
SKILL.md
OKX News & Sentiment
...
Configure credentials in ~/.okx/config.toml
Verify setup:
...
If --platform was used — broaden --begin to 7 days back, then 30 days back, before changing anything else. Bursty sources routinely return 0 items in the default window but dozens over a wider range.
If --importance high was passed — drop it (default is already low = all news).
Broaden --begin / --end for any query (not just --platform) when a narrow time window is suspected.
Drop --coins to get general news if the coin-specific query yielded nothing.
Use web search as a supplement — search the web for "<topic> news site:coindesk.com OR site:cointelegraph.com OR site:theblock.co" to gather additional context, then combine with any OKX results into a unified briefing.
Be transparent — tell the user which results came from OKX API vs. web search so they can judge source credibility.
...
Broaden --begin to 7 days back, then 30 days back, and retry.
If still empty after a 30-day window, report to the user that no recent articles were found for that source and suggest either removing --platform (to fall back to the aggregated feed) or web search.
