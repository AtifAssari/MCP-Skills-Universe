...
skills/supercent-io/skills-template/ai-tool-compliance
ai-tool-compliance
Installation
...
must_not_contain violations (requires immediate removal) → delete the code or replace with server API call
must_contain unmet (pattern needs to be added) → insert code per the remediation guide
Warning (partially met) → apply supplement only to unmet files
P0 Rule Catalog
...
/workflow-init (bmad) -- establish project structure
/compliance-init (compliance) -- generate compliance foundation
After Phase 3 Architecture completes, /compliance-scan -- architecture-level compliance check
After Phase 4 Implementation completes, /compliance-verify -- full verification + deploy gate verdict
...
Parse rules/p0-catalog.yaml (yq or grep-based)
For each rule:
...
Output results in templates/verification-run.json format
...
Render templates/risk-score-report.md
Generate templates/remediation-task.json (per FAIL item)
...
Best practices
Shift Left: At project start, generate the foundation with /compliance-init before implementing business logic
Gradual adoption: Adopt in order: pass all P0 first -> then P1. Do not start with P1
Re-verification loop: After fixing violations, always re-run /compliance-verify to confirm score changes
BMAD integration: Adopt running compliance-verify after bmad-orchestrator Phase 4 as the standard workflow
