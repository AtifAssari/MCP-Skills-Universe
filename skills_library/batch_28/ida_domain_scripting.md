---
title: ida-domain-scripting
url: https://skills.sh/hexrayssa/ida-claude-plugins/ida-domain-scripting
---

# ida-domain-scripting

skills/hexrayssa/ida-claude-plugins/ida-domain-scripting
ida-domain-scripting
Installation
...
Create a work dir in /tmp with timestamp - NEVER write scripts to skill directory; always create a workdir /tmp/ida-domain-YYYYMMDD_HHMMSS_ffffff-<name> with microseconds for uniqueness (e.g., /tmp/ida-domain-20260109_143052_847291-list-functions). Generate timestamp with: datetime.now(). strftime ('%Y%m%d_%H%M%S_%f'). This will always be referenced as <work_dir>

Check API_REFERENCE.md exists - Always check that $SKILL_DIR/API_REFERENCE.md exists. Inform the user to run the bootstrap if not.

Execute from skill directory - Always run: cd $SKILL_DIR && uv run python run.py <work_dir>/script.py -f <binary>

Ask before saving - Scripts that modify the database require explicit user confirmation before using --save

How It Works
You describe what you want to analyze/extract
I write custom IDA Domain API code in <work_dir>/script.py (timestamped with microseconds for parallel execution)
I execute it via: cd $SKILL_DIR && uv run python run.py <work_dir>/script.py -f <binary>
Results displayed in real-time
Script files auto-cleaned from /tmp by your OS
Setup (First Time)
