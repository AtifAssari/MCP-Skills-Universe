---
title: using-aisdlc
url: https://skills.sh/zixun-github/aisdlc/using-aisdlc
---

# using-aisdlc

skills/zixun-github/aisdlc/using-aisdlc
using-aisdlc
Installation
...
入口预检（Preflight）：若分支不合规或 raw.md 缺失 → 进入 R0 spec-init（细则见 routing-requirements.md）。
门禁：若下一步会读写 {FEATURE_DIR}/... 或写 demo → 先执行 spec-context 获取上下文，回显 FEATURE_DIR=...，失败则进入 spec-init。
读取上一步 ROUTER_SUMMARY（如有）：若 blocked=true → 按本文件的硬中断规则停下并给出候选下一步。
D0 强制门禁（R→I 过渡）：当 solution.md 已存在且下一步意图指向 I1 时，必须执行以下步骤，禁止未经 D0 直接进入 I1：
...
Auto-Advance：未命中硬中断则 Router 必须自动推进并立即执行下一步 worker。
最短闭环（仅作为入口提示）
