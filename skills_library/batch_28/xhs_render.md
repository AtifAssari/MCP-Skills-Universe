---
title: xhs-render
url: https://skills.sh/ing-la/agent-skills-share/xhs-render
---

# xhs-render

skills/ing-la/agent-skills-share/xhs-render
xhs-render
Installation
$ npx skills add https://github.com/ing-la/agent-skills-share --skill xhs-render
SKILL.md
...
【可选】布局计划确认（在渲染前与用户多轮交互，提高结果贴合度）：

触发条件：用户明确要求「先确认布局」「多交互几次」「详细分块」等；或文档较长、步骤多、表格多时，Agent 主动建议走此流程。
执行方式：LLM 先阅读文档，输出一份分图布局计划（不生成 blocks.json），格式示例：
## 布局分块方案（共 N 张图）
...
LLM 设计 blocks.json 与 xhs-copy（一次产出；若已走步骤 4，则严格按用户确认的布局计划执行）：

读取定位到的文档
角色：你是小红书配图设计专家，擅长把长文案重排成「有节奏、有冲击力」的视觉结构。严禁照抄原文——必须按屏上可读性重新提炼、缩写、造句。
任务：从文档中提炼核心信息，设计一套配图（blocks.json）及配套发帖文案（xhs-copy）
blocks.json 刚性约束：
Cover：text 不得超过 2 行（约 20 字），只放金句或主卖点；主文案建议 6–12 字为佳（如「AI 文案去味神器」），既能撑满版面又不会挤；必须在 cover 块中显式添加 "skill": "<技能名>"（如 "skill": "humanizer-zh"），封面会在主文案下方以「技能名」形式醒目展示；若未填则脚本会从目录名 YYYY-MM-DD-<skill-name> 自动解析
...
渲染：python .cursor/skills/xhs-render/scripts/render_images.py <输出目录>/blocks.json -t <用户所选模板> -o <输出目录>
