---
title: using-gitee-cli
url: https://skills.sh/hex2dec/gitee-cli/using-gitee-cli
---

# using-gitee-cli

skills/hex2dec/gitee-cli/using-gitee-cli
using-gitee-cli
Installation
$ npx skills add https://github.com/hex2dec/gitee-cli --skill using-gitee-cli
SKILL.md
Using gitee-cli
...
gitee-cli is intentionally shaped like gh, but it covers a smaller surface. Think in these groups first:
...
Do not assume every gh command exists. Confirm support with gitee help or gitee help <topic> before suggesting a command.
...
For create and comment commands, use one body source at a time: inline text or --body-file. Add --repo owner/repo only when local repo context is unavailable.
...
pr status requires a local git checkout and authentication. pr checkout requires a local git checkout with an origin remote. Add --repo owner/repo only when local repo context is unavailable.
