...
url: https://skills.sh/maxritter/claude-codepro/testing-standards
---

# testing standards

skills/maxritter/claude-codepro/Testing Standards
Testing Standards
Installation
...
Core Principles
Mocks are isolation tools, not test subjects - Assert on real behavior, not mock existence
Production code stays pure - Test utilities handle test-specific needs
Understand before mocking - Know what side effects you're removing
Complete mocks or none - Partial mocks create silent failures
TDD prevents anti-patterns - Write failing tests first to avoid testing mocks
...
Write test first → Forces clarity on what you're testing
Watch it fail → Confirms test verifies real behavior, not mocks
Minimal implementation → Prevents test-only methods
Real dependencies first → See actual needs before mocking
...
Write test with real dependencies first
Identify what's slow or external (API calls, DB queries, file I/O)
Mock only that specific dependency at lowest level
Verify test still validates real behavior, not mock presence
Weekly Installs
–
Repository
