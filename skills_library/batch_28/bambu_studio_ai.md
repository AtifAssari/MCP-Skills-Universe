---
title: bambu-studio-ai
url: https://skills.sh/heyixuan2/bambu-studio-ai/bambu-studio-ai
---

# bambu-studio-ai

skills/heyixuan2/bambu-studio-ai/bambu-studio-ai
bambu-studio-ai
Installation
...
Analyze → analyze.py --orient --repair on every model
Preview to chat → preview.py --views turntable → send image/GIF to user
Open Bambu Studio → open -a "BambuStudio" model.3mf (or model.stl/obj)
User slices in Bambu Studio → Tell user to slice, inspect, and confirm
...
search.py "query" --limit 5 → MakerWorld, Printables, Thingiverse, Thangs
Present results with name, source, URL
...
Confirm dimensions — MUST have before generate.py text
generate.py text "prompt" --wait --height <mm> → auto-enhances, auto-scales to exact target
preview.py model.stl --views turntable --height <mm> -o preview.gif → send GIF to chat (verifies dimensions)
→ Model Processing
...
Same disclaimer as B
Confirm dimensions only
Save image to workspace (if from chat) or note the file path/URL
...
Auto-detect single vs multi-color:
...
Preview + report to user → WAIT for approval
When user approves → Model Processing
...
Collect exact dimensions — width, height, depth, wall thickness, hole sizes, spacing
Identify standard interfaces: screw size (M3? M4?), mounting pattern, clearance vs press fit
Choose the right parametric.py command:
...
python3 scripts/parametric.py <command> [args] -o model.stl
preview.py model.stl --views turntable -o preview.gif → send GIF to chat
Report dimensions, volume, triangle count (printed by parametric.py)
WAIT for user approval — user may request dimension adjustments
If adjustments needed → modify command/spec, regenerate, re-preview
When user approves → Model Processing
...
Open in Bambu Studio: open -a "BambuStudio" model.3mf (or .stl/.obj)
Multi-color OBJ — special import procedure (BS import is unstable):
...
Close BS completely, reopen, then import again
Or drag-and-drop the OBJ file into an empty BS window
If BS still refuses to recognize vertex colors → flag as BS import compatibility issue; do NOT re-run colorize
Tell user to inspect and slice:
...
bambu.py print model.3mf --confirmed
...
Printer model — A1 Mini, A1, P1S, P2S, X1C, X1E, H2C, H2S, H2D, X2D
...
Notifications — macOS system notifications (automatic). Agent handles chat notifications via its own messaging tools.
Save — config.json + .secrets.json (chmod 600, git-ignored)
Verify (ask permission) — test connection, camera, AMS
Summary
