---
title: package-ida-plugin
url: https://skills.sh/hexrayssa/ida-claude-plugins/package-ida-plugin
---

# package-ida-plugin

skills/hexrayssa/ida-claude-plugins/package-ida-plugin
package-ida-plugin
Installation
...
Repository: Published at plugins.hex-rays.com and github.com/HexRaysSA/plugin-repository
...
Fork HexRaysSA/plugin-repository
Add your repo URL to known-repositories.txt (one per line)
...
Update plugin.version in ida-plugin.json
...
Minimal Python Plugin Checklist
 Create ida-plugin.json with required fields
 Ensure entry point file exists
 Add author with email
 Run hcli plugin lint
 Create GitHub Release with source archive
Adding Settings to Existing Plugin
 Define settings array in ida-plugin.json
 Add ida-settings to pythonDependencies if using the library
 Update plugin code to use get_current_plugin_setting()
...
Data files and assets colocated with code
 Restructure if needed: Move ida-plugin.json to where the plugin code lives
 Create ida-plugin.json with required fields
 Set correct entryPoint (relative to ida-plugin.json)
 Add appropriate idaVersions (test compatibility!)
 Declare pythonDependencies from requirements.txt
 Add README.md in plugin root for web display
 Validate and test locally: hcli plugin lint
