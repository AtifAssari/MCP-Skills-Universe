---
title: feature-video
url: https://skills.sh/everyinc/compound-engineering-plugin/feature-video
---

# feature-video

skills/everyinc/compound-engineering-plugin/feature-video
feature-video
Installation
$ npx skills add https://github.com/everyinc/compound-engineering-plugin --skill feature-video
SKILL.md
...
Check agent-browser get url -- if it shows github.com/login, the session expired. Re-run auth setup.
If still on the PR page, wait an additional 5 seconds and re-read the textarea (GitHub processing can be slow).
If validation still fails after retry, report the failure and the local video path so the user can upload manually.
