---
title: db-load-xml
url: https://skills.sh/arman-kudaibergenov/1c-ai-development-kit/db-load-xml
---

# db-load-xml

skills/arman-kudaibergenov/1c-ai-development-kit/db-load-xml
db-load-xml
Installation
...
Если ветка не совпала — используй default Если v8path не задан — автоопределение: Get-ChildItem "C:\Program Files\1cv8\*\bin\1cv8.exe" | Sort -Desc | Select -First 1 Если файла нет — предложи /db-list add. Если использованная база не зарегистрирована — после выполнения предложи добавить через /db-list add. Если в записи базы указан configSrc — используй как каталог загрузки по умолчанию.
Команда
...
1	Ошибка (см. лог)
После выполнения
Прочитай лог и покажи результат
Предложи выполнить /db-update — для применения изменений к БД
Примеры
# Полная загрузка
