---
title: contract-gen
url: https://skills.sh/zh-xx/legal-assistant-skills/contract-gen
---

# contract-gen

skills/zh-xx/legal-assistant-skills/contract-gen
contract-gen
Installation
$ npx skills add https://github.com/zh-xx/legal-assistant-skills --skill contract-gen
SKILL.md
合同生成
...
信息推断：对于用户未明确但可合理推断的信息（如国内合同适用中国法律），可直接推断填入；对于无法推断的具体数据（如金额、日期），使用 [____] 占位符
阶段二：合同生成
...
占位符规范：用户未提供的具体数字使用 [____] 占位符
仅输出合同正文：不输出说明性前后缀、致谢、提示语
...
结构完整性：必备条款是否齐全、条款编号是否连续、签字盖章区域是否完整
内容一致性：术语指代是否统一、数字金额是否一致、逻辑是否自洽、交叉引用是否正确
立场一致性：违约责任/付款条件/风险分配/争议管辖是否体现用户立场、倾斜是否合法合理
可执行性：条款是否可操作、法律引用是否准确、仲裁/管辖约定是否有效
格式规范性：Markdown 格式是否正确、法律术语是否准确、占位符格式是否统一
...
法律强制性规定 > 立场策略（如劳动合同中不得约定劳动者违约金）
合同类型适配规则 > 通用合同框架（如劳动合同必须使用"劳动报酬"而非"合同价款"）
用户明确要求 > 默认立场策略（如用户要求中立立场则不做倾斜）
立场推断规则
