...
url: https://skills.sh/lintendo/axhub-skills/generate-theme
---

# generate-theme

skills/lintendo/axhub-skills/generate-theme
generate-theme
Installation
$ npx skills add https://github.com/lintendo/axhub-skills --skill generate-theme
SKILL.md
生成主题
...
看 section 截图 sections/*/screenshot.png：
...
bg-background border border-border rounded-md px-3 py-2 text-foreground focus:ring-2 focus:ring-primary/20 focus:border-primary transition-colors
...
globals.css 生成要点
CSS 变量名遵循 shadcn/ui 命名规范（background, foreground, primary, muted, accent, destructive, border, ring）
颜色值从 theme.json 映射：
colors.background[0].value → --background
colors.text[0].value → --foreground
cssVariables 中的值优先使用（保留原始设计系统命名）
@theme inline 映射到 Tailwind
必须包含深色模式（如果 theme.json 无法推断，使用反转色）
圆角从 radius[0] 提取，其他级别用 calc 推导
token 类别对照表
