---
title: xhs-skill
url: https://skills.sh/leeguooooo/xhs-skill/xhs-skill
---

# xhs-skill

skills/leeguooooo/xhs-skill/xhs-skill
xhs-skill
Installation
...
用 agent-browser-stealth 打开登录页：
https://creator.xiaohongshu.com/login
若默认展示「手机号/验证码登录」，点击「扫码」切换到二维码视图

让 agent-browser-stealth 截图保存二维码（PNG）到 data/xhs_login_qr.png
...
用小红书 App 扫码完成登录后，导出 cookies 到 data/raw_cookies.json（不走 DevTools）：
agent-browser-stealth cookies --json > ./data/raw_cookies.json

归一化 cookies 并保存到 data/xhs_cookies.json：
...
先把发布素材整理为 data/publish_payload.json（示例）：
{
  "topic": "今日热点：xxxx",
  "source": {
    "name": "央视新闻",
...
只有当校验和审核结果都 ok=true 才允许进入发布页点击“发布/提交”。 校验策略在 ./config/verify_publish_policy.json，审核策略在 ./config/review_policy.json，分层风险路径在 ./config/review_taxonomy.json。
任一门禁失败必须中止流程并提示补齐，禁止“只传截图直接发”。
...
点击“暂存离开”后，必须等待并验证“保存成功/已保存草稿”类 toast。
进入草稿列表，验证出现“新草稿条目”。
新条目校验最少包含：
标题前缀匹配本次标题；
时间戳在本次运行窗口内（建议 2 分钟内）；
打开草稿后读回 title/body/media/tags 仍满足门禁。
任一校验失败判定为“草稿保存失败”，允许重试 1 次；重试后仍失败则中止流程。
...
第一重：发布后 URL 含 published=true。
第二重：从发布页菜单进入“笔记管理”，确认列表出现新笔记（标题前缀 + 时间窗口）。
管理页若被重定向或不可达，判定“回查未完成”，记录 run_log 并提示人工复核。
...
固定标题模板：[主题词]+[观点/结论]，目标 1218 字，留 28 字缓冲避免超长。
...
确保已登录（先完成上面的 A，或已有有效登录态）。
准备并校验 data/publish_payload.json（必须 ok=true）。
打开 https://creator.xiaohongshu.com/publish/publish，先 snapshot -i 获取最新结构。
切图文模式并上传媒体（确认缩略图比例与数量）。
用“语义定位优先 + DOM 兜底”填写标题与正文，写入后执行“双向读回校验 + 错位自愈”。
通过小红书标签交互选择标签与真实话题（至少 3 个），不要把标签写进标题。
执行发布前硬校验（标题、正文、图片、话题、无链接词）。
点击“暂存离开”并执行草稿闭环校验（toast + 列表新条目 + 读回）。
点击“发布/提交”前暂停，要求用户确认最终预览。
发布后执行双重确认（published=true + 菜单进入笔记管理二次查验），并写入 run_log。
...
确认已登录。
用 agent-browser-stealth 进入创作者中心的常用分析页（仪表盘/内容分析/粉丝分析）。
每个页面：
...
npm test
查看改动只包含预期文件：git status --short
用中文 Conventional Commit 提交（示例）：
