---
title: genie-editor-workflow
url: https://skills.sh/lintendo/axhub-skills/genie-editor-workflow
---

# genie-editor-workflow

skills/lintendo/axhub-skills/genie-editor-workflow
genie-editor-workflow
Installation
$ npx skills add https://github.com/lintendo/axhub-skills --skill genie-editor-workflow
SKILL.md
...
用户需要安装 Axhub AI Extension。安装地址：https://axhub.im/chrome/
...
确认扩展已安装并启用
在目标页面上打开 Genie Editor（点击扩展图标 → 启动编辑器）
确认后端服务正在运行（npx @axhub/genie status --json）
...
有上下文图片 → editor context-images export --output-dir <dir>
节点位置不明确 → editor node screenshot --element-key <key> --output-dir <dir>
截图返回的是 absolutePath，直接用 view_file 查看
...
前端采用指数退避重连策略（3s → 6s → 12s → ... → 48s，最多 10 次），避免服务端重启时被瞬间洪泛。

当用户切换回标签页时，前端会立即检查连接，已断线则立刻发起重连并重置计数器。
...
页面刷新后任务状态自动恢复（overlay 保留 "AI 编辑中"）
...
尽早 editing.set：在读取 snapshot 后立即标记 editing，让用户第一时间看到 AI 已领取
批量扫描一次，分批处理：先一次性读完所有待办，再逐个处理，避免反复 round-trip
小节点优先：先处理改动范围小的节点，让用户快速看到成果
并行截图：如果需要多个节点的截图，可以并行发起多个 node screenshot 请求
节流 context-images：全局上下文图片只导出一次，后续节点复用
多节点并行策略
