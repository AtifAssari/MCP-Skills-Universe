---
title: jobsearch-telegram
url: https://skills.sh/proficientlyjobs/proficiently-claude-skills/jobsearch-telegram
---

# jobsearch-telegram

skills/proficientlyjobs/proficiently-claude-skills/jobsearch-telegram
jobsearch-telegram
Installation
...
Choose a username (must end in bot, e.g., my_jobsearch_bot)
BotFather will give you a bot token — copy it and paste it here
...
Workflow
Step 1: Load Context
Read DATA_DIR/telegram-config.md — if missing, run First-Time Setup above and stop
Read DATA_DIR/telegram-state.md — if missing, create from template above
Read these if they exist: DATA_DIR/job-history.md, DATA_DIR/application-data.md, DATA_DIR/preferences.md
...
When field-approval confirmation arrives (Step 5), re-navigate to form_url, fill all fields, then send a second confirmation (submit approval) with a screenshot description and ask: "Everything looks good — submit?"

Store this as a new pending confirmation with stage: "submit-approval"

When submit-approval arrives, click Submit, then log the application (Step 9 of the apply skill).
...
Send acknowledgment: 🔍 Searching for: [keywords]...
Execute the job-search workflow from skills/job-search/SKILL.md
...
Add to Pending Confirmations with the job list so replies can be matched
Step 4c: Handle Tailor Request
Extract job reference (URL, "last", or job name)
Send acknowledgment: 📝 Tailoring resume for [job]...
Execute the tailor-resume workflow from skills/tailor-resume/SKILL.md
Send result to Telegram with key changes made
Note the file path where the tailored resume was saved
...
Look up the pending confirmation in telegram-state.md
...
Send confirmation of what was done
Remove from Pending Confirmations, add to Recent Actions
Step 6: Handle Plain Text Note
Log to DATA_DIR/telegram-inbox.md with timestamp
If it looks like a company name or job title, suggest: "Want me to search for [text] jobs?"
Otherwise confirm: "Noted 👍"
Step 7: Update State

After processing all messages:

Update last_update_id in DATA_DIR/telegram-state.md
Update Pending Confirmations (add new, remove resolved). For each apply confirmation, include:
[msg_id: X] apply/field-approval — [Role] at [Company] — waiting since DATE
job_url: https://...
form_url: https://...
field_mapping: {"First Name": "...", "Email": "...", ...}

Update Recent Actions (keep last 20, newest first)
