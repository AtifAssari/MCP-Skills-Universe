---
title: ast-grep-practice
url: https://skills.sh/mizchi/chezmoi-dotfiles/ast-grep-practice
---

# ast-grep-practice

skills/mizchi/chezmoi-dotfiles/ast-grep-practice
ast-grep-practice
Installation
$ npx skills add https://github.com/mizchi/chezmoi-dotfiles --skill ast-grep-practice
SKILL.md
ast-grep Practice
...
ワークフロー
rule-tests/ にテストファイルを書く (Red)
rules/ にルールを書く (Green)
ast-grep test --skip-snapshot-tests で確認
ast-grep test -U でスナップショット生成
スナップショットをレビューして commit
CI 統合
justfile
ast-grep-test:
  ast-grep test
...
言語別の頻出 kind カタログは references/kind-catalog.md 参照（TypeScript / Rust / Go / Python を網羅）。
...
References
本 skill 内の詳細
references/rule-yaml.md — ルール YAML 全フィールド・評価順序・メタ変数束縛スコープ・any: + fix の統合/分割・$$$ARGS 空マッチ等
references/testing.md — 分類テスト vs スナップショット、複数行コード記法、snapshot 運用
references/cli.md — サブコマンド・フラグ全般、--error / exit code / --format json
references/kind-catalog.md — 言語別 kind カタログ（TypeScript / Rust / Go / Python）
公式
