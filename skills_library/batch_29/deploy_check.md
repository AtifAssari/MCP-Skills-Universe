---
title: deploy-check
url: https://skills.sh/kazdenc/builder-skills/deploy-check
---

# deploy-check

skills/kazdenc/builder-skills/deploy-check
deploy-check
Installation
...
Identify the issue and confirm it's deploy-related.
Execute the appropriate action from the table above.
Verify the rollback resolved the issue (check error rates, run smoke tests).
Notify the team with a brief incident summary.
Smoke Tests
...
List the checks with their current status.
Flag any items that fail or need attention.
Summarize at the end: ready to deploy, or blocked with specific items to resolve.
Weekly Installs
10
Repository
