---
title: teach-oh
url: https://skills.sh/open-horizon-labs/skills/teach-oh
---

# teach-oh

skills/open-horizon-labs/skills/teach-oh
teach-oh
Installation
$ npx skills add https://github.com/open-horizon-labs/skills --skill teach-oh
SKILL.md
/teach-oh
...
Check if oh_search_context or search_symbols tools are available in the current session (RNA MCP already configured)
Check if repo-native-alignment is on PATH: which repo-native-alignment
Check if .mcp.json references repo-native-alignment or rna-server
...
Download the binary to ~/.local/bin/ (no sudo required):
...
Tell the user: "RNA is installed. Continuing with RNA CLI tools for this session — search and graph work now. MCP tools (oh_search_context, search_symbols, graph_query) will be available after you restart Claude Code."
...
Open Horizons Framework - The strategic framework for AI-assisted work (offer to include if user uses OH skills)
Project Context - Project-specific aims, constraints, and patterns (always include)
...
Fetch the hook source from GitHub and write it to the project's .omp/hooks/oh-skills-phase.ts (create the directory if needed):
...
"Install OH phase agents? Each phase gets isolated context and scoped tools — aim/problem-statement are read-only, execute gets full capabilities. Writes to .omp/agents/."
...
MCP preamble (only if OH MCP is configured — check for oh_get_endeavors in the parent session's available tools, or for .oh/mcp.json in the project): After writing all 6 files, append this block to each agent file:
...
RNA MCP preamble (only if repo-native-alignment MCP is configured — check for oh_search_context in the parent session's available tools, or for rna-server in .mcp.json): After writing all 6 files, append this block to each agent file:
...
"Install OH phase agents for Claude Code? Each phase gets its own agent with scoped tools. Writes to .claude/agents/."
