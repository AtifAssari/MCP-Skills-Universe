---
title: union-search-skill
url: https://skills.sh/runningz1/union-search-skill/union-search-skill
---

# union-search-skill

skills/runningz1/union-search-skill/union-search-skill
union-search-skill
Installation
...
使用工作流
标准搜索流程
运行前: 验证 .env 配置存在且包含有效凭据
运行: 从技能目录直接执行脚本
运行后: 检查终端输出和 responses/ 目录中的原始响应文件
通用参数
...
API 凭据获取指南 - 如何获取各平台的 API 凭据
速率限制说明 - 各平台的速率限制和配额信息
平台特定说明 - 各平台的特殊说明和注意事项
问题排查指南 - 常见问题的诊断和解决方案
Google 搜索技巧 - Google 搜索高级指令速查
常见问题速查
问题	解决方案	详细文档
...
命令行参数: --token YOUR_TOKEN 或 --api-key YOUR_KEY
环境变量: 在项目根目录的 .env 文件中配置
配置文件: 工具特定的配置文件（如 GitHub 的 ~/.github-search.json）
获取帮助
