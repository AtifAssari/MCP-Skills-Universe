---
title: autodl-elastic-deploy
url: https://skills.sh/cyicz123/autodl-elastic-deploy/autodl-elastic-deploy
---

# autodl-elastic-deploy

skills/cyicz123/autodl-elastic-deploy/autodl-elastic-deploy
autodl-elastic-deploy
Installation
...
Token 存储: token 存放在与本 SKILL.md 同级目录的 .env 文件中，格式见 .env.example
Token 初始化流程（由 agent 执行）

使用本 skill 前，先检查同级目录下是否存在 .env 文件：

若 .env 已存在，直接读取其中的 AUTODL_TOKEN 值作为后续请求的鉴权 token
若 .env 不存在，向用户提问获取 token（提示用户在 AutoDL 控制台 → 设置 → 开发者Token 中获取），然后复制 .env.example 为 .env，将 your_token_here 替换为用户输入的实际 token 值
核心概念
算力调度单元
...
使用方式
根据用户需求构造部署配置 JSON 文件（格式见下方）
执行脚本（<SKILL_DIR> 为本 SKILL.md 所在目录的绝对路径）：
...
最佳实践
镜像: 静态环境打入镜像，避免频繁更新；变更频繁的代码/模型放网络存储
启动命令: 不要后台执行（&），否则父进程退出容器即关闭
路径: 先 cd 再执行，如 cd /root && python app.py
复杂命令: 写成 shell 脚本放入镜像或文件存储中
异常调试: 将 cmd 改为 sleep infinity 保持容器运行，SSH 登录后手动排查
⚠️ 常见陷阱
...
详细参考
完整 API 参数与响应格式，见 api-reference.md
常用场景代码示例，见 examples.md
Weekly Installs
8
Repository
cyicz123/autodl…c-deploy
First Seen
Mar 7, 2026
Security Audits
