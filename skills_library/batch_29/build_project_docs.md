---
title: build-project-docs
url: https://skills.sh/zrt-ai-lab/opencode-skills/build-project-docs
---

# build-project-docs

skills/zrt-ai-lab/opencode-skills/build-project-docs
build-project-docs
Installation
$ npx skills add https://github.com/zrt-ai-lab/opencode-skills --skill build-project-docs
SKILL.md
...
先检查 .claude/docs/_progress.md——如果存在，说明上次会话中断，读取它确定：
阶段状态：哪些阶段已完成、哪个阶段进行中、哪些待执行
模块进度：阶段4/5/7中哪些模块已完成、哪些待处理
直接跳到第一个未完成的阶段继续，不重复已完成的阶段
如果 _progress.md 不存在，阅读现有 .claude/CLAUDE.md 和所有 .claude/docs/ 文件
运行阶段1获取当前状态
对比：新模块→加入，文件数变化→更新，缺失→创建，过期→更新
跳过已完成且准确的阶段
阶段7和阶段8 特殊处理：即使 _progress.md 标记为已完成，如果源码比文档更新（find -newer 检测），仍需重新运行
新项目模式（PRD驱动）
