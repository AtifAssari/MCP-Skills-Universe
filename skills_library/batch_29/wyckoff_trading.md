---
title: wyckoff-trading
url: https://skills.sh/cyrainfall/cyx_skills/wyckoff-trading
---
...
wyckoff-trading
Installation
$ npx skills add https://github.com/cyrainfall/cyx_skills --skill wyckoff-trading
SKILL.md
威科夫操盘法交易技能
...
核心能力
数据获取 - 通过 REST API 或 WebSocket 获取实时/历史行情数据
结构识别 - 识别吸筹区（Accumulation）、派发区（Distribution）、震荡区间（Trading Range）
信号判断 - 识别 Spring、JAC、UT、Shakeout 等经典威科夫信号
买卖决策 - 结合九大买入检验给出交易建议
数据获取模块
...
趋势检验 - 大盘趋势向上（利用指数数据判断）
相对强势 - 个股强于大盘（RS 线向上）
区间横盘 - 个股正在形成横盘区间（吸筹区）
因果法则 - 区间盘整时间足够长
最终震仓 - 出现了 Spring 或震仓洗盘
转强信号 - 出现了放量上涨的 SOS
最后支撑 - 出现了缩量回调的 LPS
无利空 - 市场无突发重大利空
大盘同步 - 大盘指数也处于吸筹或上涨初期
...
注意事项
数据优先 - 威科夫分析基于客观的量价数据，不要依赖主观猜测
顺势而为 - 优先选择与大盘趋势一致的股票
耐心等待 - 好的买入点需要等待，不要频繁交易
严格执行 - 设定止损后必须执行，不要临时调整
持续学习 - 威科夫方法需要大量实践才能熟练掌握
数据来源 - 本技能使用 Baostock 获取数据，支持历史K线分析；实时行情可配合其他接口使用
Weekly Installs
45
Repository
cyrainfall/cyx_skills
First Seen
Mar 3, 2026
Security Audits
