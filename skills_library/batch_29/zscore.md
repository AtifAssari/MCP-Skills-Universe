---
title: zscore
url: https://skills.sh/zerufinance/zscore/zscore
---

# zscore

skills/zerufinance/zscore/zscore
zscore
Installation
...
Collect inputs (ask, then wait).
Validate and summarise (show what you will do).
Proceed only after explicit approval (create file, then run registration).
...
Present a concise summary of what will be written (name, description length, services list, chain choice, fee).

Ask for explicit approval with a blocking question: “Approve writing agent.json with these exact details? (yes/no)” If the user does not say yes, stop.
...
Collect required inputs: name, description, at least one service with full type-specific fields, and confirm how PRIVATE_KEY will be provided.
Collect optional inputs or explicit “skip” decisions for each optional field.
Validate and summarise the exact JSON you will write.
Get explicit approval to write agent.json (yes/no).
Write agent.json.
Get explicit approval to register on-chain (yes/no).
