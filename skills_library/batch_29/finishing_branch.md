---
title: finishing-branch
url: https://skills.sh/krzysztofsurdy/code-virtuoso/finishing-branch
---

# finishing-branch

skills/krzysztofsurdy/code-virtuoso/finishing-branch
finishing-branch
Installation
...
Critical Rules
Never force-push to main, master, or any shared long-lived branch. Use --force-with-lease on feature branches only.
Never skip verification. A single untested push can break the pipeline for the entire team.
Never delete an unmerged branch without explicit confirmation. Recover with git reflog if it happens accidentally.
Never push secrets. Check the diff before every push. Rotate immediately if it happens.
Always use --force-with-lease instead of --force. It prevents overwriting someone else's work on a shared branch.
Always clean up after merge. Stale branches and orphaned worktrees accumulate and cause confusion.
Always write a meaningful PR/merge message. "Fix stuff" helps nobody -- explain what changed and why.
Rebase only your own branches. Rebasing shared branches rewrites history others depend on.
Reference Files
Reference	Contents
