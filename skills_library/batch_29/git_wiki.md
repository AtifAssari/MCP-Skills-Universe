---
title: git-wiki
url: https://skills.sh/yysun/awesome-agent-world/git-wiki
---

# git-wiki

skills/yysun/awesome-agent-world/git-wiki
git-wiki
Installation
...
Read .wiki/index.md from the working tree if it exists, even when .wiki/ is untracked — note last_commit from frontmatter
Missing .wiki/index.md or a missing last_commit means there is no prior ingest checkpoint
Run git rev-parse HEAD to get the current HEAD commit SHA
...
Update or create wiki pages under .wiki/
Mark pages for deleted source files as status: "stale" with a note in the page body, and update moved or renamed pages based on rename information from git
...
Root readme and tracked documentation under .docs/ and docs/
Package/build/workspace config (package.json, pyproject.toml, Cargo.toml, etc.)
Entry points (main.ts, index.ts, app.py, cmd/, etc.)
Core features and shared utilities → .wiki/features/
API definitions, schemas, and persistence layers → .wiki/features/ or .wiki/entities/
Notable bug fixes visible in git history → .wiki/bug-fixes/
Jobs, queues, background workers → .wiki/features/
UI structure (if present) → .wiki/features/
Tests (summarize patterns, don't document every test file)
Cross-cutting concepts, entities, and flows worth capturing
...
Read relevant wiki pages if they exist
If the wiki is missing, the relevant page is missing, or a page is marked stale, verify directly against git-tracked repository source before answering
Prefer tracked documentation in .docs/ and docs/ when it is relevant to the question, then confirm against code or config when needed
Verify against git-tracked repository source when the answer involves specific values (counts, names, configs)
Answer with a concise explanation and [[wiki-link]] citations when relevant wiki pages exist
If you created new understanding not yet in the wiki, mention it — the user can ask you to save it
Lint
