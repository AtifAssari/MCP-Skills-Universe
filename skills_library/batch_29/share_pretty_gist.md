---
title: share-pretty-gist
url: https://skills.sh/linuz90/gists.sh/share-pretty-gist
---

# share-pretty-gist

skills/linuz90/gists.sh/share-pretty-gist
share-pretty-gist
Installation
$ npx skills add https://github.com/linuz90/gists.sh --skill share-pretty-gist
SKILL.md
Share Pretty Gist
...
Edit the gist with gh gist edit {id}
POST to the refresh endpoint to bust stale cache. There is a 1-minute cooldown between purge requests:
curl -s -X POST https://gists.sh/{user}/{id}/refresh

Warm the cache with the updated content:
curl -s https://gists.sh/{user}/{id} > /dev/null
