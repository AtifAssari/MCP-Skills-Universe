...
url: https://skills.sh/mizchi/chezmoi-dotfiles/playwright-test
---

# playwright-test

skills/mizchi/chezmoi-dotfiles/playwright-test
playwright-test
Installation
$ npx skills add https://github.com/mizchi/chezmoi-dotfiles --skill playwright-test
SKILL.md
Playwright Test
設定テンプレート
...
まず waitForResponse が必須かを判定（API 呼出しが副作用の確定タイミングなら必要）
API が呼ばれないなら web-first assertion 単独 で十分（await expect(page.getByTestId('result')).toBeVisible()）
timeout を短く制限したい場合は { timeout: 5_000 } を渡す
任意レスポンスをカウントしたい場合は page.on('response', ...) で listener 化（waitFor ではなく event 集計）
API モック

page.route() は page.goto() の前に登録する:
