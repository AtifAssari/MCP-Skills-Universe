---
title: sync-fork
url: https://skills.sh/shhac/skills/sync-fork
---

# sync-fork

skills/shhac/skills/sync-fork
sync-fork
Installation
...
Check for interrupted previous run. Look for branches matching sync-fork/*. If found, a previous sync was interrupted. Show the user what backup branches exist and ask: restore from backups, or clean up (git for-each-ref --format='%(refname:short)' 'refs/heads/sync-fork/' | xargs git branch -D) and start fresh?
...
Check divergence. Run the helper script:
...
Check out the branch locally. (If no local tracking branch exists, git checkout will auto-create one.)
git reset --hard <upstream>/<branch> to align with upstream.
...
Check out the shared branch locally.
Merge each rebased fork-only branch with --no-ff in topological order (parents before children). Use the message format:
...
git push <fork> <branch> --force-with-lease to update the fork.
Phase 5: Clean Up
Delete remote branches (git push <fork> --delete <branch>) that are fully merged into upstream.
Delete all backup branches: git for-each-ref --format='%(refname:short)' 'refs/heads/sync-fork/' | xargs git branch -D
Restore the original branch saved in Phase 0: git checkout <saved-branch>.
If changes were stashed in Phase 0, restore them: git stash pop.
List any local tracking branches that can be pruned.
Show the user a final summary of what was synced, rebased, merged, deleted, and what branches remain.
Rules
