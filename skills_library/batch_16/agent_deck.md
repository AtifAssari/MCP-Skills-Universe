---
title: agent-deck
url: https://skills.sh/asheshgoplani/agent-deck/agent-deck
---

# agent-deck

skills/asheshgoplani/agent-deck/agent-deck
agent-deck
Installation
...
With --wait: polls until the agent responds, then returns the full output
Without --wait: returns immediately, check output later with agent-deck session output "Title"
Examples
...
Relevant log: tail -100 ~/.agent-deck/logs/agentdeck_<session>_*.log
...
Critical Rules
Flags before arguments: session start -m "Hello" name (not name -m "Hello")
Restart after MCP attach: Always run session restart after mcp attach
...
documentation/CONDUCTOR.md - Conductor quickstart, channel pairing, state files, multi-conductor setups
documentation/WATCHERS.md - Event-forwarding framework: doorbell model, built-in adapters, custom watchers
documentation/SKILLS.md - User-level vs pool skills, attach/detach, when to use which tier
documentation/WATCHDOG.md - Optional Python daemon that auto-restarts critical sessions
