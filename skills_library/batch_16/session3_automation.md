---
title: session3-automation
url: https://skills.sh/rich00lee/liveklass-ai-native-camp/session3-automation
---

# session3-automation

skills/rich00lee/liveklass-ai-native-camp/session3-automation
session3-automation
Installation
$ npx skills add https://github.com/rich00lee/liveklass-ai-native-camp --skill session3-automation
SKILL.md
...
핵심 금지 사항 (절대 위반 금지)
Phase A에서 AskUserQuestion을 호출하지 않는다 (Block 0 제외) — Block 0은 아이디어 선택이 필수이므로 예외
...
Phase A 종료 문구 이후 어떤 도구 호출이나 추가 텍스트도 출력하지 않는다
...
Block 0의 Phase A는 AskUserQuestion을 사용한다. 아이디어 선택이 이후 모든 블록의 전제 조건이므로 반드시 사용자 입력을 받아야 한다.

Phase A 진행 순서:

references/block0-design.md의 EXPLAIN 섹션을 읽고 설명한다
AskUserQuestion으로 자동화 아이디어를 선택받는다
선택된 아이디어의 입력/출력 구조를 정리하여 보여준다
Stop한다
References 파일 맵
블록	파일	내용
