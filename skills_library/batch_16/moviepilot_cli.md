---
title: moviepilot-cli
url: https://skills.sh/jxxghp/moviepilot/moviepilot-cli
---

# moviepilot-cli

skills/jxxghp/moviepilot/moviepilot-cli
moviepilot-cli
Installation
...
Stop — do not call get_search_results yet.
Present all filter_options fields and every value within each field to the user verbatim.
...
Add Subscription
Search for the media to get tmdb_id: Run search_media.
...
If the season exists in season_info → use that season number directly and return to the calling workflow.
If the season does not exist → the user's "season" likely maps to a later episode range within an existing TMDB season. Note the latest (highest-numbered) season from season_info, then continue to next step.
