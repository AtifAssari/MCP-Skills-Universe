---
title: pavlo-agent-skill-creator
url: https://skills.sh/pavloglushko/ai-skills/pavlo-agent-skill-creator
---

# pavlo-agent-skill-creator

skills/pavloglushko/ai-skills/pavlo-agent-skill-creator
pavlo-agent-skill-creator
Installation
...
Determine the skill name: pavlo-<topic> in kebab-case.
Create the file at .agents/skills/<skill-name>/SKILL.md.
Write the YAML front matter block first (between --- delimiters).
Add the # Skill: … title and one-sentence description.
Add a File Location & Naming section if the skill produces output files.
Write the body: structure definitions, rules, workflow, guidelines — whatever the skill needs to work without further clarification.
Keep instructions agent-actionable: avoid vague advice; write steps an agent can execute directly.
Add a row for the new skill to the skills table in .github/copilot-instructions.md — columns: Skill (display name), File (path to SKILL.md), Description (exact text from the description metadata field, collapsed to one line).
Editing an existing skill
Read the existing SKILL.md in full before making any changes.
Identify gaps: missing sections, outdated content, or conventions that changed in the codebase.
Apply surgical updates — do not rewrite sections that are still accurate.
Increment version in the metadata block (minor bump for additions, patch for corrections).
If the description field changed, update the corresponding row in the skills table in .github/copilot-instructions.md to match exactly.
Summarise what was added, modified, or removed so the user is aware of the changes.
Writing Guidelines
