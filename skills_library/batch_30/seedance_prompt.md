...
url: https://skills.sh/op7418/seedance-product-video/seedance-prompt
---

# seedance-prompt

skills/op7418/seedance-product-video/seedance-prompt
seedance-prompt
Installation
$ npx skills add https://github.com/op7418/seedance-product-video --skill seedance-prompt
SKILL.md
...
产品信息：你的产品叫什么名字？请简要描述产品定位和核心功能。
具体需求：你希望视频重点展示什么？（如某个功能、品牌调性、技术特点）
图片素材：是否有图片素材可以提供？（产品图、Logo 图、界面截图等）
如果有，请用户直接发送图片或提供图片路径
口播需求：是否需要口播（Voiceover）？如果需要，请说明语言和大致内容方向
Step 2: 分析素材
读取用户提供的产品描述，提取核心特征
...
读取 references/prompt-system.md 获取完整的提示词系统规则，包括：
...
告知用户在 Seedance 2.0 中选择 "全能参考" 模式
明确标注每张图片的占位符和用途，例如：
[图片一] → Logo 图，用于 9-12s 的 Logo 形态参考
[图片二] → 产品界面截图，用于 2-5s 的界面形态参考
说明用户需要在 Seedance 界面中将对应图片拖入对应位置
Step 5: 输出与检查
输出纯英文提示词
...
检测安装状态：运行 which dreamina 或 dreamina --version 检查是否已安装
根据结果引导：
