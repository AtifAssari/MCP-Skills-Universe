---
title: db-load-git
url: https://skills.sh/arman-kudaibergenov/1c-ai-development-kit/db-load-git
---

# db-load-git

skills/arman-kudaibergenov/1c-ai-development-kit/db-load-git
db-load-git
Installation
...
Если ветка не совпала — используй default Если v8path не задан — автоопределение: Get-ChildItem "C:\Program Files\1cv8\*\bin\1cv8.exe" | Sort -Desc | Select -First 1 Если файла нет — предложи /db-list add. Если использованная база не зарегистрирована — после выполнения предложи добавить через /db-list add. Если в записи базы указан configSrc — используй как каталог конфигурации.
Команда
...
XML объекта верхнего уровня (напр. Catalogs/Номенклатура.xml)
Все файлы из каталога Ext/ этого объекта (связанные модули)
...
Предложить /db-update — для применения изменений к БД
Примеры
