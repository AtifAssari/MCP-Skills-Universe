---
title: brainstorming
url: https://skills.sh/jsonlee12138/agent-team/brainstorming
---

# brainstorming

skills/jsonlee12138/agent-team/brainstorming
brainstorming
Installation
...
Standard flow
Explore project context: inspect only the files, docs, and recent changes needed for the topic.
Ask clarifying questions: prefer one focused question at a time when ambiguity remains.
Propose 2-3 approaches when trade-offs matter; include a recommendation.
Present the design in concise sections covering only the dimensions that matter for the request.
When the topic is planning-oriented, explicitly ask which target object the brainstorming is for: roadmap, milestone, phase, task, or generic topic.
When relevant, ask whether the brainstorming should build on an existing object or reference set.
If the user wants a saved artifact, ask for save location choice before writing.
Stop after delivering the brainstorming output unless the user explicitly asks to continue.
Lightweight flow

Use this lighter path for narrow requirement shaping:

Confirm the goal and key constraint.
Present the recommended approach first.
Mention alternatives only if they materially change scope, risk, or cost.
Deliver the result inline unless the user asks to save it.
Process Flow
...
Standard choices
Default directory: docs/brainstorming/
Custom directory: user-provided path
...
The user has chosen one of the allowed save destinations for the doc (docs/brainstorming/, target object directory when applicable, custom directory, or skip saving).
The brainstorming doc has been written to file (default/target-object/custom) or output inline (skip saving), and shared.
Weekly Installs
49
Repository
