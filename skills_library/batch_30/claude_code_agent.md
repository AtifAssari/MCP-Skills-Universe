...
url: https://skills.sh/ben2pc/g-claude-code-plugins/claude-code-agent
---

# claude-code-agent

skills/ben2pc/g-claude-code-plugins/claude-code-agent
claude-code-agent
Installation
$ npx skills add https://github.com/ben2pc/g-claude-code-plugins --skill claude-code-agent
SKILL.md
Claude Code Agent
...
确保已完成认证：claude auth login
可用 claude auth status --text 快速确认当前登录状态
建议先切到目标项目目录再运行；自动化调用默认用 claude -p
调用方式
新建会话（默认）
...
加 --no-session-persistence 后，本次会话不会写入磁盘，因此事后无法被 --resume 或 --continue 恢复。仅在以下场景使用：
...
多轮对话
首次 claude -p ... --output-format json 获取 session_id
后续追问用 claude -p ... --resume "session_id"，或在同目录用 --continue
自动跟踪 session_id，用户无需手动理解完整协议
不同任务各自新建会话；多个 session_id 互不干扰
模型与 effort 选择
...
使用规则
自动化调用默认用 -p + --output-format json：拿稳定的单对象结果；只有确实需要事件流时再切 stream-json
stream-json 一律配 --verbose：这是 CLI 的硬约束，不是建议
始终显式传 --model 和 --effort：避免默认值漂移
始终在目标项目目录运行：先 cd /path/to/project，不要让 Claude 在错误仓库执行
编码任务用 bypassPermissions，审查任务用 plan + 工具白名单：plan 不是硬沙箱，仍要配 --allowedTools
保持对话连续：只要后续可能追问，就不要加 --no-session-persistence
--continue 适合“继续刚才那个任务”；需要可追踪脚本行为、跨目录稳定性或明确会话绑定时，用 --resume
需要隔离修改时优先 --worktree：比手工切目录更稳，也更适合子 agent 独立执行
向用户报告结果时优先取 result 字段：必要时再补 session_id、成本和 token 使用
如果只允许读，不要只依赖提示词：同时加 plan、工具白名单或 --tools 限制，避免子进程意外写文件
需要更可控的自动化边界时，加 --max-turns：避免子会话在模糊任务里跑得过深
scripted / SDK 调用默认优先 --bare：这样不会继承本地 CLAUDE.md、skills、plugins、MCP 和 auto memory，结果也更可复现
-p 模式里不要依赖 /commit 之类交互式 slash commands：直接用自然语言描述任务；必要时显式给 --append-system-prompt、--allowedTools 或 --json-schema
Prompt References
...
委派 handoff / 独立 worker / 续接同一 session：读 references/delegation-prompt-recipes.md
