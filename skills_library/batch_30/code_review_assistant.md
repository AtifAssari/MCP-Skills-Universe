...
url: https://skills.sh/iceleaf916/my-cc-plugins/code-review-assistant
---

# code-review-assistant

skills/iceleaf916/my-cc-plugins/code-review-assistant
code-review-assistant
Installation
...
识别审核模式：是本地 Git 还是 Gerrit Change ID
验证范围有效性：
...
获取文件的差异内容（diff）
识别文件类型（.go、.cpp、.hpp、.yml、.yaml 等）
应用排除规则（vendor、generated 等目录）
将文件分发给对应语言的分析器
阶段 3: 语言专项检查
Golang 分析器

检查规则详见 references/golang-checks.md。
...
检查规则详见 references/cpp-checks.md
...
检查规则详见 references/ansible-checks.md
...
阶段 4: 问题聚合与分级
将各语言检查结果汇总
按严重程度分类（Must Fix / Should Fix / Nice to Have）
去重和优先级排序
计算 Review 评分：
...
优先定位：第一个 Must Fix 问题的文件路径和行号
备选方案：如果无 Must Fix，使用第一个 Should Fix 问题
兜底方案：如果无问题（Score +1），使用第一个修改文件的第一行
...
Markdown 审核报告（始终生成）：在本地终端完整展示，适用于本地代码审核
Gerrit Review 评论（可选，仅在 Gerrit 模式且用户明确要求时发布）：发布到 Gerrit Change
Markdown 审核报告（本地输出）
...
问题描述: 自定义的字符串分割函数可被 strings.Split 替代。
...
Found Issues: 🔴 Must Fix: 3 items 🟡 Should Fix: 1 items 🔵 Nice to Have: 2 items Recommended Score: -2 (需修复严重问题后合并)

Positive Points: • 添加了完整的测试用例覆盖新增功能 • 使用接口抽象依赖，便于测试和维护 • 密码修改流程有完整的降级机制 • 日志记录详尽，便于排查问题

Must Fix Issues

[app/api/service/eia.go:239] 日志中使用未定义变量 departmentIDs Suggestion: 修复日志语句，移除或使用正确的变量。

[app/api/service/uim_client.go:57] 全局变量缺少并发安全保护
...
[app/web/handler/user.go:1227] 用户 ID 检查逻辑不正确 Suggestion: 修复 ID 验证逻辑，添加边界检查。

Should Fix Issues
[pkg/middleware/auth.go:45] 函数命名不符合 Go 规范 Suggestion: 使用驼峰命名 getAuthToken → GetAuthToken。
Nice to Have Issues

[main.go:120] 可使用标准库 strings.Split 替代自定义实现

[utils/string.go:78] 删除未使用的辅助函数
