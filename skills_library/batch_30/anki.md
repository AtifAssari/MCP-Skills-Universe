---
title: anki
url: https://skills.sh/ishiko732/anki-skills/anki
---

# anki

skills/ishiko732/anki-skills/anki
anki
Installation
...
Read deck and model from .claude/anki.json config (create with defaults {"deck": "program::claude", "model": "Basic"} if missing)
Call modelFieldNames to verify the configured model's fields
Proceed directly with addNotes — no user confirmation needed
...
Split the rewritten content into sections, each focusing on one main point
Generate multiple flashcards from each section; for sections with more than 10 words, split and summarize before creating cards
Quality Rules
...
File Integration
PDF → Flashcards
Use the pdf skill to extract text from the PDF
Apply the flashcard generation guide above to create Q&A pairs
Present the table to the user for review
Confirm: target deck, note type (model), and tags
Use addNotes to batch-add
CSV / Excel → Flashcards
Use the xlsx skill to read and parse the file
Inspect the columns — if data is already in Q/A format, map directly to Front/Back fields
If not Q/A format, apply the flashcard generation guide to the content
Present the mapping/cards to the user for review
Confirm: target deck, note type (model), and tags
Use addNotes to batch-add
...
Apply flashcard generation guide — rewrite, split, generate Q&A pairs
Present table for review
Follow Pre-Add Checklist: a. Fetch models: bash scripts/anki-connect.sh modelNames b. Ask user to choose model via AskUserQuestion c. Verify fields: bash scripts/anki-connect.sh modelFieldNames '{"modelName":"<user-chosen-model>"}' d. Show field mapping to user and confirm e. Confirm deck, model, and tags — present final summary
Only after user confirms all of the above:
bash scripts/anki-connect.sh addNotes '{
  "notes": [
...
Follow "Flashcard Generation Guide" on extracted text
Present cards for review
Follow Pre-Add Checklist (fetch models → user picks → verify fields → confirm all)
Batch-add with addNotes
4. Import Flashcards from CSV/Excel
Use xlsx skill to parse the file
Map columns to Front/Back or generate cards
Present mapping for review
Follow Pre-Add Checklist (fetch models → user picks → verify fields → confirm all)
Batch-add with addNotes
