---
title: autostrategy
url: https://skills.sh/zhangchao0911/autostrategy/autostrategy
---

# autostrategy

skills/zhangchao0911/autostrategy/autostrategy
autostrategy
Installation
...
不得在无 STRATEGY_DESIGN.md 的情况下直接进入 Phase 3
...
将完整 Markdown 写入 STRATEGY_DESIGN.md
必须真实落盘到策略目录
...
未收到用户明确的「确认 / OK / 继续生成代码 / 进入 Phase 2」之前，不得开始生成 strategy.py、config.yaml、README.md、requirements.txt 或任何 Phase 2 文件
...
✅ 策略目录已创建，路径: ___________
✅ STRATEGY_DESIGN.md 已写入磁盘（通过 ls/cat 验证，非假设）
✅ quality_check.py 已运行并通过（退出码 0），或脚本不存在时已跳过并确认文件存在
✅ 设计摘要已展示给用户
✅ 用户已明确回复「确认 / OK / 继续」
...
STRATEGY_DESIGN.md 文件存在于策略目录中（通过 ls 或 cat 验证文件存在且非空，不可假设）
数据源已就绪 如果 Phase 1 选择了跳过安装，此时必须先运行： python3 scripts/env_setup.py --install [市场类型] 安装完成并验证数据可用后，才可继续。
...
ftshare-all-in-one	A股全品类数据获取（非凸科技 FTShare 免费数据）	A股策略（Phase 1 首选推荐）安装: npx skills add zhangchao0911/all-in-one --yes
