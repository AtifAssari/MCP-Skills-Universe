---
title: erc8004-agent-creator
url: https://skills.sh/justinedevs/collection/erc8004-agent-creator
---

# erc8004-agent-creator

skills/justinedevs/collection/erc8004-agent-creator
erc8004-agent-creator
Installation
...
For full wizard order and chain/feature matrix, see references/wizard-options.md.
...
Configure – cd <projectDir>, edit .env: PRIVATE_KEY (or use generated), OPENAI_API_KEY, PINATA_JWT (pinata.cloud, pinJSONToIPFS scope). Back up .env if wallet was generated.
Fund wallet – Testnet ETH (Sepolia, Base Sepolia, etc.) or Solana Devnet SOL.
Register on-chain – npm run register (uploads metadata to IPFS, mints Identity Registry NFT). View on 8004scan.io.
Start servers – If A2A: npm run start:a2a (then set public URL in registration and run npm run register again if needed). If MCP: npm run start:mcp.
Update agent – Edit src/register.ts for name/description/image/skills, then run npm run register again.
