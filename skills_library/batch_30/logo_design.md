---
title: logo design
url: https://skills.sh/seb1n/awesome-ai-agent-skills/logo-design
---

# logo design

skills/seb1n/awesome-ai-agent-skills/Logo Design
Logo Design
Installation
$ npx skills add https://github.com/seb1n/awesome-ai-agent-skills --skill 'Logo Design'
SKILL.md
Logo Design
...
Document Brand Guidelines: Deliver a concise style reference listing the exact hex/RGB/HSL color values, the typeface names and weights, the logo safe zone (clear space equal to the height of a key letter), and explicit misuse examples (stretching, recoloring outside the palette, placing on busy backgrounds).
...
Concept A — Orbital Node A hexagonal node with three orbital paths forming an abstract "N". Conveys distributed systems and connectivity.
...
Concept B — Byte Stack Three stacked parallelograms forming an upward arrow, representing layered infrastructure and growth.
...
Minimum size: Icon mark must not be rendered below 24px. Wordmark must not appear below 80px wide. Clear space: Maintain padding equal to the height of the "o" in "Nova" on all sides.
...
Primary combination mark: Full circle-leaf icon + "GreenLeaf Organics" wordmark to the right. Use on website headers, business cards, and letterheads.
Stacked combination mark: Icon centered above the wordmark. Use on social media profiles, signage, and packaging labels where vertical space is available.
Icon-only mark: Circle-leaf symbol without text. Use at sizes below 48px, as app icons, and for watermarks. Ensure the icon is recognizable without the wordmark at 32x32.
Wordmark only: "GreenLeaf Organics" in the brand typeface without the icon. Use in email signatures, footer text, and inline document references.
Favicon: Simplified leaf silhouette (no circle) optimized for 16x16 and 32x32. Remove fine details that disappear at small sizes.
Monochrome versions: Solid black and solid white versions of all marks above for single-color printing, embossing, and engraving.
