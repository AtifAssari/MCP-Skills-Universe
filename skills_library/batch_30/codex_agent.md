---
title: codex-agent
url: https://skills.sh/ben2pc/g-claude-code-plugins/codex-agent
---

# codex-agent

skills/ben2pc/g-claude-code-plugins/codex-agent
codex-agent
Installation
$ npx skills add https://github.com/ben2pc/g-claude-code-plugins --skill codex-agent
SKILL.md
Codex Agent
...
确保已完成 Codex 登录认证（codex 首次运行会引导登录）
建议在目标项目目录下运行，或显式传 -C /path/to/project
调用方式
新建会话（默认）
...
加 --ephemeral 后，本次会话不会写入磁盘（不持久化到 ~/.codex/sessions/），因此事后无法被 codex exec resume 或 --last 恢复。仅在以下场景使用：
...
-o, --output-last-message FILE	最后消息写入文件
多轮对话
首次 codex exec --json ... 获取 thread_id
后续追问用 codex exec resume --json "thread_id" "prompt"
自动跟踪 thread_id，用户无需关心
不同任务各自新建会话，多个 thread_id 互不干扰
模型选择
...
使用规则
自动化调用一律加 --json：确保输出可解析，提取 thread_id 和回复内容
始终显式传 --model：避免默认模型漂移
始终在目标项目目录运行：优先 cd /path/to/project 或 -C /path/to/project
编码任务用 workspace-write：通常直接用 --sandbox workspace-write 或 --full-auto
审查任务用 read-only 或 codex exec review：防止意外修改
保持对话连续：同一任务的追问复用 thread_id；只要可能追问就别加 --ephemeral
需要稳定下游解析时，用 --output-schema + -o：把最终结果约束成机器可消费的结构
长 prompt 用 stdin 管道：prompt 超过约 500 字符或包含多行/特殊字符时，不要继续用位置参数；先写入文件，再用 - < file.txt 传入，避免卡在 Reading additional input from stdin...
向用户报告结果：每次调用后，从 JSONL 中提取最终回复，简要总结给用户
区分 -o 和 --json 的职责：-o 负责把最后一条回复落文件；--json 负责把整段事件流打印到 stdout。脚本常见组合是两者一起用。
非编码的本机浏览器任务优先 read-only：如果只是让 Codex 用 Computer Use 打开 Chrome、浏览网页、总结内容，不需要 --full-auto；提示词里再补一句 Do not modify local files. 作为双保险。
Prompt References
