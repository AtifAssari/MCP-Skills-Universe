---
title: vuln-research
url: https://skills.sh/lu1sdv/skillsmd/vuln-research
---

# vuln-research

skills/lu1sdv/skillsmd/vuln-research
vuln-research
Installation
$ npx skills add https://github.com/lu1sdv/skillsmd --skill vuln-research
SKILL.md
...
Enumerate all source files — exclude vendored/generated code (node_modules/, vendor/, dist/, generated protobuf)
...
Enumerate languages: scan file extensions, shebangs, package.json/composer.json/pom.xml/go.mod/Cargo.toml/mix.exs/Gemfile/requirements.txt
Identify the stack layers: e.g., PHP backend + Node.js build tooling + Python microservice + Java auth service
Load matching sink files: for each language present, load the corresponding references/sinks/<lang>.md — load multiple if the target is polyglot
Load the SAST/DAST router: references/sinks-catalog.md for cross-language Semgrep/CodeQL/SonarQube rules
...
Run SAST, sort by severity descending
For P0/P1: manually verify each — trace source to sink, confirm controllability
For P2/P3: sample 10 findings, measure FP rate, extrapolate to decide effort allocation
Suppress confirmed false positives with inline comments or tool-specific ignore rules
Track FP rate per rule — disable rules consistently above 90% FP in your stack
...
Can I control the input? — Is user input actually controllable, or is it server-generated/hardcoded?
Does it reach the sink? — Does the tainted data survive all transforms, sanitizers, and WAF rules?
Can I prove impact? — Do I have a working payload that demonstrates real-world consequences?
Where is the defense layer? — Is the mitigation in the vulnerable code itself, in a framework layer (e.g. Django auto-escaping, Rails CSRF tokens), in runtime config (e.g. disable_functions, WAF rules), or in the deployment environment (e.g. network segmentation, read-only filesystem)? Framework and runtime defenses can be bypassed or misconfigured — verify the defense is actually active, not just assumed.
...
Identify the defense boundary — sandbox, hardened allocator, kernel separation, WAF, hypervisor
Treat the defense itself as a new attack surface — it's software too, with its own bugs
Apply the same methodology recursively — sweep or audit the defense layer's code for bypasses
Chain across boundaries — vuln in app + sandbox escape + kernel bug = full-chain exploit
...
Confidence Score (1-10) — exploitability certainty
Exploitability Likelihood (High/Medium/Low) — how likely is real-world exploitation given attack complexity, auth requirements, and conditions
Auth Level — Unauthenticated / Authenticated (specify minimum role) / Multi-level (specify each)
Intent Verification Gate — double-check this is NOT an intended feature (admin RCE via plugin upload = intended; editor accessing admin plugin upload = real bug). See full gate in audit-poc-report.md
Intent Classification — Actual bug / Intended feature / Design weakness
Justification — who has access? Is this expected for that role? Would fixing it break legitimate workflows?
Exact payload — copy-pasteable, not screenshots
Server response — proving impact
Video recording — full exploit chain end-to-end
