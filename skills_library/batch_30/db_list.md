---
title: db-list
url: https://skills.sh/arman-kudaibergenov/1c-ai-development-kit/db-list
---

# db-list

skills/arman-kudaibergenov/1c-ai-development-kit/db-list
db-list
Installation
...
По aliases (совпадение в массиве с учётом морфологии: «тестовую» = «тестовая» = «тестовой»)
По name (нечёткое совпадение с учётом морфологии и регистра)
Если пользователь не указал базу — сопоставь текущую ветку Git с databases[].branches:
Точное совпадение: ветка dev → "branches": ["dev"]
Glob-паттерн: ветка release/2.1 → "branches": ["release/*"]
Если ветка не совпала — используй default
Если не найдено или неоднозначно — спроси пользователя
Если файл .v8-project.json не найден — спроси параметры подключения и предложи создать файл
