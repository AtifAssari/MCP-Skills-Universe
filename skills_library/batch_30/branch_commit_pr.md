---
title: branch-commit-pr
url: https://skills.sh/nianyi778/branch-commit-pr/branch-commit-pr
---

# branch-commit-pr

skills/nianyi778/branch-commit-pr/branch-commit-pr
branch-commit-pr
Installation
$ npx skills add https://github.com/nianyi778/branch-commit-pr --skill branch-commit-pr
SKILL.md
Branch → Commit → PR
...
Validate that branch X exists (git branch -r or git branch)
Run git config --local bcp.default-branch X
Confirm: "Default branch set to X for this repo. All future branches will be created from X."
...
Copy SKILL.md into the agent's instruction/rules directory
Or paste content into the agent's system prompt configuration
Ensure the agent has git and gh CLI access in its shell
Cross-Model Tips
...
NEVER create a branch without fetching latest default branch first
NEVER force-push without explicit user request
NEVER guess issue numbers — only link what user explicitly mentions
NEVER transition Jira tickets without user confirmation
NEVER commit with empty message or generic "update" message
NEVER create PR from default branch
NEVER push to default branch directly
NEVER delete branches without asking
NEVER make a single commit from 5+ unrelated files — split atomically
NEVER skip the context gathering phase — conventions matter
Weekly Installs
9
Repository
nianyi778/branc…ommit-pr
GitHub Stars
1
First Seen
Mar 2, 2026
